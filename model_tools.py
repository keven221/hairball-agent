...
def _run_async(coro):
    ...
    if loop and loop.is_running():
        ...
        # async tools resolve get_hairball_home() under the active profile.
        ...
        try:
            ...
            if loop_ready.wait(timeout=1.0) and worker_loop is not None:
                ...
                # The worker never started and cancellation proves it can no
                # longer consume ``coro``.  Release the coroutine explicitly;
                # otherwise Python reports "was never awaited" and retains
                # its frame until GC.  Never close after cancel() returns
                # False: the worker then owns (or is about to own) it.
...
#   - cli.py, hairball_cli/*      -> inline on startup (no event loop)
...
def get_tool_definitions(
    ...
    visible_toolsets: Optional[List[str]] = None,
) -> List[Dict[str, Any]]:
    """
    ...
    Args:
        ...
        visible_toolsets: Keep tools from these toolsets directly visible and
            defer other authorized tools behind Tool Search. This affects
            model-facing schema visibility only; it never grants a tool.
    ...
    if quiet_mode:
        ...
        cache_key = (
            ...
            frozenset(visible_toolsets) if visible_toolsets is not None else None,
            registry._generation,
            cfg_fp,
            bool(os.environ.get("HAIRBALL_KANBAN_TASK")),
        ...
        if cached is not None:
            ...
            # Return a fully independent schema tree.  Provider recovery paths
            # intentionally sanitize ``agent.tools`` in place, so sharing
            # nested dictionaries here would let one session poison this
            # process-wide cache and every later agent snapshot.
    ...
    if quiet_mode:
        ...
        # both list edits (memory/LCM appends) and nested schema sanitization
        # remain session-local. Without this, a long-lived Gateway process can
        # accumulate duplicate tools or leak provider-specific schema stripping
        # into later agents. Mirrors the cache-hit path above. (issue #17335)
...
def _compute_tool_definitions(
    ...
    if enabled_toolsets is not None:
        ...
        if os.environ.get("HAIRBALL_KANBAN_TASK") and "kanban" not in effective_enabled_toolsets:
            # Dispatcher-spawned workers are scoped by HAIRBALL_KANBAN_TASK and
    ...
    # This ensures that even if a composite toolset (like hairball-cli)
    ...
    if disabled_toolsets:
        for toolset_name in disabled_toolsets:
            if validate_toolset(toolset_name):
                ...
                if toolset_name.startswith("hairball-") or (get_toolset(toolset_name) or {}).get("posture"):
                    # Platform bundles (hairball-*) include _HAIRBALL_CORE_TOOLS, and
                    ...
                    if (not quiet_mode and toolset_name.startswith("hairball-")
    ...
    # window). Core Hairball tools (toolsets._HAIRBALL_CORE_TOOLS) are never
    # deferred by the general threshold policy. A session-static visibility
    # scope may explicitly place authorized core tools behind the bridge.
    #
    ...
    try:
        ...
        if not skip_tool_search_assembly and ts_cfg.enabled != "off":
            ...
            forced_deferred_names = _forced_deferred_tool_names(
                filtered_tools,
                visible_toolsets,
            )
            assembly = assemble_tool_defs(
                ...
                config=ts_cfg,
                forced_deferred_names=forced_deferred_names,
            )
...
def _forced_deferred_tool_names(
    tool_defs: List[Dict[str, Any]],
    visible_toolsets: Optional[List[str]],
) -> frozenset[str]:
    """Return authorized tool names omitted from the session's initial schema.

    ``visible_toolsets`` is a visibility filter only. The input ``tool_defs``
    has already passed the enabled/disabled entitlement filters and each
    registry ``check_fn``, so this function cannot widen authorization.
    """
    if visible_toolsets is None:
        return frozenset()

    visible_names: set[str] = set()
    for toolset_name in visible_toolsets:
        if validate_toolset(toolset_name):
            visible_names.update(resolve_toolset(toolset_name))
        elif toolset_name in _LEGACY_TOOLSET_MAP:
            visible_names.update(_LEGACY_TOOLSET_MAP[toolset_name])

    authorized_names = {
        (tool.get("function") or {}).get("name", "")
        for tool in tool_defs
    }
    authorized_names.discard("")
    return frozenset(authorized_names - visible_names)
...
def _resolve_active_context_length() -> int:
    ...
    """
    try:
        from hairball_cli.config import load_config as _load
        cfg = _load() or {}
...
def is_agent_loop_tool(function_name: str) -> bool:
    """Whether a tool requires live AIAgent state instead of registry dispatch."""
    return str(function_name or "") in _AGENT_LOOP_TOOLS


class SessionToolRouter:
    """Session-scoped execution registry plus its model-visible projection.

    The process registry remains the canonical handler store, while this view
    fixes which registered names a session may execute.  Model visibility is a
    projection of that scope: deferred tools can be authorized without being
    present in the initial request schema.
    """
    ...
    def __init__(
        ...
        *,
        ...
        self.authorized_tool_names = frozenset(
            str(name) for name in authorized_tool_names if str(name)
        )
        self.model_visible_specs = tuple(model_visible_specs or ())
        self.model_visible_tool_names = frozenset(
        ...
        self.registry_generation = (
            ...
            else int(registry_generation)
        )

    @classmethod
    def from_names(cls, names) -> "SessionToolRouter":
        """Build a scoped router for adapters that already own their schemas."""
        return cls(authorized_tool_names=names)

    def authorization_error(self, function_name: str) -> Optional[str]:
        name = str(function_name or "")
        if name in self.authorized_tool_names:
            return None
        return json.dumps(
            {
    ...
    def filter_authorized_definitions(self, definitions):
        """Keep only definitions registered in this session snapshot."""
        return [
            definition
            for definition in definitions or ()
            if isinstance(definition, dict)
            and str(definition.get("function", {}).get("name") or "")
            in self.authorized_tool_names
        ]

    def dispatch(self, function_name: str, function_args: dict, **kwargs) -> str:
        """Dispatch only when the name belongs to this session registry."""
        error = self.authorization_error(function_name)
        if error is not None:
            return error
        return registry.dispatch(function_name, function_args, **kwargs)


def build_session_tool_router(
    *,
    model_visible_specs,
    enabled_toolsets: Optional[List[str]] = None,
    disabled_toolsets: Optional[List[str]] = None,
    ...
    """Build one execution scope and its visibility projection from one config."""
    authorized_defs = get_tool_definitions(
    ...
    authorized_names = {
        ...
        for definition in authorized_defs
        if isinstance(definition, dict)
        and definition.get("function", {}).get("name")
    }
    # Agent-owned state tools and bridge tools may be appended outside the
    # process registry. They are authorized only when present in this exact
    # session's visible snapshot.
    authorized_names.update(
        str(definition.get("function", {}).get("name") or "")
        for definition in model_visible_specs or ()
        if isinstance(definition, dict)
        and definition.get("function", {}).get("name")
    )
    return SessionToolRouter(
        authorized_tool_names=authorized_names,
        model_visible_specs=model_visible_specs,
        registry_generation=registry._generation,
    )


def session_tool_router_for_agent(agent) -> SessionToolRouter:
    """Return the initialized router, with a deny-by-default legacy fallback."""
    router = getattr(agent, "_session_tool_router", None)
    if isinstance(router, SessionToolRouter):
        return router
    return SessionToolRouter(
        authorized_tool_names=getattr(agent, "valid_tool_names", ()) or (),
        model_visible_specs=getattr(agent, "tools", ()) or (),
    )


def _session_tool_scope_error(
    function_name: str,
    session_tool_router: Optional[SessionToolRouter],
) -> Optional[str]:
    """Return a stable JSON error when a session calls an unregistered tool.

    ``None`` is retained only for direct trusted-host compatibility. Every
    model execution path supplies a concrete session router, including an
    empty (deny-all) router when the session has no tools.
    """
    if session_tool_router is None:
        return None
    return session_tool_router.authorization_error(function_name)
...
def _normalize_json_strings_for_schema(value: Any, schema: Any) -> Any:
    ...
    Ported from cline/cline#11803, adapted to hairball-agent's coercion layer.
...
def handle_function_call(
    ...
    disabled_toolsets: Optional[List[str]] = None,
    visible_toolsets: Optional[List[str]] = None,
    session_tool_router: Optional[SessionToolRouter] = None,
) -> str:
    """
    ...
    Args:
        ...
        enabled_tools: Tool names exposed inside ``execute_code``. The model's
                       execution authority comes from ``session_tool_router``;
                       this legacy argument remains the nested-code catalog.
        ...
        visible_toolsets: The session's initial-schema visibility toolsets.
                       Authorized tools outside this set are reachable only
                       through the Tool Search bridge.
        session_tool_router: The exact execution registry snapshot paired with
                       this model request. Model paths must provide it; trusted
                       host adapters may omit it only when they enforce their
                       own explicit registration/allowlist boundary.
    ...
    scope_error = _session_tool_scope_error(function_name, session_tool_router)
    ...
    if _ts_mod is not None and _ts_mod.is_bridge_tool(function_name):
        try:
            ...
            current_defs = []
        if session_tool_router is not None:
            current_defs = session_tool_router.filter_authorized_definitions(
                current_defs
            )
        _forced_deferred = _forced_deferred_tool_names(
            current_defs,
            visible_toolsets,
        )
        _scoped_deferrable = _ts_mod.scoped_deferrable_names(
            current_defs,
            forced_deferred_names=_forced_deferred,
        )
        ...
        if function_name == _ts_mod.TOOL_CALL_NAME:
            underlying_name, underlying_args, err = _ts_mod.resolve_underlying_call(
                function_args or {},
                allowed_deferred_names=_scoped_deferrable,
            )
            ...
            # Recurse with the same session router. Deferred visibility never
            # grants execution authority: the underlying runtime must already
            # exist in the router built for this request.
    ...
    try:
        if is_agent_loop_tool(function_name):
        ...
        try:
            if function_name == "execute_code":
                ...
                def _dispatch(next_args: Dict[str, Any]) -> Any:
                    dispatcher = session_tool_router or registry
