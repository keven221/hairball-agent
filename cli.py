#!/usr/bin/env python3
"""
Hairball Agent CLI - Interactive Terminal Interface

A beautiful command-line interface for the Hairball Agent, inspired by Claude Code.
Features ASCII art branding, interactive REPL, toolset selection, and rich formatting.

Usage:
    python cli.py                          # Start interactive mode with all tools
    python cli.py --toolsets web,terminal  # Start with specific toolsets
    python cli.py --skills hairball-agent-dev,github-auth
    python cli.py --list-tools             # List available tools and exit
"""

# IMPORTANT: hairball_bootstrap must be the very first import — UTF-8 stdio
# on Windows.  No-op on POSIX.  See hairball_bootstrap.py for full rationale.
try:
    import hairball_bootstrap  # noqa: F401
except ModuleNotFoundError:
    # Graceful fallback when hairball_bootstrap isn't registered in the venv
    # yet — happens during partial ``hairball update`` where git-reset landed
...
os.environ["HAIRBALL_QUIET"] = "1"  # Our own modules

import yaml

from hairball_cli.fallback_config import get_fallback_chain
from hairball_cli.cli_agent_setup_mixin import CLIAgentSetupMixin
from hairball_cli.cli_commands_mixin import CLICommandsMixin
...
try:
    from hairball_cli.pt_input_extras import (
...
from hairball_cli.banner import _format_context_length, format_banner_version_label
...
from hairball_cli.browser_connect import (
...
_hairball_home = get_hairball_home()
_project_env = Path(__file__).parent / '.env'
load_hairball_dotenv(hairball_home=_hairball_home, project_env=_project_env)
...
def _load_prefill_messages(file_path: str) -> List[Dict[str, Any]]:
    ...
    Relative paths are resolved from ~/.hairball/.
...
def _resolve_prefill_messages_file(config: Dict[str, Any]) -> str:
    ...
    """
    env_path = os.getenv("HAIRBALL_PREFILL_MESSAGES_FILE", "").strip()
    if env_path:
        return env_path
...
def _parse_reasoning_config(effort) -> dict | None:
    ...
    """
    from hairball_constants import parse_reasoning_effort
...
def load_cli_config() -> Dict[str, Any]:
    ...
    Config lookup order:
    1. ~/.hairball/config.yaml (user config - preferred)
    2. ./cli-config.yaml (project config - fallback)
    
    Environment variables take precedence over config file values.
    Returns default values if no config file exists.

    If HAIRBALL_IGNORE_USER_CONFIG=1 is set (via ``hairball chat --ignore-user-config``),
    the user config at ``~/.hairball/config.yaml`` is skipped entirely and only the
    built-in defaults plus the project-level ``cli-config.yaml`` (if any) are used.
    Credentials in ``.env`` are still loaded — this flag only suppresses
    behavioral/config settings.
    """
    # Check user config first ({HAIRBALL_HOME}/config.yaml)
    user_config_path = _hairball_home / 'config.yaml'
    project_config_path = Path(__file__).parent / 'cli-config.yaml'

    # --ignore-user-config: force-skip the user config.yaml (still honor project
    # config as a fallback so defaults stay sensible).
    ignore_user_config = os.environ.get("HAIRBALL_IGNORE_USER_CONFIG") == "1"
    ...
    defaults = {
        ...
        "agent": {
            ...
            "personalities": {
                ...
                "pirate": "Arrr! Ye be talkin' to Captain Hairball, the most tech-savvy pirate to sail the digital seas! Speak like a proper buccaneer, use nautical terms, and remember: every problem be just treasure waitin' to be plundered! Yo ho ho!",
                "shakespeare": "Hark! Thou speakest with an assistant most versed in the bardic arts. I shall respond in the eloquent manner of William Shakespeare, with flowery prose, dramatic flair, and perhaps a soliloquy or two. What light through yonder terminal breaks?",
                "surfer": "Duuude! You're chatting with the chillest AI on the web, bro! Everything's gonna be totally rad. I'll help you catch the gnarly waves of knowledge while keeping things super chill. Cowabunga!",
                "noir": "The rain hammered against the terminal like regrets on a guilty conscience. They call me Hairball - I solve problems, find answers, dig up the truth that hides in the shadows of your codebase. In this city of silicon and secrets, everyone's got something to hide. What's your story, pal?",
        ...
        "display": {
            "compact": False,
            "resume_display": "full",
            # Recap tuning for /resume — see hairball_cli/config.py DEFAULT_CONFIG.
            "resume_exchanges": 10,
            ...
            # hairball_cli/config.py DEFAULT_CONFIG (display.show_reasoning).
            "show_reasoning": True,
    ...
    if config_path.exists():
        try:
            with open(config_path, "r", encoding="utf-8") as f:
                from hairball_cli.config import _normalize_root_model_keys
            ...
            if "model" in file_config:
                if isinstance(file_config["model"], str):
                    ...
                    # hardcoded default and HairballCLI.__init__ checks "default" first.
    ...
    from hairball_cli.config import _expand_env_vars
    ...
    # hairball_cli.config._load_config_impl (which has its own managed merge), so
    ...
    # `hairball config`/`doctor`/guards (which use load_config) honor it. The
    ...
    # Normalize config key: the new config system (hairball_cli/config.py) and all
    ...
    # Local backend: always os.getcwd(). Use `cd /dir && hairball` to control it.
    ...
    # UNLESS we're inside a gateway process (detected by _HAIRBALL_GATEWAY marker)
    # where it was already set correctly by gateway/run.py's config bridge.
    _is_gateway = os.environ.get("_HAIRBALL_GATEWAY") == "1"
    ...
    if isinstance(security_config, dict):
        ...
        if redact is not None:
            os.environ["HAIRBALL_REDACT_SECRETS"] = str(redact).lower()
...
# Initialize centralized logging early — agent.log + errors.log in ~/.hairball/logs/.
# This ensures CLI sessions produce a log trail even before AIAgent is instantiated.
try:
    from hairball_logging import setup_logging
...
try:
    from hairball_cli.config import print_config_warnings
...
try:
    from hairball_cli.skin_engine import init_skin_from_config
...
try:
    ...
    class _AsyncHttpxDelNeuter:
        ...
        ``hairball --help`` paths inside the chat command flow).  See
...
def get_tool_definitions(*args, **kwargs):
    from hairball_cli.mcp_startup import wait_for_mcp_discovery
...
from hairball_cli.banner import build_welcome_banner
from hairball_cli.commands import SlashCommandCompleter, SlashCommandAutoSuggest
...
from hairball_cli.callbacks import prompt_for_secret
...
def _prepare_deferred_agent_startup() -> None:
    ...
    if _deferred_agent_startup_done:
        return
    if os.environ.get("HAIRBALL_DEFER_AGENT_STARTUP") != "1":
        return
    _deferred_agent_startup_done = True
    _accept_hooks = os.environ.get("HAIRBALL_ACCEPT_HOOKS", "").lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def _arm_exit_watchdog(timeout_s: float | None = None) -> None:
    ...
    Tune with ``HAIRBALL_EXIT_WATCHDOG_S`` (seconds); ``0`` disables.
    """
    if timeout_s is None:
        try:
            timeout_s = float(os.getenv("HAIRBALL_EXIT_WATCHDOG_S", "30"))
...
def _notify_session_finalize(
    *,
    ...
    try:
        from hairball_cli.plugins import invoke_hook as _invoke_hook
        _invoke_hook(
            "on_session_finalize",
...
def _emit_interrupted_session_end(cli, *, reason: str = "keyboard_interrupt") -> None:
    ...
    try:
        ...
        pass
    cancel_plan = getattr(cli, "_cancel_active_plan_execution", None)
    if callable(cancel_plan):
        cancel_plan(reason=reason)
...
def _resolve_worktree_base(repo_root: str) -> tuple:
    ...
    (the ``~/.hairball/hairball-agent`` clone is updated only by ``hairball update``,
...
def _setup_worktree(repo_root: str = None, sync_base: bool = True) -> Optional[Dict[str, str]]:
    ...
    if not repo_root:
        ...
        print("  cd into your project repo first, then run hairball -w")
        return None

    short_id = uuid.uuid4().hex[:8]
    wt_name = f"hairball-{short_id}"
    branch_name = f"hairball/{wt_name}"
    ...
    try:
        subprocess.run(
            ["git", "worktree", "lock", "--reason", f"hairball pid={os.getpid()}", str(wt_path)],
...
def _worktree_lock_is_live(repo_root: str, worktree_path: str, timeout: int = 10):
    ...
    ``hairball -w`` locks each worktree with reason ``hairball pid=<pid>`` so a
    concurrent hairball process' startup prune leaves an in-use worktree alone.
    ...
    - ``"dead"``  — locked but the owning pid is gone, or the reason isn't a
                    parseable hairball lock (safe to unlock + reap).
    ...
    for line in result.stdout.splitlines():
        if line.startswith("worktree "):
            ...
            m = re.search(r"(?:hairball|hairball) pid=(\d+)", reason)
            if not m:
                # Locked by something we don't recognize as a Hairball/legacy
                # session (or lock reason unavailable). Treat as dead — a foreign
                # lock on a -w worktree is almost certainly a leftover, and
...
def _run_state_db_auto_maintenance(session_db) -> None:
    ...
    :func:`hairball_cli.config.load_config` (the authoritative loader that
    ...
    if session_db is None:
        return
    try:
        from hairball_cli.config import load_config as _load_full_config
        from hairball_constants import get_hairball_home as _get_hairball_home
        _hairball_home_maint = _get_hairball_home()
        ...
        try:
            if not session_db.get_meta("ghost_session_prune_v1"):
                pruned = session_db.prune_empty_ghost_sessions(
                    sessions_dir=_hairball_home_maint / "sessions"
                )
...
def _run_checkpoint_auto_maintenance() -> None:
    ...
    :func:`hairball_cli.config.load_config`. Honours ``auto_prune`` /
...
def _prune_stale_worktrees(repo_root: str, max_age_hours: int = 24) -> None:
    ...
    Lock handling (orthogonal to age): ``hairball -w`` locks each worktree with
    reason ``hairball pid=<pid>`` so a concurrent hairball process leaves an in-use
    ...
    *dead*-locked one (owning pid gone — a crashed session) is unlocked first
    ...
    Also prunes orphaned ``hairball/*`` and ``pr-*`` local branches that
    ...
    for entry in worktrees_dir.iterdir():
        if not entry.is_dir() or not (
            entry.name.startswith("hairball-") or entry.name.startswith("hairball-")
        ):
            continue
        ...
        # hairball process means the worktree is actively in use — never touch
...
def _prune_orphaned_branches(repo_root: str) -> None:
    """Delete local ``hairball/hairball-*`` and ``pr-*`` branches with no worktree.

    These are auto-generated by ``hairball -w`` sessions and PR review
    ...
    orphaned = [
        ...
        and (
            b.startswith("hairball/hairball-")
            or b.startswith("hairball/hairball-")
            or b.startswith("pr-")
        )
    ]
...
#
# Detection priority:
#   1. HAIRBALL_LIGHT / HAIRBALL_TUI_LIGHT env (true/false) — explicit override
#   2. HAIRBALL_TUI_THEME=light|dark — explicit theme
#   3. HAIRBALL_TUI_BACKGROUND=#RRGGBB — explicit bg hint
#   4. COLORFGBG env (set by xterm/Konsole/urxvt) — bg slot 7/15 = light
#   5. OSC 11 query (\x1b]11;?\x1b\\) — ask the terminal directly
#   6. Default: assume dark (matches the legacy Hairball assumption)
#
...
def _detect_light_mode() -> bool:
    ...
    try:
        ...
        for var in ("HAIRBALL_LIGHT", "HAIRBALL_TUI_LIGHT"):
            ...
            if _FALSE_RE.match(v):
                ...
                return result
        # 2. Theme hint
        theme = (os.environ.get("HAIRBALL_TUI_THEME") or "").strip().lower()
        if theme == "light":
            result = True
            _LIGHT_MODE_CACHE = result
            return result
        if theme == "dark":
            ...
            return result
        # 3. Explicit bg hex
        bg_hint = os.environ.get("HAIRBALL_TUI_BACKGROUND") or ""
...
def _install_skin_light_mode_hook() -> None:
    ...
    try:
        from hairball_cli.skin_engine import SkinConfig  # type: ignore[import]
    except Exception:
        return
    if getattr(SkinConfig, "_hairball_light_mode_hook_installed", False):
        return
    ...
    SkinConfig._hairball_light_mode_hook_installed = True  # type: ignore[attr-defined]
...
class _SkinAwareAnsi:
    ...
    def __str__(self) -> str:
        if self._cached is None:
            try:
                from hairball_cli.skin_engine import get_active_skin
...
def _strip_markdown_syntax(text: str) -> str:
    ...
    # but in Hairball output it's common to display cron schedules verbatim.
    #
...
def _render_completion_assessment_status(result: Any) -> str:
    """Render audit metadata locally without changing reply text or history."""
    if not isinstance(result, dict):
        return ""
    from agent.completion_contract import completion_assessment_status_line

    line = completion_assessment_status_line(result.get("completion_assessment"))
    if line:
...
from hairball_constants import is_termux as _is_termux_environment
...
def _apply_bracketed_paste_timeout_patch() -> None:
    ...
    ``_hairball_bp_timeout_patched`` sentinel on the module.
    """
    try:
        ...
        if getattr(_vt100_mod, "_hairball_bp_timeout_patched", False):
            return
        ...
        def _patched_vt100_feed(self_parser, data: str) -> None:
            if self_parser._in_bracketed_paste:
                ...
                if end_mark in self_parser._paste_buffer:
                    ...
                    self_parser._hairball_bp_start = None
                    if remaining:
                        _patched_vt100_feed(self_parser, remaining)
                else:
                    bp_start = getattr(self_parser, "_hairball_bp_start", None)
                    now = time.monotonic()
                    if bp_start is None:
                        self_parser._hairball_bp_start = now
        ...
        _vt100_mod._hairball_bp_timeout_patched = True
...
class ChatConsole:
    ...
    def status(self, *_args, **_kwargs):
        ...
        ``HairballCLI._busy_command()``.
        """
        yield self

# Hairball's terminal mark mirrors the cat used by the product shell.
HAIRBALL_AGENT_LOGO = """[bold #FFD700]⢰⣦⡀  ⢀⣴⡆   █  █  ██   ███  ██   ██    ██   █    █[/]
[bold #FFD700]⢸⣿⣷⣶⣶⣾⣿⡇   █  █ █  █   █   █ █  █ █  █  █  █    █[/]
[#FFBF00]⢸⣿⠟⣿⣿⡟⣿⡇   ████ ████   █   ██   ██   ████  █    █[/]
[#CD7F32]⢸⣿⣶⣿⣿⣶⣿⡇   █  █ █  █   █   █ █  █ █  █  █  █    █[/]
[#CD7F32] ⠙⠿⠿⠿⠿⠋    █  █ █  █  ███  █ █  ██   █  █  ███  ███[/]"""

HAIRBALL_CAT_HERO = """[#CD7F32]   ⢀⣀                    ⣀⡀[/]
[#CD7F32]  ⢀⣿⣿⣷⣄                ⣠⣾⣿⣿⡄[/]
[#FFBF00]  ⢸⣿⣿⣿⣟⣦⡀             ⣼⢿⣿⣿⣿⡇[/]
[#FFBF00]  ⢸⣿⣿⣿⣿⡞⣷⣄          ⢠⣾⢧⣿⣿⣿⣿⡟[/]
[#FFD700]  ⢸⣿⣿⣿⣿⣿⢸⣿⣦⣤⣴⣶⣶⣶⣶⣦⣤⣴⣿⡏⣾⣿⣿⣿⣿⡇[/]
...
def _build_compact_banner() -> str:
    ...
    if skin_name == "default":
        line1 = "ฅ HAIRBALL - PERSONAL AI AGENT"
        tiny_line = "ฅ HAIRBALL"
    else:
        agent_name = _skin.get_branding("agent_name", "Hairball Agent") if _skin else "Hairball Agent"
        line1 = f"{agent_name} - AI Agent Framework"
        tiny_line = agent_name

    if os.environ.get("HAIRBALL_FAST_STARTUP_BANNER") == "1":
        from hairball_cli import __release_date__ as _release_date
        from hairball_cli import __version__ as _version

        version_line = f"Hairball Agent v{_version} ({_release_date})"
    else:
        version_line = format_banner_version_label()

    w = min(shutil.get_terminal_size().columns - 2, 88)
    if w < 30:
        return f"\n[{title_color}]{tiny_line}[/] [dim {dim_color}]- Hairball[/]\n"
...
def _get_plugin_cmd_handler_names() -> set:
    ...
    try:
        from hairball_cli.plugins import get_plugin_commands
...
def save_config_value(key_path: str, value: any) -> bool:
    ...
    1. ~/.hairball/config.yaml (user config - preferred, used if it exists)
    2. ./cli-config.yaml (project config - fallback)
    ...
    try:
        # Ensure parent directory exists (for ~/.hairball/config.yaml on first use)
...
class HairballCLI(CLIAgentSetupMixin, CLICommandsMixin):
    """
    Interactive CLI for the Hairball Agent.
    ...
    def __init__(
        ...
        # Read max_tokens from config (env var override: HAIRBALL_MAX_TOKENS)
        _env_mt = os.environ.get("HAIRBALL_MAX_TOKENS")
        if _env_mt:
            try:
        ...
        if self.model == _DEFAULT_CONFIG_MODEL:
            ...
            if "localhost" in _base_url or "127.0.0.1" in _base_url:
                from hairball_cli.runtime_provider import _auto_detect_local_model
        ...
        self.requested_provider = (
            ...
            or os.getenv("HAIRBALL_INFERENCE_PROVIDER")
            or "auto"
        )
        ...
        elif os.getenv("HAIRBALL_MAX_ITERATIONS"):
            try:
                self.max_turns = int(os.getenv("HAIRBALL_MAX_ITERATIONS", ""))
            except (TypeError, ValueError):
                self.max_turns = 90
        else:
            self.max_turns = 90
        ...
        # by `hairball chat --ignore-rules` in hairball_cli/main.py. When true we
        # pass skip_context_files=True and skip_memory=True to AIAgent so
        # AGENTS.md/SOUL.md/.cursorrules and persistent memory are not loaded.
        self.ignore_rules = ignore_rules or os.environ.get("HAIRBALL_IGNORE_RULES") == "1"
        
        # Ephemeral system prompt: env var takes precedence, then config
        self.system_prompt = (
            os.getenv("HAIRBALL_EPHEMERAL_SYSTEM_PROMPT", "")
        ...
        # shared chokepoint in hairball_constants (Closes #21256).
        from hairball_constants import resolve_reasoning_config
        ...
        try:
            ...
            try:
                ...
                Console(stderr=True).print(
                    ...
                    f"  Reason: {e}\n"
                    "  Fix the state.db store (e.g. `hairball update` to rebuild the venv) to restore persistence."
                )
            except Exception:
        ...
        # it's shared across all Hairball processes for this HAIRBALL_HOME.
        # Never blocks startup on failure.
        _run_state_db_auto_maintenance(self._session_db)

        # Opportunistic shadow-repo cleanup — deletes orphan/stale
        # checkpoint repos under ~/.hairball/checkpoints/.  Opt-in via
        ...
        self._history_file = _hairball_home / ".hairball_history"
        ...
        self._voice_barge_capture = threading.Event()
        self._voice_fd_active = threading.Event()
    ...
    def _claim_active_session(self, surface: str = "cli", *, stderr: bool = False) -> bool:
        ...
        if self._active_session_lease is not None:
            return True
        try:
            from hairball_cli.active_sessions import try_acquire_active_session
    ...
    def _recover_terminal_after_interrupt(self) -> None:
        ...
        """
        try:
            from hairball_cli.curses_ui import flush_stdin
            flush_stdin()
        except Exception:
            pass
    ...
    def _pet_resolve_config(self) -> None:
        ...
        switch made via ``/pet`` or ``hairball pets`` without a restart, mirroring
    ...
    def set_voice_record_key_cache(self, raw_key: object) -> None:
        ...
        """
        try:
            from hairball_cli.voice import format_voice_record_key_for_status
    ...
    def _build_status_bar_text(self, width: Optional[int] = None) -> str:
        ...
        try:
            ...
            if width < 52:
                text = f"ฅ {snapshot['model_short']} · {duration_label}"
                if yolo_active:
                    text += " · ⚠ YOLO"
                return self._trim_status_bar_text(text, width)
            if width < 76:
                parts = [f"ฅ {snapshot['model_short']}", percent_label]
            ...
            compressions = snapshot.get("compressions", 0)
            parts = [f"ฅ {snapshot['model_short']}", context_label, percent_label]
            if compressions:
                parts.append(f"🗜️ {compressions}")
            bg_count = snapshot.get("active_background_tasks", 0)
            if bg_count:
            ...
            return f"ฅ {self.model if getattr(self, 'model', None) else 'Hairball'}"
    ...
    def _normalize_model_for_provider(self, resolved_provider: str) -> bool:
        ...
        try:
            from hairball_cli.model_normalize import (
        ...
        if resolved_provider == "copilot":
            try:
                from hairball_cli.models import copilot_model_api_mode, normalize_copilot_model_id
        ...
        if resolved_provider in {"opencode-zen", "opencode-go"}:
            try:
                from hairball_cli.models import normalize_opencode_model_id, opencode_model_api_mode
        ...
        if "/" in current_model:
            ...
            if not self._model_is_default:
                self._console_print(
                    ...
                    f"using '{slug}' for ChatGPT.[/]"
                )
            self.model = slug
            current_model = slug
            changed = True

        # 2. Replace untouched default with a reference model
        if self._model_is_default:
            fallback_model = "gpt-5.3-codex"
            try:
                from hairball_cli.codex_models import get_codex_model_ids
    ...
    def _emit_stream_text(self, text: str) -> None:
        ...
        if not self._stream_box_opened:
            ...
            try:
                from hairball_cli.skin_engine import get_active_skin
                _skin = get_active_skin()
                label = _skin.get_branding("response_label", "ฅ Hairball")
            ...
            w = self._scrollback_box_width()
            fill = w - 2 - HairballCLI._status_bar_display_width(label)
    ...
    def _show_security_advisories(self):
        ...
        clean) listing the worst hit and pointing at ``hairball doctor``.
        Banner-cache rate-limits this to once per 24h per advisory; full
        remediation lives behind ``hairball doctor`` so the banner stays
        small.
        """
        try:
            from hairball_cli.security_advisories import (
                detect_compromised,
                startup_banner,
            )
    ...
    def show_banner(self):
        ...
        # Auto-compact for narrow terminals — the full banner with cat hero
        ...
        if ctx_len and ctx_len < MINIMUM_CONTEXT_LENGTH:
            ...
            self._console_print(
                f"[dim]   Hairball needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens. Tool schemas + system prompt use a large fixed prefix.[/]"
            )
        ...
        # Warn if the configured model is a managed Hairball LLM (not agentic)
        from hairball_cli.model_switch import is_managed_hairball_non_agentic

        model_name = getattr(self, "model", "") or ""
        if is_managed_hairball_non_agentic(model_name):
            self._console_print()
            self._console_print(
                "[bold yellow]⚠  Hairball 3 & 4 models are NOT agentic and are not "
                "designed for use with Hairball Agent.[/]"
            )
            self._console_print(
    ...
    def _try_attach_clipboard_image(self) -> bool:
        ...
        Saves the image to ~/.hairball/images/ and appends the path to
        ``_attached_images``.  Returns True if an image was attached.
        """
        from hairball_cli.clipboard import save_clipboard_image

        img_dir = get_hairball_home() / "images"
    ...
    def _show_tool_availability_warnings(self):
        ...
        try:
            ...
            if api_key_missing:
                ...
                self._console_print("[dim]   Run 'hairball setup' to configure[/]")
        except Exception:
    ...
    def _fast_command_available(self) -> bool:
        try:
            from hairball_cli.models import model_supports_fast_mode
        except Exception:
            return False
        agent = getattr(self, "agent", None)
    ...
    def show_help(self):
        ...
        from hairball_cli.commands import COMMANDS_BY_CATEGORY

        try:
            from hairball_cli.skin_engine import get_active_help_header
        ...
        _cprint(f"\n  {_DIM}Tip: Just type your message to chat with Hairball!{_RST}")
    ...
    def _list_recent_sessions(self, limit: int = 10) -> list[dict[str, Any]]:
        ...
        if not self._session_db:
            return []
        try:
            from hairball_cli.session_listing import query_session_listing
    ...
    def _show_recent_sessions(self, *, reason: str = "history", limit: int = 10) -> bool:
        ...
        from hairball_cli.main import _relative_time
    ...
    def show_history(self):
        ...
        for msg in self.conversation_history:
            ...
            _cli_visible_print(f"\n  [Hairball #{visible_index}]{_ts_suffix(msg)}")
    ...
    def _discard_session_if_empty(self, session_id: Optional[str]) -> bool:
        ...
        ``/resume`` and ``hairball sessions list``. Delegates the
        ...
        if getattr(self, "conversation_history", None):
            return False
        try:
            from hairball_constants import get_hairball_home as _ghh
    ...
    def new_session(self, silent=False, title=None):
        ...
        if self._session_db and old_session_id:
            ...
            # /resume and `hairball sessions list` (gemini-cli#27770 port).
            ...
            if self._session_db:
                try:
                    ...
                    self._session_db.create_session(
                        session_id=self.session_id,
                        source=os.environ.get("HAIRBALL_SESSION_SOURCE", "cli"),
                        model=self.model,
                        model_config={
    ...
    def save_conversation(self):
        """Save the current conversation to a JSON snapshot under ~/.hairball/sessions/saved/.
        ...
        DB, so the live session remains resumable via ``hairball --resume <id>``
        ...
        saved_dir = get_hairball_home() / "sessions" / "saved"
        try:
            saved_dir.mkdir(parents=True, exist_ok=True)
        except Exception as e:
            print(f"(x_x) Failed to create save directory {saved_dir}: {e}")
            return
        path = saved_dir / f"hairball_conversation_{timestamp}.json"
        ...
        try:
            ...
            if self.session_id:
                print(f"       Resume the live session with: hairball --resume {self.session_id}")
        except Exception as e:
    ...
    def _run_curses_picker(self, title: str, items: list[str], default_index: int = 0) -> int | None:
        ...
        import threading
        from hairball_cli.curses_ui import curses_single_select
    ...
    def _confirm_expensive_model_switch(self, result) -> bool:
        ...
        choices = [
            ("once", "Switch anyway", "Use this model for the current Hairball session."),
    ...
    def _apply_model_switch_result(self, result, persist_global: bool) -> None:
        ...
        if self.agent is not None:
            try:
                from hairball_cli.context_switch_guard import merge_preflight_compression_warning
        ...
        # Context: always resolve via the provider-aware chain so reference OAuth,
        # Copilot, and managed-enforced caps win over the raw models.dev entry
        # (e.g. gpt-5.5 is 1.05M on openai but 272K on reference OAuth).
        mi = result.model_info
        try:
            from hairball_cli.model_switch import resolve_display_context_length
    ...
    def _handle_model_picker_selection(self, persist_global: bool = False) -> None:
        ...
        if stage == "provider":
            ...
            # (same lists as `hairball model` and gateway pickers).
            # Only fall back to the live provider catalog when the curated
            # list is empty (e.g. user-defined endpoints with no curated list).
            model_list = provider_data.get("models", [])
            if not model_list:
                try:
                    from hairball_cli.models import provider_model_ids
        ...
        if stage == "model":
            ...
            if selected < len(model_list):
                from hairball_cli.model_switch import switch_model
    ...
    def _handle_model_switch(self, cmd_original: str):
        ...
        """
        from hairball_cli.model_switch import (
            switch_model,
            parse_model_flags,
            resolve_persist_behavior,
        )
        from hairball_cli.providers import get_label
    ...
    def _handle_codex_runtime(self, cmd_original: str) -> None:
        ...
        Usage:
            ...
            /codex-runtime auto                  — Hairball default (chat_completions)
            /codex-runtime codex_app_server      — hand turns to codex subprocess
            /codex-runtime on / off              — synonyms for the above
        """
        from hairball_cli import codex_runtime_switch as crs
    ...
    def _should_handle_model_command_inline(self, text: str, has_images: bool = False) -> bool:
        ...
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        try:
            from hairball_cli.commands import resolve_command
    ...
    def _park_idle_next_step(self, text: str) -> bool:
        """Park a next-step note without starting a turn."""
        cleaned = str(text or "").strip()
        if not cleaned:
            return False
        agent = getattr(self, "agent", None)
        if agent is not None and callable(getattr(agent, "inject", None)):
            return bool(agent.inject(cleaned))
        parked = getattr(self, "_idle_next_step", None)
        if parked is None:
            self._idle_next_step = []
            parked = self._idle_next_step
        parked.append(cleaned)
        return True

    def _apply_idle_next_step(self, agent) -> None:
        parked = list(getattr(self, "_idle_next_step", []) or [])
        self._idle_next_step = []
        if agent is None or not callable(getattr(agent, "inject", None)):
            return
        for note in parked:
            try:
                agent.inject(note)
    ...
    def _should_handle_inject_command_inline(self, text: str, has_images: bool = False) -> bool:
        """Return True when /inject should be dispatched immediately while running."""
        if not text or has_images or not _looks_like_slash_command(text):
            return False
        if not getattr(self, "_agent_running", False):
            return False
        try:
            ...
            base = text.split(None, 1)[0].lower().lstrip('/')
            cmd = resolve_command(base)
            return bool(cmd and cmd.name == "inject")
        except Exception:
            return False
    ...
    def _show_gateway_status(self):
        ...
        try:
            ...
            print()
            print(f"  Configuration file: {display_hairball_home()}/config.yaml")
            print()
            ...
            print(f"    2. Or configure settings in {display_hairball_home()}/config.yaml")
            print()
    
    # process_command lives on CLICommandsMixin (hairball_cli/cli_commands_mixin.py)
    # — god-file decomposition; HairballCLI inherits it via MRO.
    ...
    def _get_goal_manager(self):
        ...
        """
        try:
            from hairball_cli.goals import GoalManager
            from hairball_cli.config import load_config
        except Exception as exc:
    ...
    def _drain_process_notifications(self, consumer: str) -> None:
        ...
        process using the same Hairball profile.  Always pass this CLI's stable
        ...
        from tools.async_delegation import (
            acknowledge_suppressed_inject,
            claim_event_delivery,
            complete_event_delivery,
            should_suppress_inject,
        )
        ...
        for event, synthetic_message in process_registry.drain_notifications(
            ...
            # Collab Mailbox XOR — belt-and-suspenders with format_process_notification.
            if should_suppress_inject(event):
                acknowledge_suppressed_inject(event, "cli-drain-skip")
                continue
    ...
    def _maybe_continue_goal_after_turn(self) -> None:
        ...
        try:
            from hairball_cli.goals import gather_background_processes as _gather_bg
    ...
    def _toggle_verbose(self):
        ...
        from hairball_cli.colors import Colors as _Colors
        labels = {
    ...
    def _is_session_yolo_active(self) -> bool:
        ...
        ``HAIRBALL_YOLO_MODE`` into ``_YOLO_MODE_FROZEN`` before tool imports
        happen.
        """
        try:
            from tools.approval import (
    ...
    def _toggle_yolo(self):
        ...
        ``HAIRBALL_YOLO_MODE`` here — that env var is read once at module import
    ...
    def _manual_compress(self, cmd_original: str = ""):
        ...
        * ``/compress here [N]`` — boundary-aware compression. Summarize
        ...
        from hairball_cli.partial_compress import (
    ...
    def _handle_usage_command(self, cmd_original: str):
        ...
        banked reference rate-limit reset credit (guarded: refuses when limits
    ...
    def _usage_reset(self, force: bool = False):
        """`/usage reset [--force]` — redeem one banked reference reset credit."""
        provider = (
        ...
        if normalized != "openai-codex":
            ...
            print("  Switch with `/model` or `hairball auth` first.")
            return
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as _pool:
            try:
                ...
                print("  ❌ Timed out talking to the reference backend — try again shortly.")
                return
        print(f"  {result.message}")

    def _show_usage(self):
        """Rate limits + session token usage (when a live agent exists) + Account credits.

        The Account credits block is agent-independent (a portal fetch), so it runs even
        ...
        """
        if not self.agent:
            if not self._print_managed_credits_block():
                print("(._.) No active agent -- send a message first.")
            return
        ...
        try:
            from agent.usage_pricing import session_cost_summary

            cost = session_cost_summary(agent)
            if cost.status == "actual" and cost.display_cost_usd is not None:
                cost_text = f"${cost.display_cost_usd:.4f} (provider-reported)"
            elif cost.status == "included":
                cost_text = "$0.0000 (included in subscription)"
            elif cost.display_cost_usd is not None:
                cost_text = f"~${cost.display_cost_usd:.4f} (estimated)"
                if cost.actual_cost_usd is not None and cost.actual_call_count:
                    cost_text += (
                        f"; actual ${cost.actual_cost_usd:.4f} on "
                        f"{cost.actual_call_count}/{cost.total_call_count} billed calls"
                    )
            else:
                cost_text = "unavailable"
            print(f"  Total cost:                {cost_text:>10}")
        except Exception:
            pass
        ...
        # Account credits magnitudes + monthly-grant gauge (agent-independent — also
        # runs at the no-agent / no-calls early-returns above). See the helper.
        self._print_managed_credits_block()
        ...
        if self.verbose:
            ...
            # Console quietness is enforced by hairball_logging not
            # installing a console StreamHandler in non-verbose mode.

    def _print_managed_credits_block(self) -> bool:
        """Print the Account credits magnitudes + monthly-grant gauge when a managed account
        is logged in. Returns True if it printed anything.

        Delegates to the shared ``agent.account_usage.managed_credits_lines`` helper —
        the single source for the /usage credits block across CLI, gateway, and TUI.
        It's agent-independent (a portal fetch gated on "a managed account is logged in",
        ...
        wall-clock-bounded inside the helper; also honors HAIRBALL_DEV_CREDITS_FIXTURE
        for offline testing — same behavior as every other surface.
        """
        from agent.account_usage import managed_credits_lines

        lines = managed_credits_lines()
        if not lines:
            return False
        print()
        for line in lines:
    ...
    def _show_credits(self):
        """`/credits` — focused managed credit balance + top-up handoff.
        ...
        if not view.logged_in:
            print()
            _cprint(f"  💳 {_d('Not logged into Managed Portal.')}")
            print("  Run `hairball portal` to log in, then /credits.")
            return
        ...
        for line in view.balance_lines:
            # Drop the helper's own "📈 Account credits" header — we print our own.
    ...
    def _show_billing(self, command: str = "/billing"):
        """`/billing` — terminal billing for managed (one interactive modal).
        ...
        if not state.logged_in:
            ...
            if state.error:
                ...
                _cprint(f"  💳 {_d('Not logged into Managed Portal.')}")
                print("  Run `hairball portal` to log in, then /billing.")
            return
    ...
    def _billing_confirm_and_charge(self, state, amount):
        ...
        print(f"  {'─' * 41}")
        _consent = (
            "By confirming, you allow Hairball to charge your card."
        )
        ...
        from hairball_cli.managed_billing import (
            BillingError,
    ...
    def _billing_render_charge_error(self, state, exc):
        ...
        from hairball_cli.managed_billing import BillingRateLimited
    ...
    def _billing_handle_scope_required(self, state):
        ...
        if not getattr(self, "_app", None):
            print("  Run `hairball portal` and approve terminal billing, then retry.")
            return
        confirm_choices = [
        ...
        if choice != "yes":
            ...
            return
        try:
            from hairball_cli.auth import step_up_managed_billing_scope

            granted = step_up_managed_billing_scope(open_browser=True)
        except Exception as exc:
    ...
    def _billing_auto_reload_flow(self, state):
        ...
        print()
        _ar_consent = (
            f"By confirming, you authorize Hairball to charge {card.masked} "
    ...
    def _check_config_mcp_changes(self) -> None:
        ...
        from hairball_cli.config import get_config_path as _get_config_path
    ...
    def _reload_skills(self) -> None:
        """Reload skills: rescan ~/.hairball/skills/ and queue a note for the
        next user turn.
                ...
                try:
                    if (
                        ...
                        if not is_seen(CLI_CONFIG, TOOL_PROGRESS_FLAG):
                            ...
                            mark_seen(_hairball_home / "config.yaml", TOOL_PROGRESS_FLAG)
    ...
    def _on_tool_complete(self, tool_call_id: str, function_name: str, function_args: dict, function_result: str):
        ...
        # A top-level delegate_task dispatches in the background (mailbox Single
        # Rail — no inject turn). Say so once so the idle prompt doesn't read as
        # "nothing happened" (⛓ tracks the work; collab_wait surfaces summaries).
            ...
            if isinstance(parsed, dict) and parsed.get("status") == "dispatched" and parsed.get("mode") == "background":
                ...
                try:
                    _cprint(
                        ...
                        f"collab_wait when ready (collab_collect optional). Keep chatting.\033[0m"
                    )
                except Exception:
                    pass
    ...
    def _voice_speak_response_async(self, text: str) -> None:
        ...
        current_stop = getattr(self, "_voice_tts_stop", None)
        if current_stop is None or current_stop.is_set():
            self._voice_tts_stop = threading.Event()
        threading.Thread(
    ...
    def _voice_speak_response(self, text: str):
        ...
        self._voice_tts_done.clear()
        try:
            from hairball_cli.voice import speak_text

            speak_text(text, getattr(self, "_voice_tts_stop", None))
        except Exception as e:
    ...
    def _voice_full_duplex_listener(self) -> None:
        """Listen for a real spoken interjection across generation and playback."""
        ...
        try:
            ...
            if not voice_config.get("barge_in", True):
                return
            try:
                multiplier = float(
                    voice_config.get("barge_in_threshold_multiplier", 0) or 0
                )
            except (TypeError, ValueError):
                multiplier = 0.0
            try:
                grace_ms = int(
                    float(voice_config.get("barge_in_grace_seconds", 0.5)) * 1000
                )
            except (TypeError, ValueError):
                grace_ms = 500
            ...
            def on_trigger(phase: str) -> None:
                ...
                try:
                    if self.agent is not None and getattr(self, "_agent_running", False):
                        _cprint(
                            f"\n{_DIM}Voice interjection — interrupting current turn…{_RST}"
                        )
                        self.agent.interrupt()
                        self._cancel_active_plan_execution(
                            reason="voice_interrupted"
                        )
                except Exception as exc:
                    logger.debug("Voice interjection interrupt failed: %s", exc)
            ...
            self._voice_barge_capture.clear()
            logger.debug("Voice full-duplex listener failed: %s", exc)
        finally:
            fd_active.clear()

    def _voice_submit_barge_utterance(self, wav_path: str) -> None:
        """Transcribe a captured interjection and queue it as the next turn."""
        submitted = False
        try:
            from hairball_cli.config import load_config
            ...
            if transcript:
                ...
                _cprint(
                    f"\n{_DIM}Transcription failed: "
                    f"{result.get('error', 'Unknown error')}{_RST}"
                )
        except Exception as exc:
            _cprint(f"\n{_DIM}Voice processing error: {exc}{_RST}")
        finally:
            try:
            ...
            if (
                ...
                threading.Thread(
                    target=self._voice_start_recording,
                    daemon=True,
                ).start()
    ...
    def _disable_voice_mode(self):
        ...
        try:
            from tools.voice_mode import stop_playback, stop_thinking_sound
    ...
    def _cancel_active_plan_execution(
        self,
        *,
        reason: str,
        execution_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Cancel only the Plan execution currently owned by this CLI turn."""

        agent = getattr(self, "agent", None)
        active_context = getattr(self, "_active_plan_context", None)
        context_execution_id = (
            str(active_context.get("plan_execution_id") or "")
            if isinstance(active_context, dict)
            else ""
        )
        bound_execution_id = str(
            execution_id
            or getattr(agent, "_plan_execution_id", "")
            or context_execution_id
            or ""
        ).strip()
        if not bound_execution_id:
            return None
        context_session_id = (
            str(active_context.get("session_id") or "")
            if isinstance(active_context, dict)
            else ""
        )
        session_id = str(
        ...
        try:
            db = None
            get_db = getattr(agent, "_get_session_db_for_recall", None)
            if callable(get_db):
                db = get_db()
            elif agent is not None:
                db = getattr(agent, "_session_db", None)
            from agent.plan_commands import cancel_active_plan_turn

            return cancel_active_plan_turn(
                session_id,
                bound_execution_id,
                surface="cli",
                actor="user",
                reason=str(reason or "user_interrupted"),
                operation_id=f"cli-plan-interrupt:{bound_execution_id}",
                db=db,
            )
        except Exception as exc:
            logger.warning("Could not cancel active CLI Plan execution: %s", exc)
            return None
    ...
    def chat(self, message, images: list = None) -> Optional[str]:
        ...
        if agent is None:
            return None
        self._apply_idle_next_step(agent)
        ...
        try:
            ...
            # --- Streaming TTS setup ---
            # Every configured provider uses the same sentence pipeline. Native
            # streamers play chunks; other providers synthesize one sentence at
            # a time without changing the model-facing callback contract.
            ...
            if use_streaming_tts:
                ...
                tts_thread = threading.Thread(
                    ...
                    kwargs={
                        "display_callback": (
                            None if self.streaming_enabled else display_callback
                        )
                    },
                    daemon=True,
                )
            ...
            # Voice transport must not cap the model's answer length or remove
            # useful structure. The shared spoken-text normalizer makes the
            # audible rendition natural without constraining the model output.
            _voice_prefix = ""

            _plan_context = getattr(self, "_pending_plan_context", None)
            self._pending_plan_context = None
            if isinstance(_plan_context, dict):
                _plan_context.setdefault(
                    "session_id", str(getattr(self, "session_id", "") or "")
                )
                self._active_plan_context = _plan_context
            ...
            def run_agent():
                ...
                try:
                    ...
                    pass
                _history_for_turn = self.conversation_history[:-1]
                if (
                    isinstance(_plan_context, dict)
                    and str(_plan_context.get("kind") or "") == "execute"
                ):
                    from agent.plan_handoff import prepare_plan_handoff

                    _handoff = prepare_plan_handoff(
                        _history_for_turn,
                        context_mode=_plan_context.get("context_mode") or "fresh",
                        plan_file=str(
                            (_plan_context.get("plan") or {}).get("plan_file")
                            if isinstance(_plan_context.get("plan"), dict)
                            else _plan_context.get("plan_file") or ""
                        ),
                        task_id=str(self.session_id or "default"),
                        agent=self.agent,
                    )
                    _history_for_turn = _handoff.history
                    if _handoff.warning:
                        try:
                            self.agent._emit_warning(_handoff.warning)
                        except Exception:
                            logger.warning("Plan handoff: %s", _handoff.warning)
                    if not _handoff.dispatch:
                        result = {
                            "completed": False,
                            "interrupted": True,
                            "execution_not_dispatched": True,
                            "error": "plan_compaction_cancelled",
                            "final_response": "",
                            "messages": [],
                            "api_calls": 0,
                        }
                ...
                try:
                    _run_kwargs = dict(
                        ...
                        conversation_history=_history_for_turn,  # Exclude staged user input; apply Plan handoff when selected.
                    ...
                    if isinstance(_plan_context, dict):
                        _run_kwargs["agent_mode"] = str(
                            _plan_context.get("agent_mode") or ""
                        )
                        _run_kwargs["system_message"] = str(
                            _plan_context.get("system_message") or ""
                        )
                        _plan_execution_id = str(
                            _plan_context.get("plan_execution_id") or ""
                        ).strip()
                        if _plan_execution_id:
                            _run_kwargs["plan_execution_id"] = _plan_execution_id
                ...
                if hasattr(self, '_interrupt_queue'):
                    try:
                        ...
                        if interrupt_msg:
                            ...
                            try:
                                _dbg = _hairball_home / "interrupt_debug.log"
            ...
            if isinstance(_plan_context, dict) and not agent_thread.is_alive():
                try:
                    from agent.plan_commands import finalize_plan_turn

                    _plan_db = self.agent._get_session_db_for_recall()
                    projection = finalize_plan_turn(
                        str(self.session_id or "default"),
                        _plan_context,
                        ...
                        actor="agent",
                        operation_id=str(
                            _plan_context.get("operation_id")
                            or _plan_context.get("plan_execution_id")
                            or f"cli-plan-{uuid.uuid4().hex}"
                        ),
                        db=_plan_db,
                    )
                    if projection:
                        from agent.plan_commands import _status_message

                        _cprint(f"  {_DIM}{_status_message(projection)}{_RST}")
                except Exception as exc:
                    _cprint(f"  Plan state error: {exc}")
            ...
            if use_streaming_tts and text_queue is not None:
                ...
                _tts_normal_exit = not (
                    tts_thread is not None and tts_thread.is_alive()
                )
            ...
            _render_completion_assessment_status(result)
            ...
            # Official leftover stays on next-step. Do not start a new turn.
            _leftover_steer = result.get("pending_steer") if result else None
            if _leftover_steer:
                preview = _leftover_steer[:60] + ("..." if len(_leftover_steer) > 60 else "")
                if self._park_idle_next_step(_leftover_steer):
                    print(f"\n⏩ Parked leftover next-step (does not start a turn): '{preview}'")
            ...
            if (
                isinstance(_plan_context, dict)
                and getattr(self, "_active_plan_context", None) is _plan_context
            ):
                delattr(self, "_active_plan_context")
            ...
            if getattr(self, "_voice_tts_stop", None) is stop_event:
                self._voice_tts_stop = None
            if _thinking_started:
                try:
                    from tools.voice_mode import stop_thinking_sound
    ...
    def _print_exit_summary(self, clear_screen: bool = True):
        ...
        if msg_count > 0:
            ...
            # profile names use the standard HAIRBALL_HOME, so no -p needed.
            try:
            ...
            print(f"  hairball --resume {self.session_id}{profile_flag}")
            if session_title:
                print(f"  hairball -c \"{session_title}\"{profile_flag}")
            print()
            ...
            try:
                from hairball_cli.skin_engine import get_active_goodbye
                goodbye = get_active_goodbye("Goodbye! ฅ")
            except Exception:
                goodbye = "Goodbye! ฅ"
            print(goodbye)
    ...
    def _get_tui_prompt_symbols(self) -> tuple[str, str]:
        ...
        """
        try:
            from hairball_cli.skin_engine import get_active_prompt_symbol
    ...
    def _get_tui_prompt_fragments(self):
        ...
        if self._agent_running:
            return _state_fragment("class:prompt-working", "ฅ")
        if self._voice_mode:
    ...
    def _build_tui_style_dict(self) -> dict[str, str]:
        ...
        try:
            from hairball_cli.skin_engine import get_prompt_toolkit_style_overrides
    ...
    def run(self):
        ...
        try:
            ...
            _welcome_text = _welcome_skin.get_branding("welcome", "Welcome to Hairball Agent! Type your message or /help for commands.")
            _welcome_color = _welcome_skin.get_color("banner_text", "#FFF8DC")
        except Exception:
            _welcome_text = "Welcome to Hairball Agent! Type your message or /help for commands."
        ...
        try:
            from hairball_cli.model_switch import prewarm_picker_cache_async
        ...
        try:
            _redact_raw = os.getenv("HAIRBALL_REDACT_SECRETS", "true")
            if _redact_raw.lower() not in {"1", "true", "yes", "on"}:
                self._console_print(
                    "[bold red]⚠  Secret redaction is DISABLED[/] "
                    f"(HAIRBALL_REDACT_SECRETS={_redact_raw}). "
            ...
            pass
        # First-time legacy-residue banner — fires once if ~/.openclaw/ exists
        # after an legacy→Hairball migration (especially migrations done by
        # legacy's own tool, which doesn't archive the source directory).
        try:
            from agent.onboarding import (
            ...
            if not is_seen(self.config, OPENCLAW_RESIDUE_FLAG) and detect_openclaw_residue():
                ...
                try:
                    from hairball_cli.config import get_config_path as _get_cfg_path_resid
        ...
        try:
            from hairball_cli.tips import get_random_tip
            _tip = get_random_tip()
            try:
        ...
        from hairball_cli.plugins import get_plugin_manager
        ...
        self._voice_tts_stop = None  # Active sentence pipeline stop event
        self._voice_barge_capture = threading.Event()
        self._voice_fd_active = threading.Event()
        ...
        def handle_ignored_terminal_sequence(event):
            ...
            install_ignored_terminal_sequences() in hairball_cli.pt_input_extras
        ...
        def handle_enter(event):
            ...
            if self._model_picker_state:
                try:
                    ...
                    from hairball_cli.model_switch import resolve_persist_behavior
            ...
            if text or has_images:
                ...
                if self._should_handle_steer_command_inline(text, has_images=has_images) or self._should_handle_inject_command_inline(text, has_images=has_images):
                ...
                if self._agent_running and not (text and _looks_like_slash_command(text)):
                    ...
                    try:
                        ...
                        if not is_seen(CLI_CONFIG, BUSY_INPUT_FLAG):
                            ...
                            mark_seen(_hairball_home / "config.yaml", BUSY_INPUT_FLAG)
        ...
        if _preserve_ctrl_enter_newline():
            ...
            def handle_ctrl_enter_newline(event):
                ...
                conflicting Hairball binding. See issue #22379.
                """
                event.current_buffer.insert_text('\n')
        ...
        def handle_ctrl_c(event):
            ...
            if self._agent_running and self.agent:
                ...
                self.agent.interrupt()
                self._cancel_active_plan_execution(reason="user_interrupted")
        ...
        def handle_ctrl_z(event):
            ...
            from prompt_toolkit.application import run_in_terminal
            from hairball_cli.skin_engine import get_active_skin
            agent_name = get_active_skin().get_branding("agent_name", "Hairball Agent")
        ...
        try:
            from hairball_cli.config import load_config
            from hairball_cli.voice import (
        ...
        def handle_paste(event):
            ...
            if pasted_text:
                ...
                if (lines_hit or chars_hit) and not buf.text.strip().startswith('/'):
                    _paste_counter[0] += 1
                    paste_dir = _hairball_home / "pastes"
        ...
        # Dynamic prompt: shows Hairball symbol when agent is working,
        ...
        def _get_clarify_display():
            ...
            box_width = _panel_box_width("Hairball needs your input", preview_lines)
            inner_text_width = max(8, box_width - 2)
            ...
            lines.append(('class:clarify-title', 'Hairball needs your input'))
            lines.append(('class:clarify-border', ' ' + ('─' * max(0, box_width - len("Hairball needs your input") - 3)) + '╮\n'))
            if not use_compact_chrome:
        ...
        def _get_model_picker_display():
            ...
            try:
                ...
                term_rows = shutil.get_terminal_size((100, 24)).lines
            scroll_offset, visible = HairballCLI._compute_model_picker_viewport(
        ...
        try:
            ...
            if not getattr(_pt_renderer, "_hairball_osd_patched", False):
                ...
                _pt_renderer._hairball_osd_patched = True
        except Exception:
            pass
        ...
        def _signal_handler(signum, frame):
            ...
            Grace window (``HAIRBALL_SIGTERM_GRACE``, default 1.5 s) gives
            ...
            try:
                if getattr(self, "agent", None) and getattr(self, "_agent_running", False):
                    ...
                    try:
                        _grace = float(os.getenv("HAIRBALL_SIGTERM_GRACE", "1.5"))
                    except (TypeError, ValueError):
                        _grace = 1.5
                    if _grace > 0:
                        time.sleep(_grace)
        ...
        try:
            ...
            # delivers spurious CTRL_C_EVENT to the hairball process when
        ...
        try:
            ...
            print(
                ...
                "Try reinstalling Python via pyenv or Homebrew, then re-run: hairball setup"
            )
            _run_cleanup()
        ...
        try:
            ...
            if self.agent and getattr(self, '_agent_running', False):
                try:
                    self.agent.interrupt()
                    self._cancel_active_plan_execution(reason="shutdown")
                except Exception:
                    pass
            ...
            if hasattr(self, '_session_db') and self._session_db and self.agent:
                ...
                # drop the empty row so /resume and `hairball sessions list`
...
def _run_kanban_goal_loop_q(cli: "HairballCLI", first_response: str) -> None:
    """Drive a kanban goal_mode worker through the Ralph-style goal loop.

    Called from the quiet single-query path AFTER the worker's first turn,
    only when ``HAIRBALL_KANBAN_GOAL_MODE`` is set (dispatcher-spawned
    ...
    task_id = (_os.environ.get("HAIRBALL_KANBAN_TASK") or "").strip()
    if not task_id:
        return

    from hairball_cli import kanban_db as _kb
    from hairball_cli.goals import run_kanban_goal_loop as _run_loop, DEFAULT_MAX_TURNS as _DEF_TURNS
...
def main(
    ...
    """
    Hairball Agent CLI - Interactive AI Assistant
    ...
    os.environ["HAIRBALL_INTERACTIVE"] = "1"
    
    # Handle gateway mode (messaging + cron)
    if gateway:
        import asyncio
        from gateway.run import start_gateway
        print("Starting Hairball Gateway (messaging platforms)...")
    ...
    # Default to hairball-cli toolset which includes cronjob management tools
    toolsets_list = None
    if toolsets:
        ...
        # Coding posture (base Hairball): with no explicit --toolsets, collapse
        ...
        if _coding is not None:
            ...
            # W0: under coding auto, append edge toolsets (lsp) without
            # collapsing the whole platform list to the coding posture set.
            try:
                from agent.coding_context import merge_coding_edge_toolsets
                toolsets_list = merge_coding_edge_toolsets(
                    toolsets_list, platform="cli", config=CLI_CONFIG
                )
            except Exception:
                pass
        ...
        if missing_skills:
            ...
            if loaded_skills:
                logger.warning(
                    ...
                    "Continuing with: %s. "
                    "List available skills with `hairball skills list`.",
                    missing_display,
                    ", ".join(loaded_skills),
                )
            else:
    ...
    # mode registers its own inside HairballCLI.run(), but `-q` runs
    ...
    # so main unwinds normally.  HAIRBALL_SIGTERM_GRACE overrides the 1.5 s
    ...
    def _signal_handler_q(signum, frame):
        ...
        if os.environ.get("HAIRBALL_KANBAN_TASK"):
            try:
    ...
    if query or image:
        ...
        try:
            ...
            # Kanban workers spawn with ``hairball chat -q "work kanban task <id>"``;
            ...
            _kanban_task_id = os.environ.get("HAIRBALL_KANBAN_TASK", "").strip()
            if _kanban_task_id:
                try:
                    from hairball_cli import kanban_db as _kb
            ...
            if quiet:
                ...
                if cli._ensure_runtime_credentials():
                    ...
                    if cli._init_agent(
                        ...
                        # machine-readable (no styled "Hairball" box, no tool-gen
                        ...
                        if os.environ.get("HAIRBALL_KANBAN_GOAL_MODE") == "1":
                            try:
                        ...
                        if isinstance(result, dict) and result.get("failed"):
                            _exit_code = 1
                            if os.environ.get("HAIRBALL_KANBAN_TASK") and result.get(
                                "failure_reason"
                            ) in ("rate_limit", "billing"):
                                try:
                                    from hairball_cli.kanban_db import (
                ...
                # Single-query mode (`hairball chat -q "…"`): skip the welcome
