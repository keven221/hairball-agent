...
- ``set_secret_scope(mapping)`` installs the active profile's secrets for the
  ...
  via ``copy_context()`` exactly like the HAIRBALL_HOME override).
...
_GLOBAL_ENV_EXACT = frozenset({
    # Hairball runtime / deployment
    "HAIRBALL_HOME", "HAIRBALL_PROFILE", "HAIRBALL_GATEWAY_LOCK_DIR",
    "HAIRBALL_MAX_ITERATIONS", "HAIRBALL_MAX_TOKENS", "HAIRBALL_API_TIMEOUT",
    "HAIRBALL_REDACT_SECRETS", "HAIRBALL_MANAGED_TIMEOUT_SECONDS",
    "_HAIRBALL_GATEWAY",
    # OS / interpreter
    "PATH", "HOME", "USER", "LANG", "LC_ALL", "TZ", "PWD", "SHELL", "TMPDIR",
    "VIRTUAL_ENV", "PYTHONPATH", "SSL_CERT_FILE",
    # Kanban paths (per-board, not per-profile-secret)
    "HAIRBALL_KANBAN_DB", "HAIRBALL_KANBAN_WORKSPACES_ROOT", "HAIRBALL_KANBAN_BOARD",
})
_GLOBAL_ENV_PREFIXES = (
    "HAIRBALL_KANBAN_",
    "HAIRBALL_TELEGRAM_",   # tuning knobs (batch delays, fallback toggles) — NOT the token
...
def build_profile_secret_scope(hairball_home: Path) -> Dict[str, str]:
    ...
    """
    return load_env_file(Path(hairball_home) / ".env")
