...
* Python plugins and shell hooks compose naturally: both flow through
  :func:`hairball_cli.plugins.invoke_hook` and its aggregators.  Python
...
* First-use consent is gated by the allowlist under
  ``~/.hairball/shell-hooks-allowlist.json``.  Non-TTY callers must pass
  ``accept_hooks=True`` (resolved from ``--accept-hooks``,
  ``HAIRBALL_ACCEPT_HOOKS``, or ``hooks_auto_accept: true`` in config)
  for registration to succeed without a prompt.
* Registration is idempotent — safe to invoke from both the CLI entry
  point (``hairball_cli/main.py``) and the gateway entry point
  (``gateway/run.py``).
...
**stdout** (JSON, optional — anything else is ignored)::
    ...
    {"decision": "block", "reason":  "Forbidden command"}   # accepted adapter shape
    {"action":   "block", "message": "Forbidden command"}   # Hairball-canonical
...
``subagent_complete`` (emitted from ``tools/delegate_tool.py``)::
...
def register_from_config(
    ...
    *,
    ...
    ``cfg`` is the full parsed config dict (``hairball_cli.config.load_config``
    ...
    setting.  ``HAIRBALL_ACCEPT_HOOKS=1`` and ``hooks_auto_accept: true`` are
    ...
    # Safe mode (--safe-mode / HAIRBALL_SAFE_MODE=1): shell hooks are user
    ...
    if env_var_enabled("HAIRBALL_SAFE_MODE"):
        logger.info("HAIRBALL_SAFE_MODE=1 — shell-hook registration skipped")
        return []
    ...
    for spec in specs:
        ...
        if not already_allowlisted:
            if not _prompt_and_record(
                ...
                logger.warning(
                    ...
                    "Use --accept-hooks / HAIRBALL_ACCEPT_HOOKS=1 / "
        ...
        with _registered_lock:
            if key in _registered:
                continue
            # Shell hooks already enforce their own subprocess timeout and
            # consent policy inside ``_make_callback``/``_spawn``. Register
            # through the same host-owned hook seam for deterministic order
            # and payload isolation, without stacking a second timeout.
            manager._register_hook(
                spec.event,
                _make_callback(spec),
                plugin_id=f"shell:{spec.command}",
            )
            _registered.add(key)
...
def iter_configured_hooks(cfg: Optional[Dict[str, Any]]) -> List[ShellHookSpec]:
    ...
    registering anything.  Used by ``hairball hooks list`` and ``doctor``."""
    if not isinstance(cfg, dict):
        return []
...
def _make_callback(spec: ShellHookSpec) -> Callable[..., Optional[Dict[str, Any]]]:
    ...
    def _callback(**kwargs: Any) -> Optional[Dict[str, Any]]:
        ...
        # faithful_port: Stop handlers use exit code 2 plus non-empty
        # stderr as their non-JSON continuation protocol.  Scope this adapter
        # to Stop only; every other event keeps the existing stdout contract.
        if spec.event in {"stop", "subagent_stop"} and r["returncode"] == 2:
            if not stderr:
                return None
            return {"decision": "block", "reason": stderr}
...
def _serialize_payload(event: str, kwargs: Dict[str, Any]) -> str:
    ...
    if event in {"stop", "subagent_stop"}:
        # faithful_port: match the external ``StopCommandInput`` and
        # ``SubagentStopCommandInput`` at the shell
        # boundary.  Keep the generic ``extra`` object too so existing
        # Hairball hook scripts do not lose their former compatibility view.
        payload.update(
            {
                "hook_event_name": (
                    "SubagentStop" if event == "subagent_stop" else "Stop"
                ),
                "turn_id": str(kwargs.get("turn_id") or ""),
                "transcript_path": kwargs.get("transcript_path"),
                "cwd": str(kwargs.get("cwd") or payload["cwd"]),
                "model": str(kwargs.get("model") or ""),
                "permission_mode": str(kwargs.get("permission_mode") or ""),
                "stop_hook_active": bool(kwargs.get("stop_hook_active", False)),
                "last_assistant_message": kwargs.get("last_assistant_message"),
            }
        )
            ...
            payload.update(
                {
                    ...
                    "agent_transcript_path": kwargs.get("agent_transcript_path"),
                }
            )
...
def _parse_response(event: str, stdout: str) -> Optional[Dict[str, Any]]:
    """Translate stdout JSON into a Hairball wire-shape dict.

    For ``pre_tool_call`` the accepted ``{"decision": "block",
    "reason": "..."}`` payload is translated into the canonical Hairball
    ``{"action": "block", "message": "..."}`` shape expected by
    :func:`hairball_cli.plugins.get_pre_tool_call_block_message`.  This is
    ...
    if event in {"pre_verify", "stop", "subagent_stop"}:
        # "continue" / "block" (block the stop) both
        ...
        if event in {"stop", "subagent_stop"} and data.get("continue") is False:
            reason = data.get("stopReason") or data.get("stop_reason")
            return {
                "continue": False,
                "stopReason": str(reason).strip() if reason else None,
            }
        ...
        if action in {"continue", "block"}:
            ...
            if isinstance(message, str) and message.strip():
                if event in {"stop", "subagent_stop"}:
                    return {"decision": "block", "reason": message.strip()}
...
def allowlist_path() -> Path:
    ...
    return get_hairball_home() / ALLOWLIST_FILENAME
...
def save_allowlist(data: Dict[str, Any]) -> None:
    ...
    try:
        ...
        logger.warning(
            ...
            "runs without --accept-hooks / HAIRBALL_ACCEPT_HOOKS).",
            p, exc,
        )
...
def _locked_update_approvals() -> Iterator[Dict[str, Any]]:
    ...
    cannot clobber each other's changes (the race reference reproduced with
...
def _prompt_and_record(
    ...
    print(
        f"\n⚠ Hairball is about to register a shell hook that will run a\n"
...
def _resolve_effective_accept(
    ...
    Precedence (any truthy source flips us on):
      ...
      2. ``HAIRBALL_ACCEPT_HOOKS`` env var
      3. ``hooks_auto_accept: true`` in ``cli-config.yaml``
    """
    if accept_hooks_arg:
        return True
    env = os.environ.get("HAIRBALL_ACCEPT_HOOKS", "").strip().lower()
...
# ---------------------------------------------------------------------------
# Introspection (used by `hairball hooks` CLI)
# ---------------------------------------------------------------------------
...
def run_once(
    ...
    Used by ``hairball hooks test`` and ``hairball hooks doctor``.

    ``kwargs`` is the same dict that :func:`hairball_cli.plugins.invoke_hook`
    ...
    produce — otherwise scripts tested via ``hairball hooks test`` could
    diverge silently from production behaviour.

    Returns the :func:`_spawn` diagnostic dict plus a ``parsed`` field
    holding the canonical Hairball-wire-shape response."""
