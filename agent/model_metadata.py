...
def _resolve_requests_verify() -> bool | str:
    ...
    by default. Hairball also honours HAIRBALL_CA_BUNDLE (its own convention)
    ...
    """
    for env_var in ("HAIRBALL_CA_BUNDLE", "REQUESTS_CA_BUNDLE", "SSL_CERT_FILE"):
...
def _get_model_metadata_cache_path() -> Path:
    ...
    return get_hairball_home() / "cache" / "openrouter_model_metadata.json"
...
# Minimum context length required to run Hairball Agent.  Models with fewer
...
DEFAULT_CONTEXT_LENGTHS = {
    ...
    # ChatGPT OAuth caps it at 272K; both paths resolve via their own
    # provider-aware branches (_resolve_codex_oauth_context_length + models.dev).
    # This hardcoded value is only reached when every probe misses.
    # GPT-5.6 series (Sol/Terra/Luna, GA 2026-07-09) — 1.05M on the direct
    # OpenAI API (same as gpt-5.5). reference OAuth caps these at 272K.
    ...
    # gpt-5.3-codex-spark is reference-OAuth-only (ChatGPT Pro entitlement) and
    ...
    # (native MiniMax-M3, OpenRouter/managed minimax/minimax-m3).
    ...
    # so these hardcoded fallbacks prevent Hairball from probing-down to
    ...
    "XiaomiMiMo/provider-V2-Flash": 262144,
    "mimo-v2-pro": 1048576,
...
_URL_TO_PROVIDER: Dict[str, str] = {
    ...
    "inference.example.invalid": "managed",
    ...
    # (often 8K) makes it unusable for Hairball' system prompt, but mapping
...
def _maybe_cache_local_context_length(
    ...
    """Persist a locally probed context length only when it meets Hairball minimum.
...
def _get_context_cache_path() -> Path:
    ...
    from hairball_constants import get_hairball_home
    return get_hairball_home() / "context_length_cache.yaml"
...
def parse_available_output_tokens_from_error(error_msg: str) -> Optional[int]:
    ...
    is_output_cap_error = (
        ...
        # OpenRouter/managed phrasing of the same condition.
    ...
    # OpenRouter/managed format: "maximum context length is N … (A of text input,
...
def is_output_cap_error(error_msg: str) -> bool:
    ...
    output_cap_signal = (
        ...
        or ("in the output" in error_lower                  # OpenRouter / managed
            and "maximum context length" in error_lower)
...
def _model_name_suggests_minimax_m3(model: str) -> bool:
    ...
    across surfaces (native MiniMax-M3, OpenRouter/managed minimax/minimax-m3).
        ...
        with httpx.Client(timeout=3.0, headers=headers) as client:
            ...
            if server_type == "ollama":
                ...
                if resp.status_code == 200:
                    ...
                    # Hairball grow conversations past the runtime limit and Ollama
...
def _normalize_model_version(model: str) -> str:
    ...
    managed uses dashes: claude-opus-4-6, claude-sonnet-4-5
...
# Known ChatGPT OAuth context windows (observed via live
# chatgpt.com/backend-api/codex/models probe, Apr 2026). These are the
# `context_window` values, which are what reference actually enforces — the
# direct OpenAI API has larger limits for the same slugs, but reference OAuth
# caps lower (e.g. gpt-5.5 is 1.05M on the API, 272K on reference).
#
...
_CODEX_OAUTH_CONTEXT_FALLBACK: Dict[str, int] = {
    ...
    # 128k window than other reference OAuth slugs. Listed explicitly so the
    ...
    # gated by ChatGPT Pro entitlement on the reference backend.
...
def _fetch_codex_oauth_context_lengths(access_token: str) -> Dict[str, int]:
    """Probe the ChatGPT /models endpoint for per-slug context windows.

    reference OAuth imposes its own context limits that differ from the direct
    OpenAI API (e.g. gpt-5.5 is 1.05M on the API, 272K on reference). The
    ...
    try:
        ...
        if resp.status_code != 200:
            logger.debug(
                "reference /models probe returned HTTP %s; falling back to hardcoded defaults",
                resp.status_code,
            )
            return {}
        data = resp.json()
    except Exception as exc:
        logger.debug("reference /models probe failed: %s", exc)
        return {}
...
def _resolve_codex_oauth_context_length(
    ...
    """Resolve a reference OAuth model's real context window.
...
def _resolve_managed_context_length(
    ...
    """Resolve Managed Portal model context length.

    Tries the live managed inference endpoint first (authoritative), then falls
    back to OpenRouter metadata with suffix/version matching.

    managed model IDs are bare after prefix-stripping (e.g. 'qwen3.6-plus',
    ...
    Returns ``(context_length, source)`` where ``source`` is one of:
      ...
      - ``""``           — could not resolve
    """
    # Portal first — the managed /models endpoint is authoritative for what our
    ...
    def _safe_ctx(or_id: str, entry: dict) -> Optional[int]:
        ...
        if ctx <= 32768 and _model_name_suggests_kimi(or_id):
            logger.info(
                ...
                "(Kimi-family underreport, managed path); falling through to hardcoded defaults",
                ctx, or_id,
            )
            return None
        return ctx
...
def get_model_context_length(
    ...
    1. Persistent cache (previously discovered via probing).  managed URLs
    ...
    if base_url and not _skip_persistent_context_cache(base_url, provider):
        ...
        if cached is not None:
            # Invalidate stale reference OAuth cache entries: pre-PR #14935 builds
            # resolved gpt-5.x to the direct-API value (e.g. 1.05M) via
            # models.dev and persisted it. reference OAuth caps at 272K for every
            # slug, so any cached reference entry at or above 400K is a leftover
            # from the old resolution path. Drop it and fall through to the
            # live /models probe in step 5 below.
            if provider == "openai-codex" and cached >= 400_000:
                logger.info(
                    "Dropping stale reference cache entry %s@%s -> %s (pre-fix value); "
            ...
            # leftover — drop it and fall through to the hardcoded default.
                ...
                _invalidate_cached_context_length(model, base_url)
            # Managed Portal: the portal /v1/models endpoint is authoritative.
            ...
            # cost amortise to ~0 within a process.
                ...
                logger.debug(
                    "Bypassing persistent cache for %s@%s (Managed Portal authoritative)",
                    model, base_url,
                )
    ...
    if effective_provider in {"copilot", "copilot-acp", "github-copilot"}:
        try:
            from hairball_cli.models import get_copilot_model_context
    ...
    if effective_provider == "managed":
        ...
        if ctx:
            ...
            # want it leaking into the persistent cache for managed URLs.
            if base_url and source == "portal":
                save_context_length(model, base_url, ctx)
            return ctx
    if effective_provider == "openai-codex":
        # reference OAuth enforces lower context limits than the direct OpenAI
        # API for the same slug (e.g. gpt-5.5 is 1.05M on the API but 272K
        # on reference). Authoritative source is reference's own /models endpoint.
    ...
    # the dedicated managed/Copilot/GMI branches above.
    ...
    # ``Hairball-3-Llama-3.1-70B`` substring-match ``llama`` (131072) even when
...
def estimate_request_tokens_rough(
    ...
    *,
    ...
    Includes the major payload buckets Hairball sends to providers:
