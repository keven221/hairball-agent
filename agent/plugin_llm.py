...
Plugins built on Hairball Agent often need to make their own LLM calls
...
Today the only stable plugin surfaces extend an existing Hairball
...
:class:`~hairball_cli.plugins.PluginContext`:
...
embedded slugs, no shorthands. This mirrors Hairball' main config
...
handles every provider, fallback chain, and per-task override Hairball
supports.
"""
...
def _build_structured_messages(
    *,
    ...
    if json_schema is not None and include_schema_in_prompt:
        try:
...
class PluginLlm:
    ...
    Instances are constructed by :class:`hairball_cli.plugins.PluginContext`
    ...
    def complete_structured(
        ...
        *,
        ...
        ``include_schema_in_prompt=False`` keeps the schema exclusively in
        the provider's structured-output request contract. The default stays
        ``True`` for compatibility with existing plugins and transports.
