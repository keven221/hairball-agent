"""reference Responses API adapter.

Pure format-conversion and normalization logic for the OpenAI Responses API
(used by ChatGPT, xAI, GitHub Models, and other Responses-compatible endpoints).
...
def _classify_responses_issuer(
    *,
    ...
    replaying a reference-minted blob against xAI (or vice versa) deterministically
...
# Matches reference/Harmony tool-call serialization that occasionally leaks into
...
# output items.  Hairball injects xAI's native ``web_search`` for the xAI
...
# HTTP 400 ("string too long"). reference-issued assistant message ids are
# server-assigned base64 blobs that can run 400+ chars, while Hairball-minted
...
def _normalize_responses_message_status(value: Any, *, default: str = "completed") -> str:
    ...
    incomplete reference continuation turns don't get falsely marked completed.
    """
...
def _chat_messages_to_responses_input(
    ...
    *,
    ...
    explicitly relies on Hairball threading encrypted reasoning back across
    ...
    transport (xAI, native reference, custom relays) and let xAI tell us
    ...
    endpoint that minted it — replaying a reference-issued blob against xAI
                ...
                if isinstance(codex_reasoning, list):
                    for ri in codex_reasoning:
                        if isinstance(ri, dict) and ri.get("encrypted_content"):
                            ...
                            # it is a Hairball-side metadata key and not part
...
def _preflight_codex_input_items(
    ...
    *,
    ...
    if not isinstance(raw_items, list):
        raise ValueError("reference Responses input must be a list of input items.")

    normalized: List[Dict[str, Any]] = []
    seen_ids: set = set()
    for idx, item in enumerate(raw_items):
        if not isinstance(item, dict):
            raise ValueError(f"reference Responses input[{idx}] must be an object.")
        ...
        if item_type == "function_call":
            ...
            name = item.get("name")
            if not isinstance(call_id, str) or not call_id.strip():
                raise ValueError(f"reference Responses input[{idx}] function_call is missing call_id.")
            if not isinstance(name, str) or not name.strip():
                raise ValueError(f"reference Responses input[{idx}] function_call is missing name.")
        ...
        if item_type == "function_call_output":
            call_id = item.get("call_id")
            if not isinstance(call_id, str) or not call_id.strip():
                raise ValueError(f"reference Responses input[{idx}] function_call_output is missing call_id.")
        ...
        if item_type == "message":
            role = item.get("role")
            if role != "assistant":
                raise ValueError(f"reference Responses input[{idx}] message items must have role='assistant'.")
            content = item.get("content")
            if not isinstance(content, list):
                raise ValueError(f"reference Responses input[{idx}] message item must have content list.")
            normalized_content = []
            for part_idx, part in enumerate(content):
                if not isinstance(part, dict):
                    raise ValueError(
                        f"reference Responses input[{idx}] message content[{part_idx}] must be an object."
                    )
                part_type = part.get("type")
                if part_type not in {"output_text", "text"}:
                    raise ValueError(
                        f"reference Responses input[{idx}] message content[{part_idx}] has unsupported type {part_type!r}."
                    )
                text = part.get("text", "")
                if text is None:
                    text = ""
            ...
            if not normalized_content:
                raise ValueError(f"reference Responses input[{idx}] message item must contain at least one text part.")
        ...
        if role in {"user", "assistant"}:
            ...
            if isinstance(content, list):
                ...
                for part_idx, part in enumerate(content):
                    ...
                    if not isinstance(part, dict):
                        raise ValueError(
                            f"reference Responses input[{idx}].content[{part_idx}] must be an object or string."
                        )
                    ptype = str(part.get("type") or "").strip().lower()
                    if ptype in {"input_text", "text", "output_text"}:
                        ...
                        raise ValueError(
                            f"reference Responses input[{idx}].content[{part_idx}] has unsupported type {part.get('type')!r}."
                        )
        ...
        raise ValueError(
            f"reference Responses input[{idx}] has unsupported item shape (type={item_type!r}, role={role!r})."
        )
...
def _preflight_codex_api_kwargs(
    ...
    *,
    ...
    if not isinstance(api_kwargs, dict):
        raise ValueError("reference Responses request must be a dict.")

    required = {"model", "instructions", "input"}
    missing = [key for key in required if key not in api_kwargs]
    if missing:
        raise ValueError(f"reference Responses request missing required field(s): {', '.join(sorted(missing))}.")

    model = api_kwargs.get("model")
    if not isinstance(model, str) or not model.strip():
        raise ValueError("reference Responses request 'model' must be a non-empty string.")
    model = model.strip()
    ...
    if tools is not None:
        if not isinstance(tools, list):
            raise ValueError("reference Responses request 'tools' must be a list when provided.")
        normalized_tools = []
        for idx, tool in enumerate(tools):
            if not isinstance(tool, dict):
                raise ValueError(f"reference Responses tools[{idx}] must be an object.")
            ...
            if tool_type != "function":
                raise ValueError(f"reference Responses tools[{idx}] has unsupported type {tool.get('type')!r}.")

            name = tool.get("name")
            parameters = tool.get("parameters")
            if not isinstance(name, str) or not name.strip():
                raise ValueError(f"reference Responses tools[{idx}] is missing a valid name.")
            if not isinstance(parameters, dict):
                raise ValueError(f"reference Responses tools[{idx}] is missing valid parameters.")
    ...
    if store is not False:
        raise ValueError("reference Responses contract requires 'store' to be false.")
    ...
    if extra_headers is not None:
        if not isinstance(extra_headers, dict):
            raise ValueError("reference Responses request 'extra_headers' must be an object.")
        normalized_headers: Dict[str, str] = {}
        for key, value in extra_headers.items():
            if not isinstance(key, str) or not key.strip():
                raise ValueError("reference Responses request 'extra_headers' keys must be non-empty strings.")
            if value is None:
                continue
    ...
    if extra_body is not None:
        if not isinstance(extra_body, dict):
            raise ValueError("reference Responses request 'extra_body' must be an object.")
    ...
    if allow_stream:
        ...
        if stream is not None and stream is not True:
            raise ValueError("reference Responses 'stream' must be true when set.")
        if stream is True:
            normalized["stream"] = True
        allowed_keys.add("stream")
    elif "stream" in api_kwargs:
        raise ValueError("reference Responses stream flag is only allowed in fallback streaming requests.")
    ...
    #
    # Gated on the model name pattern because native reference (OpenAI) DOES
    ...
    if unexpected:
        raise ValueError(
            f"reference Responses request has unsupported field(s): {', '.join(unexpected)}."
        )
...
def _normalize_codex_response(
    ...
    *,
    ...
    if not isinstance(output, list) or not output:
        # The reference backend can return empty output when the answer was
        # delivered entirely via stream events. Check output_text as a
        # last-resort fallback before raising.
        out_text = getattr(response, "output_text", None)
        if isinstance(out_text, str) and out_text.strip():
            logger.debug(
                "reference response has empty output but output_text is present (%d chars); "
    ...
    # retries before failing with "reference response remained incomplete
    ...
    for item in output:
        ...
        if item_type == "message":
            ...
            if message_text:
                ...
                # (reference CLI excludes it from last-message extraction; issues
            ...
            if isinstance(encrypted, str) and encrypted:
                ...
                if isinstance(item_id, str) and item_id.startswith("rs_tmp_"):
                    logger.debug(
                        "Skipping transient reference reasoning item during normalization: %s",
                        item_id,
                    )
                    continue
                if isinstance(item_id, str) and item_id:
    ...
    # gpt-5.x on the reference Responses API sometimes degenerates and emits
    # what should be a structured `function_call` item as plain assistant
    # text using the Harmony/reference serialization (``to=functions.foo
    ...
    # so the existing reference-incomplete continuation path (3 retries,
    ...
    if final_text and not tool_calls and _TOOL_CALL_LEAK_PATTERN.search(final_text):
        ...
        logger.warning(
            "reference response contains leaked tool-call text in assistant content "
    ...
    if tool_calls:
        ...
        # the reference continuation path handles it correctly.
        finish_reason = "incomplete"
    else:
        finish_reason = "stop"
