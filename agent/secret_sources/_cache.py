...
cache problem must never block Hairball startup.
"""
...
def resolve_cache_home(home_path: Optional[Path] = None) -> Path:
    """Resolve the Hairball home used for cache paths.

    ``home_path`` is whatever ``load_hairball_dotenv()`` already resolved;
    falling back to ``$HAIRBALL_HOME`` / ``~/.hairball`` keeps direct callers
    (and tests that don't thread a home through) working.
    """
    if home_path is None:
        home_path = Path(os.getenv("HAIRBALL_HOME", Path.home() / ".hairball"))
    return home_path
...
class DiskCache(Generic[K]):
    ...
    One JSON object per backend lives at ``<hairball_home>/cache/<basename>``::
    ...
    def write(
        ...
        try:
            ...
            # metadata isn't exposed if HAIRBALL_HOME is ever made traversable.
            try:
