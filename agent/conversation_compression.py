...
def _lock_api_is_absent_on_session_db(lock_db: Any) -> bool:
    ...
    imported ``hairball_state.SessionDB`` class (and its live instances) is old.
    ...
    try:
        ...
        if client is None or not aux_model:
            if _aux_cfg_provider and _aux_cfg_provider != "auto":
                ...
                msg = (
                    ...
                    "compression will drop middle turns without a summary. "
                    "Run `hairball setup` or set OPENROUTER_API_KEY."
                )
        ...
        if aux_context and aux_context < MINIMUM_CONTEXT_LENGTH:
            raise ValueError(
                ...
                f"minimum {MINIMUM_CONTEXT_LENGTH:,} required by Hairball "
...
def _invoke_compaction_hook(
    ...
    *,
    ...
    pre_approx_tokens: Optional[int] = None,
    post_approx_tokens: Optional[int] = None,
    force: bool = False,
    ...
    """Emit one compact lifecycle hook without copying the transcript.

    The compression path already owns message/session mutation. Plugins receive
    immutable-style scalar metadata so an observer cannot rewrite the live
    transcript or invalidate the prompt cache by mutating hook arguments.
    """
    ...
    invoke_hook(
        ...
        pre_approx_tokens=pre_approx_tokens,
        post_approx_tokens=post_approx_tokens,
        force=force,
...
def compress_context(
    ...
    *,
    ...
    Args:
        ...
        task_id: Tool task scope retained across the compression boundary.
    ...
    """
    # reference app-server sessions: the codex agent owns the real thread context;
    # Hairball' summarizer would only rewrite a local mirror without shrinking
    # the actual thread (#36801). Route compaction to the app server's own
    # thread/compact mechanism. Behavior is controlled by
    # ``compression.codex_app_server_auto`` (native|hairball|off).
    ...
    # This is deliberately after the session-lineage lock is acquired. A
    # skipped contender must not appear as a completed compaction, and the
    # event must remain side-band to the lock/session mutation sequence.
    from agent.kernel.native_events import NativeCompactionEvents

    _native_compaction = NativeCompactionEvents.open(agent)
    if _native_compaction is not None:
        _native_compaction.started(
    ...
    try:
        _invoke_compaction_hook(
            "pre_compact",
            agent,
            runtime="hairball",
            pre_message_count=_pre_msg_count,
            pre_approx_tokens=approx_tokens,
            force=force,
            focus_topic=focus_topic,
            in_place=in_place,
        )
        ...
        if _native_compaction is not None:
            _native_compaction.skipped("pre_compact_hook_error")
        _release_lock()
        raise
    ...
    try:
        ...
        try:
            ...
            if _native_compaction is not None:
                _native_compaction.skipped("compressor_error")
            _release_lock()
            _invoke_compaction_hook(
                "post_compact",
                agent,
    ...
    if getattr(agent.context_compressor, "_last_compress_aborted", False):
        try:
            ...
            if not _existing_sp:
                _existing_sp = agent._build_system_prompt(system_message)
            if _native_compaction is not None:
                _native_compaction.skipped("summary_aborted")
            _invoke_compaction_hook(
                "post_compact",
                agent,
                runtime="hairball",
    ...
    if compressed is messages:
        ...
        if not _existing_sp:
            _existing_sp = agent._build_system_prompt(system_message)
        if _native_compaction is not None:
            _native_compaction.skipped("no_structural_progress")
        try:
            _invoke_compaction_hook(
                "post_compact",
                agent,
    ...
    # Optional vision-frame rail (compression.vision_frames). Best-effort —
    # only after a successful structural compress. Default config leaves this
    # off. Inject OpenAI-style image_url parts onto the first non-system turn.
    try:
        from agent.snapcompact import (
        ...
        if is_available():
            ...
            for msg in compressed or []:
                ...
                if isinstance(content, str) and content.strip():
                    summary_bits.append(content[:4000])
                elif isinstance(content, list):
                    for part in content:
                        if isinstance(part, dict) and part.get("type") == "text":
                            t = str(part.get("text") or "").strip()
                            if t:
                                summary_bits.append(t[:4000])
                if len(summary_bits) >= 3:
                    break
            blob = "\n\n".join(summary_bits)
            attached = maybe_attach_vision_frames(
                blob,
                model_id=getattr(agent, "model", None) or "",
                api=getattr(agent, "api_mode", None) or "",
                provider=getattr(agent, "provider", None) or "",
                session_id=getattr(agent, "session_id", None) or "",
            )
            agent._last_vision_compaction = attached
            frames = attached.get("frames") if isinstance(attached, dict) else None
            if isinstance(frames, list) and frames:
                detail = None
                shape_meta = attached.get("shape") if isinstance(attached, dict) else None
                if isinstance(shape_meta, dict):
                    detail = shape_meta.get("image_detail")
                compressed = inject_vision_frames_into_messages(
                    compressed, frames, image_detail=detail
                )
                # Keep metadata only — raw PNG bytes are already base64'd into
                # message image_url parts; retaining them breaks JSON APIs.
                meta = {k: v for k, v in attached.items() if k != "frames"}
                meta["injected"] = True
                meta["injected_count"] = len(frames)
                meta["frame_count"] = len(frames)
                agent._last_vision_compaction = meta
        else:
            agent._last_vision_compaction = {
                "enabled": False,
                "frames": [],
                "reason": "disabled",
            }
    except Exception as _vf_err:  # noqa: BLE001
        logger.debug("vision_frames attach skipped: %s", _vf_err)

    _post_compact_emitted = False
    try:
        ...
        if agent._session_db:
            try:
                ...
                if in_place:
                    ...
                    # from ``hairball_logging._session_context`` (set once per turn in
                    ...
                    try:
                        agent._session_db.create_session(
                            ...
                            source=agent.platform or os.environ.get("HAIRBALL_SESSION_SOURCE", "cli"),
                            model=agent.model,
                            model_config=agent._session_init_model_config,
                            parent_session_id=old_session_id,
                        )
                        # A rotation changes only the transcript storage id, not
                        # the logical conversation.  Move the exact canonical
                        # Plan snapshot before any caller can observe the child;
                        # if this fails, use the existing orphan-avoidance
                        # rollback below rather than stranding an approved or
                        # running Plan on the ended parent.
                        from agent.plan_commands import migrate_plan_to_session
                        ...
                        logger.warning(
                            "Compression child session create/migration failed (%s) — "
                            "rolling back to parent session %s to avoid an orphan.",
                            _cs_err, old_session_id,
                        )
                        failed_child_id = agent.session_id
                        try:
                            agent._session_db.delete_session(failed_child_id)
                        except Exception:
                            logger.debug(
                                "Could not remove failed compression child %s",
                                failed_child_id,
                                exc_info=True,
                            )
                    ...
                    try:
                        from hairball_cli.goals import migrate_goal_to_session
        ...
        # engines (e.g. hairball-lcm) use boundary_reason="compression" to preserve
        # DAG lineage / checkpoint per-session state across the boundary instead of
        # re-initializing fresh. See hairball-lcm#68. Built-in ContextCompressor
        ...
        if _native_compaction is not None:
            _native_compaction.completed(
                pre_message_count=_pre_msg_count,
                post_message_count=len(compressed),
                pre_approx_tokens=approx_tokens,
                post_approx_tokens=_compressed_est,
                in_place=in_place,
                old_session_id=_old_sid or _native_compaction.session_id,
        ...
        # and all post-rotation bookkeeping (memory manager and context engine)
        # ran. A concurrent path that wakes up the moment we
...
def _compress_context_via_codex_app_server(
    ...
    *,
    ...
    """Route compaction to reference app-server for reference-owned threads.

    Hairball' normal compressor rewrites the local OpenAI-style transcript.
    That does not shrink the actual reference app-server thread context. For this
    runtime, ask reference to compact its own thread and keep Hairball' transcript
    unchanged.
    """
    auto_mode = str(
        getattr(agent, "codex_app_server_auto_compaction", "native") or "native"
    ).lower()
    if auto_mode not in {"native", "hairball", "off"}:
        auto_mode = "native"
    if not force and auto_mode != "hairball":
        logger.info(
    ...
    try:
        ...
        if _native_compaction is not None:
            _native_compaction.skipped("app_server_error")
        _invoke_compaction_hook(
            "post_compact",
            agent,
            runtime="codex_app_server",
            ...
            post_message_count=len(messages),
            pre_approx_tokens=approx_tokens,
            post_approx_tokens=approx_tokens,
            force=force,
            in_place=True,
            outcome="error",
            reason=type(exc).__name__,
    ...
    if getattr(result, "interrupted", False) or getattr(result, "error", None):
        try:
            agent._emit_warning(
                f"⚠ reference app-server compaction failed: {result.error}"
            )
        except Exception:
            pass
        existing_prompt = getattr(agent, "_cached_system_prompt", None)
        if not existing_prompt:
            existing_prompt = agent._build_system_prompt(system_message)
        if _native_compaction is not None:
            _native_compaction.skipped("app_server_rejected")
        _invoke_compaction_hook(
            "post_compact",
            agent,
            runtime="codex_app_server",
    ...
    try:
        ...
        # reference turn supplies usage. Minimal external test engines may not expose
    ...
    if _native_compaction is not None:
        _native_compaction.completed(
            ...
            post_approx_tokens=approx_tokens,
            in_place=True,
            old_session_id=_native_compaction.session_id,
            new_session_id=str(getattr(agent, "session_id", "") or ""),
            compression_count=getattr(agent.context_compressor, "compression_count", 0),
            made_progress=True,
            used_fallback=False,
    ...
    def _shrink_data_url(url: str) -> tuple:
        ...
        try:
            ...
            tmp = tempfile.NamedTemporaryFile(
                prefix="hairball_shrink_", suffix=suffix, delete=False,
            )
            try:
                tmp.write(raw)
                tmp.close()
