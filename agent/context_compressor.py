...
def _redact_compaction_text(text: Any) -> str:
    """Strict secret redaction at every compaction text boundary.

    ``force=True`` ignores the user's logging redaction preference — compaction
    summaries persist and are re-fed to models. ``redact_url_credentials=True``
    additionally masks credential-named query params and URL userinfo that the
    default pass leaves alone for navigation/tool flows.
    """
...
# heuristics. See https://github.com/managed/hairball-agent/issues/38389
#
...
# Pass-4 pressure demotion (#61932): keep this many trailing messages verbatim
# while reclaiming bulky tool bodies inside an oversized protected tail.
...
# invisible to ``msg["content"]``/``msg["tool_calls"]`` accounting.  reference
...
def _strip_historical_media(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    shape the hairball compressor emits).
    """
    if not messages:
        return messages
...
class ContextCompressor(ContextEngine):
    ...
    # rationale as the gpt-5.5/reference 85% autoraise.
    ...
    def _effective_threshold_percent(
        ...
        e.g. reference gpt-5.5's 85%) always wins; only lower values are raised.
    ...
    def __init__(
        ...
        # REAL user messages guaranteed in the uncompressed tail
        # (compression.min_tail_user_messages). Default 1 = historical
        # single-user anchor; values > 1 extend the guarantee.
        self.min_tail_user_messages = max(1, int(min_tail_user_messages or 1))
    ...
    def should_defer_preflight_to_real_usage(self, rough_tokens: int) -> bool:
        ...
        overestimates schema-heavy requests so Hairball compresses before a
    ...
    def _prune_old_tool_results(
        ...
        # bodies through the ordinary prune passes.
        ...
        # When the protected region itself exceeds the soft budget, demote
        # bulky tool bodies inside it — including just-loaded skills — so
                ...
                if _protected_region_tokens() > soft_ceiling:
                    ...
                    for i in range(max(0, prune_boundary), len(result)):
                        if last_tool_idx is not None and i == last_tool_idx:
                            continue
                        if result[i].get("role") == "tool" and _demote_at(i):
                            pressure_hits += 1
                    if (
...
Summary generation was unavailable, so this is a best-effort deterministic fallback for {len(turns_to_summarize)} compacted message(s).{reason_text}"""
        ...
        # routinely cuts [SKILL_PRUNED: ...] markers out of the compacted turns.
    ...
    def _generate_summary(
        ...
        # P2 ghost-skill defense (#32106): collect ghosted skills BEFORE the
        # aux LLM call; reinject markers the model paraphrased away AFTER.
        ...
        # user's configured timezone via hairball_time.now(). The compaction summary
        ...
        try:
            ...
            _today_str = _hairball_now().strftime("%Y-%m-%d")
...
This compaction should PRIORITISE preserving all information related to the focus topic above. For content related to "{focus_topic}", include full detail — exact values, file paths, command outputs, error messages, and decisions. For content NOT related to the focus topic, summarise more aggressively (brief one-liners or omit if truly irrelevant). The focus topic sections should receive roughly 60-70% of the summary token budget. Even for the focus topic, NEVER preserve API keys, tokens, passwords, or credentials — use [REDACTED]."""
    ...
    @staticmethod
    def _get_tool_call_ids(tc) -> set[str]:
        """Return every wire ID that may identify one tool call.

        Responses-format calls can carry both a response-item ``id`` and a
        function ``call_id``. Different adapters legitimately use either on
        the following tool result, so local pairing must accept both aliases
        without allowing one call to consume two results.
        """
        if isinstance(tc, dict):
    ...
    def _sanitize_tool_pairs(self, messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        Pairing is scoped to the consecutive ``tool`` group immediately after
        each assistant message. A user/assistant boundary closes the group:
        a delayed result after that boundary is not a valid answer to the old
        call. Each call consumes at most one result, so retry/resume duplicates
        ...
        disagree (reference Responses API format: ``id != call_id``), stubs get
        ...
        while index < len(messages):
            ...
            call_aliases = [self._get_tool_call_ids(tc) for tc in tool_calls]
            ...
            while (
                ...
                result = messages[group_end]
                result_id = str(result.get("tool_call_id") or "")
                match_index = next(
                    (
                        call_index
                        for call_index, aliases in enumerate(call_aliases)
                        if call_index not in matched_call_indexes
                        and result_id in aliases
                    ),
                    None,
                )
                if match_index is None:
                    ...
                    matched_call_indexes.add(match_index)
            ...
            kept_calls = [
                ...
                if call_index in matched_call_indexes
            ]
            stripped_calls += len(tool_calls) - len(kept_calls)
        ...
        if not self.quiet_mode and removed_results:
            logger.info(
                "Compression sanitizer: removed %d orphaned/duplicate tool result(s)",
                removed_results,
            )
        if not self.quiet_mode and stripped_calls:
            logger.info(
                "Compression sanitizer: stripped %d unanswered tool_call(s) from assistant messages",
                stripped_calls,
            )
    ...
    def _ensure_last_n_user_messages_in_tail(
        ...
        """Guarantee the last N real user messages are in the protected tail.
        ...
        Only REAL user turns count toward N — compaction handoffs are
        skipped via the same filter as ``_find_last_user_message_idx``.
    ...
    def _find_tail_cut_by_tokens(
        ...
        # (compression.min_tail_user_messages > 1).  Gated at the call site
        # so the default (1) path is byte-identical to the historical
        ...
        # Re-align FORWARD after floor/anchors so a raised cut does not land
        # inside a tool_call/result group (legacy parity).
    ...
    def compress(self, messages: List[Dict[str, Any]], current_tokens: int = None, focus_topic: str = None, force: bool = False, memory_context: str = "") -> List[Dict[str, Any]]:
        ...
        # ``hairball kanban`` worker seeded with a single short
