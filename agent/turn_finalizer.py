...
decomposition campaign (``~/.hairball/plans/god-file-decomposition.md``, Phase 1
...
def finalize_turn(
    ...
    *,
    ...
    if continuation_budget_exhausted:
        ...
        # Console/_emit_status is CLI-visible; UI/gateway watch windows only see
        # tool_progress_callback. Relay the same line so subagent cards stay
        # "running / summarising" instead of looking finished at the tool cap.
        _progress_cb = getattr(agent, "tool_progress_callback", None)
        if callable(_progress_cb):
            try:
                _progress_cb("subagent.progress", preview=_budget_msg)
            except Exception:
                pass
    ...
    if iteration_limit_fallback:
        ...
        _kanban_task = os.environ.get("HAIRBALL_KANBAN_TASK")
        if _kanban_task:
            try:
    ...
    try:
        ...
        # Completion contracts observe the terminal fact at this one shared
        # exit seam.  They do not alter the transcript, response, or control
        # flow: cancellation/budget/provider outcomes are durable evidence for
        # a future resume, never a reason to make another model call.
        try:
            _completion_runtime = getattr(agent, "_completion_contract_runtime", None)
            if _completion_runtime is not None:
                if interrupted:
                    _completion_reason = "cancelled"
                elif failed:
                    _completion_reason = "failed"
                elif budget_exhausted:
                    _completion_reason = "budget_iterations"
                elif str(_turn_exit_reason).startswith("text_response("):
                    _completion_reason = "natural"
                else:
                    _completion_reason = "failed"
                _completion_assessment = _completion_runtime.assess(
                    terminal_reason=_completion_reason,
                    phase="terminal",
                )
        except Exception:
            logger.debug("completion contract terminal assessment skipped", exc_info=True)
    ...
    if final_response and not interrupted:
        try:
            ...
            reconcile_turn_workspace(agent)
    ...
    # providers (Claude thinking, DeepSeek v4, reference Responses) emit
    ...
    # Cost display is derived from separate complete-estimate and
    # provider-reported-actual counters. A partial actual subtotal must never
    # replace the full estimate in downstream UI/API surfaces.
    from agent.usage_pricing import session_cost_summary

    _cost = session_cost_summary(agent)
    ...
    result = {
        ...
        "cost_usd": _cost.display_cost_usd,
        "estimated_cost_usd": _cost.estimated_cost_usd,
        "actual_cost_usd": _cost.actual_cost_usd,
        "cost_actual_call_count": _cost.actual_call_count,
        "cost_total_call_count": _cost.total_call_count,
        "actual_cost_coverage": _cost.actual_coverage,
        "actual_cost_complete": _cost.actual_complete,
        "cost_status": _cost.status,
        "cost_source": _cost.source,
        # Requested service tier (from request_overrides.extra_body), for
        # billing audits by callers like `hairball -z --usage-file`.
        "service_tier": (
    ...
    if _completion_assessment is not None:
        try:
            result["completion_assessment"] = _completion_assessment.to_dict()
        except Exception:
            logger.debug("completion contract result projection skipped", exc_info=True)
