...
from hairball_cli.auth import AuthError, _read_codex_tokens, resolve_codex_runtime_credentials
...
def build_managed_credits_snapshot(account_info) -> Optional[AccountUsageSnapshot]:
    """Map a ManagedPortalAccountInfo into an AccountUsageSnapshot for /usage.
    ...
    """
    try:
        from hairball_cli.managed_account import managed_portal_topup_url
        ...
        details.append(f"Top up: {managed_portal_topup_url(account_info)}")
...
def managed_credits_lines(*, markdown: bool = False, timeout: float = 10.0) -> list[str]:
    """Return rendered managed-credits /usage lines, or [] when there's nothing to show.

    Account-independent of any live agent: gated on "a managed account is logged in"
    ...
    Dev override: when HAIRBALL_DEV_CREDITS_FIXTURE selects a fixture state, /usage
    ...
    try:
        ...
        tok = (get_provider_auth_state("managed") or {}).get("access_token")
    ...
    try:
        ...
        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(
                get_managed_portal_account_info, force_fresh=True
            ).result(timeout=timeout)
        snapshot = build_managed_credits_snapshot(account)
        return render_account_usage_lines(snapshot, markdown=markdown)
    except Exception:
...
def _snapshot_from_credits_state(state) -> Optional[AccountUsageSnapshot]:
    ...
    so HAIRBALL_DEV_CREDITS_FIXTURE can exercise /usage without a live account. The
    *_usd strings are mock display values here (not server balance to compute on);
    ...
    try:
        ...
        details.append("(dev fixture — HAIRBALL_DEV_CREDITS_FIXTURE)")
        return AccountUsageSnapshot(
            provider="managed",
            source="dev-fixture",
            fetched_at=_utc_now(),
...
def build_credits_view(*, markdown: bool = False, timeout: float = 10.0) -> CreditsView:
    ...
    try:
        ...
        from hairball_cli.managed_account import (
            ...
            managed_portal_topup_url,
        )

        with concurrent.futures.ThreadPoolExecutor(max_workers=1) as pool:
            account = pool.submit(get_managed_portal_account_info, force_fresh=True).result(
                timeout=timeout
            )
    except Exception:
    ...
    # ("Top up: <url>" + "(or run /credits)") that build_managed_credits_snapshot
    ...
    return CreditsView(
        ...
        topup_url=managed_portal_topup_url(account),
        depleted=getattr(account, "paid_service_access", None) is False,
    )


def _codex_backend_urls(base_url: str) -> tuple[str, str, str]:
    """Resolve the reference backend endpoints (usage, reset-credits list, consume).

    Mirrors the reference CLI's PathStyle split (codex-rs backend-client): base URLs
...
def _resolve_codex_usage_credentials(
    ...
    """Resolve reference quota credentials from the native runtime path.

    Prefer explicit live-agent credentials, then the legacy singleton OAuth
    state, then the credential pool.  Hairball's native OAuth setup now stores
...
class CodexResetRedeemResult:
    """Outcome of a `/usage reset` attempt against the reference backend."""
...
def redeem_codex_reset_credit(
    *,
    ...
    """Redeem one banked reference rate-limit reset credit (`/usage reset`).

    Flow (mirrors the reference CLI's reset-credits picker, codex-rs
    ``backend-client``):
    ...
    try:
        ...
        return CodexResetRedeemResult(
            status="unavailable",
            message="No reference credentials available. Run `hairball auth` to sign in with your ChatGPT account.",
        )
    ...
    try:
        ...
        if code in (401, 403):
            return CodexResetRedeemResult(
                status="unavailable",
                message=(
                    "reference backend rejected the request (HTTP "
                    f"{code}). Reset credits require ChatGPT-account (OAuth) auth — "
                    "run `hairball auth` and sign in with your ChatGPT account."
                ),
            )
        return CodexResetRedeemResult(
            status="unavailable",
            message=f"reference backend error (HTTP {code}) — try again shortly.",
        )
    except Exception as exc:
        return CodexResetRedeemResult(
            status="unavailable",
            message=f"Could not reach the reference backend: {exc}",
        )
    ...
    return CodexResetRedeemResult(
        status="unavailable",
        message=f"Unexpected response from the reference backend: {body!r}",
    )
