...
from hairball_constants import PARTIAL_STREAM_STUB_ID
from hairball_logging import set_session_context
...
def _apply_active_turn_redirect(agent: Any, messages: List[Dict[str, Any]], text: str) -> None:
    ...
    Preserve only what was actually displayed, demoted to ordinary text,
    ...
    redirect_cb = getattr(agent, "active_turn_redirect_callback", None)
    if callable(redirect_cb):
        try:
            redirect_cb(visible)
        except Exception:
            logger.debug("active_turn_redirect_callback error", exc_info=True)
...
def _ollama_context_limit_error(agent: Any, request_tokens: int) -> Optional[str]:
    ...
    logger.warning(
        "Ollama runtime context too small for Hairball tool use: "
    ...
    return (
        ...
        f"context, but Hairball needs at least {MINIMUM_CONTEXT_LENGTH:,} tokens "
        "for reliable tool use.\n\n"
        "Increase the Ollama context for this model and restart/reload the "
        "model before trying again. A known-good starting point is 65,536 "
        "tokens. In Hairball config, set `model.ollama_num_ctx: 65536` "
...
def _is_managed_inference_route(provider: str, base_url: str) -> bool:
    ...
    return (
        base_url_host_matches(base, "inference.example.invalid")
    )
...
def _try_refresh_managed_paid_entitlement_credentials(agent) -> bool:
    """Refresh managed runtime credentials after a fresh paid-entitlement check."""
    try:
...
_CONTENT_POLICY_RECOVERY_HINT = (
    ...
    "adding a fallback provider with `hairball fallback add`."
)
...
def _run_conversation_impl(
    ...
    if moa_config is None:
        try:
            from hairball_cli.moa_config import decode_moa_turn
    ...
    # all run inside reference). Default Hairball path is bypassed entirely.
    ...
    while (api_call_count < agent.max_iterations and agent.iteration_budget.remaining > 0) or agent._budget_grace_call:
        ...
        # ── Pre-step inbox claim ────────────────────────────────────────
        # Tool execution has fully settled before this boundary, so pending
        # next-step input can now enter history as a real user message. An
        # opening wake user stays last; claimed next-step is inserted before
        # it instead of being welded into the typed text.
        from agent.turn_inbox import (
            claim_next_step_into_messages,
            notify_next_step_claimed,
        )

        _claimed_next_step = claim_next_step_into_messages(agent, messages)
        if _claimed_next_step:
            logger.debug(
                "Pre-step inbox claim: delivered %d user message batch(es)",
                len(_claimed_next_step),
            )
            try:
                notify_next_step_claimed(
                    agent,
                    "\n".join(
                        ...
                        for row in _claimed_next_step
                    ),
                    merged=False,
                )
            except Exception:
                logger.debug("next-step claimed notify failed", exc_info=True)

        # ── Pre-API collab mailbox autosurface (④ cross-turn) ───────────
        # Surface completed collab summaries onto the last tool message when
        # idle/continuing — same piggyback pattern as /steer. Skip on
        # interrupt/cancel so envelopes are not drained mid-abort.
        if not getattr(agent, "_interrupt_requested", False):
            try:
                from tools.collab_control import apply_collab_cross_turn_autosurface

                apply_collab_cross_turn_autosurface(agent, messages)
            except Exception:
        ...
        for idx, msg in enumerate(messages):
            ...
            # Strip reference Responses API fields (call_id, response_item_id) for
            # strict providers like Mistral, Fireworks, etc. that reject unknown fields.
            # Uses new dicts so the internal messages list retains the fields
            # for reference Responses compatibility.
        ...
        # cache prefix.  The system prompt is reserved for Hairball internals.
        #
        # Hairball invariant: the system prompt is built ONCE per session
        ...
        if _runtime_context_error:
            ...
            agent._emit_status("❌ Ollama runtime context is too small for Hairball tool use")
            api_call_count -= 1
            agent._api_call_count = api_call_count
            try:
        ...
        # live 271k/272k reference failure). The post-response should_compress
        ...
        # Error recovery can run even when _build_api_kwargs() fails before the
        # request payload exists, so resolve the per-session controller before
        # entering the retry body.  Keeping this outside the try also makes the
        # controller identity stable across provider retries/fallbacks.
        _stop_controller = getattr(agent, "_stop_controller", None)
        api_request_id = f"{turn_id}:api:{api_call_count}"
        agent._current_api_request_id = api_request_id

        while retry_count < max_retries:
            # ── Managed Portal rate limit guard ──────────────────────
            # If another session already recorded that managed is rate-
            ...
            if agent.provider == "managed":
                try:
                    ...
                    if _managed_remaining is not None and _managed_remaining > 0:
                        _managed_msg = (
                            f"Managed Portal rate limit active — "
            ...
            try:
                ...
                # A blocked stop is retried against the exact same valid
                # message prefix with full tools/reasoning and automatic tool
                # selection. The host vetoes an early stop but never chooses
                # the model's next action or appends a synthetic user message.
                if _stop_controller is not None:
                    _stop_controller.apply_continuation_policy(
                        api_kwargs, agent.api_mode
                    )
                ...
                try:
                    from hairball_cli.middleware import apply_llm_request_middleware
                ...
                # Middleware may reshape provider payloads, including setting a
                # forced tool choice. Restore the autonomy-preserving policy on
                # the final payload that reaches hooks and the provider.
                if _stop_controller is not None:
                    _stop_controller.apply_continuation_policy(
                        api_kwargs, agent.api_mode
                    )
                ...
                if env_var_enabled("HAIRBALL_DUMP_REQUESTS"):
                ...
                from hairball_cli.middleware import run_llm_execution_middleware
                ...
                _native_events = getattr(agent, "_native_turn_events", None)
                _native_request_started_at = time.monotonic()
                if _native_events is not None:
                    _native_events.provider_request_started(
                        api_call_count,
                        provider_ref=agent.provider,
                ...
                except Exception as _native_provider_error:
                    if (
                        _native_events is not None
                        and not isinstance(_native_provider_error, InterruptedError)
                    ):
                        _native_events.provider_request_failed(
                            api_call_count,
                            time.monotonic() - _native_request_started_at,
                            _native_provider_error,
                            provider_ref=agent.provider,
                            model_ref=agent.model,
                        )
                    raise
                ...
                if _native_events is not None:
                    _native_events.provider_request_completed(
                        api_call_count, api_duration, response
                    )
                    ...
                    if not _ct_v.validate_response(response):
                        if response is None:
                            ...
                            if _codex_resp_status in {"failed", "cancelled"}:
                                ...
                                logger.warning(
                                    "reference response status='%s' (error=%s). Routing to fallback. %s",
                                ...
                                if _out_text_stripped:
                                    logger.debug(
                                        "reference response.output is empty but output_text is present "
                                    ...
                                    logger.warning(
                                        "reference response.output is empty after stream backfill "
                ...
                if response_invalid:
                    if _native_events is not None:
                        _native_events.provider_response_rejected(
                            api_call_count,
                            api_duration,
                ...
                if agent.api_mode == "codex_responses":
                    ...
                    if status == "incomplete" and incomplete_reason in {"max_output_tokens", "length"}:
                        ...
                        # reference incomplete turn.  Let the reference-specific
                ...
                if finish_reason == "content_filter":
                    ...
                    _refusal_response = (
                        ...
                        "(safety refusal — not a Hairball/gateway failure).\n\n"
                        f"{_refusal_detail}\n\n"
                ...
                if hasattr(response, 'usage') and response.usage:
                    # MoA's client facade represents a virtual preset, but the
                    # response came from the concrete aggregator route. Usage
                    # wire-shape normalization and billing attribution must use
                    # that route, not ``provider=moa``.
                    ...
                    _agg_api_mode = agent.api_mode
                    if isinstance(_agg_slot, dict) and _agg_slot.get("model"):
                        ...
                        _agg_api_mode = _agg_slot.get("api_mode") or agent.api_mode
                    ...
                    _moa_reference_accounting = None
                    _moa_ref_cost = None
                    if agent.provider == "moa" and _moa_client is not None and hasattr(
                        _moa_client, "consume_reference_accounting"
                    ):
                        try:
                            _moa_reference_accounting = (
                                _moa_client.consume_reference_accounting()
                            )
                            _ref_usage = _moa_reference_accounting.usage
                            _moa_ref_cost = (
                                _moa_reference_accounting.estimated_cost_usd
                            )
                    ...
                    elif agent.provider == "moa" and _moa_client is not None and hasattr(
                        _moa_client, "consume_reference_usage"
                    ):
                        # Compatibility with third-party MoA facades that have
                        # not adopted the route-preserving accounting contract.
                        try:
                ...
                if hasattr(response, 'usage') and response.usage:
                    ...
                    _aggregator_actual_cost = extract_provider_cost_usd(
                        response.usage,
                        provider=_agg_cost_provider,
                        base_url=_agg_cost_base_url,
                    )
                    _aggregator_billable_calls = (
                        0 if cost_result.status == "included" else 1
                    )
                    _aggregator_recorded_actual = (
                        _aggregator_actual_cost
                        if _aggregator_billable_calls
                        else None
                    )
                    _session_cost = accumulate_session_cost(
                        agent,
                        estimated_cost_usd=cost_result.amount_usd,
                        actual_cost_usd=_aggregator_recorded_actual,
                        actual_call_count=(
                            1 if _aggregator_recorded_actual is not None else 0
                        ),
                        billable_call_count=_aggregator_billable_calls,
                        estimated_status=cost_result.status,
                        estimated_source=cost_result.source,
                    )
                    if _moa_reference_accounting is not None:
                        _session_cost = accumulate_session_cost(
                            agent,
                            estimated_cost_usd=(
                                _moa_reference_accounting.estimated_cost_usd
                            ...
                            actual_cost_usd=(
                                _moa_reference_accounting.actual_cost_usd
                            ),
                            actual_call_count=(
                                _moa_reference_accounting.actual_call_count
                            ),
                            billable_call_count=(
                                _moa_reference_accounting.billable_call_count
                            ),
                            estimated_status=cost_result.status,
                            estimated_source=cost_result.source,
                        )
                    elif _moa_ref_cost is not None:
                        # Legacy facade: retain its estimate, but never invent
                        # actual-cost coverage that the facade cannot prove.
                        _session_cost = accumulate_session_cost(
                            agent,
                            estimated_cost_usd=_moa_ref_cost,
                            estimated_status=cost_result.status,
                            estimated_source=cost_result.source,
                        )
                    ...
                    if agent._session_db and agent.session_id:
                        try:
                            ...
                            # Persist the acting aggregator under its concrete
                            # provider/model. Advisor rows are written below,
                            # one real route at a time; collapsing all of them
                            # under virtual provider=moa makes fallback and
                            # cost attribution impossible to audit.
                            agent._session_db.update_token_counts(
                                ...
                                cost_total_call_count=_aggregator_billable_calls,
                                cost_status=(
                                    "actual"
                                    if _aggregator_recorded_actual is not None
                                ...
                                billing_provider=_agg_cost_provider,
                                billing_base_url=_agg_cost_base_url,
                            ...
                            if _moa_reference_accounting is not None:
                                for _route in _moa_reference_accounting.routes:
                                    _route_usage = _route.usage
                                    agent._session_db.update_token_counts(
                                        agent.session_id,
                                        input_tokens=_route_usage.input_tokens,
                                        output_tokens=_route_usage.output_tokens,
                                        cache_read_tokens=_route_usage.cache_read_tokens,
                                        cache_write_tokens=_route_usage.cache_write_tokens,
                                        reasoning_tokens=_route_usage.reasoning_tokens,
                                        estimated_cost_usd=(
                                            float(_route.cost_usd)
                                            if _route.cost_usd is not None
                                            else None
                                        ),
                                        actual_cost_usd=_route.actual_cost_usd,
                                        cost_actual_call_count=(
                                            _route.actual_call_count
                                        ),
                                        cost_total_call_count=(
                                            _route.billable_call_count
                                        ),
                                        cost_status=_route.cost_status,
                                        cost_source=_route.cost_source,
                                        billing_provider=_route.provider,
                                        billing_base_url=_route.base_url,
                                        billing_mode="subscription_included"
                                        if _route.cost_status == "included"
                                        else None,
                                        model=_route.model,
                                        api_call_count=_route_usage.request_count,
                                    )
                            # The last route write may carry a route-local
                            # status. Restore the session-level hybrid verdict
                            # without creating a duplicate per-model row.
                            if (
                                _moa_reference_accounting is not None
                                and _moa_reference_accounting.routes
                            ):
                                agent._session_db.update_token_counts(
                                    agent.session_id,
                                    cost_status=_session_cost.status,
                                    cost_source=_session_cost.source,
                                )
                    ...
                    # three API shapes (Anthropic / reference / OpenAI-chat)
                ...
                # Clear managed rate limit state on successful request —
                # proves the limit has reset and other sessions can
                # resume hitting managed.
                if agent.provider == "managed":
                    try:
                ...
                _native_events = getattr(agent, "_native_turn_events", None)
                if _native_events is not None:
                    _native_events.provider_request_cancelled(
                        api_call_count,
                        time.monotonic() - _native_request_started_at,
                    )
                if thinking_spinner:
                ...
                _IMAGE_REJECTION_PHRASES = (
                    ...
                    # ChatGPT-account reference backend
                ...
                # Learn only definite 4xx tool-choice capability rejections.
                # Retrying with implicit auto preserves tools, reasoning,
                # messages, and the user's selected model.
                if (
                    _stop_controller is not None
                    and _stop_controller.note_tool_choice_rejection(
                        api_error, api_kwargs
                    )
                ):
                    continue
                ...
                if (
                    ...
                    if _try_refresh_managed_paid_entitlement_credentials(agent):
                        agent._vprint(
                            f"{agent.log_prefix}🔐 managed paid access verified — "
                ...
                if (
                    ...
                    if agent._try_refresh_codex_client_credentials(force=True):
                        _label = "xAI OAuth" if agent.provider == "xai-oauth" else "reference"
                ...
                if (
                    ...
                    if agent._try_refresh_managed_client_credentials(force=True):
                        print(f"{agent.log_prefix}🔐 managed agent key refreshed after 401. Retrying request...")
                        continue
                    ...
                    try:
                        ...
                        pass
                    print(f"{agent.log_prefix}🔐 managed 401 — Portal authentication failed.")
                    if _body_text:
                        print(f"{agent.log_prefix}   Response: {_body_text}")
                    if not _print_managed_entitlement_guidance(agent, "managed model access"):
                        print(f"{agent.log_prefix}   Most likely: Portal OAuth expired, account out of credits, or agent key revoked.")
                    print(f"{agent.log_prefix}   Troubleshooting:")
                    print(f"{agent.log_prefix}     • Re-authenticate: hairball auth add managed")
                ...
                if (
                    ...
                    if is_token_provider(key):
                        ...
                        print(f"{agent.log_prefix}   Run `hairball doctor` for credential-chain diagnostics, or")
                    ...
                    from hairball_constants import display_hairball_home as _dhh_fn
                    _dhh = _dhh_fn()
                    print(f"{agent.log_prefix}     • Check ANTHROPIC_TOKEN in {_dhh}/.env for Hairball-managed OAuth/setup tokens")
                    ...
                    print(f"{agent.log_prefix}     • Legacy cleanup: hairball config set ANTHROPIC_TOKEN \"\"")
                    print(f"{agent.log_prefix}     • Clear stale keys: hairball config set ANTHROPIC_API_KEY \"\"")
                ...
                # ── Managed Portal: record rate limit & skip retries ─────
                # When managed returns a 429 that is a genuine account-
                ...
                #
                # IMPORTANT: Managed Portal multiplexes multiple upstream
                # providers (DeepSeek, Kimi, provider, Hairball).  A 429 can
                # also mean an UPSTREAM provider is out of capacity
                # for one specific model -- transient, clears in
                # seconds, nothing to do with the caller's quota.
                # Tripping the cross-session breaker on that would
                # block every managed model for minutes.  We use
                ...
                if (
                    ...
                    try:
                        ...
                        if _genuine_managed_rate_limit:
                            ...
                            logger.info(
                                "managed 429 looks like upstream capacity "
                    ...
                    if _genuine_managed_rate_limit:
                        ...
                        # top-of-loop managed guard handles fallback or
                ...
                # which Hairball' system prompt + tool schemas alone exceed.
                ...
                if (
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      request at ~8K tokens. Hairball' system prompt + tool schemas baseline",
                        force=True,
                    )
                    agent._vprint(
                    ...
                    agent._vprint(
                        f"{agent.log_prefix}      Use the `copilot` provider with a Copilot subscription token (`hairball",
                        force=True,
                    )
                    agent._vprint(
                ...
                if is_context_length_error:
                    ...
                    if available_out is not None:
                        ...
                        # Hairball may add API-only content not present in persisted
                ...
                is_local_validation_error = (
                    ...
                    # shape mismatches from upstream (e.g. chatgpt.com reference
                ...
                if is_client_error:
                    ...
                    if classified.is_auth or classified.reason == FailoverReason.billing:
                        if classified.reason == FailoverReason.billing and _print_billing_or_entitlement_guidance(
                            ...
                            if _provider == "openai-codex":
                                agent._vprint(f"{agent.log_prefix}   💡 reference OAuth token was rejected (HTTP 401). Your token may have been", force=True)
                                agent._vprint(f"{agent.log_prefix}      refreshed by another client (reference CLI, VS Code). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      1. Run `codex` in your terminal to generate fresh tokens.", force=True)
                                agent._vprint(f"{agent.log_prefix}      2. Then run `hairball auth` to re-authenticate.", force=True)
                            elif _provider == "xai-oauth":
                                agent._vprint(f"{agent.log_prefix}   💡 xAI OAuth token was rejected (HTTP 401). To fix:", force=True)
                                agent._vprint(f"{agent.log_prefix}      re-authenticate with xAI Grok OAuth (SuperGrok / Premium+) from `hairball model`.", force=True)
                            else:  # managed
                                try:
                                    ...
                                    _managed_portal_url = portal_base_url()
                                    _managed_disabled_hint = managed_foreign_disabled_message()
                                except Exception:
                                    _managed_portal_url = ""
                                    _managed_disabled_hint = (
                                        "Configure a provider API key with `hairball model`."
                                    )
                                agent._vprint(f"{agent.log_prefix}   💡 Managed Portal OAuth token was rejected (HTTP 401). Your token may be", force=True)
                                if _managed_portal_url:
                                    agent._vprint(f"{agent.log_prefix}      expired, revoked, or your account may be out of credits. To fix:", force=True)
                                    agent._vprint(f"{agent.log_prefix}      1. Re-authenticate: hairball portal", force=True)
                                    agent._vprint(
                                        f"{agent.log_prefix}      2. Check your portal account: {_managed_portal_url}",
                                        force=True,
                                    )
                                else:
                                    agent._vprint(
                                        f"{agent.log_prefix}      rejected while the managed backend is disabled. {_managed_disabled_hint}",
                                        force=True,
                                    )
                                # ``:free`` is OpenRouter slug syntax; Managed Portal will reject
                                ...
                                if isinstance(_model, str) and _model.endswith(":free"):
                                    ...
                                    agent._vprint(f"{agent.log_prefix}         Managed Portal won't recognize that model name. Either switch to a", force=True)
                                    agent._vprint(f"{agent.log_prefix}         managed catalog model, or run `/model openrouter:{_model}` to use OpenRouter.", force=True)
                        else:
                            agent._vprint(f"{agent.log_prefix}   💡 Your API key was rejected by the provider. Check:", force=True)
                            agent._vprint(f"{agent.log_prefix}      • Is the key valid? Run: hairball setup", force=True)
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        ...
                        agent._vprint(
                            f"{agent.log_prefix}        hairball fallback add   (interactive picker — same as `hairball model`)",
                            force=True,
                        )
                    ...
                    if classified.reason == FailoverReason.content_policy_blocked:
                        _policy_response = (
                            ...
                            "(not a Hairball/gateway failure).\n\n"
                ...
                if retry_count >= max_retries:
                    ...
                    if _is_thinking_timeout:
                        ...
                        agent._vprint(
                            ...
                            f"in `~/.hairball/config.yaml` to extend the per-call "
                            f"timeout. (Hairball's built-in floor is 600s for "
        ...
        try:
            ...
            if agent.api_mode == "codex_responses" and finish_reason == "incomplete":
                ...
                if agent._codex_incomplete_retries < 3:
                    if not agent.quiet_mode:
                        agent._vprint(f"{agent.log_prefix}↻ reference response incomplete; continuing turn ({agent._codex_incomplete_retries}/3)")
                    agent._session_messages = messages
                    continue
                ...
                agent._persist_session(messages, conversation_history)
                return {
                    "final_response": "reference response remained incomplete after 3 continuation attempts",
                    "messages": messages,
                    "api_calls": api_call_count,
                    "completed": False,
                    "partial": True,
                    "error": "reference response remained incomplete after 3 continuation attempts",
                }
            ...
            if assistant_message.tool_calls:
                ...
                _native_events = getattr(agent, "_native_turn_events", None)
                if _native_events is not None:
                    _native_events.assistant_message(
                        assistant_msg.get("content"), source="tool_call"
                    )
                agent._emit_interim_assistant_message(assistant_msg)
                try:
                    # Persist the assistant tool-call turn before any tool
                    # side effects run. If a destructive tool restarts or
                    # terminates Hairball mid-turn, resume logic still sees the
                ...
                if not agent._has_content_after_think_block(final_response):
                    ...
                    if (
                        ...
                        from agent.empty_recovery import build_post_tool_empty_nudge

                        messages.append({
                            "role": "user",
                            "content": build_post_tool_empty_nudge(),
                ...
                _stop_controller = getattr(agent, "_stop_controller", None)
                if _stop_controller is not None:
                    _stop_decision = _stop_controller.evaluate_candidate(
                        text=final_response,
                        messages=messages,
                        enabled_tools=agent.valid_tool_names,
                    )
                    if _stop_decision.should_continue:
                        # Stop-hook contract: retain the real assistant
                        # candidate, then record a typed user-role hook prompt.
                        # The next sample keeps the same tools, reasoning and
                        # automatic tool policy.
                        _stop_retry_available = bool(
                            (
                        ...
                        if _stop_retry_available:
                            _hook_prompt = _stop_controller.build_continuation_message()
                            if _hook_prompt is not None:
                                messages.extend([final_msg, _hook_prompt])
                        # A composed report held for verification is useful if
                        # the retry budget ends.  A bare "I'll do it now"
                        # acknowledgement is not a deliverable and must never
                        # beat the normal incomplete-turn summary.
                        if set(_stop_decision.reasons) != {"intermediate_ack"}:
                            _pending_verification_response = final_response
                        logger.debug(
                            "stop controller blocked terminal response: %s",
                            ",".join(_stop_decision.reasons),
                        )
                        # Text-only responses stream before the terminal
                        # policy can inspect the completed candidate. Close
                        # that live display segment immediately so UI/gateway
                        # adapters reclassify it as commentary instead of
                        # concatenating it with the eventual final answer.
                        # This is display-only: TTS and provider history are
                        # untouched. Provider history receives only the valid
                        # typed boundary above.
                        if agent.stream_delta_callback:
                            try:
                                agent.stream_delta_callback(None)
                            except Exception:
                                logger.debug(
                                    "stop controller stream segment break failed",
                                    exc_info=True,
                                )
                        final_response = None
                        continue
                ...
                # Official leftover: if next-step remains after a no-tool
                # completion, claim it as a real user message and run another
                # step in this same turn. Do not promote to next-turn.
                _next_users: list = []
                if not getattr(agent, "_interrupt_requested", False):
                    from agent.turn_inbox import (
                        claim_next_step_as_user_messages,
                        notify_next_step_claimed,
                    )

                    _next_users = claim_next_step_as_user_messages(agent)
                if _next_users:
                    _budget_ok = (
                        api_call_count < agent.max_iterations
                        and agent.iteration_budget.remaining > 0
                    ...
                    if _budget_ok:
                        ...
                        try:
                            notify_next_step_claimed(
                                ...
                                "\n".join(
                                    str(row.get("content") or "")
                                    for row in _next_users
                                ),
                                merged=False,
                            )
                        except Exception:
                            logger.debug(
                                "next-step claimed notify failed",
                                exc_info=True,
                            )
                        final_response = None
                        continue
                    agent._pending_steer = "\n".join(
                        str(row.get("content") or "") for row in _next_users
                    )
...
def run_conversation(
    ...
    """Run one native core turn and emit durable facts from its real seams.

    This is deliberately colocated with the established conversation loop, not
    an outer product facade.  It only attaches a turn-scoped event writer; the
    implementation below still owns prompts, provider calls, tool execution,
    callback delivery, persistence, and the returned result.
    """
    previous = getattr(agent, "_native_turn_events", None)
    if previous is not None:
        # A same-agent nested call is already inside a durable turn.  Starting a
        # second scope would split one model-visible turn into two transcripts.
        return _run_conversation_impl(
            ...
            persist_user_timestamp=persist_user_timestamp, moa_config=moa_config,
        )

    from agent.kernel.native_events import NativeTurnEvents

    recorder = NativeTurnEvents.open(agent, user_message)
    if recorder is None:
        return _run_conversation_impl(
            agent, user_message, system_message, conversation_history, task_id,
            stream_callback, persist_user_message,
    ...
    agent._native_turn_events = recorder
    try:
        ...
        recorder.finish(result, agent)
        return result
    finally:
        delattr(agent, "_native_turn_events")
