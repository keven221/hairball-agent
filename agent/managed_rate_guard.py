"""Cross-session rate limit guard for Managed Portal.

Writes rate limit state to a shared file so all sessions (CLI, gateway,
cron, auxiliary) can check whether Managed Portal is currently rate-limited
before making requests.  Prevents retry amplification when RPH is tapped.

Each 429 from managed triggers up to 9 API calls per conversation turn
(3 SDK retries x 3 Hairball retries), and every one of those calls counts
...
_STATE_FILENAME = "managed.json"


def _state_path() -> str:
    """Return the path to the managed rate limit state file."""
    try:
        from hairball_constants import get_hairball_home
        base = get_hairball_home()
    except ImportError:
        base = os.path.join(os.path.expanduser("~"), ".hairball")
...
def record_managed_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    error_context: Optional[dict[str, Any]] = None,
    default_cooldown: float = 300.0,
) -> None:
    """Record that Managed Portal is rate-limited.
    ...
    try:
        ...
        logger.info(
            "managed rate limit recorded: resets in %.0fs (at %.0f)",
            reset_at - now, reset_at,
        )
    except Exception as exc:
        logger.debug("Failed to write managed rate limit state: %s", exc)


def managed_rate_limit_remaining() -> Optional[float]:
    """Check if Managed Portal is currently rate-limited.
...
def clear_managed_rate_limit() -> None:
    """Clear the rate limit state (e.g., after a successful managed request)."""
    try:
        os.unlink(_state_path())
    except FileNotFoundError:
        pass
    except OSError as exc:
        logger.debug("Failed to clear managed rate limit state: %s", exc)
...
def is_genuine_managed_rate_limit(
    *,
    headers: Optional[Mapping[str, str]] = None,
    last_known_state: Optional[Any] = None,
) -> bool:
    """Decide whether a 429 from Managed Portal is a real account rate limit.

    Managed Portal multiplexes multiple upstream providers (DeepSeek, Kimi,
    provider, Hairball, ...) behind one endpoint.  A 429 can mean either:

      (a) The caller's own RPM / RPH / TPM / TPH bucket on managed is
      ...
      (b) The upstream provider is out of capacity for a specific model
          ...
          the caller's quota on managed.

    Tripping the cross-session breaker on (b) blocks ALL managed requests
    (and all models, since managed is one provider key) for minutes even
    ...
    We tell the two apart by looking at:
      ...
      1. The 429 response's own ``x-ratelimit-*`` headers.  managed emits
