    ...
    while message_index < len(messages):
        ...
        for tool_call in tool_calls:
            ...
            try:
                ...
                # (reference Responses format) and insert a duplicate stub that
...
def repair_message_sequence(agent, messages: List[Dict]) -> int:
    ...
    #
    # reference Responses interim turns are exempt: the codex_responses
    ...
    # tool_call. In the reference Responses API format the two differ
...
def strip_think_blocks(agent, content: str) -> str:
    ...
    #     legacy's intentional asymmetry.)
    content = re.sub(
...
def try_recover_primary_transport(
    ...
    Skipped for proxy/aggregator providers (OpenRouter, managed) which
    ...
    if provider_lower in {"managed", "managed-research"}:
        return False
...
def dump_api_request_debug(
    ...
    *,
    ...
    try:
        ...
        # originate from untrusted input (X-Hairball-Session-Id header), and an
        ...
        if env_var_enabled("HAIRBALL_DUMP_REQUEST_STDOUT"):
...
def anthropic_prompt_cache_policy(
    ...
    *,
    ...
    if eff_provider.strip().lower() == "moa":
        try:
            from hairball_cli.config import load_config as _load_moa_cfg
    ...
    # The managed provider uses the OpenAI-wire envelope cache_control layout.
    # Identify that route from its protocol provider id, not from a hostname
    # substring: Hairball-owned/self-hosted endpoints may use any domain, and a
    # custom endpoint containing "hairball" must not inherit managed semantics.
    from hairball_cli.provider_ids import is_managed_provider_id

    is_managed_portal = is_managed_provider_id(eff_provider)
    ...
    if is_native_anthropic:
        return True, True
    if (is_openrouter or is_managed_portal) and (is_claude or is_kimi):
        return True, False
    # Managed Portal Qwen (e.g. qwen3.6-plus) takes the same envelope-layout
    ...
    if is_managed_portal and "qwen" in model_lower:
        return True, False
...
def create_openai_client(agent, client_kwargs: dict, *, reason: str, shared: bool) -> Any:
    ...
    # Delegate all rate-limit / 5xx retry to hairball's outer conversation loop,
...
def switch_model(agent, new_model, new_provider, api_key='', base_url='', api_mode=''):
    ...
    turn-scoped).
    """
    from hairball_cli.providers import determine_api_mode
    ...
    if hasattr(agent, "context_compressor") and agent.context_compressor:
        ...
        try:
            from hairball_cli.config import load_config, get_compatible_custom_providers
            _sm_cfg = load_config()
    ...
    try:
        ...
        from hairball_cli.config import load_config as _sm_load_config
...
def invoke_tool(agent, function_name: str, function_args: dict, effective_task_id: str,
    ...
    # Collab V2 tools need the parent AIAgent; registry dispatch has no agent kw.
    _collab_token = None
    try:
...
def sanitize_api_messages(messages: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
    ...
    for msg in messages:
        ...
        if role not in _ra().AIAgent._VALID_API_ROLES:
            ...
            continue
        if role == "user" and msg.get("finish_reason") == "hook_prompt":
            # Durable typed-history marker; not part of provider wire shapes.
            msg = {key: value for key, value in msg.items() if key != "finish_reason"}
        filtered.append(msg)
    messages = filtered
    ...
    #
    # We do NOT drop the call: hairball' own dispatch loop intentionally keeps an
...
def copy_reasoning_content_for_api(agent, source_msg: dict, api_msg: dict) -> None:
    ...
    # (DeepSeek / Kimi / provider), preserve it verbatim — that includes their
    ...
    # reasoning primary (DeepSeek/Kimi/provider) pads history with " ", then a
...
def reapply_reasoning_echo_for_provider(agent, api_messages: list) -> int:
    ...
    * Switching TO a require-side provider (DeepSeek / Kimi / provider thinking
...
def apply_pending_steer_to_tool_results(agent, messages: list, num_tool_msgs: int) -> None:
    """Compatibility no-op for the former tool-result injection boundary.

    Callers still invoke this after partial and complete tool batches. Draining
    there can place a user instruction between sibling tool results, so the
    inbox remains untouched until ``conversation_loop`` reaches the next
    pre-step boundary and records a real user message.
    """
    return None
