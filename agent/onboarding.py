...
def openclaw_residue_hint_cli() -> str:
    """Banner shown the first time Hairball starts and finds ``~/.openclaw/``.

    Points users at ``hairball claw migrate`` (non-destructive port of config,
    memory, and skills) first. ``hairball claw cleanup`` is mentioned as the
    follow-up step for users who have already migrated and want to archive
    the old directory — with a warning that archiving breaks legacy.
    """
    return (
        "A legacy legacy directory was detected at ~/.openclaw/.\n"
        "To port your config, memory, and skills over to Hairball, run "
        "`hairball claw migrate`.\n"
        "If you've already migrated and want to archive the old directory, "
        "run `hairball claw cleanup` (renames it to ~/.openclaw.pre-migration — "
        "legacy will stop working after this).\n"
        "This tip only shows once."
    )


def detect_openclaw_residue(home: Optional[Path] = None) -> bool:
    """Return True if an legacy workspace directory is present in ``$HOME``.
