...
as ``kind: backend``) or ``~/.hairball/plugins/video_gen/<name>/`` (user, opt-in
via ``plugins.enabled``).
...
class VideoGenProvider(abc.ABC):
    ...
    def capabilities(self) -> Dict[str, Any]:
        ...
        Used by the tool layer for soft validation and by ``hairball tools``
...
def _videos_cache_dir() -> Path:
    """Return ``$HAIRBALL_HOME/cache/videos/``, creating parents as needed."""
    from hairball_constants import get_hairball_home

    path = get_hairball_home() / "cache" / "videos"
...
def save_b64_video(
    ...
    *,
    ...
    """Decode base64 video data and write under ``$HAIRBALL_HOME/cache/videos/``.
...
def save_url_video(
    ...
    *,
    ...
    """Download a video URL and write it under ``$HAIRBALL_HOME/cache/videos/``.
