...
class LSPClient:
    ...
    def __init__(
        ...
        *,
        ...
        # Serialize document lifecycle operations per absolute path.  A single
        # LSP client is shared by every synchronous caller for this
        # (server, workspace) pair, so two concurrent writes can otherwise
        # both observe a file as unopened and emit duplicate didOpen(version=0)
        # notifications.  Locks live for the client lifetime to avoid deleting
        # a lock while another waiter is about to acquire it.
        self._file_operation_locks: Dict[str, asyncio.Lock] = {}
    ...
    async def _spawn(self) -> None:
        ...
        cwd = self._cwd
        if cwd:
            # write_file snapshots LSP *before* creating the file; the
            # workspace directory may not exist yet.  Create it so
            # create_subprocess_exec(cwd=...) does not raise a misleading
            # "binary not found" FileNotFoundError.
            try:
                os.makedirs(cwd, exist_ok=True)
            except OSError as e:
                raise LSPProtocolError(
                    f"LSP workspace cwd unavailable: {cwd} ({e})"
                ) from e
        ...
        try:
            ...
            # Unix raises FileNotFoundError for a missing *cwd* as well as
            # a missing executable — classify so operators/models don't
            # chase a phantom missing pyright binary.
            if cwd and not os.path.isdir(cwd):
                raise LSPProtocolError(
                    f"LSP workspace cwd not found: {cwd} ({e})"
                ) from e
            raise LSPProtocolError(
    ...
    async def _send_request(self, method: str, params: Any) -> Any:
        ...
        try:
            ...
            # ``asyncio.wait_for`` cancels this coroutine on timeout.  Remove
            # the server-side work as well instead of merely forgetting the
            # local future; cancellation is best-effort and deliberately
            # bounded so a wedged stdin cannot stall the caller's timeout.
    ...
    def _file_operation_lock(self, path: str) -> asyncio.Lock:
        """Return the client-local lifecycle lock for an absolute file path."""
        lock = self._file_operation_locks.get(path)
        if lock is None:
            lock = asyncio.Lock()
            self._file_operation_locks[path] = lock
        return lock
    ...
    async def open_file(self, path: str, *, language_id: str = "plaintext") -> int:
        ...
        """
        abs_path = os.path.abspath(path)
        async with self._file_operation_lock(abs_path):
    ...
    async def save_file(self, path: str) -> None:
        ...
        abs_path = os.path.abspath(path)
        async with self._file_operation_lock(abs_path):
            if not self.is_running or abs_path not in self._files:
                return
            await self._send_notification(
    ...
    async def close_file(self, path: str) -> None:
        """Send didClose and drop local open-file state for ``path``."""
        abs_path = os.path.abspath(path)
        async with self._file_operation_lock(abs_path):
            if not self.is_running or abs_path not in self._files:
                return
    ...
    # ------------------------------------------------------------------
    # navigation / rename / code actions (model-facing; tools/lsp_tool.py)
    # ------------------------------------------------------------------

    async def _ensure_open(self, path: str, *, language_id: str = "plaintext") -> str:
        """Open ``path`` if needed; return absolute path."""
        abs_path = os.path.abspath(path)
        if abs_path not in self._files:
            await self.open_file(abs_path, language_id=language_id)
        return abs_path

    async def request_definition(
        self, path: str, line: int, character: int, *, language_id: str = "plaintext", timeout: float = 20.0
    ) -> Any:
        abs_path = await self._ensure_open(path, language_id=language_id)
    ...
    async def request_document_symbols(
        self, path: str, *, language_id: str = "plaintext", timeout: float = 20.0
    ) -> Any:
        abs_path = await self._ensure_open(path, language_id=language_id)
        return await self._send_request_with_retry(
    ...
    async def request_workspace_symbol(self, query: str, *, timeout: float = 20.0) -> Any:
        return await self._send_request_with_retry(
            "workspace/symbol",
            {"query": query or ""},
            timeout=timeout,
    ...
    async def request_code_actions(
        ...
        *,
        ...
        uri = file_uri(abs_path)
        position = {"line": line, "character": character}
        diagnostics = self.diagnostics_for(abs_path)
    ...
    async def request_code_action_resolve(
        self, action: Dict[str, Any], *, timeout: float = 20.0
    ) -> Any:
        return await self._send_request_with_retry(
            "codeAction/resolve",
            action,
    ...
    async def send_raw_request(
        ...
        *,
        ...
        """Send an arbitrary LSP request (model ``action=request`` escape hatch).

        When ``path`` is set, ensures the document is open first (didOpen),
        matching oh-my-pi bare-request behaviour. ``params`` is forwarded
        as-is (including JSON ``null``).
        """
        if path:
            await self._ensure_open(path, language_id=language_id)
        return await self._send_request_with_retry(method, params, timeout=timeout)

    def supports_document_formatting(self) -> bool:
        """True when initialize advertised ``documentFormattingProvider``."""
        caps = (self._initialize_result or {}).get("capabilities") or {}
        return bool(caps.get("documentFormattingProvider"))

    async def request_document_formatting(
        self,
        path: str,
        *,
        language_id: str = "plaintext",
        ...
        """``textDocument/formatting`` after syncing on-disk content to the server.

        Always calls :meth:`open_file` so a just-written buffer is pushed via
        didOpen/didChange before the format request (format-on-write path).
        Returns ``None`` when the server does not advertise formatting.
        """
        if not self.supports_document_formatting():
            return None
        abs_path = os.path.abspath(path)
        await self.open_file(abs_path, language_id=language_id)
        return await self._send_request_with_retry(
    ...
    async def request_will_rename_files(
        ...
        *,
        ...
        """``workspace/willRenameFiles`` — returns WorkspaceEdit or None.

        Method-not-found is treated as empty (server does not support it).
        """
        try:
            return await self._send_request_with_retry(
            ...
            if e.code == ERROR_METHOD_NOT_FOUND or _is_method_not_found_message(e.message):
                return None
            raise

    async def notify_did_rename_files(self, pairs: List[Dict[str, str]]) -> None:
        await self._send_notification("workspace/didRenameFiles", {"files": pairs})

    async def reload_workspace(self, *, timeout: float = 20.0) -> str:
        """Best-effort reload: rust-analyzer request, else didChangeConfiguration.

        Returns a short status string. Never raises for unsupported methods.
        """
        try:
            await asyncio.wait_for(
                self._send_request("rust-analyzer/reloadWorkspace", None),
                timeout=timeout,
            )
            return f"Reloaded {self.server_id}"
        except Exception:  # noqa: BLE001 — fall through like OMP
            pass
        # Notification (not request) — OMP notes request hangs on many servers.
        try:
            await self._send_notification(
                "workspace/didChangeConfiguration",
                {"settings": self._init_options or {}},
            )
            return f"Reloaded {self.server_id}"
        except Exception as e:  # noqa: BLE001
            logger.debug("[%s] reload notify failed: %s", self.server_id, e)
            return f"Reload notify failed for {self.server_id}: {e}"
...
def _is_method_not_found_message(message: str) -> bool:
    msg = (message or "").lower()
    return (
        "method not found" in msg
        or "unhandled method" in msg
