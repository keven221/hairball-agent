...
appropriate.  All installs go to a Hairball-owned bin staging dir,
``<HAIRBALL_HOME>/lsp/bin/``, so we don't pollute the user's global
toolchain.
...
- ``manual`` — never install; if a binary is missing, the server is
  silently skipped and the user is told about it via ``hairball lsp
  status``.
...
# ``<HAIRBALL_HOME>/lsp/bin/`` first, then on PATH.
#
# Optional fields:
...
INSTALL_RECIPES: Dict[str, Dict[str, Any]] = {
    ...
    # `hairball lsp status` reports the host's presence.
...
def hairball_lsp_bin_dir(*, create: bool = True) -> Path:
    """Return the Hairball-owned bin staging dir for LSP servers.

    ``create=False`` is a probe-only path (spawn ``_which``) so listing a
    missing server does not mkdir under ``HAIRBALL_HOME``.
    """
    home = os.environ.get("HAIRBALL_HOME")
    if home is None:
        home = os.path.join(os.path.expanduser("~"), ".hairball")
    p = Path(home) / "lsp" / "bin"
    if create:
...
def _existing_binary(name: str) -> Optional[str]:
    ...
    for staged in _native_binary_candidates(hairball_lsp_bin_dir() / name):
...
def _install_npm(
    ...
    if npm is None:
        ...
        return None
    staging = hairball_lsp_bin_dir().parent  # <HAIRBALL_HOME>/lsp/
    ...
    for c in _native_binary_candidates(nm_bin):
        if c.exists():
            ...
            link = hairball_lsp_bin_dir() / c.name
            if not link.exists():
                try:
                    link.symlink_to(c)
...
def _install_pip(pkg: str, bin_name: str) -> Optional[str]:
    """Install a Python package into a hairball-owned target dir.
    ...
    console script.
    """
    pip_target = hairball_lsp_bin_dir().parent / "python-packages"
    ...
    for script_dir in script_dirs:
        for bin_path in _native_binary_candidates(script_dir / bin_name):
            if bin_path.exists():
                link = hairball_lsp_bin_dir() / bin_path.name
                if not link.exists():
                    try:
...
def detect_status(pkg: str) -> str:
    ...
    Used by the ``hairball lsp status`` CLI to give users a quick
