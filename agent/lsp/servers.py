...
def _which(*names: str) -> Optional[str]:
    """Return the first binary in Hairball ``lsp/bin``, then PATH.

    Probe order matches ``agent.lsp.install._existing_binary`` so an
    auto-installed ``typescript-language-server`` / ``gopls`` is found
    without mutating process PATH. Does not create the staging dir.
    """
    staged_dir = None
    try:
        from agent.lsp.install import hairball_lsp_bin_dir

        staged_dir = hairball_lsp_bin_dir(create=False)
    except Exception:
        staged_dir = None
    suffixes = (".cmd", ".exe", ".bat") if os.name == "nt" else ()
    for n in names:
        if staged_dir is not None:
            candidates = [staged_dir / n]
            candidates.extend(staged_dir / f"{n}{suffix}" for suffix in suffixes)
...
def _find_pses_bundle(ctx: ServerContext) -> Optional[str]:
    ...
    4. ``<HAIRBALL_HOME>/lsp/PowerShellEditorServices`` staging dir (where a
    ...
    home = os.environ.get("HAIRBALL_HOME") or os.path.join(
        os.path.expanduser("~"), ".hairball"
    )
...
def _spawn_powershell_es(root: str, ctx: ServerContext) -> Optional[SpawnSpec]:
    ...
    if bundle is None:
        ...
        if not _PSES_BUNDLE_WARNED:
            ...
            logger.warning(
                ...
                "<HAIRBALL_HOME>/lsp/PowerShellEditorServices."
            )
        return None
    start_script = os.path.join(
    ...
    session_path = os.path.join(
        hairball_lsp_session_dir(), f"pses-session-{os.getpid()}.json"
    )
    log_path = os.path.join(hairball_lsp_session_dir(), "pses.log")
    inner = (
        f"& '{start_script}' "
        ...
        f"-HostName Hairball -HostProfileId hairball -HostVersion 1.0.0 "
...
def hairball_lsp_session_dir() -> str:
