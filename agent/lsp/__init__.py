"""Language Server Protocol (LSP) integration for Hairball Agent.

Hairball runs full language servers (pyright, gopls, rust-analyzer,
typescript-language-server, etc.) as subprocesses and:

1. Pipes ``textDocument/publishDiagnostics`` into the post-write lint
   delta filter used by ``write_file`` and ``patch``.
2. Exposes navigation / rename to the model via the ``lsp`` tool
   (``tools/lsp_tool.py`` → :meth:`LSPService.request_sync`).
...
Public API:
    ...
    if svc and svc.enabled_for(path):
        diags = svc.get_diagnostics_sync(path, delta=True)
        locs = svc.request_sync("definition", file_path=path, line=0, character=0)
...
def get_service() -> Optional[LSPService]:
    ...
    with _service_lock:
        ...
        if not _atexit_registered:
            ...
            # ``hairball chat`` exit would leak pyright processes that
