...
class MemoryManager:
    ...
    def add_provider(self, provider: MemoryProvider) -> None:
        ...
        _core_tool_names = set(_HAIRBALL_CORE_TOOLS)
    ...
    def _strip_skill_scaffolding(text: str) -> Optional[str]:
        ...
        When a user invokes a /skill or /bundle, Hairball expands the turn into
    ...
    def initialize_all(self, session_id: str, **kwargs) -> None:
        ...
        Automatically injects ``hairball_home`` into *kwargs* so that every
        provider can resolve profile-scoped storage paths without importing
        ``get_hairball_home()`` themselves.
        """
        if "hairball_home" not in kwargs:
            from hairball_constants import get_hairball_home
            kwargs["hairball_home"] = str(get_hairball_home())
        for provider in self._providers:
            try:
