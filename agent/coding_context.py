"""Coding-context awareness — base Hairball, every interactive surface.

When the user runs Hairball inside a code workspace (CLI, TUI, desktop app, or an
editor over ACP), Hairball shifts into a **coding posture**. This module is the
...
domain reads the same resolved object instead of probing git/config itself:
  ...
  * **Toolset** — ``RuntimeMode.toolset_selection()`` → the ``coding`` toolset
    ...
    the default ``focus`` mode the toolset collapses; ``auto`` stays
    prompt-only and never touches the user's configured toolsets (toolsets
    like messaging / smart-home / music are off-by-default anyway, and someone
    who explicitly enabled image-gen or Spotify can keep them via ``auto``).
...
Activation (config ``agent.coding_context``):
  ...
  * ``focus`` (default) — posture (brief + snapshot) on an interactive coding
    surface in a code workspace, plus collapse the toolset to the ``coding``
    set + enabled MCP servers and demote non-coding skill categories to
    names-only in the prompt's skill index (no skill is ever hidden).
  * ``auto`` — same posture detection, but prompt-only: toolsets and the
    skill index untouched (wider schema).
...
from hairball_cli._subprocess_compat import IS_WINDOWS, windows_hide_flags

logger = logging.getLogger("hairball.coding_context")
...
# was trained reduces mistakes and wasted reasoning (ChatGPT handle
...
#
# GPT/reference get V4A for ALL edits, single-file included: in codex-rs,
...
_EDIT_FORMAT_GUIDANCE: dict[str, tuple[tuple[str, ...], str]] = {
    ...
    "replace": (
        ("claude", "sonnet", "opus", "haiku",
         ...
         "hairball", "llama", "mistral", "devstral", "minimax"),
        "- Edit format: author new files with `write_file`; for edits to "
...
# Edge toolsets appended on interactive coding surfaces under auto mode
# (focus already includes them via the ``coding`` toolset list). Messaging
# platforms never get these — see CODING_CAPABILITY_ALIGNMENT_ROADMAP W0.
CODING_EDGE_TOOLSETS = ("lsp", "ast", "debug")

# ── W4-S1 schema policy (measure first; do not demote without data) ─────────
#
# Three layers from CODING_CAPABILITY_ALIGNMENT_ROADMAP §3.2 / §7 — Hairball
# names, no xd://. These frozensets are governance contracts for audits and
# tests. They do NOT hide tools from the live schema by themselves.

CODING_ESSENTIAL_TOOLS = frozenset(
    {
        "read_file",
        "write_file",
        "patch",
...
# Model-facing tool *names* behind CODING_EDGE_TOOLSETS (toolset ids → tools).
CODING_EDGE_TOOLS = frozenset({"lsp", "ast_grep", "ast_edit", "debug"})

# Demoted from the ``coding`` / ``hairball-acp`` posture sets (W4-S2). Still on
# ``_HAIRBALL_CORE_TOOLS`` so auto CLI/messaging keep browser. Re-enable on
# focus/ACP via the ``browser`` toolset.
CODING_BROWSER_TOOLS = frozenset(
    {
        "browser_navigate",
        "browser_snapshot",
        "browser_click",
...
# Soft ceiling for the ``coding`` posture toolset (documentation target).
# Not a change-detector hard fail — tests only warn via ``over_soft_budget``.
# Post W4-S2 demotion the focus set is ~31 tools.
CODING_SCHEMA_SOFT_BUDGET = 40


def classify_coding_schema_tools(tool_names: list[str] | tuple[str, ...] | set[str]) -> dict[str, Any]:
    """Partition a resolved tool-name list into W4 schema layers.

    Pure / side-effect free. Used by audits and invariant tests — not by the
    live ``get_tool_definitions`` path (demotion is a later gated slice).
    """
    names = {str(n) for n in tool_names if n}
    essential = sorted(names & CODING_ESSENTIAL_TOOLS)
    edge = sorted(names & CODING_EDGE_TOOLS)
    browser = sorted(names & CODING_BROWSER_TOOLS)
    other = sorted(names - CODING_ESSENTIAL_TOOLS - CODING_EDGE_TOOLS - CODING_BROWSER_TOOLS)
    missing_essential = sorted(CODING_ESSENTIAL_TOOLS - names)
    return {
        "total": len(names),
        "essential": essential,
        "edge": edge,
        ...
        "over_soft_budget": len(names) > CODING_SCHEMA_SOFT_BUDGET,
        "soft_budget": CODING_SCHEMA_SOFT_BUDGET,
    }


def audit_coding_schema_policy() -> dict[str, Any]:
    """Snapshot coding vs messaging schema policy (W4).

    Reads ``toolsets`` only. Asserts governance invariants callers can check:
    coding keeps essentials + edges and excludes browser (W4-S2); core/messaging
    never pay for coding edges but core still keeps browser for auto surfaces.
    """
    from toolsets import _HAIRBALL_CORE_TOOLS, resolve_toolset

    coding_tools = list(resolve_toolset(CODING_TOOLSET))
    core_tools = list(_HAIRBALL_CORE_TOOLS)
    messaging_tools = list(resolve_toolset("hairball-telegram"))
    coding = classify_coding_schema_tools(coding_tools)
    core = classify_coding_schema_tools(core_tools)
    messaging = classify_coding_schema_tools(messaging_tools)
    return {
        "coding": coding,
        "core": core,
        "messaging": messaging,
        "invariants": {
            "coding_has_essentials": not coding["missing_essential"],
            "coding_has_edges": set(coding["edge"]) == set(CODING_EDGE_TOOLS),
            "coding_excludes_browser": not coding["browser"],
            "core_keeps_browser": bool(core["browser"]),
            "core_excludes_edges": not core["edge"],
            "messaging_excludes_edges": not messaging["edge"],
            "edge_toolsets_match_tools": set(CODING_EDGE_TOOLSETS)
            == {"lsp", "ast", "debug"},
        },
    }
...
# search_files, patch, write_file, terminal, todo, lsp, ast_grep, ast_edit, debug)
# are in the coding toolset; ``lsp`` / ``ast`` / ``debug`` are also edge-mounted
# on interactive auto sessions when their check_fn gates pass.
CODING_AGENT_GUIDANCE = (
    ...
    "`search_files` before changing anything. When a language server is "
    "available, prefer `lsp` (definition / references / hover / symbols) to "
    "trace a symbol — text search misses re-exports and shadowed names. "
    "LSP and AST (`ast_grep` / `ast_edit` default path=.) share one session "
    "coding workspace pin (seeded from the project cwd; writing outside that "
    "tree re-pins; a pin that is too narrow lifts when package imports need "
    "the parent). Non-git trees are allowed via project markers or the file's "
    "parent; only home and filesystem root are refused. "
    "Inspect or override the pin with `lsp` action=workspace / workspace_set. "
    "When writing DONE/status markers, use a real UTC timestamp from the clock "
    "(not a placeholder like T00:00:00Z). "
    "For server-specific or less-common methods (formatting, signatureHelp, "
    "call hierarchy, extensions), use `lsp` action=request with query=method "
    "and optional payload JSON — it returns JSON only and does not apply edits. "
    "When syntax shape matters more than text (calls, declarations, "
    "language constructs) and `ast_grep` is available, use it with a narrow "
    "`path`; keep plain text/regex search on `search_files`. Match results may "
    "include a flat `meta` map of metavariable captures. For top-level "
    "structure (classes/functions/members) without a pattern, use "
    "`ast_grep` action=outline. Directory scans can take paths=[…] and "
    "optional glob/globs filters. When you need an "
    "interactive Python breakpoint session and `debug` is available, use it "
    "(launch|attach → stack_trace / continue) instead of sprinkling print() "
    "calls.\n"
    ...
    "- Edit with `patch`/`write_file`. For structural codemods (same AST shape "
    "across many call sites), prefer `ast_edit` with apply=false preview first, "
    "then apply=true. Pass ops=[{pattern,rewrite},…] for multiple rewrite rules "
    "in one call, or paths=[…] for several scopes. Do NOT print code blocks to "
    "the user as a substitute for editing — apply the change, then summarise "
    "it. Only show code when the user explicitly asks to see it.\n"
    "- For cross-file identifier renames, use `lsp` action=rename (preview with "
    "apply=false first). For moving/renaming source files, use `lsp` "
    "action=rename_file so the language server can rewrite imports. Do not "
    "hand-edit every callsite with patch/search when rename can update "
    "imports and references, and do not use `ast_grep`/`ast_edit` as a "
    "substitute for symbol rename. Prefer `lsp` action=code_actions for "
    "organize-imports / quickfixes when the server offers them.\n"
...
def _coding_mode(config: Optional[dict[str, Any]]) -> str:
    ...
    if mode in {"auto"}:
        return "auto"
    # Unknown tokens fall back to the product default (focus).
    return "focus"
...
class RuntimeMode:
    ...
    def toolset_selection(self, config: Optional[dict[str, Any]] = None) -> Optional[list[str]]:
        ...
        selection (``--toolsets``, ``HAIRBALL_TUI_TOOLSETS``, …); they never
    ...
    def edge_toolsets(self) -> list[str]:
        """Extra toolsets to merge onto the platform default under coding auto.

        Interactive coding surfaces (CLI/TUI/desktop/ACP) append these so
        model-facing edges like ``lsp`` / ``ast`` are visible without requiring
        ``focus``. Messaging platforms never activate coding auto, so they
        never receive these. Empty when posture is general or coding is off.
        """
        ...
        if surface and surface not in INTERACTIVE_CODING_PLATFORMS:
            return []
        return list(CODING_EDGE_TOOLSETS)
...
def is_coding_context(
    *,
    ...
    """Whether Hairball should operate in its coding posture right now."""
...
def coding_edge_toolsets(
    *,
    platform: Optional[str] = None,
    cwd: Optional[str | Path] = None,
    config: Optional[dict[str, Any]] = None,
) -> list[str]:
    """Edge toolsets to merge into platform defaults on coding auto sessions.

    See :meth:`RuntimeMode.edge_toolsets`. Callers that resolve platform
    toolsets under ``auto`` should union this list so ``lsp`` (etc.) is
    visible without collapsing to the full ``coding`` toolset.
    """
    return resolve_runtime_mode(
        platform=platform, cwd=cwd, config=config
    ).edge_toolsets()
...
def merge_coding_edge_toolsets(
    ...
    *,
    ...
    """Return ``toolsets`` with coding edge toolsets unioned in (sorted unique)."""
    base = list(toolsets or [])
    edge = coding_edge_toolsets(platform=platform, cwd=cwd, config=config)
    if not edge:
        return base
    return sorted({*base, *edge})
...
def build_coding_workspace_block(cwd: Optional[str | Path] = None) -> str:
    ...
    When a session coding-workspace pin differs from the UI/project root,
    both are listed so LSP/AST and relative-path tools stay unambiguous.
    """
    coding_ws: Optional[str] = None
    coding_roots: list[str] = []
    try:
        from agent.coding_workspace import get_coding_roots, get_coding_workspace

        coding_ws = get_coding_workspace()
        coding_roots = get_coding_roots()
    except Exception:
        coding_ws = None
        coding_roots = []
    ...
    if root is None and not coding_ws:
        return ""
    ...
    if coding_ws:
        lines.append(f"- Coding workspace (LSP/AST): {coding_ws}")
        extras = [r for r in coding_roots if os.path.abspath(r) != os.path.abspath(coding_ws)]
        if extras:
            lines.append(f"- Coding roots: {', '.join(extras)}")
        if root is not None and os.path.abspath(str(root)) != os.path.abspath(coding_ws):
            lines.append(
                "- Note: relative file paths use Root; language intelligence "
                "defaults to Coding workspace (sibling scratch trees may share "
                "one umbrella root)"
            )
    elif root is not None:
        lines.append(f"- Coding workspace (LSP/AST): {root}")
    ...
    facts_root = root or coding_ws
    if facts_root is not None:
        lines.extend(_project_facts(facts_root))
    return "\n".join(lines)
