...
from hairball_cli.config import load_env
from agent.secret_scope import get_secret as _get_secret
from agent.credential_persistence import (
    is_borrowed_credential_source,
    sanitize_borrowed_credential_payload,
)
import hairball_cli.auth as auth_mod
...
# ChatGPT Responses API, Anthropic, xAI, and Google OAuth spec.
_TERMINAL_AUTH_REASONS = frozenset({
    "token_invalidated",   # ChatGPT: "Your authentication token has been invalidated."
...
class PooledCredential:
    ...
    def runtime_api_key(self) -> str:
        if self.provider == "managed":
            # managed stores the runtime inference credential in agent_key for
...
def _write_through_provider_state_to_global_root(
    ...
    (#48415 / #43589): managed, openai-codex, and xai-oauth rotate the
    ...
    ``hairball_cli.auth._write_through_xai_oauth_to_global_root`` (which covers
...
class CredentialPool:
    ...
    def _sync_codex_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a reference device_code pool entry from auth.json if tokens differ.

        When a reference OAuth access token expires (or the ChatGPT account hits
        ...
        Meanwhile the user may run ``hairball model`` / ``hairball auth`` which
        ...
        Mirrors the managed/Anthropic resync paths above.  Only applies to
        ...
        try:
            ...
            # Adopt auth.json tokens when either side differs.  reference refresh
            ...
            if store_access and (
                ...
                logger.debug(
                    "Pool entry %s: syncing reference tokens from auth.json "
            ...
            logger.debug("Failed to sync reference entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_xai_oauth_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        ...
        xAI OAuth refresh tokens are single-use.  When another Hairball process
    ...
    def _sync_managed_entry_from_auth_store(self, entry: PooledCredential) -> PooledCredential:
        """Sync a managed pool entry from auth.json if tokens differ.

        managed OAuth refresh tokens are single-use.  When another process
        ...
        avoiding a "refresh token reuse" revocation on the Managed Portal.
        """
        ...
        try:
            ...
            if should_sync:
                logger.debug(
                    "Pool entry %s: syncing managed state from auth.json",
                    entry.id,
                )
                field_updates: Dict[str, Any] = {
                    "last_status": None,
            ...
            logger.debug("Failed to sync managed entry from auth.json: %s", exc)
        return entry
    ...
    def _sync_device_code_entry_to_auth_store(self, entry: PooledCredential) -> None:
        ...
        (currently managed, ChatGPT, and xAI Grok OAuth).
        ...
        here would mean every managed/reference/xAI refresh in a multi-provider
        setup silently flips the ``active_provider`` flag — the next
        ``hairball`` invocation that defaults to the active provider
        (e.g. setup wizard, ``hairball auth status``) would land on
        ...
        # device-code sources (managed, openai-codex, xAI) use ``device_code``.
        ...
        try:
            with _auth_store_lock():
                ...
                # hairball_cli.auth._save_xai_oauth_tokens (#43589); the pool
                # refresh path is the reference/xAI analog reported in #48415.
                _wt_provider_id = {
                    "managed": "managed",
    ...
    def _refresh_entry(self, entry: PooledCredential, *, force: bool) -> Optional[PooledCredential]:
        ...
        # reference OAuth refresh tokens are single-use.  The sync→POST→write-back
        # sequence below must run atomically across Hairball processes: otherwise
        ...
        if self.provider == "openai-codex":
            refresh_timeout_seconds = auth_mod.env_float(
                "HAIRBALL_CODEX_REFRESH_TIMEOUT_SECONDS", 20
            )
            lock_timeout = max(
        ...
        try:
            if self.provider == "anthropic":
                ...
                refreshed = refresh_anthropic_oauth_pure(
                    entry.refresh_token,
                    use_json=entry.source.endswith("hairball_pkce"),
                )
                updated = replace(
                    entry,
                ...
                # refresh_token — single-use tokens consumed by another Hairball
                ...
                if synced.refresh_token != entry.refresh_token:
                    ...
                    try:
                        ...
                        refreshed = refresh_anthropic_oauth_pure(
                            ...
                            use_json=synced.source.endswith("hairball_pkce"),
                        )
                        updated = replace(
                            synced,
                            access_token=refreshed["access_token"],
            ...
            if self.provider == "xai-oauth":
                ...
                # pool. Mirrors the managed quarantine path above.
                if auth_mod._is_terminal_xai_oauth_refresh_error(exc):
                    ...
                    return None
            # For openai-codex: same race as xAI/managed — another Hairball process
            ...
            if self.provider == "openai-codex":
                ...
                if synced.refresh_token != entry.refresh_token:
                    logger.debug(
                        "reference OAuth refresh failed but auth.json has newer tokens — adopting"
                    )
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                ...
                # in-memory pool.  Mirrors the xAI and managed quarantine paths.
                if auth_mod._is_terminal_codex_oauth_refresh_error(exc):
                    logger.debug(
                        "reference OAuth refresh token is terminally invalid; clearing local token state"
                    )
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                        ...
                        logger.debug(
                            "Failed to clear terminal reference OAuth state: %s", clear_exc
                        )
                    removed_ids = [
                        item.id for item in self._entries
                        if item.source == "device_code"
            ...
            if self.provider == "managed":
                synced = self._sync_managed_entry_from_auth_store(entry)
                if synced.refresh_token != entry.refresh_token:
                    logger.debug("managed refresh failed but auth.json has newer tokens — adopting")
                    updated = replace(
                        synced,
                        last_status=STATUS_OK,
                        last_status_at=None,
                ...
                if auth_mod._is_terminal_managed_refresh_error(exc):
                    logger.debug("managed refresh token is terminally invalid; clearing local token state")
                    try:
                        with _auth_store_lock():
                            auth_store = _load_auth_store()
                        ...
                        logger.debug("Failed to clear terminal managed OAuth state: %s", clear_exc)
    ...
    def _entry_needs_refresh(self, entry: PooledCredential) -> bool:
        ...
        if self.provider == "managed":
            # managed refresh can require network access and should happen when
    ...
    def _available_entries(self, *, clear_expired: bool = False, refresh: bool = False) -> List[PooledCredential]:
        ...
        for entry in self._entries:
            ...
            # by other processes (Claude Code CLI, other Hairball profiles).
            if (self.provider == "anthropic" and entry.source == "claude_code"
            ...
            # re-authed via `hairball model` / `hairball auth` after a 429/401,
            ...
            # tokens that another process (or a fresh `hairball model` ->
            ...
            if entry.last_status == STATUS_DEAD:
                ...
                # user can always re-add via ``hairball auth add``.  Singleton-
                ...
                if _is_manual_source(entry.source):
                    ...
                    if dead_at and now - dead_at > DEAD_MANUAL_PRUNE_TTL_SECONDS:
                        ...
                        logger.warning(
                            ...
                            "(reason=%s, age=%.1fh) — re-add via `hairball auth add %s`",
                            _label,
...
def _seed_from_singletons(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # `hairball auth remove <provider> <N>` is stable across all source types.
    try:
        from hairball_cli.auth import is_source_suppressed as _is_suppressed
    except ImportError:
        def _is_suppressed(_p, _s):  # type: ignore[misc]
            return False

    if provider == "anthropic":
        # Only auto-discover external credentials (Claude Code, Hairball PKCE)
        ...
        # API-key vs OAuth is a user-visible choice at `hairball setup` ("Claude
        ...
        # Claude Code CLI, hairball_pkce creds from a previous OAuth login)
        ...
        # that masquerade.  Prefer ~/.hairball/.env over os.environ for the
        # same reason `_seed_from_env` does — that's the authoritative file
        # that `hairball setup` writes.
        _env_file = load_env()
        ...
        if api_key_path_explicit:
            ...
            retained = [
                ...
                if entry.source not in {"hairball_pkce", "claude_code"}
            ]
        ...
        from agent.anthropic_adapter import read_claude_code_credentials, read_hairball_oauth_credentials

        for source_name, creds in (
            ("hairball_pkce", read_hairball_oauth_credentials()),
        ...
        if state and has_runtime_material and not _is_suppressed(provider, "device_code"):
            ...
            # `hairball auth add managed --label <name>`).  Fall back to the
        ...
        # Hairball auth store or env vars, so resolve them here.
        # Use refresh_if_expiring=False to avoid network calls during
        # pool loading / provider discovery.
        try:
            from hairball_cli.auth import resolve_qwen_runtime_credentials
        ...
        # MiniMax OAuth tokens live in ~/.hairball/auth.json providers.minimax-oauth.
        # Seed the pool so `/auth list` reflects the logged-in state and the
        # standard `hairball auth remove minimax-oauth <N>` flow works.
        ...
        # Respect user suppression — `hairball auth remove openai-codex` marks
        # the device_code source as suppressed so it won't be re-seeded from
        # the Hairball auth store.  Without this gate the removal is instantly
        ...
        # Hairball owns its own reference auth state — we do NOT auto-import from
        # ~/.codex/auth.json at pool-load time.  OAuth refresh tokens are
        # single-use, so sharing them with reference CLI / VS Code causes
        # refresh_token_reused race failures.  Users who want to adopt
        # existing reference CLI credentials get a one-time, explicit prompt
        # via `hairball auth openai-codex`.
        ...
        # When the user logs in via ``hairball model`` -> xAI Grok OAuth,
        # tokens are written to the auth.json singleton
        # (``providers["xai-oauth"]``).  Surface them in the pool too so
        # ``hairball auth list`` reflects the logged-in state and so the pool
        ...
        if isinstance(tokens, dict) and tokens.get("access_token"):
            ...
            # always surfaced as ``device_code`` (consistent with managed/codex).
            source = "device_code"
            if _is_suppressed(provider, source):
                return changed, active_sources
            active_sources.add(source)
            from hairball_cli.auth import DEFAULT_XAI_OAUTH_BASE_URL
...
def _seed_from_env(provider: str, entries: List[PooledCredential]) -> Tuple[bool, Set[str]]:
    ...
    # Prefer ~/.hairball/.env over os.environ — the user's config file is the
    # authoritative source for Hairball credentials. Stale env vars from parent
    # processes (reference CLI, test scripts, etc.) should not override deliberate
    ...
    def _get_env_prefer_dotenv(key: str) -> str:
        ...
        # load_hairball_dotenv() -> apply_onepassword_secrets()).  The raw
    ...
    # Honour user suppression — `hairball auth remove <provider> <N>` for an
    # env-seeded credential marks the env:<VAR> source as suppressed so it
    # won't be re-seeded from the user's shell environment or ~/.hairball/.env.
    ...
    try:
        from hairball_cli.auth import is_source_suppressed as _is_source_suppressed
    except ImportError:
    ...
    def _secret_source_for_env(env_var: str) -> Optional[str]:
        try:
            from hairball_cli.env_loader import get_secret_source
    ...
    if provider == "openrouter":
        # Prefer ~/.hairball/.env over os.environ
...
def _prune_stale_seeded_entries(
    ...
    *,
    ...
    def _is_prunable(entry: PooledCredential) -> bool:
        ...
        # (e.g. an `hairball auth` command that confirmed the source is gone).
        if entry.source.startswith("env:"):
            return prune_env_sources
        # File-backed singletons (device-code OAuth, claude_code) and Hairball
