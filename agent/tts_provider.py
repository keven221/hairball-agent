...
2. **Command-type providers** declared under ``tts.providers.<name>:
   ...
   CLI into Hairball with shell-template placeholders. **Wins over a
...
shipped today) or ``~/.hairball/plugins/tts/<name>/`` (user-installed).
...
``{success: False, error: …}`` JSON envelope the rest of Hairball
expects.
"""
...
class TTSProvider(abc.ABC):
    ...
    def display_name(self) -> str:
        """Human-readable label shown in ``hairball tools``.
    ...
    def is_available(self) -> bool:
        ...
        Must NOT raise — used by the picker and ``hairball setup`` for
