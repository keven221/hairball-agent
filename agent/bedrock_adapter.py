"""AWS Bedrock Converse API adapter for Hairball Agent.
...
Reference: legacy's ``extensions/amazon-bedrock/`` plugin, which implements
...
def _require_boto3():
    ...
    try:
        ...
        raise ImportError(
            ...
            "Or install Hairball with Bedrock support: pip install -e '.[bedrock]'"
        )
    # converse() / converse_stream() were added in boto3 1.34.59.
    # When Hairball is installed editable into system Python, the system boto3
...
# Priority order matches legacy's resolveAwsSdkEnvVarName():
...
def resolve_aws_auth_env_var(env: Optional[Dict[str, str]] = None) -> Optional[str]:
    ...
    This mirrors legacy's ``resolveAwsSdkEnvVarName()`` — used to detect
...
def has_aws_credentials(env: Optional[Dict[str, str]] = None) -> bool:
    ...
    This two-tier approach mirrors the pattern from legacy PR #62673:
...
def discover_bedrock_models(
    ...
    Mirrors legacy's ``discoverBedrockModels()`` in
...
# ---------------------------------------------------------------------------
# Mirrors legacy's classifyFailoverReason() and matchesContextOverflowError()
