...
Optional hooks (override to opt in):
  ...
  backup_paths() -> list[str]            — extra on-disk paths to include in `hairball backup`
"""
...
class MemoryProvider(ABC):
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        kwargs always include:
          - hairball_home (str): The active HAIRBALL_HOME directory path. Use this
            for profile-scoped storage instead of hardcoding ``~/.hairball``.
        ...
        kwargs may also include:
          ...
          - agent_workspace (str): Shared workspace name (e.g. "hairball").
    ...
    def get_config_schema(self) -> List[Dict[str, Any]]:
        ...
        Used by 'hairball memory setup' to walk the user through configuration.
    ...
    def save_config(self, values: Dict[str, Any], hairball_home: str) -> None:
        """Write non-secret config to the provider's native location.

        Called by 'hairball memory setup' after collecting user inputs.
        ``values`` contains only non-secret fields (secrets go to .env).
        ``hairball_home`` is the active HAIRBALL_HOME directory path.
    ...
    def backup_paths(self) -> List[str]:
        """Return extra on-disk paths this provider stores OUTSIDE HAIRBALL_HOME.

        ``hairball backup`` only walks HAIRBALL_HOME, so any provider state kept
        ...
        of the archive, and ``hairball import`` restores them to their original
