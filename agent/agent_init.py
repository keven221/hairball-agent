...
from hairball_cli.timeouts import get_provider_request_timeout
...
def _build_codex_gpt5_autoraise_notice(autoraise: Dict[str, Any]) -> str:
    """Build the one-time notice shown when reference gpt-5.x raises compaction.
    ...
    # is capped at 272K by the reference OAuth backend.
    cap = "128K" if model.startswith("gpt-5.3-codex-spark") else "272K"
    from_pct = int(round(autoraise["from"] * 100))
    to_pct = int(round(autoraise["to"] * 100))
    return (
        f"ℹ reference {model} caps context at {cap}, so auto-compaction was raised "
        f"to {to_pct}% (from {from_pct}%) to use more of the window before "
        f"summarizing.\n"
        f"  Opt back out: hairball config set compression.codex_gpt55_autoraise false"
    )
...
def _resolve_compression_threshold(
    ...
    *,
    ...
    ``{"model": <slug>, "from": <old>, "to": <new>}`` only when a reference
    autoraise (gpt-5.4/5.5 272K family or gpt-5.3-codex-spark) actually raises
    the threshold, otherwise ``None``.

    The reference overrides are *autoraises*: they must never LOWER a higher
...
def _codex_gpt55_autoraise_notice_marker():
    ...
    Lives under ``$HAIRBALL_HOME`` (which is profile-scoped) alongside the other
    ...
    """
    return get_hairball_home() / ".codex_gpt55_autoraise_notice"
...
def _codex_gpt55_autoraise_notice_state(autoraise: Dict[str, Any]) -> str:
    ...
    different autoraised reference model) re-notifies once.
    """
...
def _record_codex_gpt55_autoraise_notice(autoraise: Dict[str, Any]) -> None:
    ...
    Best-effort: a read-only or missing ``$HAIRBALL_HOME`` just means the notice
...
def init_agent(
    ...
    Args:
        ...
        visible_toolsets (List[str]): Toolsets kept in the initial model schema;
            other authorized tools remain available through Tool Search.
        ...
        skip_context_files (bool): If True, skip auto-injection of project context files
            (SOUL.md, .hairball.md, AGENTS.md, CLAUDE.md, .cursorrules) from the cwd / HAIRBALL_HOME
            into the system prompt. Use this for batch processing and data generation to avoid
            polluting trajectories with user-specific persona or project instructions.
        load_soul_identity (bool): If True, still use ~/.hairball/SOUL.md as the primary
    ...
    # Enterprise policy is enforced at the shared AIAgent seam so desktop,
    # CLI/TUI, gateway/API, cron/batch, group chat, background runs, and
    # delegated children cannot accidentally disagree. With governance absent
    # this returns the original values unchanged and adds no model schema.
    from hairball_cli.governance import apply_runtime_policy, filter_provider_routes

    _policy_scope = apply_runtime_policy(
        provider=agent.provider or "auto",
        enabled_toolsets=enabled_toolsets,
        disabled_toolsets=disabled_toolsets,
        visible_toolsets=visible_toolsets,
    )
    enabled_toolsets = _policy_scope.enabled_toolsets
    disabled_toolsets = _policy_scope.disabled_toolsets
    visible_toolsets = _policy_scope.visible_toolsets
    if isinstance(fallback_model, list):
        fallback_model = filter_provider_routes(fallback_model)
    elif isinstance(fallback_model, dict):
        _allowed_fallbacks = filter_provider_routes([fallback_model])
        fallback_model = _allowed_fallbacks[0] if _allowed_fallbacks else None
    ...
    # /steer and /inject share the durable next-step inbox. It is opened lazily
    # after session_id and the optional SessionDB are bound below, so parked
    # input can replay from the canonical ledger before the first model step.
    agent._turn_inbox = None
    ...
    agent.visible_toolsets = visible_toolsets
    ...
    agent._cache_ttl = "5m"
    try:
        from hairball_cli.config import load_config as _load_pc_cfg
    ...
    # x-managed-credits-* response headers after each API call.  Session-start
    # remaining is latched the first time a header is ever seen so we can
    # report cumulative micros spent.  Surfaced behind HAIRBALL_DEV_CREDITS.
    ...
    # both live under ~/.hairball/logs/.  Idempotent, so gateway mode
    # (which creates a new AIAgent per message) won't duplicate handlers.
    from hairball_logging import setup_logging, setup_verbose_logging
    setup_logging(hairball_home=_ra()._hairball_home)
    ...
    if agent.verbose_logging:
        ...
        # on the fact that hairball_logging.setup_logging() does not
        ...
        # handler level inside hairball_logging.py, not here.
        pass
    ...
    # reference/Anthropic wrapping for all known providers.
    # raw_codex=True because the main agent needs direct responses.stream()
    # access for reference Responses API streaming.
    ...
    if agent.api_mode == "anthropic_messages":
        ...
        if _is_bedrock_anthropic:
            ...
            if agent.provider == "minimax-oauth" and isinstance(effective_key, str) and effective_key:
                try:
                    from hairball_cli.auth import build_minimax_oauth_token_provider
        ...
        try:
            from hairball_cli.config import load_config as _load_br_cfg
        ...
        if api_key and base_url:
            ...
            if _routed_client is not None:
                ...
                if not getattr(agent, "_fallback_activated", False):
                    ...
                    raise RuntimeError(
                        "No LLM provider configured. Run `hairball model` to "
                        "select a provider, or run `hairball setup` for first-time "
                        "configuration."
                    )
    ...
    # worker (kanban_show tool is present iff HAIRBALL_KANBAN_TASK is set).
    ...
    try:
        ...
        os.environ["HAIRBALL_SESSION_ID"] = agent.session_id

    # Session logs go into ~/.hairball/sessions/ alongside gateway sessions
    hairball_home = get_hairball_home()
    agent.logs_dir = hairball_home / "sessions"
    agent.logs_dir.mkdir(parents=True, exist_ok=True)
    # Per-session JSON snapshot writer (~/.hairball/sessions/session_{sid}.json)
    ...
    try:
        from hairball_cli.config import load_config as _load_sess_cfg
    ...
    try:
        from hairball_cli.config import load_config as _load_agent_config
            ...
            if _mem_provider_name and _mem_provider_name.strip():
                ...
                if agent._memory_manager.providers:
                    ...
                    try:
                        ...
                        _init_kwargs["agent_workspace"] = "hairball"
                    except Exception:
                        pass
    ...
    # Preserve 9810's model-neutral finish-and-verify prompt contract.  Pin the
    # toggle for the session so a config edit cannot mutate the cached prefix
    # in the middle of a conversation.
    agent._task_completion_guidance = bool(
        _agent_section.get("task_completion_guidance", True)
    )

    # Pin Todo lifecycle policy for the session. StopController reads only
    # this normalized copy, so changing config mid-conversation cannot change
    # control flow or invalidate the cached prompt prefix.
    _todo_reminder_cfg = _agent_section.get("todo", {})
    if not isinstance(_todo_reminder_cfg, dict):
        _todo_reminder_cfg = {}
    try:
        _todo_reminders_max = max(
            0, min(10, int(_todo_reminder_cfg.get("reminders_max", 3)))
        )
    except (TypeError, ValueError):
        _todo_reminders_max = 3
    _todo_eager = (
        str(_todo_reminder_cfg.get("eager", "default") or "default")
        .strip()
        .casefold()
    )
    if _todo_eager not in {"default", "preferred", "always"}:
        _todo_eager = "default"
    agent._todo_reminder_settings = {
        "enabled": bool(_todo_reminder_cfg.get("enabled", True)),
        "reminders": bool(_todo_reminder_cfg.get("reminders", True)),
        "reminders_max": _todo_reminders_max,
        "mid_run_nudges": bool(_todo_reminder_cfg.get("mid_run_nudges", True)),
        "eager": _todo_eager,
    }

    # One session-stable Stop-hook boundary, shared by both agent loops. Install
    # it before the first provider call so runtime state cannot alter the prompt
    # or model-tool schema mid-conversation.
    from agent.stop_controller import StopController

    StopController.install(agent)

    # Universal parallel-tool-call guidance toggle.  Default True and
    # independently configurable.  It
    # steers the model to batch independent tool calls into a
    ...
    # Lets an enterprise admin append to or replace Hairball' built-in
    ...
    # Per-model/route compaction-threshold override. reference gpt-5.4 / gpt-5.5
    # raise to 85% (the reference backend caps both families at 272K, so the
    ...
    try:
        ...
        # The reference autoraises (gpt-5.4/5.5 272K family and gpt-5.3-codex-spark)
    ...
    # Minimum REAL user messages guaranteed to survive in the uncompressed
    # tail (compression.min_tail_user_messages). Default 1 preserves current
    # behavior — the existing single-user tail anchor. Values > 1 extend
    # the guarantee. Booleans rejected (bool subclasses int).
    ...
    if codex_app_server_auto_compaction not in {"native", "hairball", "off"}:
        _ra().logger.warning(
            "Invalid compression.codex_app_server_auto=%r; using 'native'. "
            "Valid values are: native, hairball, off.",
    ...
    if _config_context_length is None and _custom_providers:
        try:
            from hairball_cli.config import get_custom_provider_context_length
    ...
    if _engine_name != "compressor":
        ...
        if _selected_engine is None:
            _candidate = None
            try:
                from hairball_cli.plugins import get_plugin_context_engine
    ...
    if _selected_engine is not None:
        ...
        # threshold (including the reference gpt-5.5 autoraise above) only
    ...
    if _ctx and _ctx < MINIMUM_CONTEXT_LENGTH:
        raise ValueError(
            ...
            f"by Hairball Agent.  Choose a model with at least "
    ...
    # managed Hairball 3/4 are chat models, not tool-call-tuned. The interactive
    ...
    if not agent.quiet_mode and (agent.platform or "cli") != "cli":
        try:
            from hairball_cli.model_switch import _check_hairball_model_warning

            _hairball_warn = _check_hairball_model_warning(agent.model or "")
            if _hairball_warn:
                _user_msg = (
                    "⚠ Hairball 3 & 4 models are NOT agentic — they "
                ...
                if hasattr(agent, "_emit_warning"):
                    ...
                    print(f"\n{_user_msg}\n", file=sys.stderr)
                _ra().logger.warning(_hairball_warn)
        except Exception:
            pass
    ...
    # Finalize one session-scoped execution registry only after every
    # agent-owned schema family (memory/context/bridges) has been injected.
    # ``agent.tools`` remains the model-visible projection; deferred registry
    # tools stay executable through their explicit discovery bridge without
    # being added to that initial schema.
    agent._session_tool_router = _ra().build_session_tool_router(
        model_visible_specs=agent.tools or (),
    ...
    agent.session_cost_actual_call_count = 0
    agent.session_cost_total_call_count = 0
    ...
    # reference gpt-5.x autoraise notice: show at most once per profile/config
    ...
    # Gateway parity for the reference gpt-5.x autoraise notice: the startup print
