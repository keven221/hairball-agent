...
def make_tool_result_message(
    ...
    *,
    ...
    Content from externally controlled tools (``web_*``, ``browser_*``,
    ``mcp_*``, computer-use/vision observations) gets wrapped in semantic
    delimiters telling the model the content is untrusted data, not
    instructions. Local content-bearing tools (file/search/terminal/code/LSP)
    use the same delimiter only when the advisory scanner finds an injection
    signal. This closes poisoned-repository and command-output paths without
    adding wrapper tokens to ordinary local work.
    ...
    try:
        ...
        risk_metadata = None
    wrapped = _maybe_wrap_untrusted(name, content, risk_metadata=risk_metadata)
    message = {
        "role": "tool",
        "name": name,
        "tool_name": name,
...
# Tools whose results are controlled by a remote page/service/visual surface.
# Their longer text is always framed. A short result is also framed whenever
# the deterministic scanner finds a threat, closing the old <32-char bypass.
_ALWAYS_UNTRUSTED_TOOL_NAMES = frozenset({
    "computer_use",
    "vision_analyze",
    "web_extract",
    "web_search",
...
_ALWAYS_UNTRUSTED_TOOL_PREFIXES = (
    "browser_",
    "mcp_",
)

# Local tools normally return trusted operational data, so wrapping every
# result would add prompt tokens and make coding output harder to read. Their
# text is nevertheless attacker-controllable when a repository, build, test,
# terminal pane, or language server contains poisoned content. Scan these
# outputs and frame only positive findings. Clean output stays byte-identical.
_SCAN_GATED_UNTRUSTED_TOOL_NAMES = frozenset({
    "ast_grep",
    "debug",
    "execute_code",
    "lsp",
...
def _is_untrusted_tool(name: Optional[str]) -> bool:
    """Return whether textual output needs unconditional or scan-gated care."""
    if not name:
        return False
    if name in _ALWAYS_UNTRUSTED_TOOL_NAMES or name in _SCAN_GATED_UNTRUSTED_TOOL_NAMES:
        return True
    return any(name.startswith(p) for p in _ALWAYS_UNTRUSTED_TOOL_PREFIXES)


def _tool_output_trust_mode(name: Optional[str]) -> str:
    """Return ``external``, ``scan_gated``, or ``trusted`` for one tool."""
    if not name:
        return "trusted"
    if name in _ALWAYS_UNTRUSTED_TOOL_NAMES or any(
        name.startswith(prefix) for prefix in _ALWAYS_UNTRUSTED_TOOL_PREFIXES
    ):
        return "external"
    if name in _SCAN_GATED_UNTRUSTED_TOOL_NAMES:
        return "scan_gated"
    return "trusted"
...
def _tool_output_risk_metadata(name: str, content: Any) -> Optional[Dict[str, Any]]:
    ...
    """
    trust_mode = _tool_output_trust_mode(name)
    ...
    if trust_mode == "scan_gated" and not findings:
        return None
    return {
...
def _maybe_wrap_untrusted(
    ...
    *,
    ...
    """Frame external content, plus suspicious local output, as data.
    ...
    - the tool is not in either untrusted-output set
    - the tool is scan-gated and no threat finding exists
    - the content is neither a string nor a list (dict, None, …)
    - external string content is short and has no threat finding
    ...
    if risk_metadata is None:
        try:
            ...
            logger.debug("Tool output risk scan failed while framing %s: %s", name, exc)
            risk_metadata = None
    high_risk = bool(
        isinstance(risk_metadata, dict)
        and risk_metadata.get("risk") != "low"
        and risk_metadata.get("findings")
    )
    if trust_mode == "scan_gated" and not high_risk:
        return content
    if isinstance(content, str):
        if len(content) < _UNTRUSTED_WRAP_MIN_CHARS and not high_risk:
            return content
