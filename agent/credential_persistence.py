...
``auth.json``.  They intentionally have no dependency on ``hairball_cli.auth`` so
...
# Sources Hairball owns and can intentionally persist in auth.json.  Everything
...
def sanitize_borrowed_credential_payload(
    ...
    Owned sources (manual entries and Hairball-owned OAuth/device-code state)
