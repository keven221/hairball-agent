...
from hairball_cli.timeouts import get_provider_request_timeout, get_provider_stale_timeout
from hairball_constants import PARTIAL_STREAM_STUB_ID, FINISH_REASON_LENGTH
...
def estimate_request_context_tokens(api_payload: Any) -> int:
    ...
    reference / Responses API requests carry the conversational payload in
    ``input`` (with additional load in ``instructions`` and ``tools``), so the
    legacy estimator reported ~0 tokens for every reference turn and the
...
def openai_codex_stale_timeout_floor(est_tokens: int) -> float:
    ...
    instructions before the first user message. Subscription-backed reference can
...
def _check_stale_giveup(agent) -> None:
    ...
    _giveup = env_int("HAIRBALL_STREAM_STALE_GIVEUP", 5)
...
def should_use_direct_api_call(agent) -> bool:
    ...
    gateway cron thread stack. Keep native/reference/Bedrock/MoA transports on their
...
def direct_api_call(agent, api_kwargs: dict):
    ...
    timeout (provider ``request_timeout_seconds`` / ``HAIRBALL_API_TIMEOUT``) bounds
...
def interruptible_api_call(agent, api_kwargs: dict):
    ...
    # ── reference Responses stream watchdogs ────────────────────────────────
    ...
    # main retry loop can reconnect promptly. Large subscription-backed reference
    ...
    # read; for that we watch the gap since the last reference stream event. This
    # matches reference CLI's stream_idle_timeout model: any valid SSE event is
    # activity. Operators can tune via HAIRBALL_CODEX_TTFB_TIMEOUT_SECONDS and
    # HAIRBALL_CODEX_EVENT_STALE_TIMEOUT_SECONDS (0 disables each).
    ...
    # killed subscription-backed reference requests mid-prefill before the backend
    ...
    # HAIRBALL_CODEX_TTFB_TIMEOUT_SECONDS=0 to disable this watchdog entirely.
    _ttfb_enabled = _codex_watchdog_enabled
    _ttfb_timeout = _env_float("HAIRBALL_CODEX_TTFB_TIMEOUT_SECONDS", 120.0)
    if _ttfb_timeout <= 0:
        _ttfb_enabled = False
    elif _openai_codex_backend:
        _ttfb_disable_above = _env_float("HAIRBALL_CODEX_TTFB_DISABLE_ABOVE_TOKENS", 10_000.0)
        _ttfb_strict = os.environ.get("HAIRBALL_CODEX_TTFB_STRICT", "").strip().lower() in {
            "1", "true", "yes", "on"
        }
        if (
            not _ttfb_strict
            ...
            logger.info(
                ...
                "Set HAIRBALL_CODEX_TTFB_STRICT=1 to force early reconnects.",
                f"{_est_tokens_for_codex_watchdog:,}",
                _ttfb_disable_above,
            )
        else:
            _ttfb_cap = _env_float("HAIRBALL_CODEX_TTFB_MAX_SECONDS", 120.0)
            if _ttfb_cap > 0 and _ttfb_timeout > _ttfb_cap:
                logger.info(
                    "Capping openai-codex no-byte TTFB timeout from %.0fs to %.0fs "
                    "(context=~%s tokens). Set HAIRBALL_CODEX_TTFB_MAX_SECONDS to tune.",
                    _ttfb_timeout,
                    _ttfb_cap,
                    f"{_est_tokens_for_codex_watchdog:,}",
                )
    ...
    _codex_idle_timeout = _env_float(
        "HAIRBALL_CODEX_EVENT_STALE_TIMEOUT_SECONDS",
    ...
    while t.is_alive():
        ...
        # TTFB detector: the reference stream has produced no event at all and
        ...
        if (
            ...
            if callable(_hint_fn):
                try:
                    ...
                    _silent_hint = None
            logger.warning(
                "reference stream produced no bytes within TTFB cutoff "
            ...
            if result["error"] is None and result["response"] is None:
                if _silent_hint:
                    result["error"] = TimeoutError(
                        f"reference stream produced no bytes within {int(_elapsed)}s "
        ...
        # Stream-idle detector: the reference backend emitted at least one SSE
        ...
        if (
            ...
            logger.warning(
                "reference stream produced no SSE events for %.0fs after first byte "
            ...
            agent._buffer_status(
                f"⚠️ reference stream sent no events for {int(_event_stale_elapsed)}s "
            ...
            if result["error"] is None and result["response"] is None:
                result["error"] = TimeoutError(
                    f"reference stream produced no SSE events for {int(_event_stale_elapsed)}s "
...
def build_api_kwargs(agent, api_messages: list) -> dict:
    ...
    _is_managed = "hairball" in agent._base_url_lower
    ...
    # like write_file/patch.  OpenRouter/managed were the only routes covered
    ...
    if _is_qwen:
        _qwen_meta = {
            "sessionId": agent.session_id or "hairball",
...
def build_assistant_message(agent, assistant_message, finish_reason: str) -> dict:
    ...
    # Respects HAIRBALL_REDACT_SECRETS via redact_sensitive_text — no-op
    # when disabled. (#19798)
    if isinstance(_san_content, str) and _san_content:
    ...
    # reference Responses API: preserve encrypted reasoning items for
    # multi-turn continuity. These get replayed as input on the next turn.
    codex_items = getattr(assistant_message, "codex_reasoning_items", None)
    if codex_items:
        msg["codex_reasoning_items"] = codex_items

    # reference Responses API: preserve exact assistant message items (with
...
def _fallback_entry_unavailable_without_network(agent, fb: dict) -> Optional[str]:
    ...
    try:
        ...
        return f"managed_auth_unreadable:{type(exc).__name__}"
...
def try_activate_fallback(agent, reason: "FailoverReason | None" = None) -> bool:
    ...
    # access for reference providers.
    try:
        ...
        if not fb_api_key_hint:
            ...
            # _normalize_custom_provider_entry in hairball_cli/config.py).
...
def handle_max_iterations(agent, messages: list, api_call_count: int) -> str:
    ...
    print(_summary_msg)
    _progress_cb = getattr(agent, "tool_progress_callback", None)
    if callable(_progress_cb):
        try:
            _progress_cb("subagent.progress", preview=_summary_msg.strip())
        except Exception:
            pass
    ...
    try:
        ...
        for msg in messages:
            ...
            # and every Hairball-internal underscore-prefixed scaffolding key.
        ...
        if _is_managed:
            from agent.portal_tags import managed_portal_tags as _portal_tags
...
def interruptible_streaming_api_call(agent, api_kwargs: dict, *, on_first_delta=None):
    ...
    if agent.api_mode == "codex_responses":
        # reference streams internally via _run_codex_stream. The main dispatch
    ...
    def _call_chat_completions():
        ...
        # wins over the HAIRBALL_API_TIMEOUT env default if the user set it.
        _provider_timeout_cfg = get_provider_request_timeout(agent.provider, agent.model)
        _base_timeout = (
            _provider_timeout_cfg
            if _provider_timeout_cfg is not None
            else env_float("HAIRBALL_API_TIMEOUT", 1800.0)
        )
        # Read timeout: config wins here too.  Otherwise use
        # HAIRBALL_STREAM_READ_TIMEOUT (default 120s) for cloud providers.
        if _provider_timeout_cfg is not None:
            _stream_read_timeout = _provider_timeout_cfg
        else:
            _stream_read_timeout = env_float("HAIRBALL_STREAM_READ_TIMEOUT", 120.0)
            # Local providers (Ollama, llama.cpp, vLLM) can take minutes for
            # prefill on large contexts before producing the first token.
            # Auto-increase the httpx read timeout unless the user explicitly
            # overrode HAIRBALL_STREAM_READ_TIMEOUT.
        ...
        # stream is somehow still alive, this claim supersedes it so its late
        # chunks are fenced out of the turn instead of interleaving with ours.
        ...
        usage_obj = None
        for chunk in stream:
            # Stop the moment a newer attempt has claimed the delta sink (#65991).
        ...
        #      managed dedicated endpoint) stall for minutes during large
    ...
    def _call():
        ...
        _max_stream_retries = env_int("HAIRBALL_STREAM_RETRIES", 2)
    ...
    if _cfg_stale is not None:
        ...
        _stream_stale_timeout_base = env_float("HAIRBALL_STREAM_STALE_TIMEOUT", 180.0)
    # Local providers (Ollama, oMLX, llama-cpp) can take 300+ seconds
    # for prefill on large contexts.  Disable the stale detector unless
    # the user explicitly set HAIRBALL_STREAM_STALE_TIMEOUT.
