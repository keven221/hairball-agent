...
# Internal retry/continuation turns are API control flow, not user-visible
# conversation history.  Keep this classifier here (rather than in run_agent)
# because CLI, TUI, UI, gateway and kernel-v2 all replay through this module.
# A new continuation flag is incomplete until it is listed here and exercised
# by every durable-transcript path.
EPHEMERAL_SCAFFOLDING_FLAGS = (
    ...
    "_completion_audit_synthetic",
    "_kanban_stop_synthetic",
)


def is_ephemeral_scaffolding(message: Any) -> bool:
    """Whether *message* is internal control flow and must not be replayed."""
...
def is_real_user_turn(message: Any) -> bool:
    """Whether a row is a user-authored, rewindable conversation turn.

    A typed Stop continuation is stored as role=user for the model,
    but is not a user action and must not shift titles, rewind ordinals,
    branches, retry, or history pickers.
    """
    if not isinstance(message, dict) or message.get("role") != "user":
        return False
    try:
        from agent.stop_controller import (
            HOOK_PROMPT_FINISH_REASON,
            is_hook_prompt_message,
        )

        if (
            message.get("finish_reason") == HOOK_PROMPT_FINISH_REASON
            or is_hook_prompt_message(message)
        ):
            return False
    except Exception:
        logger.debug("Hook-prompt classification failed open", exc_info=True)
    if message.get("synthetic") is True or is_ephemeral_scaffolding(message):
        return False
    if str(message.get("synthetic_kind") or "").casefold() in {
        "async_delegation",
        "completion",
        "slash_exec",
    }:
        return False
    if message.get("slash_exec") is True or message.get("rewindable") is False:
        return False
    body = str(message.get("content") or "").strip().upper()
    return not body.startswith("[ASYNC DELEGATION")


def strip_ephemeral_scaffolding(
    agent_history: List[Dict[str, Any]],
) -> List[Dict[str, Any]]:
    """Remove only marked internal turns, preserving real message identity/order."""
    if not agent_history:
        return agent_history
    kept = [message for message in agent_history if not is_ephemeral_scaffolding(message)]
    return agent_history if len(kept) == len(agent_history) else kept
...
def strip_dangling_tool_call_tail(
    ...
    ``systemctl restart``, ``kill``, ``hairball gateway restart``), the process
    ...
    if any(
        ...
        for call in tool_calls:
            ...
            content = (
                "[Orphan recovery: this tool may have executed before Hairball stopped; "
...
def sanitize_replay_history(
    ...
    if not agent_history:
        return agent_history
    without_scaffolding = strip_ephemeral_scaffolding(agent_history)
    return strip_dangling_tool_call_tail(strip_interrupted_tool_tails(without_scaffolding))
