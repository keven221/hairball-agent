...
Storage
-------
Bundles live in ``~/.hairball/skill-bundles/*.yaml`` (and the equivalent
profile-aware directory under ``HAIRBALL_HOME``). Each file looks like::
...
- :func:`list_bundles` — return rich info for display (``hairball bundles``)
...
def _bundles_dir() -> Path:
    """Return the canonical bundles directory under HAIRBALL_HOME.

    Honors ``HAIRBALL_BUNDLES_DIR`` for tests; falls back to
    ``<HAIRBALL_HOME>/skill-bundles``.
    """
    override = os.environ.get("HAIRBALL_BUNDLES_DIR")
    if override:
        return Path(override).expanduser()
    return get_hairball_home() / "skill-bundles"
...
# ---------------------------------------------------------------------------
# File-level CRUD helpers — used by `hairball bundles` CLI subcommand.
# ---------------------------------------------------------------------------
