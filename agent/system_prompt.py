...
upstream prefix cache warm.  See ``hairball-agent-dev``'s
...
def _tui_embedded_pane_clarifier(hint: str) -> str:
    ...
    Triggered by ``HAIRBALL_DESKTOP_TERMINAL=1`` (set by ``main.cjs`` only on the
    ...
    if _TUI_EMBEDDED_PANE_CLARIFIER in hint:
        return hint
    if not is_truthy_value(os.getenv("HAIRBALL_DESKTOP_TERMINAL")):
        return hint
...
def build_system_prompt_parts(agent: Any, system_message: Optional[str] = None) -> Dict[str, str]:
    ...
    AIAgent.  Hairball never re-renders parts of this string mid-
    ...
    # Some execution modes (cron) still want HAIRBALL_HOME persona while keeping
    ...
    # Pointer to the hairball-agent skill + docs for user questions about Hairball itself.
    stable_parts.append(HAIRBALL_AGENT_HELP_GUIDANCE)

    # Preserve the frozen product's model-neutral completion discipline.  This
    # is prompt guidance, not a stop gate: it never forces a tool, appends a
    # message, or changes the loop.  Resolve the config once on agent creation
    # so the stable prompt remains byte-identical for the session lifetime.
    ...
    # HAIRBALL_KANBAN_TASK env var). Normal chat sessions never see
    ...
    if agent.valid_tool_names:
        ...
        if _inject:
            ...
            # Concise file/tool discipline for selected model families.
            if "gemini" in _model_lower or "gemma" in _model_lower:
                stable_parts.append(CONCISE_PARALLEL_TOOL_GUIDANCE)
            # Capability-selected execution discipline: tool persistence,
            # prerequisite checks, verification and grounded tool use.
    ...
    # Coding posture (base Hairball, any interactive coding surface in a code
    ...
    # Active-profile hint — names the Hairball profile the agent is running
    # under so it doesn't conflate ~/.hairball/skills/ (default profile) with
    # ~/.hairball/profiles/<active>/skills/ (this profile's). Deterministic
    ...
    if active_profile == "default":
        stable_parts.append(
            "Active Hairball profile: default. Other profiles (if any) live "
            "under ~/.hairball/profiles/<name>/. Each profile has its own "
        ...
        stable_parts.append(
            f"Active Hairball profile: {active_profile}. This session reads "
            f"and writes ~/.hairball/profiles/{active_profile}/. The default "
            f"profile's data lives at ~/.hairball/skills/, ~/.hairball/plugins/, "
            f"~/.hairball/cron/, ~/.hairball/memories/ — those belong to a "
...
def build_system_prompt(agent: Any, system_message: Optional[str] = None) -> str:
    ...
    one cached block — Hairball never rebuilds or reinjects parts of it
