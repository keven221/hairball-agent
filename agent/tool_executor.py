...
def tool_progress_api_call_id_enabled() -> bool:
    """Gate: pass API tool_call_id on tool.started/completed progress (Phase2).

    Aligns with reference/reference/reference/Cursor single-rail ledger. Default on.
    Set ``HAIRBALL_UI_TOOL_PROGRESS_API_CALL_ID=0|false|off`` to roll back
    (live subagent progress falls back to ``name:n`` again).
    """
    raw = str(os.environ.get("HAIRBALL_UI_TOOL_PROGRESS_API_CALL_ID", "1") or "1").strip().lower()
    return raw not in {"0", "false", "off", "no"}


def _progress_tool_call_id_kwargs(tool_call: Any) -> dict[str, str]:
    """Kwargs fragment for tool_progress_callback — empty when gated off or no id."""
    if not tool_progress_api_call_id_enabled():
        return {}
    call_id = str(getattr(tool_call, "id", None) or "").strip()
    if not call_id:
        return {}
    return {"tool_call_id": call_id}


def _emit_native_tool_started(agent: Any, tool_call: Any, name: str, arguments: dict) -> None:
    """Record the real execution boundary without touching UI callbacks."""
    recorder = getattr(agent, "_native_turn_events", None)
    if recorder is None:
        return
    try:
        recorder.tool_started(getattr(tool_call, "id", None), name, arguments)
    except Exception:
        logger.debug("native tool-start event failed", exc_info=True)


def _emit_native_tool_completed(
    agent: Any,
    tool_call: Any,
    name: str,
    arguments: dict,
    ...
    """Record completion at the executor boundary, never by callback capture."""
    recorder = getattr(agent, "_native_turn_events", None)
    if recorder is None:
        return
    try:
        recorder.tool_completed(
            getattr(tool_call, "id", None), name, arguments, result,
            duration_seconds, is_error,
        )
    except Exception:
        logger.debug("native tool-complete event failed", exc_info=True)
...
def _resolve_concurrent_tool_timeout() -> float | None:
    raw = os.getenv("HAIRBALL_CONCURRENT_TOOL_TIMEOUT_S", "").strip()
    if not raw:
    ...
    try:
        ...
        logger.warning(
            "invalid HAIRBALL_CONCURRENT_TOOL_TIMEOUT_S=%r; using %.0fs",
            raw,
...
def _flush_session_db_after_tool_progress(
    ...
    *,
    ...
    current Hairball process before the normal turn-end persistence path runs.
...
def _emit_tool_output_risk(
    ...
    *,
    ...
    """Project one high-risk tool result to UI callbacks and durable audit.

    Raw tool output deliberately never enters either projection. The model's
    normal tool result remains available (inside a semantic data boundary), so
    this observation does not block, redact, or reduce model autonomy.

    ``audit_config`` is an explicit test/embedding seam. Production callers
    omit it and the existing profile-local governance configuration decides
    whether the hash-chained audit ledger is enabled or required.
    """
    if not isinstance(risk_metadata, dict) or risk_metadata.get("risk") == "low":
        return
    findings = [
        str(item)[:120]
        for item in risk_metadata.get("findings", [])
        if str(item).strip()
    ][:32]
    safe_metadata = {
        "risk": str(risk_metadata.get("risk") or "high")[:32],
        "findings": findings,
        "redacted": bool(risk_metadata.get("redacted", False)),
    }
    ...
    if callback:
        try:
            callback(
                ...
                risk_metadata=safe_metadata,
            )
        except Exception as exc:
            logger.debug("Tool output risk callback error: %s", exc)

    # This is the BB22 governance seam, not a parallel security log. It is a
    # no-op unless governance.audit.enabled; required audit failures preserve
    # the existing fail-closed policy by propagating from audit_event_if_enabled.
    from hairball_cli.governance import audit_event_if_enabled

    audit_event_if_enabled(
        {
            "event": "security.tool_output_risk",
            "actor": str(getattr(agent, "user_id", None) or ""),
            "subject": str(tool_name or "")[:160],
            "payload": {
                "session_id": str(getattr(agent, "session_id", None) or "")[:240],
                "platform": str(getattr(agent, "platform", None) or "")[:80],
...
def _record_completion_tool_evidence(
    agent,
    *,
    function_name: str,
    function_result: Any,
    ...
    """Adapt the v1 executor to the shared completion-contract interface."""
    try:
        tracker = getattr(agent, "_completion_contract_runtime", None)
        if tracker is None:
            return
        ...
        logger.debug("completion contract tool evidence skipped", exc_info=True)
...
def _tool_search_scoped_names(agent) -> frozenset:
    ...
    visible = getattr(agent, "visible_toolsets", None)
    session_router = model_tools.session_tool_router_for_agent(agent)
    cache_key = (
        getattr(_registry, "_generation", 0),
        session_router.registry_generation,
        frozenset(enabled) if enabled is not None else None,
        frozenset(disabled) if disabled is not None else None,
        frozenset(visible) if visible is not None else None,
    )
    ...
    try:
        ...
        forced = model_tools._forced_deferred_tool_names(scoped_defs, visible)
        names = _ts.scoped_deferrable_names(
            scoped_defs,
            forced_deferred_names=forced,
        )
        names = frozenset(names) & session_router.authorized_tool_names
    except Exception:
        names = frozenset()
    try:
...
def _session_tool_scope_block(agent: Any, function_name: str) -> Optional[dict[str, str]]:
    """Resolve the model-originated call against the session execution router."""
    try:
        import model_tools

        result = model_tools.session_tool_router_for_agent(
            agent
        ).authorization_error(function_name)
    except Exception:
        result = json.dumps(
            {
                "error": f"'{function_name}' is not available in this session.",
                "error_type": "tool_not_enabled",
            },
            ensure_ascii=False,
        )
    if result is None:
    ...
    return {
        "result": result,
        "message": str(payload.get("error") or result),
        "error_type": str(payload.get("error_type") or "tool_not_enabled"),
    }


def _plan_mode_tool_block(
    agent: Any,
    function_name: str,
    function_args: dict[str, Any],
) -> Optional[dict[str, str]]:
    """Return a structured Plan denial or ``None``.

    This is intentionally an execution-time gate.  The model-facing tool
    schema remains stable for the lifetime of a conversation, preserving the
    prompt-cache prefix, while every real dispatch path applies the same
    decision after ``tool_call`` has been unwrapped to its underlying tool.
    """

    mode = str(getattr(agent, "agent_mode", "build") or "build")
    from agent.plan_execution import bind_plan_tool_context, gate_side_effect

    plan_file = getattr(agent, "_plan_file_path", None)
    # Bind before the fast path so a Build call on a reused worker clears any
    # Plan context inherited from its previous dispatch.
    bind_plan_tool_context(mode, plan_file)
    if mode.strip().lower() != "plan" and function_name != "plan_exit":
        return None
    decision = gate_side_effect(mode, function_name, function_args, plan_file)
    logger.debug(
        "plan_tool_decision mode=%s tool=%s permission=%s decision=%s plan_file=%s",
        mode.strip().lower() or "build",
        str(function_name or ""),
        str(decision.get("permission") or ""),
        str(decision.get("decision") or ""),
        str(plan_file or ""),
    )
    if decision.get("decision") != "allow":
        logger.info(
            "plan_tool_blocked mode=%s tool=%s code=%s plan_file=%s",
            mode.strip().lower() or "build",
            str(function_name or ""),
            str(decision.get("code") or "plan_side_effect_blocked"),
            str(plan_file or ""),
        )
        payload = {
            "error": str(decision.get("reason") or "tool execution denied"),
            "code": str(decision.get("code") or "plan_side_effect_blocked"),
            "mode": mode.strip().lower() or "discover",
            "tool": str(function_name or ""),
        }
        return {
            "message": payload["error"],
            "error_type": payload["code"],
...
def _apply_tool_request_middleware_for_agent(
    ...
    *,
    ...
    try:
        from hairball_cli.middleware import apply_tool_request_middleware
...
def _run_agent_tool_execution_middleware(
    ...
    *,
    ...
    from hairball_cli.middleware import run_tool_execution_middleware
...
def execute_tool_calls_concurrent(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    if agent._interrupt_requested:
        ...
        for tc in tool_calls:
            ...
            _record_completion_tool_evidence(
                agent,
                function_name=tc.function.name,
                function_result=cancelled_result,
                tool_call_id=getattr(tc, "id", "") or "",
                cancelled=True,
                details={"reason": "user interrupt before concurrent execution"},
            )
    ...
    for tool_call in tool_calls:
        ...
        # the legacy lesson: hooks must observe the real tool name.
        #
        ...
        try:
            ...
            if function_name == _ts.TOOL_CALL_NAME:
                _scope = _tool_search_scoped_names(agent)
                _underlying, _underlying_args, _err = _ts.resolve_underlying_call(
                    function_args,
                    allowed_deferred_names=_scope,
                )
                if not _err and _underlying:
                    if _underlying in _scope:
        ...
        session_scope_block = _session_tool_scope_block(agent, function_name)
        ...
        plan_block = _plan_mode_tool_block(agent, function_name, function_args)
        ...
        elif session_scope_block is not None:
            block_result = session_scope_block["result"]
            _emit_terminal_post_tool_call(
                agent,
                function_name=function_name,
                function_args=function_args,
                ...
                tool_call_id=getattr(tool_call, "id", "") or "",
                status="blocked",
                error_type=session_scope_block["error_type"],
                error_message=session_scope_block["message"],
                middleware_trace=list(middleware_trace),
            )
        elif plan_block is not None:
            block_result = plan_block["result"]
            _emit_terminal_post_tool_call(
                agent,
                function_name=function_name,
                function_args=function_args,
                ...
                tool_call_id=getattr(tool_call, "id", "") or "",
                status="blocked",
                error_type=plan_block["error_type"],
                error_message=plan_block["message"],
                middleware_trace=list(middleware_trace),
            )
        else:
            try:
                from hairball_cli.plugins import resolve_pre_tool_block
    ...
    for tc, name, args, middleware_trace, block_result, blocked_by_guardrail in parsed_calls:
        if block_result is not None:
            continue
        _emit_native_tool_started(agent, tc, name, args)
        if agent.tool_progress_callback:
            try:
                ...
                agent.tool_progress_callback(
                    ...
                    preview,
                    display_args,
                    **_progress_tool_call_id_kwargs(tc),
                )
    ...
    for i, (tc, name, args, middleware_trace, block_result, blocked_by_guardrail) in enumerate(parsed_calls):
        ...
        if i in timed_out_indices and r is None:
            ...
            completion_details = {"status": "timeout", "duration_ms": int(tool_duration * 1000)}
            _record_completion_tool_evidence(
                agent,
                function_name=name,
            ...
            _emit_native_tool_completed(
                agent, tc, name, args, function_result, tool_duration, True,
            )
        elif r is None:
            ...
            if agent._interrupt_requested:
                ...
                completion_details = {"status": "cancelled", "reason": "user interrupt"}
            else:
                ...
                completion_failed = True
                completion_details = {"status": "error", "error_type": "thread_missing_result"}
            tool_duration = 0.0
            _record_completion_tool_evidence(
                agent,
                function_name=name,
            ...
            if blocked:
                effect_disposition = "none"
            completion_failed = bool(is_error)
            completion_details = {"duration_ms": int(tool_duration * 1000)}
            # Capture the canonical result before display/guardrail hints can
            # append text and turn a verifier JSON payload into plain prose.
            _record_completion_tool_evidence(
                agent,
                function_name=name,
                function_result=function_result,
            ...
            try:
                controller = getattr(agent, "_stop_controller", None)
                ...
                logger.debug("Plan decision tool observation skipped", exc_info=True)
            ...
            if not blocked:
                _emit_native_tool_completed(
                    agent, tc, function_name, function_args, function_result,
                    tool_duration, is_error,
                )
    ...
    if finalize and num_tools > 0:
        ...
        try:
            from tools.collab_control import apply_collab_completion_reminders

            apply_collab_completion_reminders(agent, messages, num_tools)
        except Exception:
            logging.debug("collab completion reminder failed", exc_info=True)
...
def execute_tool_calls_sequential(agent, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0, *, finalize: bool = True) -> None:
    ...
    for i, tool_call in enumerate(assistant_message.tool_calls, 1):
        ...
        if _ts_scope_block is not None:
            ...
            _block_msg = session_scope_block["message"]
            _block_error_type = session_scope_block["error_type"]
            _block_result_override = session_scope_block["result"]
        elif plan_block is not None:
            _block_msg = plan_block["message"]
            _block_error_type = plan_block["error_type"]
            _block_result_override = plan_block["result"]
        else:
            try:
                from hairball_cli.plugins import resolve_pre_tool_block
        ...
        if not _execution_blocked:
            _emit_native_tool_started(agent, tool_call, function_name, function_args)
        ...
        if not _execution_blocked and agent.tool_progress_callback:
            try:
                ...
                agent.tool_progress_callback(
                    ...
                    preview,
                    display_args,
                    **_progress_tool_call_id_kwargs(tool_call),
                )
            except Exception as cb_err:
                logging.debug(f"Tool progress callback error: {cb_err}")
        ...
        if _block_msg is not None:
            ...
            function_result = _block_result_override or json.dumps(
            ...
            def _execute(next_args: dict) -> Any:
                ...
                if not session_db:
                    from hairball_state import format_session_db_unavailable
            ...
            def _execute(next_args: dict) -> Any:
                ...
                return _clarify_tool(
                    ...
                    surface=str(getattr(agent, "platform", "") or "agent"),
                )
            function_result, function_args = _run_agent_tool_execution_middleware(
                agent,
                function_name=function_name,
            ...
            _collab_token = None
            try:
                if str(function_name or "").startswith("collab_"):
                    from tools.collab_control import bind_collab_parent_agent

                    _collab_token = bind_collab_parent_agent(agent)
                function_result = _ra().handle_function_call(
                    ...
                    turn_id=getattr(agent, "_current_turn_id", "") or "",
                    api_request_id=getattr(agent, "_current_api_request_id", "") or "",
                    enabled_tools=list(getattr(agent, "valid_tool_names", ())),
                    skip_pre_tool_call_hook=True,
                    skip_tool_request_middleware=True,
                    enabled_toolsets=getattr(agent, "enabled_toolsets", None),
                    disabled_toolsets=getattr(agent, "disabled_toolsets", None),
                    visible_toolsets=getattr(agent, "visible_toolsets", None),
                    session_tool_router=_ra().session_tool_router_for_agent(agent),
                    tool_request_middleware_trace=list(middleware_trace),
                )
                ...
                _record_completion_tool_evidence(
                    ...
                    tool_call_id=getattr(tool_call, "id", "") or "",
                    cancelled=True,
                    details={"reason": "keyboard interrupt"},
                )
                _spinner_result = function_result
                try:
                    agent.interrupt("keyboard interrupt")
                ...
                if _collab_token is not None:
                    try:
                        from tools.collab_control import reset_collab_parent_agent

                        reset_collab_parent_agent(_collab_token)
                    except Exception:
                        pass
                tool_duration = time.time() - tool_start_time
        ...
        _record_completion_tool_evidence(
            ...
            details={"duration_ms": int(tool_duration * 1000)},
        )
        ...
        if not _execution_blocked:
            _emit_native_tool_completed(
                agent, tool_call, function_name, function_args, function_result,
                tool_duration, _is_error_result,
            )
    ...
    if finalize and num_tools_seq > 0:
        ...
        try:
            ...
            apply_collab_completion_reminders(agent, messages, num_tools_seq)
        except Exception:
            logging.debug("collab completion reminder failed", exc_info=True)
    ...
    *k+1..n* without executing them while preserving one result per
    ...
    if total_tools > 0:
        ...
        try:
            ...
            apply_collab_completion_reminders(agent, messages, total_tools)
        except Exception:
            logging.debug("collab completion reminder failed", exc_info=True)
