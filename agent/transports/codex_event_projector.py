"""Projects codex app-server events into Hairball' messages list.

The translator that lets Hairball' memory/skill review keep working under the
reference runtime: it converts reference `item/*` notifications into the standard
OpenAI-shaped `{role, content, tool_calls, tool_call_id}` entries that
`agent/curator.py` already knows how to read.

reference emits items with a discriminator field `type`:
...
Hairball' message-alternation invariants (system → user → assistant → user/tool
→ assistant → ...). Multiple reference tool calls within one reference turn produce
multiple consecutive (assistant, tool) pairs, which is the same shape Hairball
...
def _format_tool_args(d: dict) -> str:
    """Format a dict as JSON the way Hairball' existing tool_calls path does."""
    return json.dumps(d, ensure_ascii=False, sort_keys=True)


@dataclass
class ProjectionResult:
    """Output of projecting one reference item.

    `messages` is a list because some reference items produce two messages
...
class CodexEventProjector:
    """Stateful projector consuming reference notifications in arrival order.

    Owns the in-progress reasoning content (codex emits reasoning as separate
    items but Hairball stashes it on the next assistant message)."""
    ...
    def project(self, notification: dict) -> ProjectionResult:
        ...
        # don't enter the messages list — same way Hairball already only writes
    ...
    def _project_user_message(self, item: dict) -> ProjectionResult:
        ...
        # non-text parts (images, etc.) — Hairball' messages store text only.
        text_parts: list[str] = []
