"""OpenAI Responses API (reference) transport.
...
class ResponsesApiTransport(ProviderTransport):
        ...
        **params,
        ...
        params:
            ...
            session_id: str | None — transcript/session id; drives the xAI
                x-grok-conv-id header and the reference cache-scope headers, and is
        ...
        if "gpt-5.6" in (model or "").lower():
            # Ultra is the reference product tier; the Responses API wire value is max.
        ...
        # turn then burns 3 continuation retries and fails with "reference
        ...
        # that would silently route around Hairball's web-provider config and
        ...
        # native search engine for xAI sessions instead of Hairball's
        # configured web provider (Tavily/etc.), and those results bypass
        # Hairball's tool-trace / citation plumbing (they arrive baked into the
        ...
        # is silently dropped on the main agent reference path while the
        # chat_completions path and auxiliary reference adapter both forward it.
        ...
        if is_codex_backend:
            # The reference backend rejects body-level ``extra_headers`` with
    ...
    def normalize_response(self, response: Any, **kwargs) -> NormalizedResponse:
        """Normalize reference Responses API response to NormalizedResponse."""
        from agent.codex_responses_adapter import (
        ...
        return NormalizedResponse(
            ...
            usage=None,  # reference usage is extracted separately in normalize_usage()
            provider_data=provider_data or None,
        )

    def validate_response(self, response: Any) -> bool:
        """Check reference Responses API response has valid output structure.
    ...
    def preflight_kwargs(
        ...
        *,
        ...
        """Validate and sanitize reference API kwargs before the call.
    ...
    def map_finish_reason(self, raw_reason: str) -> str:
        """Map reference response.status to OpenAI finish_reason.

        reference uses response.status ('completed', 'incomplete') +
