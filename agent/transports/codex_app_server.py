"""reference app-server JSON-RPC client.
...
projection into Hairball' display, approval bridging, transcript projection into
...
"codex_app_server"`. Hairball' default tool dispatch is unchanged when this
...
class CodexAppServerClient:
    ...
    def __init__(
        ...
        # `os.environ.copy()` also handed it every Tier-1 Hairball secret — gateway
        ...
        spawn_env = hairball_subprocess_env(inherit_credentials=True)
        if env:
        ...
        # reference sandbox on, but add the Kanban root as the only extra writable
        ...
        if spawn_env.get("HAIRBALL_KANBAN_TASK"):
            kanban_db = spawn_env.get("HAIRBALL_KANBAN_DB")
            kanban_root = (
                os.path.dirname(kanban_db)
                if kanban_db
                else spawn_env.get(
                    "HAIRBALL_KANBAN_ROOT",
                    os.path.join(
                        spawn_env.get("HAIRBALL_HOME", os.path.expanduser("~/.hairball")),
                        "kanban",
                    ),
                )
            )
        ...
        # reference emits tracing to stderr; default WARN keeps it quiet for users.
    ...
    def initialize(
        self,
        client_name: str = "hairball",
        client_title: str = "Hairball Agent",
