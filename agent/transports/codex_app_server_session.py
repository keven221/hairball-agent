...
Owns one reference thread per Hairball session. Drives `turn/start`, consumes
...
# Hairball' tools.terminal.security_mode → reference's permissions profile id.
# Defaults if config is missing → workspace-write (matches reference's own default).
_HAIRBALL_TO_CODEX_PERMISSION_PROFILE = {
...
def _coerce_turn_input_text(user_input: Any) -> str:
    """Collapse Hairball/OpenAI rich content into app-server text input.
...
def _classify_oauth_failure(*parts: str) -> Optional[str]:
    ...
    for needle in _OAUTH_REFRESH_FAILURE_HINTS:
        if needle in haystack:
            return (
                "reference authentication failed — your ChatGPT/reference login "
...
class CodexAppServerSession:
    """One reference thread per Hairball session, lifetime owned by AIAgent.
    ...
    def __init__(
        ...
        *,
        ...
        self._permission_profile = (
            permission_profile or _HAIRBALL_TO_CODEX_PERMISSION_PROFILE.get(
                os.environ.get("HAIRBALL_TERMINAL_SECURITY_MODE", "auto"),
                "workspace-write",
            )
        )
    ...
    def ensure_started(self) -> str:
        ...
        self._client.initialize(
            client_name="hairball",
            client_title="Hairball Agent",
            client_version=_get_hairball_version(),
        )
    ...
    def run_turn(
        ...
        *,
        ...
        # supports rich content but Hairball' text path is the common case).
        try:
        ...
        while time.monotonic() < deadline and not turn_complete:
            ...
            if projection.final_text is not None:
                # reference can emit multiple agentMessage items in one turn
    ...
    def compact_thread(
        ...
        *,
        ...
        """Trigger reference-native history compaction for the current thread.
    ...
    def _handle_server_request(self, req: dict) -> None:
        """Translate a codex server request (approval) into Hairball' approval
        ...
        if method == "item/commandExecution/requestApproval":
            ...
            # reference sometimes asks to escalate permissions mid-turn. We
            ...
            # reference's MCP layer asks the user for structured input on
            # behalf of an MCP server (e.g. tool-call confirmation,
            # OAuth, form data). For our own hairball-tools callback we
            # auto-accept — the user already approved Hairball' tools
    ...
    def _decide_exec_approval(self, params: dict) -> str:
        ...
        # reference's CommandExecutionRequestApprovalParams has cwd as Optional —
        ...
        description = f"reference requests exec in {cwd}"
        if reason:
    ...
    def _decide_apply_patch_approval(self, params: dict) -> str:
        ...
        if self._approval_callback is not None:
            ...
            description = (
                ...
                else "reference requests to apply a patch"
            )
            command_label = (
...
def _apply_token_usage_notification(result: TurnResult, note: dict) -> None:
    """Capture reference app-server token usage updates for caller accounting.

    reference does not put token usage on turn/completed. It emits a separate
...
def _apply_compaction_notification(result: TurnResult, note: dict) -> None:
    """Capture reference-native context compaction boundaries.
    ...
    mean the underlying reference thread history has been compacted.
    """
...
def _approval_choice_to_codex_decision(choice: str) -> str:
    """Map Hairball approval choices onto codex's CommandExecutionApprovalDecision
    / FileChangeApprovalDecision wire values.

    Hairball returns 'once', 'session', 'always', or 'deny'.
    reference expects 'accept', 'acceptForSession', 'decline', or 'cancel'
...
def _has_turn_aborted_marker(text: str) -> bool:
    ...
    reference emits `<turn_aborted>` (and sometimes `<turn_aborted/>`) as raw
...
def _get_hairball_version() -> str:
    """Best-effort Hairball version string for codex's userAgent line."""
    try:
