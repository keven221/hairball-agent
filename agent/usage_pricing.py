...
CostStatus = Literal[
    "actual", "partial_actual", "estimated", "included", "unknown"
]
CostSource = Literal[
    "provider_cost_api",
...
class SessionCostSummary:
    """Honest session-level cost view.

    ``actual_cost_usd`` is only an authoritative total when every billable
    response represented by ``total_call_count`` reported a USD charge.  A
    partial actual subtotal remains visible for reconciliation, but it never
    replaces the complete estimate.
    """

    display_cost_usd: Optional[float]
...
def _finite_nonnegative_float(value: Any) -> Optional[float]:
    """Coerce a real numeric USD value without accepting strings or bools."""
...
def extract_provider_cost_usd(
    response_usage: Any,
    *,
    provider: Optional[str] = None,
    base_url: Optional[str] = None,
) -> Optional[float]:
    """Return a provider-reported USD charge, never an estimate.

    ``cost_usd`` is an explicit unit-bearing contract and is safe on any
    route.  The shorter ``cost`` field is accepted only on OpenRouter, whose
    usage-accounting protocol defines it as the account charge in USD.  Other
    providers use similarly named fields for credits, points, component maps,
    or unspecified units; treating those as dollars would corrupt accounting.

    Missing and malformed values return ``None``.  A reported numeric zero is
    preserved as a real observation.
    """
    if response_usage is None:
        return None

    explicit = _usage_field(response_usage, "cost_usd")
    if explicit is not None:
        return _finite_nonnegative_float(explicit)

    provider_name = str(provider or "").strip().lower()
    is_openrouter = provider_name == "openrouter" or base_url_host_matches(
        base_url or "", "openrouter.ai"
    )
    if not is_openrouter:
        return None
    return _finite_nonnegative_float(_usage_field(response_usage, "cost"))
...
def session_cost_summary(agent: Any) -> SessionCostSummary:
    """Read the session cost counters without mutating the agent."""
    estimated = _finite_nonnegative_float(
        getattr(agent, "session_estimated_cost_usd", 0.0)
    )
    estimated = estimated if estimated is not None else 0.0
    actual = _finite_nonnegative_float(
        getattr(agent, "session_actual_cost_usd", None)
    )
    actual_calls = _nonnegative_count(
        getattr(agent, "session_cost_actual_call_count", 0)
    )
    total_calls = _nonnegative_count(
        getattr(agent, "session_cost_total_call_count", 0)
    )
    if total_calls:
        actual_calls = min(actual_calls, total_calls)
        coverage: Optional[float] = actual_calls / total_calls
    else:
        actual_calls = 0
        coverage = None

    actual_complete = bool(
        total_calls > 0 and actual_calls == total_calls and actual is not None
    )
    stored_status = str(getattr(agent, "session_cost_status", "unknown") or "unknown")
    stored_source = str(getattr(agent, "session_cost_source", "none") or "none")
    source: CostSource = (
        stored_source
        if stored_source in {
            "provider_cost_api",
    ...
    elif stored_status == "included" and total_calls == 0:
        status = "included"
        display = 0.0
        source = "none"
    elif estimated > 0 or stored_status == "estimated":
        status = "estimated"
        display = estimated
    elif actual is not None and actual_calls > 0:
...
def accumulate_session_cost(
    ...
    *,
    ...
    billable_call_count: int = 0,
    estimated_status: CostStatus = "unknown",
    estimated_source: CostSource = "none",
) -> SessionCostSummary:
    """Accumulate one or more route observations and return the honest view."""
    estimated_delta = _finite_nonnegative_float(estimated_cost_usd)
    if estimated_delta is not None:
        current_estimated = _finite_nonnegative_float(
            getattr(agent, "session_estimated_cost_usd", 0.0)
        ) or 0.0
        agent.session_estimated_cost_usd = current_estimated + estimated_delta

    billable_calls = _nonnegative_count(billable_call_count)
    actual_calls = min(_nonnegative_count(actual_call_count), billable_calls)
    agent.session_cost_total_call_count = _nonnegative_count(
        getattr(agent, "session_cost_total_call_count", 0)
    ) + billable_calls
    agent.session_cost_actual_call_count = _nonnegative_count(
        getattr(agent, "session_cost_actual_call_count", 0)
    ) + actual_calls

    actual_delta = _finite_nonnegative_float(actual_cost_usd)
    if actual_delta is not None and actual_calls > 0:
        current_actual = _finite_nonnegative_float(
            getattr(agent, "session_actual_cost_usd", None)
        )
        agent.session_actual_cost_usd = (current_actual or 0.0) + actual_delta

    # Preserve the latest estimate provenance until the session is fully
    # reconciled.  session_cost_summary promotes the source to provider cost
    # only when actual coverage reaches 100%.
    agent.session_cost_status = estimated_status
    agent.session_cost_source = estimated_source
    summary = session_cost_summary(agent)
    agent.session_cost_status = summary.status
    agent.session_cost_source = summary.source
    return summary
...
_OFFICIAL_DOCS_PRICING: Dict[tuple[str, str], PricingEntry] = {
    ...
    (
        ...
        source="official_docs_snapshot",
        source_url="https://api-docs.deepseek.com/quick_start/pricing",
        pricing_version="deepseek-pricing-2026-08-06",
    ),
    # Google Gemini
    (
        "google",
    ...
    # Zhipu / Z.AI GLM — USD list prices from https://docs.z.ai/guides/overview/pricing
    # (CN bigmodel.cn publishes CNY rates that differ; Hairball's cost layer is USD).
    (
        "zhipu",
        "glm-4.7",
    ): PricingEntry(
        ...
        pricing_version="zhipu-zai-pricing-2026-07",
    ),
    (
        "zhipu",
        "glm-4.6",
...
def resolve_billing_route(
    ...
    if provider_name == "managed" or base_url_host_matches(base_url or "", "inference.example.invalid"):
        return BillingRoute(provider="managed", model=model, base_url=base_url or _MANAGED_DEFAULT_BASE_URL, billing_mode="official_models_api")
    if provider_name == "anthropic":
        return BillingRoute(provider="anthropic", model=model.split("/")[-1], base_url=base_url or "", billing_mode="official_docs_snapshot")
    # DeepSeek is often called through an OpenAI-compatible client (provider
    # slug "openai") with base_url pointing at api.deepseek.com. Normalize to
    # the "deepseek" billing provider so _OFFICIAL_DOCS_PRICING keys resolve.
    ...
    # Zhipu Open Platform / Z.AI — same GLM list prices; normalize alias slugs.
...
_LIVE_PROVIDER_CATALOG_SOURCE_URLS = {
    "deepinfra": "https://api.deepinfra.com/v1/openai/models",
    "novita": "https://api.novita.ai/openai/v1/models",
    "managed": "provider-models-api",
}


def _provider_catalog_pricing_entry(
    route: BillingRoute,
    original_model_name: str,
) -> Optional[PricingEntry]:
    """Reuse Hairball's existing live picker catalog for session accounting.

    The model picker already fetches and caches pricing for these providers.
    Session accounting previously ignored that cache, so the picker could show
    a price while chat/Trace reported an unknown amount.  Match exact ids first;
    a bare-name fallback is accepted only when it resolves to one catalog row.
    """
    if route.provider not in _LIVE_PROVIDER_CATALOG_SOURCE_URLS:
        return None
    try:
        from hairball_cli.models import get_pricing_for_provider

        catalog = get_pricing_for_provider(route.provider) or {}
    except Exception:
        return None
    if not isinstance(catalog, dict) or not catalog:
        return None

    original = str(original_model_name or "").strip()
    candidates = [original, route.model]
    provider_prefix = route.provider + "/"
    if original.lower().startswith(provider_prefix):
        candidates.insert(0, original[len(provider_prefix) :])

    pricing = None
    matched_model = ""
    for candidate in candidates:
        if candidate and isinstance(catalog.get(candidate), dict):
            matched_model = candidate
            pricing = catalog[candidate]
            break
    if pricing is None and route.model:
        suffix_matches = [
            (model_id, value)
            for model_id, value in catalog.items()
            if isinstance(model_id, str)
            and model_id.split("/")[-1] == route.model
            and isinstance(value, dict)
        ]
        if len(suffix_matches) == 1:
            matched_model, pricing = suffix_matches[0]
    if pricing is None:
        return None

    source_url = _LIVE_PROVIDER_CATALOG_SOURCE_URLS[route.provider]
    if route.base_url:
        source_url = route.base_url.rstrip("/") + "/models"
    return _pricing_entry_from_metadata(
        {matched_model: {"pricing": pricing}},
        matched_model,
        source_url=source_url,
        pricing_version=f"{route.provider}-models-api",
    )
...
def get_pricing_entry(
    ...
    if route.base_url:
        ...
        if entry:
            return entry
    catalog_entry = _provider_catalog_pricing_entry(route, model_name)
    if catalog_entry:
...
def normalize_usage(
    ...
    *,
    ...
    - reference Responses: input_tokens includes cache tokens; input_tokens_details.cached_tokens separates them
    - OpenAI Chat Completions: prompt_tokens includes cache tokens; prompt_tokens_details.cached_tokens separates them

    In both reference and OpenAI modes, input_tokens is derived by subtracting cache
    ...
    if mode == "anthropic_messages" or provider_name == "anthropic":
        ...
        # Primary: OpenAI-style prompt_tokens_details. DeepSeek exposes the
        # equivalent read bucket as top-level ``prompt_cache_hit_tokens``.
        # Anthropic-style top-level fields are the final fallback for compatible
        # proxies (OpenRouter, Cline) routing Claude models — without this
        # fallback, cache writes are undercounted as 0 and cache reads can be
        # missed when the proxy only surfaces them at the top level.
        # Port of cline/cline#10266.
        nested_cached = getattr(details, "cached_tokens", None) if details else None
        deepseek_cached = getattr(response_usage, "prompt_cache_hit_tokens", None)
        if nested_cached is not None:
            # Match the OpenAI-compatible ``cached_tokens ?? native_hit``
            # contract: an explicit zero is data, not a missing field.
            cache_read_tokens = _to_int(nested_cached)
            if not cache_read_tokens and deepseek_cached is None:
                # Preserve the established OpenRouter/Cline compatibility path.
                cache_read_tokens = _to_int(
                    getattr(response_usage, "cache_read_input_tokens", 0)
                )
        elif deepseek_cached is not None:
            cache_read_tokens = _to_int(deepseek_cached)
        else:
            cache_read_tokens = _to_int(getattr(response_usage, "cache_read_input_tokens", 0))
        cache_write_tokens = _to_int(
