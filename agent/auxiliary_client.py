...
Resolution order for text tasks (auto mode):
  1. User's main provider + main model (used regardless of provider type —
     aggregators, direct API-key providers, native Anthropic, reference, etc.)
  2. OpenRouter  (OPENROUTER_API_KEY)
  3. Managed Portal (~/.hairball/auth.json active provider)
...
reference OAuth (ChatGPT-account auth) is intentionally NOT in either
fallback chain: OpenAI gates this endpoint behind an undocumented,
shifting model allow-list, so "just try reference with a hardcoded model"
rots on its own.  reference is used only when the user's main provider *is*
...
Payment / credit exhaustion fallback:
  ...
  their OpenRouter balance but has reference OAuth or another provider available.
"""
...
from hairball_constants import OPENROUTER_BASE_URL
...
def _resolve_aux_verify(base_url: Optional[str]) -> Any:
    ...
    ``ssl_ca_cert`` / ``ssl_verify`` config and the ``HAIRBALL_CA_BUNDLE`` /
...
def _create_openai_client(*, api_key: str, base_url: str, **kwargs: Any) -> Any:
    ...
    # Hairball owns auxiliary retry + provider/model fallback policy (the
    ...
    # Hairball sees a single failure (issue #54465). Disable SDK-internal retries
    # by default and let Hairball control the budget; explicit callers can still
    # override via kwargs.
...
# task mark its in-flight LLM call as interrupt-protected; the reference
...
# Context window enforced by ChatGPT's reference OAuth backend for the
# gpt-5.4 / gpt-5.5 / gpt-5.6 families. The raw OpenAI API and OpenRouter
# expose 1.05M for the same slugs, but the reference backend hard-caps at 272K
...
# 272K reference cap — see _CODEX_OAUTH_CONTEXT_FALLBACK in model_metadata.py).
...
# on this exact route so reference gpt-5.4 / gpt-5.5 / gpt-5.6 sessions use the
# window they actually have.
_CODEX_GPT54_GPT55_COMPACTION_THRESHOLD = 0.85

# gpt-5.3-codex-spark is reference-OAuth-only (ChatGPT Pro entitlement) with a
...
def _is_codex_gpt54_or_gpt55(model: Optional[str], provider: Optional[str] = None) -> bool:
    """True for gpt-5.4 / gpt-5.5 / gpt-5.6 on the ChatGPT OAuth backend.

    Matches only the reference OAuth route (provider ``openai-codex``), not the
...
def _is_codex_spark(model: Optional[str], provider: Optional[str] = None) -> bool:
    """True for ``gpt-5.3-codex-spark`` on the ChatGPT OAuth backend.

    The model is reference-OAuth-only (ChatGPT Pro entitlement) with a native
    128K context window.  Only the reference OAuth route (provider
...
def _compression_threshold_for_model(
    ...
    *,
    ...
    consumed before Hairball triggers summarization.  Higher values delay
    compression and preserve more raw context.

    Per-model/route overrides:
      - Arcee Trinity Large Thinking → 0.75 (preserve reasoning context).
      - gpt-5.4 / gpt-5.5 / gpt-5.6 on the reference OAuth route → 0.85, because
        reference caps all three families at 272K and the default 50% trigger
      ...
      - gpt-5.3-codex-spark on the reference OAuth route → 0.70, because the model
...
def build_or_headers(or_config: dict | None = None) -> dict:
    ...
    Environment variables:
        ``HAIRBALL_OPENROUTER_CACHE`` — truthy (``1``/``true``/``yes``/``on``)
            enables caching; ``0``/``false``/``no``/``off`` disables.
            Overrides ``openrouter.response_cache`` in config.yaml.
        ``HAIRBALL_OPENROUTER_CACHE_TTL`` — integer seconds (1-86400).
    ...
    *or_config* is the ``openrouter`` section from config.yaml.  When *None*,
    ...
    env_cache = os.environ.get("HAIRBALL_OPENROUTER_CACHE", "").strip().lower()
    if env_cache:
    ...
    env_ttl = os.environ.get("HAIRBALL_OPENROUTER_CACHE_TTL", "").strip()
    if env_ttl:
        if env_ttl.isdigit():
            ttl = int(env_ttl)
            if 1 <= ttl <= 86400:
...
_NVIDIA_NIM_CLOUD_HEADERS = {
    "X-BILLING-INVOKE-ORIGIN": "HairballAgent",
}
...
# Managed Portal extra_body for product attribution.
# Callers should pass this as extra_body in chat.completions.create()
# when the auxiliary client is backed by Managed Portal.
#
# The tags are computed from agent.portal_tags so the client= marker stays
# in lockstep with hairball_cli.__version__ across every Portal call site
# (main loop, aux, compression, web_extract). Do not inline a literal here;
# see agent/portal_tags.py for the rationale.
from agent.portal_tags import managed_portal_tags as _managed_portal_tags


def _managed_extra_body() -> dict:
    """Return a fresh Managed Portal ``extra_body`` dict.

    Computed at call time so a hot-reloaded ``hairball_cli.__version__`` is
    reflected without restarting long-running processes.
    """
    return {"tags": _managed_portal_tags()}
...
# ``_managed_extra_body()`` or import ``managed_portal_tags`` directly.
MANAGED_EXTRA_BODY = _managed_extra_body()

# Set at resolve time — True if the auxiliary client points to Managed Portal
auxiliary_is_managed: bool = False

# Default auxiliary models per provider
_OPENROUTER_MODEL = "google/gemini-3-flash-preview"
_MANAGED_MODEL = "google/gemini-3-flash-preview"
_MANAGED_DEFAULT_BASE_URL = "https://inference.example.invalid/v1"
_ANTHROPIC_DEFAULT_BASE_URL = "https://api.anthropic.com"
_AUTH_JSON_PATH = get_hairball_home() / "auth.json"

# reference OAuth endpoint used when a caller explicitly requests
...
# or the user's active reference model selection).
...
def _codex_cloudflare_headers(access_token: str) -> Dict[str, str]:
    ...
    The Cloudflare layer in front of the reference endpoint whitelists a small set of
    first-party originators (``codex_cli_rs``, ``codex_vscode``, ``codex_sdk_ts``,
    anything starting with ``reference``). Requests from non-residential IPs (VPS,
    ...
    """
    headers = {
        "User-Agent": "codex_cli_rs/0.0.0 (Hairball Agent)",
...
def _pool_runtime_base_url(entry: Any, fallback: str = "") -> str:
    ...
    if getattr(entry, "provider", None) == "managed":
        # Funnel through the canonical auth-layer reader so the env override
        # shares one normalization path with the rest of the MANAGED resolution.
        from hairball_cli.auth import _managed_inference_env_override

        env_url = _managed_inference_env_override()
        if env_url:
            return env_url
...
def _managed_min_key_ttl_seconds() -> int:
    try:
        return max(60, int(os.getenv("HAIRBALL_MANAGED_MIN_KEY_TTL_SECONDS", "1800")))
    except (TypeError, ValueError):
        return 1800


# ── reference Responses → chat.completions adapter ─────────────────────────────
# All auxiliary consumers call client.chat.completions.create(**kwargs) and
# read response.choices[0].message.content. This adapter translates those
# calls to the reference Responses API so callers don't need any changes.


class _CodexCompletionsAdapter:
    """Drop-in shim that accepts chat.completions.create() kwargs and
    routes them through the reference Responses streaming API."""
    ...
    def create(self, **kwargs) -> Any:
        ...
        # forwarded and enforced, a reference Responses stream can sit behind a
        # dead-looking CLI until the user force-interrupts the whole session.
        timeout = kwargs.get("timeout")
        if timeout is not None:
            resp_kwargs["timeout"] = timeout

        # Note: the reference endpoint (chatgpt.com/backend-api/codex) does NOT
        ...
        # same behavior as the main agent's reference transport.
        ...
        if isinstance(extra_body, dict):
            ...
            if isinstance(reasoning_cfg, dict):
                if reasoning_cfg.get("enabled") is False:
                    ...
                    # or include.  The reference backend still thinks by
                    ...
                    # reference backend, which rejects e.g. {"effort": null}
                    # with a 400.
                    effort = reasoning_cfg.get("effort") or "medium"
                    # reference backend rejects "minimal"; clamp to "low" to
                    # match the main-agent reference transport behavior.
                    if effort == "minimal":
                        effort = "low"
        ...
        if tools:
            ...
            try:
                ...
                logger.warning(
                    ...
                    "reference/xAI Responses path: %s", exc,
                )
            converted = []
            for t in tools:
        ...
        # Stable prompt-cache routing for the reference/Responses aux path, mirroring
        ...
        try:
            ...
            logger.debug(
                "reference auxiliary: prompt_cache_key derivation skipped", exc_info=True
            )
        ...
        def _timeout_message() -> str:
            return f"reference auxiliary Responses stream exceeded {float(total_timeout):.1f}s total timeout"
        ...
        def _close_client_on_timeout() -> None:
            ...
            if callable(close):
                try:
                    ...
                    logger.debug("reference auxiliary: client close during timeout failed", exc_info=True)
            ...
            try:
                ...
                logger.debug("reference auxiliary: cache eviction on timeout failed", exc_info=True)
        ...
        def _check_cancelled() -> None:
            ...
            try:
                ...
                if is_interrupted() and not _aux_interrupt_protected():
                    raise InterruptedError("reference auxiliary Responses stream interrupted")
        ...
        try:
            ...
            # which the chatgpt.com reference backend has been observed to
            ...
            if final is None:
                raise RuntimeError("reference auxiliary Responses stream did not return a final response")
            ...
            logger.debug("reference auxiliary Responses API call failed: %s", exc)
            raise
        finally:
...
class CodexAuxiliaryClient:
    """OpenAI-client-compatible wrapper that routes through reference Responses API.
...
class _AsyncCodexCompletionsAdapter:
    """Async version of the reference Responses adapter.
...
def _endpoint_speaks_anthropic_messages(base_url: str) -> bool:
    ...
    Mirrors ``hairball_cli.runtime_provider._detect_api_mode_for_url`` so the
...
def _maybe_wrap_anthropic(
    ...
    - It's already an Anthropic/reference/Gemini/CopilotACP wrapper.
...
def _read_managed_auth() -> Optional[dict]:
    """Read and validate ~/.hairball/auth.json for an active managed provider.

    Returns the provider state dict if managed is active with tokens,
    otherwise None.
    """
    ...
    try:
        ...
        logger.debug("Could not read managed auth: %s", exc)
        return None


def _managed_api_key(provider: dict) -> str:
    """Extract a usable managed inference JWT from stored auth state."""
    from hairball_cli.auth import _managed_invoke_jwt_is_usable
...
def _managed_base_url() -> str:
    """Resolve the managed inference base URL from env or default."""
    return os.getenv("MANAGED_INFERENCE_BASE_URL", _MANAGED_DEFAULT_BASE_URL)


def _resolve_managed_pool_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Resolve managed auxiliary credentials from the selected pool entry."""
    try:
        from hairball_cli.auth import _agent_key_is_usable

        pool = load_pool("managed")
    except Exception as exc:
        logger.debug("Auxiliary managed pool credential resolution failed: %s", exc)
        return None
    ...
    try:
        ...
        logger.debug("Auxiliary managed pool selection failed: %s", exc)
        return None
    ...
    if force_refresh or not _agent_key_is_usable(state, _managed_min_key_ttl_seconds()):
        try:
            refreshed = pool.try_refresh_current()
        except Exception as exc:
            logger.debug("Auxiliary managed pool refresh failed: %s", exc)
            refreshed = None
    ...
    api_key = _managed_api_key(provider)
    base_url = _pool_runtime_base_url(entry, _MANAGED_DEFAULT_BASE_URL)
...
def _resolve_managed_runtime_api(*, force_refresh: bool = False) -> Optional[tuple[str, str]]:
    """Return fresh managed runtime credentials when available.
    ...
    """
    pooled = _resolve_managed_pool_runtime_api(force_refresh=force_refresh)
    if pooled is not None:
        return pooled

    try:
        from hairball_cli.auth import resolve_managed_runtime_credentials

        creds = resolve_managed_runtime_credentials(
            timeout_seconds=env_float("HAIRBALL_MANAGED_TIMEOUT_SECONDS", 15),
            force_refresh=force_refresh,
        )
    except Exception as exc:
        logger.debug("Auxiliary managed runtime credential resolution failed: %s", exc)
        return None
...
def _resolve_xai_oauth_for_aux() -> Optional[Tuple[str, str]]:
    ...
    compression report "no provider configured" even though ``hairball auth
    status`` shows xAI OAuth as logged in.

    Falls back to ``hairball_cli.auth``'s singleton runtime resolver for older
    ...
    try:
        from hairball_cli.auth import resolve_xai_oauth_runtime_credentials
...
def _read_codex_access_token() -> Optional[str]:
    """Read a valid, non-expired reference OAuth access token from Hairball auth store.

    If a credential pool exists but currently has no selectable runtime entry
    (for example all pool slots are marked exhausted), fall back to the
    profile's auth.json token instead of hard-failing. This keeps explicit
    fallback-to-reference working when the pool state is stale but the stored OAuth
    ...
    try:
        from hairball_cli.auth import _read_codex_tokens
        ...
        try:
            ...
            if exp and time.time() > exp:
                logger.debug("reference access token expired (exp=%s), skipping", exp)
                return None
        except Exception:
        ...
        logger.debug("Could not read reference auth for auxiliary client: %s", exc)
        return None
...
def _resolve_api_key_provider() -> Tuple[Optional[OpenAI], Optional[str]]:
    ...
    """
    try:
        from hairball_cli.auth import PROVIDER_REGISTRY, resolve_api_key_provider_credentials
    except ImportError:
    ...
    for provider_id, pconfig in PROVIDER_REGISTRY.items():
        ...
        if provider_id == "anthropic":
            ...
            try:
                from hairball_cli.auth import is_provider_explicitly_configured
        ...
        if pool_present:
            ...
            if base_url_host_matches(base_url, "api.kimi.com"):
                ...
                from hairball_cli.models import copilot_default_headers
...
def _try_managed(vision: bool = False) -> Tuple[Optional[OpenAI], Optional[str]]:
    # Check cross-session rate limit guard before attempting managed —
    # if another session already recorded a 429, skip managed entirely
    ...
    try:
        ...
        if _remaining is not None and _remaining > 0:
            logger.debug(
                "Auxiliary: skipping Managed Portal (rate-limited, resets in %.0fs)",
                _remaining,
            )
    ...
    if runtime is None and not managed:
        logger.warning(
            "Auxiliary managed client unavailable: no managed authentication found "
            "(run: hairball auth)."
        )
    ...
    if runtime is None and managed:
        logger.debug(
            "Auxiliary managed: runtime JWT refresh failed; checking stored "
            "auth.json token."
        )
    global auxiliary_is_managed
    auxiliary_is_managed = True
    logger.debug("Auxiliary client: Managed Portal")
    ...
    try:
        from hairball_cli.models import get_managed_recommended_aux_model
    ...
    if runtime is not None:
        ...
        if not api_key:
            logger.warning(
                "Auxiliary managed client unavailable: no usable inference JWT found "
                "(run: hairball auth add managed)."
            )
            _mark_provider_unhealthy("managed", ttl=60)
            return None, None
...
def _refresh_managed_recommended_model(
    *, vision: bool, stale_model: Optional[str]
    ...
    """Re-fetch the Managed Portal's recommended model after a stale-model 404.
    ...
    the managed → OpenRouter catalog, every auxiliary call 404s with
    ...
    try:
        ...
        logger.debug(
            "managed recommended-model refresh failed (%s); using default %s",
            exc, _MANAGED_MODEL,
        )
...
def _validate_base_url(base_url: str) -> None:
    ...
    try:
        ...
        raise RuntimeError(
            ...
            "Run `hairball setup` or `hairball model` and enter a valid http(s) base URL."
        ) from exc
...
def _build_codex_client(model: str) -> Tuple[Optional[Any], Optional[str]]:
    ...
    There is no auto-selection of the reference model: the ChatGPT-account
    reference endpoint's accepted model list is an undocumented, drifting
    ...
    Returns (None, None) when no reference OAuth token is available.
    """
    if not model:
        logger.warning(
    ...
    if pool_present:
        ...
        if not codex_token:
            return None, None
        base_url = _CODEX_AUX_BASE_URL
    logger.debug("Auxiliary client: reference OAuth (%s via Responses API)", model)
...
def _try_azure_foundry(
    *,
    ...
    :func:`hairball_cli.runtime_provider._resolve_azure_foundry_runtime` —
    ...
    * Non-default ``model.base_url`` overrides are honored.
    ...
    """
    try:
        from hairball_cli.runtime_provider import _resolve_azure_foundry_runtime
...
def _try_anthropic(explicit_api_key: str = None) -> Tuple[Optional[Any], Optional[str]]:
    ...
    #      base_url, e.g. reference endpoint, would leak into Anthropic requests), AND
...
def _get_provider_chain() -> List[tuple]:
    ...
    ChatGPT-account reference endpoint only accepts a shifting, undocumented
    allow-list of model IDs, so falling back to it with a guessed model
    fails more often than not.  reference is used only when the user's main
...
# manual intervention.
#
# Failure isolation: the cache is in-process only. A second hairball
...
def _reset_aux_unhealthy_cache() -> None:
    ...
    user trigger (e.g. ``hairball config aux reset``)."""
...
def _managed_portal_account_has_fresh_paid_access() -> bool:
    """Return True only when the fresh managed account API says paid access is allowed."""
    try:
        ...
        logger.debug("Auxiliary managed paid-entitlement refresh check failed: %s", exc)
        return False
...
def _is_model_not_found_error(exc: Exception) -> bool:
    ...
    that has since been dropped from the managed → OpenRouter catalog. The managed
...
def _is_model_incompatible_error(exc: Exception) -> bool:
    ...
    ChatGPT-account fallback asked to compress a ``glm-5.2`` conversation::
        ...
        when using reference with a ChatGPT account."}
...
def _recoverable_pool_provider(
    ...
    if base_url_host_matches(base, "openrouter.ai"):
        return "openrouter"
    if base_url_host_matches(base, "inference.example.invalid"):
        return "managed"
...
def _refresh_provider_credentials(provider: str) -> bool:
    ...
    try:
        ...
        if normalized == "openai-codex":
            from hairball_cli.auth import resolve_codex_runtime_credentials
...
def _auth_refresh_provider_for_route(
    ...
    Copilot/reference/Anthropic/managed routes too. (#20832)
    """
    normalized = _normalize_aux_provider(resolved_provider)
    ...
    if base_url_host_matches(client_base_url, "api.anthropic.com"):
        return "anthropic"
    if base_url_host_matches(client_base_url, "inference.example.invalid"):
        return "managed"
    return normalized
...
def _call_fallback_candidate_sync(
    ...
    *,
    ...
    healthy candidates remained. Live case: a reference-timeout → Anthropic
...
def _try_main_fallback_chain(
    ...
    main fallback policy before dropping into Hairball' built-in discovery
...
def _resolve_auto(
    ...
    Priority:
      1. User's main provider + main model, regardless of provider type.
         ...
         chat.  Users on OpenRouter/managed get their chosen chat model; users
      ...
      2. OpenRouter → managed → custom → reference → API-key providers (fallback
    ...
    #    scenario where a user switches providers via `hairball model` but the
    #    old OPENAI_BASE_URL lingers in ~/.hairball/.env. ──
    if not _stale_base_url_warned:
        ...
        if (_env_base and _cfg_provider
            ...
            logger.warning(
                ...
                "Run: hairball model to reconfigure, or remove "
                "OPENAI_BASE_URL from ~/.hairball/.env",
    ...
    # on aggregators (OpenRouter, managed) who previously got routed to a
    ...
    if main_provider == "moa":
        try:
            from hairball_cli.config import load_config
            from hairball_cli.moa_config import resolve_moa_preset
...
# (Chat Completions vs Responses API for reference).
#
...
def _to_async_client(sync_client, model: str, is_vision: bool = False):
    """Convert a sync client to its async counterpart, preserving reference routing.
    ...
    if base_url_host_matches(sync_base_url, "openrouter.ai"):
        ...
        from hairball_cli.copilot_auth import copilot_request_headers
    ...
    # See _create_openai_client: disable SDK-internal retries so Hairball owns
...
def resolve_provider_client(
    ...
    reference/Responses API providers, an adapter handles the translation
    transparently.
    ...
    Args:
        ...
        raw_codex: If True, return a raw OpenAI client for reference providers
    ...
    # sent to reference after the main lane fell back to gpt-5.5). Let _resolve_auto()
    ...
    if provider == "managed":
        ...
        if client is None:
            logger.warning("resolve_provider_client: managed requested "
                           "but Managed Portal not configured (run: hairball auth)")
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
                else (client, final_model))
        ...
        if raw_codex:
            ...
            if not codex_token:
                logger.warning("resolve_provider_client: openai-codex requested "
                               "but no reference OAuth token found (run: hairball model)")
                return None, None
    ...
    # OpenRouter / managed bills for side tasks they thought were running on
    ...
    if provider == "xai-oauth":
        ...
        if client is None:
            logger.warning(
                ...
                "OAuth token found (run: hairball model -> xAI Grok OAuth — SuperGrok / Premium+)"
            )
            return None, None
        final_model = _normalize_resolved_model(model or default, provider)
        return (_to_async_client(client, final_model, is_vision=is_vision) if async_mode
    ...
    if provider == "custom":
        ...
        # Try custom first, then API-key providers (reference excluded here:
        # falling through to reference with no model is a stale-constant trap).
    ...
    try:
        from hairball_cli.runtime_provider import _get_named_custom_provider
    ...
    if provider == "azure-foundry":
        ...
        if client is None:
            logger.warning(
                ...
                "runtime resolution failed (run: hairball doctor for "
                "diagnostics)"
            )
            return None, None
    ...
    try:
        ...
        logger.debug("hairball_cli.auth not available for provider %s", provider)
        return None, None
    ...
    if pconfig.auth_type == "api_key":
        ...
        if provider == "copilot" and final_model and not raw_codex:
            try:
                from hairball_cli.models import _should_use_copilot_responses_api
...
def get_async_text_auxiliary_client(task: str = "", *, main_runtime: Optional[Dict[str, Any]] = None):
    ...
    For standard providers returns (AsyncOpenAI, model). For reference returns
...
def _resolve_strict_vision_backend(
    ...
    if provider == "openai-codex":
        ...
        # model is used.  There is no safe default reference model (shifting
...
def get_available_vision_backends() -> List[str]:
    ...
    Order: active provider → OpenRouter → managed → stop.  This is the single
    ...
    if main_provider and main_provider not in {"auto", ""}:
        if main_provider in _VISION_AUTO_PROVIDER_ORDER:
            ...
            if client is not None:
                available.append(main_provider)
    # 2. OpenRouter, 3. managed — skip if already covered by main provider.
...
def resolve_vision_provider_client(
    ...
    *,
    ...
    if requested == "auto":
        ...
        #      :func:`_resolve_provider_vision_default`). managed is the
        ...
        #   3. Managed Portal (vision-capable aggregator fallback)
...
def get_auxiliary_extra_body() -> dict:
    ...
    Includes Managed Portal product tags when the auxiliary client is backed
    by Managed Portal. Returns empty dict otherwise.
    """
...
def auxiliary_max_tokens_param(value: int, *, model: Optional[str] = None) -> dict:
    ...
    The reference adapter translates max_tokens internally, so we use max_tokens
...
def _refresh_managed_auxiliary_client(
    *,
    ...
    """Refresh managed runtime creds, rebuild the client, and replace the cache entry."""
...
def _resolve_task_provider_model(
    ...
    def _preserve_provider_with_base_url(prov: Optional[str]) -> bool:
        ...
        if normalized in {"", "auto", "custom"} or normalized.startswith("custom:"):
            return False
        try:
            from hairball_cli.providers import get_provider
...
# model (e.g. reference / GPT-5.5) can legitimately take longer than the default
...
def _get_auxiliary_task_config(task: str) -> Dict[str, Any]:
    ...
    :meth:`hairball_cli.plugins.PluginContext.register_auxiliary_task`) the
...
def _get_task_extra_body(task: str) -> Dict[str, Any]:
    ...
    chat.completions passes it through, the reference Responses adapter maps it
...
def _build_call_kwargs(
    ...
    # body fields (Gemini ``thinking_config``), and OpenRouter/managed use
    ...
    # ``extra_body.reasoning`` fallback used by reference-compatible adapters.
    ...
    if provider == "managed" and "tags" not in merged_extra:
        merged_extra["tags"] = _managed_portal_tags()
    if merged_extra:
        kwargs["extra_body"] = merged_extra

    # Native Anthropic Messages adapters do not consume ``extra_body``. Carry
    # the normalized Hairball reasoning config through a private kwarg so the
...
def call_llm(
    ...
    *,
    ...
    Args:
        ...
        reasoning_config: Optional Hairball reasoning config for direct model calls
            ...
            if _explicit and _explicit not in {"auto", "openrouter", "custom"}:
                ...
                if fb_client is not None:
                    ...
                    raise RuntimeError(
                        ...
                        f"variable, or switch to a different provider with `hairball model`."
                    )
    ...
    try:
        ...
        # ── Stale-model self-heal (Managed Portal recommendation drift) ───
        # A long-lived process can pin a Portal-recommended model that has
        # since been dropped from the managed → OpenRouter catalog, so every
        ...
        # known-good default). Only applies to managed-routed calls.
        _heal_is_managed = (
            resolved_provider == "managed"
            or base_url_host_matches(_base_info, "inference.example.invalid")
        )
        if _is_model_not_found_error(first_err) and _heal_is_managed:
            ...
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s: model %r no longer in managed catalog; "
        ...
        # ── managed auth refresh parity with main agent ──────────────────
        client_is_managed = (
            resolved_provider == "managed"
            or base_url_host_matches(_base_info, "inference.example.invalid")
        )
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s: refreshed managed runtime credentials after paid account check, retrying",
                    task or "call",
                )
        ...
        if _is_auth_error(first_err) and client_is_managed:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s: refreshed managed runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
        ...
        # reference OAuth or another provider available.
        #
        ...
        # reference/OAuth tokens that authenticate but whose endpoint is down,
        ...
        # When the resolved provider returns 401 and neither the managed
...
async def async_call_llm(
    ...
    *,
    ...
    try:
        ...
        # the managed → OpenRouter catalog, 404'ing every auxiliary call. Force a
        # fresh Portal fetch and retry once with the current recommendation.
        _heal_is_managed = (
            resolved_provider == "managed"
            or base_url_host_matches(_client_base, "inference.example.invalid")
        )
        if _is_model_not_found_error(first_err) and _heal_is_managed:
            healed_model = _refresh_managed_recommended_model(
                vision=(task == "vision"), stale_model=kwargs.get("model"))
            if healed_model and healed_model != kwargs.get("model"):
                logger.warning(
                    "Auxiliary %s (async): model %r no longer in managed catalog; "
                    "retrying with refreshed recommendation %r",
                    task or "call", kwargs.get("model"), healed_model,
                )
                kwargs["model"] = healed_model
        ...
        if (
            ...
            if refreshed_client is not None:
                logger.info(
                    "Auxiliary %s (async): refreshed managed runtime credentials after paid account check, retrying",
                    task or "call",
                )
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
        ...
        if _is_auth_error(first_err) and client_is_managed:
            ...
            if refreshed_client is not None:
                logger.info("Auxiliary %s (async): refreshed managed runtime credentials after 401, retrying",
                            task or "call")
                if refreshed_model and refreshed_model != kwargs.get("model"):
                    kwargs["model"] = refreshed_model
                return _validate_llm_response(
