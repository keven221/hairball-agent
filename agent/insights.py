"""
Session Insights Engine for Hairball Agent.

Analyzes historical session data from the SQLite state database to produce
comprehensive usage insights — token consumption, cost estimates, tool usage
patterns, activity trends, model/platform breakdowns, and session metrics.

Inspired by Claude Code's /insights command, adapted for Hairball Agent's
...
class InsightsEngine:
    ...
    def __init__(self, db):
        ...
        Args:
            db: A SessionDB instance (from hairball_state.py)
        """
        self.db = db
        self._conn = db._conn
    ...
    _SESSION_COLS = ("id, source, model, started_at, ended_at, "
                     ...
                     "actual_cost_usd, cost_actual_call_count, "
                     "cost_total_call_count, cost_status, cost_source, api_call_count")
    ...
    def _compute_overview(
        ...
        for s in sessions:
            ...
            cost_actual_call_count += s.get("cost_actual_call_count") or 0
            cost_total_call_count += s.get("cost_total_call_count") or 0
    ...
    _GET_MODEL_USAGE_WITH_SOURCE = (
        ...
        " u.cost_actual_call_count, u.cost_total_call_count, u.cost_status,"
        " u.cost_source, u.billing_mode, u.last_seen"
    ...
    def _compute_model_breakdown(
        ...
        model_data = defaultdict(lambda: {
            ...
            "cost_actual_call_count": 0, "cost_total_call_count": 0,
        })
        ...
        def _accumulate(model, provider, base_url, session_id, inp, out,
                        ...
                        actual_call_count=0, total_call_count=0,
                        last_seen=None):
            model = model or "unknown"
            # Normalize: strip provider prefix for display
            display_model = model.split("/")[-1] if "/" in model else model
            route_key = (model, provider or "", base_url or "")
            d: Dict[str, Any] = model_data[route_key]
            d["model"] = display_model
            d["model_id"] = model
            d["provider"] = provider or ""
            d["base_url"] = base_url or ""
            if last_seen is not None:
                d["last_used_at"] = max(d.get("last_used_at") or 0, last_seen)
            ...
            d["cost_actual_call_count"] += int(actual_call_count or 0)
            d["cost_total_call_count"] += int(total_call_count or 0)
        ...
        for r in usage_rows:
            ...
            totals["cost_actual_call_count"] += r["cost_actual_call_count"] or 0
            totals["cost_total_call_count"] += r["cost_total_call_count"] or 0
            d = _accumulate(
                ...
                stored_cost=(
                    ...
                    if (
                        r.get("estimated_cost_usd") not in (None, 0, 0.0)
                        or r.get("cost_status")
                        or r.get("cost_source")
                    )
                    else None
                ),
                actual_cost=r["actual_cost_usd"],
                cost_status=r.get("cost_status"),
                actual_call_count=r.get("cost_actual_call_count"),
                total_call_count=r.get("cost_total_call_count"),
        ...
        for s in sessions:
            ...
            residual_actual_calls = max(
                0,
                (s.get("cost_actual_call_count") or 0)
                - totals["cost_actual_call_count"],
            )
            residual_total_calls = max(
                0,
                (s.get("cost_total_call_count") or 0)
                - totals["cost_total_call_count"],
            )
            if not (
                inp or out or cache_read or cache_write or residual_cost
                or residual_actual or residual_calls
                or residual_actual_calls or residual_total_calls
            ):
                continue
            d = _accumulate(
                ...
                actual_call_count=residual_actual_calls,
                total_call_count=residual_total_calls,
                last_seen=s.get("started_at"),
            )
        ...
        for s in sessions:
            ...
            base_url = s.get("billing_base_url") or ""
            route_key = (model, provider, base_url)
            d = model_data[route_key]
            d.setdefault("model", model.split("/")[-1] if "/" in model else model)
            d.setdefault("model_id", model)
            d.setdefault("provider", provider)
            d.setdefault("base_url", base_url)
            d["tool_calls"] += tool_calls

        result = []
        for _route_key, data in model_data.items():
            entry = dict(data)
            ...
            total_cost_calls = entry.get("cost_total_call_count") or 0
            entry["actual_cost_coverage"] = (
                (entry.get("cost_actual_call_count") or 0) / total_cost_calls
                if total_cost_calls > 0
                else None
            )
            result.append(entry)
    ...
    def format_terminal(self, report: Dict) -> str:
        ...
        lines.append("")
        lines.append("  ╔══════════════════════════════════════════════════════════╗")
        lines.append("  ║                    📊 Hairball Insights                    ║")
    ...
    def format_gateway(self, report: Dict) -> str:
        ...
        lines.append(f"📊 **Hairball Insights** — Last {days} days\n")
