...
_AD_HOC_SCRIPT_NAME_PREFIXES = ("hairball-verify-", "hairball-ad-hoc-")
# A small repository or generated fixture can be a valid coding workspace
# without carrying a package manifest.  In that case project discovery has no
# canonical command to return, but an explicit, well-known test runner is still
# direct verification evidence.  Keep this list test-only: lint/build commands
# remain project-owned because their meaning is far more configuration-specific.
_MANIFESTLESS_TEST_COMMANDS = (
    "pytest",
    "python -m unittest",
    "python3 -m unittest",
    "tox",
...
def _db_path() -> Path:
    return get_hairball_home() / "verification_evidence.db"
...
def _find_manifestless_test_match(
    command: str,
) -> Optional[tuple[str, list[str]]]:
    """Recognize an explicit test runner when project metadata has no command.

    This is intentionally narrower than general command inference.  It never
    runs anything, never trusts prose such as ``echo pytest``, and rejects
    discovery/help invocations that can exit zero without executing tests.
    """

    match = _find_canonical_match(command, list(_MANIFESTLESS_TEST_COMMANDS))
    if match is None:
        return None
    _canonical, trailing_args = match
    if any(arg in _NON_EXECUTING_TEST_FLAGS for arg in trailing_args):
        return None
    return match


def _mounted_host_workspace_cwd(cwd: str | Path | None) -> Optional[Path]:
    """Map an explicit Docker ``/workspace`` cwd back to its host mount.

    Verification classification runs in the Hairball process, while terminal
    commands can run inside a container.  Project discovery therefore cannot
    stat the container-only ``/workspace`` path directly.  The terminal
    contract already exposes the corresponding host checkout in
    ``TERMINAL_CWD``; use it only when the Docker mount flag explicitly states
    that the two paths are the same workspace.
    """

    if str(os.environ.get("TERMINAL_ENV") or "").casefold() != "docker":
        return None
    if str(
        os.environ.get("TERMINAL_DOCKER_MOUNT_CWD_TO_WORKSPACE") or ""
    ).casefold() not in {"1", "true", "yes", "on"}:
        return None

    raw_cwd = str(cwd or "").strip()
    raw_host_root = str(os.environ.get("TERMINAL_CWD") or "").strip()
    if not raw_cwd or not raw_host_root:
        return None
    try:
        container_cwd = PurePosixPath(raw_cwd)
        workspace_root = PurePosixPath("/workspace")
        relative = container_cwd.relative_to(workspace_root)
        host_root = Path(raw_host_root).expanduser().resolve()
        mapped = host_root.joinpath(*relative.parts).resolve()
...
def classify_verification_command(
    ...
    *,
    ...
    if not command or not isinstance(command, str):
        return None
    evidence_cwd = Path(cwd or ".").resolve()
    try:
        from agent.coding_context import project_facts_for

        facts = project_facts_for(cwd)
        if not facts:
            mounted_cwd = _mounted_host_workspace_cwd(cwd)
            if mounted_cwd is not None:
                facts = project_facts_for(mounted_cwd)
                if facts:
                    evidence_cwd = mounted_cwd
    except Exception:
        facts = None
    if not facts:
        return None
    ...
    if match is None and not verify_commands:
        match = _find_manifestless_test_match(command)
        if match is None:
