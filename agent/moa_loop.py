...
MoA-enabled; the normal Hairball agent loop still owns tool calling and turn
...
class _RefAccounting:
    ...
    def __init__(
        ...
        *,
        ...
        self.billable_call_count = billable_call_count
...
class ReferenceAccountingBatch:
    """One destructive read of freshly executed MoA reference calls.

    ``routes`` intentionally keeps one entry per provider response instead of
    collapsing advisors under the virtual ``moa`` provider.  The conversation
    ledger can therefore persist tokens and cost against the route that really
    incurred them, while the aggregate fields remain convenient for the
    session total.
    """
...
def _slot_runtime(slot: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        # bedrock SigV4 signing, managed Portal tags — still fire. Those branches
...
def _run_reference(
    ...
    *,
    ...
    try:
        ...
        cost_usd = None
        actual_cost_usd = extract_provider_cost_usd(
            raw_usage,
            provider=runtime.get("provider"),
            base_url=runtime.get("base_url"),
        )
        actual_call_count = 1 if actual_cost_usd is not None else 0
        billable_call_count = 0
        cost_status = None
        cost_source = None
        try:
            cost = estimate_usage_cost(
                runtime.get("model") or slot.get("model") or "",
                usage,
                provider=runtime.get("provider"),
                base_url=runtime.get("base_url"),
            ...
            billable_call_count = 0 if cost.status == "included" else int(raw_usage is not None)
        except Exception:  # pragma: no cover - defensive
            billable_call_count = int(raw_usage is not None)
        if billable_call_count == 0:
            # Subscription-included routes are not reconciled as USD-billed
            # calls even if a compatible backend happens to expose a numeric
            # cost-looking field.
            actual_cost_usd = None
            actual_call_count = 0
        elif actual_call_count >= billable_call_count:
            cost_status = "actual"
        ...
        acct = _RefAccounting(
            ...
            messages=messages,
            output=_output_text,
            model=runtime.get("model") or slot.get("model"),
...
def _reference_messages(messages: list[dict[str, Any]]) -> list[dict[str, Any]]:
    ...
    conversation flow, but flatten it into clean user/assistant *text* turns:
      ...
      - system prompt: dropped (8K of Hairball boilerplate, not advisory signal).
...
def _preset_temperature(preset: dict[str, Any], key: str) -> float | None:
    ...
    single-model Hairball agent (which never sends temperature unless
...
def aggregate_moa_context(
    *,
    ...
    synth_prompt = (
        ...
        "Hairball agent. Focus on next steps, tool-use strategy, risks, and any "
    ...
    return (
        ...
        "normal Hairball agent loop. You may call tools, continue reasoning, or "
        "finish normally.]\n"
...
class MoAChatCompletions:
    ...
    def __init__(self, preset_name: str, reference_callback: Any = None):
        ...
        self._pending_reference_accounting = ReferenceAccountingBatch(
            usage=CanonicalUsage(request_count=0)
        )
    ...
    def consume_reference_usage(self) -> tuple[Any, Any]:
        ...
        accounting = self.consume_reference_accounting()
        return accounting.usage, accounting.estimated_cost_usd

    def consume_reference_accounting(self) -> ReferenceAccountingBatch:
        """Pop the complete per-route advisor ledger exactly once.

        The older :meth:`consume_reference_usage` pair remains as a
        compatibility adapter, but new callers use this richer value so actual
        provider charges, coverage, and route identity are not discarded.
        """
        from agent.usage_pricing import CanonicalUsage

        accounting = self._pending_reference_accounting
        self._pending_reference_accounting = ReferenceAccountingBatch(
            usage=CanonicalUsage(request_count=0)
        )
        return accounting
    ...
    def create(self, **api_kwargs: Any) -> Any:
        ...
        if _refs_from_cache:
            ...
            _ref_cost: Any = None
            _ref_actual_cost: Any = None
            _ref_actual_calls = 0
            _ref_billable_calls = 0
            _ref_routes: list[_RefAccounting] = []
            for _lbl, _txt, _acct in reference_outputs:
                if isinstance(_acct, _RefAccounting):
                    _ref_routes.append(_acct)
                    ...
                    if _acct.actual_cost_usd is not None:
                        _ref_actual_cost = (
                            (_ref_actual_cost or 0) + _acct.actual_cost_usd
                        )
                    _ref_actual_calls += int(_acct.actual_call_count or 0)
                    _ref_billable_calls += int(_acct.billable_call_count or 0)
            self._pending_reference_accounting = ReferenceAccountingBatch(
                usage=_ref_usage,
                estimated_cost_usd=_ref_cost,
                actual_cost_usd=_ref_actual_cost,
                actual_call_count=_ref_actual_calls,
                billable_call_count=_ref_billable_calls,
                routes=tuple(_ref_routes),
            )
...
class MoAClient:
    ...
    def consume_reference_accounting(self) -> ReferenceAccountingBatch:
        """Pop full MoA advisor accounting, including route attribution."""
        return self.chat.completions.consume_reference_accounting()
