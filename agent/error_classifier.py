...
class FailoverReason(enum.Enum):
    ...
    multimodal_tool_content_unsupported = "multimodal_tool_content_unsupported"  # Provider rejected list-type content in tool messages (e.g. Xiaomi) — downgrade to text and retry
...
# ``content`` to be a string.  Some (Anthropic native, reference Responses,
...
# (Xiaomi, some Alibaba endpoints, a long tail of OpenAI-compatible
...
#
# See: https://github.com/managed/hairball-agent/issues/27344
_MULTIMODAL_TOOL_CONTENT_PATTERNS = [
    # Xiaomi: {"error":{"code":"400","message":"Param Incorrect","param":"text is not set"}}
    "text is not set",
...
#   • ChatGPT cybersecurity refusal (gpt-5.5, the case from #18028)
...
_CONTENT_POLICY_BLOCKED_PATTERNS = [
    # ChatGPT (#18028) — message may arrive without an HTTP status
...
def classify_api_error(
    ...
    *,
    ...
    # (ChatGPT SDK can raise without one) isn't left in the retryable
...
def _classify_by_status(
    ...
    *,
    ...
    if status_code == 401:
        ...
        # provider-specific refresh (reference, Anthropic, managed) run before
    ...
    if status_code == 404:
        # managed API currently surfaces HA/NAS credit depletion as a paid model
...
def _classify_402(error_msg: str, result_fn) -> ClassifiedError:
    ...
    The key insight from legacy: some 402s are transient rate limits
