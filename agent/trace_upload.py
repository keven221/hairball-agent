"""Upload a Hairball session transcript to Hugging Face as an agent trace.

Hairball stores sessions in its own SQLite store (``hairball_state.SessionDB``),
...
auto-detects (Claude Code / reference / Pi). No dataset-side preprocessing is
...
* **Zero LLM turn.** This is a deterministic export — it never spends a
  model call. The ``hairball trace upload`` subcommand calls
...
* **Private by default.** Traces can contain prompts, tool output, local
  ...
  is passed through Hairball' secret redactor (``force=True``) unless the
...
DEFAULT_DATASET_NAME = "hairball-traces"
_HAIRBALL_VERSION = "hairball-agent"
...
# ---------------------------------------------------------------------------
# Conversion: Hairball OpenAI-format messages -> Claude Code JSONL
# ---------------------------------------------------------------------------
...
def _redact(text: Any, enabled: bool) -> Any:
    ...
    Non-strings pass through untouched. Uses Hairball' shared redactor with
...
def build_trace_jsonl(
    ...
    *,
    ...
    """Render Hairball conversation messages as Claude Code JSONL text.
...
_NO_TOKEN_MESSAGE = (
    ...
    "2. Add it to your environment as HF_TOKEN (e.g. in ~/.hairball/.env):\n"
    "     HF_TOKEN=hf_xxxxxxxxxxxxxxxxxxxx\n"
    "3. Run /upload-trace again (or `hairball trace upload`)."
)
...
def upload_session_trace(
    ...
    *,
    ...
    the user's private ``{user}/hairball-traces`` dataset. Returns a
