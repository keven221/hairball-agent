...
def build_turn_context(
    ...
    *,
    ...
    # Tag log records on this thread with the session ID for ``hairball logs``.
    ...
    # Official first preStep: claimed next-step is its own user/message,
    # inserted before the waking turn. Do not weld it into the typed text.
    _claimed_opening = claim_next_step_into_messages(agent, messages)
    if _claimed_opening:
        notify_next_step_claimed(
            agent,
            "\n".join(str(row.get("content") or "") for row in _claimed_opening),
            merged=False,
        )
    ...
    if agent.compression_enabled and _should_run_preflight_estimate(
        ...
        # reference app-server threads are compacted by the codex agent itself;
        # Hairball only initiates compaction in "hairball" mode (#36801).
        _codex_native_auto = (
        ...
        if _preflight_deferred:
            ...
            logger.info(
                "Skipping Hairball preflight compression for codex app-server "
                "(mode=%s); Hairball will not start thread compaction here.",
    ...
    try:
        ...
        begin_turn_workspace_tracking(agent)
    except Exception:
        # Remote/non-Git workspaces retain the existing structured-tool path.
        agent._turn_workspace_baseline = None
        agent._turn_workspace_latest = None
    agent._verification_stop_nudges = 0
    agent._pre_verify_nudges = 0
    # Plan/Execute guidance is turn-scoped model input.  Attach it to the
    # current real user turn so a restored system-prompt cache cannot swallow
    # it, and keep the durable transcript clean via the existing persistence
    # override.  This also preserves strict role alternation: no synthetic
    # follow-up user message is inserted inside the tool loop.
    _mode_instruction = str(
        getattr(agent, "_agent_mode_instruction", "") or ""
    ).strip()
    if _mode_instruction:
        _mode_note = f"\n\n[Hairball turn mode]\n{_mode_instruction}"
        _mode_content = user_msg.get("content")
        if isinstance(_mode_content, str):
            user_msg["content"] = f"{_mode_content}{_mode_note}"
            user_message = user_msg["content"]
            agent._persist_user_message_override = original_user_message
        elif isinstance(_mode_content, list):
            user_msg["content"] = [
                *_mode_content,
                {"type": "text", "text": _mode_note},
            ]
            user_message = user_msg["content"]
            agent._persist_user_message_override = original_user_message

    # Reset the unified settle controller at this real user-turn boundary.
    # Optional eager Todo guidance is folded into this real request before its
    # first provider call; no synthetic mid-history user turn is created.
    try:
        _stop_controller = getattr(agent, "_stop_controller", None)
        _todo_prelude = (
            _stop_controller.start_turn(
                ...
                enabled_tools=getattr(agent, "valid_tool_names", ()),
            )
            if _stop_controller is not None
            else ""
        )
        if _todo_prelude and isinstance(user_msg.get("content"), str):
            user_msg["content"] = f"{user_msg['content']}\n\n{_todo_prelude}"
            user_message = user_msg["content"]
            agent._persist_user_message_override = original_user_message
    except Exception:
        logger.debug("settle controller turn setup skipped", exc_info=True)

    # Completion contracts are an opt-in, session-pinned experiment.  The
    # tracker is constructed after the stable turn id exists and before the
    # first provider call, but it never changes the system prompt/tool schema.
    try:
        from agent.completion_contract import completion_contract_runtime_for_turn

        agent._completion_contract_runtime = completion_contract_runtime_for_turn(
            agent, original_user_message
        )
    except Exception:
        logger.debug("completion contract turn setup skipped", exc_info=True)
        agent._completion_contract_runtime = None
