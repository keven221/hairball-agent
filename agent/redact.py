...
# Structured handoff metadata is already parsed, so it can use an exact-key
# policy instead of relying on the text regexes below. Keep this deliberately
# narrow: canonicalised whole-key matches only, never substring matches, so
# ordinary fields such as ``token_count`` and ``session_id`` survive intact.
_SENSITIVE_STRUCTURED_KEYS = _SENSITIVE_BODY_KEYS | frozenset({
    "cookie",
    "set_cookie",
    "session",
    "session_cookie",
...
# `export HAIRBALL_REDACT_SECRETS=false`) cannot disable redaction
# mid-session.  ON by default — secure default per issue #17691. Users who
# need raw credential values in tool output (e.g. working on the redactor
# itself) can opt out via `security.redact_secrets: false` in config.yaml
# (bridged to this env var in hairball_cli/main.py, gateway/run.py, and
# cli.py) or `HAIRBALL_REDACT_SECRETS=false` in ~/.hairball/.env. An opt-out
# warning is logged at gateway and CLI startup so operators see the
# downgrade — see `_log_redaction_status()` in gateway/run.py and cli.py.
_REDACT_ENABLED = os.getenv("HAIRBALL_REDACT_SECRETS", "true").lower() in {"1", "true", "yes", "on"}
...
_PREFIX_PATTERNS = [
    ...
    r"gAAAA[A-Za-z0-9_=-]{20,}",        # reference encrypted tokens
...
def mask_secret(
    ...
    *,
    ...
    Canonical helper for display-time redaction across Hairball — used by
    ``hairball config``, ``hairball status``, ``hairball dump``, and anywhere
...
def redact_sensitive_text(
    ...
    *,
    ...
    for URLs, ``"eyJ" in text`` for JWTs). On a typical hairball log line
...
def redact_sensitive_value(value, *, force: bool = False):
    """Return a recursively redacted copy of a JSON-like value.

    Secret-bearing keys are matched by canonicalised exact name while every
    string value still receives the normal credential-pattern redaction.
    Containers are copied so callers do not lose or mutate their input.
    """
    if isinstance(value, dict):
        ...
        for key, item in value.items():
            ...
            if canonical in _SENSITIVE_STRUCTURED_KEYS and item is not None:
                redacted[key] = "«redacted-secret»"
            else:
                redacted[key] = redact_sensitive_value(item, force=force)
        return redacted
    if isinstance(value, list):
        return [redact_sensitive_value(item, force=force) for item in value]
    if isinstance(value, tuple):
        return tuple(redact_sensitive_value(item, force=force) for item in value)
