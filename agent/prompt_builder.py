...
from hairball_constants import get_hairball_home, get_skills_dir, is_wsl
...
# Prefer Hairball names; keep legacy filenames so existing repos keep working
# until users rename. Order within a directory is first-match.
_HAIRBALL_MD_NAMES = (".hairball.md", "HAIRBALL.md", ".hairball.md", "HAIRBALL.md")


def _find_hairball_md(cwd: Path) -> Optional[Path]:
    """Discover the nearest Hairball (or legacy) project-rules file.
    ...
    # could pick up a rules file planted in /tmp, /home, etc.
    search_dirs = [current, *current.parents] if stop_at else [current]

    for directory in search_dirs:
        for name in _HAIRBALL_MD_NAMES:
...
DEFAULT_AGENT_IDENTITY = (
    "You are Hairball Agent, an intelligent AI assistant created by Hairball. "
...
HAIRBALL_AGENT_HELP_GUIDANCE = (
    "You run on Hairball Agent (by Hairball). When the user needs help with "
    "Hairball itself — configuring, setting up, using, extending, or troubleshooting "
    "it — or when you need to understand your own features, tools, or capabilities, "
    "the in-repo documentation under ./website/docs/ is your authoritative reference "
    "and always holds the latest, most up-to-date information. Load the "
    "`hairball-agent` skill with skill_view(name='hairball-agent') for additional "
    "guidance and proven workflows, but treat the docs as the source of truth when "
    "the two differ."
)
...
KANBAN_GUIDANCE = (
    ...
    "the shared board at `~/.hairball/kanban.db`. Your task id is in "
    "`$HAIRBALL_KANBAN_TASK`; your workspace is `$HAIRBALL_KANBAN_WORKSPACE`. "
    ...
    "ground truth.\n"
    "2. **Work inside the workspace.** `cd $HAIRBALL_KANBAN_WORKSPACE` before "
    ...
    "\n"
    "- **Workspace.** `cd $HAIRBALL_KANBAN_WORKSPACE` first. For a `worktree` kind "
    "with no `.git`, `git worktree add <path> "
    "${HAIRBALL_KANBAN_BRANCH:-wt/$HAIRBALL_KANBAN_TASK}` from the main repo, then "
    "cd there. For a project-linked task the workspace is a fresh "
    "`<repo>/.worktrees/<task-id>` and `$HAIRBALL_KANBAN_BRANCH` a deterministic "
    ...
    "every assignee in a real profile (`hairball profile list`, or ask the user), "
    "and express dependencies via `parents=[...]` on `kanban_create`, not prose.\n"
    "\n"
    "## Do NOT\n"
    "\n"
    "- Do not shell out to `hairball kanban <verb>` for board operations. Use "
...
# Source-adapted autonomy contract.  This is static for the lifetime of a
# conversation so it preserves Hairball's prompt-cache invariant; it describes
# when to act and persist, but leaves completion decisions to the model instead
# of a regex/scoring layer in the agent loop.
TASK_COMPLETION_GUIDANCE = (
    "# Autonomy and persistence\n"
    "Unless the user explicitly asks only for a plan, an explanation, or "
    "brainstorming, treat an actionable request as permission to use the available "
    "tools and carry the task through implementation and verification. Persist "
    "until the task is handled end-to-end whenever feasible. If blocked, try "
    "reasonable alternatives and then report the concrete blocker. Never present "
    "intended work or invented output as completed work."
)
...
#
# The hairball-agent runtime already executes a batch of tool calls
...
# CONCISE_PARALLEL_TOOL_GUIDANCE — some model families got it, every other model
...
#
# Implemented on Hairball's existing prompt-assembly and concurrent dispatch
# seams; source provenance belongs in internal design evidence, not product code.
...
# Execution guidance for model families that benefit from explicit tool
# persistence, prerequisite checks and verification.  The Hairball runtime
# name describes the behavior rather than the product that informed it; source
# provenance remains in internal design evidence.  The prompt body is
# deliberately family-neutral and is selected by the existing model-capability
# mapping in ``agent.system_prompt``.
...
# Operational guidance selected for model families that benefit from concise,
# explicit file/tool discipline. Source provenance is kept in internal design
# evidence rather than the Hairball runtime name or prompt body.
CONCISE_PARALLEL_TOOL_GUIDANCE = (
...
def computer_use_guidance(platform_name: Optional[str] = None) -> str:
    ...
    return (
        ...
        "2. For buttons and rows, click by element index: "
        "`action='click', element=14`. For text fields, target the field in "
        "the type call itself: `action='type', element=21, text='...'`; do "
        "not assume every text field supports the AX `press` action. The "
        "driver binds indices to the latest snapshot and rejects stale ones.\n"
        "3. Use `coordinate=[x,y]` for canvas/WebGL/custom controls or a "
        "web input that has no trustworthy AX action. The coordinates are "
        "window-local screenshot pixels from the capture result. For key combos "
        f"`action='key', keys='{save_combo}'`. For scrolling `action='scroll', "
        "direction='down', amount=3`.\n"
        "4. After any state-changing action, re-capture to verify. If a "
        "background action did not land, retry that one action with "
        "`delivery_mode='foreground'`; do not switch the whole run to "
        "foreground or retry blindly. You can "
        ...
        "run `hairball computer-use doctor` and share the output. That command "
...
STEER_CHANNEL_NOTE = (
    ...
    "While you work, the user can send an out-of-band message that Hairball "
...
# 'system' for the system prompt.  OpenAI's newer models (GPT-5, reference)
...
PLATFORM_HINTS = {
    ...
    "tui": (
        "You are running in the Hairball terminal UI (TUI). "
        "Cron jobs scheduled from this session are LOCAL-ONLY: their output is "
        "saved (viewable via cronjob action='list') but is NOT delivered back "
    ...
    "desktop": (
        "You are chatting inside the Hairball desktop app — a graphical chat "
    ...
    "webui": (
        "You are in the Hairball WebUI, a browser-based chat interface. "
...
# container / remote host rather than on the machine where Hairball itself
...
def _probe_remote_backend(env_type: str) -> str | None:
    ...
    operate on a different machine than the host Hairball runs on.
    """
...
def build_environment_hints() -> str:
    ...
    if not is_remote_backend:
        ...
        if probe:
            hints.append(
                ...
                f"where Hairball itself is running. The host OS, home, and cwd "
                f"of the Hairball process are irrelevant; only the following "
            ...
            hints.append(
                f"Terminal backend: {backend}. Your `terminal`, `read_file`, "
                f"`write_file`, `patch`, and `search_files` tools all operate "
                f"inside {description} — NOT on the machine where Hairball "
    ...
    # Embedder-supplied environment description. Lets a host that wraps Hairball
    ...
    extra = (os.getenv("HAIRBALL_ENVIRONMENT_HINT") or "").strip()
    if not extra:
        try:
...
def _skills_prompt_snapshot_path() -> Path:
    return get_hairball_home() / ".skills_prompt_snapshot.json"
...
def _current_session_platform_hint() -> str:
    ...
    platform = os.environ.get("HAIRBALL_PLATFORM") or os.environ.get("HAIRBALL_SESSION_PLATFORM")
    if platform:
        return platform
    ...
    if get_session_env is None:
        return ""
    try:
        return get_session_env("HAIRBALL_SESSION_PLATFORM") or ""
    except Exception:
        return ""
...
def build_skills_system_prompt(
    ...
    scanned alongside the local ``~/.hairball/skills/`` directory.  External dirs
    ...
    if not skills_by_category:
        ...
        result = (
            ...
            "or troubleshoot Hairball Agent itself — its CLI, config, models, providers, tools, "
            "skills, voice, gateway, plugins, or any feature — load the `hairball-agent` skill "
            "first. It has the actual commands (e.g. `hairball config set …`, `hairball tools`, "
            "`hairball setup`) so you don't have to guess or invent workarounds.\n"
...
def build_managed_subscription_prompt(valid_tool_names: "set[str] | None" = None) -> str:
    ...
    if not managed_portal_tools_enabled():
        return ""
    ...
    def _status_line(feature) -> str:
        if feature.managed_by_managed:
            return f"- {feature.label}: active via managed subscription"
        if feature.active:
            current = feature.current_provider or "configured provider"
            return f"- {feature.label}: currently using {current}"
        if feature.included_by_default and features.managed_auth_present:
            return f"- {feature.label}: included with managed subscription, not currently selected"
        if feature.key == "modal" and features.managed_auth_present:
            return f"- {feature.label}: optional via managed subscription"
        return f"- {feature.label}: not currently available"

    lines = [
        "# managed Subscription",
        "managed subscription includes managed web tools (Firecrawl), image generation (FAL), OpenAI TTS, OpenAI Whisper STT, and browser automation (Browser Use) by default. Modal execution is optional.",
        "Current capability status:",
    ]
    lines.extend(_status_line(feature) for feature in features.items())
    lines.extend(
        [
            "When a managed-managed feature is active, do not ask the user for Firecrawl, FAL, OpenAI TTS, OpenAI Whisper, or Browser-Use API keys.",
            ...
            "Useful commands: hairball setup, hairball setup tools, hairball setup terminal, hairball status.",
        ]
    )
    return "\n".join(lines)
...
def load_soul_md(context_length: Optional[int] = None) -> Optional[str]:
    """Load SOUL.md from HAIRBALL_HOME and return its content, or None.
    ...
    """
    try:
        from hairball_cli.config import ensure_hairball_home
        ensure_hairball_home()
    except Exception as e:
        logger.debug("Could not ensure HAIRBALL_HOME before loading SOUL.md: %s", e)

    soul_path = get_hairball_home() / "SOUL.md"
...
def _load_hairball_md(cwd_path: Path, context_length: Optional[int] = None) -> str:
    """.hairball.md / HAIRBALL.md (legacy .hairball.md / HAIRBALL.md) — walk to git root."""
    hairball_md_path = _find_hairball_md(cwd_path)
    if not hairball_md_path:
        return ""
    try:
        content = hairball_md_path.read_text(encoding="utf-8").strip()
        if not content:
            return ""
        content = _strip_yaml_frontmatter(content)
        rel = hairball_md_path.name
        try:
            rel = str(hairball_md_path.relative_to(cwd_path))
        except ValueError:
            pass
        content = _scan_context_content(content, rel)
        result = f"## {rel}\n\n{content}"
        return _truncate_content(
            result, hairball_md_path.name, context_length=context_length,
            read_path=str(hairball_md_path),
        )
    except Exception as e:
        logger.debug("Could not read %s: %s", hairball_md_path, e)
        return ""
...
def build_context_files_prompt(
    ...
    Priority (first found wins — only ONE project context type is loaded):
      1. .hairball.md / HAIRBALL.md (legacy .hairball.md / HAIRBALL.md)  (walk to git root)
      2. AGENTS.md / agents.md   (cwd only)
      3. CLAUDE.md / claude.md   (cwd only)
      4. .cursorrules / .cursor/rules/*.mdc  (cwd only)

    SOUL.md from HAIRBALL_HOME is independent and always included when present.
    ...
    project_context = (
        _load_hairball_md(cwd_path, context_length)
    ...
    # SOUL.md from HAIRBALL_HOME only — skip when already loaded as identity
    if not skip_soul:
