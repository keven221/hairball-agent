...
def _render_inline_unified_diff(diff: str) -> list[str]:
    """Render unified diff lines in Hairball' inline transcript style."""
    rendered: list[str] = []
    from_file = None
    to_file = None
...
class KawaiiSpinner:
    ...
    def _animate(self):
        ...
        while self.running:
            if os.getenv("HAIRBALL_SPINNER_PAUSE"):
                time.sleep(0.1)
                continue
...
def _detect_tool_failure(tool_name: str, result: str | None) -> tuple[bool, str]:
    ...
    if isinstance(data, dict):
        # Top-level success/ok wins. Nested result rows may include
        # ``"error": null`` which must NOT trip the string heuristic below
        # (collab_collect / mailbox envelopes).
        top_err = data.get("error")
        status = str(data.get("status") or "").lower()
        if data.get("success") is True or (
            status in {"ok", "success", "completed", "dispatched", "woke", "queued"}
            and not top_err
        ):
            return False, ""
        err = top_err or data.get("message")
