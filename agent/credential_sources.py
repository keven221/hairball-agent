"""Unified removal contract for every credential source Hairball reads from.

Hairball seeds its credential pool from many places:

    env:<VAR>     — os.environ / ~/.hairball/.env
    claude_code   — ~/.claude/.credentials.json
    hairball_pkce   — ~/.hairball/.anthropic_oauth.json
    ...
    manual        — user ran `hairball auth add`
...
unify here is **removal**:
    ...
    ``hairball auth remove <provider> <N>`` must make the pool entry stay gone.

Before this module, every source had an ad-hoc removal branch in
``auth_remove_command``, and several sources had no branch at all — so
``auth remove`` silently reverted on the next ``load_pool()`` call for
qwen-cli, managed device_code (partial), hairball_pkce, copilot gh_cli, and
...
def _remove_env_source(provider: str, removed) -> RemovalResult:
    ...
    Handles three user situations:
      1. Var lives only in ~/.hairball/.env  → clear it
    ...
    """
    from hairball_cli.config import get_env_path, remove_env_value
    ...
    if shell_exported:
        result.hints.extend([
            ...
            f"(not in ~/.hairball/.env).",
            "  Unset it there (shell profile, systemd EnvironmentFile, "
            "launchd plist, etc.) or it will keep being visible to Hairball.",
            f"  The pool entry is now suppressed — Hairball will ignore "
            f"{env_var} until you run `hairball auth add {provider}`.",
        ])
    else:
        result.hints.append(
...
def _remove_claude_code(provider: str, removed) -> RemovalResult:
    ...
    work.  We just suppress it so Hairball stops reading it.
    """
    return RemovalResult(hints=[
        "Suppressed claude_code credential — it will not be re-seeded.",
        "Note: Claude Code credentials still live in ~/.claude/.credentials.json",
        "Run `hairball auth add anthropic` to re-enable if needed.",
    ])


def _remove_hairball_pkce(provider: str, removed) -> RemovalResult:
    """~/.hairball/.anthropic_oauth.json is ours — delete it outright."""
    from hairball_constants import get_hairball_home

    result = RemovalResult()
    oauth_file = get_hairball_home() / ".anthropic_oauth.json"
    if oauth_file.exists():
        try:
            oauth_file.unlink()
            result.cleaned.append("Cleared Hairball Anthropic OAuth credentials")
...
def _remove_managed_device_code(provider: str, removed) -> RemovalResult:
    """managed OAuth lives in auth.json providers.managed — clear it and suppress.

    We suppress in addition to clearing because nothing else stops a future
    `hairball auth add managed` (or any other path that writes providers.managed)
    from re-seeding before the user has decided to.  Suppression forces
    them to go through `hairball auth add managed` to re-engage, which is the
...
def _remove_minimax_oauth(provider: str, removed) -> RemovalResult:
    ...
    Same pattern as managed: single-source OAuth state with refresh tokens.
...
def _remove_xai_oauth_device_code(provider: str, removed) -> RemovalResult:
    ...
    Without this step, ``hairball auth remove xai-oauth <N>`` silently undoes
    ...
    if _clear_auth_store_provider(provider):
        result.cleaned.append(f"Cleared {provider} OAuth tokens from auth store")
    result.hints.append(
        "Run `hairball model` → xAI Grok OAuth (SuperGrok / Premium+) to re-authenticate if needed."
    )
    return result


def _remove_codex_device_code(provider: str, removed) -> RemovalResult:
    """reference tokens live in TWO places: our auth store AND ~/.codex/auth.json.

    refresh_codex_oauth_pure() writes both every time, so clearing only
    the Hairball auth store is not enough — _seed_from_singletons() would
    ...
    deleting reference CLI's file, so the reference CLI itself keeps working.
    ...
    via ``hairball auth add openai-codex``), but in both cases the re-seed
    ...
    """
    from hairball_cli.auth import suppress_credential_source
    ...
    result.hints.extend([
        ...
        "Note: reference CLI credentials still live in ~/.codex/auth.json",
        "Run `hairball auth add openai-codex` to re-enable if needed.",
    ])
    return result
...
def _remove_qwen_cli(provider: str, removed) -> RemovalResult:
    ...
    return RemovalResult(hints=[
        ...
        "Run `hairball auth add qwen-oauth` to re-enable if needed.",
    ])
...
def _remove_copilot_gh(provider: str, removed) -> RemovalResult:
    ...
    Hairball stops picking the token up.
    """
    ...
    return RemovalResult(hints=[
        ...
        "Run `hairball auth add copilot` to re-enable if needed.",
    ])
...
def _register_all_sources() -> None:
    ...
    register(RemovalStep(
        provider="anthropic", source_id="hairball_pkce",
        remove_fn=_remove_hairball_pkce,
        description="~/.hairball/.anthropic_oauth.json",
    ))
    register(RemovalStep(
