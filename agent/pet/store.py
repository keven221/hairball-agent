...
Pets live under ``get_hairball_home()/pets/<slug>/`` so every profile gets its
...
_BUNDLED_PETS_DIR = Path(__file__).with_name("defaults")
_DEFAULT_SEED_MARKER = ".hairball-default-pets-v1"
...
def pets_dir() -> Path:
    ...
    path = get_hairball_home() / "pets"
    path.mkdir(parents=True, exist_ok=True)
    return path


def _usable_pet_directory(directory: Path) -> bool:
    if not directory.is_dir() or directory.name.startswith("."):
        return False
    meta = _read_pet_json(directory)
    return _resolve_spritesheet(directory, meta).is_file()


def _mark_default_seed_complete(root: Path) -> None:
    try:
        (root / _DEFAULT_SEED_MARKER).touch(exist_ok=True)
    except OSError as exc:
        # A read-only profile still gets a non-fatal empty gallery.  Do not
        # claim the seed succeeded so a later writable launch can retry.
        logger.debug("could not mark bundled Pet seed complete: %s", exc)


def _seed_bundled_pets_once(root: Path) -> None:
    """Seed Hairball's own default Pet into a brand-new profile exactly once.

    Existing profiles are never cluttered or overwritten.  The marker is
    written even when an existing usable Pet caused the seed to be skipped, so
    removing the user's last Pet later remains a durable choice.  Copying via
    a unique sibling and rename keeps readers from observing half a sprite.
    """
    marker = root / _DEFAULT_SEED_MARKER
    if marker.is_file():
        return

    try:
        if any(_usable_pet_directory(child) for child in root.iterdir()):
            _mark_default_seed_complete(root)
            return
    except OSError as exc:
        logger.debug("could not inspect Pet directory for defaults: %s", exc)
        return

    if not _BUNDLED_PETS_DIR.is_dir():
        return

    seeded = False
    for source in sorted(_BUNDLED_PETS_DIR.iterdir()):
        if not _usable_pet_directory(source):
            continue
        slug = _safe_slug(source.name)
        if not slug or slug != source.name:
            continue
        destination = root / slug
        if destination.exists():
            seeded = seeded or _usable_pet_directory(destination)
            continue
        temporary = root / f".{slug}.seed-{os.getpid()}-{uuid.uuid4().hex}"
        try:
            shutil.copytree(source, temporary)
            try:
                temporary.replace(destination)
            except OSError:
                # Another Hairball surface may have won the same first-launch
                # race.  Treat its complete destination as success.
                if not _usable_pet_directory(destination):
                    raise
            seeded = seeded or _usable_pet_directory(destination)
        except OSError as exc:
            logger.warning("could not seed bundled Pet %s: %s", slug, exc)
        finally:
    ...
    if sheet_bytes is None and source_url and _is_petdex_host(source_url):
        try:
            ...
            resp = httpx.get(
                ...
                headers={"User-Agent": "hairball-agent-petdex"},
            )
...
def remove_pet(slug: str) -> bool:
    ...
    # A direct CLI removal may happen before any list/gallery call.  Record
    # that this is an established profile before it becomes empty, otherwise
    # the next list would mistake the intentional removal for a first launch
    # and seed the bundled default back in.
    _mark_default_seed_complete(root)
