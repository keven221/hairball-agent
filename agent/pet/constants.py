...
These values are the common petdex/reference pet geometry. The real ``pet.json``
usually only carries ``id``/``displayName``/``description``/``spritesheetPath``;
row taxonomy is inferred from the atlas shape so Hairball can render both legacy
8-row sheets and current 9-row reference sheets.
"""
...
# Frame geometry (pixels). Current reference/petdex spritesheets are 8 columns x 9
# rows (1536x1872), while older Hairball/petdex sheets used 9 columns x 8 rows
...
class PetState(str, Enum):
    ...
    These are Hairball' activity state names. They are not always identical to the
    source atlas row names: reference-format pets use rows like ``jumping`` /
...
# Legacy Hairball/petdex row order (top -> bottom) used by the older 8-row,
...
# Default/fallback for callers without a sheet. Prefer the current 9-row reference
# format because generated pets and the public reference pet contract use it.
STATE_ROWS: list[str] = CODEX_STATE_ROWS

# Canonical Hairball activity names -> accepted row-name aliases in descending
