"""Deterministic spritesheet assembly — generated row strips → Hairball atlas.
...
The atlas follows the **petdex/reference standard**: 8 columns x 9 rows of
...
# driven state, and the per-row frame counts mirror the petdex/reference
...
def mirror_frames(frames: list) -> list:
    ...
    animation backwards), matching the petdex/reference mirror rule.
    """
    from PIL import Image
...
def compose_atlas(frames_by_state: dict[str, list]):
    """Pack per-state frame lists into the Hairball atlas (RGBA, residue-cleared).
