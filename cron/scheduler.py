...
Uses a file-based lock (~/.hairball/cron/.tick.lock) so only one tick
...
# Without this, standalone invocations (e.g. after `hairball update` reloads
# the module) fail with ModuleNotFoundError for hairball_time et al.
sys.path.insert(0, str(Path(__file__).parent.parent))

from hairball_constants import get_hairball_home
from hairball_cli._subprocess_compat import windows_hide_flags
from hairball_cli.config import load_config, _expand_env_vars
from hairball_cli.fallback_config import get_fallback_chain
from hairball_time import now as _hairball_now
...
def _merge_mcp_into_per_job_toolsets(per_job: list[str], cfg: dict) -> list[str]:
    ...
    if "no_mcp" in per_job:
        return result
    # lazy import: avoid heavy hairball_cli import at cron module load (matches
    # _resolve_cron_enabled_toolsets' fallback) and share one MCP-membership
    # computation with the gateway/CLI platform resolver.
    from hairball_cli.tools_config import enabled_mcp_server_names
...
def _resolve_cron_enabled_toolsets(job: dict, cfg: dict) -> list[str] | None:
    ...
    2. Per-platform ``hairball tools`` config for the ``cron`` platform.
    ...
    try:
        from hairball_cli.tools_config import _get_platform_tools  # lazy: avoid heavy import at cron module load
...
def _interpreter_shutting_down(exc: Optional[BaseException] = None) -> bool:
    ...
    ``hairball update`` / ``hairball gateway stop`` / systemd restart, or an
...
_hairball_home: Path | None = None


def _get_hairball_home() -> Path:
    """Resolve Hairball home dynamically while preserving test monkeypatch hooks.

    Cron is per-profile by design (#4707): the in-process ticker runs inside a
    profile-scoped gateway, so resolving the active HAIRBALL_HOME at call time
    ...
    """
    return _hairball_home or get_hairball_home()


def _get_lock_paths() -> tuple[Path, Path]:
    """Resolve cron lock paths at call time so profile/env changes are honored."""
    hairball_home = _get_hairball_home()
    lock_dir = hairball_home / "cron"
...
def _open_continuable_cron_thread(
    ...
    thread_name = f"Hairball — {task_name}"
    try:
...
def _get_script_timeout() -> int:
    ...
    env_value = os.getenv("HAIRBALL_CRON_SCRIPT_TIMEOUT", "").strip()
    if env_value:
        try:
            ...
            logger.warning("Invalid HAIRBALL_CRON_SCRIPT_TIMEOUT=%r; using config/default", env_value)
...
def _run_job_script(script_path: str) -> tuple[bool, str]:
    ...
    Scripts must reside within HAIRBALL_HOME/scripts/.  Both relative and
    ...
    * anything else — run with the current Python interpreter
    ...
    provider credentials and other Hairball-managed secrets are not inherited
    (SECURITY.md §2.3), matching terminal and MCP child processes.

    Args:
        script_path: Path to the script.  Relative paths are resolved
            against HAIRBALL_HOME/scripts/.  Absolute and ~-prefixed paths
    ...
    """
    scripts_dir = _get_hairball_home() / "scripts"
    ...
    # escape — scripts MUST reside within HAIRBALL_HOME/scripts/.
    try:
...
def run_job(
    ...
    if job.get("no_agent"):
        ...
        now_iso = _hairball_now().strftime("%Y-%m-%d %H:%M:%S")
    ...
    #
    # Bounded with its own timeout (separate from HAIRBALL_CRON_TIMEOUT, which
    ...
    try:
        ...
        _raw_env_timeout = os.getenv("HAIRBALL_CRON_SESSION_DB_TIMEOUT", "").strip()
        if _raw_env_timeout:
            try:
                _session_db_timeout = float(_raw_env_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid HAIRBALL_CRON_SESSION_DB_TIMEOUT=%r; using config/default",
                    _raw_env_timeout,
                )
    ...
    if script_path:
        ...
        if _ran_ok and not _parse_wake_gate(_script_output):
            ...
            silent_doc = (
                f"# Cron Job: {job_name}\n\n"
                f"**Job ID:** {job_id}\n"
                f"**Run Time:** {_hairball_now().strftime('%Y-%m-%d %H:%M:%S')}\n\n"
    ...
    origin = _resolve_origin(job)
    _cron_session_id = f"cron_{job_id}_{_hairball_now().strftime('%Y%m%d_%H%M%S')}"
    ...
    os.environ["HAIRBALL_CRON_SESSION"] = "1"
    ...
    # gateway message. Do not seed HAIRBALL_SESSION_* contextvars from the
    ...
    #     (notify_on_complete / watch_patterns) reads HAIRBALL_SESSION_PLATFORM
    #     and HAIRBALL_SESSION_CHAT_ID to populate watcher_platform / chat_id,
    #     which would route completion notifications to the origin chat
    #     instead of via HAIRBALL_CRON_AUTO_DELIVER_* below.
    #   - tools/tts_tool.py: picks Opus vs MP3 based on
    #     HAIRBALL_SESSION_PLATFORM == "telegram".
    ...
    #     HAIRBALL_SESSION_PLATFORM.
    #   - tools/send_message_tool.py: mirror source labelling and the
    #     send_message gate read HAIRBALL_SESSION_PLATFORM.
    # Cron output delivery itself reads job["origin"] directly via
    # _resolve_origin(job) and the HAIRBALL_CRON_AUTO_DELIVER_* vars set
    # below, so clearing HAIRBALL_SESSION_* here does not affect delivery.
    ...
    _cron_delivery_vars = (
        "HAIRBALL_CRON_AUTO_DELIVER_PLATFORM",
        "HAIRBALL_CRON_AUTO_DELIVER_CHAT_ID",
        "HAIRBALL_CRON_AUTO_DELIVER_THREAD_ID",
    )
    ...
    try:
        ...
        # load_hairball_dotenv (not a bare load_dotenv) and reset the secret-
        # source cache first: startup already applied external secrets and
        # recorded this HAIRBALL_HOME in _APPLIED_HOMES, so a naive reload would
        ...
        # forced re-pull off the network. load_hairball_dotenv also handles the
        # utf-8/latin-1 encoding fallback internally.
        from hairball_cli.env_loader import (
            load_hairball_dotenv,
            reset_secret_source_cache,
        )
        reset_secret_source_cache()
        load_hairball_dotenv(hairball_home=_get_hairball_home())

        delivery_target = _resolve_delivery_target(job)
        if delivery_target:
            _VAR_MAP["HAIRBALL_CRON_AUTO_DELIVER_PLATFORM"].set(delivery_target["platform"])
            _VAR_MAP["HAIRBALL_CRON_AUTO_DELIVER_CHAT_ID"].set(str(delivery_target["chat_id"]))
            _VAR_MAP["HAIRBALL_CRON_AUTO_DELIVER_THREAD_ID"].set(
                ""
                if delivery_target.get("thread_id") is None
                else str(delivery_target["thread_id"])
            )

        # Model resolution precedence: per-job override > HAIRBALL_MODEL env >
        ...
        model = job.get("model") or os.getenv("HAIRBALL_MODEL") or ""

        # Load config.yaml for model, reasoning, prefill, toolsets, provider routing
        _cfg = {}
        try:
            import yaml
            _cfg_path = str(_get_hairball_home() / "config.yaml")
        ...
        if not (isinstance(model, str) and model.strip()):
            raise RuntimeError(
                ...
                f"HAIRBALL_MODEL={os.getenv('HAIRBALL_MODEL', '')!r}, "
                "config.yaml model.default missing or empty). "
                f"Set a per-job model via "
                f"`cronjob action=update job_id={job_id} model=<name>` or set a "
                "default with `hairball model <name>`."
            )

        # Apply IPv4 preference if configured.
        try:
            from hairball_constants import apply_ipv4_preference
        ...
        # model (per-job override > HAIRBALL_MODEL env > config.yaml default).
        ...
        prefill_file = (
            os.getenv("HAIRBALL_PREFILL_MESSAGES_FILE", "")
        ...
        if prefill_file:
            ...
            if not pfpath.is_absolute():
                pfpath = _get_hairball_home() / pfpath
            if pfpath.exists():
                try:
        ...
        try:
            # Do not inject HAIRBALL_INFERENCE_PROVIDER here. resolve_runtime_provider()
        ...
        # to a paid PROVIDER (e.g. managed) OR a paid MODEL on the same provider
        ...
        agent = AIAgent(
            ...
            # HAIRBALL_HOME. When a workdir is configured, also inject project
        ...
        # override via HAIRBALL_CRON_TIMEOUT env var.  0 = unlimited.
        #
        # Uses the agent's built-in activity tracker (updated by
        # _touch_activity() on every tool call, API call, and stream delta).
        _raw_cron_timeout = os.getenv("HAIRBALL_CRON_TIMEOUT", "").strip()
        if _raw_cron_timeout:
            try:
                _cron_timeout = float(_raw_cron_timeout)
            except (ValueError, TypeError):
                logger.warning(
                    "Invalid HAIRBALL_CRON_TIMEOUT=%r; using default 600s",
                    _raw_cron_timeout,
                )
                _cron_timeout = 600.0
        else:
        ...
        if isinstance(completion_assessment, dict):
            completion_section = (
                ...
                "```json\n"
                f"{json.dumps(completion_assessment, ensure_ascii=False, sort_keys=True, indent=2)}\n"
                "```\n"
            )
...
**Job ID:** {job_id}
**Run Time:** {_hairball_now().strftime('%Y-%m-%d %H:%M:%S')}
...
"""
        ...
        if _session_db:
            ...
            try:
                ...
                _cron_title = f"{_title_base} · {_hairball_now().strftime('%b %d %H:%M')}"
...
def run_one_job(job: dict, *, adapters=None, loop=None, verbose: bool = False) -> bool:
    ...
    try:
        ...
        _scope_token = set_secret_scope(
            build_profile_secret_scope(_get_hairball_home())
        )
    ...
    *,
    ...
    try:
        ...
        if verbose and not due_jobs:
            logger.info("%s - No jobs due", _hairball_now().strftime('%H:%M:%S'))
            return 0

        if verbose:
            logger.info("%s - %s job(s) due", _hairball_now().strftime('%H:%M:%S'), len(due_jobs))
        ...
        # Set HAIRBALL_CRON_MAX_PARALLEL=1 to restore old serial behaviour.
        _max_workers: Optional[int] = None
        try:
            _env_par = os.getenv("HAIRBALL_CRON_MAX_PARALLEL", "").strip()
            if _env_par:
                _max_workers = int(_env_par) or None
        except (ValueError, TypeError):
            logger.warning("Invalid HAIRBALL_CRON_MAX_PARALLEL value; defaulting to unbounded")
