...
renders natively:
  ...
  * Docs catalog          -> a copy-paste command + a ``hairball://`` deep-link
...
def blueprint_deeplink(blueprint: AutomationBlueprint, values: Optional[Dict[str, Any]] = None) -> str:
    """Build the ``hairball://blueprint/<key>?slot=val`` deep-link URL."""
    ...
    return f"hairball://blueprint/{quote(blueprint.key)}{qs}"
