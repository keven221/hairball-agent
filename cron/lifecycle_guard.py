...
``hairball gateway restart`` (or ``launchctl kickstart ai.hairball.gateway``
or ``systemctl restart hairball-gateway``).  When the cron fires, the
...
``hairball cron create`` CLI subcommand AND the agent's ``cronjob`` model
...
command identifier (``hairball gateway``, ``launchctl ... hairball-gateway``,
``systemctl ... hairball-gateway``, ``pkill`` against the gateway) so it
...
blocks these commands at *execution* time when ``_HAIRBALL_GATEWAY=1``, and
``hairball gateway stop|restart`` refuse to self-target from inside the
...
_GATEWAY_LIFECYCLE_PATTERN = re.compile(
    r"(?i)"
    # Branch A: `hairball gateway restart|stop` — the canonical foot-gun.
    # `start` is intentionally excluded: starting a gateway from inside a
    # gateway is benign (a no-op or "already running" error), and a
    # legitimate cron job might start a sibling profile's gateway.
    r"(?:hairball\s+gateway\s+(?:restart|stop))"
    # Branch B: launchctl ops on a hairball-gateway label. macOS launchd
    # labels look like `ai.hairball.gateway` / `hairball-gateway`. Requiring the
    # gateway identifier prevents blocking unrelated hairball services (e.g.
    # `launchctl unload ai.hairball.update-checker.plist`).
    r"|(?:launchctl\s+(?:kickstart|unload|load|stop|restart)\b[^\n]*\bhairball[.\-]?gateway)"
    # Branch C: systemctl ops on a hairball-gateway unit.
    r"|(?:systemctl\s+(?:-\S+\s+)*(?:restart|stop|start)\b[^\n]*\bhairball[.\-]?gateway)"
    # Branch D: pkill / kill targeting the hairball gateway process. Both
...
def _resolve_script_path(script_path: str) -> Path:
    ...
    under ``<HAIRBALL_HOME>/scripts/`` and only accepts absolute paths as-is.
    ...
    (``~/.hairball/scripts/restart.sh``) but is passed as the bare name
    ...
    if raw.is_absolute():
        return raw
    return get_hairball_home() / "scripts" / raw
...
def check_gateway_lifecycle(
    ...
    if contains_gateway_lifecycle_command(combined):
        raise GatewayLifecycleBlocked(
            ...
            "(#30719). Run `hairball gateway restart` from a shell outside "
