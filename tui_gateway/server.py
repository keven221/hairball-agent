...
load_hairball_dotenv(
    hairball_home=_hairball_home, project_env=Path(__file__).parent.parent / ".env"
)
...
# appends every unhandled exception to ~/.hairball/logs/tui_gateway_crash.log
...
_CRASH_LOG = os.path.join(_hairball_home, "logs", "tui_gateway_crash.log")
...
try:
    _slash_timeout = float(os.environ.get("HAIRBALL_TUI_SLASH_TIMEOUT_S") or "45")
...
try:
    _ws_orphan_reap_grace = float(
        os.environ.get("HAIRBALL_TUI_WS_ORPHAN_REAP_GRACE_S") or "20"
    )
except (ValueError, TypeError):
...
try:
    _rpc_pool_workers = max(
        2, int(os.environ.get("HAIRBALL_TUI_RPC_POOL_WORKERS") or "8")
    )
except (ValueError, TypeError):
    _rpc_pool_workers = 4
...
class _SlashWorker:
    """Persistent HairballCLI subprocess for slash commands."""
    ...
    def __init__(self, session_key: str, model: str, profile_home: str | None = None):
        ...
        # slash_worker runs the Hairball agent → needs provider credentials.
        # Tier-1 secrets (gateway/GitHub/infra) are still stripped (#29157).
        env = hairball_subprocess_env(inherit_credentials=True)
        if profile_home:
            ...
            # gateway's launch HAIRBALL_HOME (#40677).
            env["HAIRBALL_HOME"] = str(profile_home)
...
def _transfer_active_session_slot(
    ...
    *,
    ...
    if lease is None:
        return True
    try:
        from hairball_cli.active_sessions import transfer_active_session
...
def _finalize_session(session: dict | None, end_reason: str = "tui_close") -> None:
    ...
    if agent is not None:
        try:
            from hairball_cli.plugins import invoke_hook
...
def _close_sessions_for_transport(
    ...
    for sid, session in owned:
        if session.get("close_on_disconnect"):
            ...
            # actually fire; a standalone `hairball --tui` keeps real _stdio.
...
try:
    _SESSION_TTL_S = float(os.environ.get("HAIRBALL_TUI_SESSION_TTL_S") or 6 * 3600)
...
def _transport_is_dead(transport) -> bool:
    ...
    # for a standalone `hairball --tui`, so it must NOT count as dead here (doing
...
def _max_live_sessions() -> int:
    try:
        from hairball_cli.active_sessions import coerce_max_concurrent_sessions
...
# ``profile`` on those calls; we open that profile's db and bind its HAIRBALL_HOME
...
def _profile_home(profile: str | None) -> Path | None:
    ...
    if home.resolve() == Path(_hairball_home).resolve():
        return None
    return home if (home / "state.db").exists() or home.exists() else None


def _profile_scoped(handler):
    """Bind ``params['profile']``'s HAIRBALL_HOME around a pet RPC handler.

    Pets are per-profile: ``display.pet.*`` lives in the profile's config.yaml and
    sprites install under its ``pets/`` dir (both resolve via ``get_hairball_home``).
    ...
    def wrapper(rid, params):
        ...
        token = set_hairball_home_override(home)
        try:
            return handler(rid, params)
        finally:
...
def _launch_configured_cwd() -> str | None:
    ...
    (wherever ``hairball dashboard`` was launched) instead of the configured
...
def _start_agent_build(sid: str, session: dict) -> None:
    ...
    Classic `hairball` shows the prompt before constructing AIAgent; the TUI used
    ...
    def _build() -> None:
        ...
        try:
            ...
            # HAIRBALL_HOME so config/skills/model resolve to it, and hand the
            # agent that profile's db so turns persist to the right state.db.
            session_db = None
            if profile_home:
                home_token = set_hairball_home_override(profile_home)
                try:
                    from hairball_state import SessionDB
            ...
            if home_token is not None:
                reset_hairball_home_override(home_token)
...
def _register_session_cwd(session: dict | None) -> None:
    ...
    try:
        ...
        try:
            ...
            if cwd:
                ensure_coding_workspace_loaded(key, seed_cwd=cwd)
                set_active_coding_task_id(key)
        except Exception:
            pass
    except Exception:
        pass
...
def _ensure_session_db_row(session: dict) -> None:
    ...
    if str(model_config.get("provider") or "").strip().lower() == "custom":
        try:
            from hairball_cli.runtime_provider import canonical_custom_identity
...
def _set_session_cwd(session: dict, cwd: str) -> str:
    from hairball_constants import translate_cwd_for_wsl_backend
...
def _load_cfg() -> dict:
    ...
    try:
        ...
        # launch profile's _hairball_home. Cache is keyed on the resolved path, so
        # profiles don't clobber each other.
        override = get_hairball_home_override()
        home = override if isinstance(override, str) and override else _hairball_home
...
def _apply_managed(cfg: dict) -> dict:
    ...
    hairball_cli.config.load_config, so without this a managed skin / reasoning_effort
...
def _save_cfg(cfg: dict):
    ...
    from hairball_cli.config import atomic_config_write

    path = _hairball_home / "config.yaml"
...
def _enable_gateway_prompts() -> None:
    ...
    os.environ["HAIRBALL_GATEWAY_SESSION"] = "1"
    os.environ["HAIRBALL_EXEC_ASK"] = "1"
...
def resolve_skin() -> dict:
    try:
        from hairball_cli.skin_engine import init_skin_from_config, get_active_skin
...
def _resolve_model() -> str:
    env = (
        os.environ.get("HAIRBALL_MODEL", "")
        or os.environ.get("HAIRBALL_INFERENCE_MODEL", "")
    ).strip()
    if env:
        return env
    ...
    try:
        from hairball_cli.models import PREFERRED_SILENT_DEFAULT_MODEL
...
def _resolve_session_platform() -> str:
    ...
    Resolution:
      * ``HAIRBALL_DESKTOP=1`` and ``HAIRBALL_DESKTOP_TERMINAL`` unset → "desktop"
        (the chat-panel backend — a graphical React surface, not a terminal).
      * ``HAIRBALL_DESKTOP_TERMINAL=1`` → "tui"
        (``hairball --tui`` running in the desktop's embedded terminal pane;
        it IS a TUI, just embedded. The clarifier attached to the tui hint
        in system_prompt.py tells the agent about the embedding.)
      * neither set → "tui"
        (standalone ``hairball --tui``.)
    """
    if is_truthy_value(os.environ.get("HAIRBALL_DESKTOP")) and not is_truthy_value(
        os.environ.get("HAIRBALL_DESKTOP_TERMINAL")
    ):
        return "desktop"
    return "tui"
...
def _config_model_target() -> tuple[str, str]:
    ...
    Unlike `_resolve_model()`, this never reads HAIRBALL_MODEL /
    HAIRBALL_INFERENCE_MODEL. Those env vars are a launch-scoped seed
    (`hairball --tui -m <model>`, hosted-instance provisioning); if they
    ...
    # No fallback to _resolve_model() here: that reads HAIRBALL_MODEL /
    # HAIRBALL_INFERENCE_MODEL, which `hairball --tui -m <model>` sets as a
...
def _resolve_startup_runtime() -> tuple[str, str | None]:
    ...
    explicit_provider = os.environ.get("HAIRBALL_TUI_PROVIDER", "").strip()
    ...
    try:
        from hairball_cli.models import detect_static_provider_for_model
        ...
        current_provider = (
            ...
            or os.environ.get("HAIRBALL_INFERENCE_PROVIDER", "").strip().lower()
            or "auto"
        )
...
def _runtime_model_config(agent, existing: dict | None = None) -> dict:
    ...
    if provider:
        if provider.strip().lower() == "custom":
            ...
            try:
                from hairball_cli.runtime_provider import (
...
def _load_approval_mode() -> str:
    from hairball_cli.config import DEFAULT_CONFIG, _deep_merge
...
def _load_reasoning_config(model: str = "") -> dict | None:
    ...
    :func:`hairball_constants.resolve_reasoning_config` (per-model override >
...
def _load_tool_progress_mode() -> str:
    env = os.environ.get("HAIRBALL_TUI_TOOL_PROGRESS", "").strip().lower()
...
def _load_enabled_toolsets() -> list[str] | None:
    explicit = [
        item.strip()
        for item in os.environ.get("HAIRBALL_TUI_TOOLSETS", "").split(",")
        if item.strip()
    ]
    cfg = None
    fallback_notice = None

    # Coding posture (base Hairball): with no explicit pin, collapse to the
    # coding toolset (+ enabled MCP servers) when sitting in a code workspace.
    # The desktop app and `hairball --tui` both land here. See
    ...
    if not explicit:
        try:
            ...
            if selection is not None:
                ...
                # ``coding`` already lists ``lsp``; keep project gate as before.
    ...
    if explicit and validate_toolset is not None:
        ...
        if any(name in {"all", "*"} for name in built_in):
            ...
            if ignored:
                print(
                    "[tui] HAIRBALL_TUI_TOOLSETS=all enables every toolset; "
        ...
        try:
            ...
            from hairball_cli.tools_config import _parse_enabled_flag
        ...
        if unknown:
            print(
                f"[tui] ignoring unknown HAIRBALL_TUI_TOOLSETS entries: {', '.join(unknown)}",
                file=sys.stderr,
                flush=True,
            )
        if disabled:
            print(
                "[tui] ignoring disabled MCP servers in HAIRBALL_TUI_TOOLSETS "
        ...
        fallback_notice = (
            "[tui] no valid HAIRBALL_TUI_TOOLSETS entries; using configured CLI toolsets"
        )
    ...
    try:
        ...
        if not enabled:
            return None
        # The desktop Project tools are off _HAIRBALL_CORE_TOOLS (every other
        # platform would carry their schema for nothing), so the platform
        # recovery above — which keys off hairball-cli's tool universe — can't
        ...
        # W0: coding auto → merge edge toolsets (lsp) without full focus collapse.
        try:
            from agent.coding_context import merge_coding_edge_toolsets

            merged = merge_coding_edge_toolsets(
                sorted(enabled),
                platform=_resolve_session_platform(),
                config=cfg,
            )
            return sorted({*merged, "project"})
        except Exception:
            return sorted(enabled | {"project"})
    except Exception:
        if fallback_notice is not None:
            print(
                "[tui] no valid HAIRBALL_TUI_TOOLSETS entries and configured CLI toolsets could not be loaded; enabling all toolsets",
                file=sys.stderr,
                flush=True,
            )
        return None
...
def _load_tool_scopes() -> tuple[list[str] | None, list[str] | None]:
    """Return hard authorization and initial-schema visibility separately."""
    explicit = any(
        item.strip()
        for item in os.environ.get("HAIRBALL_TUI_TOOLSETS", "").split(",")
    )
    ...
    try:
        ...
        if coding_selection(platform=_resolve_session_platform()) is not None:
...
def _apply_model_switch(
    ...
    *,
    ...
    from hairball_cli.runtime_provider import resolve_runtime_provider
    ...
    custom_provs = None
    try:
        from hairball_cli.config import get_compatible_custom_providers, load_config
    ...
    #
    # We deliberately do NOT write process-global env vars (HAIRBALL_MODEL /
    # HAIRBALL_INFERENCE_MODEL / HAIRBALL_TUI_PROVIDER / HAIRBALL_INFERENCE_PROVIDER)
...
def _sync_agent_model_with_config(sid: str, session: dict) -> None:
    ...
    try:
        _apply_model_switch(
            ...
            # target the sync computed — the path that leaked `hairball --tui -m`
...
def _get_usage(agent) -> dict:
    ...
    try:
        ...
        usage.update(
            {
                "cost_usd": cost.display_cost_usd,
                "estimated_cost_usd": cost.estimated_cost_usd,
                "actual_cost_usd": cost.actual_cost_usd,
                "cost_actual_call_count": cost.actual_call_count,
                "cost_total_call_count": cost.total_call_count,
                "actual_cost_coverage": cost.actual_coverage,
                "actual_cost_complete": cost.actual_complete,
    ...
    # HAIRBALL_DEV_CREDITS so the payload stays clean when the flag is off.
    if is_truthy_value(os.environ.get("HAIRBALL_DEV_CREDITS")):
        try:
...
def _session_info(agent, session: dict | None = None) -> dict:
    ...
    try:
        ...
        pass
    try:
        from hairball_cli.banner import get_available_skills
    ...
    try:
        ...
        pass
    try:
        from hairball_cli.banner import get_update_result
...
def _on_tool_progress(
    ...
    **_kwargs,
    ...
    if event_type.startswith("subagent."):
        ...
        if _kwargs.get("phase"):
            payload["phase"] = str(_kwargs["phase"])
        if _kwargs.get("is_error") is not None:
            payload["is_error"] = bool(_kwargs["is_error"])
        if isinstance(_args, dict):
            payload["args"] = {
                str(k)[:80]: (str(v)[:800] if not isinstance(v, (dict, list)) else v)
                for k, v in list(_args.items())[:40]
            }
        if _kwargs.get("result") is not None:
            payload["result"] = str(_kwargs["result"])[:12000]
        if preview and event_type in {"subagent.tool", "subagent.tool_result"}:
...
def _mirror_subagent_to_child(event_type: str, payload: dict) -> None:
    ...
    with _child_mirrors_lock:
        ...
        elif event_type == "subagent.tool_result":
            tool = st.get("open_tool") or {
                "name": str(payload.get("tool_name") or "tool"),
                "tool_id": f"submirror:{child_key}:result",
                "args": payload.get("args") if isinstance(payload.get("args"), dict) else {},
            }
            if isinstance(payload.get("args"), dict) and not tool.get("args"):
                tool["args"] = payload["args"]
            if payload.get("result") is not None:
                tool["result"] = str(payload.get("result") or "")[:12000]
            if preview := str(payload.get("tool_preview") or payload.get("text") or ""):
                tool["preview"] = preview
...
def _wire_callbacks(sid: str):
    ...
    def secret_cb(env_var, prompt, metadata=None):
        ...
        from hairball_cli.config import save_env_value_secure
...
def _available_personalities(cfg: dict | None = None) -> dict:
    try:
        ...
        try:
            from hairball_cli.config import load_config as _load_full_cfg
...
def _cfg_max_turns(cfg: dict, default: int) -> int:
    try:
        env_max = int(os.environ.get("HAIRBALL_TUI_MAX_TURNS", "") or 0)
        if env_max > 0:
            return env_max
    except (TypeError, ValueError):
        pass
...
def _parse_tui_skills_env() -> list[str]:
    raw = os.environ.get("HAIRBALL_TUI_SKILLS", "")
    skills: list[str] = []
    seen: set[str] = set()
...
def _load_fallback_model():
    ...
    stays in parity with ``HairballCLI.__init__`` and ``gateway/run.py``:
...
def _resolve_runtime_with_fallback(
    ...
    ``hairball_cli/cli_agent_setup_mixin.py``: when the primary provider
    raises ``AuthError``, walk the configured ``fallback_providers`` /
    ``fallback_model`` chain before giving up.
    """
    from hairball_cli.auth import AuthError
    from hairball_cli.runtime_provider import resolve_runtime_provider
...
def _make_agent(
    ...
    # block. Dashboard /api/ws uses hairball_cli.mcp_startup; TUI stdio keeps
    ...
    enabled_toolsets, visible_toolsets = _load_tool_scopes()
    return AIAgent(
        model=model,
        ...
        checkpoints_enabled=is_truthy_value(os.environ.get("HAIRBALL_TUI_CHECKPOINTS")),
        pass_session_id=is_truthy_value(os.environ.get("HAIRBALL_TUI_PASS_SESSION_ID")),
        skip_context_files=is_truthy_value(os.environ.get("HAIRBALL_IGNORE_RULES")),
        skip_memory=is_truthy_value(os.environ.get("HAIRBALL_IGNORE_RULES")),
...
def _(rid, params: dict) -> dict:
    ...
    # and each turn re-bind HAIRBALL_HOME. None/own profile → launch (unchanged).
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        # custom `HAIRBALL_SESSION_SOURCE` values, and older installs with
...
def _(rid, params: dict) -> dict:
    ...
    # Completion assessments are durable UI evidence, not conversation
    # messages.  Read the latest terminal snapshot across the compression
    # lineage once for this resume and attach it to every success shape below;
    # never inject it into ``history`` or model replay.
    try:
        read_assessment = getattr(
            db, "read_latest_terminal_completion_assessment_for_lineage", None
        )
        resume_completion_assessment = (
            read_assessment(target) if callable(read_assessment) else None
        )
    except Exception:
        logger.debug("failed to read completion assessment for resume", exc_info=True)
        resume_completion_assessment = None

    def _attach_resume_completion_assessment(payload: dict) -> dict:
        if isinstance(resume_completion_assessment, dict) and resume_completion_assessment:
            payload["completion_assessment"] = resume_completion_assessment
        return payload
    ...
    def _reuse_live_payload(sid: str, session: dict) -> dict:
        ...
        return _attach_resume_completion_assessment(payload)
    ...
    if is_truthy_value(params.get("lazy", False)):
        ...
        return _ok(
            rid,
            _attach_resume_completion_assessment({
                "session_id": sid,
                "resumed": target,
    ...
    _enable_gateway_prompts()
    home_token = (
        set_hairball_home_override(str(profile_home)) if profile_home is not None else None
    )
    try:
        db.reopen_session(target)
        raw_history = db.get_messages_as_conversation(target)
    ...
    with _session_resume_lock:
        ...
        if live is not None:
            ...
            payload["resumed"] = target
            return _ok(rid, _attach_resume_completion_assessment(payload))
        try:
            init_home_token = (
                set_hairball_home_override(str(profile_home))
                if profile_home is not None
                else None
            )
            try:
                ...
                if init_home_token is not None:
                    reset_hairball_home_override(init_home_token)
            if sid in _sessions:
                ...
                # Remember the profile home so each turn re-binds HAIRBALL_HOME (the
...
def _(rid, params: dict) -> dict:
    ...
    # ``hairball --tui`` session legitimately rides the real stdio transport and
    # must stay visible.
...
def _(rid, params: dict) -> dict:
    ...
    transfer is performed by the separate ``hairball gateway`` process, whose
...
def _(rid, params: dict) -> dict:
    ...
    # Account credits block — agent-independent (a portal fetch), so it shows even
    # with zero API calls or on a resumed session. The TUI /usage panel renders
    # these lines regardless of `calls`. Fail-open: [] when not logged into managed
    # or on any portal hiccup.
    try:
        from agent.account_usage import managed_credits_lines

        credits = managed_credits_lines()
        if credits:
...
def _pet_state_rows(spritesheet) -> list[str]:
    ...
    Hairball has to support both the legacy 8-row petdex atlas and the current
    reference/petdex 9-row atlas. The desktop canvas gets this list and indexes it
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from hairball_cli.pets import _set_active
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from agent.pet import store
        from hairball_cli.pets import _clear_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if new_slug != slug:
            try:
                from hairball_cli.pets import _rename_active_if
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hairball_cli.pets import _set_enabled
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from hairball_cli.pets import set_pet_scale
...
def _pet_gen_root():
    ...
    root = get_hairball_home() / "cache" / "pet-gen"
...
try:
    _PET_REFERENCE_MAX_BYTES = max(
        1,
        int(os.environ.get("HAIRBALL_PET_REFERENCE_MAX_BYTES") or str(16 * 1024 * 1024)),
    )
except (TypeError, ValueError):
...
def _(rid, params: dict) -> dict:
    ...
    True only when a reference-capable image backend (Managed Portal / OpenRouter /
...
@method("credits.view")
def _(rid, params: dict) -> dict:
    """Structured managed credit view for the TUI /credits command.

    Account-independent (a portal fetch gated on "a managed account is logged in"),
...
# (agent/billing_view.py + hairball_cli/managed_billing.py) — same as /credits.
...
def _(rid, params: dict) -> dict:
    ...
    """
    from hairball_cli.managed_billing import BillingError, post_charge
...
def _(rid, params: dict) -> dict:
    ...
    """
    from hairball_cli.managed_billing import BillingError, get_charge_status
...
def _(rid, params: dict) -> dict:
    ...
    """
    from hairball_cli.managed_billing import BillingError, patch_auto_top_up
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        granted = step_up_managed_billing_scope(
...
def _(rid, params: dict) -> dict:
    ...
    agent = session["agent"]
    # Mirror the classic CLI /save: snapshot under the Hairball profile home
    # (~/.hairball/sessions/saved/) rather than the project/workspace CWD, and
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        title = branch.get("title") or branch_name or "branch"
    except Exception as e:
        if lease is not None:
            lease.release()
    ...
    try:
        ...
        try:
            ...
            logger.exception(
                "Failed to roll back orphan TUI branch session %s", new_key
            )
...
def _(rid, params: dict) -> dict:
    ...
    with session["history_lock"]:
        ...
        bound_plan_context = session.get("active_plan_context")
        if not isinstance(bound_plan_context, dict):
            bound_plan_context = session.get("next_plan_context")
        if not isinstance(bound_plan_context, dict):
            bound_plan_context = {}
    ...
    if should_interrupt and hasattr(session["agent"], "interrupt"):
        ...
        execution_id = str(
            bound_plan_context.get("plan_execution_id")
            or getattr(session["agent"], "_plan_execution_id", "")
            or ""
        ).strip()
        plan_session_id = str(
            bound_plan_context.get("session_id")
            or getattr(session["agent"], "session_id", "")
            or session.get("session_key")
            or ""
        ).strip()
        if execution_id and plan_session_id:
            try:
                from agent.plan_commands import cancel_active_plan_turn

                projection = cancel_active_plan_turn(
                    plan_session_id,
                    execution_id,
                    surface="tui",
                    actor="user",
                    reason="user_interrupted",
                    operation_id=f"tui-plan-interrupt:{execution_id}",
                    db=_get_db(),
                )
                session["plan_projection"] = projection
                _emit("plan.state", params.get("session_id", ""), projection or {})
            except Exception as exc:
                logger.warning("Could not cancel active TUI Plan execution: %s", exc)
...
#
# Layout:  $HAIRBALL_HOME/spawn-trees/<session_id>/<timestamp>.json
...
def _spawn_trees_root():
    ...
    root = get_hairball_home() / "spawn-trees"
    root.mkdir(parents=True, exist_ok=True)
    return root
...
def _park_tui_next_step(session: dict, text: str) -> int:
    """Park a next-step note. Does not start a turn."""
    ...
    parked = [
        str(item).strip()
        for item in (session.get("next_step") or [])
        if str(item).strip()
    ]
    parked.append(cleaned)
    session["next_step"] = parked
    return len(parked)


def _take_tui_next_step(session: dict) -> list[str]:
    """Claim parked next-step notes. Leaves queued next-turn intact."""
    parked = [
        str(item).strip()
        for item in list(session.get("next_step") or [])
        if str(item).strip()
    ]
    session["next_step"] = []
    return parked
...
def _(rid, params: dict) -> dict:
    """Park a next-step note without interrupting or starting a turn."""
    text = (params.get("text") or "").strip()
    if not text:
        return _err(rid, 4002, "text is required")
    session, err = _sess_nowait(params, rid)
    if err:
        return err
    n = _park_tui_next_step(session, text)
    if not n:
        return _ok(rid, {"status": "rejected", "woke": False, "text": text})
    return _ok(rid, {"status": "parked", "woke": False, "text": text, "queue_depth": n})
...
def _(rid, params: dict) -> dict:
    ...
    # A bare configured stop phrase is a transport control only while voice
    # chat is active. Consume it before session lookup so it can always stop
    # the shared microphone/speaker, even if the visible tab just closed.
    if _voice_mode_enabled() and isinstance(text, str):
        try:
            from tools.voice_mode import is_voice_stop_phrase

            typed_voice_stop = is_voice_stop_phrase(text)
        except Exception:
            typed_voice_stop = False
        if typed_voice_stop:
            os.environ["HAIRBALL_VOICE"] = "0"
            os.environ["HAIRBALL_VOICE_TTS"] = "0"
            _tts_stream_stop(user_barge=False)
            try:
                from hairball_cli.voice import stop_continuous
            ...
            _voice_emit(
                "voice.transcript",
                {"stop_phrase": True, "typed": True, "text": text},
            )
...
def _notification_poller_loop(
    ...
    while not stop_event.is_set() and not session.get("_finalized"):
        ...
        if should_suppress_inject(evt):
            acknowledge_suppressed_inject(evt, "tui-poller-skip")
            continue
...
def _run_prompt_submit(rid, sid: str, session: dict, text: Any) -> None:
    ...
    def run():
        approval_token = None
        session_tokens = []
        home_token = None  # per-turn HAIRBALL_HOME override for a resumed remote profile
        ...
        try:
            ...
            if _profile_home_str:
                home_token = set_hairball_home_override(_profile_home_str)
            ...
            if images:
                try:
                    ...
                    from hairball_cli.config import load_config as _tui_load_config
            ...
            plan_handoff_cancelled = False
            with session["history_lock"]:
                plan_context = session.pop("next_plan_context", None)
                if isinstance(plan_context, dict):
                    plan_context.setdefault(
                        "session_id", str(session.get("session_key") or sid)
                    )
                    session["active_plan_context"] = plan_context
            if isinstance(plan_context, dict):
                run_kwargs["agent_mode"] = str(plan_context.get("agent_mode") or "")
                run_kwargs["system_message"] = str(plan_context.get("system_message") or "")
                plan_execution_id = str(
                    plan_context.get("plan_execution_id") or ""
                ).strip()
                if plan_execution_id:
                    run_kwargs["plan_execution_id"] = plan_execution_id
                if str(plan_context.get("kind") or "") == "execute":
                    from agent.plan_handoff import prepare_plan_handoff

                    handoff = prepare_plan_handoff(
                        run_kwargs["conversation_history"],
                        context_mode=plan_context.get("context_mode") or "fresh",
                        plan_file=str(
                            (plan_context.get("plan") or {}).get("plan_file")
                            if isinstance(plan_context.get("plan"), dict)
                            else plan_context.get("plan_file") or ""
                        ),
                        task_id=str(session.get("session_key") or sid),
                        agent=agent,
                    )
                    run_kwargs["conversation_history"] = handoff.history
                    if handoff.warning:
                        _emit(
                            "status.update",
                            sid,
                            {
                                "kind": "plan_handoff",
                                "text": handoff.warning,
                                "context_mode": handoff.context_mode,
                                "outcome": handoff.compaction_outcome,
                            },
                        )
                    if not handoff.dispatch:
                        plan_handoff_cancelled = True

            def _finalize_plan_result(
                plan_result: dict[str, Any], response: str
            ) -> None:
                ...
                try:
                    ...
                    projection = finalize_plan_turn(
                        ...
                        actor="agent",
                        operation_id=str(
                            plan_context.get("operation_id")
                            or plan_context.get("plan_execution_id")
                            or f"tui-plan-{int(time.time() * 1000)}"
                        ),
                        db=_get_db(),
                    )
                    session["plan_projection"] = projection
                    _emit("plan.state", sid, projection or {})
                except Exception as exc:
                    _emit(
                        "status.update",
                        sid,
                        {"kind": "plan", "text": f"Plan state error: {exc}"},
                    )

            if plan_handoff_cancelled:
                result = {
                    "completed": False,
                    "interrupted": True,
                ...
                _emit(
                    ...
                    sid,
                    {
                        "text": "",
                        "cancelled": True,
                        "reason": "plan_compaction_cancelled",
                    },
                )
                return
            ...
            try:
                ...
                pass
            for parked in _take_tui_next_step(session):
                try:
                    if callable(getattr(agent, "inject", None)):
                        agent.inject(parked)
                except Exception:
                    pass
            try:
            ...
            if isinstance(result, dict):
                ...
                candidate_assessment = result.get("completion_assessment")
                if isinstance(candidate_assessment, dict):
                    completion_assessment = candidate_assessment
            else:
                raw = str(result)
                status = "complete"
            ...
            if completion_assessment is not None:
                # A structured side-channel for Ink/dashboard consumers. Never
                # append it to ``text``: it includes task facts and must not
                # mutate the assistant's own response or session transcript.
            ...
            leftover = ""
            if isinstance(result, dict):
                leftover = str(result.get("pending_steer") or "").strip()
            if leftover:
                _park_tui_next_step(session, leftover)
            ...
            _tts_stream_finish(tts_queue)

            # Whole-reply fallback only when the sentence pipeline could not
            # start. A live stream already received every final-answer delta.
            if (
                status == "complete"
                ...
                try:
                    threading.Thread(
                        target=lambda: _speak_text_with_barge(raw),
                        daemon=True,
                    ).start()
                except ImportError:
                    logger.warning("voice TTS skipped: hairball_cli.voice unavailable")
                except Exception as e:
                    logger.warning("voice TTS dispatch failed: %s", e)
        except Exception as e:
            current_tts_queue = locals().get("tts_queue")
            if current_tts_queue is not None:
                _tts_stream_stop(
                    user_barge=False,
                    expected_queue=current_tts_queue,
                )
            import traceback
            ...
            active_plan_context = locals().get("plan_context")
            with session["history_lock"]:
                if (
                    isinstance(active_plan_context, dict)
                    and session.get("active_plan_context") is active_plan_context
                ):
                    session.pop("active_plan_context", None)
            if locals().get("thinking_started"):
                try:
        ...
        try:
            ...
            for _evt, synth in process_registry.drain_notifications(
                ...
                if should_suppress_inject(_evt):
                    acknowledge_suppressed_inject(_evt, "tui-post-turn-skip")
                    continue
...
def _(rid, params: dict) -> dict:
    ...
    if err:
        return err
    try:
        from hairball_cli.clipboard import has_clipboard_image, save_clipboard_image
    except Exception as e:
        return _err(rid, 5027, f"clipboard unavailable: {e}")

    session["image_counter"] = session.get("image_counter", 0) + 1
    img_dir = _hairball_home / "images"
...
def _desktop_attachment_dir(session: dict) -> Path:
    root = Path(_session_cwd(session)).resolve() / ".hairball" / "desktop-attachments"
    root.mkdir(parents=True, exist_ok=True)
    return root
    ...
    *,
    ...
    Three cases:
      ...
      2. The path resolves to a gateway-visible file OUTSIDE the workspace — copy
         it into ``.hairball/desktop-attachments/`` so the ``@file:`` ref resolves.
      3. The path doesn't exist on the gateway (the common remote case: it's a
         ...
         write them into ``.hairball/desktop-attachments/``.
...
def _(rid, params: dict) -> dict:
    ...
    prompt = "\n".join(
        ...
        for line in [
            ...
            "Restart exactly the app intended for the Preview URL, not Hairball Desktop itself.",
            ...
            "First inspect what process, if any, owns the Preview URL port. If a stale server exists, inspect its cwd and prefer that cwd over the Hairball/Desktop process cwd.",
            ...
            "Before declaring success, verify the Preview URL responds with the intended app, not Hairball Desktop. If it serves Hairball/Desktop UI or another unrelated app, stop that process and report failure.",
...
def _(rid, params: dict) -> dict:
    ...
    if key == "model":
        try:
            ...
            if session:
                ...
                from hairball_cli.model_switch import parse_model_flags
    ...
    if key == "fast":
        ...
        overrides = None
        if nv == "fast":
            from hairball_cli.models import resolve_fast_mode_overrides
    ...
    if key == "yolo":
        ...
        try:
            ...
            if session:
                ...
                current = is_truthy_value(os.environ.get("HAIRBALL_YOLO_MODE"))
                enable = _resolve_toggle(current)
                if enable:
                    os.environ["HAIRBALL_YOLO_MODE"] = "1"
                    nv = "1"
                else:
                    os.environ.pop("HAIRBALL_YOLO_MODE", None)
                    nv = "0"
...
def _projects_payload(conn) -> dict:
    from hairball_cli import projects_db as pdb
...
def _is_repo_junk(root: str) -> bool:
    ...
    anything under HAIRBALL_HOME (~/.hairball by default) — config/sessions/skills,
    ...
    hairball_home = os.path.realpath(str(get_hairball_home()))

    return real == home or real == hairball_home or real.startswith(hairball_home + os.sep)
...
def _is_session_cwd_junk(cwd: str) -> bool:
    ...
    HAIRBALL_HOME may be an intentional prose/data workspace. The pre-Projects
    ...
    hairball_home = os.path.normcase(os.path.realpath(str(get_hairball_home())))
    return real == home or real == hairball_home
...
def _discover_repos_payload(db, *, conn=None, backfill: bool = True) -> list[dict]:
    ...
    repos even with zero hairball sessions. Session-derived roots cover repos
    outside the scan roots. Both are junk-filtered (hairball home subtree + bare
...
def _(rid, params: dict) -> dict:
    ...
    if key == "provider":
        try:
            from hairball_cli.models import list_available_providers, normalize_provider
    ...
    if key == "profile":
        ...
        return _ok(rid, {"home": str(_hairball_home), "display": display_hairball_home()})
    if key == "project":
    ...
    if key == "mtime":
        cfg_path = _hairball_home / "config.yaml"
        try:
            return _ok(
...
def _(rid, params: dict) -> dict:
    try:
        from hairball_cli.main import _has_any_provider_configured
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hairball_cli.runtime_provider import resolve_runtime_provider
        from hairball_cli.auth import has_usable_secret
        from hairball_cli.main import _has_any_provider_configured
        ...
        if not provider_configured and provider == "bedrock" and source in {
            ...
            return _ok(
                rid,
                {
                    ...
                    "source": source,
                    "error": "No Hairball provider is configured.",
                },
            )
...
@method("reload.env")
def _(rid, params: dict) -> dict:
    """Re-read ``~/.hairball/.env`` into the gateway process via
    ``hairball_cli.config.reload_env``, matching classic CLI's ``/reload``
    ...
    """
    try:
        from hairball_cli.config import reload_env
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hairball_cli.commands import (
            COMMAND_REGISTRY,
            SUBCOMMANDS,
            _build_description,
        )
...
def _cli_exec_blocked(argv: list[str]) -> str | None:
    ...
    if not argv:
        return "bare `hairball` is interactive — use `/hairball chat -q …` or run `hairball` in another terminal"
    a0 = argv[0].lower()
    if a0 == "setup":
        return "`hairball setup` needs a full terminal — run it outside the TUI"
    if a0 == "gateway":
        return "`hairball gateway` is long-running — run it in another terminal"
    if a0 == "sessions" and len(argv) > 1 and argv[1].lower() == "browse":
        return "`hairball sessions browse` is interactive — use /resume here, or run browse in another terminal"
    if a0 == "config" and len(argv) > 1 and argv[1].lower() == "edit":
        return "`hairball config edit` needs $EDITOR in a real terminal"
    return None


@method("cli.exec")
def _(rid, params: dict) -> dict:
    """Run `python -m hairball_cli.main` with argv; capture stdout/stderr (non-interactive only)."""
    ...
    try:
        r = subprocess.run(
            [sys.executable, "-m", "hairball_cli.main", *argv],
            capture_output=True,
            text=True,
            timeout=min(int(params.get("timeout", 240)), 600),
            cwd=os.getcwd(),
            # cli.exec runs `python -m hairball_cli.main` (can drive the agent) →
...
def _(rid, params: dict) -> dict:
    ...
    if name == "moa":
        ...
        try:
            from hairball_cli.moa_config import moa_usage, normalize_moa_config
    ...
    if name == "inject":
        if not arg:
            return _err(rid, 4004, "usage: /inject <prompt>")
        if not session:
            return _err(rid, 4001, "no active session")
        n = _park_tui_next_step(session, arg)
        if not n:
            return _err(rid, 4002, "inject rejected")
        preview = arg[:80] + ("..." if len(arg) > 80 else "")
        return _ok(
            rid,
            {
                "type": "exec",
                "output": f"Next-step note parked (does not start a turn): {preview}",
            },
        )
    ...
    if name == "plan":
        ...
        if not session_key:
            return _err(rid, 4001, "no session key")
        try:
            from agent.plan_commands import handle_plan_command

            context = handle_plan_command(
                session_key,
                arg,
                surface="tui",
                actor="user",
            ...
            code = str(getattr(exc, "code", None) or "plan_command_failed")
            return _err(rid, 4018, f"{code}: {exc}")
        if context.get("kind") in {"prompt", "execute"}:
            context["session_id"] = session_key
            context["operation_id"] = str(
                context.get("plan_execution_id")
                or f"tui-plan:{session_key}:{rid}"
            )
            with session["history_lock"]:
                session["next_plan_context"] = context
            return _ok(
                rid,
                {
                    "type": "send",
                    ...
                    "notice": (
                        "Plan mode: read-only research until exact approval."
                        if context.get("kind") == "prompt"
                        else "Executing the exact approved Plan revision."
                    ),
                },
            )
        return _ok(
            rid,
            {"type": "exec", "output": str(context.get("message") or "")},
        )
...
def _(rid, params: dict) -> dict:
    ...
    try:
        from hairball_cli.commands import SlashCommandCompleter
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        from hairball_cli.config import is_managed, save_env_value
        from hairball_cli.inventory import build_models_payload, load_picker_context
        ...
        if pconfig.auth_type != "api_key":
            return _err(
                ...
                f"run `hairball model` to configure",
            )
        if not pconfig.api_key_env_vars:
            return _err(rid, 4004, f"no env var defined for {pconfig.name}")

        # Save the key to ~/.hairball/.env
...
def _(rid, params: dict) -> dict:
    ...
    """
    try:
        from hairball_cli.auth import PROVIDER_REGISTRY, clear_provider_auth
        from hairball_cli.config import remove_env_value
...
def _voice_mode_enabled() -> bool:
    ...
    """
    return os.environ.get("HAIRBALL_VOICE", "").strip() == "1"


def _voice_tts_enabled() -> bool:
    """Whether agent replies should be spoken back via TTS (runtime only)."""
    return os.environ.get("HAIRBALL_VOICE_TTS", "").strip() == "1"


def _any_session_running() -> bool:
    """Return True while any TUI session has an agent turn in flight."""
    try:
        with _sessions_lock:
...
# One speaker and microphone are shared process-wide. Token deltas feed the
# same sentence pipeline used by the classic CLI; a new turn cuts an unfinished
# prior pipeline before taking ownership.
...
def _tts_stream_begin() -> Optional[queue.Queue]:
    ...
    # Keep the thread target self-contained. Lightweight gateway hosts and
    # deterministic tests replace ``Thread`` with a callable-only runner.
    def run_stream() -> None:
...
def _tts_stream_finish(text_queue: Optional[queue.Queue]) -> None:
    """Drain a normal stream without turning completion into cancellation."""
    if text_queue is None:
        return
    with _tts_stream_lock:
        state = _tts_stream_state
    if state is None or state.get("queue") is not text_queue:
        return
    text_queue.put(None)
    ...
    def clear_when_done() -> None:
        ...
        global _tts_stream_state
        with _tts_stream_lock:
            if _tts_stream_state is state:
                _tts_stream_state = None

    threading.Thread(target=clear_when_done, daemon=True).start()


def _tts_stream_stop(
    user_barge: bool = True,
    expected_queue: Optional[queue.Queue] = None,
) -> None:
    global _tts_stream_state
    with _tts_stream_lock:
        if (
            expected_queue is not None
            and _tts_stream_state is not None
            and _tts_stream_state.get("queue") is not expected_queue
        ):
            return
...
def _full_duplex_listener() -> None:
    ...
    try:
        ...
        def cut_all_tts() -> None:
            _tts_stream_stop(user_barge=True)
            with _fd_listener_lock:
                pipelines = list(_fd_speak_pipelines)
            for stop_event, _done_event in pipelines:
                stop_event.set()
            stop_playback()
        ...
        logger.debug("Full-duplex listener failed: %s", exc)
    finally:
        with _fd_listener_lock:
            _fd_listener_active = False
...
def _speak_text_with_barge(text: str) -> None:
    ...
    done_event = threading.Event()
    with _fd_listener_lock:
        _fd_speak_pipelines.add((stop_event, done_event))

    def speak() -> None:
        try:
            speak_text(text, stop_event)
        finally:
            done_event.set()
            with _fd_listener_lock:
                _fd_speak_pipelines.discard((stop_event, done_event))
...
def _(rid, params: dict) -> dict:
    ...
    * ``tts`` — toggle speech-output of agent replies. Requires mode on
    ...
    if action in {"on", "off"}:
        ...
        os.environ["HAIRBALL_VOICE"] = "1" if enabled else "0"
    ...
    if action == "tts":
        ...
        os.environ["HAIRBALL_VOICE_TTS"] = "1" if new_value else "0"
        if not new_value:
            _tts_stream_stop(user_barge=False)
...
def _(rid, params: dict) -> dict:
    ...
    try:
        if action == "start":
            ...
            import hairball_cli.voice as voice_runtime

            start_continuous = voice_runtime.start_continuous
            set_busy_probe = getattr(voice_runtime, "set_voice_busy_probe", None)
            if callable(set_busy_probe):
                set_busy_probe(_any_session_running)
            ...
            max_recording = voice_cfg.get("max_recording_seconds")
            safe_max_recording = (
                (max_recording if max_recording > 0 else 0.0)
                if isinstance(max_recording, (int, float))
                and not isinstance(max_recording, bool)
                else 120.0
            )

            def on_stop_phrase(transcript: str) -> None:
                os.environ["HAIRBALL_VOICE"] = "0"
                os.environ["HAIRBALL_VOICE_TTS"] = "0"
                _tts_stream_stop(user_barge=False)
                _voice_emit(
                    "voice.transcript",
                    {"stop_phrase": True, "text": transcript},
                )
            ...
            started = start_continuous(
                ...
                auto_restart=False,
                max_recording_seconds=safe_max_recording,
...
def _(rid, params: dict) -> dict:
    ...
    try:
        import hairball_cli.voice  # noqa: F401

        threading.Thread(
            target=lambda: _speak_text_with_barge(text),
            daemon=True,
        ).start()
...
def _failure_messages(url: str, port: int, system: str) -> list[str]:
    from hairball_cli.browser_connect import manual_chrome_debug_command
...
def _browser_connect(rid, params: dict) -> dict:
    ...
    from hairball_cli.browser_connect import DEFAULT_BROWSER_CDP_URL
    ...
    try:
        ...
        if parsed.scheme in {"ws", "wss"} and parsed.path.startswith(
            ...
            if not ok and _is_default_local_cdp(parsed):
                from hairball_cli.browser_connect import launch_chrome_debug
...
def _(rid, params: dict) -> dict:
    try:
        cfg = _load_cfg()
        model = _resolve_model()
        api_key = os.environ.get("HAIRBALL_API_KEY", "") or cfg.get("api_key", "")
        masked = f"****{api_key[-4:]}" if len(api_key) > 4 else "(not set)"
        base_url = os.environ.get("HAIRBALL_BASE_URL", "") or cfg.get("base_url", "")
        ...
        sections = [
            ...
            },
            {
                ...
                "rows": [
                    ...
                    ["Config File", str(_hairball_home / "config.yaml")],
                ],
            },
        ]
...
def _(rid, params: dict) -> dict:
    ...
    gateway. Shares its renderer with the ``hairball journey`` CLI.
    """
    try:
...
def _(rid, params: dict) -> dict:
    ...
    try:
        ...
        if action == "install":
            from hairball_cli.skills_hub import do_install
        ...
        if action == "browse":
            from hairball_cli.skills_hub import browse_skills
        ...
        if action == "inspect":
            from hairball_cli.skills_hub import inspect_skill
...
def _(rid, params: dict) -> dict:
    ...
    primitives as ``hairball plugins`` / the dashboard, so the three surfaces
