...
def test_matrix_extra_not_in_all():
    ...
    assert "matrix" in optional_dependencies, "[matrix] extra must still exist for explicit `pip install hairball-agent[matrix]`"
...
def test_lazy_installable_extras_excluded_from_all():
    ...
    for extra in lazy_covered_extras:
        offending = [
            ...
            if f"hairball-agent[{extra}]" in spec
        ]
...
def test_pyproject_aiohttp_pins_match_lazy_slack_pin():
    ...
    If the two drift, `hairball update` resolves the pyproject pin and
    ...
    assert not mismatches, (
        ...
        "LAZY_DEPS['platform.slack'] to avoid hairball update downgrading "
...
def test_pyproject_pins_match_lazy_deps_pins():
    ...
    places. When they drift, `hairball update` resolves the pyproject extra
    ...
    assert not drift, (
        ...
        "for every shared package — otherwise `hairball update` downgrades the "
...
def test_dingtalk_extra_includes_qrcode_for_qr_auth():
    """DingTalk's QR-code device-flow auth (hairball_cli/dingtalk_auth.py)
...
def test_nemo_relay_extra_uses_supported_official_distribution_range():
    ...
    assert optional_dependencies["nemo-relay"] == ["nemo-relay>=0.5,<0.7"]
    assert not any(
        spec == "hairball-agent[nemo-relay]"
        for spec in optional_dependencies["all"]
    )
