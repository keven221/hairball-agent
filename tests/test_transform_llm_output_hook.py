...
import hairball_cli.plugins as plugins_mod
from hairball_cli.plugins import PluginManager, VALID_HOOKS


def _make_enabled_plugin(hairball_home: Path, name: str, register_body: str) -> Path:
    """Create a plugin under <hairball_home>/plugins/<name> and opt it in."""
    plugin_dir = hairball_home / "plugins" / name
    ...
    cfg_path = hairball_home / "config.yaml"
    cfg = {}
    if cfg_path.exists():
...
def test_hook_receives_expected_kwargs(tmp_path, monkeypatch):
    ...
    hairball_home = tmp_path / "hairball_test"
    hairball_home.mkdir(exist_ok=True)
    _make_enabled_plugin(
        hairball_home, "capture_hook",
        register_body=(
...
def test_no_plugins_returns_empty_results(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("HAIRBALL_HOME", str(tmp_path / "hairball_empty"))
