...
class TestBatchWorkerResumeBehavior:
    def test_worker_keeps_completion_assessment_without_changing_completed(self, tmp_path, monkeypatch):
        assessment = {
            "contract_id": "batch-contract",
            "evidence_sequence": 1,
        ...
        written = json.loads((tmp_path / "batch_1.jsonl").read_text().strip())
        assert written["completion_assessment"] == assessment
