...
enforce uniqueness (DeepSeek, Xiaomi, Moonshot/Kimi) then reject the
API call with HTTP 400.

These tests pin:
- the cache-hit path returns a fully independent schema tree
- the first uncached call also returns an independent tree
- list and nested-dict mutations never reach the cached value
"""
...
class TestQuietModeCacheIsolation:
    ...
    def test_nested_schema_mutation_does_not_poison_cache(self):
        """Provider compatibility recovery mutates nested schemas in place.

        That mutation belongs to one agent only and must not leak through the
        process-wide quiet-mode cache into the next Gateway session.
        """
        first = model_tools.get_tool_definitions(quiet_mode=True)
        candidate = next(
            tool for tool in first
            if isinstance((tool.get("function") or {}).get("parameters"), dict)
        )
        function = candidate["function"]
        original_description = function.get("description", "")
        function["description"] = "session-local mutation"
        function["parameters"]["cache_poison"] = True

        second = model_tools.get_tool_definitions(quiet_mode=True)
        same = next(
            tool for tool in second
            if tool["function"]["name"] == function["name"]
        )["function"]
        assert same.get("description", "") == original_description
        assert "cache_poison" not in same["parameters"]
