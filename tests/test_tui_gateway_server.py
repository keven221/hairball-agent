...
from hairball_constants import reset_hairball_home_override, set_hairball_home_override
from hairball_cli.active_sessions import active_session_registry_snapshot
from hairball_cli.browser_connect import ChromeDebugLaunch
from tui_gateway import server


def test_session_create_rejects_at_active_session_limit(monkeypatch, tmp_path):
    home = tmp_path / ".hairball"
    home.mkdir()
    ...
    try:
        ...
        assert second["error"]["message"] == (
            "Hairball is at the active session limit (1/1). "
...
def test_voice_toggle_returns_configured_record_key(monkeypatch):
    ...
    # ``voice.toggle`` action=on mutates ``os.environ["HAIRBALL_VOICE"]``
    ...
    # review on #19835).
    monkeypatch.setenv("HAIRBALL_VOICE", "0")
...
def test_voice_record_start_handles_non_dict_voice_cfg(monkeypatch):
    ...
    monkeypatch.setenv("HAIRBALL_VOICE", "1")
...
def test_voice_toggle_tts_branch_also_carries_record_key(monkeypatch):
    ...
    monkeypatch.setenv("HAIRBALL_VOICE", "1")
    # Track the runtime key before the handler mutates os.environ directly;
    # teardown must not leak TTS=1 into unrelated gateway tests.
    monkeypatch.setenv("HAIRBALL_VOICE_TTS", "0")
...
def test_load_enabled_toolsets_prefers_tui_env(monkeypatch):
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "web, terminal, ,memory")
...
def test_load_enabled_toolsets_filters_invalid_tui_env(monkeypatch, capsys):
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "web, nope")
    monkeypatch.setitem(
        sys.modules,
        "hairball_cli.plugins",
...
def test_load_enabled_toolsets_accepts_plugin_env_after_discovery(monkeypatch):
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "plugin_demo")
...
def test_load_enabled_toolsets_folds_project_into_focus_posture(monkeypatch):
    ...
    monkeypatch.delenv("HAIRBALL_TUI_TOOLSETS", raising=False)
...
def test_tui_focus_is_visibility_only_but_explicit_pin_is_authorization(monkeypatch):
    import agent.coding_context as cc

    monkeypatch.delenv("HAIRBALL_TUI_TOOLSETS", raising=False)
    monkeypatch.setattr(cc, "coding_selection", lambda **_: ["coding", "figma"])
    enabled, visible = server._load_tool_scopes()
    assert enabled is None
    assert visible == ["coding", "figma", "project"]

    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "file")
    enabled, visible = server._load_tool_scopes()
    assert enabled == ["file"]
...
def test_load_enabled_toolsets_rejects_disabled_mcp_env(monkeypatch, capsys):
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "mcp-off")
    monkeypatch.setitem(
        sys.modules,
        "hairball_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    import hairball_cli.config as config_mod
    ...
    # _get_platform_tools (a non-configurable platform toolset in hairball-cli's
...
def test_load_enabled_toolsets_falls_back_when_tui_env_invalid(monkeypatch, capsys):
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "nope")
    monkeypatch.setitem(
        sys.modules,
        "hairball_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
...
def test_load_enabled_toolsets_honors_builtin_env_if_config_fails(monkeypatch):
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "web")
...
def test_load_enabled_toolsets_all_env_means_all(monkeypatch):
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "all")
...
def test_load_enabled_toolsets_all_env_warns_about_ignored_extra_entries(
    ...
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "all,nope")
...
def test_load_enabled_toolsets_reports_disabled_mcp_separately(monkeypatch, capsys):
    monkeypatch.setenv("HAIRBALL_TUI_TOOLSETS", "web,mcp-off,nope")
    monkeypatch.setitem(
        sys.modules,
        "hairball_cli.plugins",
        types.SimpleNamespace(discover_plugins=lambda: None),
    ...
    assert server._load_enabled_toolsets() == ["web"]
    err = capsys.readouterr().err
    assert "ignoring unknown HAIRBALL_TUI_TOOLSETS entries: nope" in err
    assert "ignoring disabled MCP servers" in err
    assert "mcp-off" in err
...
def test_session_resume_follows_compression_tip(monkeypatch, tmp_path):
    ...
    db.upsert_completion_contract(
        {
            "contract_id": "parent-contract",
            "session_id": "parent_root",
            "turn_id": "parent-turn",
            "digest": "d" * 64,
            "parser_version": 1,
            "clauses": [{"clause_id": "one", "status": "pending"}],
        }
    )
    ...
    db.save_completion_assessment(
        "parent-contract", 0, "terminal", completion_assessment
    )
    conn = db._conn
    ...
    assert resp["result"]["completion_assessment"] == completion_assessment
...
def test_session_resume_profile_uses_profile_db_cwd(monkeypatch, tmp_path):
    ...
    monkeypatch.setattr("hairball_state.SessionDB", lambda db_path=None: profile_db)
...
def test_resolve_model_uses_inference_model_env(monkeypatch):
    monkeypatch.delenv("HAIRBALL_MODEL", raising=False)
    monkeypatch.setenv("HAIRBALL_INFERENCE_MODEL", " anthropic/claude-sonnet-4.6\n")
...
def test_resolve_model_strips_config_model(monkeypatch):
    monkeypatch.delenv("HAIRBALL_MODEL", raising=False)
    monkeypatch.delenv("HAIRBALL_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(
        server, "_load_cfg", lambda: {"model": {"default": " managed/hairball-test "}}
    )

    assert server._resolve_model() == "managed/hairball-test"
...
def test_config_sync_switches_unpinned_session(monkeypatch):
    _patch_config_model(monkeypatch, "new/model", provider="managed")
    session = _sync_test_session(config_model_seen=("old/model", "managed"))
    calls = []
    monkeypatch.setattr(
        server,
        "_apply_model_switch",
    ...
    assert session["config_model_seen"] == ("new/model", "managed")
...
def test_config_sync_switches_when_only_provider_differs(monkeypatch):
    _patch_config_model(monkeypatch, "old/model", provider="managed")
    session = _sync_test_session(config_model_seen=("old/model", ""))
    calls = []
    monkeypatch.setattr(
        server,
    ...
    assert calls == ["old/model --provider managed"]
...
def test_config_sync_config_wins_over_env_seed(monkeypatch):
    # Hosted instances set HAIRBALL_INFERENCE_MODEL as a provision-time seed;
    # the per-turn sync must follow config.yaml edits, not stay pinned to it.
    monkeypatch.setenv("HAIRBALL_INFERENCE_MODEL", "seed/model")
    monkeypatch.delenv("HAIRBALL_MODEL", raising=False)
...
def test_config_sync_ignores_env_seed_without_config_model(monkeypatch):
    # `hairball --tui -m <model>` sets HAIRBALL_MODEL/HAIRBALL_INFERENCE_MODEL as a
    ...
    # permanently.
    monkeypatch.setenv("HAIRBALL_MODEL", "one-shot/model")
    monkeypatch.setenv("HAIRBALL_INFERENCE_MODEL", "one-shot/model")
    monkeypatch.setattr(
...
def test_config_model_target_never_reads_env(monkeypatch):
    monkeypatch.setenv("HAIRBALL_MODEL", "seed/model")
    monkeypatch.setenv("HAIRBALL_INFERENCE_MODEL", "seed/model")
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "managed"}})

    assert server._config_model_target() == ("", "managed")
...
def test_apply_model_switch_persist_override_false_never_persists(monkeypatch):
    ...
    monkeypatch.setattr(
        "hairball_cli.model_switch.switch_model", lambda **kw: result
    )
    monkeypatch.setattr(
        "hairball_cli.model_switch.resolve_persist_behavior",
    ...
    monkeypatch.setattr(
        "hairball_cli.model_cost_guard.expensive_model_warning",
        lambda *a, **k: None,
    )
    session = {"agent": None}

    out = server._apply_model_switch(
        "sid", session, "new/model --provider managed", persist_override=False
    )
...
def test_startup_runtime_uses_tui_provider_env(monkeypatch):
    monkeypatch.setenv("HAIRBALL_MODEL", "managed/hairball-test")
    monkeypatch.setenv("HAIRBALL_TUI_PROVIDER", "managed")
    monkeypatch.delenv("HAIRBALL_INFERENCE_PROVIDER", raising=False)

    assert server._resolve_startup_runtime() == ("managed/hairball-test", "managed")


def test_startup_runtime_does_not_treat_inference_provider_as_explicit(monkeypatch):
    monkeypatch.setenv("HAIRBALL_MODEL", "managed/hairball-test")
    monkeypatch.delenv("HAIRBALL_TUI_PROVIDER", raising=False)
    monkeypatch.setenv("HAIRBALL_INFERENCE_PROVIDER", "managed")
    monkeypatch.setattr(
        "hairball_cli.models.detect_static_provider_for_model",
        lambda model, provider: None,
    )

    assert server._resolve_startup_runtime() == ("managed/hairball-test", None)


def test_startup_runtime_detects_provider_for_model_env(monkeypatch):
    monkeypatch.setenv("HAIRBALL_MODEL", "sonnet")
    monkeypatch.delenv("HAIRBALL_TUI_PROVIDER", raising=False)
    monkeypatch.delenv("HAIRBALL_INFERENCE_PROVIDER", raising=False)
    ...
    monkeypatch.setattr(
        "hairball_cli.models.detect_static_provider_for_model", fake_detect
    )
...
def test_load_fallback_model_merges_chain_providers_first(monkeypatch):
    # Parity with HairballCLI / gateway: fallback_providers stays first and keeps
...
def test_make_agent_passes_configured_fallback_chain(monkeypatch):
    ...
    monkeypatch.setattr(
        "hairball_cli.runtime_provider.resolve_runtime_provider",
...
def test_startup_runtime_resolves_short_alias_without_network(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "hairball_cli.models.fetch_openrouter_models",
...
def test_startup_runtime_does_not_call_network_detector(monkeypatch):
    ...
    monkeypatch.setattr(server, "_load_cfg", lambda: {"model": {"provider": "auto"}})
    monkeypatch.setattr(
        "hairball_cli.models.detect_provider_for_model",
        lambda *_args, **_kwargs: (_ for _ in ()).throw(
...
def test_config_set_yolo_global_scope_writes_approvals_mode(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(server, "_hairball_home", tmp_path)
...
def test_config_set_fast_updates_live_agent_and_config(monkeypatch):
    ...
    monkeypatch.setattr(
        "hairball_cli.models.resolve_fast_mode_overrides",
...
def test_config_set_yolo_process_scope_treats_false_like_env_as_disabled(monkeypatch):
    monkeypatch.setenv("HAIRBALL_YOLO_MODE", "false")
    ...
    assert resp["result"]["value"] == "1"
    assert os.environ.get("HAIRBALL_YOLO_MODE") == "1"
...
def test_enable_gateway_prompts_sets_gateway_env(monkeypatch):
    monkeypatch.delenv("HAIRBALL_EXEC_ASK", raising=False)
    monkeypatch.delenv("HAIRBALL_GATEWAY_SESSION", raising=False)
    monkeypatch.delenv("HAIRBALL_INTERACTIVE", raising=False)

    server._enable_gateway_prompts()

    assert server.os.environ["HAIRBALL_GATEWAY_SESSION"] == "1"
    assert server.os.environ["HAIRBALL_EXEC_ASK"] == "1"
    assert server.os.environ["HAIRBALL_INTERACTIVE"] == "1"


def test_setup_status_reports_provider_config(monkeypatch):
    monkeypatch.setattr("hairball_cli.main._has_any_provider_configured", lambda: False)
...
def test_setup_runtime_check_rejects_empty_runtime_key(monkeypatch):
    monkeypatch.setattr("hairball_cli.main._has_any_provider_configured", lambda: True)
    monkeypatch.setattr(
        "hairball_cli.runtime_provider.resolve_runtime_provider",
...
def test_setup_runtime_check_honors_requested_provider(monkeypatch):
    ...
    scoped = server.handle_request(
        {"id": "1", "method": "setup.runtime_check", "params": {"provider": "managed"}}
    )
    assert scoped["result"]["ok"] is True
    assert scoped["result"]["provider"] == "managed"
...
def test_config_set_model_requires_confirmation_for_expensive_model(monkeypatch):
    ...
    server._sessions["sid"] = _session(agent=agent)
    monkeypatch.setattr(
        "hairball_cli.model_switch.switch_model", lambda **_kwargs: result
    )
...
def test_config_set_model_global_persists(monkeypatch):
    ...
    monkeypatch.setattr("hairball_cli.model_switch.switch_model", _switch_model)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
...
def test_config_set_model_explicit_provider_skips_broken_default_init(monkeypatch):
    ...
    monkeypatch.setattr("hairball_cli.runtime_provider.resolve_runtime_provider", fake_runtime_provider)
...
def test_config_set_model_does_not_leak_inference_provider_env(monkeypatch):
    ...
    process; writing HAIRBALL_INFERENCE_PROVIDER on a switch leaked the new
    ...
    server._sessions["sid"] = session
    monkeypatch.setenv("HAIRBALL_INFERENCE_PROVIDER", "openrouter")
    monkeypatch.setattr(
        "hairball_cli.model_switch.switch_model", lambda **_kwargs: result
    )
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    ...
    try:
        ...
        assert os.environ["HAIRBALL_INFERENCE_PROVIDER"] == "openrouter"
...
def test_config_set_model_records_per_session_override_not_env(monkeypatch):
    ...
    try:
        ...
        assert "HAIRBALL_TUI_PROVIDER" not in os.environ
        assert "HAIRBALL_INFERENCE_PROVIDER" not in os.environ
...
def test_config_set_model_switches_agent_without_touching_env(monkeypatch):
    ...
    a per-session override; it does NOT write HAIRBALL_MODEL / HAIRBALL_TUI_PROVIDER
    ...
    session = _session(agent=agent)
    server._sessions["sid"] = session
    monkeypatch.setenv("HAIRBALL_TUI_PROVIDER", "openai-codex")
    monkeypatch.delenv("HAIRBALL_MODEL", raising=False)
    monkeypatch.delenv("HAIRBALL_INFERENCE_MODEL", raising=False)
    monkeypatch.setattr(server, "_restart_slash_worker", lambda sid, session: None)
    monkeypatch.setattr(server, "_emit", lambda *args, **kwargs: None)
    ...
    monkeypatch.setattr("hairball_cli.model_switch.switch_model", fake_switch_model)
    ...
    try:
        ...
        assert os.environ["HAIRBALL_TUI_PROVIDER"] == "openai-codex"
        assert "HAIRBALL_MODEL" not in os.environ
        assert "HAIRBALL_INFERENCE_MODEL" not in os.environ
    finally:
        server._sessions.clear()
...
def test_session_compress_syncs_session_key_after_rotation(monkeypatch):
    ...
    continuation session — mirrors HairballCLI._manual_compress's
...
def test_file_attach_uploads_remote_file_into_session_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".hairball" / "desktop-attachments" / "report.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["path"] == str(stored)
        assert resp["result"]["ref_text"] == "@file:.hairball/desktop-attachments/report.txt"
...
def test_file_attach_copies_gateway_visible_file_outside_workspace(monkeypatch, tmp_path):
    ...
    try:
        ...
        stored = workspace / ".hairball" / "desktop-attachments" / "outside.txt"
        assert resp["result"]["attached"] is True
        assert resp["result"]["uploaded"] is True
        assert resp["result"]["ref_text"] == "@file:.hairball/desktop-attachments/outside.txt"
...
def test_file_attach_uses_in_workspace_file_without_copying(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert not (workspace / ".hairball" / "desktop-attachments").exists()
    finally:
        server._sessions.pop("sid", None)
...
def test_file_attach_quotes_ref_with_spaces(monkeypatch, tmp_path):
    ...
    try:
        ...
        assert resp["result"]["attached"] is True
        assert resp["result"]["ref_text"] == "@file:`.hairball/desktop-attachments/my exam schedule.csv`"
    finally:
        server._sessions.pop("sid", None)
...
def test_session_status_reads_live_gateway_agent(monkeypatch):
    ...
    assert "Hairball TUI Status" in out
...
def test_plugins_list_surfaces_loader_error(monkeypatch):
    with patch("hairball_cli.plugins.get_plugin_manager", side_effect=Exception("boom")):
        resp = server.handle_request(
...
def test_complete_slash_surfaces_completer_error(monkeypatch):
    with patch(
        "hairball_cli.commands.SlashCommandCompleter",
...
def test_session_inject_parks_without_waking_or_interrupting():
    calls = {}

    class _Agent:
        def inject(self, text):
            calls["inject_text"] = text
            return True
        ...
        resp = server.handle_request(
            {
                ...
                "params": {"session_id": "sid", "text": "remember the five-sentence cap"},
            }
        )
    finally:
        server._sessions.pop("sid", None)
    ...
    assert "result" in resp, resp
    assert resp["result"]["status"] == "parked"
    assert resp["result"]["woke"] is False
    assert calls["inject_text"] == "remember the five-sentence cap"
    assert "interrupt_called" not in calls


def test_session_inject_idle_parks_on_session_without_agent():
        ...
        resp = server.handle_request(
            {
                "id": "1",
                "method": "session.inject",
                "params": {"session_id": "sid", "text": "parked idle note"},
            }
        )
        parked = server._take_tui_next_step(server._sessions["sid"])
    finally:
        server._sessions.pop("sid", None)
    ...
    assert resp["result"]["status"] == "parked"
    assert resp["result"]["woke"] is False
    assert parked == ["parked idle note"]


def test_tui_leftover_parks_without_queued_prompt():
    session = _session(agent=types.SimpleNamespace())
    assert server._park_tui_next_step(session, "leftover note") == 1
    assert session.get("queued_prompt") is None
    assert server._take_tui_next_step(session) == ["leftover note"]
    assert server._take_tui_next_step(session) == []


def test_command_dispatch_inject_does_not_send():
    sid = "inject-session"
    server._sessions[sid] = _session(agent=types.SimpleNamespace())
    try:
        resp = server.handle_request(
            {
                "id": "1",
                "method": "command.dispatch",
                "params": {"name": "inject", "arg": "do not wake", "session_id": sid},
            }
        )
        parked = server._take_tui_next_step(server._sessions[sid])
    finally:
    ...
    assert "does not start a turn" in resp["result"]["output"]
    assert parked == ["do not wake"]
...
def test_session_info_includes_install_warning_for_pip(monkeypatch):
    ...
    monkeypatch.setattr("hairball_cli.config.detect_install_method", lambda: "pip")
...
def test_session_info_omits_install_warning_for_git(monkeypatch):
    monkeypatch.setattr("hairball_cli.config.detect_install_method", lambda: "git")
...
def test_prompt_submit_forwards_completion_assessment_without_changing_answer(monkeypatch):
    """TUI transports the side-channel assessment; chat text stays untouched."""

    assessment = {
        "state": "needs_audit",
        "clauses": [{"clause_id": "cc-1", "status": "pending"}],
    }
    ...
    class _Agent:
        def run_conversation(
            ...
            return {
                "final_response": "reply without injected contract prose",
                "messages": [{"role": "assistant", "content": "reply without injected contract prose"}],
    ...
    try:
        ...
        complete = [item for item in emits if item[0] == "message.complete"]
        assert len(complete) == 1
        payload = complete[0][2]
        assert payload["text"] == "reply without injected contract prose"
        assert payload["completion_assessment"] == assessment
    finally:
        server._sessions.pop("sid", None)


def test_prompt_submit_records_failed_plan_turn_when_agent_raises(monkeypatch):
    """A hard provider/runtime exception must not leave an approved Plan pristine."""
    ...
    context = {
        "kind": "execute",
        "agent_mode": "execute",
        "system_message": "execute exact approved plan",
        "plan_execution_id": "tui-plan-failed-1",
        "operation_id": "tui-plan-failed-1",
    }
    captured = []

    def _finalize(*args, **kwargs):
        captured.append((args, kwargs))
        return {"status": "partial", "plan_id": "plan-1"}

    server._sessions["sid"] = _session(
        agent=_Agent(),
        next_plan_context=context,
    )
    emits: list[tuple] = []
    try:
        monkeypatch.setattr(server.threading, "Thread", _ImmediateThread)
        ...
        monkeypatch.setattr(server, "render_message", lambda _t, _c: "")
        monkeypatch.setattr(server, "_emit", lambda *args: emits.append(args))
        monkeypatch.setattr("agent.plan_commands.finalize_plan_turn", _finalize)

        response = server.handle_request(
            {
                "id": "1",
                "method": "prompt.submit",
                "params": {"session_id": "sid", "text": "execute"},
            }
        )
        ...
        assert any(item[0] == "plan.state" for item in emits)
        assert any(item[0] == "error" for item in emits)
    finally:
        server._sessions.pop("sid", None)


def test_prompt_submit_forwards_plan_execution_identity_to_agent(monkeypatch):
    captured = {}
    ...
    context = {
        ...
        "agent_mode": "execute",
        "system_message": "execute exact approved plan",
        "plan_execution_id": "tui-plan-execution-1",
        "operation_id": "tui-plan-execution-1",
        "context_mode": "fresh",
        "plan_file": "/tmp/approved-plan.md",
    }
    server._sessions["sid"] = _session(
        agent=_Agent(),
        next_plan_context=context,
        history=[
            {"role": "user", "content": "old planning request"},
            {"role": "assistant", "content": "old planning discussion"},
        ],
    )
    try:
        monkeypatch.setattr(server.threading, "Thread", _ImmediateThread)
        ...
        monkeypatch.setattr(
            "agent.plan_commands.finalize_plan_turn",
        ...
        assert response.get("result")
        assert captured["plan_execution_id"] == "tui-plan-execution-1"
        assert captured["agent_mode"] == "execute"
        assert captured["system_message"] == "execute exact approved plan"
...
def test_interrupt_cancels_only_the_active_plan_execution(monkeypatch):
    from agent import plan_commands

    cancelled = []
    monkeypatch.setattr(
        plan_commands,
        "cancel_active_plan_turn",
        lambda *args, **kwargs: cancelled.append((args, kwargs)) or {
            "status": "cancelled"
        },
        raising=False,
    )
    ...
    agent = types.SimpleNamespace(
        session_id="tui-plan-session",
        _plan_execution_id="tui-plan-run-1",
    ...
    try:
        ...
        args, kwargs = cancelled[0]
        assert args == ("tui-plan-session", "tui-plan-run-1")
        assert kwargs["surface"] == "tui"
        assert kwargs["reason"] == "user_interrupted"
    finally:
        server._sessions.pop("sid", None)


@pytest.mark.parametrize("context_field", ["next_plan_context", "active_plan_context"])
def test_interrupt_cancels_plan_before_agent_identity_is_published(
    monkeypatch, context_field
):
    from agent import plan_commands
    ...
    context = {
        "kind": "execute",
        "session_id": "tui-original-plan-session",
        "plan_execution_id": "tui-pending-plan-run-1",
    }
    session = _session(
        agent=types.SimpleNamespace(
            session_id="tui-rotated-session",
            interrupt=lambda: None,
        ),
        running=True,
        _run_thread=_LiveThread(),
    )
    session[context_field] = context
    server._sessions["sid"] = session
    try:
        response = server.handle_request(
            {
        ...
        assert response.get("result")
        assert len(cancelled) == 1
        args, _kwargs = cancelled[0]
        assert args == (
            "tui-original-plan-session",
            "tui-pending-plan-run-1",
        )
    finally:
        server._sessions.pop("sid", None)
...
def test_get_db_degrades_cleanly_when_sessiondb_init_fails(monkeypatch):
    fake_mod = types.ModuleType("hairball_state")
    ...
    monkeypatch.setitem(sys.modules, "hairball_state", fake_mod)
...
def test_session_delete_success_returns_deleted_id(monkeypatch):
    ...
    # too — not just the SQLite row.  The autouse _isolate_hairball_home
    # fixture pins HAIRBALL_HOME to a temp dir; the handler should append
    # /sessions to it.
...
# --------------------------------------------------------------------------
# model.options — curated-list parity with `hairball model` and classic /model
# --------------------------------------------------------------------------
...
def test_model_options_does_not_overwrite_curated_models(monkeypatch):
    ...
    list as `hairball model` and the classic CLI /model picker.
    ...
    for providers like managed whose /models endpoint returns image/video
    ...
    with patch(
        "hairball_cli.model_switch.list_authenticated_providers",
        ...
        # guarding against.
        with patch("hairball_cli.models.provider_model_ids") as live_fetch:
...
def test_model_options_hides_unconfigured_providers_by_default(monkeypatch):
    from hairball_cli.inventory import ConfigContext

    calls = []

    monkeypatch.setattr(server, "_resolve_model", lambda: "")
    monkeypatch.setattr(
        "hairball_cli.inventory.load_picker_context",
    ...
    monkeypatch.setattr(
        "hairball_cli.inventory.build_models_payload",
...
def test_session_active_list_excludes_finalized_sessions(monkeypatch):
    ...
    standalone ``hairball --tui`` case) must still be reported.
    """
    class _DB:
        def get_session_title(self, key):
...
def test_browser_manage_status_falls_back_to_config_cdp_url(monkeypatch):
    ...
    with patch.dict(sys.modules, {"hairball_cli.config": fake_cfg}):
        resp = server.handle_request(
            {"id": "1", "method": "browser.manage", "params": {"action": "status"}}
        )
...
def test_browser_manage_connect_default_local_reports_launch_hint(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        ...
        with (
            patch(
                "hairball_cli.browser_connect.launch_chrome_debug",
                return_value=ChromeDebugLaunch(),
            ),
            patch("hairball_cli.browser_connect.manual_chrome_debug_command", return_value=None),
            patch(
                "hairball_cli.browser_connect.get_chrome_debug_candidates",
                return_value=[],
            ),
        ):
            resp = server.handle_request(
...
def test_browser_manage_connect_default_local_retries_after_launch(monkeypatch):
    ...
    with patch.dict(sys.modules, {"tools.browser_tool": fake}):
        with patch(
            "hairball_cli.browser_connect.try_launch_chrome_debug", return_value=True
        ):
            resp = server.handle_request(
                {"id": "1", "method": "browser.manage", "params": {"action": "connect"}}
            )
...
def test_reload_env_rpc_calls_hairball_cli_reload_env(monkeypatch):
    """reload.env mirrors classic CLI's `/reload` — re-reads ~/.hairball/.env
    ...
    with patch.dict(sys.modules, {"hairball_cli.config": fake}):
...
def test_make_agent_waits_for_shared_mcp_discovery(monkeypatch):
    ...
    from hairball_cli import mcp_startup
...
def test_session_save_writes_under_hairball_home_with_system_prompt(monkeypatch, tmp_path):
    """TUI /save (session.save RPC) must snapshot under the Hairball profile
    ...
    Regression: the gateway handler wrote ``hairball_conversation_*.json`` to
    ``os.path.abspath(...)`` (the workspace CWD) and only exported ``model``
    and ``messages``, so ``system_prompt`` was missing.
    """
    home = tmp_path / ".hairball"
    home.mkdir()
    monkeypatch.setenv("HAIRBALL_HOME", str(home))
    ...
    agent = types.SimpleNamespace(
        ...
        _cached_system_prompt="You are Hairball.",
    )
    history = [
        {"role": "user", "content": "hi"},
        {"role": "assistant", "content": "hello"},
    ...
    assert not list(work.glob("hairball_conversation_*.json"))
    ...
    assert payload["model"] == "hairball-test"
    assert payload["session_id"] == "20260101_120000_abc123"
    assert payload["session_start"] == "2026-01-01T12:00:00"
    assert payload["system_prompt"] == "You are Hairball."
...
def test_persist_model_switch_preserves_sibling_model_keys(tmp_path, monkeypatch):
    ...
    # save_config_value() resolves the config path from cli._hairball_home, which
    # is captured at import time — patch it directly (set_hairball_home_override
    # does NOT affect this snapshot).
    monkeypatch.setattr(cli, "_hairball_home", tmp_path)
    ...
    def test_auth_error_tries_fallback_chain(self, monkeypatch):
        ...
        def fake_resolve(**kwargs):
            if kwargs.get("requested") == "openai-codex":
                raise AuthError("No reference credentials stored")
...
def test_get_usage_exposes_complete_actual_cost():
    agent = types.SimpleNamespace(
        model="anthropic/claude-sonnet-4.6",
        provider="openrouter",
        base_url="https://openrouter.ai/api/v1",
        ...
        session_actual_cost_usd=0.10,
        session_cost_actual_call_count=1,
        session_cost_total_call_count=1,
    ...
    assert usage["estimated_cost_usd"] == pytest.approx(0.12)
    assert usage["actual_cost_usd"] == pytest.approx(0.10)
    assert usage["cost_status"] == "actual"
    assert usage["actual_cost_complete"] is True


def test_get_usage_exposes_partial_actual_without_replacing_estimate():
    agent = types.SimpleNamespace(
        model="review",
        provider="moa",
        base_url="",
        session_total_tokens=340,
        session_estimated_cost_usd=0.30,
        session_actual_cost_usd=0.08,
        session_cost_actual_call_count=1,
        session_cost_total_call_count=3,
    ...
    assert usage["actual_cost_usd"] == pytest.approx(0.08)
    assert usage["actual_cost_coverage"] == pytest.approx(1 / 3)
    assert usage["actual_cost_complete"] is False
