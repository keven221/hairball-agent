"""Tests for hairball_state.py — SessionDB SQLite CRUD, FTS5 search, export."""
...
import hairball_state
from hairball_state import SCHEMA_SQL, SCHEMA_VERSION, SessionDB
...
class TestSessionLifecycle:
    ...
    def test_provider_actual_cost_and_coverage_persist_per_route(self, db):
        """Actual cost is useful only with its observation denominator."""
        db.create_session(session_id="cost-routes", source="cli")
        db.update_token_counts(
            "cost-routes",
            input_tokens=100,
            output_tokens=20,
            ...
            cost_actual_call_count=1,
            cost_total_call_count=1,
            cost_status="actual",
        ...
        session = db.get_session("cost-routes")
        assert session["estimated_cost_usd"] == pytest.approx(0.10)
        assert session["actual_cost_usd"] == pytest.approx(0.02)
        assert session["cost_actual_call_count"] == 1
        assert session["cost_total_call_count"] == 2
        ...
        assert rows["model-a"]["actual_cost_usd"] == pytest.approx(0.02)
        assert rows["model-a"]["cost_actual_call_count"] == 1
        assert rows["model-a"]["cost_total_call_count"] == 1
        assert rows["model-b"]["actual_cost_usd"] == 0
        assert rows["model-b"]["cost_actual_call_count"] == 0
        assert rows["model-b"]["cost_total_call_count"] == 1

    def test_actual_only_delta_is_not_dropped_from_per_model_usage(self, db):
        """A zero-token reconciliation event must still reach the route row."""
        db.create_session(session_id="cost-only", source="cli", model="m")
        db.update_token_counts(
            "cost-only",
            model="m",
            billing_provider="openrouter",
        ...
        row = db._conn.execute(
            "SELECT * FROM session_model_usage WHERE session_id = 'cost-only'"
        ).fetchone()
        assert row is not None
        assert row["actual_cost_usd"] == pytest.approx(0.004)
        assert row["cost_actual_call_count"] == 1

    def test_absolute_cost_coverage_overwrites_summary_without_route_double_count(self, db):
        db.create_session(session_id="cost-absolute", source="telegram", model="m")
        db.update_token_counts(
            "cost-absolute",
            input_tokens=100,
            estimated_cost_usd=0.5,
            actual_cost_usd=0.4,
            cost_actual_call_count=2,
            cost_total_call_count=3,
            api_call_count=3,
            absolute=True,
        )
        session = db.get_session("cost-absolute")
        assert session["cost_actual_call_count"] == 2
        assert session["cost_total_call_count"] == 3
        assert db._conn.execute(
            "SELECT COUNT(*) FROM session_model_usage WHERE session_id = 'cost-absolute'"
        ).fetchone()[0] == 0
    ...
    def test_db_initializes_without_fts5_module(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("hairball_state.sqlite3.connect", connect_without_fts)
    ...
    def test_fts_runtime_restores_triggers_after_no_fts_open(
        ...
        monkeypatch.setattr("hairball_state.sqlite3.connect", real_connect)
    ...
    def test_base_fts_rebuilds_after_trigger_repair_without_trigram(
        ...
        monkeypatch.setattr("hairball_state.sqlite3.connect", connect_without_trigram)
        restored = SessionDB(db_path=db_path)
        try:
            assert restored._fts_enabled is True
...
class TestMessageStorage:
    ...
    def test_codex_reasoning_items_persisted_and_restored(self, db):
        """codex_reasoning_items (encrypted blobs for reference Responses API) are
...
class TestFTS5Search:
    ...
    def test_sanitize_fts5_query_runtime_is_bounded(self):
        ...
        from hairball_state import MAX_FTS5_QUERY_CHARS, SessionDB
...
class TestDeleteAndExport:
    ...
    def test_import_exported_session_round_trips(self, db, tmp_path):
        ...
        try:
            ...
            assert imported["estimated_cost_usd"] == pytest.approx(0.30)
            assert imported["actual_cost_usd"] == pytest.approx(0.08)
            assert imported["cost_actual_call_count"] == 1
            assert imported["cost_total_call_count"] == 3
...
class TestPruneSessionFilters:
    ...
    def test_title_and_message_count_filters(self, db):
        self._mk(db, "smoke1", age_seconds=60, title="reference Smoke Test 1",
                 message_count=2)
    ...
    def test_cost_filter_uses_actual_only_when_coverage_is_complete(self, db):
        self._mk_rich(
            db,
            "partial",
        ...
        self._mk_rich(
            ...
            cost_actual_call_count=3,
            cost_total_call_count=3,
        )

        # Partial actual is a subtotal, so the complete estimate remains the
        # filter value. Complete actual is authoritative.
        rows = db.list_prune_candidates(min_cost=0.7)
        assert {row["id"] for row in rows} == {"partial"}
        rows = db.list_prune_candidates(max_cost=0.25)
        assert {row["id"] for row in rows} == {"complete"}
...
class TestSchemaInit:
    def test_wal_mode(self, db):
        from hairball_state import _sqlite_wal_reset_bug_is_fixed

        cursor = db._conn.execute("PRAGMA journal_mode")
        mode = cursor.fetchone()[0]
        expected = "wal" if _sqlite_wal_reset_bug_is_fixed() else "delete"
    ...
    def test_schema_version(self, db):
        from hairball_state import SCHEMA_VERSION
    ...
    def test_topic_mode_schema_is_not_auto_migrated_on_open(self, tmp_path):
        ...
        The gateway must remain rollback-safe: simply upgrading Hairball and starting
    ...
    def test_v9_migration_skips_v10_trigram_backfill_before_v11_rebuild(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr("hairball_state.sqlite3.connect", connect_with_trace)
        migrated_db = SessionDB(db_path=db_path)
        try:
    ...
    def test_schema_sql_is_source_of_truth(self, db):
        ...
        """
        from hairball_state import SCHEMA_SQL
...
class TestConcurrentWriteSafety:
    ...
    def test_sqlite_timeout_is_at_least_30s(self, db):
        ...
        import inspect
        from hairball_state import SessionDB as _SessionDB
...
class TestApplyWalProbe:
    ...
    def _fixed_sqlite_runtime(self, monkeypatch):
        """Exercise the WAL branches independently of the host Python build."""
        import hairball_state

        monkeypatch.setattr(
            hairball_state,
            "_sqlite_wal_reset_bug_is_fixed",
            lambda version=None: True,
        )
    ...
    def test_macos_checkpoint_fullsync_barrier_applied(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hairball_state.sys, "platform", "darwin")
    ...
    def test_checkpoint_fullsync_barrier_skipped_off_darwin(self, tmp_path, monkeypatch):
        ...
        monkeypatch.setattr(hairball_state.sys, "platform", "linux")
    ...
    def test_macos_synchromanaged_full_enforced_fresh(self, tmp_path, monkeypatch):
    ...
    def test_macos_synchromanaged_full_enforced_already_wal(self, tmp_path, monkeypatch):
...
def test_refresh_compression_lock_requires_holder_and_preserves_reclaimability(db, monkeypatch):
    ...
    monkeypatch.setattr(hairball_state.time, "time", lambda: 1000.0)
    ...
    monkeypatch.setattr(hairball_state.time, "time", lambda: 1005.0)
    ...
    monkeypatch.setattr(hairball_state.time, "time", lambda: 1016.0)
