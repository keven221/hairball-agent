"""Tests for hairball_bootstrap — Windows UTF-8 stdio shim.

The bootstrap module is imported at the top of every Hairball entry point
(hairball, hairball-agent, hairball-acp, gateway, batch_runner, cli.py).  It
...
Key invariants covered by these tests:
  ...
  5. Load order: every Hairball entry point imports hairball_bootstrap as its
...
def _fresh_import():
    """Return a freshly-imported hairball_bootstrap module.

    Drops any cached copy from sys.modules first so module-level code
    runs again and the platform check re-evaluates.
    """
    sys.modules.pop("hairball_bootstrap", None)
    import hairball_bootstrap  # noqa: WPS433
    return hairball_bootstrap
...
class TestWindowsBehavior:
    ...
    def test_stdout_reconfigured_to_utf8_on_windows(self):
        # The live process's stdout should now be UTF-8 (the Hairball CLI
...
class TestEntryPointsImportBootstrap:
    """Every Hairball entry point must import hairball_bootstrap as its
    ...
    # Entry points that invoke Hairball as a process.  Each one must
    # import hairball_bootstrap before doing any file I/O or stdout writes.
    ENTRY_POINTS = [
        "hairball_cli/main.py",   # hairball CLI (console_script)
        "run_agent.py",          # hairball-agent (console_script)
        "acp_adapter/entry.py",  # hairball-acp (console_script)
    ...
    def test_entry_point_imports_bootstrap(self, path):
        """The file must contain 'import hairball_bootstrap' and that
        ...
        half-finished ``hairball update`` (git-reset landed new code but
        ``uv pip install -e .`` didn't finish re-registering
        ``hairball_bootstrap`` as a top-level module) leaves hairball
        ...
        """
        # Resolve relative to the hairball-agent repo root.  Tests live
        # at tests/test_hairball_bootstrap.py, so go up one dir.
        import pathlib
        ...
        for node in ast.iter_child_nodes(tree):
            ...
            # hairball start even when hairball_bootstrap hasn't been
        ...
        assert first_import_name == "hairball_bootstrap", (
            f"{path}: first top-level import is {first_import_name!r}, "
            f"but it must be 'hairball_bootstrap' so UTF-8 stdio is "
            f"configured before anything else initializes.  Move the "
            f"'import hairball_bootstrap' line to be the first import."
        )


class TestHardenImportPath:
    """harden_import_path() must keep a same-named package in the launch
    directory from shadowing Hairball's own top-level modules — covering both
    the relative ('' / '.') and absolute-path forms the cwd can take on
    sys.path (issue #51286)."""

    def _run(self, hb, path_seed, env=None):
        original = sys.path[:]
        original_env = os.environ.get("HAIRBALL_PYTHON_SRC_ROOT")
        try:
            sys.path[:] = path_seed
            if env is not None:
                os.environ["HAIRBALL_PYTHON_SRC_ROOT"] = env
            elif "HAIRBALL_PYTHON_SRC_ROOT" in os.environ:
                del os.environ["HAIRBALL_PYTHON_SRC_ROOT"]
            hb.harden_import_path(src_root="/opt/hairball")
            return sys.path[:]
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HAIRBALL_PYTHON_SRC_ROOT", None)
            else:
                os.environ["HAIRBALL_PYTHON_SRC_ROOT"] = original_env

    def test_relative_cwd_forms_removed(self):
        hb = _fresh_import()
        result = self._run(hb, ["", ".", "/opt/hairball", "/usr/lib/python"])
    ...
    def test_absolute_cwd_path_loses_to_src_root(self):
        ...
        # Hairball root.  The guard must relocate Hairball to the front.
        hb = _fresh_import()
        result = self._run(hb, ["/home/user/tg-ws-proxy", "/opt/hairball"])
        assert result[0] == "/opt/hairball"
        # The cwd absolute path may still appear (it can hold legit deps),
        # but only AFTER the Hairball root.
        assert result.index("/opt/hairball") < result.index("/home/user/tg-ws-proxy")

    def test_src_root_not_duplicated(self):
        hb = _fresh_import()
        result = self._run(hb, ["/opt/hairball", "/opt/hairball", ""])
        assert result.count("/opt/hairball") == 1
    ...
    def test_env_var_used_when_no_arg(self):
        ...
        try:
            ...
            os.environ["HAIRBALL_PYTHON_SRC_ROOT"] = "/env/hairball"
            hb.harden_import_path()
            assert sys.path[0] == "/env/hairball"
        finally:
            sys.path[:] = original
            if original_env is None:
                os.environ.pop("HAIRBALL_PYTHON_SRC_ROOT", None)
