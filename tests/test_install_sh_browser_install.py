...
Browser automation is optional. The installer should not leave Hairball
...
def test_install_script_does_not_autodetect_system_browser_on_path() -> None:
    ...
    assert "Skipping Playwright browser download; Hairball will use the system browser." not in text
...
def test_agent_browser_chromium_install_is_timeout_guarded() -> None:
    """The ensure-mode Chromium download must not bypass the shared watchdog."""
    text = INSTALL_SH.read_text()
    ensure_body = re.search(
        r"^ensure_browser\(\) \{(?P<body>.*?)^\}",
        text,
        re.MULTILINE | re.DOTALL,
    )
    assert ensure_body is not None
    body = ensure_body.group("body")
    install_tail = body[body.index('local ab_bin="$HAIRBALL_HOME/node/bin/agent-browser"') :]

    assert (
        'run_browser_install_with_timeout "$NODE_DEPS_TIMEOUT" "$ab_bin" install'
        in install_tail
    )
    assert '"$ab_bin" install 2>/dev/null' not in install_tail
    assert 'if [ "$ab_rc" -eq 124 ]; then' in install_tail
    assert "tail -n 20" in install_tail


def test_agent_browser_timeout_kills_download_process_tree(tmp_path) -> None:
    """A wedged fake downloader and its child must not outlive the watchdog."""
    ...
    installer = tmp_path / "agent-browser"
    installer.write_text(
        "#!/bin/sh\n"
        f"( sleep 4; printf orphan > {str(marker)!r} ) &\n"
        "wait\n",
        encoding="utf-8",
    )
...
run_browser_install_with_timeout 1 {str(installer)!r} install
rc=$?
echo RC=$rc
exit "$rc"
"""
    ...
    assert elapsed < 5
    time.sleep(2)
    assert not marker.exists(), "timed-out Chromium downloader child survived"


def test_run_with_timeout_fast_command_is_not_misclassified(tmp_path) -> None:
    """A successful command returns immediately instead of waiting for the watchdog."""
    source = INSTALL_SH.read_text()
    functions = []
    for name in (
        "_stop_timeout_process_group",
    ...
    command = tmp_path / "fast-command"
    command.write_text("#!/bin/sh\nexit 0\n", encoding="utf-8")
    command.chmod(0o755)
    harness = f"""
{chr(10).join(functions)}
run_with_timeout 5 {str(command)!r}
echo RC=$?
"""
    started = time.monotonic()
...
def test_agent_browser_interrupt_kills_download_process_tree(tmp_path) -> None:
    """Ctrl+C must clean the isolated downloader group before returning 130."""
    source = INSTALL_SH.read_text()
    functions = []
    for name in (
        "_stop_timeout_process_group",
    ...
    marker = tmp_path / "interrupt-orphan-marker"
    ...
    harness = f"""
{chr(10).join(functions)}
run_with_timeout 30 {str(installer)!r}
rc=$?
echo RC=$rc
exit "$rc"
"""
    ...
    time.sleep(4.2)
    assert not marker.exists(), "interrupted Chromium downloader child survived"
