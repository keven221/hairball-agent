...
The ``hairball update`` Python path already clears the conflict with ``git reset``
...
def test_install_ps1_stops_venv_resident_processes_before_removing_venv() -> None:
    ...
    ``venv\\Scripts\\pythonw.exe -m hairball_cli.main gateway run`` — image name
    ``pythonw``, not ``hairball.exe`` — so the ``taskkill /IM hairball.exe`` guard
    ...
    # The hairball.exe tree-kill is preserved (kills spawned child processes too).
    assert 'taskkill /F /T /IM hairball.exe' in text
