...
from cli import HairballCLI, _build_compact_banner, _rich_text_from_ansi
from hairball_cli.skin_engine import get_active_skin, set_active_skin


def _make_cli_stub():
    cli = HairballCLI.__new__(HairballCLI)
...
class TestCliSkinPromptIntegration:
    ...
    def test_icon_only_skin_symbol_still_visible_in_special_states(self):
        ...
        with patch("hairball_cli.skin_engine.get_active_prompt_symbol", return_value="⚔ "):
            assert cli._get_tui_prompt_fragments() == [("class:sudo-prompt", "🔑 ⚔ ")]
...
class TestCompactBannerSkinIntegration:
    def test_default_compact_banner_uses_hairball_cat_branding(self):
        set_active_skin("default")

        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Hairball Agent v0.1.0 (test)"}):
            banner = _build_compact_banner()

        assert "ฅ HAIRBALL - PERSONAL AI AGENT" in banner
        assert "MANAGED" not in banner
        assert ("HER" + "MES") not in banner

    def test_poseidon_compact_banner_uses_skin_branding_instead_of_hairball_default(self):
    ...
    def test_compact_banner_shows_version_label(self):
        ...
        with patch("cli.shutil.get_terminal_size", return_value=SimpleNamespace(columns=90)), \
             patch.dict(_build_compact_banner.__globals__, {"format_banner_version_label": lambda: "Hairball Agent v1.0 (test) · upstream abc12345"}):
            banner = _build_compact_banner()
