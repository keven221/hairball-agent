...
Real incident (gateway session 1518671026962174144)::
    ...
    cannot import name 'env_float' from 'utils' (~/.hairball/hairball-agent/utils.py)
...
2. ``hairball update`` ran ``git pull``, updating ``utils.py`` (now defining
...
``hairball_cli/main.py`` (the ``hairball update`` flow, ~line 9326) already
acknowledges this exact hazard -- "source files on disk are newer than cached
Python modules in this process" -- and reloads ``hairball_constants`` after the
