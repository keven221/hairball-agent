"""
Tests for timezone support (hairball_time module + integration points).
...
import hairball_time


def _reset_hairball_time_cache():
    """Reset the hairball_time module cache (replacement for removed reset_cache)."""
    hairball_time._cached_tz = None
    hairball_time._cached_tz_name = None
    hairball_time._cache_resolved = False


# =========================================================================
# hairball_time.now() — core helper
# =========================================================================

class TestHairballTimeNow:
    """Test the timezone-aware now() helper."""

    def setup_method(self):
        _reset_hairball_time_cache()

    def teardown_method(self):
        _reset_hairball_time_cache()
        os.environ.pop("HAIRBALL_TIMEZONE", None)

    def test_valid_timezone_applies(self):
        """With a valid IANA timezone, now() returns time in that zone."""
        os.environ["HAIRBALL_TIMEZONE"] = "Asia/Kolkata"
        result = hairball_time.now()
    ...
    def test_utc_timezone(self):
        """UTC timezone works."""
        os.environ["HAIRBALL_TIMEZONE"] = "UTC"
        result = hairball_time.now()
        assert result.utcoffset() == timedelta(0)

    def test_us_eastern(self):
        """US/Eastern timezone works (DST-aware zone)."""
        os.environ["HAIRBALL_TIMEZONE"] = "America/New_York"
        result = hairball_time.now()
        assert result.tzinfo is not None
    ...
    def test_invalid_timezone_falls_back(self, caplog):
        ...
        os.environ["HAIRBALL_TIMEZONE"] = "Mars/Olympus_Mons"
        with caplog.at_level(logging.WARNING, logger="hairball_time"):
            result = hairball_time.now()
    ...
    def test_cache_invalidation(self):
        ...
        os.environ["HAIRBALL_TIMEZONE"] = "UTC"
        _reset_hairball_time_cache()
        r1 = hairball_time.now()
        assert r1.utcoffset() == timedelta(0)

        os.environ["HAIRBALL_TIMEZONE"] = "Asia/Kolkata"
        _reset_hairball_time_cache()
        r2 = hairball_time.now()
...
class TestGetTimezone:
    ...
    def test_returns_zoneinfo_for_valid(self):
        os.environ["HAIRBALL_TIMEZONE"] = "Europe/London"
        tz = hairball_time.get_timezone()
    ...
    def test_returns_none_for_invalid(self):
        os.environ["HAIRBALL_TIMEZONE"] = "Not/A/Timezone"
        tz = hairball_time.get_timezone()
        assert tz is None
...
class TestCodeExecutionTZ:
    ...
    def test_tz_injected_when_configured(self):
        """When HAIRBALL_TIMEZONE is set, child process sees TZ env var.
        ...
        #   2) HAIRBALL_TIMEZONE itself does NOT leak into the child env
        probe = (
            'import os; '
            'print("TZ=" + os.environ.get("TZ", "NOT_SET")); '
            'print("HAIRBALL_TIMEZONE=" + os.environ.get("HAIRBALL_TIMEZONE", "NOT_SET"))'
        )
        ...
        assert "HAIRBALL_TIMEZONE=NOT_SET" in result["output"], (
            "HAIRBALL_TIMEZONE should not leak into child env (only TZ)"
        )

    def test_tz_not_injected_when_empty(self):
        """When HAIRBALL_TIMEZONE is not set, child process has no TZ."""
        import json as _json
        os.environ.pop("HAIRBALL_TIMEZONE", None)
...
class TestCronTimezone:
    ...
    def test_ensure_aware_naive_preserves_absolute_time(self):
        ...
        Regression: the old code used replace(tzinfo=hairball_tz) which shifted
        absolute time when system-local tz != Hairball tz.  The fix interprets
    ...
    def test_ensure_aware_normalizes_aware_to_hairball_tz(self):
        """Already-aware datetimes should be normalized to Hairball tz."""
        from cron.jobs import _ensure_aware
        ...
        # Must be in Hairball tz (Kolkata) but same absolute instant
    ...
    def test_ensure_aware_due_job_not_skipped_when_system_ahead(self, tmp_path, monkeypatch):
        """Reproduce the actual bug: system tz ahead of Hairball tz caused
        overdue jobs to appear as not-yet-due.

        Scenario: system is Asia/Kolkata (UTC+5:30), Hairball is UTC.
    ...
    def test_get_due_jobs_naive_cross_timezone(self, tmp_path, monkeypatch):
        """Naive past timestamps must be detected as due even when Hairball tz
        ...
        # Use a Hairball timezone far behind UTC so that the numeric wall time
        # of the naive timestamp exceeds _hairball_now's wall time — this would
        # have caused a false "not due" with the old replace(tzinfo=...) approach.
        os.environ["HAIRBALL_TIMEZONE"] = "Pacific/Midway"  # UTC-11
        _reset_hairball_time_cache()
        ...
        due = get_due_jobs()
        assert len(due) == 1, (
            "Naive past timestamp should be due regardless of Hairball timezone"
        )
    ...
    def test_create_job_stores_tz_aware_timestamps(self, tmp_path, monkeypatch):
        ...
        os.environ["HAIRBALL_TIMEZONE"] = "US/Eastern"
        _reset_hairball_time_cache()
