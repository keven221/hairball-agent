...
class TestToolsetConsistency:
    ...
    def test_hairball_platforms_share_core_tools(self):
        """All hairball-* platform toolsets share the same core tools.

        Platform-specific additions (e.g. ``discord`` / ``discord_admin``
        on hairball-discord, gated on DISCORD_BOT_TOKEN) are allowed on top —
        the invariant is that the core set is identical across platforms.
        """
        platforms = ["hairball-cli", "hairball-telegram", "hairball-discord", "hairball-whatsapp", "hairball-slack", "hairball-signal", "hairball-homeassistant"]
...
class TestDefaultPlatformWebSearchCoverage:
    def test_hairball_whatsapp_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("hairball-whatsapp")

    def test_hairball_api_server_toolset_includes_web_search(self):
        assert "web_search" in resolve_toolset("hairball-api-server")
