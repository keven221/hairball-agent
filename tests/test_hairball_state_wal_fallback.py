...
(SQLITE_PROTOCOL — typical on NFS/SMB), Hairball must fall back to
``journal_mode=DELETE`` so ``state.db`` / ``kanban.db`` remain usable.

Without this fallback, users on NFS-mounted ``HAIRBALL_HOME`` silently lose
...
def _reset_last_init_error():
    ...
    hairball_state._set_last_init_error(None)
    yield
    hairball_state._set_last_init_error(None)
...
def _reset_wal_fallback_warned_paths():
    ...
    hairball_state._wal_fallback_warned_paths.clear()
    yield
    hairball_state._wal_fallback_warned_paths.clear()
...
def test_wal_reset_fix_version_matrix(version, expected):
    assert hairball_state._sqlite_wal_reset_bug_is_fixed(version) is expected


def test_vulnerable_sqlite_uses_delete_mode(tmp_path, monkeypatch, caplog):
    """Affected runtimes must not expose multi-process databases to WAL."""
    monkeypatch.setattr(
        hairball_state,
        "_sqlite_wal_reset_bug_is_fixed",
        lambda version=None: False,
    )
    conn = sqlite3.connect(str(tmp_path / "vulnerable.db"), isolation_level=None)
    try:
        with caplog.at_level("WARNING", logger="hairball_state"):
            mode = apply_wal_with_fallback(conn, db_label="vulnerable.db")
        assert mode == "delete"
        assert conn.execute("PRAGMA journal_mode").fetchone()[0] == "delete"
        assert "WAL-reset corruption bug" in caplog.text
    finally:
        conn.close()


def test_vulnerable_sqlite_atomically_downgrades_existing_wal(tmp_path, monkeypatch):
    """A quiescent existing WAL DB keeps its data when the workaround activates."""
    monkeypatch.setattr(
        hairball_state,
        "_sqlite_wal_reset_bug_is_fixed",
        lambda version=None: False,
    ...
    primer = sqlite3.connect(str(path), isolation_level=None)
    try:
        assert primer.execute("PRAGMA journal_mode=WAL").fetchone()[0] == "wal"
        primer.execute("CREATE TABLE t (value TEXT)")
        primer.execute("INSERT INTO t VALUES ('preserved')")
    finally:
        primer.close()
    ...
    try:
        ...
        assert conn.execute("SELECT value FROM t").fetchone()[0] == "preserved"
...
class TestApplyWalWithFallback:
    ...
    def _fixed_sqlite_runtime(self, monkeypatch):
        """Exercise filesystem fallback with a WAL-safe SQLite runtime."""
        monkeypatch.setattr(
            hairball_state,
            "_sqlite_wal_reset_bug_is_fixed",
    ...
    def test_warning_deduplicated_per_db_label(self, tmp_path, caplog):
        ...
        connection on every operation — see hairball_cli/kanban_db.py).
...
class TestGetLastInitError:
    ...
    def test_success_does_not_clear_prior_error(self, tmp_path):
        ...
        """
        hairball_state._set_last_init_error("OperationalError: locking protocol")
    ...
    def test_captures_cause_on_failed_init(self, tmp_path):
        ...
        with patch("hairball_state.sqlite3.connect", side_effect=gated_connect):
...
class TestFormatSessionDbUnavailable:
    ...
    def test_includes_cause(self):
        ...
        hairball_state._set_last_init_error("OperationalError: generic SQLite error")
