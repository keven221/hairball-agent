...
class TestHandleFunctionCall:
    def test_session_router_separates_authorized_from_model_visible(self):
        visible = [
            {
                "type": "function",
        ...
        with patch("model_tools.get_tool_definitions", return_value=authorized):
            router = build_session_tool_router(model_visible_specs=visible)

        assert router.model_visible_tool_names == {"tool_search"}
        assert router.authorized_tool_names == {"tool_search", "read_file"}
    ...
    def test_explicit_session_scope_rejects_ungranted_registered_tool(self):
        with patch("model_tools.registry.dispatch") as dispatch:
            result = json.loads(
                handle_function_call(
                    "web_search",
                    {"q": "probe"},
                    session_tool_router=SessionToolRouter.from_names(["read_file"]),
                )
            )
    ...
    def test_explicit_session_scope_allows_granted_registered_tool(self):
        with patch(
            "model_tools.registry.dispatch",
            return_value=json.dumps({"executed": True}),
        ) as dispatch:
            result = json.loads(
                handle_function_call(
                    "web_search",
                    {"q": "probe"},
                    session_tool_router=SessionToolRouter.from_names(
    ...
    def test_none_session_scope_preserves_trusted_host_dispatch(self):
        with patch(
            "model_tools.registry.dispatch",
            return_value=json.dumps({"executed": True}),
        ) as dispatch:
            result = json.loads(
                handle_function_call("web_search", {"q": "probe"})
            )

        assert result == {"executed": True}
        dispatch.assert_called_once()

    def test_explicit_empty_session_scope_denies_all_model_tools(self):
        with patch("model_tools.registry.dispatch") as dispatch:
            result = json.loads(
                handle_function_call(
                    "web_search",
    ...
    def test_tool_hooks_receive_session_and_tool_call_ids(self):
        with (
            ...
            patch("hairball_cli.plugins.has_hook", return_value=True),
            patch("hairball_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_no_listener_skips_post_and_transform_emit(self):
        ...
        with (
            patch("model_tools.registry.dispatch", return_value='{"ok":true}'),
            patch("hairball_cli.plugins.has_hook", return_value=False),
            patch("hairball_cli.plugins.invoke_hook") as mock_invoke_hook,
        ):
    ...
    def test_tool_request_and_execution_middleware_wrap_registry_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("hairball_cli.plugins.invoke_middleware", fake_invoke_middleware)
        monkeypatch.setattr("hairball_cli.plugins.get_plugin_manager", lambda: manager)
        hook_calls = []
        monkeypatch.setattr(
            "hairball_cli.plugins.invoke_hook",
            lambda hook_name, **kwargs: hook_calls.append((hook_name, kwargs)) or [],
        )
        monkeypatch.setattr("hairball_cli.plugins.has_hook", lambda name: True)
...
class TestAgentLoopTools:
    ...
    def test_predicate_is_the_single_routing_contract(self):
        assert all(is_agent_loop_tool(name) for name in _AGENT_LOOP_TOOLS)
        assert is_agent_loop_tool("terminal") is False
...
class TestPreToolCallBlocking:
    ...
    def test_blocked_tool_returns_error_and_skips_dispatch(self, monkeypatch):
        ...
        monkeypatch.setattr("hairball_cli.plugins.invoke_hook", fake_invoke_hook)
        monkeypatch.setattr("hairball_cli.plugins.has_hook", lambda name: True)
        monkeypatch.setattr("model_tools.registry.dispatch", fake_dispatch)
    ...
    def test_run_agent_pattern_fires_pre_tool_call_exactly_once(self, monkeypatch):
        ...
        """
        from hairball_cli.plugins import get_pre_tool_call_block_message
...
class TestDisabledToolsetsPlatformBundle:
    """Regression test for #33924: disabling a platform bundle (hairball-*)
    must not remove core tools from other enabled toolsets."""

    def test_disabling_platform_bundle_preserves_core_tools(self):
        """Disabling hairball-yuanbao should not strip core tools from hairball-telegram."""
        from model_tools import get_tool_definitions

        tools_telegram = get_tool_definitions(
            enabled_toolsets=["hairball-telegram"],
            quiet_mode=True,
        )
        tools_telegram_no_yuanbao = get_tool_definitions(
            enabled_toolsets=["hairball-telegram"],
            disabled_toolsets=["hairball-yuanbao"],
            quiet_mode=True,
        )
        ...
        assert names_telegram == names_no_yuanbao, (
            f"Tools lost after disabling hairball-yuanbao: "
            f"{names_telegram - names_no_yuanbao}"
        )

    def test_disabling_platform_bundle_removes_own_tools(self):
        """Disabling hairball-discord should remove discord-specific tools."""
        from model_tools import get_tool_definitions

        tools = get_tool_definitions(
            enabled_toolsets=["hairball-discord"],
            disabled_toolsets=["hairball-discord"],
            quiet_mode=True,
        )
    ...
    def test_disabling_non_platform_toolset_still_works(self):
        """Disabling a regular (non-hairball-) toolset still subtracts all tools."""
        from model_tools import get_tool_definitions
    ...
    def test_disabling_bundle_removes_platform_tools_but_keeps_core(self):
        """Disabling hairball-discord (when enabled) removes discord/discord_admin
        from the resolved delta but keeps core tools — via bundle_non_core_tools."""
        from toolsets import bundle_non_core_tools, _HAIRBALL_CORE_TOOLS

        delta = bundle_non_core_tools("hairball-yuanbao")
        # The delta is the bundle's platform-specific tools, NOT core.
        assert "yb_send_dm" in delta
        assert not (delta & set(_HAIRBALL_CORE_TOOLS)), "core tools must not be in the removal delta"
    ...
    def test_bundle_non_core_tools_unknown_falls_back(self):
        ...
        assert bundle_non_core_tools("hairball-does-not-exist") == set()
...
class TestDisabledToolsetsPostureToolset:
    ...
    not own -- same non-core-delta subtraction as hairball-* bundles (#33924) --
