...
binary download are exercised manually by `hairball secrets bitwarden
...
def hairball_home(tmp_path, monkeypatch):
    """Point Hairball at an isolated home directory."""
    home = tmp_path / ".hairball"
    home.mkdir()
    monkeypatch.setenv("HAIRBALL_HOME", str(home))
    # Some modules cache get_hairball_home; clear if needed.
    import hairball_constants
    if hasattr(hairball_constants, "_HAIRBALL_HOME_CACHE"):
        hairball_constants._HAIRBALL_HOME_CACHE = None  # type: ignore[attr-defined]
    return home
...
def test_install_bws_rejects_malicious_member(hairball_home, monkeypatch):
...
def test_install_bws_happy_path(hairball_home, monkeypatch):
...
def test_install_bws_checksum_mismatch(hairball_home, monkeypatch):
...
def test_install_bws_missing_checksum_entry(hairball_home, monkeypatch):
...
def test_env_loader_skips_when_disabled(tmp_path, monkeypatch):
    ...
    from hairball_cli.env_loader import _apply_external_secret_sources
...
def test_disk_cache_written_after_first_fetch(monkeypatch, tmp_path):
    """First fetch hits bws AND writes a 0600 file under hairball_home/cache/."""
    home = tmp_path / ".hairball"
    home.mkdir()
    fake_binary = tmp_path / "bws"
    fake_binary.write_text("")
