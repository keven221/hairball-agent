"""
Tests for mcp_serve — Hairball MCP server.
...
def _isolate_hairball_home(tmp_path, monkeypatch):
    """Redirect HAIRBALL_HOME to a temp directory."""
    monkeypatch.setenv("HAIRBALL_HOME", str(tmp_path))
    try:
        import hairball_constants
        monkeypatch.setattr(hairball_constants, "get_hairball_home", lambda: tmp_path)
...
def _create_test_db(db_path, session_id, messages):
    """Create a minimal SQLite DB mimicking hairball_state schema."""
...
class TestEventBridgePollE2E:
    ...
    def test_poll_picks_up_new_conversation_on_db_change(
        ...
        # _poll_once reads <HAIRBALL_HOME>/state.db for its mtime gate; the autouse
        # fixture points HAIRBALL_HOME at tmp_path.
        db_path = tmp_path / "state.db"
