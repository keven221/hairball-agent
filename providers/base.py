...
if TYPE_CHECKING:
    from providers.tool_control import ToolControlCapabilities
...
def _profile_user_agent() -> str:
    """Return a ``hairball-cli/<version>`` UA string, with a stable fallback.
    ...
    """
    try:
        from hairball_cli import __version__ as _ver  # lazy: avoid layer cycle at import time
        return f"hairball-cli/{_ver}"
    except Exception:
        return "hairball-cli"
...
class ProviderProfile:
    ...
    # (e.g. Xiaomi, which returns 400 "text is not set").
    supports_vision_tool_messages: bool = True

    # ``None`` uses the transport default.  ``False`` means the endpoint still
    # supports tools but automatic selection must be implicit (omit the
    # tool_choice control field).  This is intentionally not a forced/named
    # capability: Hairball continuation retries preserve model autonomy.
    supports_explicit_auto_tool_choice: bool | None = None
    ...
    def tool_control_capabilities(
        ...
        **context: Any,
    ) -> "ToolControlCapabilities":
        """Resolve automatic tool-selection support for one request.

        Subclasses may return a replaced capability for model-specific
        protocol restrictions.  The hook is request-scoped and read-only: it
        must never mutate reasoning config, tools, or provider/session state.
        """
        from dataclasses import replace

        from providers.tool_control import transport_default_capabilities

        caps = transport_default_capabilities(api_mode or self.api_mode)
        if self.supports_explicit_auto_tool_choice is not None:
            caps = replace(
                caps,
                supports_explicit_auto_tool_choice=bool(
                    self.supports_explicit_auto_tool_choice
                ),
                source=f"provider:{self.name}",
                reason=(
                    "provider declares explicit automatic tool choice"
                    if self.supports_explicit_auto_tool_choice
                    else "provider declares implicit automatic tool choice"
                ),
            )
        return caps
    ...
    def fetch_models(
        ...
        *,
        ...
        # hairball-cli UA so the catalog endpoint is reachable.
