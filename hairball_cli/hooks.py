"""hairball hooks — inspect and manage shell-script hooks.

Usage::

    hairball hooks list
    hairball hooks test <event> [--for-tool X] [--payload-file F]
    hairball hooks revoke <command>
    hairball hooks doctor

Consent records live under ``~/.hairball/shell-hooks-allowlist.json`` and
hook definitions come from the ``hooks:`` block in ``~/.hairball/config.yaml``
...
def hooks_command(args) -> None:
    """Entry point for ``hairball hooks`` — dispatches to the requested action."""
    sub = getattr(args, "hooks_action", None)

    if not sub:
        print("Usage: hairball hooks {list|test|revoke|doctor}")
        print("Run 'hairball hooks --help' for details.")
        return
...
def _cmd_list(_args) -> None:
    ...
    if not specs:
        print("No shell hooks configured in ~/.hairball/config.yaml.")
        print("See `hairball hooks --help` or")
                ...
                if entry and entry.get("approved_at"):
                    ...
                    if mtime_now and mtime_at and mtime_now > mtime_at:
                        print(
                            ...
                            f"run `hairball hooks doctor` to re-validate"
                        )
        print()
...
# stdin a script sees under `hairball hooks test` and `hairball hooks doctor`
...
_DEFAULT_PAYLOADS = {
    ...
    "pre_compact": {
        ...
        "post_message_count": None,
        "pre_approx_tokens": 32_000,
        "post_approx_tokens": None,
        "force": False,
        "focus_topic": None,
        "in_place": True,
        "outcome": None,
    ...
    "post_compact": {
        "session_id": "test-session",
        "old_session_id": "test-session",
        "platform": "cli",
        "model": "deepseek-v4-flash",
        "provider": "deepseek",
        "runtime": "hairball",
        "pre_message_count": 42,
        "post_message_count": 9,
        "pre_approx_tokens": 32_000,
        "post_approx_tokens": 8_000,
        "force": False,
        "focus_topic": None,
        "in_place": True,
        "outcome": "completed",
    ...
    "on_notification": {
        ...
        "platform": "cli",
        "notification": {
            "text": "Synthetic Hairball notice",
            "level": "warn",
            "kind": "sticky",
            "ttl_ms": None,
            "key": "test.notice",
    ...
    "subagent_stop": {
        ...
        "last_assistant_message": "Synthetic child response",
...
def _print_run_result(result: Dict[str, Any]) -> None:
    ...
    if parsed:
        print(f"      parsed (Hairball wire shape): {json.dumps(parsed)}")
    else:
...
def _doctor_one(spec, shell_hooks) -> int:
    ...
    if entry and entry.get("script_mtime_at_approval"):
        ...
        if mtime_now and mtime_at and mtime_now > mtime_at:
            ...
            print(f"      ⚠ script modified since approval "
                  ...
                  f"then `hairball hooks revoke` + re-approve to refresh")
    ...
    # is already allowlisted.  Otherwise `hairball hooks doctor` would execute
    ...
    if not entry:
        print("      ℹ skipped JSON smoke test — not allowlisted yet. "
              ...
              "then re-run `hairball hooks doctor`.")
