...
Toggles `model.openai_runtime` between "auto" (= chat_completions, Hairball'
...
The actual runtime resolution happens in hairball_cli.runtime_provider's
...
def parse_args(arg_string: str) -> tuple[Optional[str], list[str]]:
    ...
    if raw in {"off", "default", "disable", "hairball"}:
        return "auto", []
...
def apply(
    ...
    *,
    ...
    # the world state (managed block in ~/.codex/config.toml, hairball-tools
    ...
    if new_value == "codex_app_server":
        ...
        # Auto-migrate Hairball' MCP servers + reference's installed curated
        # plugins into ~/.codex/config.toml so the spawned codex subprocess
        # sees the same tool surface AND can call back into Hairball for
        # browser/web/delegate_task/vision/memory tools (#7 fix).
        # Failures are non-fatal — the runtime change still proceeds.
        try:
            from hairball_cli.codex_runtime_plugin_migration import migrate
            mig_report = migrate(config)
            # Tools/MCP servers (excluding the hairball-tools callback,
            # which is internal plumbing — surface separately).
            user_servers = [
                s for s in mig_report.migrated if s != "hairball-tools"
            ]
            if user_servers:
                msg_lines.append(
            ...
            # Native reference plugin migration (Linear, GitHub, etc.)
            if mig_report.migrated_plugins:
                msg_lines.append(
                    f"Migrated {len(mig_report.migrated_plugins)} native "
                    f"reference plugin(s): {', '.join(mig_report.migrated_plugins)}"
                )
            elif mig_report.plugin_query_error:
                msg_lines.append(
                    f"reference plugin discovery skipped: "
                    f"{mig_report.plugin_query_error}"
                )
            # Permissions + Hairball tool callback are always-on production
            ...
            if "hairball-tools" in mig_report.migrated:
                msg_lines.append(
                    "Hairball tool callback registered: codex can now use "
                ...
                msg_lines.append(
                    ...
                    "only on the default Hairball runtime — they need the "
                    "agent loop context.)"
                )
        ...
        msg_lines.append(
            "ChatGPT turns now run through `codex app-server` "
            "(terminal/file ops/patching inside reference; "
            "Hairball tools available via MCP callback)."
        )
        msg_lines.append(
        ...
        msg_lines.append("ChatGPT turns will use the default Hairball runtime.")
