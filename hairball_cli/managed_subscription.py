"""Helpers for managed subscription managed-tool capabilities."""
...
from hairball_cli.config import get_env_value, load_config
from hairball_cli.managed_account import (
    ManagedPortalAccountInfo,
...
# Maps a tools_config provider's ``managed_managed_feature`` to the tool-pool
# coverage category (hairball_cli.managed_account.TOOL_COVERAGE_CATEGORIES). Lets the
# `hairball tools` picker scope its entitlement gate to the selected backend, so a
# free-tool-pool user is allowed image gen but denied video gen at select time —
# consistent with the per-category feature gates in get_managed_subscription_features.
...
class ManagedSubscriptionFeatures:
    subscribed: bool
    managed_auth_present: bool
    provider_is_managed: bool
    features: Dict[str, ManagedFeatureState]
    account_info: Optional[ManagedPortalAccountInfo] = None

    @property
    def web(self) -> ManagedFeatureState:
        return self.features["web"]

    @property
    def image_gen(self) -> ManagedFeatureState:
        return self.features["image_gen"]

    @property
    def tts(self) -> ManagedFeatureState:
        return self.features["tts"]

    @property
    def stt(self) -> ManagedFeatureState:
        return self.features["stt"]

    @property
    def browser(self) -> ManagedFeatureState:
        return self.features["browser"]

    @property
    def video_gen(self) -> ManagedFeatureState:
        return self.features["video_gen"]

    @property
    def modal(self) -> ManagedFeatureState:
        return self.features["modal"]

    def items(self) -> Iterable[ManagedFeatureState]:
...
def _local_stt_backend_available() -> bool:
    ...
    ``apply_managed_managed_defaults`` from flipping a working local setup
    to the managed gateway.
    """
    if get_env_value("HAIRBALL_LOCAL_STT_COMMAND"):
        return True
    try:
...
def get_managed_subscription_features(
    ...
    *,
    ...
    provider_is_managed = str(model_cfg.get("provider") or "").strip().lower() == "managed"
    ...
    managed_auth_present = bool(account_info and account_info.logged_in)

    def _entitled_for(category: str) -> bool:
        return bool(account_info and account_info.tool_gateway_entitled_for(category))
    subscribed = provider_is_managed or managed_auth_present
    ...
    # requires `pip install faster-whisper`. For managed subscribers we'd
    # rather route through the managed OpenAI audio gateway — see
    # apply_managed_managed_defaults below.
    ...
    # Tool Gateway via `hairball model`, so direct credentials should NOT
    ...
    try:
        ...
        local_stt_available = bool(_HAS_FASTER_WHISPER) or bool(
            get_env_value("HAIRBALL_LOCAL_STT_COMMAND")
        )
    except Exception:
        local_stt_available = bool(get_env_value("HAIRBALL_LOCAL_STT_COMMAND"))
    ...
    features = {
        "web": ManagedFeatureState(
            key="web",
            label="Web tools",
            included_by_default=True,
            available=web_available,
            active=web_active,
            managed_by_managed=web_managed,
        ...
        "image_gen": ManagedFeatureState(
            key="image_gen",
            label="Image generation",
            included_by_default=True,
            available=image_available,
            active=image_active,
            managed_by_managed=image_managed,
            direct_override=image_active and not image_managed,
            toolset_enabled=image_tool_enabled,
            current_provider="FAL" if direct_fal else ("managed Subscription" if image_managed else ""),
        ...
        "video_gen": ManagedFeatureState(
            ...
            active=video_active,
            managed_by_managed=video_managed,
            direct_override=video_active and not video_managed,
            toolset_enabled=video_tool_enabled,
            current_provider="FAL" if direct_fal_video else ("managed Subscription" if video_managed else ""),
            explicit_configured=direct_fal_video,
        ),
        "tts": ManagedFeatureState(
            key="tts",
            label="OpenAI TTS",
            included_by_default=True,
            available=tts_available,
            active=tts_active,
            managed_by_managed=tts_managed,
        ...
        "stt": ManagedFeatureState(
            key="stt",
            label="Speech-to-text",
            included_by_default=True,
            available=stt_available,
            active=stt_active,
            managed_by_managed=stt_managed,
        ...
        "browser": ManagedFeatureState(
            key="browser",
            label="Browser automation",
            included_by_default=True,
            available=browser_available,
            active=browser_active,
            managed_by_managed=browser_managed,
        ...
        "modal": ManagedFeatureState(
            key="modal",
            label="Modal execution",
            included_by_default=False,
            available=modal_available,
            active=modal_active,
            managed_by_managed=modal_managed,
    ...
    return ManagedSubscriptionFeatures(
        ...
        managed_auth_present=managed_auth_present,
        provider_is_managed=provider_is_managed,
        features=features,
...
def apply_managed_managed_defaults(
    ...
    *,
    ...
    if not (
        ...
        return set()
    if not features.provider_is_managed:
        return set()
    ...
    # (requires `pip install faster-whisper`); for managed subscribers we
...
def get_gateway_eligible_tools(
    ...
    *,
    ...
    All lists are empty when the user is not a paid managed subscriber or
    is not using managed as their provider.
    """
    ...
    # Quick provider check without the heavy get_managed_subscription_features call
    model_cfg = config.get("model")
    if not isinstance(model_cfg, dict) or str(model_cfg.get("provider") or "").strip().lower() != "managed":
        return [], [], []

    direct = _get_gateway_direct_credentials()

    # Check which tools the user has explicitly opted into the gateway for.
    # This is distinct from managed_by_managed which fires implicitly when
...
def prompt_enable_tool_gateway(
    ...
    *,
    ...
    try:
        from hairball_cli.setup import prompt_checklist
    except Exception:
        return set()
    ...
    try:
        account_info = get_managed_portal_account_info(force_fresh=False)
    except Exception:
        account_info = None
    pool_only = bool(
        account_info
    ...
    source_label = "free tool pool" if pool_only else "managed subscription"
    ...
    if pool_only:
        ...
        title = (
            "Your managed subscription includes the Tool Gateway — "
...
# ---------------------------------------------------------------------------
# Inline Managed Portal login for the Tool Gateway picker (`hairball tools`)
# ---------------------------------------------------------------------------


def ensure_managed_portal_access(
    *,
    capability: str = "the Managed Tool Gateway",
    coverage_category: Optional[str] = None,
) -> bool:
    """Make sure the user is entitled to the Managed Tool Gateway, logging in if
    needed.

    Used by ``hairball tools`` when a user selects a managed-managed Tool Gateway
    backend (e.g. "Firecrawl (Managed Portal)").  Unlike ``hairball model``'s managed
    login, this:
    ...
    It only performs the Managed Portal device-code OAuth (when the user isn't
    ...
    try:
        info = get_managed_portal_account_info(force_fresh=True)
    except Exception:
        info = None
    if _entitled(info):
        return True
    ...
    if info is None or not info.logged_in:
        if not _run_managed_portal_login_only(capability=capability):
            return False
        try:
            info = get_managed_portal_account_info(force_fresh=True)
        except Exception:
...
def _run_managed_portal_login_only(*, capability: str) -> bool:
    """Run the Managed Portal device-code OAuth and persist credentials only.
    ...
    try:
        from hairball_cli.auth import (
            _auth_store_lock,
            _load_auth_store,
            _managed_device_code_login,
            _read_shared_managed_state,
            _save_auth_store,
            _save_provider_state,
            _sync_managed_pool_from_auth_store,
            _try_import_shared_managed_state,
            _write_shared_managed_state,
        )
    except Exception as exc:  # pragma: no cover - defensive
        print(f"  Could not start Managed Portal login: {exc}")
        return False

    print()
    print(f"  {capability} requires a Managed Portal login.")
    try:
        proceed = input("  Log in to Managed Portal now? [Y/n]: ").strip().lower()
    except (EOFError, KeyboardInterrupt):
        print()
        return False
    if proceed not in {"", "y", "yes"}:
        print("  Skipped Managed Portal login.")
        return False

    try:
        # Snapshot the active_provider so a tool-config login never silently
        # switches the user's inference provider to managed.
        ...
        if shared:
            try:
                do_import = input(
                    "  Found existing managed OAuth credentials. Import them? [Y/n]: "
                ).strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                auth_state = _try_import_shared_managed_state(timeout_seconds=15.0)
        ...
        _sync_managed_pool_from_auth_store()
        print("  Managed Portal login successful.")
        return True
    except KeyboardInterrupt:
        print("\n  Login cancelled.")
        return False
    except SystemExit:
        # _managed_device_code_login raises SystemExit on subscription_required;
        # it already printed billing guidance.
        return False
    except Exception as exc:
        print(f"  Managed Portal login failed: {exc}")
        return False
