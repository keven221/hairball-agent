...
``<root>`` is the **shared Hairball root** (the parent of any active
...
``hairball -p <profile>`` joins the same board as the dispatcher that
...
the profile's Hairball home but are otherwise isolated: a worker spawned
...
* ``board=`` argument passed directly to :func:`connect` / :func:`init_db`
  ...
  ``?board=...`` query param).
* ``HAIRBALL_KANBAN_BOARD`` env var (used by the dispatcher to pin workers
  to the board their task lives on — workers cannot see other boards).
* ``HAIRBALL_KANBAN_DB`` env var (pins the DB file path directly — legacy
...
* ``<root>/kanban/current`` — a one-line text file holding the slug of
  the "currently selected" board. Written by ``hairball kanban boards
  switch <slug>``. When absent, the active board is ``default``.

In standard installs ``<root>`` is ``~/.hairball``. In Docker / custom
deployments where ``HAIRBALL_HOME`` points outside ``~/.hairball`` (e.g.
``/opt/hairball``), ``<root>`` is ``HAIRBALL_HOME``. Legacy env-var
overrides still work:

* ``HAIRBALL_KANBAN_DB`` — pin the database file path directly.
* ``HAIRBALL_KANBAN_WORKSPACES_ROOT`` — pin the workspaces root directly.
* ``HAIRBALL_KANBAN_HOME`` — pin the umbrella root that anchors kanban
  paths. Useful for tests and unusual deployments.

The dispatcher injects ``HAIRBALL_KANBAN_DB``,
``HAIRBALL_KANBAN_WORKSPACES_ROOT``, and ``HAIRBALL_KANBAN_BOARD`` into
...
coding workloads.  See ``docs/hairball-kanban-v1-spec.pdf`` for the full
...
from hairball_cli.sqlite_util import add_column_if_missing as _add_column_if_missing
...
def _fire_kanban_lifecycle_hook(event: str, task_id: str, **fields: Any) -> None:
    ...
    ``profile_name`` is resolved from the active HAIRBALL_HOME so dispatcher- and
...
# ``HAIRBALL_KANBAN_CLAIM_TTL_SECONDS`` to raise the default claim window for
...
def _resolve_claim_ttl_seconds(ttl_seconds: Optional[int] = None) -> int:
    ...
    ``HAIRBALL_KANBAN_CLAIM_TTL_SECONDS`` overrides the built-in default.
    ...
    raw = os.environ.get("HAIRBALL_KANBAN_CLAIM_TTL_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_crash_grace_seconds() -> int:
    ...
    Reads ``HAIRBALL_KANBAN_CRASH_GRACE_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get("HAIRBALL_KANBAN_CRASH_GRACE_SECONDS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _resolve_rate_limit_cooldown_seconds() -> int:
    ...
    Reads ``HAIRBALL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS`` from the environment;
    ...
    """
    raw = os.environ.get(
        "HAIRBALL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS", ""
    ).strip()
    if raw:
        try:
            parsed = int(raw)
...
_CURRENT_BOARD_OVERRIDE: ContextVar[str | None] = ContextVar(
    "hairball_kanban_current_board_override",
    default=None,
)
...
# enough that kebab-case names like ``atm10-server`` or ``hairball-agent``
...
def kanban_home() -> Path:
    """Return the shared Hairball root that anchors the kanban board.

    Resolution order:

    1. ``HAIRBALL_KANBAN_HOME`` env var when set and non-empty (explicit
       override for tests and unusual deployments).
    2. ``get_default_hairball_root()``, which already returns ``<root>``
       when ``HAIRBALL_HOME`` is ``<root>/profiles/<name>``, and returns
       ``HAIRBALL_HOME`` directly for Docker / custom deployments.

    The kanban board is shared across profiles **by design** (see the
    module docstring). Resolving the kanban paths through the active
    profile's ``HAIRBALL_HOME`` would silently fork the board per profile,
    which breaks the dispatcher / worker handoff.
    """
    override = os.environ.get("HAIRBALL_KANBAN_HOME", "").strip()
    if override:
        return Path(override).expanduser()
    from hairball_constants import get_default_hairball_root
    return get_default_hairball_root()
...
def current_board_path() -> Path:
    ...
    One-line text file written by ``hairball kanban boards switch <slug>``
...
def get_current_board() -> str:
    ...
    1. ``HAIRBALL_KANBAN_BOARD`` env var (set by the dispatcher on worker
       spawn, or manually for ad-hoc overrides).
    2. ``<root>/kanban/current`` on disk (set by ``hairball kanban boards
    ...
    env = os.environ.get("HAIRBALL_KANBAN_BOARD", "").strip()
    if env:
        try:
...
def set_current_board(slug: str) -> Path:
    ...
    so that ``hairball kanban boards switch <typo>`` returns an error
...
def kanban_db_path(board: Optional[str] = None) -> Path:
    ...
    1. ``HAIRBALL_KANBAN_DB`` env var — pins the path directly. Honoured for
    ...
    """
    override = os.environ.get("HAIRBALL_KANBAN_DB", "").strip()
    if override:
        return Path(override).expanduser()
...
def workspaces_root(board: Optional[str] = None) -> Path:
    ...
    ``HAIRBALL_KANBAN_WORKSPACES_ROOT`` pins the path directly (highest
    ...
    """
    override = os.environ.get("HAIRBALL_KANBAN_WORKSPACES_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def attachments_root(board: Optional[str] = None) -> Path:
    ...
    ``HAIRBALL_KANBAN_ATTACHMENTS_ROOT`` pins the path directly (highest
    ...
    see the kanban docs.
    """
    override = os.environ.get("HAIRBALL_KANBAN_ATTACHMENTS_ROOT", "").strip()
    if override:
        return Path(override).expanduser()
    slug = _normalize_board_slug(board)
    if slug is None:
...
def worker_logs_dir(board: Optional[str] = None) -> Path:
    ...
    board — makes ``hairball kanban log`` unambiguous even when multiple
...
class Task:
    ...
    # within an agent loop that propagated ``HAIRBALL_SESSION_ID``. NULL for
...
CREATE TABLE IF NOT EXISTS tasks (
    ...
    workspace_path       TEXT,
    branch_name          TEXT,
    -- Optional link to a first-class Project (hairball_cli/projects_db). When set,
    ...
    -- Originating chat/agent session id when the task was created from
    -- inside an agent loop that propagated ``HAIRBALL_SESSION_ID``. NULL
...
def _resolve_busy_timeout_ms() -> int:
    ...
    """
    raw = os.environ.get("HAIRBALL_KANBAN_BUSY_TIMEOUT_MS", "").strip()
    if raw:
        try:
            parsed = int(raw)
        except ValueError:
...
def _dispatch_tick_lock(db_path: Path):
    ...
    Motivation (issue #35240): a ``hairball gateway run --replace`` /
...
def _guard_existing_db_is_healthy(path: Path) -> None:
    ...
    all sources Hairball treats as user-controlled-but-trusted on the
...
def connect(
    ...
    * Neither → :func:`kanban_db_path` resolves via
      ``HAIRBALL_KANBAN_DB`` env → ``HAIRBALL_KANBAN_BOARD`` env →
    ...
    # holder (e.g. an external `hairball kanban list` mid-integrity-probe) block
    ...
    with _cross_process_init_lock(path):
        ...
        try:
            ...
            with _INIT_LOCK:
                ...
                # See hairball_state._WAL_INCOMPAT_MARKERS for detection logic.
                from hairball_state import apply_wal_with_fallback
                journal_mode = apply_wal_with_fallback(
                    conn, db_label=f"kanban.db ({path.name})"
                )
...
def init_db(
    ...
    *,
    ...
    Kept as a public entry point so CLI ``hairball kanban init`` and the
...
def _migrate_add_optional_columns(conn: sqlite3.Connection) -> None:
    ...
    if "session_id" not in cols:
        ...
        # ``HAIRBALL_SESSION_ID`` (e.g. ACP). NULL on legacy rows and on any
...
def write_txn(conn: sqlite3.Connection):
    ...
    Do not inspect the live database file with Python ``open()`` after commit.
    On POSIX, closing an application-owned descriptor for a file SQLite has
    locked can release process-level advisory locks behind SQLite's back and
    allow concurrent writers to corrupt the database. Integrity checks must go
    through SQLite itself (the first-connect guard does this).
    """
...
def _canonical_assignee(assignee: Optional[str]) -> Optional[str]:
    ...
    if assignee is None:
        return None
    from hairball_cli.profiles import normalize_profile_name
...
def create_task(
    ...
    *,
    ...
    each name to ``hairball --skills ...``. Use this to pin a task to a
    ...
    if project_id:
        try:
            from hairball_cli import projects_db as _pdb
    ...
    # `hairball --skills X,Y` comma syntax is handled in the dispatcher,
    # not here.
...
# Canonical sort-order mappings for ``hairball kanban list --sort``.
...
def unlink_tasks(conn: sqlite3.Connection, parent_id: str, child_id: str) -> bool:
    ...
    if removed:
        ...
        # next dispatcher tick or a manual `hairball kanban recompute` (issue #22459).
...
def _end_run(
    ...
    *,
    ...
    existed (e.g. a CLI user calling ``hairball kanban complete`` on a
...
def _synthesize_ended_run(
    ...
    *,
    ...
    claimed (CLI user calling ``hairball kanban complete <ready-task>
...
def _has_sticky_block(conn: sqlite3.Connection, task_id: str) -> bool:
    ...
    * **Worker- or operator-initiated** — a worker called
      ...
      ``hairball kanban block <id>``).  This is a deliberate handoff that
...
def complete_task(
    ...
    *,
    ...
    completion (``hairball kanban complete <id>``) works without requiring
    ...
    and never blocks.
    """
    # Shared persistence waist for model tools, CLI, swarm, and dashboard.
    # Redact here so no entry surface can bypass the handoff contract. The
    # helper returns copies, preserving caller inputs and artifact handling.
    result = redact_sensitive_value(result, force=True)
    summary = redact_sensitive_value(summary, force=True)
    metadata = redact_sensitive_value(metadata, force=True)
    now = int(time.time())
...
def _is_managed_scratch_path(p: Path) -> bool:
    ...
    * ``HAIRBALL_KANBAN_WORKSPACES_ROOT`` when set (worker-side override
    ...
    * ``<kanban_home>/kanban/boards/<slug>/workspaces`` for each board slug
    ...
    subtrees hold Hairball' own DB, metadata, and logs, not task workspaces.

    Used by :func:`_cleanup_workspace` to refuse to ``shutil.rmtree`` paths
    outside Hairball-managed storage. A board ``default_workdir`` pointing at a
...
#      via ``hairball kanban show <id>`` and the dashboard.
...
def _resolve_worktree_workspace(
    ...
    launched from, e.g. the Hairball checkout). If no anchor is configured
...
def resolve_workspace(task: Task, *, board: Optional[str] = None) -> Path:
    ...
    - ``worktree``: a real linked git worktree. If ``workspace_path`` names
      a repo root, Hairball treats it as an anchor and materializes a linked
      worktree at ``<repo>/.worktrees/<task-id>``. If ``workspace_path`` names
      a concrete target path, Hairball creates/reuses that linked worktree. With
      no ``workspace_path``, Hairball anchors on the board's ``default_workdir``
      ...
      dispatcher's CWD. When ``branch_name`` is empty, Hairball uses
      ``wt/<task-id>``.
...
# without thrashing. Overridable via ``HAIRBALL_KANBAN_RATE_LIMIT_COOLDOWN_SECONDS``
...
class DispatchResult:
    ...
    lane (a Claude Code terminal like ``orion-cc``) rather than a Hairball
...
def _defer_reclaim_for_live_worker(
    ...
    *,
    ...
    event so the hold is visible in ``hairball kanban tail``. The next dispatch
...
def _set_worker_pid(conn: sqlite3.Connection, task_id: str, pid: int) -> None:
    ...
    The event's payload carries the pid so a human reading ``hairball kanban
...
def has_spawnable_ready(conn: sqlite3.Connection) -> bool:
    ...
    whose assignee maps to a real Hairball profile.
    ...
    if not rows:
        return False
    try:
        from hairball_cli.profiles import profile_exists  # local import: avoids cycle
    except Exception:
...
def _dispatch_once_locked(
    ...
    *,
    ...
    if _default_assignee:
        try:
            from hairball_cli.profiles import profile_exists as _pe
    ...
    for row in ready_rows:
        ...
        if not row_assignee:
            ...
            if _default_assignee and _default_assignee_resolved:
                ...
                result.skipped_unassigned.append(row["id"])
                continue
        # Skip ready tasks whose assignee is not a real Hairball profile.
        # `_default_spawn` invokes ``hairball -p <assignee>`` which fails
        # with "Profile 'X' does not exist" when the assignee names a
        # control-plane lane (e.g. an interactive Claude Code terminal
        # like ``orion-cc`` / ``orion-research``) rather than a Hairball
        ...
        if guard_reason is not None:
            ...
            # skipped when reading `hairball kanban tail` — without
    ...
    for row in review_rows:
        ...
        if not row["assignee"]:
            ...
            continue
        try:
            from hairball_cli.profiles import profile_exists
        except Exception:
            profile_exists = None  # type: ignore[assignment]
...
def _module_hairball_argv() -> list[str]:
    """Return the interpreter-bound Hairball CLI invocation."""
    # ``hairball_cli.main`` is the console-script target declared in
    # pyproject.toml, NOT a top-level ``hairball`` package — there is no
    # ``hairball`` package to import.
    return [sys.executable, "-m", "hairball_cli.main"]


def _absolute_hairball_path(path: str) -> str:
    """Return an absolute filesystem path for a resolved Hairball shim."""
...
def _hairball_path_argv(path: str) -> list[str]:
    """Return argv for a resolved Hairball executable path.
    ...
    if _IS_WINDOWS and _is_windows_batch_shim(path):
        return _module_hairball_argv()
    return [_absolute_hairball_path(path)]


def _resolve_hairball_argv() -> list[str]:
    """Resolve the ``hairball`` invocation as argv parts for ``Popen``.

    Tries in order:

    1. ``$HAIRBALL_BIN`` — explicit operator override. Path-like values are
    ...
    2. ``shutil.which("hairball")`` — the console-script shim, normalized to
       an absolute path. On Windows, ``which`` can return a relative
       ``.\\hairball.CMD`` when the current directory is on ``PATH``; directly
    ...
    3. ``sys.executable -m hairball_cli.main`` — fallback for setups where
       Hairball is launched from a venv and the ``hairball`` shim is not on
       the dispatcher's ``$PATH`` (cron, systemd ``User=`` services,
       launchd jobs, detached processes, etc.). Goes through the running
       interpreter so the result is independent of ``$PATH``.

    Mirrors ``gateway.run._resolve_hairball_bin`` for the same reason. Kept
    local (not imported from gateway) because ``hairball_cli`` sits below
    ``gateway`` in the dependency order.
    """
    import shutil

    env_bin = os.environ.get("HAIRBALL_BIN", "").strip()
    if env_bin:
        if _looks_like_path(env_bin):
            return _hairball_path_argv(env_bin)
        resolved_env_bin = _safe_which_no_cwd(env_bin)
        if resolved_env_bin:
            return _hairball_path_argv(resolved_env_bin)
        return _module_hairball_argv()

    hairball_bin = _safe_which_no_cwd("hairball") if _IS_WINDOWS else shutil.which("hairball")
    if hairball_bin:
        return _hairball_path_argv(hairball_bin)
    return _module_hairball_argv()
...
def _resolve_worker_cli_toolsets(hairball_home: Optional[str]) -> Optional[list[str]]:
    ...
    task-scoped kanban lifecycle tools when ``HAIRBALL_KANBAN_TASK`` is set.
    """
    ...
    try:
        ...
        token = set_hairball_home_override(hairball_home)
        try:
            cfg = load_config()
        ...
        _log.debug(
            "kanban worker: could not resolve CLI toolsets for HAIRBALL_HOME=%r (%s)",
            hairball_home,
            exc,
        )
        return None
...
def _default_spawn(
    ...
    *,
    ...
    """Fire-and-forget ``hairball -p <profile> chat -q ...`` subprocess.
    ...
    ``HAIRBALL_KANBAN_DB`` / ``HAIRBALL_KANBAN_BOARD`` / workspaces_root env
    ...
    # Inject HAIRBALL_HOME so the worker reads the profile-scoped config.yaml
    # (fallback_providers, toolsets, agent settings, etc.) instead of the root
    # config.  Without this, `env = dict(os.environ)` copies only the parent's
    # env, and when the child process starts `hairball -p <name>` the
    # _apply_profile_override() runs *before* hairball_constants is imported.
    # If HAIRBALL_HOME is absent from the child's env, get_hairball_home() falls
    # back to Path.home() / ".hairball" (the DEFAULT profile root), ignoring the
    # profile-specific config entirely.  Fixes profile-scoped fallback_providers
    # being invisible to kanban workers.
    from hairball_cli.profiles import resolve_profile_env
    try:
        env["HAIRBALL_HOME"] = resolve_profile_env(profile_arg)
    except FileNotFoundError:
        # Profile dir doesn't exist — defer resolution to the CLI's
        # _apply_profile_override() via HAIRBALL_PROFILE (set below).
        # This only happens in test fixtures where the isolated
        # HAIRBALL_HOME never had profiles created.
        pass
    if task.tenant:
        env["HAIRBALL_TENANT"] = task.tenant
    env["HAIRBALL_KANBAN_TASK"] = task.id
    env["HAIRBALL_KANBAN_WORKSPACE"] = workspace
    ...
    if task.branch_name:
        env["HAIRBALL_KANBAN_BRANCH"] = task.branch_name
    if task.current_run_id is not None:
        env["HAIRBALL_KANBAN_RUN_ID"] = str(task.current_run_id)
    if task.claim_lock:
        env["HAIRBALL_KANBAN_CLAIM_LOCK"] = task.claim_lock
    # Goal-loop mode: the worker reads these and wraps its run in the
    # Ralph-style /goal judge loop (see cli.py quiet-mode path). Only set
    # when enabled so non-goal tasks keep a clean env.
    if task.goal_mode:
        env["HAIRBALL_KANBAN_GOAL_MODE"] = "1"
        if task.goal_max_turns is not None:
            env["HAIRBALL_KANBAN_GOAL_MAX_TURNS"] = str(int(task.goal_max_turns))
    ...
    # that even when the worker activates a profile (`hairball -p <name>`
    # rewrites HAIRBALL_HOME), its kanban paths still match the
    # dispatcher's. Belt-and-braces with the `get_default_hairball_root()`
    # resolution in `kanban_home()` — symmetric resolution is the norm,
    # but unusual symlink / Docker layouts are caught here too.
    env["HAIRBALL_KANBAN_DB"] = str(kanban_db_path(board=board))
    env["HAIRBALL_KANBAN_WORKSPACES_ROOT"] = str(workspaces_root(board=board))
    # Board slug — the final defense-in-depth pin. If the worker ever
    # resolves kanban paths without the DB / workspaces env vars, the
    # board slug still forces it to the right directory.
    resolved_board = _normalize_board_slug(board) or get_current_board()
    env["HAIRBALL_KANBAN_BOARD"] = resolved_board
    # HAIRBALL_PROFILE is the author the kanban_comment tool defaults to.
    # `hairball -p <assignee>` activates the profile, but the env var is
    # what the tool reads — set it explicitly here so comments are
    # attributed correctly regardless of how the child loads config.
    env["HAIRBALL_PROFILE"] = profile_arg

    # A worker must NEVER boot the interactive TUI: an inherited HAIRBALL_TUI=1
    ...
    # older hairball builds on PATH that predate the flag's precedence.
    env.pop("HAIRBALL_TUI", None)

    cmd = [
        *_resolve_hairball_argv(),
        "-p", profile_arg,
        "--cli",
        # Worker subprocesses switch to a profile-scoped HAIRBALL_HOME above,
    ...
    worker_toolsets = _resolve_worker_cli_toolsets(env.get("HAIRBALL_HOME"))
    if worker_toolsets:
    ...
    # `hairball kanban log` on a specific board reads its own file and
    ...
    try:
        ...
        log_f.close()
        raise RuntimeError(
            "`hairball` executable not found on PATH. "
            "Install Hairball Agent or activate its venv before running the kanban dispatcher."
        )
...
def run_daemon(
    *,
    ...
    on SIGINT / SIGTERM so ``hairball kanban daemon`` is systemd-friendly.
...
def build_worker_context(conn: sqlite3.Connection, task_id: str) -> str:
    ...
    if shown_c:
        ...
        for c in shown_c:
            ...
            # operator-controlled HAIRBALL_PROFILE values like "hairball-system"
...
def list_profiles_on_disk() -> list[str]:
    ...
    - the implicit ``default`` profile when the default Hairball root exists

    Reads profile paths directly so this module has no import dependency on
    ``hairball_cli.profiles`` (which pulls in a large chunk of the CLI startup
    path).
    """
    try:
        from hairball_constants import get_default_hairball_root
...
def known_assignees(conn: sqlite3.Connection) -> list[dict]:
    ...
    - ``hairball kanban assignees`` for the terminal.
