"""MCP picker — interactive `hairball mcp picker` (also the default `hairball mcp`).

Lists every catalog entry plus any custom MCP servers the user has added via
``hairball mcp add``, lets them pick one, and routes to install / enable /
disable / uninstall / configure-tools flows.

Mirrors the `hairball plugin` picker UX: arrow keys to navigate, ENTER on a row
...
from hairball_cli.cli_output import prompt_yes_no
from hairball_cli.curses_ui import curses_single_select
from hairball_cli.mcp_catalog import (
    CatalogEntry,
    CatalogError,
    catalog_diagnostics,
    install_entry,
...
def _enable_disable(name: str, *, enable: bool) -> None:
    ...
    print(color(
        ...
        "Start a new Hairball session for changes to take effect.",
        Colors.GREEN,
    ))
...
def _configure_tools(name: str) -> None:
    ...
    """
    import argparse
    from hairball_cli.mcp_config import cmd_mcp_configure
...
def _print_rows_text(rows: List[_Row]) -> None:
    ...
    as the default output of `hairball mcp catalog`."""
    if not rows:
        print()
    ...
    print()
    print(color(
        "  Install: hairball mcp install <name>    Picker: hairball mcp",
        Colors.DIM,
    ))

    # Surface manifest-version warnings so users know when their Hairball is
    ...
    if future:
        ...
        for name, _, msg in future:
            print(color(
                f"  ⚠ '{name}' requires a newer Hairball — run `hairball update` "
...
def show_catalog() -> None:
    """`hairball mcp catalog` — print the curated list + custom servers, no interaction."""
    _print_rows_text(_build_rows())


def run_picker() -> None:
    """`hairball mcp picker` (and default `hairball mcp`) — interactive selector.
...
def install_by_name(identifier: str) -> int:
    """`hairball mcp install <name>` — non-interactive entry-point.

    Returns 0 on success, non-zero on failure (so the CLI can propagate
    exit codes).
    """
    from hairball_cli.mcp_catalog import get_entry

    entry = get_entry(identifier)
    if entry is None:
        print(color(
            f"  ✗ '{identifier}' is not in the catalog. "
            "Run `hairball mcp catalog` to see available entries.",
            Colors.RED,
        ))
        return 1
    try:
