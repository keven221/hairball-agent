...
Used by ``hairball profile describe <name> --auto`` and the dashboard's
...
# Only provider wires that require an explicit max_tokens value receive this
# through ``call_llm``.  Other providers use their model-native output budget.
PROFILE_DESCRIBER_MAX_TOKENS = 8192
...
_SYSTEM_PROMPT = """You are a profile-describer for the Hairball Agent kanban board.
...
Rules:
  ...
  - Never write "Hairball Agent profile" or other meta-narration.
    ...
    *,
    ...
    try:
        if canon == "default":
            ...
            profile_dir = Path(get_hairball_home())
        else:
    ...
    try:
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=PROFILE_DESCRIBER_MAX_TOKENS,
            timeout=timeout,
        )
    except Exception as exc:
    ...
    if parsed is None and finish_reason == "length":
        return DescribeOutcome(
            canon,
            False,
            "LLM exhausted its output budget before producing a profile description",
        )
    if parsed is None:
