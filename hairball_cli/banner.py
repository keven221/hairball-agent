...
Pure display functions with no HairballCLI state dependency.
"""
import json
import logging
import os
...
from hairball_cli import __version__ as VERSION, __release_date__ as RELEASE_DATE
...
# (e.g. nix-built hairball — no local git history to count against).
UPDATE_AVAILABLE_NO_COUNT = -1

_DEFAULT_UPDATE_SOURCE = resolve_update_source(DEFAULT_UPDATE_REPOSITORY)
_UPSTREAM_REPO_URL = _DEFAULT_UPDATE_SOURCE.https_git_url
_OFFICIAL_REPO_CANONICAL = _DEFAULT_UPDATE_SOURCE.canonical_remote


def _canonical_github_remote(url: str | None) -> str:
    """Back-compatible wrapper around the shared update-source normalizer."""
    return canonical_github_remote(url)
...
def _is_official_ssh_remote(url: str | None) -> bool:
    return _is_ssh_remote(url) and resolve_update_source().is_official_remote(url)
    ...
    try:
        result = subprocess.run(
            [
                "git",
                "ls-remote",
                resolve_update_source().https_git_url,
                "refs/heads/main",
            ],
...
def _fetch_pypi_latest(package: str = "hairball-agent") -> Optional[str]:
...
def check_for_updates() -> Optional[int]:
    """Check whether a Hairball update is available.

    Two paths: if ``HAIRBALL_REVISION`` is set (nix builds embed it), compare
    ...
    cache_file = hairball_home / ".update_check"
    embedded_rev = os.environ.get("HAIRBALL_REVISION") or None

    # Docker images have no working tree to count commits against — the
    # published image excludes `.git` (see .dockerignore) and sets no
    # HAIRBALL_REVISION (that's nix-only). Without this guard the checks below
    ...
    # and `hairball update` correctly refuses to run in-place inside the
    # container anyway. The dashboard's REST `/api/hairball/update/check`
    ...
    try:
        from hairball_cli.config import detect_install_method, get_project_root
    ...
    if embedded_rev:
        ...
        # $HAIRBALL_HOME/hairball-agent/ may be a stale copy from --clone-all;
        # Path(__file__) always resolves to the actual installed checkout.
        repo_dir = Path(__file__).parent.parent.resolve()
        if not (repo_dir / ".git").exists():
            repo_dir = hairball_home / "hairball-agent"
        if not (repo_dir / ".git").exists():
...
def _resolve_repo_dir() -> Optional[Path]:
    """Return the active Hairball git checkout, or None if this isn't a git install.

    Prefers the running code's location over the profile-scoped path
    because ``$HAIRBALL_HOME/hairball-agent/`` may be a stale copy carried
    over by ``--clone-all``.
    """
    repo_dir = Path(__file__).parent.parent.resolve()
    if not (repo_dir / ".git").exists():
...
def get_git_banner_state(repo_dir: Optional[Path] = None) -> Optional[dict]:
    ...
    ``hairball_cli/build_info.py``) and return it as a frozen
    ...
    if repo_dir is None:
        ...
        try:
            from hairball_cli.build_info import get_build_sha
...
def get_latest_release_tag(repo_dir: Optional[Path] = None) -> Optional[tuple]:
    ...
    Hairball checkout. Cached per-process. Release URL always points at the
    configured Hairball source repository (forks don't get a link).
    """
    ...
    url = resolve_update_source().release_url(tag)
...
def format_banner_version_label() -> str:
    ...
    base = f"Hairball Agent v{VERSION} ({RELEASE_DATE})"
...
def build_welcome_banner(console: "Console", model: str, cwd: str,
                         ...
                         provider: str = None):
    """Build and print a welcome banner with the Hairball cat on the left.
    ...
    # A custom skin still wins; the cat is only the Hairball default.
    try:
        from hairball_cli.skin_engine import get_active_skin
        _bskin = get_active_skin()
        _hero = _bskin.banner_hero if hasattr(_bskin, 'banner_hero') and _bskin.banner_hero else HAIRBALL_CAT_HERO
    except Exception:
        _bskin = None
        _hero = HAIRBALL_CAT_HERO
    left_lines = ["", _hero, ""]
    if (provider or "").strip().lower() == "moa":
        ...
        left_lines.append(f"[{accent}]MoA: {preset_name}[/]{agg_str}{ctx_str} [dim {dim}]·[/] [dim {dim}]Hairball[/]")
    else:
        ...
        ctx_str = f" [dim {dim}]·[/] [dim {dim}]{_format_context_length(context_length)} context[/]" if context_length else ""
        left_lines.append(f"[{accent}]{model_short}[/]{ctx_str} [dim {dim}]·[/] [dim {dim}]Hairball[/]")

    if os.getenv("HAIRBALL_YOLO_MODE"):
    ...
    try:
        from hairball_cli.codex_runtime_switch import get_current_runtime
    ...
    try:
        ...
        if behind is not None and behind != 0:
            from hairball_cli.config import get_managed_update_command, recommended_update_command
            if behind > 0:
                ...
                # UPDATE_AVAILABLE_NO_COUNT: nix-built hairball; we know an update
    ...
    if term_width >= 95:
        _logo = _bskin.banner_logo if _bskin and hasattr(_bskin, 'banner_logo') and _bskin.banner_logo else HAIRBALL_AGENT_LOGO
        console.print(_logo)
        console.print()
