...
- ``hairball_cli.providers``        -- canonical provider identity + overlays
- ``hairball_cli.model_normalize``  -- per-provider name formatting
...
_HAIRBALL_MODEL_WARNING = (
    "Hairball 3 & 4 models are NOT agentic and are not designed "
    "for use with Hairball Agent. They lack the tool-calling capabilities "
    "required for agent workflows. Consider using an agentic model instead "
    "(Claude, GPT, Gemini, DeepSeek, etc.)."
)

# Match only the real Hairball 3 / Hairball 4 chat families.
# The previous substring check (`"hairball" in name.lower()`) false-positived on
# unrelated local Modelfiles like ``hairball-brain:qwen3-14b-ctx16k`` that just
# happen to carry "hairball" in their tag but are fully tool-capable.
#
# Positive examples the regex must match:
#   managed/Hairball-3-Llama-3.1-70B, hairball-4-405b, openrouter/hairball3:70b
# Negative examples it must NOT match:
#   hairball-brain:qwen3-14b-ctx16k, qwen3:14b, claude-opus-4-6
_MANAGED_HAIRBALL_NON_AGENTIC_RE = re.compile(
    r"(?:^|[/:])hairball[-_ ]?[34](?:[-_.:]|$)",
    re.IGNORECASE,
)


def is_managed_hairball_non_agentic(model_name: str) -> bool:
    """Return True if *model_name* is a real managed Hairball 3/4 chat model.
    ...
    if not model_name:
        return False
    return bool(_MANAGED_HAIRBALL_NON_AGENTIC_RE.search(model_name))


def _check_hairball_model_warning(model_name: str) -> str:
    """Return a warning string if *model_name* is a managed Hairball 3/4 chat model."""
    if is_managed_hairball_non_agentic(model_name):
        return _HAIRBALL_MODEL_WARNING
    return ""
...
def _load_direct_aliases() -> dict[str, DirectAlias]:
    ...
    Also reads ``model.aliases`` (set by ``hairball config set model.aliases.xxx``)
...
def _ensure_direct_aliases() -> None:
    ...
    the module attribute. This keeps `from hairball_cli.model_switch import
...
def resolve_display_context_length(
    ...
    but provider-enforced limits can be lower (e.g. reference OAuth caps the
    same slug at 272k). The authoritative source is
    ``agent.model_metadata.get_model_context_length`` which already knows
    about reference OAuth, Copilot, managed, and falls back to models.dev for the
    rest.
...
def _configured_provider_matches(
    ...
    provider is ``openai-codex`` stays on reference and is soft-accepted as an
    unknown hidden reference model (#45006).
...
def switch_model(
    ...
    if explicit_provider:
        ...
        if pdef is None:
            _switch_err = (
                ...
                f"Check 'hairball model' for available providers, or define it "
            ...
            try:
                ...
                if _cfg_issues:
                    _switch_err += "\n\nRun 'hairball doctor' — config issues detected:"
        ...
        from hairball_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
        from hairball_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    ...
    # =================================================================
        ...
        try:
            from hairball_cli.config import load_config
            from hairball_cli.moa_config import exact_moa_preset_name, normalize_moa_config
    ...
    if provider_changed or explicit_provider:
        ...
        if explicit_provider and user_providers:
            from hairball_cli.providers import resolve_user_provider as _ruser
    ...
    # hairball_cli.runtime_provider.resolve_runtime_provider; without the strip,
    ...
    if target_provider in {"opencode-zen", "opencode-go"} and isinstance(base_url, str):
        from hairball_cli.models import normalize_opencode_base_url
    ...
    hairball_warn = _check_hairball_model_warning(new_model)
    if hairball_warn:
        warnings.append(hairball_warn)
...
def prewarm_picker_cache_async() -> Optional["_threading.Thread"]:
    ...
    def _warm() -> None:
        try:
            from hairball_cli.inventory import load_picker_context
...
def list_authenticated_providers(
    ...
    custom_providers: list | None = None,
    *,
    force_fresh_managed_tier: bool = False,
    ...
    Uses the curated model lists from hairball_cli/models.py (OPENROUTER_MODELS,
    ...
    ``force_fresh_managed_tier`` bypasses the short managed tier cache for explicit
    ...
    from hairball_cli.models import (
        ...
        clear_provider_models_cache, get_curated_managed_model_ids,
    )
    ...
    def _record_builtin_endpoint(slug: str) -> None:
        ...
        try:
            from hairball_cli.auth import PROVIDER_REGISTRY as _reg
        except Exception:
            return
        pcfg = _reg.get(slug)
        if not pcfg:
    ...
    # Build curated model lists keyed by hairball provider ID
    curated: dict[str, list[str]] = dict(_PROVIDER_MODELS)
    curated["openrouter"] = [mid for mid, _ in OPENROUTER_MODELS]
    # "managed" pulls from the remote model-catalog manifest published at
    # ./website/docs/api/model-catalog.json so
    # newly added Portal models surface in the /model picker without
    # requiring a Hairball release. Falls back to the in-repo
    ...
    if "ollama-cloud" not in curated:
        from hairball_cli.models import fetch_ollama_cloud_models
    ...
    if "lmstudio" not in curated and (
        ...
        from hairball_cli.models import fetch_lmstudio_models
    ...
    # --- 1. Check Hairball-mapped providers ---
    from hairball_cli.models import _AGGREGATOR_PROVIDERS as _AGG_PROVIDERS
    from hairball_cli.providers import ALIASES as _PROVIDER_ALIAS_TABLE
    for hairball_id, mdev_id in PROVIDER_TO_MODELS_DEV.items():
        ...
        _alias_target = _PROVIDER_ALIAS_TABLE.get(hairball_id)
        if (
            _alias_target
            and _alias_target != hairball_id
        ...
        pconfig = PROVIDER_REGISTRY.get(hairball_id)
        # Skip non-API-key auth providers here — they are handled in
        # section 2 (HAIRBALL_OVERLAYS) with proper auth store checking.
        if pconfig and pconfig.auth_type != "api_key":
            continue
        # models.dev catalogs include providers Hairball may not route yet.
        ...
        from hairball_cli.auth import is_runtime_provider_routable
        if not is_runtime_provider_routable(hairball_id):
            continue
        ...
        if not has_creds:
            try:
                from hairball_cli.auth import _load_auth_store
                store = _load_auth_store()
                raw_pool_present = bool(
                    store and store.get("credential_pool", {}).get(hairball_id)
                )
                if raw_pool_present:
                    has_creds = _credential_pool_is_usable(
                        hairball_id, raw_pool_present=True
                    )
            except Exception:
                pass
        if not has_creds:
        ...
        # /model picker sees the SAME list `hairball model` would build, with
        # disk caching to keep the picker open snappy. Falls back to the
        # curated static list when the live fetcher returns nothing.
        model_ids = cached_provider_model_ids(hairball_id)
        if not model_ids:
            model_ids = curated.get(hairball_id, [])
            if hairball_id in _MODELS_DEV_PREFERRED:
                model_ids = _merge_with_models_dev(hairball_id, model_ids)
    ...
    # --- 2. Check Hairball-only providers (managed, openai-codex, copilot, opencode-go) ---
    from hairball_cli.providers import HAIRBALL_OVERLAYS
    from hairball_cli.auth import PROVIDER_REGISTRY as _auth_registry

    # Build reverse mapping: models.dev ID → Hairball provider ID.
    # HAIRBALL_OVERLAYS keys may be models.dev IDs (e.g. "github-copilot")
    # while _PROVIDER_MODELS and config.yaml use Hairball IDs ("copilot").
    _mdev_to_hairball = {v: k for k, v in PROVIDER_TO_MODELS_DEV.items()}

    for pid, overlay in HAIRBALL_OVERLAYS.items():
        if pid.lower() in seen_slugs:
            continue

        # Resolve Hairball slug — e.g. "github-copilot" → "copilot"
        hairball_slug = _mdev_to_hairball.get(pid, pid)
        if hairball_slug.lower() in seen_slugs:
            continue

        # Check if credentials exist
        has_creds = False
        if overlay.auth_type == "aws_sdk":
            has_creds = _has_aws_sdk_creds_for_listing(hairball_slug)
        elif overlay.extra_env_vars:
            has_creds = any(os.environ.get(ev) for ev in overlay.extra_env_vars)
        # Also check api_key_env_vars from PROVIDER_REGISTRY for api_key auth_type
        if not has_creds and overlay.auth_type == "api_key":
            for _key in (pid, hairball_slug):
        ...
        if not has_creds:
            try:
                ...
                if store and (pid in providers_store or hairball_slug in providers_store):
                    has_creds = True
        ...
        # reference CLI ~/.codex/auth.json) which _seed_from_singletons()
        # imports on demand but aren't in the raw auth.json yet.
        if not has_creds:
            try:
                if _credential_pool_is_usable(hairball_slug):
                    has_creds = True
            except Exception as exc:
                logger.debug("Credential pool check failed for %s: %s", hairball_slug, exc)
        ...
        # configured.
        if not has_creds and hairball_slug == "anthropic":
            try:
                ...
                if (hairball_creds and hairball_creds.get("accessToken")) or \
        ...
        if hairball_slug in {"openai-codex", "copilot", "copilot-acp"}:
            # Use live OAuth-backed discovery so the gateway /model picker
            # matches what the user's authenticated reference/Copilot backend
            # actually serves — including ChatGPT-Pro-only reference slugs
            ...
            model_ids = cached_provider_model_ids(hairball_slug)
        # For aws_sdk providers (bedrock), use live discovery so the list
        # reflects the active region (eu.*, ap.*) not the static us.* list.
        elif overlay.auth_type == "aws_sdk":
            try:
                _ids = cached_provider_model_ids(hairball_slug)
                model_ids = _ids if _ids else (curated.get(hairball_slug, []) or curated.get(pid, []))
            except Exception:
                model_ids = curated.get(hairball_slug, []) or curated.get(pid, [])
        elif hairball_slug == "managed":
            # managed serves a large live /v1/models catalog (vendor-prefixed
            # models from many providers, returned alphabetically). The
            # `hairball model` picker deliberately shows ONLY the curated agentic
            ...
            try:
                ...
                from hairball_cli.auth import get_provider_auth_state as _managed_state
                ...
                try:
                    ...
                    _portal = ""
                if _managed_free(force_fresh=force_fresh_managed_tier):
            ...
            if not model_ids:
                model_ids = curated.get(hairball_slug, []) or curated.get(pid, [])
                if hairball_slug in _MODELS_DEV_PREFERRED:
                    model_ids = _merge_with_models_dev(hairball_slug, model_ids)
        total = len(model_ids)
        if hairball_slug in _UNCAPPED_PICKER_PROVIDERS:
            top = model_ids  # Aggregator: show full catalog regardless of max_models
        else:
            top = model_ids[:max_models] if max_models is not None else model_ids
        ...
        results.append({
            "slug": hairball_slug,
            "name": get_label(hairball_slug),
            "is_current": hairball_slug == current_provider or pid == current_provider,
            "is_user_defined": False,
            "models": top,
            "total_models": total,
            "source": "hairball",
        ...
        seen_slugs.add(hairball_slug.lower())
        _record_builtin_endpoint(hairball_slug)

    # --- 2b. Cross-check canonical provider list ---
    # Catches providers that are in CANONICAL_PROVIDERS but weren't found
    # in PROVIDER_TO_MODELS_DEV or HAIRBALL_OVERLAYS (keeps /model in sync
    # with `hairball model`).
    try:
        from hairball_cli.models import CANONICAL_PROVIDERS as _canon_provs
    except ImportError:
        _canon_provs = []
    ...
    if user_providers and isinstance(user_providers, dict):
        for ep_name, ep_cfg in user_providers.items():
            ...
            # ``base_url`` is Hairball's canonical write key (matches
            ...
            # Hairball writes ``models:`` as a dict keyed by model id, but older
            ...
            if should_probe:
                try:
                    from hairball_cli.models import fetch_api_models
    ...
    if custom_providers and isinstance(custom_providers, list):
        ...
        for entry in custom_providers:
            ...
            if group_key not in groups:
                ...
                # Hairball's own writer uses; a hyphen variant is accepted
            ...
            # active model. Hairball's own writer (main.py::_save_custom_provider)
            # stores every configured model as a dict under ``models:``;
            # downstream readers (agent/models_dev.py, gateway/run.py,
            # run_agent.py, hairball_cli/config.py) already consume that dict.
...
def _prepend_moa_picker_provider(providers: List[dict], current_provider: str = "") -> List[dict]:
    ...
    """
    try:
        from hairball_cli.inventory import _moa_provider_row
...
def list_picker_providers(
    ...
    - OpenRouter's model list is replaced with the output of
      :func:`hairball_cli.models.fetch_openrouter_models`, which filters the
    ...
    """
    from hairball_cli.models import fetch_openrouter_models
