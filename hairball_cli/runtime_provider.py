...
from hairball_cli import auth as auth_mod
...
from hairball_cli.auth import (
    ...
    _managed_inference_env_override,
    format_auth_error,
    resolve_provider,
...
def _config_base_url_trustworthy_for_bare_custom(cfg_base_url: str, cfg_provider: str) -> bool:
    ...
    try:
        from hairball_cli.auth import resolve_provider as _resolve_provider
...
def _copilot_runtime_api_mode(model_cfg: Dict[str, Any], api_key: str) -> str:
    ...
    try:
        from hairball_cli.models import copilot_model_api_mode
...
_VALID_API_MODES = {
    ...
    # so terminal/file-ops/patching/sandboxing run inside reference's own runtime
    # instead of Hairball' tool dispatch. Gated behind config key
...
def _managed_inference_base_url_override() -> str:
    """Return the trusted managed runtime base URL override, if configured.

    Delegates to ``auth._managed_inference_env_override`` so every
    ``MANAGED_INFERENCE_BASE_URL`` read shares one normalization path
    ...
    """
    return _managed_inference_env_override() or ""
...
def _maybe_apply_codex_app_server_runtime(
    *,
    ...
    """Optional opt-in: rewrite api_mode → "codex_app_server" for ChatGPT
...
def _resolve_runtime_from_pool_entry(
    *,
    ...
    if provider == "openai-codex":
        ...
        api_mode = "chat_completions"
        base_url = _managed_inference_base_url_override() or base_url
        ...
        if effective_model and api_mode != "anthropic_messages":
            try:
                from hairball_cli.models import azure_foundry_model_api_mode
        ...
        if provider in {"opencode-zen", "opencode-go"}:
            ...
            # Refs #16878.
            from hairball_cli.models import opencode_model_api_mode
    ...
    # Optional opt-in: route ChatGPT turns through `codex app-server`.
...
def resolve_requested_provider(requested: Optional[str] = None) -> str:
    ...
    env_provider = _getenv("HAIRBALL_INFERENCE_PROVIDER", "").strip().lower()
    if env_provider:
        return env_provider
...
def _get_named_custom_provider(requested_provider: str) -> Optional[Dict[str, Any]]:
    ...
    # the global default (reference) — the cause of cron jobs with
    ...
    if requested_norm != "custom" and not requested_norm.startswith("custom:"):
        try:
            ...
            # the built-in. See tests/hairball_cli/test_runtime_provider_resolution.py
    ...
    if isinstance(custom_providers, dict):
        logger.warning(
            ...
            "Run 'hairball doctor' for details."
        )
        return None
...
def canonical_custom_identity(
    *,
    ...
    2. ``config_provider`` — the active ``config.model.provider`` (or its
       ``provider``/``HAIRBALL_INFERENCE_PROVIDER`` equivalent). When the agent
    ...
    if not candidate:
        try:
            ...
            candidate = ""
    if not candidate:
        candidate = os.environ.get("HAIRBALL_INFERENCE_PROVIDER", "").strip()
...
def _resolve_azure_foundry_runtime(
    *,
    ...
    if not base_url:
        raise AuthError(
            "Azure Foundry requires a base URL. Set it via 'hairball model' or "
    ...
    if not api_key:
        raise AuthError(
            ...
            "~/.hairball/.env or run 'hairball model' to configure. To use "
            "keyless Microsoft Entra ID auth instead, set "
            "model.auth_mode: entra_id in config.yaml (or pick "
            "'Microsoft Entra ID' in 'hairball model')."
        )
...
def _resolve_explicit_runtime(
    *,
    ...
    if provider == "managed":
        ...
        base_url = (
            explicit_base_url
            or _managed_inference_base_url_override()
        ...
        api_key = explicit_api_key or (
            ...
            if _agent_key_is_usable(
                state,
                max(60, env_int("HAIRBALL_MANAGED_MIN_KEY_TTL_SECONDS", 1800)),
            )
            else ""
        )
        ...
        if not api_key:
            creds = resolve_managed_runtime_credentials(
                timeout_seconds=float(_getenv("HAIRBALL_MANAGED_TIMEOUT_SECONDS", "15")),
            )
            api_key = creds.get("api_key", "")
...
def resolve_runtime_provider(
    *,
    ...
    if requested_provider in ("vertex", "google-vertex", "vertex-ai", "gcp-vertex", "vertexai"):
        ...
        if not token or not base_url:
            raise AuthError(
                ...
                "in ~/.hairball/.env, or run 'gcloud auth application-default "
                "login' for ADC. Set the GCP project/region under vertex: in "
                "config.yaml if they aren't embedded in the credentials. "
                "Install the extra with: pip install 'hairball-agent[vertex]'."
            )
        return {
            "provider": "vertex",
            "api_mode": "chat_completions",
    ...
    if pool and pool.has_credentials():
        ...
        # For managed, the pool entry's runtime_api_key is the agent_key
        # compatibility field. It must be an invoke JWT. The pool doesn't
        # refresh it during selection (that would trigger network calls in
        # non-runtime contexts like `hairball auth list`). If the key is
        # expired/missing, refresh the selected pool entry before falling back
        # to singleton auth resolution.
        if provider == "managed" and entry is not None:
            min_ttl = max(60, env_int("HAIRBALL_MANAGED_MIN_KEY_TTL_SECONDS", 1800))
            managed_state = {
            ...
            if not _agent_key_is_usable(managed_state, min_ttl):
                logger.debug("managed pool entry agent_key expired/missing, refreshing selected pool entry")
                try:
                    refreshed = pool.try_refresh_current()
                except Exception as exc:
                    logger.debug("managed pool entry refresh failed: %s", exc)
                    refreshed = None
                ...
                if not pool_api_key or not _agent_key_is_usable(managed_state, min_ttl):
                    logger.debug("managed pool entry agent_key still unavailable, falling through to runtime resolution")
                    pool_api_key = ""
        if (
            entry is not None
            and pool_api_key
    ...
    if provider == "managed":
        try:
            ...
            if requested_provider != "auto":
                raise
            # Auto-detected managed but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected managed provider but credentials failed; "
    ...
    if provider == "openai-codex":
        try:
            ...
            return {
                ...
                "base_url": creds.get("base_url", "").rstrip("/"),
                "api_key": creds.get("api_key", ""),
                "source": creds.get("source", "hairball-auth-store"),
                "last_refresh": creds.get("last_refresh"),
                "requested_provider": requested_provider,
            }
        except AuthError:
            if requested_provider != "auto":
                raise
            # Auto-detected reference but credentials are stale/revoked —
            # fall through to env-var providers (e.g. OpenRouter).
            logger.info("Auto-detected reference provider but credentials failed; "
                        "falling through to next provider.")
    ...
    if provider == "minimax-oauth":
        ...
        if pconfig and pconfig.auth_type == "oauth_minimax":
            from hairball_cli.auth import resolve_minimax_oauth_runtime_credentials
    ...
    if provider == "anthropic":
        ...
        # base_url (e.g. reference endpoint) would leak into Anthropic requests.
        cfg_provider = str(model_cfg.get("provider") or "").strip().lower()
        cfg_base_url = ""
        if cfg_provider == "anthropic":
        ...
        if _is_azure_endpoint:
            ...
            # chain.  Accept both `key_env` (Hairball canonical — matches the
            # custom_providers field name) and `api_key_env` (documented in the
            # Azure Foundry guide and read by most Hairball-compatible importers).
