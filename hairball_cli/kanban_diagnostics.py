...
class DiagnosticAction:
    ...
    * ``cli_hint`` — print/copy a shell command (e.g.
      ``hairball -p <profile> auth``). No HTTP side effect.
...
def _rule_triage_aux_unavailable(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    With auto-decompose off, the user must run ``hairball kanban specify``,
    ...
    if auto_decompose:
        ...
        detail_path = (
            ...
            "`hairball kanban specify`, which uses auxiliary.triage_specifier."
        )
    ...
    actions = [
        DiagnosticAction(
            ...
            payload={
                "command": (
                    f"hairball config set {primary_slot}.provider auto"
                )
            },
            suggested=True,
        ),
    ...
    if not fallback_explicit and not main_visible:
        actions.append(DiagnosticAction(
            ...
            payload={
                "command": (
                    f"hairball config set {fallback_slot}.provider auto"
                )
            },
        ))
    if not auto_decompose:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Specify manually: hairball kanban specify {task_id}",
            payload={"command": f"hairball kanban specify {task_id}"},
        ))
...
def _rule_repeated_failures(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if most_recent_outcome == "spawn_failed" and assignee and assignee != "default":
        ...
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Verify profile: hairball -p {assignee} doctor",
            payload={"command": f"hairball -p {assignee} doctor"},
            suggested=True,
        ))
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Fix profile auth: hairball -p {assignee} auth",
            payload={"command": f"hairball -p {assignee} auth"},
        ))
        ...
        if task_id:
            actions.append(DiagnosticAction(
                kind="cli_hint",
                label=f"Check logs: hairball kanban log {task_id}",
                payload={"command": f"hairball kanban log {task_id}"},
                suggested=True,
            ))
...
def _rule_block_unblock_cycling(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    if task_id:
        actions.append(DiagnosticAction(
            kind="cli_hint",
            label=f"Check block reasons: hairball kanban events {task_id}",
            payload={"command": f"hairball kanban events {task_id}"},
            suggested=True,
        ))
    return [Diagnostic(
...
def _rule_stranded_in_ready(task, events, runs, now, cfg) -> list[Diagnostic]:
    ...
    * External worker pool (reference CLI, Claude Code lane, custom daemon)
      is down, hung, or wasn't started.
    * Dispatcher is misconfigured (wrong board, wrong HAIRBALL_HOME).
    ...
    works for Hairball profiles, registered lanes, external workers, and
    ...
    actions = [
        ...
        DiagnosticAction(
            ...
            payload={"command": "hairball kanban diagnostics"},
        ),
    ]
...
def config_from_runtime_config(raw_config: Optional[dict]) -> dict:
    """Build diagnostics config from the full Hairball runtime config.
