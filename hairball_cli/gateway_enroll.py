"""``hairball gateway enroll`` — enroll a self-hosted gateway with a relay connector.
...
``docs/connector-gateway-auth-design.md``:
  ...
  1. Resolve a fresh Managed Portal access token from the existing login
     (``~/.hairball/auth.json``) — the same path ``hairball dashboard register``
     uses (``resolve_managed_access_token``). This proves *which managed org (tenant)*
  ...
  4. Persist ``GATEWAY_RELAY_ID`` / ``GATEWAY_RELAY_SECRET`` /
     ...
     ``~/.hairball/.env``. The per-gateway secret authenticates the WS upgrade;
...
def _resolve_identity_token() -> str:
    """Resolve the caller-identity bearer token (generic-OIDC or Managed Portal).
    ...
    self-hosted-IdP path; otherwise Managed Portal). Raises RuntimeError on failure.
    """
...
def _post_enroll(
    *,
    ...
    try:
        ...
        if exc.code == 401:
            raise RuntimeError(
                "Connector rejected the caller identity (401). Your Managed Portal "
                "token could not be verified — try `hairball auth add managed` and retry."
            ) from exc
        if exc.code == 403:
            raise RuntimeError(
                detail
...
def cmd_gateway_enroll(args) -> None:
    ...
    if is_managed():
        print(
            "✗ `hairball gateway enroll` is not available in a managed/hosted install.\n"
    ...
    #    gapped / self-hosted-IdP, NO Managed Portal); otherwise the Managed Portal token.
    try:
    ...
    #    save_env_value writes them to ~/.hairball/.env (0600 dir) and never logs.
    to_write = {
