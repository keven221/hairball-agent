...
Add, remove, or reorder entries here — both `hairball setup` and
`hairball` provider-selection will pick up the change automatically.
"""
...
from hairball_cli.urllib_security import open_credentialed_url

# Identify ourselves so endpoints fronted by Cloudflare's Browser Integrity
# Check (error 1010) don't reject the default ``Python-urllib/*`` signature.
_HAIRBALL_USER_AGENT = f"hairball-cli/{_HAIRBALL_VERSION}"
...
def _codex_curated_models() -> list[str]:
    ...
    This keeps the gateway /model picker in sync with the CLI `hairball model`
    flow without maintaining a separate static list.
    """
    from hairball_cli.codex_models import DEFAULT_CODEX_MODELS, _add_forward_compat_models
...
# $HAIRBALL_HOME/models_dev_cache.json as of 2026-04-28. Whenever xAI renames
...
def _xai_merge_curated_extras(ids: list[str]) -> list[str]:
    """Append Hairball-curated xAI models that are missing from models.dev."""
    out = list(ids)
...
def _xai_curated_models() -> list[str]:
    ...
    Reads $HAIRBALL_HOME/models_dev_cache.json directly (no network) so this
    runs at import time without blocking. Falls back to ``_XAI_STATIC_FALLBACK``
    when the cache is empty or unreadable. Hairball refreshes the cache from
...
# ---------------------------------------------------------------------------
# Managed Portal free-model helper
# ---------------------------------------------------------------------------
# The Managed Portal models endpoint is the source of truth for which models
...
# ---------------------------------------------------------------------------
# Managed Portal account tier detection
# ---------------------------------------------------------------------------
...
def partition_managed_models_by_tier(
    ...
    """Split managed models into (selectable, unavailable) based on user tier.
...
def union_with_portal_free_recommendations(
    ...
    *,
    ...
    older Hairball that doesn't ship that model in its hardcoded curated
...
def union_with_portal_paid_recommendations(
    ...
    *,
    ...
    picker even if the user is running an older Hairball that doesn't ship
...
def check_managed_free_tier(*, force_fresh: bool = False) -> bool:
    """Check if the current Managed Portal user is on a free (unpaid) tier.
...
# ---------------------------------------------------------------------------
# Managed Portal recommended models
#
...
def _managed_recommended_disk_path() -> "Path":
    """Disk path for the persisted recommended-models cache."""
    from hairball_constants import get_hairball_home
    return get_hairball_home() / "cache" / "managed_recommended_cache.json"


def _read_managed_recommended_disk(base: str) -> dict[str, Any] | None:
    ...
    """
    try:
        with open(_managed_recommended_disk_path(), encoding="utf-8") as fh:
            blob = json.load(fh)
...
def _write_managed_recommended_disk(base: str, data: dict[str, Any]) -> None:
    ...
    if not data:
        return
    path = _managed_recommended_disk_path()
    try:
        try:
        ...
        logging.getLogger(__name__).debug(
            "managed recommended-models disk cache write failed: %s", exc
        )
...
def fetch_managed_recommended_models(
    ...
    *,
    ...
    if data:
        ...
        _write_managed_recommended_disk(base, data)
        return data
    ...
    disk = _read_managed_recommended_disk(base)
    if disk:
        _managed_recommended_cache[base] = (disk, now)
        return disk
...
# this list:  hairball model, /model, list_authenticated_providers.
#
# Fields:
#   slug        — internal provider ID (used in config.yaml, --provider flag)
#   label       — short display name
#   tui_desc    — longer description for the `hairball model` interactive picker
# ---------------------------------------------------------------------------

class ProviderEntry(NamedTuple):
    slug: str
    label: str
    tui_desc: str   # detailed description for `hairball model` TUI

CANONICAL_PROVIDERS: list[ProviderEntry] = [
    ProviderEntry("managed",           "Managed Portal",              "Legacy managed subscription (disabled by default; prefer BYOK)"),
    ProviderEntry("openrouter",     "OpenRouter",               "OpenRouter (Pay-per-use API aggregator)"),
    ProviderEntry("moa",            "Mixture of Agents",        "Mixture of Agents (named presets; aggregator acts after reference models)"),
    ProviderEntry("novita",         "NovitaAI",                 "NovitaAI (Cloud: Model API, Agent Sandbox, GPU Cloud)"),
    ProviderEntry("lmstudio",       "LM Studio",                "LM Studio (Local desktop app with built-in model server)"),
    ProviderEntry("anthropic",      "Anthropic",                "Anthropic (Claude models via API key or Claude Code)"),
    ProviderEntry("openai-codex",   "ChatGPT",             "ChatGPT (reference CLI via ChatGPT subscription or API key)"),
    ...
    ProviderEntry("xiaomi",         "Xiaomi",              "Xiaomi (provider-V2.5 and V2 models: pro, omni, flash)"),
...
#
# Some vendors expose several Hairball provider slugs (one per endpoint /
# auth method: global API, China API, OAuth coding plan, ...). Listing every
# slug as a top-level row in the interactive `hairball model` / setup wizard /
...
PROVIDER_GROUPS: dict[str, tuple[str, str, list[str]]] = {
    ...
    "openai":   ("OpenAI",          "reference CLI or direct OpenAI API",                  ["openai-codex", "openai-api"]),
...
def group_providers(slugs):
    ...
    DISPLAY ONLY. Used by every interactive picker (``hairball model``, the
...
# The model Hairball silently lands on when the user never picked one and the
...
# sensible default, but metered aggregators (Managed Portal, OpenRouter) order
...
# path. The *interactive* default (GUI onboarding / ``hairball model``) uses the
# richer free/paid-tier-aware resolver — see ``get_recommended_default_model``
# in hairball_cli/web_server.py and ``partition_managed_models_by_tier`` — which can
# hit the Portal; this fallback must stay cheap and network-free.
_PROVIDER_SILENT_DEFAULT_OVERRIDES: dict[str, str] = {
    "managed": PREFERRED_SILENT_DEFAULT_MODEL,
...
def get_default_model_for_provider(provider: str) -> str:
    ...
    model was ever selected (e.g. ``hairball auth add openai-codex`` without
    ``hairball model``, or a profile that sets ``provider`` with no ``model``).

    For most providers this is the first entry in ``_PROVIDER_MODELS`` — the
    same model the ``hairball model`` picker offers first. For metered aggregators
...
def _openrouter_model_supports_tools(item: Any) -> bool:
    ...
    hairball-agent is tool-calling-first — every provider path assumes the model
    ...
    **Permissive when the field is missing.** Some OpenRouter-compatible gateways
    (Managed Portal, private mirrors, older catalog snapshots) don't populate
...
def fetch_openrouter_models(
    ...
    *,
    ...
    try:
        from hairball_cli.model_catalog import get_curated_openrouter_models
    ...
    for preferred_id in preferred_ids:
        ...
        if live_item is None:
            continue
        # Hide models that don't advertise tool-calling support — hairball-agent
...
def get_curated_managed_model_ids() -> list[str]:
    """Return the curated Managed Portal model-id list.
    ...
    """
    try:
        from hairball_cli.model_catalog import get_curated_managed_models
...
def fetch_models_with_pricing(
    ...
    *,
    ...
    Works with any OpenRouter-compatible endpoint (OpenRouter, Managed Portal).
    """
...
_DEFAULT_MANAGED_INFERENCE_BASE = "https://inference.example.invalid"


def _resolve_managed_pricing_credentials() -> tuple[str, str]:
    """Return ``(api_key, base_url)`` for Managed Portal pricing.

    The managed inference ``/v1/models`` endpoint exposes pricing without
...
def get_pricing_for_provider(provider: str, *, force_refresh: bool = False) -> dict[str, dict[str, str]]:
    ...
    if normalized == "managed":
        ...
        if base_url:
            # managed base_url typically looks like https://inference.example.invalid/v1
...
def list_available_providers() -> list[dict[str, str]]:
    ...
    source of truth shared with ``hairball model``, ``/model``, etc.).
    """
    ...
    for pid in provider_order:
        ...
        has_creds = False
        try:
            from hairball_cli.auth import get_auth_status, has_usable_secret
            if pid == "custom":
...
def parse_model_input(raw: str, current_provider: str) -> tuple[str, str]:
    ...
    Supports ``provider:model`` syntax to switch providers at runtime::
        ...
        managed:hairball-3                           →  ("managed", "hairball-3")
...
def curated_models_for_provider(
    ...
    *,
    ...
    # Try live API first (reference, managed, etc. all support /models)
...
def _resolve_static_model_alias(
    ...
    try:
        from hairball_cli.model_switch import MODEL_ALIASES
    except Exception:
        return None
...
def normalize_provider(provider: Optional[str]) -> str:
    """Normalize provider aliases to Hairball' canonical provider ids.

    Note: ``"auto"`` passes through unchanged — use
    ``hairball_cli.auth.resolve_provider()`` to resolve it to a concrete
...
# strip the field), so false positives are harmless. reference-series models
# (gpt-5-codex, gpt-5.3-codex, etc.) are excluded — they don't expose the
# service_tier parameter through the reference Responses API.
...
def _is_openai_fast_model(model_id: Optional[str]) -> bool:
    ...
    if not base:
        return False
    # Exclude reference-series — they route through the reference Responses API
...
def model_supports_fast_mode(model_id: Optional[str]) -> bool:
    """Return whether Hairball should expose the /fast toggle for this model."""
...
def resolve_fast_mode_overrides(model_id: Optional[str]) -> dict[str, Any] | None:
    ...
    keys (service_tier for ChatGPT, speed for Anthropic Messages).
    """
...
def _resolve_copilot_catalog_api_key() -> str:
    ...
    Resolution order:
      ...
      2. ``read_credential_pool("copilot")`` — a token (typically a
         ...
         populated by ``hairball auth add copilot`` and by ``_seed_from_env``
         when the env var is set in ``~/.hairball/.env``.
    ...
    """
    try:
        from hairball_cli.auth import resolve_api_key_provider_credentials
...
def provider_model_ids(provider: Optional[str], *, force_refresh: bool = False) -> list[str]:
    ...
    Tries live API endpoints for providers that support them (reference, managed),
    falling back to static lists. For providers in ``_MODELS_DEV_PREFERRED``
    (opencode-go/zen, xiaomi, deepseek, smaller inference providers, etc.),
    models.dev entries are merged on top of curated so new models released
    on the platform appear in ``/model`` without a Hairball release.
    """
    normalized = normalize_provider(provider)
    if normalized == "openrouter":
    ...
    if normalized == "openai-codex":
        ...
        # a Hairball release). Falls back to the hardcoded catalog if no token
    ...
    if normalized == "managed":
        # Try live Managed Portal /models endpoint
        try:
            from hairball_cli.auth import fetch_managed_models, resolve_managed_runtime_credentials
            creds = resolve_managed_runtime_credentials()
            if creds:
        ...
        # added Portal models still surface without a Hairball release.
    ...
    if normalized in ("openai", "openai-api"):
        ...
        if api_key:
            ...
            # agentic catalog so ``/model`` matches what ``hairball model`` shows.
...
#
# Cache strategy:
#   - One JSON file at $HAIRBALL_HOME/provider_models_cache.json
...
def _provider_models_cache_path() -> Path:
    from hairball_constants import get_hairball_home
    return get_hairball_home() / "provider_models_cache.json"
...
def _credential_fingerprint(provider: str) -> str:
    ...
    relevant tokens live in ``$HAIRBALL_HOME/auth.json`` and external
...
def _lmstudio_request_headers(api_key: Optional[str] = None) -> dict:
    ...
    headers = {"User-Agent": _HAIRBALL_USER_AGENT}
...
_COPILOT_MODEL_ALIASES = {
    ...
    # Dash-notation fallbacks: Hairball' default Claude IDs elsewhere use
...
def opencode_model_api_mode(provider_id: Optional[str], model_id: Optional[str]) -> str:
    ...
    - GPT-5 / reference models on Zen use ``/v1/responses``
    - Claude models on Zen use ``/v1/messages``
    - MiniMax and Qwen models on Go use ``/v1/messages``
    - GLM / Kimi / DeepSeek / provider on Go use ``/v1/chat/completions``
...
def probe_api_models(
    ...
    tried: list[str] = []
    headers: dict[str, str] = {"User-Agent": _HAIRBALL_USER_AGENT}
    if urllib.parse.urlparse(normalized).hostname == "generativelanguage.googleapis.com":
        headers["X-Goog-Api-Client"] = f"hairball-agent/{_HAIRBALL_VERSION}"
    ...
    if isinstance(request_headers, dict):
        ...
        from hairball_cli.config import normalize_extra_headers
...
_DEEPINFRA_MODELS_QUERY = "filter=true&sort_by=hairball"
...
def _ollama_cloud_cache_path() -> Path:
    ...
    from hairball_constants import get_hairball_home
    return get_hairball_home() / "ollama_cloud_models_cache.json"
    ...
    *,
    ...
    if normalized == "moa":
        try:
            ...
            return {
                ...
                "message": f"MoA preset `{requested}` was not found. Run `hairball moa list`.",
            }
        except Exception as exc:
            return {
                "accepted": False, "persist": False, "recognized": False,
    ...
    if normalized == "custom" or normalized.startswith("custom:"):
        ...
        message = (
            ...
            f"Hairball will still save `{requested}`, but the endpoint should expose `/models` for verification."
        )
        if api_mode == "anthropic_messages":
            message += (
    ...
    if normalized in {"openai-codex", "xai-oauth"}:
        ...
        if catalog_models:
            ...
            if suggestions:
                suggestion_text = "\n  Similar models: " + ", ".join(f"`{s}`" for s in suggestions)
            provider_label = "ChatGPT" if normalized == "openai-codex" else "xAI Grok OAuth (SuperGrok / Premium+)"
        ...
        if catalog_models:
            ...
            return {
                ...
                "message": (
                    ...
                    f"{suggestion_text}"
                    "\n  MiniMax does not expose a /models endpoint, so Hairball cannot verify the model name."
    ...
    if api_models is not None:
        ...
        if requested_for_lookup in set(api_models):
            ...
            # fetch) so we only accept models Hairball actually ships.  (#46850)
