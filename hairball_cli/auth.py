"""
Multi-provider authentication system for Hairball Agent.

Supports OAuth device code flows (Managed Portal, future: ChatGPT) and
traditional API key providers (OpenRouter, custom endpoints). Auth state
is persisted in ~/.hairball/auth.json with cross-process file locking.
...
from hairball_constants import OPENROUTER_BASE_URL, secure_parent_dir
...
# Managed Portal defaults
DEFAULT_MANAGED_PORTAL_URL = "https://portal.example.invalid"
DEFAULT_MANAGED_INFERENCE_URL = "https://inference.example.invalid/v1"
DEFAULT_MANAGED_CLIENT_ID = "hairball-cli"
MANAGED_INFERENCE_INVOKE_SCOPE = "inference:invoke"
MANAGED_BILLING_MANAGE_SCOPE = "billing:manage"
...
DEVICE_AUTH_POLL_INTERVAL_CAP_SECONDS = 1     # poll at most every 1s; kept in sync with portal_device_auth
...
try:  # Version tag for the reference token-endpoint User-Agent; fall back if unavailable.
    from hairball_cli import __version__ as _HAIRBALL_CLI_VERSION
except Exception:  # pragma: no cover - version import should always succeed
    _HAIRBALL_CLI_VERSION = "unknown"
CODEX_OAUTH_USER_AGENT = f"hairball-cli/{_HAIRBALL_CLI_VERSION}"
...
def get_anthropic_key() -> str:
    ...
    ``~/.hairball/.env`` so a deliberate key rotation isn't shadowed by a stale
    ...
    """
    from hairball_cli.config import get_env_value_prefer_dotenv
...
def _resolve_api_key_provider_secret(
    ...
    if provider_id == "copilot":
        ...
        try:
            from hairball_cli.copilot_auth import resolve_copilot_token, get_copilot_api_token
    ...
    for env_var in pconfig.api_key_env_vars:
        # Prefer ~/.hairball/.env over os.environ so a deliberate key rotation
        # in the user's .env file isn't shadowed by a stale shell export
        # inherited from a parent process (reference CLI, test runners, etc.).
...
def is_rate_limited_auth_error(error: Exception) -> bool:
    ...
    instead of prompting the operator to run ``hairball auth``.
    """
    return (
...
def format_auth_error(error: Exception) -> str:
    ...
    if error.relogin_required:
        return f"{error} Run `hairball model` to re-authenticate."
...
def _format_managed_entitlement_auth_error(error: AuthError) -> str:
    try:
        ...
        message = format_managed_portal_entitlement_message(
            account_info,
            capability="managed model access",
        )
        if message:
            return message
    except Exception:
        pass
    return f"{error} Check credits or billing in Managed Portal, then retry."
...
def _oauth_trace_enabled() -> bool:
    raw = os.getenv("HAIRBALL_OAUTH_TRACE", "").strip().lower()
...
# =============================================================================
# Auth Store — persistence layer for ~/.hairball/auth.json
# =============================================================================

def _auth_file_path() -> Path:
    path = get_hairball_home() / "auth.json"
    # Seat belt: if pytest is running and HAIRBALL_HOME resolves to the real
    # user's auth store, refuse rather than silently corrupt it. This catches
    # tests that forgot to monkeypatch HAIRBALL_HOME, tests invoked without the
    # hermetic conftest, or sandbox escapes via threads/subprocesses. In
    # production (no PYTEST_CURRENT_TEST) this is a single dict lookup.
    if os.environ.get("PYTEST_CURRENT_TEST"):
        real_home_auth = (Path.home() / ".hairball" / "auth.json").resolve(strict=False)
        try:
        ...
        if resolved == real_home_auth:
            raise RuntimeError(
                ...
                "Set HAIRBALL_HOME to a tmp_path in your test fixture, or run "
...
def _global_auth_file_path() -> Optional[Path]:
    ...
    directory (classic mode, or custom HAIRBALL_HOME that is not a profile).
    ...
    try:
        ...
        global_root = get_default_hairball_root()
    except Exception:
        return None
    profile_home = get_hairball_home()
    try:
...
def _load_global_auth_store() -> Dict[str, Any]:
    ...
    ``~/.hairball/auth.json`` even when HAIRBALL_HOME is set to a profile
    path. The hermetic conftest does not redirect ``HOME``, so
    ``get_default_hairball_root()`` for a profile-shaped HAIRBALL_HOME can
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        ...
        if real_home_env:
            real_root = Path(real_home_env) / ".hairball" / "auth.json"
            try:
...
def _file_lock(
    ...
    (e.g. profile auth.json vs shared managed store) don't share reentrancy
...
def _auth_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    ...
    (outer) and the shared managed lock SECOND (inner). All runtime
...
def _load_auth_store(auth_file: Optional[Path] = None) -> Dict[str, Any]:
    ...
    if isinstance(raw, dict) and (
        ...
        if isinstance(raw.get("providers"), dict):
            _migrate_stale_managed_portal_url(raw["providers"])
        return raw
...
def _load_provider_state_with_source(
    ...
    global-root fallback state; persisting a rotated managed refresh token only to
    ...
    """
    try:
        from hairball_cli.provider_ids import provider_lookup_ids

        lookup_ids = provider_lookup_ids(provider_id)
    except Exception:
        lookup_ids = (provider_id,)
    ...
    if global_store:
        ...
        if isinstance(global_providers, dict):
            for key in lookup_ids:
                global_state = global_providers.get(key)
...
def _load_provider_state(auth_store: Dict[str, Any], provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    global scope. Once the user runs ``hairball auth login <provider>`` inside
...
def mark_provider_active_if_unset(provider_id: str) -> None:
    ...
    Used by ``hairball auth add`` OAuth paths that create credential-pool
...
def read_credential_pool(provider_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    ``hairball auth add <provider>`` inside the profile, profile entries
...
def get_provider_auth_state(provider_id: str) -> Optional[Dict[str, Any]]:
    ...
    OAuth or managed device-code session). See issue #18594 follow-up.
    """
    auth_store = _load_auth_store()
...
def is_provider_explicitly_configured(provider_id: str) -> bool:
    ...
    # not by the user explicitly configuring anthropic in Hairball.
    ...
    # the user initiated inside Hairball (manual add / device-code / PKCE), plus
    ...
    try:
        for entry in read_credential_pool(normalized):
            ...
            if source.startswith("env:"):
                ...
                continue
            if (
                source in {"device_code", "loopback_pkce", "hairball_pkce", "manual"}
...
def clear_provider_auth(provider_id: Optional[str] = None) -> bool:
    """
    Clear auth state for a provider. Used by `hairball logout`.
...
def _get_config_hint_for_unknown_provider(provider_name: str) -> str:
    ...
    """
    try:
        from hairball_cli.config import validate_config_structure
        issues = validate_config_structure()
        if not issues:
            return ""

        lines = ["Config issue detected — run 'hairball doctor' for full diagnostics:"]
        for ci in issues:
    ...
    *,
    ...
    if normalized != "auto":
        ...
        if _config_hint:
            ...
            msg += " Check 'hairball model' for available providers, or run 'hairball doctor' to diagnose config issues."
    ...
    # Auto-detect an OpenRouter credential added via `hairball auth add openrouter`
    ...
    # `hairball auth list` showing the credential while requests go out with no
    ...
    for pid, pconfig in PROVIDER_REGISTRY.items():
        ...
        for env_var in pconfig.api_key_env_vars:
            if has_usable_secret(os.getenv(env_var, "")):
                ...
                # OAuth but has a stale key in ~/.hairball/.env isn't silently
    ...
    raise AuthError(
        "No inference provider configured. Run 'hairball model' to choose a "
        "provider and model, or set an API key (OPENROUTER_API_KEY, "
        "OPENAI_API_KEY, etc.) in ~/.hairball/.env.",
...
_MANAGED_STALE_PORTAL_HOSTS: FrozenSet[str] = frozenset({
    "api.hairball.com",
})

# Allowlist of valid Managed Portal hosts. A portal_base_url outside this
...
_MANAGED_PORTAL_ALLOWED_HOSTS: FrozenSet[str] = frozenset({
...
def _migrate_stale_managed_portal_url(providers: Dict[str, Any]) -> None:
    managed = providers.get("managed")
    if not isinstance(managed, dict):
        return
    stored = (managed.get("portal_base_url") or "").strip()
    if stored:
        parsed = urlparse(stored)
        if parsed.hostname in _MANAGED_STALE_PORTAL_HOSTS:
            logger.warning(
                "auth: migrating stale managed portal_base_url %s -> %s",
                stored, DEFAULT_MANAGED_PORTAL_URL,
            )
            managed["portal_base_url"] = DEFAULT_MANAGED_PORTAL_URL


# Allowlist of hosts the Managed Portal proxy is willing to forward inference
...
def _managed_inference_env_override() -> Optional[str]:
    """Return the user-set ``MANAGED_INFERENCE_BASE_URL`` override, if any.
    ...
    """
    return _optional_base_url(os.getenv("MANAGED_INFERENCE_BASE_URL"))


def _managed_portal_env_override() -> Optional[str]:
    """Return the user/deployment-set Portal base URL override, if any.

    Mirrors ``_managed_inference_env_override()``: ``HAIRBALL_PORTAL_URL`` /
    ``HAIRBALL_PORTAL_BASE_URL`` / ``MANAGED_PORTAL_BASE_URL`` are the documented
    escape hatch (and ``managed_foreign.portal_url`` via the shared resolver).
    """
    from hairball_cli.managed_foreign_backends import portal_base_url as mf_portal

    env = _optional_base_url(
        os.getenv("HAIRBALL_PORTAL_URL")
        or os.getenv("HAIRBALL_PORTAL_BASE_URL")
        or os.getenv("MANAGED_PORTAL_BASE_URL")
    )
    if env:
        return env
    switched = mf_portal()
    # Only treat managed_foreign portal as an "override" on hairball backend;
    # legacy default must not bypass the allowlist path for stored hosts.
    from hairball_cli.managed_foreign_backends import is_hairball_backend

    if is_hairball_backend() and switched:
        return switched
    return None
...
def _assert_managed_inference_jwt_usable(
    ...
    *,
    ...
    if reason is None:
        return
    raise AuthError(
        "Managed Portal access token is not a usable inference JWT "
        f"({reason}). Re-authenticate with: hairball auth add managed",
        provider="managed",
        code=reason,
...
def _log_managed_invoke_jwt_selected(
    *,
    ...
    logger.info("managed inference auth: using NAS invoke JWT")
    _oauth_trace(
...
def resolve_qwen_runtime_credentials(
    *,
    ...
    base_url = os.getenv("HAIRBALL_QWEN_BASE_URL", "").strip().rstrip("/") or DEFAULT_QWEN_BASE_URL
    return {
        "provider": "qwen-oauth",
        "base_url": base_url,
...
def get_qwen_auth_status() -> Dict[str, Any]:
    ...
    try:
        ...
        # and `hairball model` walks users into a broken Qwen setup flow.
...
# =============================================================================
# Spotify auth — PKCE tokens stored in ~/.hairball/auth.json
# =============================================================================
...
def _spotify_client_id(
    ...
    candidates = (
        explicit,
        get_env_value("HAIRBALL_SPOTIFY_CLIENT_ID"),
    ...
    for candidate in candidates:
        ...
        if cleaned:
            return cleaned
    raise AuthError(
        "Spotify client_id is required. Set HAIRBALL_SPOTIFY_CLIENT_ID or pass --client-id.",
        provider="spotify",
...
def _spotify_redirect_uri(
    ...
    candidates = (
        explicit,
        get_env_value("HAIRBALL_SPOTIFY_REDIRECT_URI"),
...
def _spotify_api_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("HAIRBALL_SPOTIFY_API_BASE_URL"),
...
def _spotify_accounts_base_url(state: Optional[Dict[str, Any]] = None) -> str:
    ...
    candidates = (
        get_env_value("HAIRBALL_SPOTIFY_ACCOUNTS_BASE_URL"),
...
def _refresh_spotify_oauth_state(
    ...
    *,
    ...
    if not refresh_token:
        raise AuthError(
            "Spotify refresh token missing. Run `hairball auth spotify` again.",
            provider="spotify",
    ...
    if response.status_code >= 400:
        detail = response.text.strip()
        raise AuthError(
            "Spotify token refresh failed. Run `hairball auth spotify` again."
            + (f" Response: {detail}" if detail else ""),
            provider="spotify",
            code="spotify_refresh_failed",
            relogin_required=True,
...
def resolve_spotify_runtime_credentials(
    *,
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError(
                "Spotify is not authenticated. Run `hairball auth spotify` first.",
                provider="spotify",
        ...
        if should_refresh:
            try:
                ...
                if exc.relogin_required and state.get("refresh_token"):
                    ...
                    # Mirrors the managed / xAI-OAuth / reference-OAuth / MiniMax pattern.
    ...
    if not access_token:
        raise AuthError(
            "Spotify access token missing. Run `hairball auth spotify` again.",
            provider="spotify",
...
def _spotify_interactive_setup(redirect_uri_hint: str) -> str:
    ...
    resulting client_id to ~/.hairball/.env, and return it.
    ...
    print("       App name:     anything (e.g. hairball-agent)")
    ...
    # Persist so subsequent `hairball auth spotify` runs skip the wizard.
    save_env_value("HAIRBALL_SPOTIFY_CLIENT_ID", raw)
    ...
    if redirect_uri_hint and redirect_uri_hint != DEFAULT_SPOTIFY_REDIRECT_URI:
        save_env_value("HAIRBALL_SPOTIFY_REDIRECT_URI", redirect_uri_hint)

    print()
    print("Saved HAIRBALL_SPOTIFY_CLIENT_ID to ~/.hairball/.env")
    print()
    return raw
...
def login_spotify_command(args) -> None:
    ...
    # with "HAIRBALL_SPOTIFY_CLIENT_ID is required".
    ...
    print()
    print("Open this URL to authorize Hairball:")
    print(authorize_url)
    print()
...
def _print_loopback_ssh_hint(redirect_uri: str, *, docs_url: str | None = None) -> None:
    ...
    print(divider)
    print(f"Hairball is waiting for the OAuth callback on {redirect_uri}")
...
# =============================================================================
# ChatGPT auth — tokens stored in ~/.hairball/auth.json (not ~/.codex/)
#
# Hairball maintains its own reference OAuth session separate from the reference CLI
# and VS Code extension. This prevents refresh token rotation conflicts
# where one app's refresh invalidates the other's session.
# =============================================================================

def _read_codex_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    """Read reference OAuth tokens from Hairball auth store (~/.hairball/auth.json).
    
    Returns dict with 'tokens' (access_token, refresh_token) and 'last_refresh'.
    Raises AuthError if no reference tokens are stored.
    """
    if _lock:
        with _auth_store_lock():
            auth_store = _load_auth_store()
    ...
    if not state:
        raise AuthError(
            "No reference credentials stored. Run `hairball auth` to authenticate.",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "reference auth state is missing tokens. Run `hairball auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(access_token, str) or not access_token.strip():
        raise AuthError(
            "reference auth is missing access_token. Run `hairball auth` to re-authenticate.",
            provider="openai-codex",
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "reference auth is missing refresh_token. Run `hairball auth` to re-authenticate.",
            provider="openai-codex",
...
def _sync_codex_pool_entries(
    ...
    """Mirror a fresh reference re-auth into the credential_pool OAuth entries.
    ...
    * ``device_code`` — the singleton-seeded entry written by the device-code
      OAuth flow when the user logged in via ``hairball setup`` / the model
      picker.  Always synced with the fresh tokens.
    * ``manual:device_code`` — entries created by ``hairball auth add openai-codex``
      ...
      ``hairball auth add openai-codex`` and must not be overwritten by a
...
def _save_codex_tokens(tokens: Dict[str, str], last_refresh: str = None, label: str = None) -> None:
    """Save reference OAuth tokens to Hairball auth store (~/.hairball/auth.json)."""
    ...
    with _auth_store_lock():
        ...
        # ``hairball auth add openai-codex`` created (which must not be
...
def _recover_codex_tokens_from_cli(reason: str) -> Optional[Dict[str, str]]:
    """Adopt a valid reference CLI token pair into Hairball auth, if available."""
    ...
    if not (
        ...
        return None
    logger.info("reference auth recovered from reference CLI auth.json (%s).", reason)
...
def refresh_codex_oauth_pure(
    ...
    *,
    ...
    """Refresh reference OAuth tokens without mutating Hairball auth state."""
    ...
    if response.status_code == 429:
        ...
        # "run hairball auth" prompt (see issue #32790).
        retry_after = _parse_retry_after_seconds(getattr(response, "headers", None))
        if retry_after is not None:
            message = (
                f"reference provider quota exhausted (429); retry after {retry_after}s. "
                "Credentials are still valid."
            )
        else:
            message = (
                "reference provider quota exhausted (429). Credentials are still valid; "
    ...
    if response.status_code != 200:
        ...
        message = f"reference token refresh failed with status {response.status_code}."
        relogin_required = False
        try:
            err = response.json()
            if isinstance(err, dict):
                ...
                if isinstance(err_obj, dict):
                    ...
                    if isinstance(nested_msg, str) and nested_msg.strip():
                        message = f"reference token refresh failed: {nested_msg.strip()}"
                # OAuth spec shape: {"error": "code_str", "error_description": "..."}
                    ...
                    if isinstance(err_desc, str) and err_desc.strip():
                        message = f"reference token refresh failed: {err_desc.strip()}"
        except Exception:
            pass
        ...
        if code == "refresh_token_reused":
            message = (
                "reference refresh token was already consumed by another client "
                "(e.g. reference CLI or VS Code extension). "
                "Run `codex` in your terminal to generate fresh tokens, "
                "then run `hairball auth` to re-authenticate."
            )
            relogin_required = True
    ...
    try:
        ...
        raise AuthError(
            "reference token refresh returned invalid JSON.",
            provider="openai-codex",
    ...
    if not isinstance(refreshed_access, str) or not refreshed_access.strip():
        raise AuthError(
            "reference token refresh response was missing access_token.",
            provider="openai-codex",
...
def _refresh_codex_auth_tokens(
    ...
    """Refresh reference access token using the refresh token.
    
    Saves the new tokens to Hairball auth store automatically.
    """
    try:
        ...
        # Self-heal cross-store refresh_token rotation. Hairball keeps its OWN
        # reference OAuth token (per profile + top-level), separate from the reference
        # CLI's ~/.codex/auth.json. OAuth refresh_tokens are single-use, so when
        # the reference CLI (or another Hairball process) rotates the shared token,
        ...
        # fresh token from ~/.codex/auth.json (the reference CLI keeps it current) so
...
def _import_codex_cli_tokens() -> Optional[Dict[str, str]]:
    """Try to read tokens from ~/.codex/auth.json (reference CLI shared file).
    ...
    try:
        ...
        if _codex_access_token_is_expiring(access_token, 0):
            logger.debug(
                "reference CLI tokens at %s are expired — skipping import.", auth_path,
            )
            return None
        return dict(tokens)
    except Exception:
...
def resolve_codex_runtime_credentials(
    *,
    ...
    """Resolve runtime credentials from Hairball's own reference token store.
    ...
    if data is None:
        ...
        if pool_token:
            base_url = (
                os.getenv("HAIRBALL_CODEX_BASE_URL", "").strip().rstrip("/")
        ...
        if pool_rate_limit:
            ...
            if isinstance(reset_at, (int, float)) and reset_at > time.time():
                ...
                message = (
                    f"reference provider quota exhausted (429); retry after {remaining}s. "
                    "Credentials are still valid."
                )
            else:
                message = (
    ...
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("HAIRBALL_CODEX_REFRESH_TIMEOUT_SECONDS", 20)

    should_refresh = bool(force_refresh)
    if (not should_refresh) and refresh_if_expiring:
        should_refresh = _codex_access_token_is_expiring(access_token, refresh_skew_seconds)
    if should_refresh:
        # Re-read under lock to avoid racing with other Hairball processes
...
def _codex_pool_rate_limit_status() -> Optional[Dict[str, Any]]:
    """Return metadata for a pool-only reference credential in quota cooldown."""
    ...
    try:
        ...
        logger.debug("reference pool rate-limit lookup failed", exc_info=True)
    return None
...
def _pool_codex_access_token() -> str:
    ...
    try:
        ...
        logger.debug("reference pool fallback lookup failed", exc_info=True)
    return ""


# =============================================================================
# xAI Grok OAuth — tokens stored in ~/.hairball/auth.json
# =============================================================================
...
def _read_xai_oauth_tokens(*, _lock: bool = True) -> Dict[str, Any]:
    ...
    if not state:
        raise AuthError(
            "No xAI OAuth credentials stored. Select xAI Grok OAuth (SuperGrok / Premium+) in `hairball model`.",
            provider="xai-oauth",
    ...
    if not isinstance(tokens, dict):
        raise AuthError(
            "xAI OAuth state is missing tokens. Re-authenticate with `hairball model`.",
            provider="xai-oauth",
    ...
    if not access_token:
        raise AuthError(
            "xAI OAuth state is missing access_token. Re-authenticate with `hairball model`.",
            provider="xai-oauth",
    ...
    if not refresh_token:
        raise AuthError(
            "xAI OAuth state is missing refresh_token. Re-authenticate with `hairball model`.",
            provider="xai-oauth",
...
def _write_through_xai_oauth_to_global_root(state: Dict[str, Any]) -> None:
    ...
    # ~/.hairball/auth.json even when HAIRBALL_HOME points at a profile path
...
def _xai_proactive_refresh_skew_seconds(access_token: str) -> int:
    ...
    resolution (chat turn, Imagine tool call, ``hairball auth status``, …),
...
def _xai_validate_oauth_endpoint(url: str, *, field: str) -> str:
    ...
    output is cached in ``~/.hairball/auth.json``. A single MITM during initial
    ...
    if host != "x.ai" and not host.endswith(".x.ai"):
        raise AuthError(
            ...
            f"discovery; re-authenticate with `hairball model` to re-fetch.",
            provider="xai-oauth",
            code="xai_discovery_invalid",
        )
    return url
...
def _xai_validate_inference_base_url(value: str, *, fallback: str) -> str:
    ...
    the user's SuperGrok subscription. ``XAI_BASE_URL`` / ``HAIRBALL_XAI_BASE_URL``
...
def refresh_xai_oauth_pure(
    ...
    *,
    ...
    if not isinstance(refresh_token, str) or not refresh_token.strip():
        raise AuthError(
            "xAI OAuth is missing refresh_token. Re-authenticate with `hairball model`.",
            provider="xai-oauth",
            code="xai_auth_missing_refresh_token",
            relogin_required=True,
        )
    ...
    # written by an older Hairball (or hand-edited) may carry a non-xAI
    ...
    # with a clear error so the user can re-run `hairball model` to refetch.
    ...
    if response.status_code != 200:
        ...
        # on the allowlist for API access).  Re-running ``hairball model``
...
def resolve_xai_oauth_runtime_credentials(
    *,
    ...
    tokens = dict(data["tokens"])
    access_token = str(tokens.get("access_token", "") or "").strip()
    refresh_timeout_seconds = env_float("HAIRBALL_XAI_REFRESH_TIMEOUT_SECONDS", 20)
    ...
    base_url = _xai_validate_inference_base_url(
        os.getenv("HAIRBALL_XAI_BASE_URL", "").strip().rstrip("/")
...
def _resolve_verify(
    *,
    ...
    effective_ca = (
        ...
        or os.getenv("HAIRBALL_CA_BUNDLE")
...
# =============================================================================
# OAuth Device Code Flow — delegates to portal_device_auth (OpenCode-shaped)
# =============================================================================

from hairball_cli.portal_device_auth import (  # noqa: E402  (kept next to the flow)
    poll_for_token as _portal_poll_for_token,
    request_device_code as _portal_request_device_code,
)
...
def _request_device_code(
    ...
    return _portal_request_device_code(client, portal_base_url, client_id, scope)
...
# =============================================================================
# Managed Portal — token refresh and model discovery
# =============================================================================

# -----------------------------------------------------------------------------
# Shared managed token store — lets OAuth credentials persist across profiles
# so a new `hairball --profile <name> auth add managed --type oauth` can one-tap
# import instead of running the full device-code flow every time.
#
# File lives at ${HAIRBALL_SHARED_AUTH_DIR}/managed_auth.json, defaulting to
# ``<hairball-root>/shared/managed_auth.json`` where ``<hairball-root>`` is what
# ``get_default_hairball_root()`` returns — ``~/.hairball`` on Linux/macOS,
# ``%LOCALAPPDATA%\hairball`` on native Windows, or the Docker/custom root.
# It is OUTSIDE any named profile's HAIRBALL_HOME so named profiles (which
# typically live under ``<hairball-root>/profiles/<name>/``) all see the
# same file.
#
...
_LEGACY_SHARED_STORE_FILENAME = "no" + "us_auth.json"
_managed_shared_lock_holder = threading.local()


def _managed_shared_auth_dir() -> Path:
    """Resolve the directory that holds the shared managed token store.

    Honors ``HAIRBALL_SHARED_AUTH_DIR`` so tests can redirect it to a tmp
    path without touching the real user's home. Defaults to
    ``<hairball-root>/shared/``.
    """
    override = os.getenv("HAIRBALL_SHARED_AUTH_DIR", "").strip()
    if override:
        return Path(override).expanduser()
    from hairball_constants import get_default_hairball_root

    return get_default_hairball_root() / "shared"


def _managed_shared_store_path() -> Path:
    auth_dir = _managed_shared_auth_dir()
    path = auth_dir / MANAGED_SHARED_STORE_FILENAME
    # Dual-read: prefer new filename, fall back to pre-rename on-disk store.
    if not path.is_file():
        legacy = auth_dir / _LEGACY_SHARED_STORE_FILENAME
        if legacy.is_file():
            path = legacy
    # Seat belt: if pytest is running and this resolves to a path under the
    # real user's Hairball root, refuse rather than silently corrupt cross-profile
    # state. Tests must set HAIRBALL_SHARED_AUTH_DIR to a tmp_path (conftest
    ...
    if os.environ.get("PYTEST_CURRENT_TEST"):
        from hairball_constants import get_default_hairball_root
        real_home_shared = (
            get_default_hairball_root() / "shared" / MANAGED_SHARED_STORE_FILENAME
        ).resolve(strict=False)
        real_legacy = (
            get_default_hairball_root() / "shared" / _LEGACY_SHARED_STORE_FILENAME
        ).resolve(strict=False)
        try:
            resolved = path.resolve(strict=False)
        except Exception:
            resolved = path
        if resolved in {real_home_shared, real_legacy}:
            raise RuntimeError(
                f"Refusing to touch real user shared managed auth store during test run: "
                f"{path}. Set HAIRBALL_SHARED_AUTH_DIR to a tmp_path in your test fixture."
            )
    return path
...
def _managed_shared_store_lock(timeout_seconds: float = AUTH_LOCK_TIMEOUT_SECONDS):
    """Cross-profile lock for the shared managed OAuth store.
    ...
    try:
        ...
        # No HAIRBALL_HOME yet (pre-setup): fall through without locking.
        yield
        return
    ...
    with _file_lock(
        ...
        timeout_seconds,
        "Timed out waiting for shared managed auth lock",
    ):
        yield


def _merge_shared_managed_oauth_state(state: Dict[str, Any]) -> bool:
    """Copy fresher shared OAuth tokens into a profile-local managed state."""
...
def _write_shared_managed_state(state: Dict[str, Any]) -> None:
    """Persist a minimal copy of the managed OAuth state to the shared store.
    ...
    try:
        with _managed_shared_store_lock():
            ...
            # Create with 0o600 atomically via os.open(O_EXCL) — closes the TOCTOU
            # window where write_text() + post-write chmod briefly exposed managed
        ...
        logger.debug("Failed to write shared managed auth store: %s", exc)


def _read_shared_managed_state() -> Optional[Dict[str, Any]]:
    """Return the shared managed OAuth state if present and well-formed.
    ...
    try:
        ...
        logger.debug("Shared managed auth store at %s is unreadable: %s", path, exc)
        return None
    if not isinstance(payload, dict):
        return None
...
def _clear_shared_managed_state(reason: str) -> None:
    """Remove the shared managed OAuth store after a terminal token failure."""
    try:
        with _managed_shared_store_lock():
            path = _managed_shared_store_path()
            try:
        ...
        logger.debug("Failed to clear shared managed auth store: %s", exc)


def _is_terminal_managed_refresh_error(exc: Exception) -> bool:
    """True when retrying the same managed refresh token cannot succeed."""
    return (
...
def _is_terminal_codex_oauth_refresh_error(exc: Exception) -> bool:
    """True when retrying the same reference OAuth refresh token cannot succeed.
...
def _quarantine_managed_oauth_state(
    ...
    *,
    ...
    # sizes/booleans. NEVER pass a raw token/agent_key into the log call — Hairball
    # has a known bug class where credential-shaped literals get corrupted in logs.
    forensic: Dict[str, Any] = {
        "reason": reason,
        "error_code": error.code,
        # No session_id field exists on managed state; provenance is client_id +
    ...
    logger.warning(
        "managed OAuth state quarantined (terminal auth death): %s",
...
def _quarantine_managed_pool_entries(
    ...
    *,
    ...
    """Remove singleton-seeded managed pool entries that contain dead OAuth state."""
    pool = auth_store.get("credential_pool")
    if not isinstance(pool, dict):
        return False
...
def _try_import_shared_managed_state(
    *,
    ...
    """Attempt to rehydrate managed OAuth state from the shared store.
    ...
    try:
        ...
        logger.debug("Shared managed import failed: %s", exc)
        return None
    except Exception as exc:
        _oauth_trace(
            "managed_shared_import_failed",
...
def _refresh_access_token(
    *,
    ...
    # Detect the OAuth 2.1 "refresh token reuse" signal from the Managed Portal
    ...
    # called POST /api/oauth/token with Hairball's refresh_token without
    # persisting the rotated token back to auth.json — the server then
    # retires the original RT, Hairball's next refresh uses it, and the whole
    ...
    if code == "refresh_token_reused" or "reuse" in lowered or "reuse detected" in lowered:
        description = (
            "Managed Portal detected refresh-token reuse and revoked this session.\n"
            "This usually means an external process (monitoring script, "
            "custom self-heal hook, or another Hairball install sharing "
            "~/.hairball/auth.json) called POST /api/oauth/token with Hairball's "
            "refresh token without persisting the rotated token back.\n"
            "managed refresh tokens are single-use — only Hairball may call the "
            "refresh endpoint. For health checks, use `hairball auth status` "
            "instead.\n"
            "Re-authenticate with: hairball auth add managed"
        )
        relogin = True
...
def fetch_managed_models(
    *,
    ...
    """Fetch available model IDs from the managed inference API."""
    ...
    for item in data:
        ...
        if isinstance(model_id, str) and model_id.strip():
            mid = model_id.strip()
            # Skip Hairball models — they're not reliable for agentic tool-calling
            if "hairball" in mid.lower():
                continue
...
def resolve_managed_access_token(
    *,
    ...
    """Resolve a refresh-aware Managed Portal access token for managed tool gateways."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
        state, state_source_path = _load_provider_state_with_source(auth_store, "managed")

        if not state:
            raise AuthError(
                "Hairball is not logged into Managed Portal.",
                provider="managed",
                relogin_required=True,
            )

        # HAIRBALL_PORTAL_BASE_URL / MANAGED_PORTAL_BASE_URL is the trusted
        # operator/deployment override (mirrors MANAGED_INFERENCE_BASE_URL) and
        ...
        if env_portal_override:
            ...
            if is_hairball_backend():
                if not portal_base_url:
                    raise AuthError(
                        "Hairball managed backend needs managed_foreign.portal_url "
                        "or HAIRBALL_PORTAL_URL.",
                        provider="managed",
                        relogin_required=True,
                    )
            else:
                parsed_portal_url = urlparse(portal_base_url)
                if parsed_portal_url.hostname and parsed_portal_url.hostname not in _MANAGED_PORTAL_ALLOWED_HOSTS:
                    logger.warning(
        ...
        with _managed_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError(
                    "No access token found for Managed Portal login.",
                    provider="managed",
                    relogin_required=True,
                )
            ...
            if not _is_expiring(state.get("expires_at"), refresh_skew_seconds):
                if merged_shared:
                    _save_provider_state_to_source(auth_store, "managed", state, state_source_path)
                return access_token
...
def refresh_managed_oauth_pure(
    ...
    *,
    ...
    """Refresh managed OAuth state without mutating auth.json directly.
    ...
    with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
        ...
        if force_refresh or current_invoke_jwt_status is not None:
            ...
            if not isinstance(refresh_token_value, str) or not refresh_token_value:
                ...
                raise AuthError(
                    "No refresh token is available for Managed Portal.",
                    provider="managed",
                    relogin_required=True,
                )
            refreshed = _refresh_access_token(
            ...
            state["inference_base_url"] = refreshed_url or DEFAULT_MANAGED_INFERENCE_URL
            state["obtained_at"] = now.isoformat()
            state["expires_in"] = access_ttl
            state["expires_at"] = datetime.fromtimestamp(
...
def refresh_managed_oauth_from_state(
    ...
    *,
    ...
    """Refresh managed OAuth from a state dict. Thin wrapper around refresh_managed_oauth_pure."""
    tls = state.get("tls") or {}
    return refresh_managed_oauth_pure(
        state.get("access_token", ""),
        state.get("refresh_token", ""),
        state.get("client_id", "hairball-cli"),
...
def persist_managed_credentials(
    ...
    *,
    ...
    """Persist managed OAuth credentials as the singleton provider state
    and ensure the credential pool is in sync.

    managed credentials are read at runtime from two independent locations:
    ...
    Historically ``hairball auth add managed`` wrote a ``manual:device_code`` pool
    ...
    ``hairball auth add managed --label <name>``).  It gets embedded in the
    ...
    # these credentials via `hairball auth add managed --type oauth`. Best-
...
def _sync_managed_pool_from_auth_store() -> None:
    ...
    try:
        ...
        logger.debug("Failed to sync managed credential pool from auth store: %s", exc)
...
def resolve_managed_runtime_credentials(
    *,
    ...
    """
    Resolve managed inference credentials for runtime use.
    ...
    with _auth_store_lock():
        ...
        if not state:
            raise AuthError("Hairball is not logged into Managed Portal.",
        ...
        def _resolve_effective_routing_metadata() -> tuple[str, str, str, str]:
            ...
            if env_portal_override:
                ...
                if (
                    not portal_host
                    or portal_host not in _MANAGED_PORTAL_ALLOWED_HOSTS
                    ...
                    portal_url = DEFAULT_MANAGED_PORTAL_URL
            ...
            stored_inference_url = (
                _validate_managed_inference_url_from_network(
                    _optional_base_url(state.get("inference_base_url"))
                )
                or DEFAULT_MANAGED_INFERENCE_URL
            )
            effective_inference_url = (
                _managed_inference_env_override() or stored_inference_url
            )
            effective_client_id = str(
                state.get("client_id") or DEFAULT_MANAGED_CLIENT_ID
            )
            return (
                portal_url,
                stored_inference_url,
        ...
        def _persist_state(reason: str) -> None:
            ...
            # the mtime-keyed managed auth-status cache warm during read paths.
            if (
        ...
        with httpx.Client(timeout=timeout, headers={"Accept": "application/json"}, verify=verify) as client:
            ...
            if not isinstance(access_token, str) or not access_token:
                raise AuthError("No access token found for Managed Portal login.",
                                provider="managed", relogin_required=True)
            ...
            if force_refresh or invoke_jwt_status is not None:
                with _managed_shared_store_lock(timeout_seconds=max(timeout_seconds + 5.0, AUTH_LOCK_TIMEOUT_SECONDS)):
                    ...
                    if force_refresh or invoke_jwt_status is not None:
                        ...
                        state["token_type"] = refreshed.get("token_type") or state.get("token_type") or "Bearer"
                        state["scope"] = refreshed.get("scope") or state.get("scope")
                        # Heal a poisoned stored value (see refresh_managed_oauth_pure):
                        ...
                        # persisted to auth.json below. The MANAGED_INFERENCE_BASE_URL
                        # env override is layered on for the client/return value
                        # only (see below) — it is never persisted.
                        refreshed_url = _validate_managed_inference_url_from_network(refreshed.get("inference_base_url"))
                        stored_inference_base_url = refreshed_url or DEFAULT_MANAGED_INFERENCE_URL
                        inference_base_url = (
                            _managed_inference_env_override() or stored_inference_base_url
                        )
    ...
    if not isinstance(api_key, str) or not api_key:
        raise AuthError("Failed to resolve a managed inference API key",
...
# which does a synchronous OAuth refresh POST to portal.example.invalid. That can take
# ~350ms even on the failure path, and read-only UI surfaces (`hairball tools`, status panels,
# subscription-feature checks) call it many times per render — `hairball tools` → "All Platforms"
...
# `hairball auth login/logout/add/remove` invalidate naturally on the next call.
_MANAGED_AUTH_STATUS_CACHE_TTL = 15.0  # seconds
_managed_auth_status_cache: Optional[Tuple[float, str, Optional[float], Dict[str, Any]]] = None
...
def invalidate_managed_auth_status_cache() -> None:
    ...
    Call this from any code path that mutates managed auth state without going
...
def get_managed_auth_status() -> Dict[str, Any]:
    """Status snapshot for managed auth.
    ...
    _managed_auth_status_cache = (now, auth_file_key, mtime, dict(status))
    return status
...
# Enum values reported on the dashboard /api/status as ``managed_session_valid``.
...
MANAGED_SESSION_VALID = "valid"
MANAGED_SESSION_TERMINAL = "terminal"
MANAGED_SESSION_UNKNOWN = "unknown"


def get_managed_session_validity() -> str:
    """Classify the managed bootstrap session for the dashboard /api/status probe.

    Returns one of:
      - ``"valid"``    — a usable managed credential is present (login healthy).
      - ``"terminal"`` — the managed session has taken a terminal auth failure
      ...
      - ``"unknown"``  — indeterminate (no managed provider state, or a transient/
    ...
    # into the managed provider state when it clears dead tokens (the exact path
    ...
    if state:
        ...
        if isinstance(last_err, dict) and last_err.get("relogin_required"):
            ...
            if not (state.get("access_token") or state.get("refresh_token")):
                return MANAGED_SESSION_TERMINAL

    try:
        status = get_managed_auth_status()
    except Exception:
        # Status computation itself failed — indeterminate, not terminal.
        return MANAGED_SESSION_UNKNOWN

    if status.get("logged_in"):
        return MANAGED_SESSION_VALID
    ...
    # No managed provider state at all, or a non-terminal not-logged-in condition
...
def get_codex_auth_status() -> Dict[str, Any]:
    """Status snapshot for reference auth.
    
    Checks the credential pool first (where `hairball auth` stores credentials),
    then falls back to the legacy provider state.
    """
    # Check credential pool first — this is where `hairball auth` and
    # `hairball model` store device_code tokens.
    try:
        from agent.credential_pool import load_pool
            ...
            if rate_limit:
                return {
                    ...
                    "error": (
                        ...
                        or "reference provider quota exhausted; retry after the usage limit resets."
                    ),
...
def get_external_process_provider_status(provider_id: str) -> Dict[str, Any]:
    ...
    command = (
        os.getenv("HAIRBALL_COPILOT_ACP_COMMAND", "").strip()
        or os.getenv("COPILOT_CLI_PATH", "").strip()
        or "copilot"
    )
    raw_args = os.getenv("HAIRBALL_COPILOT_ACP_ARGS", "").strip()
...
def _get_azure_foundry_auth_status() -> Dict[str, Any]:
    ...
    checks:
      ...
      * ``auth_mode == "entra_id"`` AND ``azure-identity`` is importable
        (we do NOT mint a token here; ``hairball doctor`` runs the live
    ...
    try:
        from hairball_cli.config import load_config, get_env_value_prefer_dotenv
        cfg = load_config()
    except Exception:
        cfg = {}
        ...
        try:
            ...
            if not installed:
                info["hint"] = (
                    ...
                    "pip install azure-identity  (or rely on Hairball' "
                    "lazy-install at first use)."
                )
            else:
                info["hint"] = (
                    "azure-identity is installed; live credential validation "
                    "is skipped here. Run `hairball doctor` to verify token acquisition."
                )
            return info
        except Exception as exc:
...
def resolve_api_key_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if provider_id in {"kimi-coding", "kimi-coding-cn"}:
        ...
        try:
            from hairball_cli.copilot_auth import (
...
def resolve_external_process_provider_credentials(provider_id: str) -> Dict[str, Any]:
    ...
    if not resolved_command and not base_url.startswith("acp+tcp://"):
        raise AuthError(
            ...
            "Install GitHub Copilot CLI or set HAIRBALL_COPILOT_ACP_COMMAND/COPILOT_CLI_PATH.",
            provider=provider_id,
...
def _update_config_for_provider(
    ...
    # model.api_key.
    from hairball_cli.config import clear_model_endpoint_credentials
...
def _logout_default_provider_from_config() -> Optional[str]:
    ...
    `hairball logout` historically keyed off auth.json.active_provider only.
...
def _prompt_model_selection(
    ...
    """
    from hairball_cli.models import _format_price_per_mtok
...
def _save_model_choice(model_id: str) -> None:
    ...
    """
    from hairball_cli.config import save_config, load_config
...
def login_command(args) -> None:
    """Deprecated: use 'hairball model' or 'hairball setup' instead."""
    print("The 'hairball login' command has been removed.")
    print("Use 'hairball auth' to manage credentials,")
    print("'hairball model' to select a provider, or 'hairball setup' for full setup.")
    raise SystemExit(0)
...
def _login_openai_codex(
    ...
    *,
    ...
    """ChatGPT login via device code flow. Tokens stored in ~/.hairball/auth.json."""

    del args, pconfig  # kept for parity with other provider login helpers

    # Check for existing Hairball-owned credentials
    if not force_new_login:
        try:
            ...
            if isinstance(_resolved_key, str) and _resolved_key and not _codex_access_token_is_expiring(_resolved_key, 60):
                print("Existing reference credentials found in Hairball auth store.")
                try:
                ...
                print("Existing reference credentials are expired. Starting fresh login...")
        except AuthError:
            pass

    # Check for existing reference CLI tokens we can import
    if not force_new_login:
        cli_tokens = _import_codex_cli_tokens()
        if cli_tokens:
            print("Found existing reference CLI credentials at ~/.codex/auth.json")
            print("Hairball will create its own session to avoid conflicts with reference CLI / VS Code.")
            try:
            ...
            if do_import in {"y", "yes"}:
                ...
                base_url = os.getenv("HAIRBALL_CODEX_BASE_URL", "").strip().rstrip("/") or DEFAULT_CODEX_BASE_URL
                config_path = _update_config_for_provider("openai-codex", base_url)
                print()
                print("Credentials imported. Note: if reference CLI refreshes its token,")
                print("Hairball will keep working independently with its own session.")
                print(f"  Config updated: {config_path} (model.provider=openai-codex)")
                return

    # Run a fresh device code flow — Hairball gets its own OAuth session
    print()
    print("Signing in to ChatGPT...")
    print("(Hairball creates its own session — won't affect reference CLI or VS Code)")
    print()

    creds = _codex_device_code_login()

    # Save tokens to Hairball auth store
...
def _login_xai_oauth(
    ...
    *,
    ...
    if not force_new_login:
        try:
            ...
            if isinstance(api_key, str) and api_key and not _xai_access_token_is_expiring(api_key, 60):
                print("Existing xAI OAuth credentials found in Hairball auth store.")
                try:
                    reuse = input("Use existing credentials? [Y/n]: ").strip().lower()
                except (EOFError, KeyboardInterrupt):
                    reuse = "y"
    ...
    print("(Hairball creates its own local OAuth session)")
    print()
    ...
    # xAI credential re-enabled. ``hairball auth remove xai-oauth`` leaves a
    # ``device_code`` suppression marker that otherwise stops the singleton
    # seed from re-creating the pool entry, so ``hairball auth list`` would show
...
def _codex_device_code_login() -> Dict[str, Any]:
    ...
    if resp is not None and resp.status_code == 429:
        ...
        raise AuthError(
            "OpenAI is rate-limiting reference login requests (HTTP 429). "
    ...
    if token_resp.status_code == 429:
        ...
        raise AuthError(
            "OpenAI is rate-limiting reference login requests (HTTP 429) during "
...
def _minimax_poll_token(
    ...
    # legacy treats expired_in as a unix-ms timestamp (Date.now() < expireTimeMs).
...
def _minimax_save_auth_state(auth_state: Dict[str, Any]) -> None:
    """Persist MiniMax OAuth state to Hairball auth store (~/.hairball/auth.json)."""
    with _auth_store_lock():
        auth_store = _load_auth_store()
...
def _minimax_oauth_login(
    *, region: str = "global", open_browser: bool = True,
    ...
    print(f"Starting Hairball login via MiniMax ({region}) OAuth...")
...
def _minimax_oauth_quarantine_on_terminal_refresh(state: Dict[str, Any], exc: AuthError) -> None:
    ...
    provider. Mirrors the managed / xAI-OAuth / reference-OAuth quarantine pattern
    so subsequent calls fail fast without a network retry.
    """
...
def build_minimax_oauth_token_provider() -> Callable[[], str]:
    ...
    def _provide() -> str:
        ...
        if not state or not state.get("access_token"):
            raise AuthError(
                "Not logged into MiniMax OAuth. Run `hairball model` and select "
                "MiniMax (OAuth).",
...
def resolve_minimax_oauth_runtime_credentials(
    *, min_token_ttl_seconds: int = MINIMAX_OAUTH_REFRESH_SKEW_SECONDS,
    ...
    diagnostic call sites like ``hairball status`` that just want to know
...
def _managed_device_code_login(
    *,
    ...
    """Run the managed device-code flow and return full OAuth state without persisting."""
    ...
    print(f"Starting Hairball login via {pconfig.name}...")
    print(f"Portal: {portal_base_url}")
    if insecure:
    ...
    try:
        ...
        if exc.code == "subscription_required":
            ...
            print()
            print("After subscribing, run `hairball model` again to finish setup.")
            raise SystemExit(1)
        raise


def managed_token_has_billing_scope() -> bool:
    """Return True if the currently-held managed token carries ``billing:manage``.
    ...
    if not isinstance(scope, str):
        return False
    return MANAGED_BILLING_MANAGE_SCOPE in scope.split()


def step_up_managed_billing_scope(
    *,
    open_browser: bool = True,
    timeout_seconds: float = 15.0,
    on_verification: Optional[Callable[[str, str], None]] = None,
    ...
    targets the same deployment (incl. a preview via ``HAIRBALL_PORTAL_BASE_URL`` set
    at the original login). Persists to the auth store + shared store + pool, exactly
    like ``_login_managed`` — but WITHOUT the model picker (this is a scope upgrade, not
    a fresh login).

    Returns True iff the new token carries ``billing:manage``.
    """
    prior = get_provider_auth_state("managed") or {}
    pconfig = PROVIDER_REGISTRY["managed"]
    ...
    requested: list[str] = []
    for tok in (prior_scope.split() or [MANAGED_INFERENCE_INVOKE_SCOPE, "tool:invoke"]):
        if tok and tok not in requested:
            requested.append(tok)
    if MANAGED_BILLING_MANAGE_SCOPE not in requested:
        requested.append(MANAGED_BILLING_MANAGE_SCOPE)
    ...
    # Mirror to shared store + reseed the pool (best-effort), same as _login_managed.
    try:
    ...
    return isinstance(granted, str) and MANAGED_BILLING_MANAGE_SCOPE in granted.split()


def _login_managed(args, pconfig: ProviderConfig) -> None:
    """Managed Portal device authorization flow."""
    ...
    try:
        ...
        # session-style auto-import: before launching a fresh device-code
        # flow, check the shared store for an existing managed credential
        ...
        if shared:
            ...
            print()
            if shared_path:
                print(f"Found existing managed OAuth credentials at {shared_path}")
            else:
                print("Found existing shared managed OAuth credentials")
            try:
                do_import = input("Import these credentials? [Y/n]: ").strip().lower()
            except (EOFError, KeyboardInterrupt):
                do_import = "y"
            if do_import in {"", "y", "yes"}:
                print("Rehydrating managed session from shared credentials...")
        ...
        try:
            ...
            if model_ids:
                ...
                if free_tier:
                    try:
                        ...
                        _account_info = get_managed_portal_account_info(force_fresh=True)
                        unavailable_message = (
                            format_managed_portal_entitlement_message(
                                _account_info,
                                capability="paid managed models",
                            )
                            or ""
                        )
                    except Exception:
                    ...
                    # as free so users on older Hairball builds still see
            ...
            if model_ids:
                ...
                print("No curated models available for Managed Portal.")
        except Exception as exc:
        ...
        # them to managed with a mismatched model.  The managed OAuth tokens
        ...
        if not selected_model:
            ...
            with _auth_store_lock():
                ...
                _save_auth_store(auth_store)
            print()
            print("No provider change. managed credentials saved for future use.")
            print("  Run `hairball model` again to switch to Managed Portal.")
            return
...
def logout_command(args) -> None:
    ...
    if clear_provider_auth(target) or should_reset_config:
        ...
        if should_reset_config and os.getenv("OPENROUTER_API_KEY"):
            print("Hairball will use OpenRouter for inference.")
        elif should_reset_config:
            print("Run `hairball model` or configure an API key to use Hairball.")
        else:
