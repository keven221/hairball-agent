"""
hairball fallback — manage the fallback provider chain.
...
Subcommands:
  hairball fallback [list]   Show the current fallback chain (default when no subcommand)
  hairball fallback add      Pick provider + model via the same picker as `hairball model`,
                           then append the selection to the chain
  hairball fallback remove   Pick an entry to delete from the chain
  hairball fallback clear    Remove all fallback entries

Storage: ``fallback_providers`` in ``~/.hairball/config.yaml`` (top-level, list of
...
def _restore_auth_active_provider(value: Any) -> None:
    ...
    try:
        from hairball_cli.auth import _auth_store_lock, _load_auth_store, _save_auth_store
        ...
        # `hairball model` to fix it.  Don't fail the fallback add.
        pass
...
def cmd_fallback_list(args) -> None:  # noqa: ARG001
    ...
    if not chain:
        ...
        print()
        print("  Add one with:  hairball fallback add")
        print()
        return
...
def cmd_fallback_add(args) -> None:
    """Launch the same picker as `hairball model`, then append the selection to the chain."""
    from hairball_cli.main import _require_tty, select_provider_and_model
    ...
    print("  `hairball model` — select the provider + model you want as a fallback.")
    print()
    ...
    print()
    print("  Run `hairball fallback list` to view, or `hairball fallback remove` to delete.")
...
def cmd_fallback(args) -> None:
    """Top-level dispatcher for ``hairball fallback [subcommand]``."""
