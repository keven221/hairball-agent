...
- ``list_authenticated_providers`` already populates each row's
  ...
  curation and pulls in non-agentic models (managed /models returns ~400
...
def build_models_payload(
    ...
    *,
    ...
    - ``pricing``: enrich each row with formatted per-model pricing and,
      for managed, ``free_tier``/``unavailable_models`` so the GUI picker can
      show $/Mtok columns and gate paid models on free accounts —
      mirroring the ``hairball model`` CLI picker. Adds network calls
      (pricing fetch + managed tier check); only set for interactive pickers.
    ...
    - ``force_fresh_managed_tier``: bypass the short managed free-tier cache when
      selecting Portal-recommended managed models and applying tier gating. Keep
    ...
    rows = list_authenticated_providers(
        ...
        force_fresh_managed_tier=force_fresh_managed_tier,
    ...
    try:
        from hairball_cli.providers import is_routing_aggregator as _is_routing_aggregator
    except Exception:
    ...
    if pricing:
        _apply_pricing(rows, force_fresh_managed_tier=force_fresh_managed_tier)
    if capabilities:
...
def _append_unconfigured_rows(
    ...
    *,
    ...
    from hairball_cli.models import CANONICAL_PROVIDERS, _PROVIDER_LABELS
    ...
    for entry in CANONICAL_PROVIDERS:
        ...
        if entry.slug.lower() == cur:
            ...
            warning = (
                ...
                else "Configured provider is not authenticated; run `hairball model` to reactivate. "
                "Showing the saved model only."
            )
            extras.append(
                {
...
def _filter_explicit_provider_rows(rows: list[dict], ctx: ConfigContext) -> list[dict]:
    ...
    pickers want the narrower subset the user explicitly configured for Hairball.
    """
...
def _apply_picker_hints(rows: list[dict]) -> None:
    ...
    for row in rows:
        ...
        row["warning"] = (
            ...
            if auth_type == "api_key" and key_env
            else f"run `hairball model` to configure ({auth_type})"
        )
...
def _reorder_canonical(rows: list[dict]) -> list[dict]:
    ...
    """
    from hairball_cli.models import CANONICAL_PROVIDERS
...
def _apply_pricing(
    ...
    *,
    ...
    """Enrich each provider row with per-model pricing + managed tier gating.
    ...
    # Resolve managed free-tier once (cached in models.py for the TTL window).
        ...
        if slug == "managed":
            try:
                if managed_free_tier is None:
                    managed_free_tier = check_managed_free_tier(
                        force_fresh=force_fresh_managed_tier
                    )
...
def _moa_provider_row(current_provider: str = "") -> dict | None:
    ...
    picker path (:func:`hairball_cli.model_switch.list_picker_providers`) so the
