"""
Backup and import commands for hairball CLI.

`hairball backup` creates a zip archive of the entire ~/.hairball/ directory
(excluding the hairball-agent repo and transient files).

`hairball import` restores from a backup zip, overlaying onto the current
HAIRBALL_HOME root.
"""
...
from hairball_constants import get_default_hairball_root, get_hairball_home, display_hairball_home
...
# ``hairball-agent`` is special-cased to root level only in ``_should_exclude``
# so that skill directories like ``skills/autonomous-ai-agents/hairball-agent/``
...
# HAIRBALL_HOME gets walked file-by-file, ballooning a backup to hundreds of
...
_EXCLUDED_DIRS = {
    "hairball-agent",     # the codebase repo — re-clone instead
    ...
    # Python dependency trees (plugin / MCP-server venvs under HAIRBALL_HOME) —
...
# File names that ``hairball import`` must never overwrite, matched by basename so
...
#     stuck "starting"/"cooking", disconnecting it from the Managed Portal
...
# Reserved archive subtree for provider state that lives OUTSIDE HAIRBALL_HOME
...
def _collect_memory_provider_external_paths() -> List[Path]:
    ...
    outside HAIRBALL_HOME, resolved from config only (no network, no init).
...
def _should_exclude(rel_path: Path) -> bool:
    """Return True if *rel_path* (relative to hairball root) should be skipped."""
    parts = rel_path.parts

    for part in parts:
        if part not in _EXCLUDED_DIRS:
            continue
        # ``hairball-agent`` only matches at the root level (first component).
        # Nested directories with the same name — e.g.
        # ``skills/autonomous-ai-agents/hairball-agent/`` — must be preserved.
        if part == "hairball-agent" and part != parts[0]:
            continue
        return True
...
def _should_skip_backup_file(abs_path: Path, rel_path: Path, out_path: Path) -> bool:
    ...
    # write can copy data from outside HAIRBALL_HOME.
...
def run_backup(args) -> None:
    """Create a zip backup of the Hairball home directory."""
    hairball_root = get_default_hairball_root()

    if not hairball_root.is_dir():
        print(f"Error: Hairball home directory not found at {hairball_root}")
        sys.exit(1)
    ...
    if args.output:
        ...
        if out_path.is_dir():
            ...
            out_path = out_path / f"hairball-backup-{stamp}.zip"
    else:
        stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
        out_path = Path.home() / f"hairball-backup-{stamp}.zip"
    ...
    # Collect files
    print(f"Scanning {display_hairball_home()} ...")
    files_to_add: list[tuple[Path, Path]] = []  # (absolute, relative)
    skipped_dirs = set()

    for dirpath, dirnames, filenames in os.walk(hairball_root, followlinks=False):
        dp = Path(dirpath)
        rel_dir = dp.relative_to(hairball_root)

        # Prune excluded directories in-place so os.walk doesn't descend
        # ``hairball-agent`` is only pruned at the root level; nested dirs
        # with the same name (e.g. in skills/) must be preserved.
        is_root = rel_dir == Path(".")
        orig_dirnames = dirnames[:]
        dirnames[:] = [
            d for d in dirnames
            if d not in _EXCLUDED_DIRS or (d == "hairball-agent" and not is_root)
        ]
        ...
        for fname in filenames:
            fpath = dp / fname
            rel = fpath.relative_to(hairball_root)
    ...
    # outside HAIRBALL_HOME, so the walk above never sees it. Ask the active
    ...
    if external_to_add:
        print(
            ...
            f"stored outside {display_hairball_home()}."
        )
    ...
    print(f"\nRestore with: hairball import {out_path.name}")
...
def _validate_backup_zip(zf: zipfile.ZipFile) -> tuple[bool, str]:
    """Check that a zip looks like a Hairball backup.
    ...
    # Look for telltale files that a hairball home would have
    ...
    if not found:
        return False, (
            "zip does not appear to be a Hairball backup "
...
def _detect_prefix(zf: zipfile.ZipFile) -> str:
    ...
    Some tools zip as `.hairball/config.yaml` instead of `config.yaml`.
    ...
    if len(first_parts) == 1:
        ...
        # Only strip if it looks like a hairball dir name
        if prefix in {".hairball", "hairball"}:
            return prefix + "/"
...
def run_import(args) -> None:
    ...
    with zipfile.ZipFile(zip_path, "r") as zf:
        ...
        print(f"Target: {display_hairball_home()}")

        if prefix:
            print(f"Detected archive prefix: {prefix!r} (will be stripped)")

        # Check for existing installation
        has_config = (hairball_root / "config.yaml").exists()
        has_env = (hairball_root / ".env").exists()

        if (has_config or has_env) and not args.force:
            print()
            print("Warning: Target directory already has Hairball configuration.")
        ...
        hairball_root.mkdir(parents=True, exist_ok=True)
        ...
        for member in members:
            ...
            # location (e.g. ~/.honcho/config.json), NOT under HAIRBALL_HOME.
            ...
            # Managed Portal. Matched by basename so both the root profile and
            # named profiles (profiles/<name>/gateway_state.json) are covered.
            if Path(rel).name in _IMPORT_SKIP_NAMES:
                skipped_runtime.append(rel)
                continue

            target = hairball_root / rel

            # Security: reject absolute paths and traversals
            try:
                target.resolve().relative_to(hairball_root.resolve())
            except ValueError:
        ...
        if restored_external:
            print(
                ...
                f"their original location(s) outside {display_hairball_home()}."
            )
        ...
        profiles_dir = hairball_root / "profiles"
        restored_profiles = []
        if profiles_dir.is_dir():
            try:
                ...
                # hairball_cli.profiles might not be available (fresh install)
                if any(profiles_dir.iterdir()):
                    print("\n  Profiles detected but aliases could not be created.")
                    print("  Run: hairball profile list  (after installing hairball)")

        # Guidance
        print()
        if not (hairball_root / "hairball-agent").is_dir():
            print("Note: The hairball-agent codebase was not included in the backup.")
            print("  If this is a fresh install, run: hairball update")

        if restored_profiles:
            gw_profiles = [n for n, _ in restored_profiles]
            print("\nTo re-enable gateway services for profiles:")
            for pname in gw_profiles:
                print(f"  hairball -p {pname} gateway install")

        print("Done. Your Hairball configuration has been restored.")


# ---------------------------------------------------------------------------
# Quick state snapshots (used by /snapshot slash command and hairball backup --quick)
# ---------------------------------------------------------------------------

# Critical state files to include in quick snapshots (relative to HAIRBALL_HOME).
...
# — `hairball update` snapshots this set before pulling so approved-user lists
...
_QUICK_STATE_FILES = (
    ...
    # are at $HAIRBALL_HOME/<name> for the default/root profile; on non-root
    # profiles the real path is outside HAIRBALL_HOME and the entry is silently
...
def _quick_snapshot_root(hairball_home: Optional[Path] = None) -> Path:
...
def list_quick_snapshots(
    ...
    root = _quick_snapshot_root(hairball_home)
    if not root.exists():
        return []
...
# Relative path of the cron job database inside HAIRBALL_HOME. Kept in sync with
...
def restore_cron_jobs_if_emptied(
    ...
    """Safety net for silent cron-job loss across ``hairball update``.
    ...
    Args:
        ...
        hairball_home: Override for the Hairball home directory (tests).
...
def prune_quick_snapshots(
    ...
    return _prune_quick_snapshots(_quick_snapshot_root(hairball_home), keep=keep)


def run_quick_backup(args) -> None:
    """CLI entry point for hairball backup --quick."""
    label = getattr(args, "label", None)
    snap_id = create_quick_snapshot(label=label)
    if snap_id:
        print(f"State snapshot created: {snap_id}")
        snaps = list_quick_snapshots()
        print(f"  {len(snaps)} snapshot(s) stored in {display_hairball_home()}/state-snapshots/")
...
def _write_full_zip_backup(out_path: Path, hairball_root: Path) -> Optional[Path]:
    """Write a full zip snapshot of ``hairball_root`` to ``out_path``.
...
def _pre_update_backup_dir(hairball_home: Optional[Path] = None) -> Path:
    home = hairball_home or get_hairball_home()
...
def create_pre_update_backup(
    ...
    """Create a full zip backup of HAIRBALL_HOME under ``backups/``.

    Mirrors :func:`run_backup` (same exclusion rules, same SQLite safe-copy)
    but writes to ``<HAIRBALL_HOME>/backups/pre-update-<timestamp>.zip`` and
    auto-prunes old pre-update backups.

    Returns the path to the created zip, or ``None`` if no files were
    found or the backup could not be created.  Never raises — the caller
    (``hairball update``) should continue even if the backup fails.
    """
    hairball_root = hairball_home or get_default_hairball_root()
    if not hairball_root.is_dir():
        return None

    backup_dir = _pre_update_backup_dir(hairball_root)
    try:
    ...
    result = _write_full_zip_backup(out_path, hairball_root)
    if result is None:
        return None
...
# ---------------------------------------------------------------------------
# Pre-migration auto-backup (used by `hairball claw migrate`)
# ---------------------------------------------------------------------------
...
def create_pre_migration_backup(
    ...
    """Create a full zip backup of HAIRBALL_HOME under ``backups/`` before a
    ``hairball claw migrate`` apply.

    Shares implementation with :func:`create_pre_update_backup` via
    ``_write_full_zip_backup`` — same exclusions, same SQLite safe-copy,
    restorable with ``hairball import <archive>``.  Writes to
    ``<HAIRBALL_HOME>/backups/pre-migration-<timestamp>.zip`` and auto-prunes
    ...
    # Reuses the shared backups/ directory so `hairball import` and the
