"""
Unified tool configuration for Hairball Agent.

`hairball tools` and `hairball setup tools` both enter this module.
Select a platform → toggle toolsets on/off → for newly enabled tools
that need API keys, run through provider-aware configuration.

Saves per-platform tool configuration to ~/.hairball/config.yaml under
...
from hairball_cli.managed_subscription import (
    apply_managed_managed_defaults,
    get_managed_subscription_features,
)
from hairball_cli.managed_account import format_managed_portal_entitlement_message
...
from hairball_cli.cli_output import (  # noqa: E402 — late import block
...
# They're still in _HAIRBALL_CORE_TOOLS (available at runtime if enabled),
# but the setup checklist won't pre-select them for first-time users.
#
# Video gen is off by default — it's a niche, paid, slow feature. Users
# who want it opt in via `hairball tools` → Video Generation, which walks
...
# `hairball tools` → X (Twitter) Search setup walks users through credential
...
def _xai_credentials_present() -> bool:
    ...
    """
    try:
        from hairball_cli.auth import _read_xai_oauth_tokens
...
# Platform-scoped toolsets: only appear in the `hairball tools` checklist for
...
def _get_effective_configurable_toolsets():
    ...
    Without the dedupe, ``hairball tools`` → "reconfigure existing" would
    list the same toolset twice.
    """
    result = list(CONFIGURABLE_TOOLSETS)
    seen = {ts_key for ts_key, _, _ in result}
    try:
        from hairball_cli.plugins import discover_plugins, get_plugin_toolsets
...
def _checklist_toolset_keys(platform: str) -> Set[str]:
    """Return the toolset keys the ``hairball tools`` checklist actually offers
    for ``platform``.
    ...
    so ``hairball tools`` never claims to add or remove a toolset the user was
...
from hairball_cli.platforms import PLATFORMS as _PLATFORMS_REGISTRY
...
TOOL_CATEGORIES = {
    "tts": {
        ...
        "providers": [
            ...
            },
            {
                ...
                "env_vars": [],
                "tts_provider": "openai",
                "requires_managed_auth": True,
                "managed_managed_feature": "tts",
    ...
    "web": {
        ...
        #   - "managed Subscription" — managed Firecrawl billed via managed
        #     subscription (requires_managed_auth + override_env_vars).
    ...
    "image_gen": {
        ...
        # ChatGPT, and xAI are injected at runtime from each
        ...
        #   - "managed Subscription" — managed FAL billed via the managed
        #     subscription (requires_managed_auth + override_env_vars).
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_managed_auth": True,
                "managed_managed_feature": "image_gen",
    ...
    "video_gen": {
        ...
        "icon": "🎬",
        # "managed Subscription" row mirrors the image_gen pattern — managed
        # FAL video generation billed via the Managed Portal.  Plugin-backed
        ...
        "providers": [
            {
                ...
                "env_vars": [],
                "requires_managed_auth": True,
                "managed_managed_feature": "video_gen",
                "override_env_vars": ["FAL_KEY"],
                # The underlying plugin backend — when the user picks
                # "managed Subscription" we set video_gen.provider = "fal"
    ...
    "x_search": {
        ...
        "setup_note": (
            "Hairball routes X searches through xAI's built-in x_search "
    ...
    "browser": {
        ...
        # backend, never on the paid managed Subscription gateway row:
        #   - "Local Browser" — non-cloud option, no CloudBrowserProvider.
        #   - "managed Subscription (Browser Use cloud)" — managed Browser Use
        #     billed via managed subscription (requires_managed_auth +
        ...
        "providers": [
            ...
            {
                "name": "managed Subscription (Browser Use cloud)",
                "badge": "subscription",
                "tag": "Managed Browser Use billed to your subscription",
                "env_vars": [],
                "browser_provider": "browser-use",
                "requires_managed_auth": True,
                "managed_managed_feature": "browser",
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    ...
                    # extra keys required. Set HAIRBALL_CUA_DRIVER_CMD to use a
        ...
        "providers": [
            {
                ...
                "env_vars": [
                    {"key": "HAIRBALL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)", "url": "https://cloud.langfuse.com"},
                    {"key": "HAIRBALL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)", "url": "https://cloud.langfuse.com"},
                ],
                "post_setup": "langfuse",
            },
            {
                ...
                "env_vars": [
                    {"key": "HAIRBALL_LANGFUSE_PUBLIC_KEY", "prompt": "Langfuse public key (pk-lf-...)"},
                    {"key": "HAIRBALL_LANGFUSE_SECRET_KEY", "prompt": "Langfuse secret key (sk-lf-...)"},
                    {"key": "HAIRBALL_LANGFUSE_BASE_URL", "prompt": "Langfuse server URL (e.g. http://localhost:3000)", "default": "http://localhost:3000"},
                ],
                "post_setup": "langfuse",
            },
        ],
...
# provider+model picker like `hairball model` — NOT this single-key prompt, so
...
def _cua_driver_cmd() -> str:
    ...
    return os.environ.get("HAIRBALL_CUA_DRIVER_CMD", "").strip() or "cua-driver"


def _cua_driver_env() -> dict:
    """cua-driver child env with the Hairball telemetry policy applied.
...
def install_cua_driver(upgrade: bool = False) -> bool:
    ...
    * ``upgrade=True`` — always re-run the installer (or call ``cua-driver
      update`` if the binary supports it). Used by ``hairball update`` and
      by ``hairball computer-use install --upgrade``.
    ...
    if system not in ("Darwin", "Windows", "Linux"):
        if upgrade:
            # Silent on unsupported platforms — `hairball update` calls this
    ...
    if not _cua_install_target_writable():
        ...
        _print_info(
            "    Run `hairball computer-use install --upgrade` from an admin account to update it."
        )
        return bool(binary)
...
def _run_cua_driver_installer(label: str = "Installing", verbose: bool = True) -> bool:
    ...
    * Windows       → ``powershell -NoProfile -ExecutionPolicy Bypass -Command
    ...
    try:
        # When not verbose (e.g. `hairball update`'s refresh), capture the
        ...
        if verbose:
            ...
            # Preserve the full installer output. During `hairball update`,
            # sys.stdout is the mirroring _UpdateOutputStream whose `_log`
            # handle is ~/.hairball/logs/update.log — write straight to it so
        ...
        if result.returncode == 0 and shutil.which(driver_cmd):
            if verbose:
                ...
                if is_windows:
                    ...
                    _print_info("      System Settings > Privacy & Security > Accessibility")
                    _print_info("      System Settings > Privacy & Security > Screen Recording")
                    _print_info("    Both must allow the terminal / Hairball process.")
            return True
    ...
    if post_setup_key in {"agent_browser", "browserbase"}:
        ...
        if not node_modules.exists() and npm_bin:
            ...
            if result.returncode == 0:
                ...
                _print_warning(f"    npm install failed - run manually: cd {display_hairball_home()}/hairball-agent && npm install --workspaces=false")
                if result.stderr:
                    _print_info(f"      {result.stderr.strip()[:200]}")
        elif not node_modules.exists():
            _print_warning("    Node.js not found - browser tools require: npm install (in hairball-agent directory)")
            return
        ...
        if _running_in_docker():
            ...
            _print_info(
                "      docker pull ghcr.io/hairball-agent/hairball-agent:latest"
            )
            return
        ...
        try:
            result = _run_chromium_installer(
                install_cmd,
        ...
        _print_info("    Switch voices by setting tts.piper.voice in ~/.hairball/config.yaml")
        ...
        # Run the full `hairball auth spotify` flow — if the user has no
        # client_id yet, this drops them into the interactive wizard
        # (opens the Spotify dashboard, prompts for client_id, persists
        # to ~/.hairball/.env), then continues straight into PKCE. If they
        # already have an app, it skips the wizard and just does OAuth.
        from types import SimpleNamespace
        try:
            from hairball_cli.auth import login_spotify_command
        except Exception as exc:
            _print_warning(f"    Could not load Spotify auth: {exc}")
            _print_info("    Run manually: hairball auth spotify")
            return
        ...
        try:
            ...
            # toolset enable; they can retry with `hairball auth spotify`.
            _print_warning(f"    Spotify login did not complete: {exc}")
            _print_info("    Run later: hairball auth spotify")
        except Exception as exc:
        ...
        try:
            from hairball_cli.plugins_cmd import _get_enabled_set, _save_enabled_set
            ...
            _print_info("    Run manually: hairball plugins enable observability/langfuse")
        _print_info("    Restart Hairball for tracing to take effect.")
        _print_info("    Verify: hairball plugins list")
        ...
        try:
            ...
            _print_info("    Run later: hairball auth add xai-oauth   (or set XAI_API_KEY)")
            return
        ...
        idx = prompt_choice(
            ...
            choices=[
                ...
                "Skip — configure later via `hairball auth add xai-oauth`",
            ],
            default=0,
        )
        if idx == 0:
            if _run_xai_oauth_login_from_setup():
                ...
                _print_warning(
                    ...
                    "Run later: hairball auth add xai-oauth"
                )
        elif idx == 1:
            ...
            if api_key:
                ...
                _print_warning(
                    "    No API key provided. Run later: hairball auth add xai-oauth"
                )
        else:
...
def valid_post_setup_keys() -> Set[str]:
    ...
    ``post_setup``). This is the allowlist the ``hairball tools post-setup``
...
def run_post_setup_command(args) -> int:
    """``hairball tools post-setup <key>`` — non-interactive post-setup runner.
    ...
    if not key:
        _print_error("Usage: hairball tools post-setup <key>")
        return 2
...
def _exempt_explicit_platform_native(
    ...
    the composite they chose (e.g. ``hairball-discord``, which contains those
...
def _get_platform_tools(
    ...
    *,
    ...
    # ``hairball-discord``) is an opt-in to the platform's native default-off
    ...
    if toolset_names is None or not isinstance(toolset_names, list):
        ...
        if plat_info:
            ...
            default_ts = f"hairball-{platform}"
    ...
    # "hairball-cli" (which include all _HAIRBALL_CORE_TOOLS) cause disabled
    ...
    if has_explicit_config:
        ...
        # ``[hairball-cli, spotify]`` after enabling Spotify via ``hairball
        ...
        # (e.g. "hairball-cli") to individual tool names and reverse-mapping.
        all_tool_names = set()
        for ts_name in toolset_names:
        ...
        # through ``hairball tools`` to flip the toolset on. Only fires when
    ...
    # otherwise saving via `hairball tools` (which flips has_explicit_config
    # to True) silently drops them.
    _plat_info = PLATFORMS.get(platform)
    _default_ts = _plat_info["default_toolset"] if _plat_info else f"hairball-{platform}"
    ...
    skip |= {k for k in TOOLSETS if k.startswith("hairball-")}
    ...
    # shipped as a bundled plugin but user must opt in via `hairball tools`
    # so we don't ship 7 Spotify tool schemas to users who don't use it).
    # A plugin toolset is "known" for a platform once `hairball tools`
    ...
    if plugin_ts_keys:
        ...
        for pts in plugin_ts_keys:
            if pts in toolset_names:
                ...
                # New plugin not yet seen by hairball tools — default enabled
                enabled_toolsets.add(pts)
    ...
    # is invalid (e.g. a migration or hand-edit left `hairball` instead of
    # `hairball-cli`), resolve_toolset() returns [] for each and the platform ends
    ...
    # at runtime, not only during the next `hairball update`/`hairball doctor`.
    ...
    if isinstance(_explicit, list) and _explicit:
        ...
        if (
            ...
            logger.warning(
                ...
                "name(s): %s) - tools will be unavailable. Run `hairball tools` "
...
def _save_platform_tools(config: dict, platform: str, enabled_toolset_keys: Set[str]):
    ...
    # Also exclude platform default toolsets (hairball-cli, hairball-telegram, etc.)
    ...
    # Opening `hairball tools` is the user's opt-in to reconfigure tools, so treat
...
def _toolset_has_keys(
    ...
    *,
    ...
    if ts_key in {"web", "image_gen", "video_gen", "tts", "browser"}:
        ...
        if feature and (feature.available or feature.managed_by_managed):
            return True
...
# ("managed Subscription" managed-gateway entry, "Firecrawl Self-Hosted")
...
# ("managed Subscription", "Local Browser", "Camofox") — see the comment block
...
def _visible_providers(
    ...
    *,
    ...
    managed-managed Tool Gateway rows (``managed_managed_feature``) are always
    shown — even to logged-out / unentitled users — so the picker advertises
    that the capability exists.  Selecting one drives an inline Managed Portal
    ...
    *activates* the gateway once paid access is confirmed.
    ...
    for provider in cat.get("providers", []):
        # managed-managed Tool Gateway rows stay visible regardless of auth —
        # selecting one drives an inline Portal login. A `requires_managed_auth`
        ...
        if (
            ...
            and not provider.get("managed_managed_feature")
            and not features.managed_auth_present
        ):
            continue
        ...
        if (
            pool_only
            and provider.get("managed_managed_feature") == "video_gen"
    ...
    # later) so the picker lists them alongside FAL / managed Subscription.
    ...
    # remaining hardcoded rows ("managed Subscription", "Firecrawl
    ...
    # the hardcoded "managed Subscription" / "Local Browser" / "Camofox" rows
...
def _hidden_managed_gateway_message(
    ...
    *,
    ...
    category when its managed-managed rows were filtered out for unentitled
    ...
    (``ensure_managed_portal_access``). Kept as a no-op so call sites stay simple;
...
_POST_SETUP_INSTALLED: dict = {
    ...
    # who toggles the toolset on via `hairball tools` gets a silent no-op
...
def _configure_tool_category(
    ...
    *,
    ...
    hidden_managed_message = _hidden_managed_gateway_message(
        cat,
        config,
        f"the managed Subscription provider for {name}",
    ...
    if len(providers) == 1:
        ...
        # When the user is logged into managed, surface a marker on providers
        # whose access is included in their subscription so it's visually
        # obvious which options cost extra vs. cost nothing on top of managed.
        try:
            _managed_logged_in = bool(
                get_managed_subscription_features(
                    config,
                    force_fresh=force_fresh,
                ).managed_auth_present
            )
        except Exception:
        ...
        for p in providers:
            ...
            # Mark managed-managed entries. Logged-in paid subscribers get the
            # "included" star; everyone else gets a "via Managed Portal" hint so
            ...
            sub_marker = ""
            if p.get("managed_managed_feature"):
                if _managed_logged_in:
                    sub_marker = "  ★ Included with your managed subscription"
                else:
                    sub_marker = "  ★ via Managed Portal (login on select)"
...
def _is_provider_active(
    ...
    *,
    ...
    if video_plugin_name and not provider.get("managed_managed_feature"):
        video_cfg = config.get("video_gen", {})
        return isinstance(video_cfg, dict) and video_cfg.get("provider") == video_plugin_name

    managed_feature = provider.get("managed_managed_feature")
    if managed_feature:
        features = get_managed_subscription_features(config, force_fresh=force_fresh)
        ...
        if managed_feature == "image_gen":
            ...
            if isinstance(image_cfg, dict):
                ...
                if image_cfg.get("use_gateway") is not None and not is_truthy_value(image_cfg.get("use_gateway"), default=False):
                    return False
            return feature.managed_by_managed
        ...
        if provider.get("tts_provider"):
            return (
                feature.managed_by_managed
                and cfg_get(config, "tts", "provider") == provider["tts_provider"]
            )
        if "browser_provider" in provider:
            current = cfg_get(config, "browser", "cloud_provider")
            return feature.managed_by_managed and provider["browser_provider"] == current
        if provider.get("web_backend"):
            current = cfg_get(config, "web", "backend")
            return feature.managed_by_managed and current == provider["web_backend"]
        return feature.managed_by_managed
...
def _write_provider_config(provider: dict, config: dict, *, managed_feature) -> None:
    ...
    NOT prompt for env vars, run post-setup hooks, gate on managed auth, or run
...
def apply_provider_selection(ts_key: str, provider_name: str, config: dict) -> None:
    ...
    post-setup hooks, gate on Managed Portal auth, or run interactive model
...
def _configure_provider(
    ...
    *,
    ...
    # managed-managed Tool Gateway backends are always listed (see
    # _visible_providers), but only *activate* once the user has paid managed
    ...
    # "enable all tools" prompt (that lives in `hairball model`).
    if managed_feature:
        from hairball_cli.managed_subscription import (
            MANAGED_FEATURE_COVERAGE_CATEGORY,
            ensure_managed_portal_access,
        )

        if not ensure_managed_portal_access(
            capability=f"{provider.get('name', 'the Managed Tool Gateway')}",
            coverage_category=MANAGED_FEATURE_COVERAGE_CATEGORY.get(managed_feature),
        ):
            _print_warning(
                "  Not enabled — Managed Portal access is required for this backend."
            )
            return
    ...
    if provider.get("requires_managed_auth") and not managed_feature:
        ...
        if not features.managed_auth_present or not entitled:
            message = format_managed_portal_entitlement_message(
                features.account_info,
                capability=f"{provider.get('name', 'managed Subscription')}",
            )
            _print_warning(
                f"  {message or 'managed Subscription is only available after logging into Managed Portal.'}"
            )
            return
    ...
    # managed-managed sibling, show a single dim hint so users know
    # they can avoid the key entirely via a Portal subscription.
    # Suppressed when the user is already authed to managed.
        ...
        try:
            ...
            for _cat_key, _cat in TOOL_CATEGORIES.items():
                ...
                if provider in _providers and any(
                    sib.get("managed_managed_feature") for sib in _providers
                ):
            ...
            if _has_managed_sibling:
                ...
                _show_portal_hint = not _features.managed_auth_present
        except Exception:
            _show_portal_hint = False

    if _show_portal_hint:
        _print_info("  Available through Managed Portal subscription.")
...
def _configure_vision_backend() -> None:
    ...
    provider + model — the same surface as ``hairball model`` — or point at a
...
def _configure_vision_provider_model(config: dict, vision_cfg: dict) -> None:
    ...
    if not providers:
        _print_warning(
            ...
            "with `hairball model`, then re-run this."
        )
        return
...
def _reconfigure_provider(
    ...
    *,
    ...
    # Same inline Managed Portal login + entitlement gate as _configure_provider:
...
def _reconfigure_simple_requirements(ts_key: str):
    ...
    if ts_key == "vision":
        ...
        # `hairball model`). Run it directly so reconfigure doesn't fall back to
...
def tools_command(args=None, first_install: bool = False, config: dict = None):
    """Entry point for `hairball tools` and `hairball setup tools`.
    ...
    if getattr(args, "summary", False):
        ...
        print(color("ฅ Tool Summary", Colors.CYAN, Colors.BOLD))
        print()
        ...
        print()
        return
    print(color("ฅ Hairball Tool Configuration", Colors.CYAN, Colors.BOLD))
    ...
    if first_install:
        for pkey in enabled_platforms:
            ...
            auto_configured = apply_managed_managed_defaults(
                config,
    ...
    print()
    from hairball_constants import display_hairball_home
    print(color(f"  Tool configuration saved to {display_hairball_home()}/config.yaml", Colors.DIM))
    print(color("  Changes take effect on next 'hairball' or gateway restart.", Colors.DIM))
    print()
...
def _configure_mcp_tools_interactive(config: dict):
    ...
    for server_name, tools in server_tools.items():
        ...
        # tools.include across the codebase (catalog installs, hairball mcp
