"""``hairball debug`` debug tools for Hairball Agent.

Currently supports:
    hairball debug share    Upload debug report (system info + logs) to a
                          ...
                          ``~/.hairball/logs/*.log`` are not leaked into
                          the public paste service. Pass ``--no-redact``
                          to disable.
                          Pass ``--managed`` to upload instead to managed-internal
                          storage (AWS S3) via a signed URL minted by the
                          managed account service: the bundle is private
                          (viewable only by managed staff / allowlisted mods via
...
_REDACTION_BANNER = (
    "[hairball debug share: log content redacted at upload time. "
...
def _pending_file() -> Path:
    """Path to ``~/.hairball/pastes/pending.json``.
...
def _save_pending(entries: list[dict]) -> None:
    ...
    try:
        ...
        # Non-fatal — worst case the user has to run ``hairball debug delete``
        # manually.
        pass
...
def _sweep_expired_pastes(now: Optional[float] = None) -> tuple[int, int]:
    ...
    intended to be called from every ``hairball debug`` invocation with
    minimal noise.
    """
    entries = _load_pending()
    if not entries:
...
_GATEWAY_PRIVACY_NOTICE = (
    ...
    "Full logs are NOT included from the gateway — use `hairball debug share` "
...
def delete_paste(url: str) -> bool:
    ...
    req = urllib.request.Request(
        ...
        headers={"User-Agent": "hairball-agent/debug-share"},
    )
...
def _schedule_auto_delete(urls: list[str], delay_seconds: int = _AUTO_DELETE_SECONDS):
    ...
    every ``hairball debug share`` invocation added ~20 MB of resident Python
    interpreters that never exited until the sleep completed.

    The replacement is stateless: we append to ``~/.hairball/pastes/pending.json``
    and the gateway's cron ticker sweeps expired entries once per hour.
    ``hairball debug share`` also runs an opportunistic sweep as a fallback
...
def _upload_paste_rs(content: str) -> str:
    ...
    req = urllib.request.Request(
        ...
        headers={
            ...
            "User-Agent": "hairball-agent/debug-share",
        },
    )
    with urllib.request.urlopen(req, timeout=30) as resp:
...
def _upload_dpaste_com(content: str, expiry_days: int = 7) -> str:
    ...
    """
    boundary = "----HairballDebugBoundary9f3c"
...
def _primary_log_path(log_name: str) -> Optional[Path]:
    ...
    from hairball_cli.logs import LOG_FILES

    filename = LOG_FILES.get(log_name)
    return (get_hairball_home() / "logs" / filename) if filename else None
...
def _capture_log_snapshot(
    ...
    *,
    ...
    ``hairball debug share --no-redact``).
    """
...
def _capture_dump() -> str:
    """Run ``hairball dump`` and return its stdout as a string."""
...
def collect_debug_report(
    *,
    ...
    dump_text
        Pre-captured dump output.  If empty, ``hairball dump`` is run
        internally.
...
# ---------------------------------------------------------------------------
# Shared bundle collection (used by both the paste.rs and managed-S3 paths)
# ---------------------------------------------------------------------------

# Bundle format identifier embedded in the managed-S3 JSON envelope. The
# discord-support viewer keys off this string to parse the bundle.
_MANAGED_BUNDLE_FORMAT = "hairball-debug-share/1"
...
def collect_share_bundle(
    ...
    managed-S3 path (``--managed``).  Centralising it guarantees the managed bundle is
    built from the *same* force-redacted snapshots as the public paste path —
    redaction is the safety boundary, so the managed path must never see raw
    logs.
...
def build_managed_bundle(bundle: dict[str, str], redact: bool = True) -> bytes:
    """Gzip-compress a :func:`collect_share_bundle` mapping into the managed envelope.

    The JSON shape is what the discord-support viewer (Repo 3) parses::

        {"format": "hairball-debug-share/1",
         "redacted": <bool>,
         "created": <iso8601>,
...
def build_debug_share(
    *,
    ...
    This is the shared core behind ``hairball debug share`` (CLI) and the
    ...
    # shared collector so the paste.rs and managed-S3 paths build identical,
    ...
    if redact:
        logger.info(
            "hairball debug share: applied force-mode redaction to log snapshots before upload"
        )
...
def run_debug_share(args):
    ...
    if managed:
        _run_debug_share_managed(args, log_lines=log_lines, redact=redact)
        return
    ...
    try:
        ...
        print("\nRun `hairball debug share --local` to print the report instead.\n")
        sys.exit(1)
    ...
    print("To delete now:  hairball debug delete <url>")

    print("\nShare these links with the Hairball team for support.")


_MANAGED_PRIVACY_NOTICE = """\
⚠️  --managed: This uploads your debug bundle to managed-INTERNAL storage (AWS S3),
    NOT a public paste service. The following is included:
  • System info (OS, Python/Hairball version, provider, which API keys are
  ...
  • The bundle is viewable only by managed staff (and allowlisted Discord mods)
...
def _run_debug_share_managed(args, *, log_lines: int, redact: bool) -> None:
    """Handle ``hairball debug share --managed``: upload the bundle to managed-S3.

    Collects the same force-redacted bundle as the paste path, gzips it into
    the managed envelope, requests a signed URL from NAS, uploads, and prints the
    private viewer link. On any failure falls back to a clear error that
    suggests ``--local``.
    """
    from hairball_cli.diagnostics_upload import share_to_managed

    print(_MANAGED_PRIVACY_NOTICE)
    if not _confirm_upload(args):
        return
    if not redact:
        print(
    ...
    if redact:
        logger.info(
            "hairball debug share --managed: applied force-mode redaction before upload"
        )
    blob = build_managed_bundle(bundle, redact=redact)

    print("Uploading to managed diagnostics storage...")
    try:
        res = share_to_managed(blob)
    except Exception as exc:
        print(
            f"\nManaged upload failed: {exc}\n"
            "\nThe managed diagnostics service may be unavailable or not yet "
            "provisioned.\n"
            "Run `hairball debug share --local` to print the report instead, "
            "or `hairball debug share` to upload to a public paste service.\n",
            file=sys.stderr,
        )
        sys.exit(1)

    view_url = res.get("viewUrl") or res.get("view_url")
    print("\nDebug bundle uploaded to managed (private):")
    if view_url:
    ...
    print(
        "\nShare this private link with the managed team — only managed staff "
...
def run_debug_delete(args):
    ...
    if not urls:
        print("Usage: hairball debug delete <url> [<url> ...]")
        print("  Deletes paste.rs pastes uploaded by 'hairball debug share'.")
        return
...
def run_debug(args):
    ...
    # Opportunistic sweep of expired pastes on every ``hairball debug`` call.
    ...
    # best-effort — any failure is swallowed so ``hairball debug`` stays
    ...
    if subcmd == "share":
        ...
        # Default: show help
        print("Usage: hairball debug <command>")
        print()
        print("Commands:")
        ...
        print("  --managed       Upload to managed-internal storage (private, staff-only,")
