...
Pure logic: walks a Hairball config dict, returns issues for any reference
to a retired xAI model. No I/O, no CLI dependencies — testable in isolation
and reusable from both `hairball doctor` and a future `hairball migrate xai`.
"""
...
class RetirementIssue:
    """A reference to a retired xAI model found in a Hairball config."""
...
def find_retired_xai_refs(config: Dict[str, Any]) -> List[RetirementIssue]:
    """Walk all model slots in a Hairball config and return retirement issues.
...
def apply_migration(
    ...
    from hairball_cli.config import require_readable_config_before_write
