...
from hairball_cli.dashboard_auth import list_session_providers
from hairball_cli.dashboard_auth.audit import AuditEvent, audit_log
from hairball_cli.dashboard_auth.base import (
    DashboardAuthProvider,
    ProviderError,
    RefreshExpiredError,
)
from hairball_cli.dashboard_auth.cookies import (
...
from hairball_cli.dashboard_auth.public_paths import PUBLIC_API_PATHS
...
def _unauth_response(request: Request, *, reason: str) -> Response:
    ...
    Under a reverse proxy with ``X-Forwarded-Prefix: /hairball``, the
    ``login_url`` is prefixed (``/hairball/login?next=...``) so the
    ...
    """
    from hairball_cli.dashboard_auth.prefix import prefix_from_request
...
def _auto_sso_response(request: Request) -> Response | None:
    ...
    from hairball_cli.dashboard_auth.cookies import detect_https
    set_sso_attempt_cookie(
...
def _safe_next_target(request: Request) -> str:
    ...
    # the SPA's own ``sessionStorage["hairball.lastLocation"]`` fallback
...
async def gated_auth_middleware(
    ...
    # sends only ``hairball_session_rt``. If we bailed on ``not at`` here we'd
    ...
    if at:
        ...
        # tries to reach Managed Portal's JWKS; if that's unreachable it raises,
    ...
    if session is None:
        ...
        from hairball_cli.dashboard_auth.cookies import clear_session_cookies
        from hairball_cli.dashboard_auth.prefix import prefix_from_request
