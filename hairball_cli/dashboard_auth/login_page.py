...
Visual styling mirrors the Hairball design system (the
``@managed-research/ui`` package the React dashboard uses): the same
...
``tests/hairball_cli/test_dashboard_auth_401_reauth.py``.
"""
...
<title>Sign in — Hairball Agent</title>
<style>
  /* Brand fonts shipped by @managed-research/ui — same files the SPA loads. */
  @font-face {{
...
<body>
<main>
  <div class="brand">managed<span class="dot"></span>Research</div>
  <div class="card">
    <h1>Sign in</h1>
    <p class="subtitle">Choose a sign-in method to continue to the Hairball Agent dashboard.</p>
...
<meta charset="utf-8">
<meta name="viewport" content="width=device-width, initial-scale=1">
<title>Sign-in unavailable — Hairball Agent</title>
<style>
  @font-face {
    font-family: 'Collapse';
    font-style: normal;
...
<p>Install <code>plugins/dashboard-auth-managed</code> (default) or another
