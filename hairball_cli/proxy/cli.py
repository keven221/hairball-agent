"""CLI handlers for the ``hairball proxy`` subcommand."""
...
from hairball_cli.proxy.adapters import ADAPTERS, get_adapter
from hairball_cli.proxy.server import (
    AIOHTTP_AVAILABLE,
    DEFAULT_HOST,
    DEFAULT_PORT,
    run_server,
...
def _print_aiohttp_missing() -> None:
    print(
        "hairball proxy requires aiohttp. Install one of:\n"
        "  pip install 'hairball-agent[messaging]'\n"
        "  pip install aiohttp",
        file=sys.stderr,
    )
...
def cmd_proxy_start(args: Any) -> int:
    ...
    provider = getattr(args, "provider", None) or "managed"
    try:
    ...
    if not adapter.is_authenticated():
        auth_hint = getattr(adapter, "auth_hint", f"hairball auth add {adapter.name}")
        print(
    ...
    print(
        f"Starting Hairball proxy for {adapter.display_name}\n"
...
def cmd_proxy_status(args: Any) -> int:
    ...
    print("Hairball proxy upstream adapters\n")
    ...
    print(
        "\nStart the proxy with: hairball proxy start [--provider <name>]"
    )
    return 0
...
def cmd_proxy(args: Any) -> int:
    """Dispatch ``hairball proxy <subcommand>``."""
    ...
    print(
        "hairball proxy — local OpenAI-compatible proxy that attaches your\n"
        "OAuth-authenticated provider credentials to outbound requests.\n"
        "\n"
        "Subcommands:\n"
        "  hairball proxy start [--provider managed|xai] [--host 127.0.0.1] [--port 8645]\n"
        "      Run the proxy in the foreground.\n"
        "  hairball proxy status\n"
        "      Show which upstream adapters are ready.\n"
        "  hairball proxy providers\n"
