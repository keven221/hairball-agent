...
# xAI's public API is OpenAI-compatible for the endpoints Hairball commonly
# uses. The Responses endpoint is included because Hairball' native xAI runtime
...
class XAIGrokAdapter(UpstreamAdapter):
    """Proxy upstream for xAI Grok via Hairball-managed OAuth credentials."""

    auth_hint = "hairball auth add xai-oauth --type oauth"
        ...
        with self._lock:
            ...
            if pool is None or not pool.has_credentials():
                raise RuntimeError(
                    ...
                    "`hairball auth add xai-oauth --type oauth` first."
                )
            ...
            if entry is None:
                raise RuntimeError(
                    ...
                    "`hairball auth reset xai-oauth` or re-authenticate with "
                    "`hairball auth add xai-oauth --type oauth`."
                )
    ...
    def _credential_from_entry(self, entry: PooledCredential) -> UpstreamCredential:
        ...
        if not bearer:
            raise RuntimeError(
                ...
                "Re-authenticate with `hairball auth add xai-oauth --type oauth`."
            )
