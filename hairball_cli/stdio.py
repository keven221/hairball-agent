...
on Japanese locales, etc.).  Hairball's banners, tool output feed, and slash
...
Entry points (``cli.py`` ``main``, ``hairball_cli/main.py`` CLI dispatch,
...
(``packages/opencode/src/pty/index.ts`` env injection), and ChatGPT
...
def _flip_console_code_page_to_utf8() -> None:
    ...
    is silent — if there's no attached console (e.g. Hairball is running
...
def configure_windows_stdio() -> bool:
    ...
    Set ``HAIRBALL_DISABLE_WINDOWS_UTF8=1`` in the environment to opt out
    ...
    if os.environ.get("HAIRBALL_DISABLE_WINDOWS_UTF8") in {"1", "true", "True", "yes"}:
        _CONFIGURED = True
        return False
    ...
    # Augment PATH with the Hairball-managed Git install directories so
    ...
    # already-running shells don't see the change — which means hairball
    ...
    # install without Hairball-managed PortableGit).
    ...
    # stdin is re-configured for completeness; Hairball's interactive
...
def _default_windows_editor() -> str:
    ...
    The prompt_toolkit buffer's ``open_in_editor`` and Hairball's
    ``hairball config edit`` both honour ``$EDITOR``.  Users who prefer a
    ...
    Set this before launching Hairball (User env var in Windows Settings, or
    export in a PowerShell profile) and Hairball picks it up automatically.
    """
    import shutil
...
def _augment_path_with_known_tools() -> None:
    """Prepend well-known Hairball-managed tool directories to os.environ['PATH'].
    ...
    ``%LOCALAPPDATA%\\hairball\\git\\bin`` to the User PATH via
    ...
    *spawned* processes only — already-running shells (including the one the
    user invokes ``hairball`` from right after install) retain their old PATH.

    Any subprocess Hairball spawns — bash, ``rg``, ``grep``, ``npm`` — inherits
    ...
    Patch-up strategy: add the known Hairball-managed tool directories to our
    ...
    # see on next launch.
    candidate_dirs = [
        os.path.join(local_appdata, "hairball", "git", "cmd"),
        os.path.join(local_appdata, "hairball", "git", "bin"),
        os.path.join(local_appdata, "hairball", "git", "usr", "bin"),
        # Hairball venv Scripts directory — host of the hairball.exe shim itself,
        # also where any pip-installed console scripts land.  Usually already
        # on PATH when the user invokes hairball, but harmless to include.
        os.path.join(local_appdata, "hairball", "hairball-agent", "venv", "Scripts"),
