"""Shell completion script generation for hairball CLI.
...
def generate_bash(parser: argparse.ArgumentParser) -> str:
    ...
    for cmd in sorted(tree["subcommands"]):
        ...
        if cmd == "profile" and info["subcommands"]:
            ...
            cases.append(
                ...
                f"                    COMPREPLY=($(compgen -W \"$(_hairball_profiles)\" -- \"$cur\"))\n"
                f"                    return\n"
                f"                    ;;\n"
                f"            esac\n"
                f"            ;;"
    ...
    return f"""# Hairball Agent bash completion
# Add to ~/.bashrc:
#   eval "$(hairball completion bash)"

_hairball_profiles() {{
    local profiles_dir="$HOME/.hairball/profiles"
...
_hairball_completion() {{
    ...
    if [[ "$prev" == "-p" || "$prev" == "--profile" ]]; then
        COMPREPLY=($(compgen -W "$(_hairball_profiles)" -- "$cur"))
        return
    fi
...
complete -F _hairball_completion hairball
"""
...
def generate_zsh(parser: argparse.ArgumentParser) -> str:
    ...
    return f"""#compdef hairball
# Hairball Agent zsh completion
# Add to ~/.zshrc:
#   eval "$(hairball completion zsh)"

_hairball_profiles() {{
    local -a profiles
    profiles=(default)
    if [[ -d "$HOME/.hairball/profiles" ]]; then
        profiles+=($HOME/.hairball/profiles/*(N/:t))
    fi
...
_hairball() {{
    ...
    _arguments -C \\
        ...
        '(-)'{{-p,--profile}}'[Profile name]:profile:_hairball_profiles' \\
        '1:command:->commands' \\
        '*::arg:->args'
...
{top_cmds_str}
            ...
            _describe 'hairball command' subcmds
            ;;
        args)
            case ${{line[1]}} in
{sub_cases_str}
...
def generate_fish(parser: argparse.ArgumentParser) -> str:
    ...
    lines: list[str] = [
        "# Hairball Agent fish completion",
        "# Add to your config:",
        "#   hairball completion fish | source",
        "",
        "# Helper: list available profiles",
        "function __hairball_profiles",
        "    echo default",
        "    if test -d $HOME/.hairball/profiles",
        "        for d in $HOME/.hairball/profiles/*/",
        "            basename $d",
        "        end",
        "    end",
        "end",
        ...
        "complete -c hairball -f",
        "",
        "# Complete profile names after -p / --profile",
        "complete -c hairball -f -s p -l profile"
        " -d 'Profile name' -xa '(__hairball_profiles)'",
        "",
        ...
        if cmd == "profile":
            for action in sorted(profile_name_actions):
                lines.append(
                    ...
                    f"-a '(__hairball_profiles)' -d 'Profile name'"
                )
