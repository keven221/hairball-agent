...
# lookup, and hairball-ink reports Alt as ``key.meta`` on many terminals.
...
def normalize_voice_record_key_for_prompt_toolkit(raw: Any) -> str:
    ...
    * ``super`` / ``win`` / ``windows`` → ``c-b`` (TUI-only modifiers —
    ...
    # in prompt_toolkit (a-c-r form) and hairball-ink rejects them; collapse
...
def _debug(msg: str) -> None:
    """Emit a debug breadcrumb when HAIRBALL_VOICE_DEBUG=1.
    ...
    """
    if os.environ.get("HAIRBALL_VOICE_DEBUG", "").strip() != "1":
        return
    try:
...
def set_voice_busy_probe(probe: Optional[Callable[[], bool]]) -> None:
    """Register a cheap thread-safe probe for whether the host is mid-turn."""
    global _voice_busy_probe
    _voice_busy_probe = probe


def _voice_activity_held() -> bool:
    """Return True when user silence is expected and must not count as idle."""
        ...
        if force_transcribe and on_transcript:
            ...
            def _transcribe_and_cleanup():
                ...
                try:
                    if wav_path:
                        try:
                            ...
                            if result.get("success"):
                                text = (result.get("transcript") or "").strip()
                                if text and (
                                    not is_whisper_hallucination(text)
                                    or is_voice_stop_phrase(text)
                                ):
                                    transcript = text
                        finally:
                            if os.path.isfile(wav_path):
                    ...
                    stop_phrase = bool(
                        transcript and is_voice_stop_phrase(transcript)
                    )
                    if stop_phrase:
...
def _speak_text_streaming(
    text: str,
    stop_event: Optional[threading.Event] = None,
) -> bool:
    """Speak one complete reply through the shared sentence pipeline."""
    import queue
    ...
    return done.is_set() and not stop.is_set()
...
def speak_text(
    ...
    try:
        ...
        try:
            ...
            _debug(
                f"speak_text: streaming unavailable ({exc}); using file playback"
            )
        ...
        os.makedirs(os.path.join(tempfile.gettempdir(), "hairball_voice"), exist_ok=True)
        ...
        if not os.path.isfile(mp3_path) or os.path.getsize(mp3_path) == 0:
            audio_path = result.get("file_path") or mp3_path

        if os.path.isfile(audio_path) and os.path.getsize(audio_path) > 0:
            _debug(
                f"speak_text: playing {audio_path} "
