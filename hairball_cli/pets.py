"""CLI subcommand: ``hairball pets <subcommand>``.
...
def _cmd_list(args) -> int:
    ...
    if getattr(args, "installed", False):
        ...
        if not pets:
            _print("No pets installed. Try: hairball pets install boba")
            return 0
    ...
    _print("\nInstall one with: hairball pets install <slug>")
    return 0
...
def _cmd_install(args) -> int:
    ...
    if getattr(args, "select", False) or not _has_active_pet():
        ...
        _print(f"  Make it active with: hairball pets select {slug}")
    return 0
...
def _cmd_select(args) -> int:
    ...
    if not slug:
        pets = store.installed_pets()
        if not pets:
            _err("✗ no pets installed — run: hairball pets install boba")
            return 1
    ...
    if pet is None or not pet.exists:
        _err(f"✗ '{slug}' is not installed — run: hairball pets install {slug}")
        return 1
...
def _cmd_show(args) -> int:
    ...
    if pet is None:
        _err("✗ no pet to show — run: hairball pets install boba")
        return 1
...
def _cmd_doctor(args) -> int:
    ...
    ok = True
    if not pets:
        _print("  → no pets installed. Run: hairball pets install boba")
        ok = False
    elif active is None:
        _print("  → active pet unresolved. Run: hairball pets select <slug>")
        ok = False
    elif not enabled:
        _print("  → pet display is disabled. Run: hairball pets select " + active.slug)
...
def toggle_pet_display() -> tuple[bool, str | None, str | None]:
    ...
    if pet is None:
        ...
        # ``resolve_active_pet`` may legitimately fall back when the saved
        # slug is empty or the old Pet was removed.  Persist the Pet we are
        # actually enabling so UI gallery, TUI, CLI, and Electron overlay all
        # project the same active identity instead of enabled=true/slug="".
        _set_active(pet.slug)
    else:
        _set_enabled(True)
