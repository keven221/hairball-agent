"""Migrate Hairball' MCP server config and reference's installed curated plugins
to the format reference expects in ~/.codex/config.toml.
...
module:
  ...
  1. Reads Hairball' YAML and writes equivalent [mcp_servers.<name>]
  ...
  2. Queries codex's `plugin/list` for the openai-curated marketplace
     ...
     legacy calls "migrate native codex plugins" — the YouTube-video-
...
What translates (MCP servers):
  Hairball mcp_servers.<n>.command/args/env  → codex stdio transport
  Hairball mcp_servers.<n>.url/headers       → codex streamable_http transport
  Hairball mcp_servers.<n>.timeout           → codex tool_timeout_sec
  Hairball mcp_servers.<n>.connect_timeout   → codex startup_timeout_sec

What does NOT translate (warned + skipped):
  Hairball-specific keys (sampling, etc.) — codex's MCP client has no
  equivalent. Listed in the per-server skipped[] field of the report.

What's NOT migrated (intentional):
  AGENTS.md — codex respects this file natively in its cwd. Hairball' own
...
MIGRATION_MARKER = (
    "# managed by hairball-agent — `hairball codex-runtime migrate` regenerates this section"
)
MIGRATION_END_MARKER = (
    "# end hairball-agent managed section"
)
...
class MigrationReport:
    ...
    def summary(self) -> str:
        ...
        if self.migrated:
            ...
            lines.append("No MCP servers found in Hairball config.")
        if self.migrated_plugins:
            lines.append(
                f"Migrated {len(self.migrated_plugins)} native reference plugin(s):"
            )
            for name in self.migrated_plugins:
                lines.append(f"  - {name}")
        elif self.plugin_query_error:
            lines.append(f"reference plugin discovery skipped: {self.plugin_query_error}")
...
_KEYS_DROPPED_WITH_WARNING = {
    # Hairball' sampling subsection — codex MCP has no equivalent
    "sampling",
}


def _translate_one_server(
    name: str, hairball_cfg: dict
) -> tuple[Optional[dict], list[str]]:
    """Translate one Hairball MCP server config to the codex inline-table dict
    ...
    if not isinstance(hairball_cfg, dict):
        return None, []

    skipped: list[str] = []
    out: dict[str, Any] = {}

    has_command = bool(hairball_cfg.get("command"))
    has_url = bool(hairball_cfg.get("url"))
    ...
    if has_command:
        # Stdio transport
        out["command"] = str(hairball_cfg["command"])
        args = hairball_cfg.get("args") or []
        if args:
            out["args"] = [str(a) for a in args]
        env = hairball_cfg.get("env") or {}
        if env:
            # reference expects string values
            out["env"] = {str(k): str(v) for k, v in env.items()}
        cwd = hairball_cfg.get("cwd")
        if cwd:
            out["cwd"] = str(cwd)
    elif has_url:
        # streamable_http transport (codex covers both http and SSE here)
        out["url"] = str(hairball_cfg["url"])
        headers = hairball_cfg.get("headers") or {}
        if headers:
            out["http_headers"] = {str(k): str(v) for k, v in headers.items()}
        # Hairball' transport: sse hint is informational; codex auto-negotiates
        if hairball_cfg.get("transport") == "sse":
            skipped.append("transport=sse (codex auto-negotiates)")
    else:
        return None, ["no command or url field"]

    # Timeouts
    if "timeout" in hairball_cfg:
        try:
            out["tool_timeout_sec"] = float(hairball_cfg["timeout"])
        except (TypeError, ValueError):
            skipped.append("timeout (not numeric)")
    if "connect_timeout" in hairball_cfg:
        try:
            out["startup_timeout_sec"] = float(hairball_cfg["connect_timeout"])
        except (TypeError, ValueError):
            skipped.append("connect_timeout (not numeric)")

    # Enabled flag (codex defaults to true so we only emit when explicitly false)
    if hairball_cfg.get("enabled") is False:
        out["enabled"] = False

    # Detect keys we explicitly drop with warning
    for key in hairball_cfg:
        if key in _KEYS_DROPPED_WITH_WARNING:
            skipped.append(f"{key} (no codex equivalent)")
        elif key not in _KNOWN_HAIRBALL_KEYS:
            skipped.append(f"{key} (unknown Hairball key)")
...
def _format_toml_value(value: Any) -> str:
    ...
    if isinstance(value, str):
        ...
        # chars but env-var passthrough (HAIRBALL_HOME, PYTHONPATH) could
...
def render_codex_toml_section(
    ...
    Args:
        ...
        plugins: optional list of {name, marketplace, enabled} for native
            reference plugins to enable. (E.g. the Linear / Atlassian / Asana
    ...
    if not servers and not plugins and not default_permission_profile:
        out.append("# (no MCP servers, plugins, or permissions configured by Hairball)")
        out.append(MIGRATION_END_MARKER)
        return "\n".join(out) + "\n"

    if default_permission_profile:
        # reference's config schema: `default_permissions` is a top-level
...
def _insert_managed_block_at_top_level(user_text: str, managed_block: str) -> str:
    """Insert Hairball' managed reference TOML block while keeping root keys root-scoped.
    ...
    which reference rejects. Insert the managed block before the first table header
...
def _strip_unmanaged_plugin_tables(toml_text: str) -> str:
    ...
    reference itself writes these tables when the user runs ``codex plugins enable``
    directly (i.e. before Hairball' migrate has ever touched the file). When we
...
def _strip_existing_managed_block(toml_text: str) -> str:
    ...
    this code wrote so re-runs don't break configs from prior Hairball
    versions."""
    lines = toml_text.splitlines(keepends=True)
    out: list[str] = []
    in_managed = False
...
def _query_codex_plugins(
    ...
    try:
        with CodexAppServerClient(
            ...
            client.initialize(client_name="hairball-migration")
    ...
    for marketplace in marketplaces:
        ...
        for plugin in plugins:
            ...
            # Cf. openclaw/openclaw#80815 — legacy learned to gate
            ...
            # true for installed plugins. This is the same shape legacy
...
def _looks_like_test_tempdir(path: str) -> bool:
    ...
    what pytest's tempdir factory uses by default. If a HAIRBALL_HOME pointing
    at one of those paths is burned into ``~/.codex/config.toml``, every
    codex-routed hairball-tools call fails silently once the directory is GC'd.

    We err on the side of refusing — losing a (very unlikely) real
    ``~/.hairball`` symlink that happens to live under ``/private/var/folders``
...
def _build_hairball_tools_mcp_entry() -> dict:
    """Build the codex stdio-transport entry that launches Hairball' own
    tool surface as an MCP server. reference's subprocess will call back into
    ...
    so a hairball installed under /opt/, /usr/local/, or a venv all work.
    HAIRBALL_HOME and PYTHONPATH are passed through so the spawned process
    sees the same config + module layout the user is running."""
    import sys

    env: dict[str, str] = {}
    # HAIRBALL_HOME passes through IF SET so the MCP subprocess sees the same
    # config / auth / sessions DB as the parent CLI. Read from os.environ
    # (not get_hairball_home()) on purpose: when the env var is unset we want
    # codex's subprocess to inherit whatever HAIRBALL_HOME its launcher sets
    ...
    # — that would override the launcher's HAIRBALL_HOME and pin the subprocess
    # to the wrong profile.
    #
    # The pytest-tempdir guard below catches the issue #26250 Bug C scenario:
    # a sibling test's monkeypatch.setenv("HAIRBALL_HOME", tmp_path) would
    ...
    hairball_home = os.environ.get("HAIRBALL_HOME") or ""
    if hairball_home and _looks_like_test_tempdir(hairball_home):
        hairball_home = ""
    if hairball_home:
        env["HAIRBALL_HOME"] = hairball_home
    # PYTHONPATH passes through so a worktree-launched hairball finds the
    # branch's modules instead of the installed package.
    pythonpath = os.environ.get("PYTHONPATH")
    if pythonpath:
        env["PYTHONPATH"] = pythonpath
    # Quiet mode + redaction defaults so the MCP wire stays clean.
    env["HAIRBALL_QUIET"] = "1"
    env["HAIRBALL_REDACT_SECRETS"] = env.get("HAIRBALL_REDACT_SECRETS", "true")

    out: dict[str, Any] = {
        "command": sys.executable,
        "args": ["-m", "agent.transports.hairball_tools_mcp_server"],
    }
    if env:
        out["env"] = env
...
def migrate(
    hairball_config: dict,
    *,
    codex_home: Optional[Path] = None,
    dry_run: bool = False,
    discover_plugins: bool = True,
    default_permission_profile: Optional[str] = ":workspace",
    expose_hairball_tools: bool = True,
) -> MigrationReport:
    """Translate Hairball mcp_servers config + reference curated plugins into
    ~/.codex/config.toml.

    Args:
        hairball_config: full ~/.hairball/config.yaml dict
        ...
        expose_hairball_tools: when True (default), register Hairball' own
            tool surface (web_search, browser_*, delegate_task, vision,
            memory, skills, etc.) as an MCP server in ~/.codex/config.toml
            so the codex subprocess can call back into Hairball for tools
    ...
    hairball_servers = (hairball_config or {}).get("mcp_servers") or {}
    if not isinstance(hairball_servers, dict):
        report.errors.append(
            "mcp_servers in Hairball config is not a dict; cannot migrate."
        )
        return report

    translated: dict[str, dict] = {}
    for name, cfg in hairball_servers.items():
    ...
    # Discover installed reference curated plugins. Best-effort — never blocks
    ...
    # Inject Hairball' own tool surface as an MCP server so the spawned
    # codex subprocess can call back into Hairball for the tools codex
    # doesn't ship with — web_search, browser_*, delegate_task, vision,
    # memory, skills, session_search, image_generate, text_to_speech.
    # The server itself is agent/transports/hairball_tools_mcp_server.py
    # and is launched on demand by codex (stdio MCP).
    if expose_hairball_tools:
        translated["hairball-tools"] = _build_hairball_tools_mcp_entry()
        if "hairball-tools" not in report.migrated:
            report.migrated.append("hairball-tools")
