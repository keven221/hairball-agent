"""hairball webhook — manage dynamic webhook subscriptions from the CLI.

Usage:
    hairball webhook subscribe <name> [options]
    hairball webhook list
    hairball webhook remove <name>
    hairball webhook test <name> [--payload '{"key": "value"}']

Subscriptions persist to ~/.hairball/webhook_subscriptions.json and are
...
def _subscriptions_path() -> Path:
    return _hairball_home() / _SUBSCRIPTIONS_FILENAME
...
def _setup_hint() -> str:
  ...
  Then start the gateway: hairball gateway run
"""
...
def webhook_command(args):
    """Entry point for 'hairball webhook' subcommand."""
    sub = getattr(args, "webhook_action", None)

    if not sub:
        print("Usage: hairball webhook {subscribe|list|remove|test}")
        print("Run 'hairball webhook --help' for details.")
        return
...
def _cmd_subscribe(args):
    ...
    print("  The gateway must be running to receive events (hairball gateway run).\n")
...
def _cmd_list(args):
    ...
    if not subs:
        ...
        print("  Create one with: hairball webhook subscribe <name>")
        return
...
def _cmd_test(args):
    ...
    payload = args.payload or '{"test": true, "event_type": "test", "message": "Hello from hairball webhook test"}'
    ...
    try:
        ...
        print(f"  Error: {e}")
        print("  Is the gateway running? (hairball gateway run)")
