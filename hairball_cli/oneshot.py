...
configured for "cli" in `hairball tools`.
Rules / memory / AGENTS.md / preloaded skills = same as a normal chat turn.
Approvals = auto-bypassed (HAIRBALL_YOLO_MODE=1 is set for the call).
Working directory = the user's CWD (AGENTS.md etc. resolve from there as usual).

Model / provider selection mirrors `hairball chat`:
...
def _validate_explicit_toolsets(toolsets: object = None) -> tuple[list[str] | None, str | None]:
    ...
    try:
        ...
        return None, f"hairball -z: failed to validate --toolsets: {exc}\n"
    ...
    if any(name in {"all", "*"} for name in built_in):
        ...
        if ignored:
            sys.stderr.write(
                "hairball -z: --toolsets all enables every toolset; "
    ...
    if unknown:
        sys.stderr.write(f"hairball -z: ignoring unknown --toolsets entries: {', '.join(unknown)}\n")
    if disabled:
        sys.stderr.write(
            "hairball -z: ignoring disabled MCP servers (set enabled: true in config.yaml to use): "
            f"{', '.join(disabled)}\n"
        )

    if not valid:
        return None, "hairball -z: --toolsets did not contain any valid toolsets.\n"

    return valid, None


def _usage_failure_reason(result: dict, failure: Optional[str]) -> Optional[str]:
    """Return a stable machine-readable reason for an unsuccessful run.

    Keep the free-form exception text in the legacy ``failure`` field. Batch
    evaluators need a bounded classification here so they do not have to parse
    provider messages, while structured agent classifications retain priority
    over the coarser loop exit reason.
    """
    if failure is not None:
        return "agent_exception"

    structured_reason = result.get("failure_reason")
    if isinstance(structured_reason, str) and structured_reason.strip():
        return structured_reason.strip()

    turn_exit_reason = result.get("turn_exit_reason")
    if (
        (result.get("failed") or result.get("partial"))
        and isinstance(turn_exit_reason, str)
        and turn_exit_reason.strip()
        and turn_exit_reason.strip() != "unknown"
    ):
        return turn_exit_reason.strip()
...
def _write_usage_file(path: Optional[str], result: dict, failure: Optional[str] = None) -> None:
    ...
    try:
        ...
        report = {
            ...
            "cost_actual_call_count": result.get("cost_actual_call_count"),
            "cost_total_call_count": result.get("cost_total_call_count"),
            "actual_cost_coverage": result.get("actual_cost_coverage"),
            "actual_cost_complete": result.get("actual_cost_complete"),
            ...
            # Optional local audit metadata. Keep stdout as final-response-only;
            # automation that explicitly requested a usage artifact can inspect
            # the completion contract without parsing user-facing prose.
        ...
        failure_reason = _usage_failure_reason(result, failure)
...
def run_oneshot(
    ...
    Args:
        ...
        model: Optional model override. Falls back to HAIRBALL_INFERENCE_MODEL
    ...
    env_model_early = os.getenv("HAIRBALL_INFERENCE_MODEL", "").strip()
    if provider and not ((model or "").strip() or env_model_early):
        sys.stderr.write(
            "hairball -z: --provider requires --model (or HAIRBALL_INFERENCE_MODEL). "
    ...
    if failure is not None:
        ...
        real_stderr.write(f"hairball -z: agent failed: {failure}\n")
        real_stderr.flush()
        return 1
    ...
    if not (response or "").strip():
        real_stderr.write("hairball -z: no final response was produced; treating the run as failed.\n")
        real_stderr.flush()
        return 1
...
def _create_session_db_for_oneshot():
    """Best-effort SessionDB for ``hairball -z`` / oneshot mode.

    Oneshot bypasses ``HairballCLI._init_agent()``, so it must wire the SQLite
...
def _run_agent(
    ...
    # Imports are local so they don't run when hairball is invoked for
    # other commands (keeps top-level CLI startup cheap).
    from hairball_cli.config import load_config
    from hairball_cli.models import detect_provider_for_model
    ...
    env_model = os.getenv("HAIRBALL_INFERENCE_MODEL", "").strip()
    ...
    if effective_provider is None and (model or env_model):
        ...
        if explicit_model:
            ...
            try:
                from hairball_cli import model_switch as _ms
            ...
            if direct is not None:
                ...
                current_provider = (
                    cfg_provider
                    or os.getenv("HAIRBALL_INFERENCE_PROVIDER", "").strip().lower()
                    or "auto"
                )
    ...
    agent = AIAgent(
        ...
        #                HAIRBALL_INTERACTIVE which we never set
        #   - shell-hook approval → auto-approved via HAIRBALL_ACCEPT_HOOKS=1
        #                (set above); also falls back to deny on non-tty
        #   - dangerous-command approval → bypassed via HAIRBALL_YOLO_MODE=1
