...
TIPS = [
    ...
    "/title \"my project\" names your session — resume it later with /resume or hairball -c.",
    ...
    "Ctrl+Z suspends Hairball to the background — run fg in your shell to resume.",
    ...
    # --- CLI Flags ---
    "hairball -c resumes your most recent CLI session. hairball -c \"project name\" resumes by title.",
    "hairball -w creates an isolated git worktree — perfect for parallel agent workflows.",
    "hairball -w -q \"Fix issue #42\" combines worktree isolation with a one-shot query.",
    "hairball chat -t web,terminal enables only specific toolsets for a focused session.",
    "hairball chat -s github-pr-workflow preloads a skill at launch.",
    "hairball chat -q \"query\" runs a single non-interactive query and exits.",
    "hairball chat --max-turns 200 overrides the default 90-iteration limit per turn.",
    "hairball chat --checkpoints enables filesystem snapshots before every destructive file change.",
    "hairball --yolo bypasses all dangerous command approval prompts for the entire session.",
    "hairball chat --source telegram tags the session for filtering in hairball sessions list.",
    "hairball -p work chat runs under a specific profile without changing your default.",

    # --- CLI Subcommands ---
    "hairball doctor --fix diagnoses and auto-repairs config and dependency issues.",
    "hairball dump outputs a compact setup summary — great for bug reports.",
    "hairball config set KEY VALUE auto-routes secrets to .env and everything else to config.yaml.",
    "hairball config edit opens config.yaml in your default editor.",
    "hairball config check scans for missing or stale configuration options.",
    "hairball sessions browse opens an interactive session picker with search.",
    "hairball sessions stats shows session counts by platform and database size.",
    "hairball sessions prune --older-than 30 cleans up old sessions.",
    "hairball skills search react --source skills-sh searches the skills.sh public directory.",
    "hairball skills check scans installed hub skills for upstream updates.",
    "hairball skills tap add myorg/skills-repo adds a custom GitHub skill source.",
    "hairball skills snapshot export setup.json exports your skill configuration for backup or sharing.",
    "hairball mcp add github --command npx adds MCP servers from the command line.",
    "hairball mcp serve runs Hairball itself as an MCP server for other agents.",
    "hairball auth add lets you add multiple API keys for credential pool rotation.",
    "hairball completion bash >> ~/.bashrc enables tab completion for all commands and profiles.",
    "hairball logs -f follows agent.log in real time. --level WARNING --since 1h filters output.",
    "hairball backup creates a zip backup of your entire Hairball home directory.",
    "hairball profile create coder creates an isolated profile that becomes its own command.",
    "hairball profile create work --clone copies your current config and keys to a new profile.",
    "hairball update syncs new bundled skills to ALL profiles automatically.",
    "hairball gateway install sets up Hairball as a system service (systemd/launchd).",
    "hairball memory setup lets you configure an external memory provider (Honcho, Mem0, etc.).",
    "hairball webhook subscribe creates event-driven webhook routes with HMAC validation.",
    "Save money: hairball tools disables unused tools, hairball skills config trims skills down.",
    "/reasoning low or /reasoning minimal cuts thinking depth below the default (medium) — faster, cheaper responses.",
    "hairball models routes vision, compression, and aux tasks to cheaper models — cuts background cost 85%+ without downgrading your main chat model.",
    ...
    "Set worktree: true in config.yaml to always create a git worktree (same as hairball -w).",
    ...
    "HAIRBALL_TIMEZONE overrides the server timezone with any IANA timezone string.",
    ...
    # --- Tools & Capabilities ---
    "execute_code runs Python scripts that call Hairball tools programmatically — results stay out of context.",
    ...
    "Profile names become shell commands — 'hairball profile create coder' creates the 'coder' command.",
    "hairball profile export coder -o backup.tar.gz creates a portable profile archive.",
    ...
    "When exiting, Hairball prints a resume command with session ID and stats.",
    "hairball sessions export backup.jsonl exports all sessions for backup or analysis.",
    "hairball -r SESSION_ID resumes any specific past session by its ID.",
    ...
    "hairball skills install official/security/1password installs optional skills from the repo.",
    ...
    "The plan skill saves markdown plans under .hairball/plans/ in the active workspace.",

    # --- Cron & Scheduling ---
    "Cron jobs can attach skills: hairball cron add --skill blogwatcher \"Check for new posts\".",
    ...
    # --- Gateway & Messaging ---
    "Hairball runs on 21 messaging platforms: Telegram, Discord, Slack, WhatsApp, Signal, Matrix, IRC, Microsoft Teams, email, and more.",
    "hairball gateway install sets it up as a system service that starts on boot.",
    "DingTalk uses Stream Mode — no webhooks or public URL needed.",
    "BlueBubbles brings iMessage to Hairball via a local macOS server.",
    ...
    "Context files (.hairball.md, AGENTS.md) are security-scanned for prompt injection before loading.",
    ...
    "Hairball loads project context from .hairball.md, AGENTS.md, CLAUDE.md, or .cursorrules (first match).",
    ...
    # --- MCP ---
    "hairball mcp opens an interactive picker of catalog MCPs you can install in one keystroke.",
    "hairball mcp catalog lists catalog MCP servers shipped with the repo.",
    "hairball mcp install <name> installs a catalog entry, prompts for credentials, and lets you pick which of its tools to enable.",
    ...
    "MCP servers auto-generate toolsets at runtime — hairball tools can toggle them per platform.",
    ...
    "Checkpoints use shadow repos in ~/.hairball/checkpoints/ — your project's .git is never touched.",

    # --- Batch & Data ---
    "batch_runner.py processes hundreds of prompts in parallel for training data generation.",
    "hairball chat -Q enables quiet mode for programmatic use — suppresses banner and spinner.",
    ...
    "hairball plugins install owner/repo installs plugins directly from GitHub.",
    ...
    "hairball acp runs Hairball as an ACP server for VS Code, Zed, and JetBrains integration.",
    "Custom providers: save named endpoints in config.yaml under custom_providers.",
    "HAIRBALL_EPHEMERAL_SYSTEM_PROMPT injects a system prompt that's never persisted to history.",
    "credential_pool_strategies supports fill_first, round_robin, least_used, and random rotation.",
    "hairball auth add managed or hairball auth add openai-codex sets up OAuth-based providers.",
    ...
    "hairball status --deep runs deeper diagnostic checks across all components.",
    ...
    "Cron scripts live in ~/.hairball/scripts/ and run before the agent — perfect for data collection pipelines.",
    "prefill_messages_file in config.yaml injects few-shot examples into every API call, never saved to history.",
    "SOUL.md completely replaces the agent's default identity — rewrite it to make Hairball your own.",
    ...
    "Before a gateway session reset, Hairball auto-flushes important facts to memory in the background.",
    ...
    "write_file and patch check a content_hash from your last read_file — if the file changed on disk, the write is rejected until you re-read.",
    ...
    "agent.tool_use_enforcement steers models that describe actions instead of calling tools — auto for GPT/reference.",
    ...
    "The skills audit log at ~/.hairball/skills/.hub/audit.log tracks every install and removal operation.",
    "Stale git worktrees are auto-cleaned: 24-72h old with no unpushed commits get pruned on startup.",
    "Profiles scope Hairball state via HAIRBALL_HOME; host tool subprocesses keep your real HOME unless terminal.home_mode is profile.",
    "HAIRBALL_HOME_MODE env var (octal, e.g. 0701) sets custom directory permissions for web server traversal.",
    "Container mode: place .container-mode in HAIRBALL_HOME and the host CLI auto-execs into the container.",
    "Ctrl+C has 5 priority tiers: cancel recording → cancel prompts → cancel picker → interrupt agent → exit.",
    "Every interrupt during an agent run is logged to ~/.hairball/interrupt_debug.log with timestamps.",
    ...
    "GPT and reference models get special system prompt guidance for tool discipline and mandatory tool use.",
    ...
    "Profile names cannot collide with existing PATH binaries — 'hairball profile create ls' would be rejected.",
    "hairball profile create backup --clone-all copies everything (config, keys, SOUL.md, memories, skills, sessions).",
    ...
    "GPT-5 and reference use 'developer' role instead of 'system' in the message format.",
    ...
    "hairball claw migrate --dry-run previews legacy migration without writing anything.",
    ...
    "HAIRBALL_DEV=1 bypasses container mode detection for local development.",
    "Each MCP server gets its own toolset (mcp-servername) that can be toggled independently via hairball tools.",
    "MCP ${ENV_VAR} placeholders in config are resolved at server spawn — including vars from ~/.hairball/.env.",
    "Skills from trusted repos (managed) get a 'trusted' security level; community skills get extra scanning.",
    "The skills quarantine at ~/.hairball/skills/.hub/quarantine/ holds skills pending security review.",

    # --- Advanced Slash Commands ---
    '/steer <prompt> injects a note after the next tool call — nudge direction mid-task without interrupting.',
    '/goal <text> sets a standing Ralph-loop objective — Hairball auto-continues turn after turn until a judge says done.',
    '/snapshot create [label] saves a full state snapshot of Hairball config; /snapshot restore <id> reverts later.',
    ...
    '/busy queue|steer|interrupt controls what pressing Enter does while Hairball is working.',
    ...
    '/reload reloads ~/.hairball/.env into the running session — pick up new API keys without restarting.',
    ...
    "HAIRBALL_CRON_MAX_PARALLEL (default 4) caps how many cron jobs run per tick so bursts don't saturate your keys.",

    # --- Gateway Hooks ---
    'Gateway hooks live under ~/.hairball/hooks/<name>/ with HOOK.yaml + handler.py — handler must be named `handle`.',
    'Hook events include gateway:startup, session:start, agent:step, and command:* wildcard subscriptions.',
    'Drop a ~/.hairball/BOOT.md checklist and a gateway:startup hook runs it as a one-shot agent every boot.',

    # --- Curator ---
    'hairball curator run --dry-run previews what the curator would archive or consolidate without mutating anything.',
    "hairball curator pin <skill> hard-fences a skill against both auto-archival and the agent's skill_manage tool.",
    'hairball curator rollback restores skills from a pre-run snapshot — backups live under skills/.curator_backups/.',

    # --- Credential Pools & Routing ---
    'hairball auth reset <provider> clears all cooldowns and exhaustion flags on a credential pool.',
    ...
    # --- TUI & Dashboard ---
    'HAIRBALL_TUI_RESUME=1 auto-re-attaches to the most recent TUI session on launch — handy after SSH drops.',
    "HAIRBALL_TUI_THEME=light|dark|<hex> forces the TUI theme on terminals that don't set COLORFGBG.",
    ...
    'hairball dashboard launches a local web UI at 127.0.0.1:9119 — zero data leaves localhost.',
    'hairball dashboard embeds the full Hairball TUI in your browser via xterm.js and a WebSocket PTY.',
    'Drop a YAML in ~/.hairball/dashboard-themes/ with two palette colors to reskin the entire dashboard.',
    'Dashboard plugins are drop-in: manifest.json + JS bundle in ~/.hairball/dashboard-plugins/ — no npm build required.',
    ...
    'HAIRBALL_BACKGROUND_NOTIFICATIONS=result only pings when background tasks finish (vs all/error/off).',
    'HAIRBALL_WRITE_SAFE_ROOT restricts write_file/patch to directory prefixes; multiple paths via os.pathsep (: or ;).',
    'HAIRBALL_IGNORE_RULES skips auto-injection of AGENTS.md, SOUL.md, .cursorrules, memory, and preloaded skills.',
    'HAIRBALL_ACCEPT_HOOKS auto-approves unseen shell hooks declared in config.yaml without a TTY prompt.',
    ...
    # --- TTS ---
    'tts.provider: piper runs 44-language local TTS on CPU — voices auto-download to ~/.hairball/cache/piper-voices/.',
    ...
    '/reload-skills re-scans ~/.hairball/skills/ so drop-in skills appear without restarting the session.',
    ...
    'hairball -z "<prompt>" is the purest one-shot: final answer on stdout, nothing else — ideal for piping in scripts.',
    'hairball chat --pass-session-id injects the session ID into the system prompt so the agent can self-reference it.',
    'hairball chat --image path/to/pic.png attaches a local image to a single -q query without a separate upload step.',
    'hairball chat --ignore-user-config skips the active user config — reproducible bug reports and CI runs.',
    "hairball chat --source tool tags programmatic chats so they don't clutter hairball sessions list.",
    'hairball dump --show-keys includes redacted API key fingerprints for deeper support debugging.',
    'hairball sessions rename <ID> "new title" renames any past session; hairball sessions delete <ID> removes one.',
    'hairball import restores a full Hairball backup zip; session JSON/JSONL exports import from the dashboard Sessions page.',
    'hairball fallback manages the fallback_model chain interactively — no hand-editing config.yaml.',
    'hairball pairing rotates the DM pairing token — the first messager after rotation claims access to the bot.',
    'hairball setup walks first-time users through provider, keys, and platform wiring in one interactive flow.',
    'hairball status --deep runs the full health sweep across every component; plain hairball status is the quick view.',

    # --- Agent Behavior Env Vars ---
    'HAIRBALL_AGENT_TIMEOUT=0 disables the gateway inactivity kill for a running agent — use for long research runs.',
    'HAIRBALL_ENABLE_PROJECT_PLUGINS=1 auto-loads repo-local plugins from ./.hairball/plugins/ — trust-gated by design.',
    "HAIRBALL_DISABLE_FILE_STATE_GUARD=1 turns off the 'file changed since you read it' guard on patch and write_file.",
    'HAIRBALL_ALLOW_PRIVATE_URLS=true lets web tools hit localhost and private networks — off by default in gateway mode.',
    'HAIRBALL_OPTIONAL_SKILLS=name1,name2 auto-installs extra optional-catalog skills on first run per profile.',
    'HAIRBALL_BUNDLED_SKILLS points at a custom bundled-skill tree — used by Homebrew and Nix packaging.',
    'HAIRBALL_DUMP_REQUEST_STDOUT=1 dumps every API request payload to stdout instead of log files.',
    'HAIRBALL_OAUTH_TRACE=1 logs redacted OAuth token exchange and refresh attempts for debugging provider auth.',
    'HAIRBALL_STREAM_RETRIES (default 3) controls mid-stream reconnect attempts on transient network errors.',

    # --- Gateway Behavior Env Vars ---
    'HAIRBALL_GATEWAY_BUSY_ACK_ENABLED=false silences the ⚡/⏳/⏩ ack messages when a user messages a busy agent.',
    'HAIRBALL_AGENT_NOTIFY_INTERVAL (default 180s) sets how often the gateway pings with progress on long turns.',
    'HAIRBALL_RESTART_DRAIN_TIMEOUT (default 900s) caps how long /restart waits for in-flight runs before forcing.',
    'HAIRBALL_CHECKPOINT_TIMEOUT (default 30s) caps filesystem checkpoint creation — raise it on huge monorepos.',
    ...
    'image_gen.provider routes image generation through a plugin (OpenAI Images, reference, FAL) instead of the default.',
    'AUXILIARY_VISION_BASE_URL + AUXILIARY_VISION_API_KEY point vision analysis at any OpenAI-compatible endpoint.',

    # --- Security ---
    'security.tirith_fail_open: false makes Hairball block commands when the tirith scanner itself errors out.',
    ...
    '--source tool chats are excluded from hairball sessions list by default — set --source explicitly to see them.',
    ...
    'Dashboard plugins are served from /dashboard-plugins/<name>/ — drop files into ~/.hairball/dashboard-plugins/.',
]
