"""``hairball mcp`` subcommand parser.
...
def build_mcp_parser(subparsers, *, cmd_mcp: Callable) -> None:
    ...
    mcp_parser = subparsers.add_parser(
        "mcp",
        help="Manage MCP servers and run Hairball as an MCP server",
        description=(
            "Manage MCP server connections and run Hairball as an MCP server.\n\n"
            "MCP servers provide additional tools via the Model Context Protocol.\n"
            "Use 'hairball mcp add' to connect to a new server, or\n"
            "'hairball mcp serve' to expose Hairball conversations over MCP."
        ),
    )
    ...
    mcp_serve_p = mcp_sub.add_parser(
        "serve",
        help="Run Hairball as an MCP server (expose conversations to other agents)",
    )
    ...
    # flag is omitted, causing `hairball mcp add ...` to fall through to
    # interactive chat.
    ...
    # ── Catalog (catalog MCPs shipped with the repo) ─────────────────
    mcp_sub.add_parser(
        "picker",
        help="Interactive catalog picker (also the default for `hairball mcp`)",
    )
    mcp_sub.add_parser(
        "catalog",
        help="List catalog MCPs available for one-click install",
    )
    mcp_install_p = mcp_sub.add_parser(
        "install",
        help="Install a catalog MCP by name (e.g. `hairball mcp install n8n`)",
    )
