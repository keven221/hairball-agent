"""``hairball gateway`` and ``hairball proxy`` subcommand parsers.

Extracted verbatim from ``hairball_cli/main.py:main()`` (god-file Phase 2).
...
from hairball_cli.subcommands._shared import add_accept_hooks_flag
...
def build_gateway_parser(
    ...
    gateway_run.add_argument(
        ...
        help=(
            ...
            "if HAIRBALL_DASHBOARD is set). Pass --no-supervise to opt out and "
    ...
    gateway_migrate_legacy = gateway_subparsers.add_parser(
        "migrate-legacy",
        help="Remove legacy hairball.service units from pre-rename installs",
        description=(
            "Stop, disable, and remove legacy Hairball gateway unit files "
            "(e.g. hairball.service) left over from older installs. Profile "
            "units (hairball-gateway-<profile>.service) and unrelated "
    ...
    gateway_enroll = gateway_subparsers.add_parser(
        ...
        description=(
            ...
            "Authenticates as your Managed Portal account (the connector derives the "
            ...
            "GATEWAY_RELAY_SECRET / GATEWAY_RELAY_DELIVERY_KEY into ~/.hairball/.env. "
            "Requires being logged in (hairball setup). Not available in managed installs."
        ),
    )
    ...
    gateway_enroll.add_argument(
        ...
        help=(
            ...
            "GATEWAY_RELAY_WAKE_URL in ~/.hairball/.env and forwarded at provision. "
    ...
    proxy_parser = subparsers.add_parser(
        ...
        description=(
            ...
            "to an OAuth-authenticated provider (e.g. Managed Portal). External "
    ...
    proxy_start.add_argument(
        "--provider",
        default="managed",
        help="Upstream provider: managed or xai (default: managed). See `hairball proxy providers`.",
    )
    proxy_start.add_argument(
        "--host",
        default=None,
