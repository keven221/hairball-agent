"""``hairball hooks`` subcommand parser.
...
def build_hooks_parser(subparsers, *, cmd_hooks: Callable) -> None:
    ...
    hooks_parser = subparsers.add_parser(
        ...
        description=(
            "Inspect shell-script hooks declared in ~/.hairball/config.yaml, "
            "test them against synthetic payloads, and manage the first-use "
            "consent allowlist at ~/.hairball/shell-hooks-allowlist.json."
        ),
    )
    ...
    _hk_test.add_argument(
        "event",
        help=(
            "Hook event name (e.g. pre_tool_call, stop, subagent_stop, "
            "subagent_complete)"
        ),
    )
    _hk_test.add_argument(
