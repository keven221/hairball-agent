"""``hairball debug`` subcommand parser.
...
def build_debug_parser(subparsers, *, cmd_debug: Callable) -> None:
    ...
    debug_parser = subparsers.add_parser(
        ...
        description="Debug utilities for Hairball Agent. Use 'hairball debug share' to "
        "upload a debug report (system info + recent logs) to a paste "
        "service and get a shareable URL.",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
Examples:
    hairball debug share              Upload debug report (asks for confirmation)
    hairball debug share --yes        Skip confirmation (for scripts/CI)
    hairball debug share --lines 500  Include more log lines
    hairball debug share --expire 30  Keep paste for 30 days
    hairball debug share --local      Print report locally (no upload)
    hairball debug share --no-redact  Disable upload-time secret redaction
    hairball debug share --managed       Upload to managed-internal storage (private)
    hairball debug delete <url>       Delete a previously uploaded paste
""",
    )
    ...
    share_parser.add_argument(
        ...
        action="store_true",
        help=(
            "Upload the debug bundle to managed-internal storage (AWS S3) instead "
            "of a public paste service. The bundle is private — viewable only "
            "by managed staff (and allowlisted Discord mods) via a Google-login-"
    ...
    delete_parser = debug_sub.add_parser(
        "delete",
        help="Delete a paste uploaded by 'hairball debug share'",
    )
