...
the tool registry or launching Hairball (mirrors the decoupled-helper pattern used
elsewhere in the CLI).

Motivated by #38798: a config migration silently rewrote the valid toolset name
``hairball-cli`` to the non-existent ``hairball``. ``resolve_toolset('hairball')``
...
def validate_platform_toolsets(
    ...
    1. A toolset name that ``is_valid_toolset`` rejects — usually a corrupted or
       renamed entry. When ``hairball-<platform>`` would have been valid (the exact
       #38798 shape, where ``cli`` held ``hairball`` instead of ``hairball-cli``),
    ...
    for platform, raw in platform_toolsets.items():
        ...
        for name in names:
            ...
            if is_valid_toolset(name):
                valid_count += 1
                continue
            suggestion = f"hairball-{platform}"
            hint = (
    ...
    if valid_count == 0:
        warnings.append(
            ...
            "have no tools. Run `hairball tools` to reconfigure."
        )
    return warnings
