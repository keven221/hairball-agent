"""
Cron subcommand for hairball CLI.
...
# commands at execution time when ``_HAIRBALL_GATEWAY=1``.
...
def _warn_if_gateway_not_running() -> None:
    ...
    print(color("     Start it with: hairball gateway install", Colors.DIM))
    print(color("                    sudo hairball gateway install --system  # Linux servers", Colors.DIM))
    print(color("     Check status:  hairball cron status", Colors.DIM))
...
def cron_list(show_all: bool = False):
    ...
    if not jobs:
        ...
        print(color("Create one with 'hairball cron create ...' or the /cron command in chat.", Colors.DIM))
        return
...
def cron_status():
    ...
    if pids:
        ...
        if hb_age is not None and hb_age > STALE_AFTER:
            ...
            print("  Cron jobs may NOT be firing. Restart: hairball gateway restart")
        ...
        print("    hairball gateway install    # Install as a user service")
        print("    sudo hairball gateway install --system  # Linux servers: boot-time system service")
        print("    hairball gateway            # Or run in foreground")
...
def cron_command(args):
    ...
    print("Usage: hairball cron [list|create|edit|pause|resume|run|remove|status|tick]")
    sys.exit(1)
