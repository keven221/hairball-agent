"""
Hairball Plugin System
====================

Discovers, loads, and manages plugins from four sources:

1. **Bundled plugins** – ``<repo>/plugins/<name>/`` (shipped with hairball-agent;
   ``memory/`` and ``context_engine/`` subdirs are excluded — they have their
   own discovery paths)
2. **User plugins**   – ``~/.hairball/plugins/<name>/``
3. **Project plugins** – ``./.hairball/plugins/<name>/`` (opt-in via
   ``HAIRBALL_ENABLE_PROJECT_PLUGINS``)
4. **Pip plugins**     – packages that expose the ``hairball_agent.plugins``
   entry-point group.
...
from hairball_cli.config import cfg_get
from hairball_cli.middleware import OBSERVER_SCHEMA_VERSION, VALID_MIDDLEWARE


def get_bundled_plugins_dir() -> Path:
    """Locate the bundled ``plugins/`` directory.

    Honours ``HAIRBALL_BUNDLED_PLUGINS`` (set by the Nix wrapper / packaged
    installs) so read-only store paths are consulted first.  Falls back to
    the in-repo path used during development.
    """
    env_override = os.getenv("HAIRBALL_BUNDLED_PLUGINS")
    if env_override:
...
class PluginHookExecutionError(RuntimeError):
    """A plugin hook declared fail-closed and did not complete safely."""
    ...
    def __init__(
        ...
        self.cause = cause
        super().__init__(
            f"Fail-closed hook {hook_name!r} from plugin {plugin_id!r} failed: "
            f"{cause}"
        )


class PluginHookTimeoutError(TimeoutError):
    """A plugin hook exceeded its explicitly configured runtime budget."""


class PluginHookRecursionError(RuntimeError):
    """A hook callback re-entered its own registration recursively."""
...
# ---------------------------------------------------------------------------
#
# Set ``HAIRBALL_PLUGINS_DEBUG=1`` to surface verbose plugin-discovery logs to
# stderr in addition to ~/.hairball/logs/agent.log. Aimed at plugin authors
...
_PLUGINS_DEBUG = os.getenv("HAIRBALL_PLUGINS_DEBUG", "").strip().lower() in {
    "1", "true", "yes", "on",
}
...
def _install_plugin_debug_handler(force: bool = False) -> None:
    """When HAIRBALL_PLUGINS_DEBUG is on, tee plugin logs to stderr at DEBUG.

    Idempotent: only attaches the handler once per process unless ``force``
    is passed. Does not touch the root logger or other Hairball loggers.
    """
    ...
    logger.debug(
        "HAIRBALL_PLUGINS_DEBUG=1 — verbose plugin discovery logging enabled"
    )
...
VALID_HOOKS: Set[str] = {
    ...
    # The accepted Stop shape {"decision": "block", "reason": "..."} (block
    # the stop == keep going) is accepted too. Anything else lets the turn
    # finish. Hairball' shipped guidance lives in the evidence-based
    ...
    "on_session_finalize",
    "on_session_reset",
    # Context-compaction lifecycle. These hooks are side-band: payloads expose
    # counts, outcome, and lineage metadata without cloning the full transcript
    # into every plugin. ``pre_compact`` fires only after the session lock is
    # acquired and a real attempt is about to start; ``post_compact`` reports
    # completed, skipped, or error outcomes for that attempt.
    "pre_compact",
    "post_compact",
    # Structured, driver-independent AgentNotice lifecycle. Notification hooks
    # are always fail-open at the AIAgent emitter so an observer can never hide
    # a user-visible warning or prevent its clear operation.
    "on_notification",
    "on_notification_clear",
    "subagent_start",
    # Post-completion observation event.  It cannot veto child termination.
    "subagent_complete",
    # Child round-end control hook. It uses the same
    # block/stop contract as ``stop`` with additional child identity fields.
    "subagent_stop",
    # Round-end hook. A callback may return
    # {"decision": "block", "reason": "..."} to continue, or
    # {"continue": false, "stopReason": "..."} to force termination.
    # Stop wins when multiple handlers disagree; blocking reasons are retained
    # as typed hook-prompt items in the model transcript.
    "stop",
    ...
    # Kanban task lifecycle hooks. Fired by hairball_cli.kanban_db when a task
    ...
    # separate `hairball -p <profile> chat -q` subprocesses:
    #   - kanban_task_claimed   -> the DISPATCHER process (gateway-embedded
    #                              dispatcher or `hairball kanban dispatch`),
...
ENTRY_POINTS_GROUP = "hairball_agent.plugins"

_NS_PARENT = "hairball_plugins"
...
class PluginManifest:
    ...
    # Higher-priority plugins run their hooks first. Equal priorities preserve
    # deterministic registration order. Existing manifests default to zero,
    # which keeps the historical discovery/registration order unchanged.
    priority: int = 0
    ...
    #              in ~/.hairball/plugins/ still gated by ``plugins.enabled``
    #              (untrusted code).
    kind: str = "standalone"
    # Registry key — path-derived, used by ``plugins.enabled``/``disabled``
    # lookups and by ``hairball plugins list``. For a flat plugin at
...
class HookRegistration:
    """Host-owned hook metadata kept behind :class:`PluginManager`'s seam."""
...
_ACTIVE_HOOK_REGISTRATIONS: contextvars.ContextVar[
    frozenset[tuple[str, str, int]]
] = contextvars.ContextVar(
    "hairball_active_hook_registrations",
    default=frozenset(),
)
...
class PluginContext:
    ...
    def profile_name(self) -> str:
        """Return the active Hairball profile name (e.g. ``"default"``).

        Derived from ``HAIRBALL_HOME`` via
        :func:`hairball_cli.profiles.get_active_profile_name`, so it works in
        ...
        running under ``~/.hairball/profiles/<name>``, or ``"custom"`` when
        ``HAIRBALL_HOME`` points somewhere unrecognized.
        """
        try:
    ...
    def _tool_override_allowed(self, tool_name: str) -> bool:
        ...
        Bundled plugins (shipped with Hairball core) are trusted by default —
    ...
    def register_cli_command(
        ...
        """Register a CLI subcommand (e.g. ``hairball honcho ...``).
    ...
    def register_command(
        ...
        Unlike ``register_cli_command()`` (which creates ``hairball <subcommand>``
    ...
    def register_dashboard_auth_provider(self, provider) -> None:
        ...
        ``provider`` must be an instance of
        :class:`hairball_cli.dashboard_auth.DashboardAuthProvider`. Used by
        ...
        """
        from hairball_cli.dashboard_auth import (
    ...
    def register_secret_source(self, source) -> None:
        ...
        sources run during ``load_hairball_dotenv()`` startup — after
        ``~/.hairball/.env`` loads, before Hairball reads credentials — when
    ...
    def register_slack_action_handler(
        ...
        Hairball' Slack adapter wires registered handlers into its
    ...
    def register_auxiliary_task(
        ...
        *,
        ...
        core files. After registration, the task:
          ...
          - Appears in the ``hairball model → Configure auxiliary models`` picker
        ...
        # Lazy import to avoid circular: hairball_cli.main imports plugins indirectly
        from hairball_cli.main import _AUX_TASKS as _BUILTIN_AUX_TASKS
    ...
    def register_hook(
        ...
        *,
        ...
        Hooks run by descending ``priority`` and then registration order.
        When omitted, priority inherits ``plugin.yaml``'s top-level
        ``priority`` (default ``0``), preserving historical order for every
        existing plugin. ``timeout`` is opt-in and measured across both the
        callback and an awaitable it returns. Fail-open remains the default;
        ``fail_closed=True`` raises :class:`PluginHookExecutionError` at the
        call site when the callback raises or times out.
        ...
        effective_priority = self.manifest.priority if priority is None else priority
        self._manager._register_hook(
            hook_name,
            callback,
            plugin_id=self.manifest.key or self.manifest.name,
    ...
    def register_skill(
        ...
        ``~/.hairball/skills/`` tree and is **not** listed in the system
...
class PluginManager:
    ...
    def __init__(self) -> None:
        ...
        # Lists intentionally accept raw callbacks as a compatibility lane for
        # older in-process integrations/tests that appended directly. New
        # registrations always use HookRegistration so ownership and policy
        # remain local to this module.
        self._hooks: Dict[str, List[Union[HookRegistration, Callable]]] = {}
        self._hook_registration_order: int = 0
    ...
    def discover_and_load(self, force: bool = False) -> None:
        ...
        if self._discovered and not force:
            return
        if env_var_enabled("HAIRBALL_SAFE_MODE"):
            logger.info("HAIRBALL_SAFE_MODE=1 — plugin discovery skipped")
        ...
        if force:
            ...
            with self._hook_lock:
                self._hooks.clear()
                self._hook_registration_order = 0
    ...
    def _discover_and_load_inner(self) -> None:
        ...
        #
        # Repo-shipped plugins live next to hairball_cli/. Two layouts are
        ...
        # 2. User plugins (~/.hairball/plugins/)
        user_dir = get_hairball_home() / "plugins"
        logger.debug("Scanning user plugins: %s", user_dir)
        user_manifests = self._scan_directory(user_dir, source="user")
        logger.debug("  user: %d manifest(s)", len(user_manifests))
        manifests.extend(user_manifests)

        # 3. Project plugins (./.hairball/plugins/)
        if _env_enabled("HAIRBALL_ENABLE_PROJECT_PLUGINS"):
            project_dir = Path.cwd() / ".hairball" / "plugins"
            ...
            logger.debug(
                "Project plugins disabled (set HAIRBALL_ENABLE_PROJECT_PLUGINS=1 to enable)"
            )
        ...
        try:
            ...
            from hairball_cli.governance import evaluate_capability

            governance_config = load_config_readonly()
        except Exception:
            evaluate_capability = None
            governance_config = {}
        ...
        for manifest in winners.values():
            ...
            # Managed policy sits above normal plugins.enabled/disabled and
            # applies to bundled platform/backend plugins too. This prevents a
            # surface-specific auto-load path from bypassing an organization
            # allowlist while leaving ordinary personal installs unchanged.
            if evaluate_capability is not None:
                ...
                if not decision.allowed:
                    ...
                    loaded.error = f"blocked by managed policy: {decision.reason}"
                    self._plugins[lookup_key] = loaded
                    logger.debug(
                        "Skipping plugin '%s' (%s)", lookup_key, loaded.error
                    )
                    continue
            ...
            # Built-in backends auto-load — they ship with hairball and must
            ...
            # all ~20 of them added several seconds to every `hairball`
            # invocation — including plain `hairball chat`, which never touches a
            ...
            # path actually asks for that platform. Every platform Hairball ships
            ...
            if not is_enabled:
                loaded = LoadedPlugin(manifest=manifest, enabled=False)
                loaded.error = (
                    "not enabled in config (run `hairball plugins enable {}` to activate)"
                    .format(lookup_key)
                )
                self._plugins[lookup_key] = loaded
                logger.debug(
    ...
    def _parse_manifest(
        ...
        try:
            ...
            if isinstance(raw_priority, bool) or not isinstance(raw_priority, int):
                logger.warning(
                    "Plugin %s: invalid priority %r; using 0",
                    key,
                    raw_priority,
                )
                priority = 0
    ...
    def _register_deferred_platform(self, manifest: PluginManifest) -> None:
        ...
        ``hairball plugins list`` still shows the platform as available, and we
        hand the registry a loader that runs the normal eager-load path.
        """
        lookup_key = manifest.key or manifest.name
        platform_name = self._platform_name_from_manifest(manifest)

        # Record an enabled placeholder for introspection (`hairball plugins
    ...
    def _load_plugin(self, manifest: PluginManifest) -> None:
        ...
        # Deferred loaders and explicit force-rescans may converge on the same
        # plugin in one process. Remove only that plugin's old hook records so
        # a reload never doubles side effects; other plugins keep their order.
        ...
        try:
            ...
            if register_fn is None:
                ...
                # later plugins under-reported in `hairball plugins list`.
    ...
    def _load_directory_module(self, manifest: PluginManifest) -> types.ModuleType:
        """Import a directory-based plugin as ``hairball_plugins.<slug>``.

        The module slug is derived from ``manifest.key`` so category-namespaced
        plugins (``image_gen/openai``) import as
        ``hairball_plugins.image_gen__openai`` without colliding with any
        future ``tts/openai``.
        """
    ...
    def _register_hook(
        ...
        *,
        ...
        """Store one validated hook registration behind the manager seam."""
        if not callable(callback):
        ...
        if isinstance(timeout, bool) or not isinstance(timeout, (int, float)):
            raise TypeError("hook timeout must be a non-negative number")
        normalized_timeout = float(timeout)
        if normalized_timeout < 0 or not math.isfinite(normalized_timeout):
            raise ValueError("hook timeout must be a finite non-negative number")
        if not isinstance(fail_closed, bool):
            raise TypeError("hook fail_closed must be a boolean")

        with self._hook_lock:
            registration = HookRegistration(
                callback=callback,
                plugin_id=str(plugin_id or "<unknown>"),
                priority=normalized_priority,
                timeout=normalized_timeout,
                fail_closed=fail_closed,
                order=self._hook_registration_order,
            )
            self._hook_registration_order += 1
            self._hooks.setdefault(hook_name, []).append(registration)
        return registration

    def unregister_plugin_hooks(self, plugin_id: str) -> int:
        """Remove every owned hook for one plugin and return the count.

        Raw legacy callbacks have no trustworthy owner metadata and are left
        untouched. Force discovery still clears the entire registry, while a
        same-process reload can now remove exactly the callbacks it owns.
        """
        removed = 0
        with self._hook_lock:
            for hook_name in list(self._hooks):
                registrations = self._hooks[hook_name]
                kept: List[Union[HookRegistration, Callable]] = []
                for registration in registrations:
                    if (
                        isinstance(registration, HookRegistration)
                        and registration.plugin_id == plugin_id
                    ):
                        removed += 1
                    else:
    ...
    @classmethod
    def _isolate_hook_value(
        cls,
        value: Any,
        memo: Optional[Dict[int, Any]] = None,
    ) -> Any:
        """Clone mutable data containers without copying runtime objects.

        Hook payloads mix plain data with live objects such as ``AIAgent``,
        gateway runners, database handles, and locks. ``copy.deepcopy`` would
        either fail or duplicate those identities. Structural cloning protects
        caller-owned dict/list/set/tuple data while deliberately preserving
        ...
        if isinstance(value, bytearray):
            cloned_bytes = bytearray(value)
            memo[value_id] = cloned_bytes
            return cloned_bytes
        if isinstance(value, dict):
            cloned_dict: Dict[Any, Any] = {}
            memo[value_id] = cloned_dict
            for key, item in value.items():
                cloned_dict[cls._isolate_hook_value(key, memo)] = (
                    cls._isolate_hook_value(item, memo)
                )
            return cloned_dict
        if isinstance(value, list):
            cloned_list: List[Any] = []
            memo[value_id] = cloned_list
            cloned_list.extend(cls._isolate_hook_value(item, memo) for item in value)
            return cloned_list
        if type(value) is tuple:
            cloned_tuple = tuple(cls._isolate_hook_value(item, memo) for item in value)
            memo[value_id] = cloned_tuple
            return cloned_tuple
        if isinstance(value, set):
            cloned_set: Set[Any] = set()
            memo[value_id] = cloned_set
            cloned_set.update(cls._isolate_hook_value(item, memo) for item in value)
            return cloned_set
        if isinstance(value, frozenset):
            cloned_frozen = frozenset(
                cls._isolate_hook_value(item, memo) for item in value
            )
    ...
    @staticmethod
    def _resolve_hook_awaitable(result: Any) -> Any:
        """Resolve an async hook from synchronous core call sites."""
        ...
        thread = threading.Thread(
            target=_runner,
            name="hairball-plugin-hook-await",
            daemon=True,
        )
        thread.start()
        done.wait()
    ...
    def _invoke_hook_registration(
        ...
        active = _ACTIVE_HOOK_REGISTRATIONS.get()
        if registration_key in active:
            raise PluginHookRecursionError(
                f"recursive hook invocation blocked for {hook_name!r} "
                f"from plugin {registration.plugin_id!r}"
            )
        active_token = _ACTIVE_HOOK_REGISTRATIONS.set(
            active | frozenset((registration_key,))
        )
        isolated_kwargs = self._isolate_hook_value(kwargs)

        def _call() -> Any:
            result = registration.callback(**isolated_kwargs)
            return self._resolve_hook_awaitable(result)
        ...
        try:
            ...
            def _runner() -> None:
                try:
                    outcome["value"] = hook_context.run(_call)
                except BaseException as exc:  # re-raised in the calling thread
                    failure["exc"] = exc
                finally:
                    done.set()
            ...
            thread = threading.Thread(
                target=_runner,
                name=f"hairball-hook-{registration.plugin_id}",
                daemon=True,
            )
            thread.start()
            if not done.wait(timeout=registration.timeout):
                raise PluginHookTimeoutError(
                    f"hook exceeded {registration.timeout:.3f}s"
                )
            if "exc" in failure:
                raise failure["exc"]
            return outcome.get("value")
        finally:
            _ACTIVE_HOOK_REGISTRATIONS.reset(active_token)

    def invoke_hook(self, hook_name: str, **kwargs: Any) -> List[Any]:
        """Call all registered callbacks for *hook_name*.

        Registrations run by descending priority and then registration order.
        Each callback receives an isolated copy of mutable data containers;
        opaque runtime objects deliberately keep identity. Sync and async
        callbacks share the same contract. Exceptions and explicit timeouts
        are fail-open by default, while a registration with ``fail_closed``
        raises :class:`PluginHookExecutionError` at the owning call site.
        ...
        ordered: List[tuple[int, HookRegistration]] = []
        for index, callback in enumerate(callbacks):
            if isinstance(callback, HookRegistration):
            ...
            ordered.append((index, registration))
        ordered.sort(key=lambda item: (-item[1].priority, item[0]))

        results: List[Any] = []
        for _, registration in ordered:
            started_at = time.perf_counter()
            try:
                ret = self._invoke_hook_registration(
                    hook_name,
                    registration,
                    kwargs,
                )
                ...
                logger.debug(
                    "hook_audit hook=%s plugin=%s outcome=success "
                    "duration_ms=%.3f priority=%d fail_closed=%s",
                    hook_name,
                    registration.plugin_id,
                    duration_ms,
                    registration.priority,
                    registration.fail_closed,
                )
                if ret is not None:
                    results.append(self._isolate_hook_value(ret))
            except Exception as exc:
                duration_ms = (time.perf_counter() - started_at) * 1000.0
                if isinstance(exc, PluginHookTimeoutError):
                    outcome = "timeout"
                elif isinstance(exc, PluginHookRecursionError):
                    outcome = "recursion_blocked"
                else:
                    outcome = "error"
                logger.warning(
                    "hook_audit hook=%s plugin=%s outcome=%s "
                    "duration_ms=%.3f priority=%d fail_closed=%s error_type=%s",
                    hook_name,
                    registration.plugin_id,
                    outcome,
                    duration_ms,
                ...
                if registration.fail_closed:
                    raise PluginHookExecutionError(
                        hook_name,
                        registration.plugin_id,
                        exc,
                    ) from exc
...
def resolve_plugin_command_result(result: Any) -> Any:
    ...
    thread = threading.Thread(
        target=_runner,
        name="hairball-plugin-command-await",
        daemon=True,
    )
    thread.start()
...
def get_plugin_toolsets() -> List[tuple]:
    ...
    Used by the ``hairball tools`` TUI so plugin-provided toolsets appear
