"""``hairball logs`` — view and filter Hairball log files.
...
Usage examples::
    ...
    hairball logs                    # last 50 lines of agent.log
    hairball logs -f                 # follow agent.log in real time
    hairball logs errors             # last 50 lines of errors.log
    hairball logs gateway -n 100    # last 100 lines of gateway.log
    hairball logs gui -f            # follow gui.log (dashboard/pty/ws)
    hairball logs desktop -f        # follow desktop.log (Electron app boot/backend)
    hairball logs --level WARNING    # only WARNING+ lines
    hairball logs --session abc123   # filter by session ID substring
    hairball logs --component tools  # only tool-related lines
    hairball logs --since 1h         # lines from the last hour
    hairball logs --since 30m -f     # follow, starting 30 min ago
"""
...
def tail_log(
    ...
    *,
    ...
    log_path = get_hairball_home() / "logs" / filename
    if not log_path.exists():
        print(f"Log file not found: {log_path}")
        print("(Logs are created when Hairball runs — try 'hairball chat' first)")
        sys.exit(1)
    ...
    if follow:
        print(f"--- {display_hairball_home()}/logs/{filename}{filter_desc} (Ctrl+C to stop) ---")
    else:
        print(f"--- {display_hairball_home()}/logs/{filename}{filter_desc} (last {num_lines}) ---")
...
def list_logs() -> None:
    ...
    if not log_dir.exists():
        print(f"No logs directory at {display_hairball_home()}/logs/")
        return

    print(f"Log files in {display_hairball_home()}/logs/:\n")
    found = False
    ...
    if not found:
        print("  (no log files yet — run 'hairball chat' to generate logs)")
