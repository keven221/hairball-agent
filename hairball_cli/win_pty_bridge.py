"""Windows ConPTY bridge for the `hairball dashboard` chat tab.

Drop-in counterpart to ``hairball_cli.pty_bridge.PtyBridge`` for native
Windows. Mirrors the exact public surface the ``/api/pty`` WebSocket
handler in ``hairball_cli.web_server`` consumes: ``spawn``, ``read``,
