"""
Configuration management for Hairball Agent.

Config files are stored in ~/.hairball/ for easy access:
- ~/.hairball/config.yaml  - All settings (model, toolsets, terminal, etc.)
- ~/.hairball/.env         - API keys and secrets

This module provides:
- hairball config          - Show current configuration
- hairball config edit     - Open config in editor
- hairball config set      - Set a specific value
- hairball config wizard   - Re-run setup wizard
"""
...
def _backup_corrupt_config(config_path: Path) -> Optional[Path]:
    ...
    they re-run the setup wizard or ``hairball config set`` (which rewrites
    ...
    ``config.yaml`` in place: hairball never silently mutates the user's config,
...
def _warn_config_parse_failure(
    ...
    A YAML parse error in ``~/.hairball/config.yaml`` causes ``load_config()``
    ...
    ``agent.log`` / ``errors.log`` at WARNING level so ``hairball logs``
    ...
    logger.warning(msg)
    try:
        sys.stderr.write(f"⚠️  hairball config: {msg}\n")
        sys.stderr.flush()
    except Exception:
        pass
...
#   the next ``subprocess.run([...])`` Hairball makes loads attacker code
#   before main().
# * ``PYTHONPATH`` / ``PYTHONHOME`` / ``PYTHONSTARTUP`` /
#   ``PYTHONUSERBASE`` — Python interpreter init. Hairball itself starts
#   from one of these on every restart.
# * ``NODE_OPTIONS`` / ``NODE_PATH`` — Node interpreter; affects npm,
#   ``hairball update``, the TUI build.
...
#   on every plugin install / ``hairball update``.
...
# * ``HAIRBALL_HOME`` / ``HAIRBALL_PROFILE`` / ``HAIRBALL_CONFIG`` /
#   ``HAIRBALL_ENV`` — Hairball runtime location flags. Writing these into
...
#
# IMPORTANT: ``HAIRBALL_*`` overall is NOT blocked. Many legitimate
# integration credentials follow that prefix (HAIRBALL_LANGFUSE_PUBLIC_KEY,
# HAIRBALL_SPOTIFY_CLIENT_ID, ...). The
...
_ENV_VAR_NAME_DENYLIST: frozenset[str] = frozenset({
    ...
    "PYTHONEXECUTABLE", "PYTHONMANAGEDERSITE",
    # Node
    ...
    # Hairball runtime location — never via dashboard env writer.
    # NOT a HAIRBALL_* blanket: integration credentials (HAIRBALL_GEMINI_*,
    # HAIRBALL_LANGFUSE_*, HAIRBALL_SPOTIFY_*, ...) ARE allowed.
    "HAIRBALL_HOME", "HAIRBALL_PROFILE", "HAIRBALL_CONFIG", "HAIRBALL_ENV",
})
...
def _reject_denylisted_env_var(key: str) -> None:
    ...
    if key in _ENV_VAR_NAME_DENYLIST:
        raise ValueError(
            ...
            "PYTHONPATH, PATH, EDITOR, ...) or Hairball runtime location "
            "(HAIRBALL_HOME, HAIRBALL_PROFILE, ...) cannot be persisted via "
            "the env writer. If you really need this, edit "
            "~/.hairball/.env directly."
        )
...
_EXTRA_ENV_KEYS = frozenset({
    ...
    "HAIRBALL_TOOL_PROGRESS", "HAIRBALL_TOOL_PROGRESS_MODE",
    ...
    # Activation is via plugins.enabled (opt-in through `hairball plugins enable
    # observability/langfuse` or `hairball tools → Langfuse`); credentials gate
    # the plugin at runtime.
    "HAIRBALL_LANGFUSE_ENV",
    "HAIRBALL_LANGFUSE_RELEASE",
    "HAIRBALL_LANGFUSE_SAMPLE_RATE",
    "HAIRBALL_LANGFUSE_MAX_CHARS",
    "HAIRBALL_LANGFUSE_DEBUG",
    "LANGFUSE_PUBLIC_KEY",
    "LANGFUSE_SECRET_KEY",
    "LANGFUSE_BASE_URL",
})
...
from hairball_cli.default_soul import DEFAULT_SOUL_MD, is_legacy_template_soul
...
def get_managed_system() -> Optional[str]:
    ...
    raw = os.getenv("HAIRBALL_MANAGED", "").strip()
    if raw:
    ...
    managed_marker = get_hairball_home() / ".managed"
...
def is_managed() -> bool:
    """Check if Hairball is running in package-manager-managed mode.

    Two signals: the HAIRBALL_MANAGED env var (set by the systemd service),
    or a .managed marker file in HAIRBALL_HOME (set by the NixOS activation
...
def get_managed_update_command() -> Optional[str]:
    ...
    if managed_system == "Homebrew":
        return "brew upgrade hairball-agent"
...
def _install_method_project_root(project_root: Optional[Path] = None) -> Path:
    ...
    This is the parent of ``hairball_cli/`` — i.e. the git checkout for source
    installs, ``/opt/hairball`` inside the published image, the venv's
    site-packages root for pip installs. It is a property of the running
    interpreter, NOT of ``$HAIRBALL_HOME``, which is why a code-scoped stamp
...
def detect_install_method(project_root: Optional[Path] = None) -> str:
    """Detect how Hairball was installed: 'docker', 'nixos', 'homebrew', 'git', or 'pip'.

    Resolution order:
    1. Code-scoped stamp ``<install tree>/.install_method`` (next to the
       running code) — the authoritative marker.
    2. Legacy home-scoped stamp ``$HAIRBALL_HOME/.install_method`` — read for
       backward compatibility, but a ``docker`` value is IGNORED when we are
       not actually running inside a container (see below).
    3. HAIRBALL_MANAGED env / .managed marker (NixOS, Homebrew)
    4. .git directory presence -> 'git'
    5. Fallback -> 'pip'

    Why the stamp is code-scoped, not home-scoped (issue: shared ``~/.hairball``)
    --------------------------------------------------------------------------
    The install method describes *the binary that is running*, but
    ``$HAIRBALL_HOME`` is a shared DATA directory — the Docker docs deliberately
    bind-mount it (``~/.hairball:/opt/data``) so config/sessions/memory persist
    and can be shared with a host-side Desktop/CLI install. When a
    containerised gateway and a host install share one ``$HAIRBALL_HOME``, a
    ...
    ``docker`` and ``hairball update`` refuses to run ("doesn't apply inside the
    ...
    The supported installs self-identify via the code-scoped stamp:
      ...
      - the published ``hairball/hairball-agent`` image bakes a ``docker``
        stamp into ``/opt/hairball`` at build time.
    ...
    # 1. Code-scoped stamp — authoritative, immune to shared $HAIRBALL_HOME.
    try:
    ...
    #    host install whose shared $HAIRBALL_HOME was stamped by a co-located
    #    container, and honouring it wrongly blocks ``hairball update``.
    try:
        method = (
            (get_hairball_home() / ".install_method")
...
def _running_in_container() -> bool:
    """Thin wrapper around ``hairball_constants.is_container`` (import-safe)."""
    try:
...
def stamp_install_method(method: str, project_root: Optional[Path] = None) -> None:
    ...
    not in ``$HAIRBALL_HOME``, so that two installs sharing one data directory
    ...
    ``/opt/hairball`` in the published image, which instead bakes the stamp at
...
def is_uv_tool_install() -> bool:
    """Return True when the *running* Hairball lives in a ``uv tool`` layout.

    ``uv tool install hairball-agent`` places the install at
    ``.../uv/tools/hairball-agent/...`` (default ``~/.local/share/uv/tools``,
    ...
    ``hairball-agent`` happens to be uv-tool-installed on the machine while
    the *active* Hairball is a regular pip/venv install, causing
    ``hairball update`` to upgrade the wrong copy. It would also block on a
    ...
    def _has_uv_tool_marker(path: str) -> bool:
        ...
        return "/uv/tools/hairball-agent/" in norm + "/"
...
def recommended_update_command_for_method(method: str) -> str:
    ...
    if method == "homebrew":
        return "brew upgrade hairball-agent"
    if method == "docker":
        return "docker pull hairball/hairball-agent:latest"
    if method == "pip":
        if is_uv_tool_install():
            return "uv tool upgrade hairball-agent"
        import shutil
        if shutil.which("uv"):
            return "uv pip install --upgrade hairball-agent"
        return "pip install --upgrade hairball-agent"
    return "hairball update"
...
# banner, the TUI/desktop session info panel, and ``hairball update``. NixOS
# stays fully supported (Tier 2) and must never hit this path.

PLATFORM_SUPPORT_DOCS_URL = "./website/docs/getting-started/platform-support"
...
def format_unsupported_install_warning(method: str) -> str:
    ...
    ``hairball update`` / ``hairball update --check`` so the wording — and the
...
# Long-form text for ``hairball update`` / ``--check`` when running inside the
# Docker image.  Surfaced by ``cmd_update`` and ``_cmd_update_check`` in
# hairball_cli/main.py; lives here so the wording stays consistent and we
...
#     — that script installs a *new* host-side Hairball, it doesn't update
...
✗ ``hairball update`` doesn't apply inside the Docker container.

Hairball Agent runs as a published image (hairball/hairball-agent), not a
git checkout — the container has no working tree to pull into.  Update by
pulling a fresh image and restarting your container instead:

  docker pull hairball/hairball-agent:latest
  # then restart whatever started the container, e.g.:
  docker compose up -d --force-recreate hairball-agent
  # or, for ad-hoc runs, exit the current container and `docker run` again

Verify the new version after restart:
  docker run --rm hairball/hairball-agent:latest --version

Notes:
  • If you pinned a specific tag (e.g. ``:v0.14.0``) the ``:latest`` tag
    won't move your container — pull the newer tag you actually want, or
    switch to ``:latest`` / ``:main`` for rolling updates.  See available
    tags at https://hub.docker.com/r/hairball/hairball-agent/tags
  • Your config and session history live under ``$HAIRBALL_HOME`` (``/opt/data``
...
def format_docker_update_message() -> str:
    """Return the user-facing message for ``hairball update`` inside Docker.
...
def format_managed_message(action: str = "modify this Hairball installation") -> str:
    """Build a user-facing error for managed installs."""
    managed_system = get_managed_system() or "a package manager"
    raw = os.getenv("HAIRBALL_MANAGED", "").strip().lower()

    if managed_system == "NixOS":
        env_hint = "true" if raw in _MANAGED_TRUE_VALUES else raw or "true"
        return (
            f"Cannot {action}: this Hairball installation is managed by NixOS "
            f"(HAIRBALL_MANAGED={env_hint}).\n"
            "Edit services.hairball-agent.settings in your configuration.nix and run:\n"
    ...
    if managed_system == "Homebrew":
        ...
        return (
            f"Cannot {action}: this Hairball installation is managed by Homebrew "
            f"(HAIRBALL_MANAGED={env_hint}).\n"
            "Use:\n"
            "  brew upgrade hairball-agent"
        )

    return (
        f"Cannot {action}: this Hairball installation is managed by {managed_system}.\n"
        "Use your package manager to upgrade or reinstall Hairball."
    )
...
def get_container_exec_info() -> Optional[dict]:
    """Read container mode metadata from HAIRBALL_HOME/.container-mode.

    Returns a dict with keys: backend, container_name, exec_user, hairball_bin
    or None if container mode is not active, we're already inside the
    container, or HAIRBALL_DEV=1 is set.

    The .container-mode file is written by the NixOS activation script when
    container.enable = true. It tells the host CLI to exec into the container
    instead of running locally.
    """
    if os.environ.get("HAIRBALL_DEV") == "1":
        return None
    ...
    container_mode_file = get_hairball_home() / ".container-mode"
    ...
    container_name = info.get("container_name", "hairball-agent")
    exec_user = info.get("exec_user", "hairball")
    hairball_bin = info.get("hairball_bin", "/data/current-package/bin/hairball")

    return {
        "backend": backend,
        "container_name": container_name,
        "exec_user": exec_user,
        "hairball_bin": hairball_bin,
    }
...
# Re-export from hairball_constants — canonical definition lives there.
from hairball_constants import get_hairball_home  # noqa: F811,E402
from utils import atomic_replace, fast_safe_load

def get_config_path() -> Path:
    """Get the main config file path."""
    return get_hairball_home() / "config.yaml"

def get_env_path() -> Path:
    """Get the .env file path (for API keys)."""
    return get_hairball_home() / ".env"

def get_project_root() -> Path:
    """Get the project installation directory."""
    return Path(__file__).parent.parent.resolve()

def _resolve_hairball_uid_gid() -> tuple[Optional[int], Optional[int]]:
    """Read the HAIRBALL_UID / HAIRBALL_GID env vars set by Docker deployments.

    Docker containers running Hairball commonly set these to map the in-container
    user to a host user so volume-mounted state files end up with the right
    ownership. The entrypoint chowns the top-level HAIRBALL_HOME once, but
    subdirectories created at runtime by ``ensure_hairball_home()`` (especially
    ...
    if sys.platform == "win32":
        return None, None
    uid_str = os.environ.get("HAIRBALL_UID", "").strip()
    gid_str = os.environ.get("HAIRBALL_GID", "").strip()
    try:
...
def _chown_to_hairball_uid(path) -> None:
    """Chown ``path`` to ``HAIRBALL_UID:HAIRBALL_GID`` if those env vars are set.
    ...
    directories created by :func:`ensure_hairball_home` on Docker deployments.
    See #34107.
    """
    uid, gid = _resolve_hairball_uid_gid()
...
def _secure_dir(path):
    ...
    permissions (0750) so interactive users in the hairball group can
    share state with the gateway service.

    The mode can be overridden via the HAIRBALL_HOME_MODE environment variable
    (e.g. HAIRBALL_HOME_MODE=0701) for deployments where a web server (nginx,
    caddy, etc.) needs to traverse HAIRBALL_HOME to reach a served subdirectory.
    The execute-only bit on a directory permits cd-through without exposing
    directory listings.

    Also applies ``HAIRBALL_UID``/``HAIRBALL_GID``-based ownership when those env
    ...
    if is_managed():
        return
    try:
        mode_str = os.environ.get("HAIRBALL_HOME_MODE", "").strip()
    ...
    try:
        ...
        pass
    _chown_to_hairball_uid(path)


def _is_container() -> bool:
    """Detect if we're running inside a Docker/Podman/LXC container.

    When Hairball runs in a container with volume-mounted config files, forcing
    ...
    """
    # Explicit opt-out
    if os.environ.get("HAIRBALL_CONTAINER") or os.environ.get("HAIRBALL_SKIP_CHMOD"):
        return True
...
def _secure_file(path):
    ...
    permissions.  Set HAIRBALL_SKIP_CHMOD=1 to force-skip on other systems.
    """
...
def _ensure_default_soul_md(home: Path) -> None:
    """Seed a default SOUL.md into HAIRBALL_HOME, upgrading legacy empty templates.
...
def ensure_hairball_home():
    """Ensure ~/.hairball directory structure exists with secure permissions.
    ...
    # Named profiles must be created explicitly (e.g. ``hairball profile create``).
    # If a stale process keeps running after the profile was renamed/deleted,
    # silently mkdir-ing the old HAIRBALL_HOME would resurrect an empty skeleton
    ...
    if is_managed():
        ...
        try:
            _ensure_hairball_home_managed(home)
        finally:
            os.umask(old_umask)
    else:
...
def _ensure_hairball_home_managed(home: Path):
    """Managed-mode variant: verify dirs exist (activation creates them), seed SOUL.md."""
    if not home.is_dir():
        raise RuntimeError(
            f"HAIRBALL_HOME {home} does not exist. "
...
DEFAULT_CONFIG = {
    ...
    "agent": {
        ...
        # the Hairball-level retry loop that wraps the whole call.  Lower
        # this to 1 if you use fallback providers and want fast failover
        # on flaky primaries; raise it if you prefer to tolerate longer
        # provider hiccups on a single provider.
        "api_max_retries": 3,
        # Content-hash edit gate (Phase 1 fingerprint only — not full OMP
        # hashline/recovery). Values: "warn" (default — write + warning until
        # recovery exists), "require" (reject mismatch), "off".
        # Tests may override via HAIRBALL_CONTENT_HASH_MODE.
        "content_hash_mode": "warn",
        "service_tier": "",
        ...
        # Model-neutral finish-and-verify guidance inherited from 9810.  This
        # adds no hard gate or tool choice; it asks every tool-capable model to
        # finish the artifact, verify it with real output, and report blockers
        # instead of fabricating a result. Set False for a leaner prompt.
        ...
        # prompt's environment-hints block. Lets a host that wraps Hairball
        ...
        # identity slot (SOUL.md). Empty by default. The HAIRBALL_ENVIRONMENT_HINT
        # env var overrides this (build-time/container mechanism).
        "environment_hint": "",
        # Coding posture — on interactive coding surfaces (CLI, TUI, desktop
        # app, ACP) in a code workspace, Hairball adds a coding operating brief
        # + a live git/workspace snapshot to the system prompt. See
        # agent/coding_context.py.
        #   "focus" (default) — prompt posture + collapse the toolset to the
        #                      lean coding set (+ enabled MCP servers) + demote
        #                      non-coding skill categories to names-only in the
        #                      skill index. Messaging platforms unaffected
        #                      (they never activate coding auto/focus).
        #   "auto"           — prompt-only posture when the surface is
        #                      interactive AND cwd is a code workspace.
        #                      Toolsets are never touched.
        ...
        "max_verify_nudges": 3,
        # Passive completion-evidence ledger. It never changes model input,
        # tool choice, stop policy, or retry count.
        "completion_contract": {
            "mode": "off",
        },
        # Canonical Todo lifecycle. Completion reminders only inspect the
        # model-authored pending/in_progress list, require real tool progress
        # before repeating, and stop after the configured bound. ``eager`` is
        # opt-in so the default preserves the model's planning autonomy.
        "todo": {
            "enabled": True,
            "reminders": True,
            "reminders_max": 3,
    ...
    "terminal": {
        ...
        #             HAIRBALL_HOME/home for persistent state (default)
        #   real    — force the real OS-user HOME
        #   profile — force HAIRBALL_HOME/home when it exists (old strict
        #             per-profile CLI config isolation)
        "home_mode": "auto",
        # Host-local execution uses the platform's native process sandbox at
        # the final spawn boundary.  Network remains available so ordinary
        # developer workflows keep working; filesystem writes are limited to
        # the active workspace, the platform temp directory, and these
        # additional absolute roots.
        "local_network": True,
        "local_writable_roots": [],
        ...
        # skipped. When empty, Hairball auto-sources ``~/.profile``,
        # ``~/.bash_profile``, and ``~/.bashrc`` (in that order) if the
        # snapshot shell is bash (this is the ``auto_source_bashrc``
        # behaviour — disable with that key if you want strict login-only
        # semantics).
        "shell_init_files": [],
        # When true (default), Hairball sources the user's shell rc files
        ...
        # docker_env lets you specify exact key-value pairs — useful when Hairball
        ...
        #  "/home/user/.hairball/cache/documents:/output"]
        ...
        # bundled Hairball image, which drops to the `hairball` user via
    ...
    "web": {
        ...
        # Runtime failure recovery for web_search: tried in order when the
        # selected backend errors out (quota exhausted, rate limit, vendor
        # outage). Keyless by default so a dead paid key still leaves search
        # working; set to [] to disable and surface the primary error as-is.
    ...
    "browser": {
        ...
        "camofox": {
            # When true, Hairball sends a stable profile-scoped userId to Camofox
            # so the server maps it to a persistent Firefox profile automatically.
            # When false (default), each session gets a random userId (ephemeral).
            "managed_persistence": False,
            # Optional externally managed Camofox identity. Useful when another
            # app owns the visible browser and Hairball should operate in it.
            "user_id": "",
            "session_key": "",
    ...
    # Opt in via ``hairball chat --checkpoints`` or set enabled=True here.
    "checkpoints": {
        "enabled": False,
        ...
        "max_snapshots": 20,
        # Hard ceiling on total ``~/.hairball/checkpoints/`` size (MB).  When
        ...
        "max_file_size_mb": 10,
        # Auto-maintenance: hairball sweeps the checkpoint base at startup
    ...
    # Kernel v2 — the Hairball-owned agent core built beside the v1
    # conversation loop.  See docs/design/2026-07-29-hairball-kernel-v2-*.md.
    #
    # Off by default and inert when off: nothing is written to state.db, no
    # callbacks are wrapped, and run_turn is a plain passthrough to the
    # existing loop.  Turning it on records what v1 does as a canonical event
    # stream so v1 and v2 can be compared before anything switches over.
    "kernel": {
        # Master gate. False means the kernel never touches the runtime.
        "enabled": False,
        # Kernel version assigned to NEW sessions. Existing sessions keep the
        # version they were pinned to on first use — a session must never
        # switch cores mid-life, so flipping this never rewrites user data.
        # "v1" = conversation_loop; "v2" = KernelEngine (OpenCode-shaped loop).
        "default_version": "v1",
        # Record canonical events while v1 executes. This is the whole point of
        # the current phase; disabling it leaves the kernel with nothing to
        # observe.
        "observe_v1": True,
        # Persist token-level assistant/reasoning deltas to the ledger. Off
        # because deltas are notification, not truth (the completed message
        # carries the full text), and persisting them multiplies ledger volume
        # by the length of every response. Live subscribers get them regardless.
        "persist_stream_deltas": False,
        # Raise on a durable invariant violation (double terminal for a turn or
        # tool call) instead of writing a corrupt ledger. Keep true; it exists
        # to make the failure loud in tests rather than silent in production.
        "enforce_invariants": True,
        # Read models built by `Kernel.project()`. Available:
        # chat, workspace, sidebar, agent_tree, metrics.
        "projectors": ["chat", "workspace", "sidebar", "agent_tree"],
    },

    # No-source foreign-service boundaries (Portal OAuth, account/billing,
    # tool-gateway host). Default ``off``: BYOK provider keys
    # + direct tool keys (no community Portal/Tool Gateway replacement).
    # ``legacy`` keeps historical hosts; ``hairball`` requires explicit
    # portal_url / tool_gateway_domain — never silently falls
    # back to a foreign host.
    "managed_foreign": {
        "backend": "off",  # off | legacy | hairball
        "portal_url": "",
        "tool_gateway_domain": "",
        "hairball_tool_gateway_domain": "",
        # Deprecated compatibility input. New installs use updates.repository.
        "update_repo": "",
    },

    # Hard cap (chars) for a single automatic context file such as SOUL.md,
    # AGENTS.md, CLAUDE.md, .hairball.md, or .cursorrules before Hairball applies
    ...
    # single read_file page exceeds these limits, Hairball truncates the
    ...
    "compression": {
        ...
        "min_tail_user_messages": 1,  # REAL user messages guaranteed in the tail
                                      # (1 = existing single last-user anchor).
                                      # Raise to e.g. 3 to keep the last 3 real
                                      # user turns verbatim even when bulky tool
        ...
        "codex_gpt55_autoraise": True,  # Historical key name kept for compatibility.
                                      ...
                                      # ChatGPT OAuth route raise their compaction
                                      # trigger to 85% (vs the global `threshold` above).
                                      # reference hard-caps these families at a 272K window, so
                                      # the default 50% would compact at ~136K and waste half
                                      # the usable context. Set to False to opt back down to
                                      # the global threshold (e.g. 0.50) for those reference
                                      ...
                                      # regardless.
        "codex_gpt55_autoraise_notice": True,  # Display the one-time reference gpt-5.4/5.5/5.6
                                      # autoraise banner. Set False to keep the
                                      # 85% threshold autoraise but suppress the
                                      # user-facing notice in CLI/gateway output.
        "codex_app_server_auto": "native",  # reference app-server (codex CLI runtime) thread
                                      # compaction mode. The codex agent owns the real
                                      # thread context, so Hairball' summarizer cannot
                                      # shrink it (#36801). native = codex decides when
                                      # to compact its own thread (default); hairball =
                                      # Hairball' compression threshold triggers
        ...
        # Opt-in vision glyph frames for overflow text (snapcompact-style).
        # Default off — requires Pillow and a vision-capable model path.
        "vision_frames": False,
        "vision_frames_max": 4,
        # Shape variant name or "auto" (provider/model eval winners).
        "vision_frames_shape": "auto",
        # Append estimated savings to ~/.hairball/snapcompact-savings.jsonl.
        "vision_frames_journal": True,
    },

    # DAP / debug-session policy (Hairball-owned upgrades over raw DAP parity).
    "debug": {
        # Drop DAP Output events with category=telemetry from captured_output.
        # Program stdout/stderr still join("") like the upstream contract.
        "filter_telemetry_output": True,
    },

    # Kanban subsystem (orchestrator workers + dispatcher-driven child tasks).
    # See tools/kanban_tools.py and hairball_cli/kanban_db.py for the actual
    # implementations. Per-platform notification opt-out is handled by the
    # kanban dashboard (see ``hairball dashboard`` -> Notifications).
    "kanban": {
    ...
    "auxiliary": {
        ...
        # Invoked by ``hairball kanban specify`` (single id or --all). Set a
        ...
        # Invoked by ``hairball kanban decompose`` and the kanban
        ...
        # ``hairball profile describe <name> --auto`` and the dashboard's
        ...
        # model; override via `hairball model` → auxiliary → Curator to route
    ...
    "display": {
        ...
        # Which interface bare `hairball` (and `hairball chat`) launches by default:
        ...
        # REPL and `--tui` (or HAIRBALL_TUI=1) forces the TUI regardless of config.
        "interface": "cli",
        # When true, `hairball --tui` auto-resumes the most recent human-
        # facing session on launch instead of forging a fresh one.
        # Mirrors `hairball -c` muscle memory.  Default off so existing
        # users aren't surprised.  HAIRBALL_TUI_RESUME=<id> always wins.
        "tui_auto_resume_recent": False,
        # When true (default), `hairball --tui` drops a one-time hint
        ...
        # Account credits status-bar notices (usage bands, grant-spent, depleted /
        ...
        # can set `path`/`paths` to HAIRBALL_HOME-relative YAML files/directories
        ...
        # e.g. `model · 68% · ~/projects/hairball`. Per-platform overrides go under
        ...
        # CLI, TUI, and desktop app. Manage with `hairball pets`. Disabled until
        ...
        "pet": {
            ...
            # get_hairball_home()/pets/. Empty → first installed pet.
            "slug": "",
    ...
    "dashboard": {
        ...
        # ``--insecure`` is not). The bundled Managed Portal plugin reads
        # both keys at startup; they are the canonical surface for these
        # settings. Each can be overridden by an environment variable —
        # ``HAIRBALL_DASHBOARD_OAUTH_CLIENT_ID`` and
        # ``HAIRBALL_DASHBOARD_PORTAL_URL`` respectively — and the env var
        ...
        # (``HAIRBALL_DASHBOARD_BASIC_AUTH_USERNAME`` /
        ...
        # managed-account-service at deploy time via the
        # ``HAIRBALL_DASHBOARD_DRAIN_SECRET`` env var (the .env-is-for-secrets
        ...
        # Public URL override (env: ``HAIRBALL_DASHBOARD_PUBLIC_URL``).
        # When set, this is the complete authority — scheme + host +
        # optional path prefix (e.g. ``https://example.com/hairball``) —
    ...
    "tts": {
        ...
        "gemini": {
            ...
            # transcript section; Hairball appends the live transcript when absent.
            "persona_prompt_file": "",
        },
        "xai": {
        ...
        "piper": {
            ...
            # "voices_dir": "",        # Override voice cache dir; default = ~/.hairball/cache/piper-voices/
    ...
    "voice": {
        ...
        "beep_volume": 0.3,           # Audio cue volume (0.0-1.0)
        "silence_threshold": 200,     # RMS below this = silence (0-32767)
        "silence_duration": 3.0,      # Seconds of silence before auto-stop
        "barge_in": True,             # Let speech interrupt generation/playback
        "barge_in_threshold_multiplier": 3.0,
        "barge_in_grace_seconds": 0.5,
        "thinking_sound": True,       # Quiet status cue during long voice turns
        "stop_phrases": ["stop"],    # Exact whole-utterance voice stop commands
    },
    ...
    # a plugin in plugins/context_engine/<name>/ or ~/.hairball/plugins/.
    "context": {
        "engine": "compressor",
    },
    ...
    # configured providers (OpenRouter, managed, Z.ai, Kimi, etc.) are supported.
    "delegation": {
        ...
        # trim, the full text is spilled to ~/.hairball/cache/delegation/
        ...
        # Collab Mailbox Single Rail (doorbell wait + collect; no inject turn).
        # Default ON — primary multi-agent orchestration. Tools gated by
        # check_fn; async_delegation always suppress_inject regardless.
        "collab_v2": {
            "enabled": True,
            "max_concurrent": 4,
            ...
            # Soft caps for fork_turns=all / N and followup history resume.
            "fork_turns_max_messages": 40,
            "history_max_messages": 60,
            # Autosurface (wait woke / tool-tail reminder / cross-turn).
            # Off → classic wait+collect only (rollback knob).
            "autosurface_enabled": True,
        },
    },
    ...
    # response. If not, Hairball feeds a continuation prompt back into the
    ...
    "goals": {
        # Max continuation turns before Hairball auto-pauses the goal and
    ...
    "moa": {
        ...
        # <hairball_home>/moa-traces/<session_id>.jsonl. Off by default — turn it
    ...
    # always goes to ~/.hairball/skills/.
    "skills": {
        "external_dirs": [],   # e.g. ["~/.agents/skills", "/shared/team-skills"]
        # Substitute ${HAIRBALL_SKILL_DIR} and ${HAIRBALL_SESSION_ID} in SKILL.md
    ...
    # cron daemon.
    #
    # See `hairball curator status` for the last run summary.
    "curator": {
        "enabled": True,
        ...
        # class-level umbrellas. `hairball curator run --consolidate` overrides
        ...
        # normally restored on every `hairball update`, so pruning them only
        ...
        # skipped), snapshot ~/.hairball/skills/ into
        # ~/.hairball/skills/.curator_backups/<utc-iso>/skills.tar.gz so the
        # user can roll back with `hairball curator rollback`.
        "backup": {
            "enabled": True,
    ...
    # This section is only needed for hairball-specific overrides; everything else
    ...
    "whatsapp": {
        ...
        # Default (None) uses the built-in "ฅ *Hairball Agent*" header.
    ...
    "approvals": {
        ...
        # false.  TUI has its own modal overlay (HAIRBALL_TUI_NO_CONFIRM=1 to
        # opt out there).
    ...
    # replace Hairball' built-in platform hint for a single messaging platform
    ...
    # stored approval from ~/.hairball/shell-hooks-allowlist.json.
    # See `website/docs/user-guide/features/hooks.md` for schema + examples.
    "hooks": {},

    # Auto-accept shell-hook registrations without a TTY prompt.  Also
    # toggleable per-invocation via --accept-hooks or HAIRBALL_ACCEPT_HOOKS=1.
    ...
    "security": {
        ...
        # longer trigger the startup banner. Add via `hairball doctor --ack
        # <id>`; remove by editing the list directly. See
        # ``hairball_cli/security_advisories.py`` for the catalog.
        "acked_advisories": [],
        # Allow Hairball to lazy-install opt-in backend packages from PyPI
    ...
    "cron": {
        ...
        # $HAIRBALL_HOME/plugins/<name>/) to relocate the trigger — e.g. "chronos",
        ...
        # agent's existing Managed Portal token — there is no token key here.
        "chronos": {
        ...
        # Also overridable via HAIRBALL_CRON_MAX_PARALLEL env var.
        ...
        # HAIRBALL_CRON_SESSION_DB_TIMEOUT env var. 0 = unlimited (skip the bound).
    ...
    # todos to ready, and fires `hairball -p <assignee> chat -q ...` for
    ...
    "kanban": {
        ...
        # one `hairball` launches with no -p flag). This does not control the
        ...
        # decomposition is manual via `hairball kanban decompose <id>` or
    ...
    "code_execution": {
        ...
        #     hairball-agent's own python (sys.executable). Maximum isolation
    ...
    #
    # Core Hairball tools (terminal, read_file, write_file, patch,
    ...
    # Logging — controls file logging to ~/.hairball/logs/.
    ...
    # curated model lists for OpenRouter and Managed Portal from this URL,
    # falling back to the in-repo snapshot on network failure.  Lets us
    # update model picker lists without shipping a hairball-agent release.
    # The default URL is served by the docs site GitHub Pages deploy.
    "model_catalog": {
        "enabled": True,
        "url": "./website/docs/api/model-catalog.json",
        # Disk cache TTL in hours.  Beyond this, the CLI refetches on the
        # next /model or `hairball model` invocation; network failures
    ...
    "gateway": {
        ...
        # internal HAIRBALL_GATEWAY_PLATFORM_CONNECT_TIMEOUT env var, which still
        ...
        # producing ~/.hairball/sessions/sessions.json entirely.
        ...
        # the HAIRBALL_SCALE_TO_ZERO env stamp) AND messaging is relay-only/absent
        ...
        # `launchctl kickstart ai.hairball.gateway` that defenses 1-2 don't
        ...
        # system-path denylist (/etc, /proc, ~/.ssh, ~/.aws, ~/.hairball/.env,
        ...
        # behavior: files must live under the Hairball cache, under
        ...
        # user. Bridged to HAIRBALL_MEDIA_DELIVERY_STRICT.
        "strict": False,
        # Extra directories from which model-emitted bare file paths may be
        # uploaded as native gateway attachments. Files inside the Hairball
        # cache (~/.hairball/cache/{documents,images,audio,video,screenshots})
        ...
        # to HAIRBALL_MEDIA_ALLOW_DIRS at gateway startup. Tilde paths are
        ...
        # HAIRBALL_MEDIA_TRUST_RECENT_FILES. Only consulted when ``strict``
        # is true; in default mode the denylist alone gates delivery.
        "trust_recent_files": True,
        # Recency window in seconds. 600 (10 min) comfortably covers a
        # multi-tool agent turn. Bridged to HAIRBALL_MEDIA_TRUST_RECENT_SECONDS.
    ...
    # Session storage — controls automatic cleanup of ~/.hairball/state.db.
    ...
    "sessions": {
        ...
        # default of ``hairball sessions prune``.
        "retention_days": 90,
        ...
        # rewrites ``~/.hairball/sessions/session_{sid}.json`` on every turn
    ...
    # ``hairball update`` behaviour.
    "updates": {
        # Public source repository or private mirror used by install/update,
        # release links, fork detection, and the Windows ZIP fallback. This is
        # independent from Managed Portal entitlement and remains available in
        # personal/BYOK mode. Use owner/repository form.
        "repository": "hairball-agent/hairball-agent",
        # Run a full ``hairball backup``-style zip of HAIRBALL_HOME before every
        # ``hairball update``.  Backups land in ``<HAIRBALL_HOME>/backups/`` and
        # can be restored with ``hairball import <path>``.  Off by default:
        # zipping a large HAIRBALL_HOME (sessions DB, caches, skills) can add
        # minutes to every update.  The #48200 incident — a ``hairball update
        ...
        "backup_keep": 5,
        # What `hairball update` does with uncommitted local changes to the
    ...
    # ``patch``.
    #
    # Workspace gate: prefer git worktree (cwd or file).  Otherwise
    # fall back to project markers / file parent so scratch trees
    # (e.g. /tmp demos) still get servers.  Home and filesystem root
    # are never synthetic workspaces (messaging-on-home stays safe).
    # Set ``require_git_workspace: true`` for the old git-only gate.
    "lsp": {
        ...
        # When true, the model-facing ``lsp`` tool may appear in schema
        # (still gated by check_fn and coding-surface mount).  Post-write
        # diagnostics on write_file/patch are independent of this flag.
        "model_tools": True,

        # When true, write_file/patch ask the language server to format
        # the file after the first atomic write (textDocument/formatting)
        # and rewrite if edits apply.  Default false (matches common
        # agent settings — avoid silent indent changes). Independent of
        # model_tools and of post-write diagnostics.
        "format_on_write": False,

        # When true, post-write diagnostics (and format-on-write for JS/TS/
        # JSON/CSS) may use the Biome CLI when ``biome`` is on PATH.
        # OMP-style: CLI JSON reporter, not a Biome LSP ServerDef (stale
        # diagnostics). Set false to disable even if the binary exists.
        "biome": True,

        # When true, LSP only runs inside a git worktree (legacy legacy
        # gate — keeps messaging-on-home from spawning servers).  Default
        # false: non-git trees (e.g. /tmp demos) use project markers or
        # the file's parent directory as the workspace.  Home `/` still
        # refused as a synthetic root.
        "require_git_workspace": False,

        # Seconds to keep a (server, workspace) pair in the broken map
        # after spawn/initialize failure before allowing a retry.
        # Matches OMP-style negative cache (~3 min).  0 = permanent until
        # reload/restart.  reload/restart always clear immediately.
        "broken_backoff_seconds": 180,

        # Post-write diagnostics on write_file/patch (OMP writethrough).
        # Independent of ``model_tools``. When false, writes skip the
        # LSP/Biome diagnostics attach (format_on_write still separate).
        "writethrough": True,

        # Inline wait budget for post-write diagnostics (milliseconds).
        # Matches Oh My Pi ``INLINE_DIAGNOSTICS_WAIT_TIMEOUT_MS`` (=500).
        # On timeout the write still succeeds with no ``lsp_diagnostics``
        # field — never fail the edit. Deferred late-inject is Phase 2.
        "inline_diagnostics_timeout_ms": 500,

        # Diagnostic-wait mode for the post-write check.
        # ``"document"`` waits up to the inline budget (or ``wait_timeout``
        # when callers pass an explicit longer timeout); ``"full"``
        # additionally requests workspace-wide diagnostics (slower).
        "wait_mode": "document",
        "wait_timeout": 5.0,

        # How to handle missing server binaries.
        # ``"auto"`` — try to install via npm/go/pip into
        #              ``<HAIRBALL_HOME>/lsp/bin/`` on first use.
    ...
    # `hairball tools`. These settings tune the backing Responses API call.
    "x_search": {
    ...
    # rather than storing them in ~/.hairball/.env.
    "secrets": {
        ...
        "bitwarden": {
            ...
            # in ~/.hairball/.env (or your shell) and never in config.yaml.
            ...
            # ~/.hairball/bin/ on first use.  When False you must install
            ...
            # `hairball secrets bitwarden setup`.
            "server_url": "",
        },
        "onepassword": {
            ...
            # for headless auth.  Sourced from ~/.hairball/.env (or the shell)
    ...
    "computer_use": {
        ...
        # by default upstream. Hairball disables it for our users unless they
        # explicitly opt in here. When false (default), Hairball sets
    ...
    # Hairball Desktop (Electron app) launch options. These only affect
    # `hairball desktop`; they do not touch the CLI/gateway.
    "desktop": {
        ...
        # Bridged to the HAIRBALL_DESKTOP_DISABLE_GPU env var the Electron app reads.
        "disable_gpu": "auto",
    },
...
# selection step (Managed Portal / OpenRouter / Custom endpoint), so this
...
OPTIONAL_ENV_VARS = {
    ...
    "MANAGED_BASE_URL": {
        "description": "Managed Portal base URL override",
        "prompt": "Managed Portal base URL (leave empty for default)",
        "url": None,
        "password": False,
        "category": "provider",
        "advanced": True,
    ...
    "VERTEX_CREDENTIALS_PATH": {
        "description": "Path to a Google Cloud service account JSON for Vertex AI (Gemini). "
                       ...
                       "credentials Hairball mints short-lived tokens from. Falls back to "
    ...
    "XIAOMI_API_KEY": {
        "description": "Xiaomi API key for provider models (mimo-v2.5-pro, mimo-v2.5, mimo-v2-pro, mimo-v2-omni, mimo-v2-flash)",
        "prompt": "Xiaomi API Key",
        "url": "https://platform.xiaomimimo.com",
        "password": True,
        "category": "provider",
    },
    "XIAOMI_BASE_URL": {
        "description": "Xiaomi base URL override (default: https://api.xiaomimimo.com/v1)",
    ...
    "AZURE_FOUNDRY_BASE_URL": {
        "description": "Azure Foundry base URL (set via 'hairball model' for endpoint-specific config)",
    ...
    "FIRECRAWL_GATEWAY_URL": {
        "description": "Exact Firecrawl tool-gateway origin override for managed Subscribers only (optional)",
    ...
    "TOOL_GATEWAY_DOMAIN": {
        "description": "Shared tool-gateway domain suffix for managed Subscribers only, used to derive vendor hosts, e.g. hairball.com -> firecrawl-gateway.hairball.com",
    ...
    "TOOL_GATEWAY_SCHEME": {
        "description": "Shared tool-gateway URL scheme for managed Subscribers only, used to derive vendor hosts (`https` by default, set `http` for local gateway testing)",
    ...
    "TOOL_GATEWAY_USER_TOKEN": {
        "description": "Explicit managed Subscriber access token for tool-gateway requests (optional; otherwise read from the Hairball auth store)",
    ...
    "SLACK_ALLOWED_USERS": {
        "description": "Comma-separated Slack member IDs allowed to use Hairball, e.g. U01ABC2DEF3. Without this, Slack may connect but deny messages by default.",
    ...
    "MATRIX_USER_ID": {
        "description": "Matrix user ID (e.g. @hairball:example.org)",
    ...
    "MATRIX_DEVICE_ID": {
        "description": "Stable Matrix device ID for E2EE persistence across restarts (e.g. HAIRBALL_BOT)",
    ...
    "IRC_CHANNEL": {
        "description": "IRC channel to join (e.g. #hairball)",
        "prompt": "IRC channel",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "IRC_NICKNAME": {
        "description": "Bot nickname on IRC (default: hairball-bot)",
        "prompt": "IRC nickname",
        "url": None,
        "password": False,
        "category": "messaging",
    ...
    "API_SERVER_MODEL_NAME": {
        "description": "Model name advertised on /v1/models. Defaults to the profile name (or 'hairball-agent' for the default profile). Useful for multi-user setups with OpenWebUI.",
    ...
    "GATEWAY_PROXY_URL": {
        "description": "URL of a remote Hairball API server to forward messages to (proxy mode). When set, the gateway handles platform I/O only — all agent work is delegated to the remote server. Use for Docker E2EE containers that relay to a host agent. Also configurable via gateway.proxy_url in config.yaml.",
        "prompt": "Remote Hairball API server URL (e.g. http://192.168.1.100:8642)",
        "url": None,
        "password": False,
        "category": "messaging",
        "advanced": True,
    ...
    "GATEWAY_PROXY_KEY": {
        "description": "Bearer token for authenticating with the remote Hairball API server (proxy mode). Must match the API_SERVER_KEY on the remote host.",
    ...
    # HAIRBALL_TOOL_PROGRESS and HAIRBALL_TOOL_PROGRESS_MODE are deprecated —
    ...
    "HAIRBALL_PREFILL_MESSAGES_FILE": {
    ...
    "HAIRBALL_EPHEMERAL_SYSTEM_PROMPT": {
...
def get_missing_skill_config_vars() -> List[Dict[str, Any]]:
    ...
    Scans all enabled skills for ``metadata.hairball.config`` entries, then checks
    ...
    try:
        ...
        # should never break `hairball update`.  Skill-config prompting is a
...
def _normalize_custom_provider_entry(
    ...
    *,
    ...
    _KNOWN_KEYS = {
        ...
        # unused here, but Hairball' own config writer has historically emitted it
    ...
    if isinstance(models, dict) and models:
        ...
        # Hand-edited configs (and older Hairball versions) may write
...
def get_custom_provider_context_length(
    ...
    used by:
      ...
      * ``hairball_cli.model_switch.resolve_display_context_length`` (``/model`` confirmation display)
...
def validate_config_structure(config: Optional[Dict[str, Any]] = None) -> List["ConfigIssue"]:
    ...
    if config is None:
        try:
            ...
            return [ConfigIssue("error", "Could not load config.yaml", "Run 'hairball setup' to create a valid config")]
    ...
    if cp and not model_cfg:
        issues.append(ConfigIssue(
            "warning",
            "custom_providers defined but no 'model' section — Hairball won't know which provider to use",
            "Add a model section:\n"
            "  model:\n"
            "    provider: custom\n"
...
def print_config_warnings(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    lines.append("  \033[2mRun 'hairball doctor' for fix suggestions.\033[0m")
    sys.stderr.write("\n".join(lines) + "\n\n")
...
def warn_deprecated_cwd_env_vars(config: Optional[Dict[str, Any]] = None) -> None:
    ...
    if lines:
        hint_path = os.environ.get("HAIRBALL_HOME", "~/.hairball")
...
def _persist_migration(config: Dict[str, Any]) -> None:
    ...
    DEFAULT_CONFIG dumps (the "hairball update / hairball -p blows up my config"
    reports).
...
def migrate_config(interactive: bool = True, quiet: bool = False) -> Dict[str, Any]:
    ...
    if current_ver < 4:
        ...
        if "tool_progress" not in display:
            old_enabled = get_env_value("HAIRBALL_TOOL_PROGRESS")
            old_mode = get_env_value("HAIRBALL_TOOL_PROGRESS_MODE")
            if old_enabled and old_enabled.lower() in {"false", "0", "no"}:
                display["tool_progress"] = "off"
                results["config_added"].append("display.tool_progress=off (from HAIRBALL_TOOL_PROGRESS=false)")
            elif old_mode and old_mode.lower() in {"new", "all", "verbose"}:
                display["tool_progress"] = old_mode.lower()
                results["config_added"].append(f"display.tool_progress={old_mode.lower()} (from HAIRBALL_TOOL_PROGRESS_MODE)")
            else:
    ...
    if current_ver < 5:
        ...
        if "timezone" not in config:
            old_tz = os.getenv("HAIRBALL_TIMEZONE", "")
            if old_tz and old_tz.strip():
                config["timezone"] = old_tz.strip()
                results["config_added"].append(f"timezone={old_tz.strip()} (from HAIRBALL_TIMEZONE)")
            else:
                config["timezone"] = ""
    ...
    if current_ver < 21:
        ...
        if "enabled" not in plugins_cfg:
            ...
            # Scan ``$HAIRBALL_HOME/plugins/`` for currently installed user plugins.
            grandfathered: List[str] = []
            try:
                user_plugins_dir = get_hairball_home() / "plugins"
            ...
            if not quiet:
                if grandfathered:
                    ...
                    print(
                        ...
                        "Use `hairball plugins enable <name>` to activate."
                    )
    ...
    # settings in their `config.yaml`, and `hairball curator status` has no
    ...
    #   3. Creates `~/.hairball/logs/curator/` if missing (belt-and-suspenders
    #      on top of ensure_hairball_home() — old profiles that predate this
    #      migration still benefit).
    if current_ver < 23:
        try:
            curator_dir = get_hairball_home() / "logs" / "curator"
            ...
            if added_curator:
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_curator)}) — edit via `hairball config set`"
                    )
            if added_aux:
                results["config_added"].append(
                ...
                if not quiet:
                    print(
                        ...
                        f"({', '.join(added_aux)}) — edit via `hairball config set`"
                    )
    ...
    # behavior set it to true explicitly via `hairball config set`.
    ...
    # ── Version 33 → 34: coding focus default + Biome post-write rail ──
    # Product default for agent.coding_context is now ``focus`` (lean coding
    # schema). Migrate lingering ``auto`` from the previous default so existing
    # installs pick up the lean rail; users who want the wide schema can set
    # ``auto`` again after migration. Also ensure ``lsp.biome`` exists.
    if current_ver < 34:
        config = read_raw_config()
        touched = False
        ...
        if not isinstance(agent, dict):
            agent = {}
            config["agent"] = agent
        if agent.get("coding_context") == "auto":
            agent["coding_context"] = "focus"
            touched = True
            results["config_added"].append(
                "agent.coding_context=focus (migrated from previous default auto)"
            )
        lsp = config.get("lsp")
        ...
        if "biome" not in lsp:
            lsp["biome"] = True
            touched = True
            results["config_added"].append("lsp.biome=true (default)")
        comp = config.get("compression")
        if not isinstance(comp, dict):
            comp = {}
            config["compression"] = comp
        if "vision_frames" not in comp:
            comp["vision_frames"] = False
            touched = True
            results["config_added"].append(
                "compression.vision_frames=false (snapcompact gate, off by default)"
            )
        if touched:
            _persist_migration(config)
            if not quiet:
                print(
                    "  ✓ Coding default is focus; Biome post-write rail on; "
                    "vision_frames compression gated off"
                )
    ...
    if isinstance(raw_mcp_servers, dict):
        try:
            from hairball_cli.mcp_security import validate_mcp_server_entry as _validate_mcp_server_entry
        except Exception:
    ...
    try:
        ...
        from hairball_cli.toolset_validation import validate_platform_toolsets
    ...
    if new_var_names and interactive and not quiet:
        ...
        if new_and_unset:
            ...
            if answer in {"y", "yes"}:
                ...
                print("  Set later with: hairball config set <key> <value>")
    ...
    # `hairball update`, but only the version bump is persisted.
    ...
    # metadata.hairball.config in their SKILL.md frontmatter.
...
def _env_ref_snapshot(obj, snapshot=None):
    ...
    ``load_hairball_dotenv()`` populated the process env, or an env var
...
def _normalize_root_model_keys(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    intuitive name OpenAI-SDK / LiteLLM users reach for, and ``hairball config set``
    ...
    backends) — while display paths (``hairball status``/``dump``) read ``name``
...
def cfg_get(cfg: Optional[Dict[str, Any]], *keys: str, default: Any = None) -> Any:
    ...
    ubiquitous ``cfg_path = _hairball_home / "config.yaml"`` local variable
...
def read_raw_config() -> Dict[str, Any]:
    """Read ~/.hairball/config.yaml as-is, without merging defaults or migrating.
...
def load_config() -> Dict[str, Any]:
    """Load configuration from ~/.hairball/config.yaml.
    ...
    (which change ``HAIRBALL_HOME`` and therefore ``get_config_path()``)
    don't collide.
...
TERMINAL_CONFIG_ENV_MAP = {
    ...
    "local_network": "TERMINAL_LOCAL_NETWORK",
    "local_writable_roots": "TERMINAL_LOCAL_WRITABLE_ROOTS",
...
def _load_config_impl(*, want_deepcopy: bool) -> Dict[str, Any]:
    with _CONFIG_LOCK:
        ...
        # cache signature so editing /etc/hairball/config.yaml invalidates the
        ...
        if cached is not None and cache_sig is not None and cached[:4] == cache_sig:
            ...
            # Without this, a load_config() that ran before load_hairball_dotenv()
...
#   openai-codex (OAuth — hairball auth) — ChatGPT
#   managed         (OAuth — hairball auth) — Managed Portal
...
def save_config(
    ...
    *,
    ...
    """Save configuration to ~/.hairball/config.yaml.\n
...
def _parse_env_value(raw_value: str) -> str:
    """Parse the small .env value subset Hairball writes itself."""
...
def load_env() -> Dict[str, str]:
    """Load environment variables from ~/.hairball/.env.
    ...
    interactive menu render (`hairball tools`, `hairball setup`, status
    panels). Sanitisation is O(lines × known-keys), so re-parsing the
    same file on every call was burning ~300ms of CPU per `hairball tools`
...
def _sanitize_env_lines(lines: list) -> list:
    ...
    split on real Hairball env var names, avoiding false positives from values
...
def sanitize_env_file() -> int:
    """Read, sanitize, and rewrite ~/.hairball/.env in place.
...
def save_env_value(key: str, value: str):
    """Save or update a value in ~/.hairball/.env."""
    if is_managed():
...
def remove_env_value(key: str) -> bool:
    """Remove a key from ~/.hairball/.env and os.environ.
...
def reload_env() -> int:
    """Re-read ~/.hairball/.env into os.environ. Returns count of vars updated.

    Adds/updates vars that changed and removes vars that were deleted from
    the .env file (but only vars known to Hairball — OPTIONAL_ENV_VARS and
    ...
    for key, value in env_vars.items():
        if os.environ.get(key) != value:
            os.environ[key] = value
            count += 1
    # Remove known Hairball vars that are no longer in .env
...
def get_env_value(key: str) -> Optional[str]:
    """Get a value from ~/.hairball/.env or environment."""
...
def get_env_value_prefer_dotenv(key: str) -> Optional[str]:
    """Resolve a credential env value, preferring ``~/.hairball/.env`` over ``os.environ``.

    Used for Hairball-managed credentials where a deliberate edit to ``.env``
    must take precedence over a stale value inherited from the parent shell
    (reference CLI, test scripts, login profile exports). Without this, rotating
...
def show_config():
    ...
    print(color("│              ฅ Hairball Configuration                    │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    from hairball_cli.auth import get_anthropic_key
    ...
    # Warn on stale HAIRBALL_MAX_ITERATIONS ghost in .env that disagrees with
    ...
    try:
        _env_ghost = load_env().get("HAIRBALL_MAX_ITERATIONS")
        if _env_ghost is not None and str(_env_ghost).strip() != str(_cfg_max_turns).strip():
            print(color(
                f"                ⚠ .env has stale HAIRBALL_MAX_ITERATIONS={_env_ghost} "
                f"(run 'hairball doctor --fix' to remove)",
                Colors.YELLOW,
            ))
    except Exception:
        pass
    ...
    print(color("  hairball config edit     # Edit config file", Colors.DIM))
    print(color("  hairball config set <key> <value>", Colors.DIM))
    print(color("  hairball setup           # Run setup wizard", Colors.DIM))
    print()
...
def set_config_value(key: str, value: str):
    ...
    # so a fresh `hairball config set model.api_base ...` lands on the canonical
    ...
    # — e.g. `hairball config set model.api_key cfut_...` routes to config.yaml
...
def config_command(args):
    ...
    if subcmd is None or subcmd == "show":
        ...
        if not key or value is None:
            print("Usage: hairball config set <key> <value>")
            print()
            print("Examples:")
            print("  hairball config set model anthropic/claude-sonnet-4")
            print("  hairball config set terminal.backend docker")
            print("  hairball config set OPENROUTER_API_KEY sk-or-...")
            sys.exit(1)
        ...
        if missing_config:
            ...
            print("    Run 'hairball config migrate' to add them")
        ...
        print("  hairball config           Show current configuration")
        print("  hairball config edit      Open config in editor")
        print("  hairball config set <key> <value>   Set a config value")
        print("  hairball config check     Check for missing/outdated config")
        print("  hairball config migrate   Update config with new options")
        print("  hairball config path      Show config file path")
        print("  hairball config env-path  Show .env file path")
        sys.exit(1)
...
# ``_inject_profile_env_vars`` surfaces them in ``hairball config`` UI so users
