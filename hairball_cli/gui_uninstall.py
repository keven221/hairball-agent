"""
Hairball Desktop (Chat GUI) uninstaller.
...
the Python agent or the user's config/data:
  ...
  1. Source-built GUI (``hairball desktop`` / ``hairball gui``)
     Built inside the agent checkout under ``$HAIRBALL_HOME/hairball-agent/``:
       ...
       - ``$HAIRBALL_HOME/desktop-build-stamp.json`` (the build freshness stamp)
  ...
  2. Packaged distributable (DMG / NSIS / AppImage / deb / rpm)
     ...
     own bundled Electron + a per-user Electron ``userData`` directory:
       - macOS:   ``/Applications/Hairball.app`` or ``~/Applications/Hairball.app``
       - Windows: ``%LOCALAPPDATA%\\Programs\\Hairball`` (NSIS per-user)
...
the app name ("Hairball"), separate from ``$HAIRBALL_HOME``:
  - macOS:   ``~/Library/Application Support/Hairball``
  - Windows: ``%APPDATA%\\Hairball``
  - Linux:   ``$XDG_CONFIG_HOME/Hairball`` (default ``~/.config/Hairball``)
...
``hairball uninstall --gui`` (and friends) without paying for the full CLI.
"""
...
def _agent_root(hairball_home: Path) -> Path:
    """The agent checkout root — same layout install.sh / install.ps1 use."""
    return hairball_home / "hairball-agent"


def desktop_userdata_dir() -> Path:
    """Return the Electron ``userData`` directory for the desktop app.

    Mirrors Electron's ``app.getPath('userData')`` for an app named "Hairball"
    ...
    if sys.platform == "darwin":
        return home / "Library" / "Application Support" / "Hairball"
...
def source_built_gui_artifacts(hairball_home: Path) -> "list[Path]":
    """GUI build artifacts produced by ``hairball desktop`` inside the checkout.

    These are removable on a GUI uninstall without harming the agent: the
    Python agent runs from ``hairball-agent/`` source + ``venv/`` and never
    needs the Electron build output or node_modules.
    """
    agent_root = _agent_root(hairball_home)
    ...
    return [
        ...
        hairball_home / "desktop-build-stamp.json",
    ]
...
def packaged_gui_app_paths() -> "list[Path]":
    ...
    electron-builder output locations for the "Hairball" product.
    """
    home = Path.home()
    paths: list[Path] = []
    if sys.platform == "darwin":
        paths += [
            Path("/Applications/Hairball.app"),
            home / "Applications" / "Hairball.app",
        ]
    elif sys.platform == "win32":
        local = os.environ.get("LOCALAPPDATA")
        local_base = Path(local) if local else (home / "AppData" / "Local")
        paths += [
            # NSIS per-user install (perMachine=false → Programs\Hairball).
            local_base / "Programs" / "Hairball",
            # Older / alternate layout some builds used.
            local_base / "hairball-desktop",
        ]
        program_files = os.environ.get("ProgramFiles")
        if program_files:
            # NSIS per-machine fallback (needs admin to remove).
            paths.append(Path(program_files) / "Hairball")
    else:
        ...
        paths += [
            data_base / "applications" / "hairball.desktop",
            data_base / "applications" / "Hairball.desktop",
        ]
    return paths


def agent_is_installed(hairball_home: Path) -> bool:
    """Return True when a usable Python agent install exists under HAIRBALL_HOME.
    ...
    if (agent_root / "hairball_cli").is_dir():
        return True
...
def gui_is_installed(hairball_home: Path) -> bool:
    """Return True when any desktop GUI artifact exists (built or packaged)."""
    for p in source_built_gui_artifacts(hairball_home):
        if p.exists():
            return True
...
def gui_install_summary(hairball_home: "Path | None" = None) -> dict:
    ...
    """
    home: Path = hairball_home if hairball_home is not None else get_hairball_home()
...
def uninstall_gui(hairball_home: "Path | None" = None, *, remove_userdata: bool = True) -> "list[Path]":
    ...
    Never touches ``hairball-agent/hairball_cli`` (agent source), ``venv/``, or any
    config / sessions / .env under ``$HAIRBALL_HOME``.
    ...
    if sys.platform.startswith("linux"):
        log_info(
            ...
            "with your package manager (e.g. 'sudo apt remove hairball' or "
            "'sudo dnf remove hairball'). AppImage builds are a single file you "
