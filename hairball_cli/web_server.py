"""
Hairball Agent — Web UI server.

Provides a FastAPI backend serving the Vite/React frontend and REST API
endpoints for managing configuration, environment variables, and sessions.

Usage:
    python -m hairball_cli.main web          # Start on http://127.0.0.1:9119
    python -m hairball_cli.main web --port 8080
"""
...
from hairball_cli._subprocess_compat import windows_detach_flags, windows_hide_flags
...
from hairball_cli.memory_providers import (
...
try:
    ...
    # running `hairball dashboard` needs fastapi+uvicorn; lazy install keeps
...
WEB_DIST = Path(os.environ["HAIRBALL_WEB_DIST"]) if "HAIRBALL_WEB_DIST" in os.environ else Path(__file__).parent / "web_dist"
...
def _start_desktop_cron_ticker(stop_event: "threading.Event", interval: int = 60) -> None:
    ...
    The scheduler tick loop normally lives in ``hairball gateway run`` — but the
    desktop app spawns a ``hairball dashboard`` backend, not a gateway, so a cron
    ...
    real gateway on the same HAIRBALL_HOME — whichever process grabs the lock
    first wins the tick.
    """
...
def _warm_gateway_module() -> None:
    try:
        import hairball_cli.gateway  # noqa: F401
    except Exception:
        pass
...
def _resolve_restart_drain_timeout() -> float:
    try:
        from hairball_cli.gateway import _get_restart_drain_timeout
...
async def _lifespan(app: "FastAPI"):
    ...
    # Fire hairball_cli.gateway import into a background thread so the event
    # loop is not blocked and HAIRBALL_DASHBOARD_READY fires without delay.
    ...
    # Desktop-spawned backends (HAIRBALL_DESKTOP=1) fire cron jobs themselves,
    # since the app has no gateway running the scheduler. Server `hairball
    # dashboard` is unaffected — it relies on its own gateway.
    cron_stop: "threading.Event | None" = None
    cron_thread: "threading.Thread | None" = None
    if os.getenv("HAIRBALL_DESKTOP") == "1":
...
app = FastAPI(title="Hairball Agent", version=__version__, lifespan=_lifespan)

# Memory-provider OAuth connect routes live in the memory layer, not here.
from hairball_cli.memory_oauth import router as _memory_oauth_router  # noqa: E402
...
# HAIRBALL_DASHBOARD_SESSION_TOKEN so its main process can authenticate the
...
# ---------------------------------------------------------------------------
_SESSION_TOKEN = os.environ.get("HAIRBALL_DASHBOARD_SESSION_TOKEN") or secrets.token_urlsafe(32)
_SESSION_HEADER_NAME = "X-Hairball-Session-Token"
...
#
# This list is defined in ``hairball_cli.dashboard_auth.public_paths`` so the
...
# ---------------------------------------------------------------------------
from hairball_cli.dashboard_auth.public_paths import (
...
def _require_token(request: Request) -> None:
    ...
    * **Loopback / ``--insecure`` mode** (``auth_required`` False): the
      ...
      back via ``X-Hairball-Session-Token`` (or the legacy ``Bearer`` header).
      Validate it here.
...
def should_require_auth(host: str, allow_public: bool = False) -> bool:
    ...
    unauthenticated-public-dashboard hole behind the June 2026 ``hairball-0day``
    ...
    if path.startswith("/api/plugins/"):
        ...
        if _authed:
            ...
            if len(parts) >= 4:
                ...
                if plugin_name:
                    try:
                        from hairball_cli.plugins_cmd import (
                            _get_enabled_set,
                            _get_disabled_set,
                        )
...
async def _dashboard_auth_gate(request: Request, call_next):
    from hairball_cli.dashboard_auth.middleware import gated_auth_middleware
...
async def _token_auth_seam(request: Request, call_next):
    ...
    through untouched.
    """
    from hairball_cli.dashboard_auth.token_auth import token_auth_middleware
...
_SCHEMA_OVERRIDES: Dict[str, Dict[str, Any]] = {
    ...
    "updates.non_interactive_local_changes": {
        "type": "select",
        "description": (
            "When the chat app / gateway updates Hairball (no terminal prompt), "
    ...
    "updates.repository": {
        "type": "string",
        "description": (
            "Hairball source or private mirror in owner/repository form. Used "
            "consistently by update checks, release links, fork detection, and "
            "the Windows ZIP fallback; independent from Managed Portal access."
        ),
    },
    "updates.refresh_cua_driver": {
        "type": "bool",
        "description": (
            "Refresh an already-installed cua-driver during hairball update. "
...
_CATEGORY_MERGE: Dict[str, str] = {
    ...
    # orphan category.
    "computer_use": "agent",
    # ``debug.filter_telemetry_output`` is the only dashboard-facing debug
    # policy today. Keep it with the agent runtime controls instead of
    # creating a one-setting tab.
    "debug": "agent",
}
...
class ModelAssignment(BaseModel):
    ...
    # ``hairball model`` custom flow collects. Honored only on the main slot for
...
def _normalize_main_model_assignment(provider: str, model: str) -> tuple[str, str]:
    ...
    - The "Change" picker sends a real Hairball provider slug — fine.
    ...
    1. Vendor-name → Hairball-provider mapping: when the provider string is
       not a known Hairball provider/alias (e.g. ``moonshotai``, ``x-ai`` is
    ...
    """
    from hairball_cli.config import get_compatible_custom_providers
    from hairball_cli.models import _KNOWN_PROVIDER_NAMES, normalize_provider
    from hairball_cli.model_normalize import normalize_model_for_provider
    from hairball_cli.providers import resolve_custom_provider, resolve_user_provider
    ...
    if canonical not in _KNOWN_PROVIDER_NAMES and "/" in model_in:
        ...
        try:
            ...
            cur_provider = ""
        from hairball_cli.models import _AGGREGATOR_PROVIDERS
...
def _apply_main_model_assignment(
    ...
    - Otherwise, a stale ``base_url`` is cleared ONLY when switching to a
      *different* provider — that URL belonged to the old provider. When the
      ...
      Xiaomi Token Plan host, ``https://token-plan-*.xiaomimimo.com/v1``)
...
def _count_status_active_sessions() -> int:
    ...
    while another Hairball process is writing to it.
    """
    from hairball_state import DEFAULT_DB_PATH, SessionDB
...
_MANAGED_FILES_ROOT_ENV = "HAIRBALL_DASHBOARD_FILES_ROOT"
...
# HAIRBALL_HOME itself, at which point every one of these basenames is a live
...
# without needing to resolve them relative to HAIRBALL_HOME.
...
def _is_sensitive_filename(name: str) -> bool:
    ...
    canonical Hairball credential-store basenames (see
...
def _is_sensitive_path(path: Path) -> bool:
    ...
    and needs no HAIRBALL_HOME resolution, so it blocks these trees wherever
...
def _default_hairball_root_is_opt_data() -> bool:
    raw = os.environ.get("HAIRBALL_HOME", "").strip()
    if not raw:
        return False
    try:
        from hairball_constants import get_default_hairball_root

        root = get_default_hairball_root().expanduser().resolve(strict=False)
    except (OSError, RuntimeError):
...
def _dashboard_local_update_managed_externally() -> bool:
    """Return true when the dashboard should not offer ``hairball update``.
    ...
    a container — e.g. the hairball-webui image sharing the Hairball source tree),
    the dashboard's ``hairball update`` button is the correct update path and
    ...
    """
    if _default_hairball_root_is_opt_data():
        return True
    try:
...
def _managed_files_policy(request: Request, *, create_root: bool = True) -> ManagedFilesPolicy:
    ...
    # to /opt/data only when the installation's Hairball root is actually /opt/data
    # (the container/hosted layout) or when HAIRBALL_DASHBOARD_FILES_ROOT is set.
    if _default_hairball_root_is_opt_data():
...
async def upload_chat_image(payload: ChatImageUpload, profile: Optional[str] = None):
    ...
    The dashboard /chat page runs Hairball inside an xterm.js PTY. Browser
...
async def fs_write_text(payload: FsWriteText):
    ...
    Mirrors the local Electron ``hairball:fs:writeText`` hardening: the path is
    ...
    tmp = target.with_name(f".{target.name}.hairball-tmp-{os.getpid()}")
    try:
...
# gate + path hardening as /api/fs). Logic lives in ``hairball_cli.web_git``;
# these are thin, executor-offloaded wrappers (git/gh can block).
# ---------------------------------------------------------------------------

from hairball_cli import web_git as _web_git  # noqa: E402
...
def _collect_profile_gateway_topology() -> Dict[str, Any]:
    ...
    * ``gateway_mode`` — ``"multiplex"`` when the default gateway serves
    ...
    """
    try:
        from hairball_cli.profiles import _check_gateway_running, profiles_to_serve
...
async def get_status(profile: Optional[str] = None):
    ...
    # across that await. Status only resolves get_hairball_home() at call time
    ...
    try:
        ...
        # Windows install the first import of hairball_cli.gateway blocks the
        ...
        # and which providers are registered so ``hairball status`` and the
        # SPA's StatusPage can show "OAuth gate ON via Hairball" or
        # "loopback only — no auth gate" with no extra round trips.
        auth_required = bool(getattr(app.state, "auth_required", False))
        auth_providers: list[str] = []
        try:
            from hairball_cli.dashboard_auth import list_providers as _list_providers
            auth_providers = [p.name for p in _list_providers()]
        except Exception:
            # Module not importable yet (early startup) — leave as [].
            pass

        # managed bootstrap-session validity for the NAS health sweep. A hosted
        # agent whose managed auth dies terminally (invalid_grant / quarantine)
        ...
        managed_session_valid = "unknown"
        try:
            from hairball_cli.auth import get_managed_session_validity
            managed_session_valid = get_managed_session_validity()
        except Exception:
            managed_session_valid = "unknown"
        ...
        status = {
            ...
            "can_update_hairball": not _dashboard_local_update_managed_externally(),
            "can_update_hairball": not _dashboard_local_update_managed_externally(),
            "gateway_running": gateway_running,
            "gateway_state": gateway_state,
            ...
            "managed_session_valid": managed_session_valid,
        }
        ...
        # ``gateway_mode`` are low-sensitivity PRODUCT surface — Hairball Cloud
        ...
        if not auth_required:
            status.update({
                "hairball_home": str(get_hairball_home()),
...
async def get_system_stats():
    ...
    and non-sensitive (no env values, no paths beyond the hairball home root).
    """
    ...
    try:
        ...
        try:
            du = psutil.disk_usage(str(get_hairball_home()))
            info["disk"] = {
                "total": du.total,
                "used": du.used,
                "free": du.free,
...
async def run_curator():
    ...
    try:
        proc = _spawn_hairball_action(["curator", "run"], "curator-run")
    except Exception as exc:
...
# ---------------------------------------------------------------------------
# Portal endpoint — Managed Portal auth + Tool Gateway routing status (read-only).
# ---------------------------------------------------------------------------
...
async def get_portal_status():
    try:
        from hairball_cli.managed_foreign_backends import (
            is_managed_foreign_off,
            managed_foreign_disabled_message,
            portal_base_url,
        )
        ...
        if is_managed_foreign_off():
            return {
                ...
                "disabled": True,
                "message": managed_foreign_disabled_message(),
                "features": [],
            }
    except Exception:
        pass
    ...
    auth: Dict[str, Any] = {}
    try:
        from hairball_cli.auth import get_managed_auth_status
    ...
    features = []
    try:
        from hairball_cli.managed_subscription import get_managed_subscription_features

        feats = get_managed_subscription_features(cfg)
        if feats is not None:
            for feat in feats.items():
                if getattr(feat, "managed_by_managed", False):
                    state = "via Managed Portal"
    ...
    # Only advertise a subscription URL when an explicit portal host is configured
    # (legacy/hairball backends). Never hardcode a foreign default here.
    try:
        from hairball_cli.managed_foreign_backends import portal_base_url as mf_portal

        portal = (auth.get("portal_base_url") or mf_portal() or "").rstrip("/")
    except Exception:
        portal = str(auth.get("portal_base_url") or "").rstrip("/")
    subscription_url = f"{portal}/manage-subscription" if portal else None
    return {
...
async def run_prompt_size():
    try:
        proc = _spawn_hairball_action(["prompt-size"], "prompt-size")
    except Exception as exc:
...
async def run_dump():
    try:
        proc = _spawn_hairball_action(["dump"], "dump")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_config_migrate():
    try:
        proc = _spawn_hairball_action(["config", "migrate"], "config-migrate")
    except Exception as exc:
        raise HTTPException(status_code=500, detail=f"Failed: {exc}")
...
async def run_debug_share_endpoint(body: DebugShareRequest | None = None):
    ...
    *synchronous*: the whole point of ``debug share`` is the set of shareable
    ...
    """
    from hairball_cli.debug import build_debug_share
...
# streamed to a per-action log file under ``~/.hairball/logs/<action>.log`` so
# the dashboard can tail them back to the user.
# ---------------------------------------------------------------------------

_ACTION_LOG_DIR: Path = get_hairball_home() / "logs"
...
_ACTION_LOG_FILES: Dict[str, str] = {
    ...
    "hairball-update": "hairball-update.log",
    "hairball-update": "hairball-update.log",
...
def _spawn_hairball_action(subcommand: List[str], name: str) -> subprocess.Popen:
    """Spawn ``hairball <subcommand>`` detached and record the Popen handle.

    Uses the running interpreter's ``hairball_cli.main`` module so the action
    ...
    cmd = [_dashboard_spawn_executable(), "-m", "hairball_cli.main", *subcommand]
    ...
    popen_kwargs: Dict[str, Any] = {
        ...
        "env": {**os.environ, "HAIRBALL_NONINTERACTIVE": "1"},
    }
    if sys.platform == "win32":
...
def _gateway_display_command(profile: Optional[str], verb: str) -> str:
    return " ".join(["hairball", *_gateway_subcommand(profile, verb)])
...
def _spawn_gateway_restart(profile: Optional[str] = None) -> Tuple[subprocess.Popen, bool]:
    """Spawn ``hairball gateway restart``, reusing an in-flight restart.
    ...
    concurrent ``hairball gateway restart`` children race each other on the
    ...
    return _spawn_hairball_action(subcommand, "gateway-restart"), False
...
async def restart_gateway(profile: Optional[str] = None):
    """Kick off a ``hairball gateway restart`` in the background."""
    try:
...
async def gateway_drain(request: Request):
    ...
    active (no ``HAIRBALL_DASHBOARD_DRAIN_SECRET``), the route is NOT a token
...
@app.post("/api/hairball/update")
@app.post("/api/hairball/update")  # legacy path alias
async def update_hairball():
    """Kick off ``hairball update`` in the background."""
    if _dashboard_local_update_managed_externally():
        message = (
            "Hairball updates are managed outside this dashboard in "
            "containerized environments. The built-in local updater is "
            "disabled here."
        )
        _record_completed_action("hairball-update", message, exit_code=1)
        return {
            "ok": False,
            "pid": None,
            "name": "hairball-update",
    ...
    try:
        proc = _spawn_hairball_action(["update"], "hairball-update")
    except Exception as exc:
        _log.exception("Failed to spawn hairball update")
...
# Back-compat alias for older callers/tests that still import the old name.
update_hairball = update_hairball
...
@app.get("/api/hairball/update/check")
async def check_hairball_update(force: bool = False):
    """Report whether a Hairball update is available, without applying it.

    Powers the dashboard's "check before you update" flow: the System page
    shows the commit-behind count and asks the user to confirm before
    ``POST /api/hairball/update`` actually runs ``hairball update``.

    Returns:
        install_method: 'git' | 'pip' | 'docker' | 'nixos' | 'homebrew' | ...
        current_version: installed Hairball version string
    ...
    try:
        ...
        if force:
            try:
                (get_hairball_home() / ".update_check").unlink()
            except OSError:
                pass
...
async def speak_text(payload: TTSSpeakRequest):
    ...
    configured in ``~/.hairball/config.yaml`` under ``tts.``.
    """
...
def get_profiles_sessions(
    ...
    from hairball_cli import profiles as profiles_mod
...
async def search_sessions(q: str = "", limit: int = 20, profile: Optional[str] = None):
    ...
    try:
        ...
        try:
            ...
            # logs, or another Hairball surface. FTS can't find those unless the
...
def _normalize_config_for_web(config: Dict[str, Any]) -> Dict[str, Any]:
    ...
    Hairball supports ``model`` as either a bare string (``"anthropic/claude-sonnet-4"``)
...
def _read_memory_provider_existing_values(name: str) -> Dict[str, Any]:
    ...
    for path in (
        hairball_home / f"{name}.json",
        hairball_home / name / "config.json",
    ):
    ...
    # Holographic stores under plugins.hairball-memory-store.
    plugins_cfg = cfg.get("plugins") if isinstance(cfg, dict) else {}
    if name == "holographic" and isinstance(plugins_cfg, dict):
        holographic_cfg = plugins_cfg.get("hairball-memory-store")
...
def _save_memory_provider_native_config(name: str, provider: Any, values: Dict[str, Any]) -> None:
    if provider is not None and hasattr(provider, "save_config"):
        try:
            ...
            provider.save_config(values, str(get_hairball_home()))
            return
...
# ---------------------------------------------------------------------------
# Declared surface — curated desktop schema from hairball_cli.memory_providers.
...
def _declared_provider_file_path(provider: DeclaredMemoryProvider) -> Path:
    return get_hairball_home() / provider.name / "config.json"
...
# in hairball_cli/config.py — listed here for deterministic ordering in the UI.
...
def get_model_options(
    ...
    """
    try:
        from hairball_cli.inventory import build_models_payload, load_picker_context
...
def get_recommended_default_model(provider: str = ""):
    ...
    Mirrors the model-curation `hairball model` does so GUI onboarding lands on a
    sensible default instead of blindly taking the first curated entry. For
    managed this honors the user's free/paid tier: free users get a free model,
    ...
    where free_tier is True/False for managed and None otherwise. `model` may be
    ...
    if slug == "managed":
        try:
            ...
            from hairball_cli.auth import get_provider_auth_state
    ...
    # Non-managed: preferred silent default when the provider's curated list
    ...
    try:
        from hairball_cli.inventory import build_models_payload, load_picker_context
        from hairball_cli.models import pick_silent_default_model
...
def get_moa_models(profile: Optional[str] = None):
    ...
    try:
        from hairball_cli.moa_config import normalize_moa_config
...
async def set_model_assignment(body: ModelAssignment, profile: Optional[str] = None):
    ...
    Writes to ``~/.hairball/config.yaml`` — applies to **new** sessions only.
    ...
    try:
        ...
        if model and not body.confirm_expensive_model:
            try:
                from hairball_cli.model_cost_guard import expensive_model_warning
...
def _apply_model_assignment_sync(
    ...
    if scope == "main":
        ...
        # When switching the main provider to managed, mirror the CLI's
        # post-model-selection behaviour (hairball_cli/main.py
        # prompt_enable_tool_gateway / tools_config apply_managed_managed_defaults):
        # auto-route any *unconfigured* tools through the Managed Tool Gateway.
        # This is purely additive — apply_managed_managed_defaults skips every
        ...
        if provider.strip().lower() == "managed":
            try:
                from hairball_cli.managed_subscription import apply_managed_managed_defaults
                from hairball_cli.tools_config import _get_platform_tools

                enabled = _get_platform_tools(
                    cfg, "cli", include_default_mcp_servers=False
                )
                changed = apply_managed_managed_defaults(
                    cfg,
                ...
                _log.debug("apply_managed_managed_defaults skipped", exc_info=True)

        save_config(cfg)

        # Register a named ``custom_providers`` entry for a custom/local
        # endpoint, mirroring the ``hairball model`` custom flow
        ...
        if provider.strip().lower() in {"custom", "local"} and base_url:
            try:
                from hairball_cli.main import _auto_provider_name, _save_custom_provider
        ...
        # a now-unpaid provider (e.g. managed with $0 balance) keeps paying 402s
...
def _catalog_provider_env_metadata() -> dict:
    ...
    (i.e. the ``hairball model`` universe). This is what lets the desktop Keys tab
    ...
    """
    try:
        from hairball_cli.provider_catalog import provider_catalog
    except Exception:
        return {}
    ...
    try:
        from hairball_cli.config import OPTIONAL_ENV_VARS as _OPT
    except Exception:
        _OPT = {}
    _non_provider_keys = {
    ...
    for d in provider_catalog():
        ...
        # Keys tab (otherwise Bedrock — a `hairball model` provider — would be
        ...
        # Vertex — a `hairball model` provider — would be invisible in the desktop
...
@app.get("/api/env")
def _get_env_vars_sync(profile: Optional[str] = None) -> dict:
    """Build the Keys/Env catalog (sync; safe to run off the event loop)."""
    # Config/env only — do not take _profile_scope / skills RLock (await-unsafe).
    ...
    def _row(var_name: str, info: dict, *, custom: bool = False) -> dict:
        ...
        return {
            ...
            # CLI `hairball model` picker uses (not desktop-only prefix guesses).
...
async def get_env_vars(profile: Optional[str] = None):
    # Catalog assembly is sync/CPU-bound; keep it off the UI event loop so a
    # busy chat turn cannot starve Keys into client-side fetch aborts.
...
_PLATFORM_OVERRIDES: dict[str, dict[str, Any]] = {
    "telegram": {
        "name": "Telegram",
        "description": "Run Hairball from Telegram DMs, groups, and topics.",
    ...
    "discord": {
        "name": "Discord",
        "description": "Connect Hairball to Discord DMs, channels, and threads.",
    ...
    "slack": {
        "name": "Slack",
        "description": "Use Hairball from Slack via Socket Mode. Add allowed Slack member IDs so connected bots can respond.",
    ...
    "mattermost": {
        "name": "Mattermost",
        "description": "Connect Hairball to Mattermost channels and direct messages.",
    ...
    "matrix": {
        "name": "Matrix",
        "description": "Use Hairball in Matrix rooms and direct messages.",
    ...
    "whatsapp": {
        "name": "WhatsApp",
        "description": "Use Hairball through the bundled WhatsApp bridge with QR-based auth.",
    ...
    "homeassistant": {
        "name": "Home Assistant",
        "description": "Control your smart home from Hairball via Home Assistant.",
    ...
    "email": {
        "name": "Email",
        "description": "Talk to Hairball through an IMAP/SMTP mailbox.",
    ...
    "dingtalk": {
        "name": "DingTalk",
        "description": "Connect Hairball to DingTalk groups (钉钉).",
    ...
    "feishu": {
        "name": "Feishu / Lark",
        "description": "Use Hairball inside Feishu / Lark.",
    ...
    "google_chat": {
        "name": "Google Chat",
        "description": "Connect Hairball to Google Chat via Cloud Pub/Sub.",
    ...
    "bluebubbles": {
        ...
        "description": "Use Hairball through iMessage via a BlueBubbles server.",
    ...
    "qqbot": {
        "name": "QQ Bot",
        "description": "Connect Hairball to a QQ Bot from the QQ Open Platform.",
    ...
    "yuanbao": {
        "name": "Yuanbao (元宝)",
        "description": "Connect Hairball to Tencent Yuanbao.",
        "docs_url": "",
        "required_env": (),
    },
    "api_server": {
        "name": "API server",
        "description": "Expose Hairball as an OpenAI-compatible HTTP API for tools like Open WebUI.",
        "docs_url": "./website/docs/user-guide/messaging/",
        "env_vars": (
            "API_SERVER_ENABLED",
            "API_SERVER_KEY",
...
_MESSAGING_ENV_FALLBACKS: dict[str, dict[str, Any]] = {
    ...
    "WEIXIN_ACCOUNT_ID": {
        "description": "iLink Bot account ID obtained through QR login in hairball gateway setup",
        "prompt": "iLink Bot account ID",
    },
    "WEIXIN_TOKEN": {
        "description": "iLink Bot token obtained through QR login in hairball gateway setup",
...
def _whatsapp_session_path() -> Path:
    from hairball_constants import get_hairball_dir

    return get_hairball_dir("platforms/whatsapp/session", "whatsapp/session")
...
_TELEGRAM_ONBOARDING_DEFAULT_URL = "https://setup.local-docs"
_TELEGRAM_ONBOARDING_USER_AGENT = f"HairballDashboard/{__version__}"
...
async def start_telegram_onboarding(body: TelegramOnboardingStart):
    bot_name = (body.bot_name or "Hairball Agent").strip() or "Hairball Agent"
...
def _restart_gateway_after_telegram_onboarding(profile: Optional[str] = None) -> dict[str, Any]:
    ...
    saved token waits on a separate dashboard restart click, Hairball appears
...
async def get_messaging_platforms(profile: Optional[str] = None):
    ...
    # all resolve against the requested profile's HAIRBALL_HOME.
...
# Anthropic, device-code for managed/reference) still runs in the CLI for now;
# Phase 2 will add in-browser flows. For unconnected providers we return
# the canonical ``hairball auth add <provider>`` command so the dashboard
...
def _anthropic_oauth_status() -> Dict[str, Any]:
    ...
    1. ``~/.hairball/.anthropic_oauth.json`` — Hairball-managed PKCE flow (what
    ...
    2. ``ANTHROPIC_API_KEY`` → ``ANTHROPIC_TOKEN`` → ``CLAUDE_CODE_OAUTH_TOKEN``
       ...
       env during ``load_hairball_dotenv()``, so the same check covers them)
    ...
    try:
        from agent.anthropic_adapter import (
            read_hairball_oauth_credentials,
            _get_hairball_oauth_file,
        )
    except ImportError:
        read_hairball_oauth_credentials = None  # type: ignore
        _get_hairball_oauth_file = None  # type: ignore

    hairball_creds = None
    if read_hairball_oauth_credentials:
        try:
            hairball_creds = read_hairball_oauth_credentials()
        except Exception:
            hairball_creds = None
    if hairball_creds and hairball_creds.get("accessToken"):
        return {
            "logged_in": True,
            "source": "hairball_pkce",
            "source_label": f"Hairball PKCE ({_get_hairball_oauth_file() if _get_hairball_oauth_file else None})",
            "token_preview": _truncate_token(hairball_creds.get("accessToken")),
            "expires_at": hairball_creds.get("expiresAt"),
            "has_refresh_token": bool(hairball_creds.get("refreshToken")),
        }
...
def _claude_code_only_status() -> Dict[str, Any]:
    ...
    Claude Code subscription tokens are actively flowing into Hairball even
    when they also have a separate Hairball-managed PKCE login.
    """
    try:
...
def _copilot_acp_status() -> Dict[str, Any]:
    ...
    Hairball never claims a login state it can't verify.
    """
    return {
        "logged_in": False,
        "source": "copilot_cli",
...
_OAUTH_PROVIDER_CATALOG: tuple[Dict[str, Any], ...] = (
    {
        ...
        "name": "Managed Portal",
        "flow": "device_code",
        "cli_command": "hairball auth add managed",
        "docs_url": "#",
    ...
    },
    {
        ...
        "flow": "device_code",
        "cli_command": "hairball auth add openai-codex",
    ...
    },
    {
        ...
        "flow": "external",
        "cli_command": "hairball auth add qwen-oauth",
    ...
    },
    {
        ...
        # as managed's device-code flow; the PKCE bit is a security
        # extension that doesn't change the operator experience.
        "flow": "device_code",
        "cli_command": "hairball auth add minimax-oauth",
    ...
    },
    {
        ...
        # 127.0.0.1 callback.
        "flow": "device_code",
        "cli_command": "hairball auth add xai-oauth",
        "docs_url": "./website/docs/guides/xai-grok-oauth",
    ...
    # usage credits on top of a Claude Max plan — see disclaimer in name).
    {
        ...
        "flow": "pkce",
        "cli_command": "hairball auth add anthropic",
...
def _resolve_provider_status(provider_id: str, status_fn) -> Dict[str, Any]:
    ...
    try:
        from hairball_cli import auth as hauth
        if provider_id == "managed":
            raw = hauth.get_managed_auth_status()
            return {
                "logged_in": bool(raw.get("logged_in")),
                "source": "managed_portal",
                "source_label": raw.get("portal_base_url") or "Managed Portal",
        ...
        if provider_id == "openai-codex":
            ...
            return {
                ...
                "source_label": raw.get("auth_mode") or "ChatGPT",
...
def _oauth_provider_disconnect_command(provider: Dict[str, Any]) -> Optional[str]:
    ...
    External providers store their credentials outside Hairball, so the disconnect
    ...
    user sees exactly what executes, and Hairball then stops resolving the token.
...
def _oauth_provider_disconnect_hint(provider: Dict[str, Any], status: Dict[str, Any]) -> Optional[str]:
    ...
    if provider.get("flow") == "external":
        if _oauth_provider_disconnect_command(provider):
            ...
            return "Managed outside Hairball — run the disconnect command to remove it."
...
def _build_oauth_catalog() -> list[Dict[str, Any]]:
    ...
    MEMBERSHIP is the union of:
      ...
      2. every accounts-tab provider in the unified ``provider_catalog()`` (the
         ``hairball model`` universe) — so any OAuth/external provider added as a
    ...
    #    in lockstep with the `hairball model` universe (zero-edit for new plugins).
    try:
        from hairball_cli.provider_catalog import provider_catalog
        for d in provider_catalog():
            ...
            rows.append({
                ...
                "name": d.label,
                "flow": "external",
                "cli_command": f"hairball auth add {d.slug}",
...
async def list_oauth_providers(profile: Optional[str] = None):
    ...
    Response shape (per provider):
        ...
        status:
          ...
          source           short slug ("hairball_pkce", "claude_code", ...)
    ...
    sync with the `hairball model` picker; _OAUTH_OVERRIDES supplies per-provider
...
async def disconnect_oauth_provider(
    ...
    with _profile_scope(profile):
        ...
        # Anthropic clears only the Hairball-managed PKCE file and auth-store entry.
        ...
        if provider_id == "anthropic":
            cleared = False
            try:
                from agent.anthropic_adapter import _get_hairball_oauth_file
                oauth_file = _get_hairball_oauth_file()
            ...
            try:
                from hairball_cli.auth import clear_provider_auth
        ...
        try:
            from hairball_cli.auth import clear_provider_auth, invalidate_managed_auth_status_cache
...
#          → persists to ~/.hairball/.anthropic_oauth.json AND credential pool
#          → returns { ok: true, status: "approved" }
#
#   Device code (managed, ChatGPT):
...
# Guarded so hairball web still starts if anthropic_adapter is unavailable;
...
def _save_anthropic_oauth_creds(access_token: str, refresh_token: str, expires_at_ms: int) -> None:
    """Persist Anthropic PKCE creds to both Hairball file AND credential pool.

    Mirrors what auth_commands.add_command does so the dashboard flow leaves
    the system in the same state as ``hairball auth add anthropic``.
    """
    from agent.anthropic_adapter import _get_hairball_oauth_file
    oauth_file = _get_hairball_oauth_file()
    payload = {
    ...
    for _endpoint in _ANTHROPIC_OAUTH_TOKEN_URLS:
        req = urllib.request.Request(
            ...
            headers={
                ...
                "User-Agent": "hairball-dashboard/1.0",
            },
            method="POST",
        )
        try:
...
async def _start_device_code_flow(
    ...
    """Initiate a device-code flow (managed, ChatGPT, MiniMax, or xAI).
    ...
    if provider_id == "managed":
        ...
        portal_base_url = (
            os.getenv("HAIRBALL_PORTAL_BASE_URL")
        ...
        sid, sess = _new_oauth_session("managed", "device_code", profile=profile)
    ...
    if provider_id == "openai-codex":
        # reference uses fixed OpenAI device-auth endpoints; reuse the helper.
    ...
    if provider_id == "minimax-oauth":
        ...
        # operator's perspective it's identical to managed's device-code
    ...
    if provider_id == "xai-oauth":
        from hairball_cli.auth import _xai_oauth_request_device_code
        import httpx
...
def _managed_poller(session_id: str) -> None:
    """Background poller that drives a managed device-code flow to completion."""
    from hairball_cli.auth import (
        _poll_for_token,
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            from hairball_cli.auth import persist_managed_credentials
...
def _minimax_poller(session_id: str) -> None:
    ...
    the ``device_code`` field used by managed. On success, builds the same
    auth_state dict that ``_minimax_oauth_login`` (the CLI flow) builds
    and persists via ``_minimax_save_auth_state`` — so the dashboard
    path leaves the system in the same state as
    ``hairball auth add minimax-oauth``.
    """
    from hairball_cli.auth import (
        _minimax_poll_token,
...
def _xai_device_poller(session_id: str) -> None:
    ...
    try:
        ...
        with _profile_scope(_oauth_session_profile(session_id)):
            ...
            # prior ``hairball auth remove xai-oauth`` (mirrors auth_add_command
            # and the ``hairball model`` re-login path in _login_xai_oauth).
...
def _codex_device_code_start_error(resp: Any) -> str:
    """Dashboard-facing ChatGPT device-code start failure."""
    ...
    if "device" in lower and ("authori" in lower or "enable" in lower):
        message = (
            ...
            "account may need device-code authorization enabled before Hairball "
...
def _codex_full_login_worker(session_id: str) -> None:
    """Run the complete ChatGPT device-code flow.

    reference doesn't use the standard OAuth device-code endpoints; it has its
    ...
    try:
        ...
        from hairball_cli.auth import _save_codex_tokens
...
async def poll_oauth_session(
    ...
    Shared by the device-code flows (managed, ChatGPT, MiniMax, xAI).
...
async def import_sessions_endpoint(request: Request):
    ...
    restores a whole Hairball backup archive, while this endpoint is scoped to
...
async def get_session_stats(profile: Optional[str] = None):
    """Session-store statistics for the Sessions page (mirrors `hairball sessions stats`).
...
async def prune_sessions_endpoint(body: SessionPrune):
    """Delete ended sessions matching filters (mirrors `hairball sessions prune`)."""
    has_window = (
    ...
    profile_home = _cron_profile_home(body.profile)[1] if body.profile else get_hairball_home()
    db = _open_session_db_for_profile(body.profile)
    try:
        filters = dict(
...
async def get_logs(
    ...
    from hairball_cli.logs import _read_tail, LOG_FILES

    log_name = LOG_FILES.get(file)
    if not log_name:
        raise HTTPException(status_code=400, detail=f"Unknown log file: {file}")
    log_path = get_hairball_home() / "logs" / log_name
    if not log_path.exists():
        return {"file": file, "lines": []}

    try:
        from hairball_logging import COMPONENT_PREFIXES
    except ImportError:
        COMPONENT_PREFIXES = {}
...
def _cron_profile_home(profile: Optional[str]) -> Tuple[str, Path]:
    """Resolve a profile query value to (profile_name, HAIRBALL_HOME)."""
    from hairball_cli import profiles as profiles_mod
...
def _annotate_cron_job(job: Dict[str, Any], profile: str, home: Path) -> Dict[str, Any]:
    ...
    annotated["hairball_home"] = str(home)
...
def _call_cron_for_profile(target_profile: Optional[str], func_name: str, *args, **kwargs):
    ...
    from hairball_constants import (
        reset_hairball_home_override,
        set_hairball_home_override,
    )

    token = set_hairball_home_override(str(home))
    try:
        with cron_jobs.use_cron_store(home):
            result = getattr(cron_jobs, func_name)(*args, **kwargs)
    finally:
        reset_hairball_home_override(token)
...
def _fire_cron_job_for_profile(profile: str, job_id: str) -> bool:
    ...
    Scope both cron storage and the runtime Hairball home so the job's store,
...
#
# Wraps the same config data layer the CLI uses (hairball_cli.mcp_config), so
# servers managed here show up under `hairball mcp list` and vice versa.  Secrets
...
async def list_mcp_servers(profile: Optional[str] = None):
    from hairball_cli.mcp_config import _get_mcp_servers
...
async def add_mcp_server(body: MCPServerCreate, profile: Optional[str] = None):
    from hairball_cli.mcp_config import _get_mcp_servers, _save_mcp_server
...
async def replace_mcp_servers(body: MCPServersReplace, profile: Optional[str] = None):
    ...
    """
    from hairball_cli.mcp_config import _replace_mcp_servers
...
async def remove_mcp_server(name: str, profile: Optional[str] = None):
    from hairball_cli.mcp_config import _remove_mcp_server
...
async def test_mcp_server(name: str, profile: Optional[str] = None):
    ...
    from hairball_cli.mcp_config import (
        _get_mcp_servers,
    ...
    def _probe_scoped():
        ...
        # The probe touches no skills globals; it only needs the HAIRBALL_HOME
...
async def auth_mcp_server(name: str, profile: Optional[str] = None):
    ...
    Mirrors ``hairball mcp login``: wipe cached OAuth state so the probe forces
    ...
    def _run():
        from tools.mcp_oauth import HairballTokenStorage, force_interactive_oauth
        ...
        # resolve HAIRBALL_HOME via the contextvar override, which is all they need.
        with _config_profile_scope(profile), force_interactive_oauth():
            storage = HairballTokenStorage(name)
...
async def list_mcp_catalog(profile: Optional[str] = None):
    """Browse the catalog MCP catalog (the optional-mcps/ manifests).
...
async def install_mcp_catalog_entry(body: MCPCatalogInstall, profile: Optional[str] = None):
    ...
    # The -p subprocess rebinds HAIRBALL_HOME-derived paths in the child.
...
#
# Wraps the same JSON store the CLI uses (hairball_cli.webhook); the webhook
...
async def list_webhooks():
    import hairball_cli.webhook as wh
...
# spawn the real `hairball gateway <verb>` so behaviour matches the CLI exactly.
...
async def start_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_hairball_action(_gateway_subcommand(profile, "start"), "gateway-start")
    except HTTPException:
        raise
    except Exception as exc:
...
async def stop_gateway(profile: Optional[str] = None):
    try:
        proc = _spawn_hairball_action(_gateway_subcommand(profile, "stop"), "gateway-stop")
    except HTTPException:
        raise
    except Exception as exc:
...
async def list_credential_pool():
    ...
    from hairball_cli.auth import read_credential_pool
...
async def run_doctor():
    try:
        proc = _spawn_hairball_action(["doctor"], "doctor")
    except Exception as exc:
...
async def run_security_audit():
    try:
        proc = _spawn_hairball_action(["security", "audit"], "security-audit")
    except Exception as exc:
...
def _dashboard_backup_dir() -> Path:
    return get_hairball_home() / "backups"


def _new_dashboard_backup_path() -> Path:
    stamp = datetime.now().strftime("%Y-%m-%d-%H%M%S")
    return _dashboard_backup_dir() / f"hairball-backup-{stamp}-{secrets.token_hex(4)}.zip"
...
async def run_backup(body: BackupRequest):
    ...
    try:
        proc = _spawn_hairball_action(args, "backup")
    except Exception as exc:
...
class ImportRequest(BaseModel):
    archive: str
    # Pass --force to `hairball import`. The spawned action runs with
...
async def run_import(body: ImportRequest):
    ...
    if body.force:
        args.append("--force")
    try:
        proc = _spawn_hairball_action(args, "import")
    except Exception as exc:
...
async def list_hooks():
    ...
    form can offer them.
    """
    from hairball_cli.config import load_config as _load_config
    from agent import shell_hooks

    try:
        from hairball_cli.plugins import VALID_HOOKS
...
async def list_checkpoints():
    ...
    # Checkpoints live under <hairball_home>/checkpoints/.  Surface a count +
    ...
    cp_dir = get_hairball_home() / "checkpoints"
    sessions = []
    total_bytes = 0
    if cp_dir.is_dir():
...
async def prune_checkpoints():
    try:
        proc = _spawn_hairball_action(["checkpoints", "prune"], "checkpoints-prune")
    except Exception as exc:
...
def _profile_cli_args(profile: Optional[str]) -> List[str]:
    ...
    Hub install/uninstall/update run in a fresh ``hairball`` subprocess, and
...
def _hub_action_name(verb: str, key: str) -> str:
    ...
    ``_spawn_hairball_action`` tracks one process/log per name, so a shared
...
# Human-readable labels for each hub source id (matches `hairball skills search`
# provenance).  Keep in sync with create_source_router()'s source list.
_SKILL_HUB_SOURCE_LABELS = {
    "official": "Official (managed)",
    "hairball-index": "Hairball Index",
    "skills-sh": "skills.sh",
...
async def list_skills_hub_sources(profile: Optional[str] = None):
    ...
    def _run():
        from tools.skills_hub import (
            ...
            refresh_hairball_index_in_background,
        )

        with _config_profile_scope(profile):
            # Source metadata is a page-shell request.  Never make it wait for
            # the large remote index; use any local cache and refresh that
            # cache through the same loader in a single background flight.
            sources = create_source_router(allow_index_network=False)
        out = []
        ...
        index_refreshing = refresh_hairball_index_in_background()
        return {
            "sources": out,
            "index_available": index_available,
            "index_refreshing": index_refreshing,
            "featured": featured,
...
async def preview_skill_hub(identifier: str = "", profile: Optional[str] = None):
    ...
    def _run():
        from hairball_cli.skills_hub import _resolve_source_meta_and_bundle
...
class ProfileCreate(BaseModel):
    ...
    # via a subprocess scoped to the profile (`hairball -p <name> skills install`)
    # because skills_hub.SKILLS_DIR is import-time-bound and the HAIRBALL_HOME
...
def _fallback_profile_dicts(profiles_mod) -> List[Dict[str, Any]]:
    ...
    default_home = profiles_mod._get_default_hairball_home()
...
def _profile_setup_command(name: str) -> str:
    ...
    return "hairball setup" if name == "default" else f"{name} setup"
...
def _write_profile_model(profile_dir: Path, provider: str, model: str) -> None:
    ...
    context-local HAIRBALL_HOME override so the write lands in the target
    ...
    """
    from hairball_constants import set_hairball_home_override, reset_hairball_home_override

    token = set_hairball_home_override(str(profile_dir))
    try:
        provider, model = _normalize_main_model_assignment(provider, model)
        cfg = load_config()
...
def _write_profile_mcp_servers(profile_dir: Path, servers: List["MCPServerCreate"]) -> int:
    ...
    Scopes ``load_config``/``save_config`` to ``profile_dir`` via the
    context-local HAIRBALL_HOME override (same mechanism as
    ...
    """
    from hairball_constants import set_hairball_home_override, reset_hairball_home_override
    from hairball_cli.mcp_security import validate_mcp_server_entry
...
def _disable_unselected_skills(profile_dir: Path, keep: List[str]) -> int:
    ...
    install.) Scoped to the profile via the HAIRBALL_HOME override. Returns the
    number of skills newly disabled.
    """
    from hairball_constants import set_hairball_home_override, reset_hairball_home_override
    from hairball_cli.skills_config import get_disabled_skills, save_disabled_skills
...
async def create_profile_endpoint(body: ProfileCreate):
    ...
    # profile's HAIRBALL_HOME at import). Returns PIDs for the UI to poll.
...
async def get_active_profile_endpoint():
    ...
    ``active`` is the sticky default written by ``hairball profile use`` —
    the profile new CLI invocations pick up. ``current`` is the profile
    the running dashboard/gateway is scoped to (derived from HAIRBALL_HOME).
    """
    from hairball_cli import profiles as profiles_mod
    try:
...
async def set_active_profile_endpoint(body: ProfileActiveUpdate):
    """Set the sticky active profile (mirrors ``hairball profile use``).
...
async def update_profile_model_endpoint(name: str, body: ProfileModelUpdate):
    ...
    to the named profile via the HAIRBALL_HOME override.
    """
    profile_dir = _resolve_profile_dir(name)
    provider = (body.provider or "").strip()
    model = (body.model or "").strip()
...
async def describe_profile_auto_endpoint(name: str, body: ProfileDescribeAuto):
    ...
    (``auxiliary.profile_describer``). Mirrors ``hairball profile describe
    <name> --auto``.
    ...
    _resolve_profile_dir(name)
    try:
        from hairball_cli import profile_describer
...
def _profile_scope(profile: Optional[str]):
    ...
    1. ``load_config``/``save_config`` resolve ``get_hairball_home()`` at call
       time — the context-local override from ``set_hairball_home_override``
    ...
    retargeted to the *current* ``get_hairball_home()`` so writes land in the
    live home even when the import-time binding is stale (e.g. the process
    imported the modules before a HAIRBALL_HOME override, or under test
    isolation).
    """
    requested = (profile or "").strip()
...
def _config_profile_scope(profile: Optional[str]):
    ...
    ``set_hairball_home_override`` contextvar — it does NOT swap the
    ...
    which is all endpoints that resolve ``get_hairball_home()`` at call time
...
async def get_skills(profile: Optional[str] = None):
    ...
    from hairball_cli.skills_config import get_disabled_skills
...
async def get_toolsets(profile: Optional[str] = None):
    from hairball_cli.tools_config import (
...
async def toggle_toolset(name: str, body: ToolsetToggle, profile: Optional[str] = None):
    ...
    helper the CLI ``hairball tools`` picker uses, so the GUI and CLI stay in
...
async def get_toolset_config(name: str, profile: Optional[str] = None):
    ...
    Surfaces the same provider rows the CLI ``hairball tools`` picker shows
...
def _resolve_toolset_model_plugin(ts_key: str, provider_row: dict) -> Optional[str]:
    ...
    the managed "managed Subscription" image row instead carries the legacy
...
async def get_toolset_models(
    ...
    The GUI counterpart of the model picker `hairball tools` runs after a
...
async def select_toolset_provider(
    ...
    core extracted from the CLI configurator — so the GUI and ``hairball tools``
...
async def save_toolset_env(name: str, body: ToolsetEnvUpdate, profile: Optional[str] = None):
    ...
    Writes each ``key: value`` to ``~/.hairball/.env`` via ``save_env_value`` —
    the same store ``hairball tools`` writes when it prompts for keys. Keys are
...
async def run_toolset_post_setup(
    ...
    ``hairball tools post-setup <key>`` and the frontend tails the log via
    ...
    ``profile`` spawns the hook as ``hairball -p <profile> tools post-setup``.
    ...
    write per-profile state must see the same HAIRBALL_HOME the rest of the
...
# (which attach to cua-driver's OWN identity, com.trycua.driver — not Hairball,
...
async def grant_computer_use_permissions(profile: Optional[str] = None):
    """Spawn ``hairball computer-use permissions grant`` as a background action.
    ...
    try:
        ...
        cur = db._conn.execute("""
            SELECT date(started_at, 'unixepoch') as day,
                   ...
                   SUM(COALESCE(cost_actual_call_count, 0)) as cost_actual_call_count,
                   SUM(COALESCE(cost_total_call_count, 0)) as cost_total_call_count,
                   COUNT(*) as sessions,
        ...
        for row in daily:
            actual_calls = int(row.get("cost_actual_call_count") or 0)
            total_calls = int(row.get("cost_total_call_count") or 0)
            complete = total_calls > 0 and actual_calls >= total_calls
            row["actual_cost_coverage"] = (
                min(1.0, actual_calls / total_calls) if total_calls else None
            )
            row["actual_cost_complete"] = complete
            row["display_cost"] = (
                row.get("actual_cost") if complete else row.get("estimated_cost")
            )
        ...
        total_actual_calls = int(totals.get("cost_actual_call_count") or 0)
        total_cost_calls = int(totals.get("cost_total_call_count") or 0)
        totals["actual_cost_coverage"] = (
            min(1.0, total_actual_calls / total_cost_calls)
            if total_cost_calls
            else None
        )
        totals["actual_cost_complete"] = (
            total_cost_calls > 0 and total_actual_calls >= total_cost_calls
        )
        totals["display_cost"] = (
            totals.get("total_actual_cost")
            if totals["actual_cost_complete"]
            else totals.get("total_estimated_cost")
        )
        insights_report = InsightsEngine(db).generate(days=days)
        by_model = []
        for model_row in insights_report.get("models", []):
            by_model.append(
                {
                    "model": model_row.get("model_id") or model_row.get("model"),
                    "provider": model_row.get("provider") or "",
                    "input_tokens": model_row.get("input_tokens") or 0,
                    "output_tokens": model_row.get("output_tokens") or 0,
                    "estimated_cost": model_row.get("cost") or 0.0,
                    "actual_cost": model_row.get("actual_cost") or 0.0,
                    "cost_actual_call_count": (
                        model_row.get("cost_actual_call_count") or 0
                    ),
                    "cost_total_call_count": (
                        model_row.get("cost_total_call_count") or 0
                    ),
                    "actual_cost_coverage": model_row.get("actual_cost_coverage"),
                    "actual_cost_complete": bool(
                        (model_row.get("cost_total_call_count") or 0) > 0
                        and (model_row.get("cost_actual_call_count") or 0)
                        >= (model_row.get("cost_total_call_count") or 0)
                    ),
                    "sessions": model_row.get("sessions") or 0,
                    "api_calls": model_row.get("api_calls") or 0,
                }
            )
            by_model[-1]["display_cost"] = (
                by_model[-1]["actual_cost"]
                if by_model[-1]["actual_cost_complete"]
                else by_model[-1]["estimated_cost"]
            )
...
async def get_models_analytics(days: int = 30, profile: Optional[str] = None):
    ...
    try:
        ...
        # The session summary row can only name one route. Use the shared
        # per-call ledger projection so MoA references, fallbacks, and mid-
        # session model switches remain attributed to the provider/model that
        # actually incurred their tokens and cost. Insights also reconciles
        # legacy rows and absolute gateway totals, so no historical data drops.
        session_summary_by_route = {
            (r.get("model") or "", r.get("billing_provider") or ""): r
            for r in rows
        }
        route_rows = InsightsEngine(db).generate(days=days).get("models", [])
        rows = [
            {
                "model": r.get("model_id") or r.get("model") or "unknown",
                "billing_provider": r.get("provider") or "",
                "input_tokens": r.get("input_tokens") or 0,
                "output_tokens": r.get("output_tokens") or 0,
                "cache_read_tokens": r.get("cache_read_tokens") or 0,
                "reasoning_tokens": r.get("reasoning_tokens") or 0,
                "estimated_cost": r.get("cost") or 0.0,
                "actual_cost": r.get("actual_cost") or 0.0,
                "cost_actual_call_count": r.get("cost_actual_call_count") or 0,
                "cost_total_call_count": r.get("cost_total_call_count") or 0,
        ...
        for row in rows:
            session_summary = session_summary_by_route.get(
                (row["model"], row["billing_provider"])
            )
            if session_summary is not None:
                row["sessions"] = session_summary.get("sessions") or row["sessions"]
                row["last_used_at"] = (
                    session_summary.get("last_used_at") or row["last_used_at"]
                )
                row["avg_tokens_per_session"] = (
                    session_summary.get("avg_tokens_per_session")
                    if session_summary.get("avg_tokens_per_session") is not None
                    else row["avg_tokens_per_session"]
                )
        ...
        for row in rows:
            ...
            actual_calls = int(row.get("cost_actual_call_count") or 0)
            total_cost_calls = int(row.get("cost_total_call_count") or 0)
            actual_complete = (
                total_cost_calls > 0 and actual_calls >= total_cost_calls
            )
            caps = {}
            try:
                from agent.models_dev import get_model_capabilities
            ...
            models.append({
                ...
                "cost_actual_call_count": actual_calls,
                "cost_total_call_count": total_cost_calls,
                "actual_cost_coverage": (
                    min(1.0, actual_calls / total_cost_calls)
                    if total_cost_calls
                    else None
                ),
...
#
# The endpoint spawns the same ``hairball --tui`` binary the CLI uses, behind
...
if sys.platform.startswith("win"):
    try:
        from hairball_cli.win_pty_bridge import WinPtyBridge as PtyBridge, PtyUnavailableError
    ...
    try:
        from hairball_cli.pty_bridge import PtyBridge, PtyUnavailableError
        _PTY_BRIDGE_AVAILABLE = True
...
from hairball_cli.pty_session import PtySessionRegistry, RegistryFull, run_reaper  # noqa: E402
...
def _ws_auth_reason(ws: "WebSocket") -> tuple[Optional[str], str]:
    ...
    * ``?internal=<process-credential>`` — the process-lifetime internal
    ...
    if auth_required:
        ...
        from hairball_cli.dashboard_auth.audit import AuditEvent, audit_log
        from hairball_cli.dashboard_auth.ws_tickets import (
            TicketInvalid,
...
def _resolve_chat_argv(
    ...
    Default: whatever ``hairball --tui`` would run.  Tests monkeypatch this
    function to inject a tiny fake command (``cat``, ``sh -c 'printf …'``)
    so nothing has to build Node or the TUI bundle.

    Session resume is propagated via the ``HAIRBALL_TUI_RESUME`` env var —
    matching what ``hairball_cli.main._launch_tui`` does for the CLI path.
    Appending ``--resume <id>`` to argv doesn't work because ``ui-tui`` does
    not parse its argv.

    ``HAIRBALL_TUI_GATEWAY_URL`` is injected so the PTY child can attach to
    this process's in-memory ``tui_gateway`` instance instead of spawning
    its own Python gateway subprocess.

    `sidecar_url` (when set) is forwarded as ``HAIRBALL_TUI_SIDECAR_URL`` so
    the spawned ``tui_gateway.entry`` can mirror dispatcher emits to the
    dashboard's ``/api/pub`` endpoint (see :func:`pub_ws`).

    `active_session_file` (when set) is forwarded as
    ``HAIRBALL_TUI_ACTIVE_SESSION_FILE``. The TUI writes the current session id
    ...
    ``HAIRBALL_HOME`` at the profile dir in the child env. Every spawned
    process (the TUI and the ``tui_gateway.entry`` it launches) resolves
    ``get_hairball_home()`` from that env var at its own import, so the child
    binds the profile's config, skills, memory, and state.db from the start
    — the same propagation ``hairball -p <name>`` performs. The in-process
    ``HAIRBALL_TUI_GATEWAY_URL`` attach is SKIPPED for scoped chats: the
    ...
    """
    from hairball_cli.main import PROJECT_ROOT, _make_tui_argv
    ...
    env.setdefault("HAIRBALL_TUI_DISABLE_MOUSE", "1")
    env.setdefault("HAIRBALL_TUI_INLINE", "1")
    ...
    env["HAIRBALL_TUI_DASHBOARD"] = "1"

    if profile_dir is not None:
        env["HAIRBALL_HOME"] = str(profile_dir)
    ...
    if resume:
        ...
        env["HAIRBALL_TUI_RESUME"] = resume

    if sidecar_url:
        env["HAIRBALL_TUI_SIDECAR_URL"] = sidecar_url
    ...
    # inherits the profile HAIRBALL_HOME set above.
    if profile_dir is None:
        if gateway_ws_url := _build_gateway_ws_url():
            env["HAIRBALL_TUI_GATEWAY_URL"] = gateway_ws_url
...
def _resolve_client_ws_host() -> Optional[str]:
    ...
    1. Explicit ``HAIRBALL_DASHBOARD_WS_HOST`` env var — wins always. Operators
    ...
    """
    explicit = os.environ.get("HAIRBALL_DASHBOARD_WS_HOST", "").strip()
    if explicit:
        return explicit
...
def _build_gateway_ws_url() -> Optional[str]:
    ...
    if getattr(app.state, "auth_required", False):
        from hairball_cli.dashboard_auth.ws_tickets import internal_ws_credential
...
def _active_session_file_for_channel(app: "FastAPI", channel: str) -> Path:
    ...
    fd, raw_path = tempfile.mkstemp(prefix="hairball-pty-active-", suffix=".json")
    os.close(fd)
    path = Path(raw_path)
    files[channel] = path
    return path
...
# ---------------------------------------------------------------------------
# /api/console — safe Hairball Console command WebSocket.
#
# Unlike /api/pty, this endpoint never spawns a PTY, shell, or full Hairball CLI
        ...
        with _console_executor_lock:
            if _console_executor is None:
                _console_executor = concurrent.futures.ThreadPoolExecutor(
                    ...
                    thread_name_prefix="hairball-console",
                )
...
def _dashboard_console_context() -> str:
    ...
    return "hosted" if _default_hairball_root_is_opt_data() else "local"
...
async def console_ws(ws: WebSocket) -> None:
    ...
    try:
        from hairball_cli.console_engine import HairballConsoleEngine

        engine = HairballConsoleEngine(
            ...
            if command_id == command_generation:
                ...
                await _console_send(
                    ...
                    send_lock,
                    {
                        ...
                        "id": command_id,
                        "message": (
                            "Command timed out. Hairball Console returned to the prompt."
                        ),
                        "command": line,
                    },
                )
...
async def pty_ws(ws: WebSocket) -> None:
    ...
    if not _PTY_BRIDGE_AVAILABLE:
        await ws.send_text(
            ...
            "\x1b[33mInstall Hairball inside WSL2 to use the dashboard's /chat "
...
# HAIRBALL_TUI_SIDECAR_URL set in /api/pty's PTY env) and writes every
...
def _normalise_prefix(raw: Optional[str]) -> str:
    ...
    Thin re-export of :func:`hairball_cli.dashboard_auth.prefix.normalise_prefix`
    ...
    """
    from hairball_cli.dashboard_auth.prefix import normalise_prefix
...
def mount_spa(application: FastAPI):
    ...
    ``mission-control.tilos.com/hairball/*`` -> local Caddy -> :9119), the
    proxy injects ``X-Forwarded-Prefix: /hairball`` on every request. We
    rewrite the served ``index.html`` so absolute asset URLs (``/assets/...``)
    and the SPA's runtime ``__HAIRBALL_BASE_PATH__`` honour that prefix
    without rebuilding the bundle.
    """
    # `hairball serve` is the headless backend: it must NEVER serve the browser
    ...
    _headless = os.environ.get("HAIRBALL_SERVE_HEADLESS") == "1"
    if _headless or not WEB_DIST.exists():
        _msg = (
            "Headless backend (hairball serve): web UI disabled — use "
            "`hairball dashboard` for the browser UI."
            if _headless
    ...
    def _serve_index(prefix: str = ""):
        ...
        ``prefix`` is the normalised ``X-Forwarded-Prefix`` (e.g. ``/hairball``)
        ...
        ``__HAIRBALL_AUTH_REQUIRED__`` flag lets the SPA pick the right
        ...
        if gated:
            bootstrap_script = (
                f"<script>"
                f"window.__HAIRBALL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HAIRBALL_BASE_PATH__="{prefix}";'
                f"window.__HAIRBALL_AUTH_REQUIRED__={gated_js};"
                f"</script>"
            )
        else:
            bootstrap_script = (
                f'<script>window.__HAIRBALL_SESSION_TOKEN__="{_SESSION_TOKEN}";'
                f"window.__HAIRBALL_DASHBOARD_EMBEDDED_CHAT__={chat_js};"
                f'window.__HAIRBALL_BASE_PATH__="{prefix}";'
                f"window.__HAIRBALL_AUTH_REQUIRED__={gated_js};"
                f"</script>"
    ...
    # under ``/hairball`` they'd hit ``mission-control.tilos.com/fonts/...``
    # (the MC Pages app), not the Hairball backend. Intercept CSS asset
...
_BUILTIN_DASHBOARD_THEMES = [
    {"name": "default",       "label": "Hairball Teal",         "description": "Classic dark teal — the canonical Hairball look"},
    {"name": "default-large", "label": "Hairball Teal (Large)", "description": "Hairball Teal with bigger fonts and roomier spacing"},
    {"name": "managed-blue",     "label": "Hairball Blue",           "description": "Light mode — vivid Hairball-blue accents on cream canvas"},
...
def _normalise_theme_definition(data: Dict[str, Any]) -> Optional[Dict[str, Any]]:
    ...
    # YAML in ~/.hairball/, same trust level as the config file itself.
...
def _discover_user_themes() -> list:
    """Scan ~/.hairball/dashboard-themes/*.yaml for user-created themes.

    Returns a list of fully-normalised theme definitions ready to ship
    to the frontend, so the client can apply them without a secondary
    round-trip or a built-in stub.
    """
    themes_dir = get_hairball_home() / "dashboard-themes"
...
async def get_dashboard_themes():
    ...
    from `~/.hairball/dashboard-themes/*.yaml` ship with their full
...
def _discover_dashboard_plugins() -> list:
    ...
    Checks three plugin sources (same as hairball_cli.plugins):
    1. User plugins:    ~/.hairball/plugins/<name>/dashboard/manifest.json
    2. Bundled plugins: <repo>/plugins/<name>/dashboard/manifest.json  (memory/, etc.)
    3. Project plugins: ./.hairball/plugins/  (only if HAIRBALL_ENABLE_PROJECT_PLUGINS)
    """
    plugins = []
    seen_names: set = set()

    from hairball_cli.plugins import get_bundled_plugins_dir
    bundled_root = get_bundled_plugins_dir()
    search_dirs = [
        (get_hairball_home() / "plugins", "user"),
    ...
    # ``hairball_cli/plugins.py`` and the documented user contract.
    if env_var_enabled("HAIRBALL_ENABLE_PROJECT_PLUGINS"):
        search_dirs.append((Path.cwd() / ".hairball" / "plugins", "project"))
...
async def get_dashboard_plugins():
    ...
    try:
        from hairball_cli.plugins_cmd import _get_enabled_set, _get_disabled_set
        enabled_set = _get_enabled_set()
        disabled_set = _get_disabled_set()
    except Exception:
        enabled_set = set()
...
def _merged_plugins_hub() -> Dict[str, Any]:
    ...
    plugins_root_resolved = (get_hairball_home() / "plugins").resolve()
    rows: List[Dict[str, Any]] = []
            ...
            try:
                ...
                for tname in provides_tools:
                    ...
                    if entry and entry.check_fn and not entry.check_fn():
                        auth_required = True
                        auth_command = f"hairball auth {name}"
                        break
            except Exception:
                pass
...
async def post_agent_plugin_install(request: Request, body: _AgentPluginInstallBody):
    _require_token(request)
    from hairball_cli.plugins_cmd import dashboard_install_plugin
...
async def post_agent_plugin_enable(request: Request, name: str):
    ...
    from hairball_cli.plugins_cmd import dashboard_set_agent_plugin_enabled
...
async def post_agent_plugin_update(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from hairball_cli.plugins_cmd import dashboard_update_user_plugin
...
async def delete_agent_plugin(request: Request, name: str):
    _require_token(request)
    name = _validate_plugin_name(name)
    from hairball_cli.plugins_cmd import dashboard_remove_user_plugin
...
def _mount_plugin_api_routes():
    ...
    Project plugins (``./.hairball/plugins/``) ship with the CWD and are
    ...
    for plugin in _get_dashboard_plugins():
        ...
        if plugin.get("source") == "project":
            _log.warning(
                ...
                "~/.hairball/plugins/ if you trust it)",
        ...
        if not api_path.exists():
            ...
            continue
        try:
            module_name = f"hairball_dashboard_plugin_{plugin['name']}"
...
from hairball_cli.dashboard_auth.routes import router as _dashboard_auth_router  # noqa: E402
...
def _write_dashboard_ready_file(actual_port: int) -> None:
    ...
    so Electron passes ``HAIRBALL_DESKTOP_READY_FILE`` and waits for this JSON.
    Normal CLI/dashboard launches still use the stdout READY line below.
    """
    target = os.environ.get("HAIRBALL_DESKTOP_READY_FILE")
    if not target:
        return
...
def start_server(
    ...
    build and no SPA mount (mount_spa() honours ``HAIRBALL_SERVE_HEADLESS``), so
    the banner announces the bind rather than a browser URL.
    """
    import uvicorn

    try:
        from hairball_cli.managed_auth_keepalive import start_managed_auth_keepalive

        start_managed_auth_keepalive()
    except Exception as exc:
        _log.debug("managed auth keepalive did not start: %s", exc)
    ...
    # the hairball-0day MCP-persistence campaign abused unauthenticated public
    ...
    if app.state.auth_required:
        ...
        if not list_providers():
            ...
            # to register (e.g. missing HAIRBALL_DASHBOARD_OAUTH_CLIENT_ID).
            # Each provider plugin that ships with Hairball Agent exposes a
            ...
            _fix_hint = (
                ...
                "  • OAuth: run `hairball dashboard register` (Managed Portal) or "
    ...
    # HAIRBALL_DASHBOARD_READY, open the browser, *then* serve.
    #
    ...
    async def _serve():
        ...
        with server.capture_signals():
            ...
            ready_token = "HAIRBALL_BACKEND_READY" if headless else "HAIRBALL_DASHBOARD_READY"
            print(f"{ready_token} port={actual_port}", flush=True)
            if headless:
                # No SPA, and the JSON-RPC/WS endpoints are auth-gated — don't
                # advertise a paste-and-connect URL, just announce the bind.
                print(f"  Hairball backend listening on {host}:{actual_port}")
            else:
                print(f"  Hairball Web UI → http://{host}:{actual_port}")
