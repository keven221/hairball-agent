"""Normalized Managed Portal account entitlement helpers."""
...
ManagedAccountInfoSource = Literal["jwt", "account_api", "inference_key", "none", "error"]

# Free tool-pool coverage categories. Kept byte-for-byte aligned with the
# Portal's TOOL_COVERAGE_CATEGORIES (managed-account-service
...
_account_info_cache: tuple[str, float, "ManagedPortalAccountInfo"] | None = None
_ACCOUNT_INFO_CACHE_LOCK = threading.Lock()


@dataclass(frozen=True)
class ManagedPortalSubscriptionInfo:
...
@dataclass(frozen=True)
class ManagedPaidServiceAccessInfo:
...
@dataclass(frozen=True)
class ManagedPortalAccountInfo:
    logged_in: bool
    source: ManagedAccountInfoSource
    fresh: bool
    ...
    managed_client: Optional[str] = None
    ...
    subscription: Optional[ManagedPortalSubscriptionInfo] = None
    paid_service_access: Optional[bool] = None
    paid_service_access_info: Optional[ManagedPaidServiceAccessInfo] = None
    tool_access: Optional[ManagedToolAccessInfo] = None
...
def managed_portal_billing_url(account_info: Optional[ManagedPortalAccountInfo] = None) -> str:
    """Return the billing URL for a normalized managed account snapshot."""
    base = None
    if account_info is not None:
        base = account_info.portal_base_url
    if not isinstance(base, str) or not base.strip():
        try:
            from hairball_cli.managed_foreign_backends import portal_base_url

            base = portal_base_url()
        except Exception:
            base = ""
    if not isinstance(base, str) or not base.strip():
        # The product default is BYOK/off. Keep the UI link local instead of
        # silently resurrecting a foreign portal origin.
        return "#/billing"
    return f"{base.rstrip('/')}/billing"


def managed_portal_topup_url(account_info: Optional[ManagedPortalAccountInfo] = None) -> str:
    ...
    """
    base_billing = managed_portal_billing_url(account_info)  # {base}/billing
...
def format_managed_portal_entitlement_message(
    account_info: Optional[ManagedPortalAccountInfo],
    *,
    capability: str = "this feature",
    include_refresh_hint: bool = True,
    coverage_category: Optional[str] = None,
) -> Optional[str]:
    """Return user-facing guidance for a missing managed tool-gateway entitlement.
    ...
    """
    billing_url = managed_portal_billing_url(account_info)
    ...
    if account_info is not None:
        if coverage_category is not None:
            ...
            if account_info.tool_gateway_entitled:
                ...
                return (
                    f"{capability} isn't included with your current Managed Portal "
    ...
    if account_info is None:
        return (
            f"Hairball could not verify your Managed Portal entitlement, so {capability} "
            f"is unavailable. Run `hairball model` to refresh your login, or check "
    ...
    if not account_info.logged_in:
        if account_info.inference_credential_present:
            return (
                f"managed inference credentials are configured, but Hairball cannot verify "
                f"your Managed Portal paid access for {capability}. Log in with "
                f"`hairball model` to enable Portal-managed features. Billing and "
                f"credits are managed at {billing_url}."
            )
        return (
            f"Log in to Managed Portal to use {capability}: run `hairball model`. "
            f"Billing and credits are managed at {billing_url}."
        )

    if account_info.paid_service_access is None:
        detail = (
            f"Hairball could not verify your Managed Portal paid access, so {capability} "
            f"is unavailable."
        )
        if account_info.error:
            detail += f" Account lookup failed: {account_info.error}."
        if include_refresh_hint:
            detail += " Run `hairball model` to refresh your session."
    ...
    if reason == "account_missing":
        return (
            f"Hairball could not find a Managed Portal account or organisation for this "
            f"login, so {capability} is unavailable. Run `hairball model` to "
            f"authenticate again; if the problem persists, contact managed support."
        )
    ...
    if reason == "no_usable_credits" or account_info.paid_service_access is False:
        ...
        if include_refresh_hint and not account_info.fresh:
            message += " If you recently bought credits, run `hairball model` to refresh Hairball."
        return message

    return (
        f"Your Managed Portal account does not currently have paid service access, "
        f"so {capability} is unavailable. Add credits or update billing at {billing_url}."
    )


def _no_paid_access_message(
    account_info: ManagedPortalAccountInfo,
    capability: str,
    billing_url: str,
) -> str:
    access = account_info.paid_service_access_info
    ...
    if has_active_subscription and active_subscription_is_paid:
        ...
        return (
            f"Your Managed Portal credits are exhausted{credit_detail}, so {capability} "
    ...
    if has_active_subscription and active_subscription_is_paid is False:
        return (
            f"Your current Managed Portal plan does not include paid service access, "
    ...
    if has_active_subscription is False:
        credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
        return (
            f"Your Managed Portal account has no active subscription or usable credits"
    ...
    credit_detail = _credit_detail(total_usable, subscription_credits, purchased_credits)
    return (
        f"Your Managed Portal account has no usable paid credits{credit_detail}, so "
...
def get_managed_portal_account_info(
    *,
    ...
    When ``managed_foreign.backend`` is ``off``, returns a logged-out empty
    snapshot without contacting any portal host.
    """
    try:
        from hairball_cli.managed_foreign_backends import is_managed_foreign_off

        if is_managed_foreign_off():
            return ManagedPortalAccountInfo(
                logged_in=False,
                source="none",
                fresh=False,
            )
...
def _fresh_account_info(
    *,
    ...
    try:
        from hairball_cli.auth import get_provider_auth_state, resolve_managed_access_token

        access_token = resolve_managed_access_token()
        ...
        payload = _fetch_managed_account_info(access_token, portal_base_url)
        if not payload:
            return _error_info(
...
def _info_from_inference_key_pool(
    portal_base_url: Optional[str],
) -> Optional[ManagedPortalAccountInfo]:
    """Return an explicit unknown-entitlement snapshot for opaque managed keys."""
    try:
        entry = _select_managed_pool_entry()
        if entry is None:
            return None
...
def _info_from_oauth_pool(
    *,
    ...
    try:
        payload = _fetch_managed_account_info(access_token, entry_portal_url)
    except Exception as exc:
        return _error_info(
            error=exc,
            logged_in=True,
...
def _select_managed_pool_entry() -> Optional[Any]:
...
def _fetch_managed_account_info(
    access_token: str,
    portal_base_url: Optional[str] = None,
) -> dict[str, Any]:
    from hairball_cli.managed_foreign_backends import resolve_portal_base_url

    base = resolve_portal_base_url(portal_base_url)
...
def _info_from_valid_jwt(
    ...
    *,
    ...
    try:
        from hairball_cli.auth import _decode_jwt_claims
    except Exception:
        return None
    ...
    access_info = ManagedPaidServiceAccessInfo(
        allowed=paid_access,
    ...
    return ManagedPortalAccountInfo(
        ...
        managed_client=_coerce_str(claims.get("managed_client")),
        portal_base_url=portal_base_url,
...
def _tool_access_from_value(value: Any) -> Optional[ManagedToolAccessInfo]:
    """Parse a Portal ``tool_access`` object (from the JWT claim or the account
    API) into :class:`ManagedToolAccessInfo`. Fails closed: a non-object value
    ...
    return ManagedToolAccessInfo(enabled=enabled, coverage=coverage)


def _subscription_from_payload(value: Any) -> Optional[ManagedPortalSubscriptionInfo]:
    if not isinstance(value, dict):
        return None
    return ManagedPortalSubscriptionInfo(
...
def _paid_service_access_from_payload(value: Any) -> Optional[ManagedPaidServiceAccessInfo]:
    if not isinstance(value, dict):
        return None
    allowed = _coerce_bool(value.get("allowed"))
    paid_access = _coerce_bool(value.get("paid_access"))
    return ManagedPaidServiceAccessInfo(
        allowed=allowed,
        paid_access=paid_access,
