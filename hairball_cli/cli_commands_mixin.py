...
This module hosts the ``_handle_*_command`` slash-command handlers and
``process_command`` dispatch lifted out of ``cli.py``'s ``HairballCLI`` class.
``HairballCLI`` inherits ``CLICommandsMixin`` so every ``self.<handler>`` /
``self.process_command`` call resolves unchanged via the MRO — behavior-neutral.
...
from hairball_constants import display_hairball_home, is_termux as _is_termux_environment
...
class CLICommandsMixin:
    ...
    def _handle_rollback_command(self, command: str):
        ...
        if not mgr.enabled:
            ...
            print("  Enable with: hairball --checkpoints")
    ...
    def _handle_snapshot_command(self, command: str):
        """Handle /snapshot — lightweight state snapshots for Hairball config/state.
        ...
        """
        from hairball_cli.backup import (
        ...
        if subcmd in {"list", "ls"}:
            ...
            if not snaps:
                ...
                return
            print(f"  State snapshots ({display_hairball_home()}/state-snapshots/):\n")
    ...
    def _handle_stop_command(self):
        ...
        Inspired by ChatGPT's separation of interrupt (stop current turn)
    ...
    def _handle_journey_command(self, cmd_original: str) -> None:
        """Handle /journey — the learning timeline (see `hairball journey`).
        ...
        from cli import _cprint
        from hairball_cli.journey import register_cli
    ...
    def _handle_paste_command(self):
        ...
        from hairball_cli.clipboard import has_clipboard_image
    ...
    def _handle_image_command(self, cmd_original: str):
        ...
        if _remainder:
            ...
            _cprint(f"  {_DIM}Tip: type your next message, or run hairball chat -q --image {_termux_example_image_path(image_path.name)} \"What do you see?\"{_RST}")
    ...
    def _handle_tools_command(self, cmd: str):
        ...
        def _run_capture(ns: Namespace) -> None:
            ...
            # Buffer reports isatty()=True so color() in hairball_cli/colors.py
    ...
    def _handle_handoff_command(self, cmd_original: str) -> bool:
        ...
        try:
            ...
            pass
        _cprint("  Timed out waiting for the gateway. Is `hairball gateway` running?")
    ...
    def _handle_resume_command(self, cmd_original: str) -> None:
        ...
        from cli import (
            _cprint,
            _render_completion_assessment_status,
        ...
        if not target:
            ...
            if self._show_recent_sessions(reason="resume"):
                ...
                return
            _cprint("  Tip:   Use /history or `hairball sessions list` to find sessions.")
            return
        ...
        if target.isdigit():
            ...
            from hairball_cli.main import _resolve_session_by_name_or_id
        ...
        if not session_meta:
            ...
            _cprint("  Use /history or `hairball sessions list` to see available sessions.")
            return
        ...
        try:
            completion_assessment = (
                self._session_db
                .read_latest_terminal_completion_assessment_for_lineage(target_id)
            )
        except Exception:
    ...
    def _handle_branch_command(self, cmd_original: str) -> None:
        ...
        # Flush un-persisted messages before the atomic fork (#47202). A failed
        # flush must abort instead of silently producing a stale child.
        if self.agent:
            try:
                self.agent._flush_messages_to_session_db(
                    self.conversation_history
                ...
                _cprint(f"  Failed to flush current session before branching: {e}")
                return

        # One SessionDB transaction creates the child, copies full replay
        # metadata, applies the title/lineage marker, and ends the parent.
        # No partial branch is observable if any part fails.
        try:
            branch = self._session_db.fork_session(
                parent_session_id,
                new_session_id,
            ...
            return
        branch_title = branch.get("title") or branch_name or "branch"
    ...
    def _handle_pet_command(self, cmd: str):
        ...
        from hairball_cli.pets import _set_active, _set_enabled, print_pet_gallery, set_pet_scale, toggle_pet_display
    ...
    def _handle_suggestions_command(self, cmd: str):
        ...
        args = " ".join(tokens)
        try:
            from hairball_cli.suggestions_cmd import handle_suggestions_command
    ...
    def _handle_blueprint_command(self, cmd: str):
        ...
        try:
            from hairball_cli.blueprint_cmd import handle_blueprint_command
    ...
    def _handle_curator_command(self, cmd: str):
        ...
        Delegates to hairball_cli.curator so the CLI and the `hairball curator`
        ...
        try:
            from hairball_cli.curator import cli_main
            cli_main(tokens)
        except SystemExit:
    ...
    def _handle_kanban_command(self, cmd: str):
        ...
        """
        from hairball_cli.kanban import run_slash
    ...
    def _handle_skills_command(self, cmd: str):
        """Handle /skills slash command — delegates to hairball_cli.skills_hub."""
        ...
        if args and args[0].lower() in {"pending", "approve", "apply", "reject",
            ...
            from hairball_cli.write_approval_commands import handle_pending_subcommand
            ...
            if out is not None:
                print(out)
                return
        from hairball_cli.skills_hub import handle_skills_slash
    ...
    def _handle_background_command(self, cmd: str):
        ...
        """
        from cli import AIAgent, ChatConsole, _accent_hex, _cprint, _maybe_remap_for_light_mode, _render_completion_assessment_status, _render_final_assistant_content, set_approval_callback, set_secret_capture_callback, set_sudo_password_callback
    ...
    def _handle_bundles_command(self, cmd: str) -> None:
        ...
        Mirrors ``hairball bundles list`` but renders inside the running
        ...
        if not bundles:
            ...
            _cprint(
                f"  {_DIM}Create one with: hairball bundles create "
        ...
        _cprint(
            ...
            f"Manage with `hairball bundles`.{_RST}"
        )
        ...
        if sub.startswith("connect"):
            ...
            if hasattr(self, '_pending_input'):
                self._pending_input.put(
                    ...
                    "just because CDP is connected. This is typically a Hairball-managed isolated debug "
    ...
    def _handle_goal_command(self, cmd: str) -> None:
        ...
        # from reference's "let the agent draft the goal" guidance: the contract
        ...
        from hairball_cli.goals import parse_contract
        ...
        _cprint(
            ...
            f"Hairball keeps working until it is, you pause/clear it, or the budget is "
    ...
    def _handle_plan_command(self, cmd: str) -> None:
        """Run the shared durable Plan control contract for the classic CLI."""

        from cli import _cprint
        from agent.plan_commands import handle_plan_command

        parts = str(cmd or "").strip().split(None, 1)
        argument = parts[1].strip() if len(parts) > 1 else "status"
        db = None
        ...
        try:
            context = handle_plan_command(
                str(getattr(self, "session_id", None) or "default"),
                argument,
                surface="cli",
                actor="user",
                db=db,
            ...
            _cprint(f"  Plan error [{code}]: {exc}")
            return

        if context.get("kind") in {"prompt", "execute"}:
            context["operation_id"] = str(
                context.get("plan_execution_id") or f"cli-plan-{uuid.uuid4().hex}"
            )
            self._pending_plan_context = context
            self._pending_input.put(str(context.get("message") or ""))
            notice = (
                "Plan mode queued (read-only until exact approval)."
                if context.get("kind") == "prompt"
                else "Exact approved Plan queued for execution."
            )
            _cprint(f"  {notice}")
            return
        _cprint(f"  {str(context.get('message') or '')}")
    ...
    def _handle_goal_draft(self, objective: str) -> None:
        ...
        from cli import _DIM, _RST, _cprint
        from hairball_cli.goals import draft_contract
    ...
    def _handle_skin_command(self, cmd: str):
        ...
        try:
            from hairball_cli.skin_engine import list_skins, set_active_skin, get_active_skin_name
        except ImportError:
        ...
        if len(parts) < 2 or not parts[1].strip():
            ...
            print("\n  Usage: /skin <name>")
            print(f"  Custom skins: drop a YAML file in {display_hairball_home()}/skins/\n")
            return
    ...
    def _compose_in_editor(self, initial_text: str = "") -> str:
        ...
        fd, path = tempfile.mkstemp(suffix=".md", prefix="hairball_prompt_")
        try:
    ...
    def _handle_busy_command(self, cmd: str):
        """Handle /busy — control what Enter does while Hairball is working.
        ...
        if save_config_value("display.busy_input_mode", arg):
            if arg == "queue":
                behavior = "Enter will queue follow-up input while Hairball is busy."
            elif arg == "steer":
                behavior = "Enter will steer your message into the current run (after the next tool call)."
            else:
                behavior = "Enter will interrupt the current run while Hairball is busy."
    ...
    def _handle_fast_command(self, cmd: str):
        ...
        try:
            from hairball_cli.models import _is_anthropic_fast_model
            agent = getattr(self, "agent", None)
    ...
    def _handle_debug_command(self, cmd_original: str = ""):
        ...
        - ``/debug managed``   → upload to managed-internal storage (private, staff-only)
        - ``/debug local``  → render the report to stdout, no upload

        ``managed`` and ``local`` are mutually exclusive; if both are given,
        ``local`` wins (it never touches the network).
        """
        from hairball_cli.debug import run_debug_share
        from types import SimpleNamespace

        words = {w.lower() for w in cmd_original.split()[1:]}
        local = "local" in words
        managed = "managed" in words and not local
        ...
        args = SimpleNamespace(
            lines=200, expire=7, local=local, managed=managed, yes=True
        )
        run_debug_share(args)

    def _handle_update_command(self) -> bool:
        """Handle /update — update Hairball Agent to the latest version.

        In the classic CLI this exits the session and relaunches as
        ``hairball update`` so the user sees update output directly and gets
        ...
        when cancelled.
        """
        from hairball_cli.config import is_managed, format_managed_message

        if is_managed():
            print(f"  ✗ {format_managed_message('update Hairball Agent')}")
            return False
        ...
        choices = [
            ("once", "Update Now", "exit the current session and update Hairball Agent"),
            ("cancel", "Cancel", "keep the current session"),
        ]
        raw = self._prompt_text_input_modal(
            title="ฅ  Update Hairball Agent",
            detail="This will exit the current session and run `hairball update`.",
            choices=choices,
        )
        if raw is None:
    ...
    def process_command(self, command: str) -> bool:
        ...
        """
        # Lazy imports from cli.py (avoid mixin↔cli import cycle at module load).
        from cli import (
            ChatConsole,
            _ACCENT,
            _DIM,
        ...
        # change in hairball_cli/commands.py instead of touching every dispatch site.
        from hairball_cli.commands import resolve_command as _resolve_cmd
        ...
        if canonical in {"quit", "exit"}:
            ...
            from hairball_cli.main import _print_version_info
            ...
            try:
                # Discover from disk (bundled + user), matching `hairball plugins
                ...
                # catalog lives behind `hairball plugins list`.
                user_entries = [e for e in entries if e[3] != "bundled"]
                bundled_count = len(entries) - len(user_entries)

                if not user_entries:
                    print("No user plugins installed.")
                    print("  Install one: hairball plugins install owner/repo")
                    print(f"  Or drop a plugin directory into {display_hairball_home()}/plugins/")
                    if bundled_count:
                        print(f"  ({bundled_count} bundled plugins available — see: hairball plugins list)")
                else:
                    ...
                    if bundled_count:
                        print(f"  (+{bundled_count} bundled — see: hairball plugins list)")
                    print("  Enable/disable: hairball plugins enable/disable <name>")
            except Exception as e:
        ...
        elif canonical == "inject":
            parts = cmd_original.split(None, 1)
            payload = parts[1].strip() if len(parts) > 1 else ""
            if not payload:
                _cprint("  Usage: /inject <prompt>")
            elif self._agent_running and self.agent is not None and hasattr(self.agent, "inject"):
                try:
                    accepted = self.agent.inject(payload)
                except Exception as exc:
                    _cprint(f"  Inject failed: {exc}")
                else:
                    if accepted:
                        _cprint(f"  Next-step note parked (no interrupt): {payload[:80]}{'...' if len(payload) > 80 else ''}")
                    else:
                        _cprint("  Inject rejected (empty payload).")
            else:
                park = getattr(self, "_park_idle_next_step", None)
                accepted = bool(park(payload)) if callable(park) else False
                if accepted:
                    _cprint(f"  Next-step note parked (does not start a turn): {payload[:80]}{'...' if len(payload) > 80 else ''}")
                else:
                    _cprint("  Inject rejected (empty payload).")
        elif canonical == "goal":
            ...
            from hairball_cli.moa_config import (
                moa_usage,
            ...
            # Check for skill slash commands (/gif-search, /axolotl, etc.)
                ...
                from hairball_cli.commands import COMMANDS
