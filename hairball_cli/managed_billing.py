"""Managed Portal terminal-billing HTTP client (Phase 2b).

Thin, fail-loud client for the four ``/api/billing/*`` endpoints the terminal
billing screens drive. Companion to ``hairball_cli/managed_account.py`` (which owns
...
- **Auth** = the OAuth bearer JWT Hairball already holds for inference
  (``get_provider_auth_state("managed")["access_token"]``). No API-key auth on these.
- **Portal base URL** resolves with the same precedence as the device-flow login
  (``auth.py``): ``HAIRBALL_PORTAL_BASE_URL`` → ``MANAGED_PORTAL_BASE_URL`` → the
...
# hairball_cli.auth.MANAGED_BILLING_MANAGE_SCOPE (kept here too so this module has no
...
def resolve_portal_base_url(state: Optional[dict[str, Any]] = None) -> str:
    ...
    ``managed_foreign`` / ``HAIRBALL_PORTAL_*`` → stored auth-state
    ``portal_base_url`` → registry default. Trailing slash stripped.
    When ``managed_foreign.backend`` is ``off``, never falls back to the
    legacy portal host unless an explicit URL is set.
    """
    ...
    if state:
        ...
        if isinstance(stored, str) and stored.strip():
            if is_managed_foreign_off():
                # Stored legacy URL is ignored while managed foreign is off.
                pass
            else:
    ...
    if switched:
        return switched
    if is_managed_foreign_off():
        raise BillingError(managed_foreign_disabled_message(), error="managed_foreign_off")
...
# Short-lived cache for the resolved (token, base). `resolve_managed_access_token`
...
def _billing_not_logged_in(exc: Optional[BaseException] = None) -> "BillingAuthError":
    ...
    err = BillingAuthError(
        "Not logged into Managed Portal — run `hairball portal` to log in.",
        status=401,
        error="invalid_token",
    )
    if exc is not None:
...
def _resolve_token_and_base(*, use_cache: bool = True) -> tuple[str, str]:
    ...
    (``resolve_managed_access_token``), so a short-lived (~15 min) access token that
    ...
    when there is no usable managed session at all.
...
def _request(
    ...
    *,
    ...
    try:
        from hairball_cli.managed_foreign_backends import (
            account_billing_available,
            managed_foreign_disabled_message,
        )

        if not account_billing_available():
            raise BillingError(
                managed_foreign_disabled_message(), error="managed_foreign_off"
            )
    except BillingError:
        raise
    except Exception:
    ...
    try:
        ...
        raise BillingError(
            f"Could not reach Managed Portal: {exc.reason}", error="network_error"
        ) from exc
