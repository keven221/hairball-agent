...
the existing module-level functions in hairball_cli.gateway and
hairball_cli.gateway_windows directly. This protocol is a thin facade
...
``hairball gateway start/stop/restart`` when running inside a container.
"""
...
def detect_service_manager() -> ServiceManagerKind:
    ...
    in ``hairball gateway start/stop/restart``).
    """
    ...
    # `hairball gateway start/stop/restart` fell through to host code that spawns
...
def _s6_running() -> bool:
    ...
    Detection has to work for **both** root and the unprivileged hairball
    ...
    detection always fails. Since every Hairball runtime call inside the
    container drops to hairball via ``s6-setuidgid``, that silent failure
...
# in ``hairball_cli.gateway`` (systemd/launchd) and ``hairball_cli.gateway_windows``
...
# active (set via the ``hairball -p <profile>`` flag before the call). This
...
class SystemdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``systemd_*`` functions in hairball_cli.gateway.
    ...
    def start(self, name: str) -> None:
        from hairball_cli.gateway import systemd_start
        systemd_start()

    def stop(self, name: str) -> None:
        from hairball_cli.gateway import systemd_stop
        systemd_stop()

    def restart(self, name: str) -> None:
        from hairball_cli.gateway import systemd_restart
        systemd_restart()

    def is_running(self, name: str) -> bool:
        from hairball_cli.gateway import _probe_systemd_service_running
        _, running = _probe_systemd_service_running()
        return running


class LaunchdServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around the ``launchd_*`` functions in hairball_cli.gateway."""

    kind: ServiceManagerKind = "launchd"

    def start(self, name: str) -> None:
        from hairball_cli.gateway import launchd_start
        launchd_start()

    def stop(self, name: str) -> None:
        from hairball_cli.gateway import launchd_stop
        launchd_stop()

    def restart(self, name: str) -> None:
        from hairball_cli.gateway import launchd_restart
        launchd_restart()

    def is_running(self, name: str) -> bool:
        from hairball_cli.gateway import _probe_launchd_service_running
        return _probe_launchd_service_running()


class WindowsServiceManager(_RegistrationUnsupportedMixin):
    """Thin wrapper around ``hairball_cli.gateway_windows`` (Scheduled Task /
...
#
# Per-profile gateways are registered dynamically when `hairball profile create`
# runs inside the container (Phase 4). Static services (main-hairball, dashboard)
...
def _profile_dir_for_gateway_service(name: str) -> Path:
    ...
    ``gateway stop --all``. Do not write the caller's HAIRBALL_HOME blindly;
    derive the shared profile root from the current HAIRBALL_HOME and map the
    ...
    hairball_home = Path(os.environ.get("HAIRBALL_HOME", "/opt/data"))
    if hairball_home.parent.name == "profiles":
        root = hairball_home.parent.parent
    else:
        root = hairball_home
...
# UID/GID of the in-image ``hairball`` user. Hardcoded to match what
...
def _seed_supervise_skeleton(svc_dir: Path) -> None:
    ...
    inside a service directory, owned by the hairball user.
    ...
    (the ``hairball`` user — UID 10000 — running every Hairball runtime
    ...
    is dispatched as the hairball user.
    ...
    with hairball ownership before triggering ``s6-svscanctl -a``,
    s6-supervise inherits our layout and never touches it.

    Layout produced
    ---------------
    ``svc_dir/``                           hairball:hairball, 0755 (parent must already exist)
    ``svc_dir/event/``                     hairball:hairball, 03730   (setgid + g+rwx + sticky)
    ``svc_dir/supervise/``                 hairball:hairball, 0755
    ``svc_dir/supervise/event/``           hairball:hairball, 03730
    ``svc_dir/supervise/control``          hairball:hairball, 0660    (FIFO)
    ...
    slot's supervise/ was hairball-owned.
    ...
    def _set_runtime_ownership(path: Path) -> None:
        try:
            os.chown(path, _HAIRBALL_UID, _HAIRBALL_GID)
        except PermissionError:
            # An unprivileged caller cannot assign the container-only
            # 10000:10000 identity.  The node was just created by this process,
            # so keep it locally operable and move it onto the caller's own
            # effective group.  This matters on macOS /private/tmp, whose new
            # children can inherit wheel(0): chmod would otherwise silently
            # clear the requested setgid bit because the caller is not a
    ...
    def _mkdir_owned(path: Path, mode: int) -> None:
        ...
        _set_runtime_ownership(path)
        # POSIX chown may clear setuid/setgid bits even when chmod ran first.
        # Apply the final requested mode after ownership is settled so event/
        # reliably retains 03730 under root-run s6 container setup as well as
        # under an unprivileged local test process.
        path.chmod(mode)
    ...
    if not control.exists():
        ...
        _set_runtime_ownership(control)
        control.chmod(0o660)
    ...
    # when the parent slot's supervise/ is hairball-owned.
    log_dir = svc_dir / "log"
    if log_dir.is_dir():
        ...
        if not log_control.exists():
            ...
            _set_runtime_ownership(log_control)
...
class GatewayNotRegisteredError(S6Error):
    ...
    Most commonly: ``hairball -p typo gateway start`` when no profile
    ...
    def __init__(self, profile: str) -> None:
        ...
        super().__init__(
            ...
            f"`hairball profile create {profile}` first, or pass "
...
class S6ServiceManager:
    ...
    ``S6_DYNAMIC_SCANDIR``. Static services (main-hairball, dashboard)
    ...
    def _render_run_script(
        ...
        The script:
          1. Sources HAIRBALL_HOME (and any extra env) via with-contenv —
             so e.g. ``-e HAIRBALL_HOME=/data/hairball`` is honored at run
          ...
          4. Drops to the hairball user and exec's
             ``hairball -p <profile> gateway run`` (or just ``hairball
             gateway run`` for the default profile — see below).

        Special case: ``profile == "default"`` emits ``hairball gateway
        run`` with **no** ``-p`` flag. This is the sentinel for "the
        root HAIRBALL_HOME profile" (the implicit profile that exists at
        the top of $HAIRBALL_HOME, not under profiles/). It must be
        spelled this way because ``_profile_suffix()`` returns the
        empty string for the root profile, and the dispatcher in
        ``hairball_cli.gateway`` maps that empty string to the
        ``gateway-default`` service slot. Passing ``-p default`` here
        would instead look up ``$HAIRBALL_HOME/profiles/default/`` — a
        ...
        each supervised profile gateway loads its own ``HAIRBALL_HOME``,
        ...
        allocator in ``hairball_cli.profiles._allocate_gateway_port``).
        ...
        lines = [
            ...
            "cd /opt/data",
            ". /opt/hairball/.venv/bin/activate",
        ]
        ...
        # guard.
        lines.append("export HAIRBALL_S6_SUPERVISED_CHILD=1")
        # ``--replace`` makes the supervised gateway authoritative for its
        # profile's HAIRBALL_HOME. Without it, a gateway started OUTSIDE s6
        # (a stray ``hairball gateway run`` from a shell, an agent action, or
        # the Open WebUI helper) grabs the per-HAIRBALL_HOME PID lock first;
        ...
        # gateway/run.py) so s6 always wins. The HAIRBALL_S6_SUPERVISED_CHILD
        # sentinel above prevents the run→start→run redirect recursion.
        # Each profile is scoped to its own HAIRBALL_HOME and s6 guarantees a
        # single supervised instance per slot, so there is no legitimate
        # supervised sibling for ``--replace`` to clobber.
        if profile == "default":
            gateway_cmd = "hairball gateway run --replace"
        else:
            gateway_cmd = f"hairball -p {shlex.quote(profile)} gateway run --replace"
        # Skip the drop when already non-root (setgroups() lacks CAP_SETGID →
        # s6 boot-loop).
        lines.append(f'[ "$(id -u)" = 0 ] || exec {gateway_cmd}')
        lines.append(f"exec s6-setuidgid hairball {gateway_cmd}")
        return "\n".join(lines) + "\n"
    ...
    def _render_log_run(profile: str) -> str:
        ...
        OQ8-C: persist to ``${HAIRBALL_HOME}/logs/gateways/<profile>/``.
        CRITICAL: the HAIRBALL_HOME path is sourced from the runtime env
        via with-contenv — NOT Python-substituted at registration time
        — so a container started with ``-e HAIRBALL_HOME=/data/hairball``
        gets its logs under /data/hairball/logs/..., not the build-time
        default.
        ...
        per line, in order:
          ...
          2. ``T <log_dir>`` — also write a timestamped copy to the
             ...
             files). This is what ``hairball logs`` reads and what
        ...
        return (
            ...
            f': "${{HAIRBALL_HOME:=/opt/data}}"\n'
            f'log_dir="$HAIRBALL_HOME/logs/gateways/{prof}"\n'
            f'mkdir -p "$log_dir"\n'
            ...
            # later then runs its log service as hairball and crash-loops on
            ...
            f'chown hairball:hairball "$HAIRBALL_HOME/logs/gateways" 2>/dev/null || true\n'
            f'chown -R hairball:hairball "$log_dir" 2>/dev/null || true\n'
            f'rm -f "$log_dir/lock"\n'
            # Skip the drop when already non-root (CAP_SETGID).
            f'[ "$(id -u)" = 0 ] || exec s6-log 1 n10 s1000000 T "$log_dir"\n'
            f'exec s6-setuidgid hairball s6-log 1 n10 s1000000 T "$log_dir"\n'
        )
    ...
    def stop(self, name: str) -> None:
        ...
        intent). Without the marker, an intentional ``hairball gateway
    ...
    def register_profile_gateway(
        ...
        *,
        ...
        user explicitly runs ``hairball -p <profile> gateway start``.
        ...
        try:
            ...
            # Pre-create the supervise/ skeleton with hairball ownership
            ...
            # the hairball user) won't hit EACCES on root-owned 0700
    ...
    def unregister_profile_gateway(self, profile: str) -> None:
        ...
        **before** ``rmtree`` so s6-svscan reaps the supervise child
        ...
        # is hairball-owned (see ``_seed_supervise_skeleton``), and on
