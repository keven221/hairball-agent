...
Invoked by ``hairball kanban decompose [task_id | --all]`` and the
...
* Mirrors the shape of ``hairball_cli/kanban_specify.py``: lazy aux
...
# This value is only sent on provider wires that require an explicit output
# budget (for example Anthropic-compatible endpoints).  The shared auxiliary
# caller intentionally omits it on providers such as DeepSeek so a reasoning
# model cannot spend the entire cap before producing the JSON task graph.
KANBAN_DECOMPOSE_MAX_TOKENS = 8192


_SYSTEM_PROMPT = """You are the Kanban decomposer for the Hairball Agent board.
...
def _profile_author() -> str:
    """Mirror of ``hairball_cli.kanban._profile_author``."""
    return (
        os.environ.get("HAIRBALL_PROFILE")
    ...
    *,
    ...
    try:
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=KANBAN_DECOMPOSE_MAX_TOKENS,
            timeout=timeout,
        )
    except Exception as exc:
    ...
    try:
        ...
        # A length-truncated reasoning response is scratch work, not a task
        # graph.  Do not feed it to the lenient JSON parser.
        raw = content if finish_reason == "length" else extract_content_or_reasoning(resp)
    except Exception:
        raw = ""
        finish_reason = ""
    ...
    if parsed is None:
        if finish_reason == "length":
            return DecomposeOutcome(
                task_id,
                False,
                "LLM exhausted its output budget before producing a valid JSON task graph",
            )
