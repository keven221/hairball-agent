...
- **Aggregators** (OpenRouter, managed, AI Gateway, Kilo Code) need
...
- **DeepSeek** accepts ``deepseek-chat`` (V3), ``deepseek-reasoner``
  ...
  Hairball revisions folded every non-reasoner input into
...
# slug used by aggregator APIs (OpenRouter, managed, etc.).
#
...
# ``api.xiaomimimo.com`` rejects mixed-case names like ``provider-V2.5-Pro``
...
def _normalize_provider_alias(provider_name: str) -> str:
    """Resolve provider aliases to Hairball' canonical ids."""
    raw = (provider_name or "").strip().lower()
    if not raw:
        return raw
    try:
        from hairball_cli.models import normalize_provider
...
def normalize_model_for_provider(model_input: str, target_provider: str) -> str:
    ...
    Args:
        ...
        target_provider: The canonical Hairball provider id, e.g.
            ``"openrouter"``, ``"anthropic"``, ``"copilot"``,
            ``"deepseek"``, ``"custom"``.  Should already be normalised
            via ``hairball_cli.models.normalize_provider()``.
    ...
    Examples::
        ...
        >>> normalize_model_for_provider("provider-V2.5-Pro", "xiaomi")
        'mimo-v2.5-pro'
    """
    ...
    if provider in {"copilot", "copilot-acp"}:
        try:
            from hairball_cli.models import normalize_copilot_model_id
    ...
    if provider in _MATCHING_PREFIX_STRIP_PROVIDERS:
        ...
        # rejects "provider-V2.5-Pro" but accepts "mimo-v2.5-pro").
        if provider in _LOWERCASE_MODEL_PROVIDERS:
            result = result.lower()
        # ``mimo-*`` is Xiaomi's wire-level model namespace. A historical
        # display-only alias exposed the same IDs as ``provider-*``; repair
        # that alias only on the Xiaomi route so the native API receives the
        # protocol identifier it accepts. Custom and other providers remain
        # untouched even when their own model happens to start with that word.
        if provider == "xiaomi" and result.startswith("provider-"):
            result = f"mimo-{result.removeprefix('provider-')}"
        return result
