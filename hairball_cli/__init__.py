"""
Hairball CLI - Unified command-line interface for Hairball Agent.

Provides subcommands for:
- hairball chat          - Interactive chat (same as ./hairball)
- hairball gateway       - Run gateway in foreground
- hairball gateway start - Start gateway service
...
def _ensure_utf8():
    ...
    The CLI prints box-drawing characters (┌│├└─) and the ฅ glyph in the setup
    ...
    can even start — e.g. `hairball setup` on a fresh Pi.
    ...
    hairball_cli/stdio.py::configure_windows_stdio() runs later from the entry
