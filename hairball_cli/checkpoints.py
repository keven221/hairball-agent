"""`hairball checkpoints` CLI subcommand.

Gives users direct visibility and control over the filesystem checkpoint
store at ``~/.hairball/checkpoints/``.  Actions:

    hairball checkpoints               # same as `status`
    hairball checkpoints status        # total size, project count, breakdown
    hairball checkpoints list          # per-project checkpoint counts + workdir
    hairball checkpoints prune [opts]  # force a sweep (ignores the 24h marker)
    hairball checkpoints clear [-f]    # nuke the entire base (asks first)
    hairball checkpoints clear-legacy  # delete just the legacy-* archives

Examples::

    hairball checkpoints
    hairball checkpoints prune --retention-days 3 --max-size-mb 200
    hairball checkpoints clear -f
...
def cmd_status(args: argparse.Namespace) -> int:
    ...
    if legacy:
        ...
        print()
        print("Clear with: hairball checkpoints clear-legacy")
    return 0
...
def register_cli(parser: argparse.ArgumentParser) -> None:
    """Wire subcommands onto the ``hairball checkpoints`` parser."""
    parser.set_defaults(func=cmd_status)  # bare `hairball checkpoints` → status
