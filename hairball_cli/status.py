"""
Status command for hairball CLI.

Shows the status of all Hairball Agent components.
"""
...
from hairball_cli.auth import AuthError, resolve_provider
from hairball_cli.colors import Colors, color
from hairball_cli.config import get_env_path, get_env_value, get_hairball_home, load_config
from hairball_cli.models import provider_label
...
from hairball_cli.runtime_provider import resolve_requested_provider
...
def redact_key(key: str) -> str:
    ...
    "(not set)" placeholder in dim color to match ``hairball config``'s
...
def show_status(args):
    """Show status of all Hairball Agent components."""
    deep = getattr(args, 'deep', False)

    print()
    print(color("┌─────────────────────────────────────────────────────────┐", Colors.CYAN))
    print(color("│                 ฅ Hairball Agent Status                  │", Colors.CYAN))
    print(color("└─────────────────────────────────────────────────────────┘", Colors.CYAN))
    ...
    if (
        managed_status.get("logged_in")
        or managed_status.get("access_token")
        or managed_status.get("portal_base_url")
        or managed_status.get("inference_credential_present")
        or managed_status.get("error_code")
    ):
        try:
            managed_account_info = get_managed_portal_account_info()
        except Exception:
            managed_account_info = None

    managed_logged_in = bool(
        managed_status.get("logged_in")
        or (managed_account_info and managed_account_info.logged_in)
    )
    managed_inference_present = bool(
        managed_status.get("inference_credential_present")
        or (managed_account_info and managed_account_info.inference_credential_present)
    )
    managed_error = managed_status.get("error")
    if managed_logged_in:
        managed_label = "logged in"
    elif managed_inference_present:
        managed_label = "not logged in (managed inference key configured)"
    else:
        managed_label = "not logged in (run: hairball portal)"
    print(
        f"  {'Managed Portal':<12}  {check_mark(managed_logged_in)} "
        f"{managed_label}"
    )
    portal_url = managed_status.get("portal_base_url") or "(unknown)"
    inference_url = (
        managed_status.get("inference_base_url")
        or (managed_account_info.inference_base_url if managed_account_info else None)
    )
    access_exp = _format_iso_timestamp(managed_status.get("access_expires_at"))
    key_exp = _format_iso_timestamp(managed_status.get("agent_key_expires_at"))
    refresh_label = "yes" if managed_status.get("has_refresh_token") else "no"
    if managed_logged_in or portal_url != "(unknown)" or managed_error:
        print(f"    Portal URL: {portal_url}")
    if managed_inference_present and inference_url:
        print(f"    Inference:  {inference_url}")
    if managed_logged_in or managed_status.get("access_expires_at"):
        print(f"    Access exp: {access_exp}")
    if managed_logged_in or managed_inference_present or managed_status.get("agent_key_expires_at"):
        print(f"    Key exp:    {key_exp}")
    if managed_logged_in or managed_status.get("has_refresh_token"):
        print(f"    Refresh:    {refresh_label}")
    if managed_error:
        print(f"    Error:      {managed_error}")

    codex_logged_in = bool(codex_status.get("logged_in"))
    print(
        f"  {'ChatGPT':<12}  {check_mark(codex_logged_in)} "
        f"{'logged in' if codex_logged_in else 'not logged in (run: hairball model)'}"
    )
    ...
    print(
        ...
        f"{'logged in' if minimax_logged_in else 'not logged in (run: hairball auth add minimax-oauth)'}"
    )
    ...
    # disrupt the already-printed managed/reference/Qwen/MiniMax rows above.
    try:
    ...
    print(
        ...
        f"{'logged in' if xai_oauth_logged_in else 'not logged in (run: hairball auth add xai-oauth)'}"
    )
    ...
    if managed_portal_tools_enabled():
        features = get_managed_subscription_features(config)
        print()
        print(color("◆ Managed Tool Gateway", Colors.CYAN, Colors.BOLD))
        if not features.managed_auth_present:
            print("  Managed Portal   ✗ not logged in")
        else:
            print("  Managed Portal   ✓ managed tools available")
        for feature in features.items():
            if feature.managed_by_managed:
                state = "active via managed subscription"
            elif feature.active:
                current = feature.current_provider or "configured provider"
                state = f"active via {current}"
            elif feature.included_by_default and features.managed_auth_present:
                state = "included by subscription, not currently selected"
            elif feature.key == "modal" and features.managed_auth_present:
                state = "available via subscription (optional)"
            else:
                state = "not configured"
            print(f"  {feature.label:<15} {check_mark(feature.available or feature.active or feature.managed_by_managed)} {state}")
    elif managed_logged_in or managed_inference_present:
        # managed OAuth without entitlement, or an opaque inference key without
    ...
    for pname, env_vars in apikey_providers.items():
        ...
        label = "configured" if configured else "not configured (run: hairball model)"
    ...
    if _effective_provider_label() == "LM Studio":
        from hairball_cli.models import probe_lmstudio_models
        model_cfg = config.get("model")
    ...
    try:
        from hairball_cli.gateway import get_gateway_runtime_snapshot, _format_gateway_pids
        ...
        if snapshot.has_process_service_mismatch:
            ...
            print("  Start with:   hairball gateway")
    ...
    jobs_file = get_hairball_home() / "cron" / "jobs.json"
    if jobs_file.exists():
        import json
        try:
    ...
    print(color("  Run 'hairball doctor' for detailed diagnostics", Colors.DIM))
    print(color("  Run 'hairball setup' to configure", Colors.DIM))
    print()
