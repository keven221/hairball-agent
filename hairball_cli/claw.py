"""hairball claw — legacy migration commands.

Usage:
    hairball claw migrate              # Preview then migrate (always shows preview first)
    hairball claw migrate --dry-run    # Preview only, no changes
    hairball claw migrate --yes        # Skip confirmation prompt
    hairball claw migrate --preset full --overwrite --migrate-secrets  # Full run w/ secrets
    hairball claw migrate --no-backup  # Skip pre-migration snapshot
    hairball claw cleanup              # Archive leftover legacy directories
    hairball claw cleanup --dry-run    # Preview what would be archived
"""
...
from hairball_cli.config import get_hairball_home, get_config_path, load_config, save_config
...
# Known legacy directory names (current + legacy)
_OPENCLAW_DIR_NAMES = (".openclaw", ".clawdbot", ".moltbot")

def _detect_openclaw_processes() -> list[str]:
    """Detect running legacy processes and services.
    ...
    if sys.platform == "win32":
        try:
            ...
            # Node.js-hosted legacy — tasklist doesn't show command lines,
...
def _warn_if_openclaw_running(auto_yes: bool) -> None:
    """Warn if legacy is still running before migration.

    Telegram, Discord, and Slack only allow one active connection per bot
    token. Migrating while legacy is running causes both to fight for the
    same token.
    """
    ...
    print()
    print_error("legacy appears to be running:")
    for detail in running:
        print_info(f"  * {detail}")
    print_info(
        "Messaging platforms (Telegram, Discord, Slack) only allow one "
        "active session per bot token. If you continue, both legacy and "
        "Hairball may try to use the same token, causing disconnects."
    )
    print_info("Recommendation: stop legacy before migrating.")
    print()
    if auto_yes:
        return
    ...
    if not prompt_yes_no("Continue anyway?", default=False):
        print_info("Migration cancelled. Stop legacy and try again.")
        sys.exit(0)


def _warn_if_gateway_running(auto_yes: bool) -> None:
    """Check if a Hairball gateway is running with connected platforms.
    ...
    print()
    print_error(
        "Hairball gateway is running with active connections: "
        + ", ".join(connected)
    )
    print_info(
    ...
    print_info("Recommendation: stop the gateway first with 'hairball gateway stop'.")
    print()
    if not auto_yes and not prompt_yes_no("Continue anyway?", default=False):
        print_info("Migration cancelled. Stop the gateway and try again.")
        sys.exit(0)

# State files commonly found in legacy workspace directories — listed
...
def _find_migration_script() -> Path | None:
    """Find the openclaw_to_hairball.py script in known locations."""
...
def _load_migration_module(script_path: Path):
    ...
    spec = importlib.util.spec_from_file_location("openclaw_to_hairball", script_path)
...
def _find_openclaw_dirs() -> list[Path]:
    """Find all legacy directories on disk."""
    found = []
...
def _scan_workspace_state(source_dir: Path) -> list[tuple[Path, str]]:
    """Scan an legacy directory for workspace state files.
...
def _archive_directory(source_dir: Path, dry_run: bool = False) -> Path:
    """Rename an legacy directory to .pre-migration.
...
def claw_command(args):
    """Route hairball claw subcommands."""
    ...
    if action == "migrate":
        ...
        print("Usage: hairball claw <command> [options]")
        print()
        print("Commands:")
        print("  migrate          Migrate settings from legacy to Hairball")
        print("  cleanup          Archive leftover legacy directories after migration")
        print()
        print("Run 'hairball claw <command> --help' for options.")


def _cmd_migrate(args):
    """Run the legacy → Hairball migration."""
    # Check current and legacy legacy directories
    ...
    # via --migrate-secrets, even under --preset full.  This mirrors legacy's
    # migrate-hairball posture (two-phase: run once without secrets, rerun with
    ...
    if not source_dir.is_dir():
        print()
        print_error(f"legacy directory not found: {source_dir}")
        print_info("Make sure your legacy installation is at the expected path.")
        print_info("You can specify a custom path: hairball claw migrate --source /path/to/.openclaw")
        return
    ...
    print_info(f"Target:      {hairball_home}")
    ...
    # Check if legacy is still running — migrating tokens while both are
    ...
    if preview_count == 0 and preview_conflicts == 0:
        print()
        print_info("Nothing to migrate from legacy.")
    ...
    # Modelled on legacy's assertConflictFreePlan() — apply is a safe no-op
    ...
    if preview_conflicts > 0 and not overwrite:
        ...
        print_info(
            "Each conflict is an item whose target already exists in ~/.hairball/. "
    ...
    if not auto_yes:
        if not sys.stdin.isatty():
            ...
            print_info("To execute, re-run with: hairball claw migrate --yes")
            return
        if not prompt_yes_no("Proceed with migration?", default=True):
            print_info("Migration cancelled.")
            return

    # ── Phase 2b: Pre-apply backup of the Hairball home ─────────
    # Delegates to hairball_cli.backup.create_pre_migration_backup(), which
    # shares implementation with the pre-update backup (same exclusion
    # rules, same SQLite safe-copy, zip format) so the archive is
    # restorable with `hairball import`.  Mirrors legacy's
    ...
    if not no_backup:
        try:
            from hairball_cli.backup import create_pre_migration_backup, _format_size
            backup_archive = create_pre_migration_backup(hairball_home=hairball_home)
            if backup_archive:
                size_str = _format_size(backup_archive.stat().st_size)
                print()
                print_success(f"Pre-migration backup: {backup_archive} ({size_str})")
                print_info(f"Restore with: hairball import {backup_archive.name}")
        except Exception as e:
            print()
            print_error(f"Could not create pre-migration backup: {e}")
            print_info(
                "Re-run with --no-backup to skip, or free up disk space under the Hairball home."
            )
    ...
    # 'hairball claw cleanup' separately.


def _cmd_cleanup(args):
    """Archive leftover legacy directories after migration.

    Scans for legacy directories that still exist after migration and offers
    ...
    if not dirs_to_check:
        print()
        print_success("No legacy directories found. Nothing to clean up.")
        return

    # Warn if legacy is still running — archiving while the service is
    # active causes it to recreate an empty skeleton directory (#8502).
    running = _detect_openclaw_processes()
    if running:
        print()
        print_error("legacy appears to be still running:")
        for detail in running:
            print_info(f"  * {detail}")
        print_info(
        ...
        print_info("Stop legacy first: systemctl --user stop openclaw-gateway.service")
        print()
        if not auto_yes:
            if not sys.stdin.isatty():
                print_info("Non-interactive session — aborting. Stop legacy and re-run.")
                return
            if not prompt_yes_no("Proceed anyway?", default=False):
                print_info("Aborted. Stop legacy first, then re-run: hairball claw cleanup")
                return
    ...
    for source_dir in dirs_to_check:
        ...
        if dry_run:
            ...
            print_info("To execute, re-run with: hairball claw cleanup --yes")
    ...
    if dry_run:
        ...
        print_success(
            f"Cleaned up {total_archived} legacy "
...
def _print_migration_report(report: dict, dry_run: bool):
    ...
    if dry_run:
        ...
        print_info(f"  hairball claw migrate --preset {report.get('preset', 'full')}")
    elif migrated:
        print()
        ...
        if skipped_keys:
            ...
            print_info("  hairball claw migrate --migrate-secrets")
            print()
            print_info("Or add your key manually:")
            print_info("  hairball config set OPENROUTER_API_KEY sk-or-v1-...")
