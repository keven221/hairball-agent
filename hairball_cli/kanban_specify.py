...
Used by ``hairball kanban specify [task_id | --all]``. Takes a task that
...
* This module intentionally mirrors ``hairball_cli/goals.py`` — same aux
...
HAIRBALL_KANBAN_SPECIFY_MAX_TOKENS = max(
    1500,
    env_int("HAIRBALL_KANBAN_SPECIFY_MAX_TOKENS", 6000),
)

logger = logging.getLogger(__name__)


_SYSTEM_PROMPT = """You are the Kanban triage specifier for the Hairball Agent board.
...
def _profile_author() -> str:
    """Mirror of ``hairball_cli.kanban._profile_author``. Kept local to
    ...
    *,
    ...
    try:
        resp = call_llm(
            ...
            temperature=0.3,
            max_tokens=HAIRBALL_KANBAN_SPECIFY_MAX_TOKENS,
            timeout=timeout,
        )
    ...
    if parsed is None and finish_reason == "length":
        return SpecifyOutcome(
            task_id,
            False,
            "LLM exhausted its output budget before producing a complete task spec",
        )
