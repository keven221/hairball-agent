# ============================================================================
# Hairball Agent Installer for Windows
# ============================================================================
# Installation script for Windows (PowerShell).
# Uses uv for fast Python provisioning and package management.
#
# Usage:
#   iex (irm https://raw.githubusercontent.com/hairball-agent/hairball-agent/main/scripts/install.ps1)
#
...
param(
    ...
    [string]$Commit = "",
    [string]$Tag = "",
    [string]$HairballHome = $(if ($env:HAIRBALL_HOME) { $env:HAIRBALL_HOME } else { "$env:LOCALAPPDATA\hairball" }),
    [string]$InstallDir = $(if ($env:HAIRBALL_HOME) { "$env:HAIRBALL_HOME\hairball-agent" } else { "$env:LOCALAPPDATA\hairball\hairball-agent" }),
    ...
    # builds apps/desktop into a launchable Hairball.exe.
    #
    # Why opt-in:
    #   * Hairball-Setup.exe (the signed Tauri bootstrap installer) passes
    ...
    #     from inside an already-launched Hairball.exe; if THAT recursively
    #     built apps/desktop it would try to overwrite the live Hairball.exe
    ...
    #     `hairball desktop` already builds on demand.
...
$UpdateRepository = if ($env:HAIRBALL_UPDATE_REPO) {
    $env:HAIRBALL_UPDATE_REPO.Trim().Trim('/')
} else {
    "hairball-agent/hairball-agent"
}
if ($UpdateRepository -notmatch '^[A-Za-z0-9][A-Za-z0-9_.-]*/[A-Za-z0-9][A-Za-z0-9_.-]*$') {
    throw "HAIRBALL_UPDATE_REPO must use owner/repository form"
}
$RepoUrlSsh = "git@github.com:$UpdateRepository.git"
$RepoUrlHttps = "https://github.com/$UpdateRepository.git"
$PythonVersion = "3.11"
...
function Write-Banner {
    ...
    Write-Host "|             * Hairball Agent Installer                    |" -ForegroundColor Magenta
    Write-Host "+---------------------------------------------------------+" -ForegroundColor Magenta
...
function Write-BrowserEnv {
    ...
    if (-not (Test-Path $HairballHome)) {
        New-Item -ItemType Directory -Force -Path $HairballHome | Out-Null
    }
    $envFile = Join-Path $HairballHome ".env"
...
function Invoke-AgentBrowserInstallWithTimeout {
    param(
        [string]$AgentBrowserPath,
        [string]$LogPath,
        [int]$TimeoutSeconds = 600,
        [string]$CommandArguments = "install",
        [string]$WorkingDirectory,
        [switch]$StreamOutput
    )

    # agent-browser.cmd is a batch shim, so launch it through cmd.exe rather
    # than Start-Process directly ("%1 is not a valid Win32 application" on
    # Windows PowerShell 5.1).  Redirect inside cmd so the child cannot fill a
    # pipe while the parent is waiting.
    $escapedExe = $AgentBrowserPath.Replace('"', '""')
    $escapedLog = $LogPath.Replace('"', '""')
    if ([string]::IsNullOrWhiteSpace($CommandArguments)) {
        $CommandArguments = "install"
    }
    $commandLine = "/d /s /c `"`"$escapedExe`" $CommandArguments > `"$escapedLog`" 2>&1`""
    $startParams = @{
    ...
    while (-not $proc.WaitForExit(1000)) {
        if ($StreamOutput -and (Test-Path $LogPath)) {
            $lines = @(Get-Content $LogPath -ErrorAction SilentlyContinue)
            for ($i = $emittedLineCount; $i -lt $lines.Count; $i++) {
                $line = $lines[$i]
                Write-Host $line
            }
    ...
    # PowerShell 5.1 / .NET Framework lacks Process.Kill(entireProcessTree).
    # taskkill /T is the portable Windows boundary that also stops npm/node and
    # the Chromium downloader grandchildren.  Keep Stop-Process as a fallback.
    $prevEAP = $ErrorActionPreference
    $ErrorActionPreference = "Continue"
    try {
...
function Install-AgentBrowser {
    ...
    $prefixDir = Join-Path $HairballHome "node"
    ...
    if (-not $SkipChromium) {
        ...
        if ($sysBrowser) {
            ...
            if (Test-Path $abExe) {
                ...
                $abResult = Invoke-AgentBrowserInstallWithTimeout `
                    -AgentBrowserPath $abExe `
                    -LogPath $abLog `
                    -TimeoutSeconds 600
                $abExit = $abResult.ExitCode
                if ($abExit -ne 0) {
                    $abDetail = Get-Content $abLog -Raw -ErrorAction SilentlyContinue
                    if ($abResult.TimedOut) {
                        Write-Warn "Chromium install timed out after 600s; the installer process tree was stopped."
                    } else {
                        Write-Warn "Chromium install failed (exit $abExit)."
                    }
                    if ($abDetail) {
                        $abTail = ($abDetail -split "`r?`n" | Select-Object -Last 20) -join [Environment]::NewLine
                        Write-Info "Last Chromium installer output: $abTail"
                    }
                }
...
function Install-Uv {
    # Hairball owns its own uv at $HairballHome\bin\uv.exe.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hairball_cli/managed_uv.py) looks in the same
    # place, so install.ps1 and `hairball update` stay in sync.
    $managedUv = Join-Path $HairballHome "bin\uv.exe"
    ...
    Write-Info "Installing managed uv into $HairballHome\bin ..."
    New-Item -ItemType Directory -Path (Join-Path $HairballHome "bin") -Force | Out-Null

    # UV_INSTALL_DIR tells the astral installer to place the binary
    # directly into $HairballHome\bin instead of ~/.local/bin.
    $prevEAP = $ErrorActionPreference
    try {
        $ErrorActionPreference = "Continue"
        $env:UV_INSTALL_DIR = Join-Path $HairballHome "bin"
...
function Resolve-UvCmd {
    ...
    # sibling process and HAIRBALL_HOME wasn't propagated).
...
function Resolve-AvailablePythonVersion {
    ...
    # ``$script:PythonVersion = $fallbackVer`` mutation.  Under Hairball-Setup.exe
...
function Install-Git {
    ...
    which Hairball uses to run shell commands.
    ...
    package manager):
      ...
      2. Download **PortableGit** from the official git-for-windows GitHub
         ...
         ``%LOCALAPPDATA%\hairball\git`` -- never touches system Git, never
    ...
    **Why PortableGit, not MinGit:**  MinGit is the minimal-automation
    ...
    Hairball needs ``bash.exe`` to run shell commands.  PortableGit is the
    ...
    blocked).  Owning the Hairball copy of Git ourselves is predictable and
    recoverable: if it ever breaks, ``Remove-Item %LOCALAPPDATA%\hairball\git``
    and re-running this installer fully recovers.

    After install we locate ``bash.exe`` and persist the path in
    ``HAIRBALL_GIT_BASH_PATH`` (User scope) so Hairball can find it in a fresh
    ...
    # Download PortableGit into $HairballHome\git.  Always works as long as
    # we can reach github.com -- no admin, no winget, no reliance on the
    # user's possibly-broken system Git install.
    Write-Info "Git not found -- downloading PortableGit to $HairballHome\git\ ..."
    ...
    try {
        ...
        if ($arch -eq "32-bit-mingit") {
            Write-Warn "32-bit Windows detected -- PortableGit is 64-bit only.  Installing MinGit 32-bit as a last resort; bash-dependent Hairball features (terminal tool, agent-browser) will not work on this machine."
        ...
        Write-Info "then re-run this installer.  Hairball needs Git Bash on Windows to run"
...
function Set-GitBashEnvVar {
    ...
    ``HAIRBALL_GIT_BASH_PATH`` (User env scope) so Hairball can find it even before
    ...
    #
    # Layouts:
    #   PortableGit (our default): $HairballHome\git\bin\bash.exe
    #   MinGit (32-bit fallback):  $HairballHome\git\usr\bin\bash.exe
    $candidates += "$HairballHome\git\bin\bash.exe"       # PortableGit layout (primary)
    $candidates += "$HairballHome\git\usr\bin\bash.exe"   # MinGit / PortableGit usr\bin fallback
    ...
    foreach ($candidate in $candidates) {
        if ($candidate -and (Test-Path $candidate)) {
            [Environment]::SetEnvironmentVariable("HAIRBALL_GIT_BASH_PATH", $candidate, "User")
            $env:HAIRBALL_GIT_BASH_PATH = $candidate
            Write-Info "Set HAIRBALL_GIT_BASH_PATH=$candidate"
            return
        }
    }

    Write-Warn "Could not locate bash.exe -- Hairball may not find Git Bash."
    Write-Info "If needed, set HAIRBALL_GIT_BASH_PATH manually to your bash.exe path."
}
...
function Test-Node {
    ...
    $managedNode = "$HairballHome\node\node.exe"
    if ((Test-Path $managedNode) -and (Test-NodeVersionOk (& $managedNode --version))) {
        $version = & $managedNode --version
        $env:Path = "$HairballHome\node;$env:Path"
        Write-Success "Node.js $version found (Hairball-managed)"
        $script:HasNode = $true
        return $true
    }

    Write-Info "Installing Hairball-managed Node.js $NodeVersion LTS..."
    ...
    # The portable zip path drops node.exe + npm into $HairballHome\node\
    ...
    Write-Info "Downloading portable Node.js $NodeVersion to $HairballHome\node\ ..."
    ...
    try {
        ...
        if ($zipName) {
            ...
            $tmpDir = "$env:TEMP\hairball-node-extract"
            ...
            if ($extractedDir) {
                if (Test-Path "$HairballHome\node") { Remove-Item -Recurse -Force "$HairballHome\node" }
                Move-Item $extractedDir.FullName "$HairballHome\node"
                ...
                $version = & "$HairballHome\node\node.exe" --version
                Write-Success "Node.js $version installed to $HairballHome\node\ (portable, user-scoped)"
                $script:HasNode = $true
...
function Install-SystemPackages {
    ...
    if ($hasWinget) {
        ...
        foreach ($pkg in $wingetPkgs) {
            $log = "$env:TEMP\hairball-winget-$($pkg -replace '[^A-Za-z0-9]','_')-$(Get-Random).log"
            $pkgLogs[$pkg] = $log
...
function Install-Repository {
        ...
        if ($repoValid) {
            ...
            try {
                ...
                if (-not [string]::IsNullOrWhiteSpace(($statusOut -join "`n"))) {
                    ...
                    # the index conflict state is dropped. Mirrors the `hairball
                    # update` path (#4735).
                    ...
                    $stashName = "hairball-install-autostash-" + (Get-Date -Format "yyyyMMdd-HHmmss")
                ...
                if ($Commit) {
                    ...
                    # ff-only pull cannot succeed — mirror ``hairball update`` and
                ...
                if ($autostashRef) {
                    ...
                    if ($restoreNow) {
                        ...
                        if ($LASTEXITCODE -eq 0) {
                            ...
                            Write-Warn "Review git diff / git status if Hairball behaves unexpectedly."
                        } else {
            ...
            try {
                ...
                Write-Info "terminals, running hairball processes) and try again."
                throw
            }
        }
    }
    ...
    if (-not $didUpdate) {
        ...
        if (-not $cloneSuccess) {
            ...
            try {
                ...
                if ($Commit) {
                    $zipUrl = "https://github.com/$UpdateRepository/archive/$Commit.zip"
                    $zipLabel = $Commit
                } elseif ($Tag) {
                    $zipUrl = "https://github.com/$UpdateRepository/archive/refs/tags/$Tag.zip"
                    $zipLabel = $Tag
                } else {
                    $zipUrl = "https://github.com/$UpdateRepository/archive/refs/heads/$Branch.zip"
                    $zipLabel = $Branch
                }
                $zipPath = "$env:TEMP\hairball-agent-$zipLabel.zip"
                $extractPath = "$env:TEMP\hairball-agent-extract"
    ...
    # next `hairball update` checkout aborts on a "dirty" tree the user never
...
function Install-Venv {
    ...
    # Re-resolve the interpreter before creating the venv.  Under Hairball-Setup.exe
    ...
    if (Test-Path "venv") {
        ...
        # speedups.pyd) are loaded as DLLs by any running hairball process.
        ...
        if ($env:OS -eq "Windows_NT") {
            $myPid = $PID
            Write-Info "Stopping any running hairball processes before recreating venv..."
            ...
            try {
                schtasks /Query /FO CSV 2>$null | ConvertFrom-Csv | Where-Object { $_.TaskName -like '*Hairball_Gateway*' } | ForEach-Object {
                    $tn = $_.TaskName
            ...
            # The launcher CLI (hairball.exe) plus its child tree.
            & taskkill /F /T /IM hairball.exe /FI "PID ne $myPid" 2>$null | Out-Null
            # taskkill /IM hairball.exe is NOT enough: the gateway/agent that a
            # scheduled task or watchdog autostarts runs as
            # `pythonw.exe -m hairball_cli.main gateway run` straight out of
            # venv\Scripts\, so its image name is python/pythonw, not hairball.exe.
            ...
            # user (see hairball_cli/gateway_windows.py), so the installer always
    ...
    if (Test-Path $venvPythonExe) {
        ...
        # the task fires normally on next logon and `hairball update` / the
...
function Install-Dependencies {
    ...
    if (Test-Path "uv.lock") {
        ...
        # empty and producing the broken state where `hairball.exe` exists
...
"@ 2>$null
    ...
    if (-not $installed) {
        throw "Failed to install hairball-agent package even with no extras. Inspect the uv pip install output above."
    }
    ...
    # isn't set, leaving venv\ empty and hairball.exe broken with
    ...
    # here, not 30 seconds later when the user runs `hairball`.
    if (-not $NoVenv) {
    ...
    if (-not $NoVenv) {
        # uv on Windows can register hairball.exe in dist-info/RECORD but fail to
        ...
        # `hairball` command while hairball-agent.exe / hairball-acp.exe exist
...
"@ 2>$null
            if ($LASTEXITCODE -eq 0 -and $scriptNames) {
                ...
                if ($missing.Count -gt 0) {
                    ...
                    if ($stillMissing.Count -gt 0) {
                        ...
                        Write-Info "Workaround: `"$pythonExe`" -m hairball_cli.main <command>"
                    } else {
    ...
    # users hit and lazy-import errors from `hairball dashboard` are confusing.
    ...
    if (Test-Path $pythonExe) {
        ...
        try {
            & $pythonExe -m py_compile "$InstallDir\hairball_cli\web_server.py" 2>&1 | Out-Null
            if ($LASTEXITCODE -eq 0) { $webServerSyntaxOk = $true }
        } catch { }
        $ErrorActionPreference = $prevEAP
        if (-not $webOk) {
            Write-Warn "fastapi/uvicorn not importable -- `hairball dashboard` will not work."
            Write-Info "Attempting targeted install of [web] extra as last resort..."
            & $UvCmd pip install -e ".[web]"
            if ($LASTEXITCODE -eq 0) {
                Write-Success "[web] extra installed; `hairball dashboard` should now work."
            } else {
        ...
        if (-not $webServerSyntaxOk) {
            throw "dashboard backend source failed syntax check: hairball_cli/web_server.py"
        }
    }
...
function Set-PathVariable {
    Write-Info "Setting up hairball command..."
    
    if ($NoVenv) {
        $hairballBin = "$InstallDir"
    } else {
        $hairballBin = "$InstallDir\venv\Scripts"
    }
    
    # Add the venv Scripts dir to user PATH so hairball is globally available
    # On Windows, the hairball.exe in venv\Scripts\ has the venv Python baked in
    $currentPath = [Environment]::GetEnvironmentVariable("Path", "User")
    
    if ($currentPath -notlike "*$hairballBin*") {
        [Environment]::SetEnvironmentVariable(
            "Path",
            "$hairballBin;$currentPath",
            "User"
        )
        Write-Success "Added to user PATH: $hairballBin"
    } else {
        Write-Info "PATH already configured"
    }
    
    # Set HAIRBALL_HOME so the Python code finds config/data in the right place.
    # Only needed on Windows where we install to %LOCALAPPDATA%\hairball instead
    # of the Unix default ~/.hairball
    $currentHairballHome = [Environment]::GetEnvironmentVariable("HAIRBALL_HOME", "User")
    if (-not $currentHairballHome -or $currentHairballHome -ne $HairballHome) {
        [Environment]::SetEnvironmentVariable("HAIRBALL_HOME", $HairballHome, "User")
        Write-Success "Set HAIRBALL_HOME=$HairballHome"
    }
    $env:HAIRBALL_HOME = $HairballHome
    
    # Update current session
    $env:Path = "$hairballBin;$env:Path"
    
    Write-Success "hairball command ready"
}

function Write-BootstrapMarker {
    # Writes $InstallDir\.hairball-bootstrap-complete which tells the Hairball
    ...
    # Hairball-Setup.exe) or fall back to whatever git resolves in the
    # checkout. The desktop validates schemaVersion + pinnedCommit
    # length but doesn't enforce that HEAD matches the pin (users
    # update via `hairball update` which moves HEAD legitimately).
    ...
    $markerPath = Join-Path $InstallDir ".hairball-bootstrap-complete"
    $marker = [ordered]@{
        schemaVersion = 1
...
function Copy-ConfigTemplates {
    ...
    # Create the HAIRBALL_HOME directory structure ($HairballHome, default %LOCALAPPDATA%\hairball)
    New-Item -ItemType Directory -Force -Path "$HairballHome\cron" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HairballHome\sessions" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HairballHome\logs" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HairballHome\pairing" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HairballHome\hooks" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HairballHome\image_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HairballHome\audio_cache" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HairballHome\memories" | Out-Null
    New-Item -ItemType Directory -Force -Path "$HairballHome\skills" | Out-Null

    
    # Create .env
    $envPath = "$HairballHome\.env"
    ...
    # Create config.yaml
    $configPath = "$HairballHome\config.yaml"
    ...
    # (the default PS5 behaviour), and Hairball's prompt-injection scanner
    ...
    # PowerShell version.
    $soulPath = "$HairballHome\SOUL.md"
    if (-not (Test-Path $soulPath)) {
        # MUST match DEFAULT_SOUL_MD in hairball_cli/default_soul.py. The runtime
...
"@
    ...
    Write-Success "Configuration directory ready: $HairballHome"
    
    # Seed bundled skills into $HairballHome\skills (manifest-based, one-time per skill)
    Write-Info "Syncing bundled skills to $HairballHome\skills ..."
    $pythonExe = "$InstallDir\venv\Scripts\python.exe"
    if (Test-Path $pythonExe) {
        try {
            & $pythonExe "$InstallDir\tools\skills_sync.py" 2>$null
            Write-Success "Skills synced to $HairballHome\skills"
        } catch {
            # Fallback: simple directory copy
            $bundledSkills = "$InstallDir\skills"
            $userSkills = "$HairballHome\skills"
            if ((Test-Path $bundledSkills) -and -not (Get-ChildItem $userSkills -Exclude '.bundled_manifest' -ErrorAction SilentlyContinue)) {
                Copy-Item -Path "$bundledSkills\*" -Destination $userSkills -Recurse -Force -ErrorAction SilentlyContinue
                Write-Success "Skills copied to $HairballHome\skills"
            }
        }
    }
}
...
function Install-NodeDeps {
    if (-not $HasNode) {
        # Cross-process driver mode (Hairball-Setup.exe runs each -Stage NAME
    ...
    if (-not $npmCmd) {
        ...
        Write-Info "Open a new PowerShell window and re-run 'hairball setup tools' later."
        return
    }
    $npmExe = $npmCmd.Source
    if ($npmExe -like "*.ps1") {
    ...
    if (Test-Path "$InstallDir\package.json") {
        ...
        $browserLog = "$env:TEMP\hairball-npm-browser-$(Get-Random).log"
        ...
        if ($browserNpmOk) {
            ...
            if (-not $npxExe) {
                ...
                $pwLog = "$env:TEMP\hairball-playwright-install-$(Get-Random).log"
                Push-Location $InstallDir
                ...
                try {
                    ...
                    # download regularly takes 3-10 minutes on a fresh VM.
                    # Use the same process-tree timeout boundary as the
                    # agent-browser installer while relaying its log so a
                    # failed CDN cannot leave Setup blocked indefinitely.
                    ...
                    #
                    $ErrorActionPreference = "Continue"
                    $pwResult = Invoke-AgentBrowserInstallWithTimeout `
                        -AgentBrowserPath $npxExe `
                        -CommandArguments "--yes playwright install chromium" `
                        -LogPath $pwLog `
                        -TimeoutSeconds 600 `
                    ...
                    if ($pwCode -eq 0) {
                        ...
                        if ($pwResult.TimedOut) {
                            Write-Warn "Playwright Chromium install timed out after 600s; the installer process tree was stopped."
                        } else {
    ...
    if (Test-Path "$tuiDir\package.json") {
        ...
        $tuiLog = "$env:TEMP\hairball-npm-tui-$(Get-Random).log"
...
# the final `electron` -> `Hairball` rename dies with ENOENT and every re-run
...
function Install-Desktop {
    # Build apps/desktop into a launchable Hairball.exe. Only called from
    ...
    #
    # The Tauri bootstrap installer's launch_hairball_desktop command
    # resolves apps/desktop/release/win-unpacked/Hairball.exe directly,
    ...
    # enforces the build floor (^20.19 || >=22.12) and prepends the Hairball-managed
    ...
    # The --dir mode produces an unpacked Hairball.exe in
    ...
    # are NOT trying to work around). The Hairball icon + product name are
    # stamped onto Hairball.exe by our own rcedit step (Set-DesktopExeIdentity)
    ...
    $buildLog = "$env:TEMP\hairball-desktop-build-$(Get-Random).log"
    ...
    $exeCandidates = @(
        "$desktopDir\release\win-unpacked\Hairball.exe",
        "$desktopDir\release\win-arm64-unpacked\Hairball.exe"
    )
    $found = $false
    $desktopExe = $null
    ...
    if (-not $found) {
        throw "Desktop build completed but no Hairball.exe was found under $desktopDir\release\*-unpacked\"
    }

    # 3b. The Hairball icon + identity are stamped onto Hairball.exe by the
    ...
    #    Hairball.exe. We deliberately do NOT point them at `hairball desktop`: that
...
function New-DesktopShortcuts {
    ...
    try {
        ...
        $targets = @(
            (Join-Path ([Environment]::GetFolderPath('Programs')) 'Hairball.lnk'),
            (Join-Path ([Environment]::GetFolderPath('Desktop')) 'Hairball.lnk')
        )
        ...
        # with the Hairball icon, but without this the shortcut can keep drawing
...
function Install-PlatformSdks {
    ...
    # ~/.hairball/.env are importable.  Two problems this solves:
    #
...
function Invoke-SetupWizard {
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Skipping setup wizard (non-interactive). Configure via the GUI or 'hairball setup'."
        return
    }
    ...
    if (-not $NoVenv) {
        & ".\venv\Scripts\python.exe" -m hairball_cli.main setup
    } else {
        python -m hairball_cli.main setup
    }
...
function Start-GatewayIfConfigured {
    ...
    $hairballCmd = "$InstallDir\venv\Scripts\hairball.exe"
    if (-not (Test-Path $hairballCmd)) {
        $hairballCmd = "hairball"
    }

    # If WhatsApp is enabled but not yet paired, run foreground for QR scan
    $whatsappEnabled = $content | Where-Object { $_ -match "^WHATSAPP_ENABLED=true" }
    $whatsappSession = "$HairballHome\whatsapp\session\creds.json"
    if ($whatsappEnabled -and -not (Test-Path $whatsappSession)) {
        Write-Host ""
        Write-Info "WhatsApp is enabled but not yet paired."
        Write-Info "Running 'hairball whatsapp' to pair via QR code..."
        Write-Host ""
    ...
    if ($NonInteractive) {
        ...
        Write-Info "Start the gateway later with: hairball gateway"
        return
    }
    ...
    if ($response -eq "" -or $response -match "^[Yy]") {
        ...
        try {
            $logFile = "$HairballHome\logs\gateway.log"
            Start-Process -FilePath $hairballCmd -ArgumentList "gateway" `
                -RedirectStandardOutput $logFile `
                -RedirectStandardError "$HairballHome\logs\gateway-error.log" `
                -WindowStyle Hidden
            Write-Success "Gateway started! Your bot is now online."
            Write-Info "Logs: $logFile"
            Write-Info "To stop: close the gateway process from Task Manager"
        } catch {
            Write-Warn "Failed to start gateway. Run manually: hairball gateway"
        }
    } else {
        Write-Info "Skipped. Start the gateway later with: hairball gateway"
    }
}
...
function Write-Completion {
    ...
    Write-Host "$HairballHome\config.yaml"
    Write-Host "   API Keys:  " -NoNewline -ForegroundColor Yellow
    Write-Host "$HairballHome\.env"
    Write-Host "   Data:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HairballHome\cron\, sessions\, logs\"
    Write-Host "   Code:      " -NoNewline -ForegroundColor Yellow
    Write-Host "$HairballHome\hairball-agent\"
    Write-Host ""
    ...
    Write-Host ""
    Write-Host "   hairball              " -NoNewline -ForegroundColor Green
    Write-Host "Start chatting"
    Write-Host "   hairball setup        " -NoNewline -ForegroundColor Green
    Write-Host "Configure API keys & settings"
    Write-Host "   hairball config       " -NoNewline -ForegroundColor Green
    Write-Host "View/edit configuration"
    Write-Host "   hairball config edit  " -NoNewline -ForegroundColor Green
    Write-Host "Open config in editor"
    Write-Host "   hairball gateway      " -NoNewline -ForegroundColor Green
    Write-Host "Start messaging gateway (Telegram, Discord, etc.)"
    Write-Host "   hairball update       " -NoNewline -ForegroundColor Green
...
$InstallStages = @(
    ...
    @{ Name = "repository";       Title = "Cloning Hairball repository";            Category = "install";      NeedsUserInput = $false; Worker = "Stage-Repository" }
...
if ($IncludeDesktop) {
    ...
    # (Hairball-Setup.exe), never via the irm|iex CLI one-liner.
    $InstallStages += @{ Name = "desktop"; Title = "Building desktop app"; Category = "install"; NeedsUserInput = $false; Worker = "Stage-Desktop" }
}
$InstallStages += @(
    @{ Name = "path";             Title = "Adding Hairball to PATH";                Category = "finalize";     NeedsUserInput = $false; Worker = "Stage-Path" }
...
function Invoke-EnsureMode {
    ...
    foreach ($dep in $depList) {
        ...
        switch ($dep) {
            "python" {
                if (-not (Install-Uv)) {
                    Write-Err "uv could not be installed"
                    exit 1
                }
                Resolve-UvCmd
                if (-not (Test-Python)) {
                    Write-Err "Python $PythonVersion could not be installed"
                    exit 1
                }
            }
            "node" {
...
try {
    ...
    Write-Host "  Invoke-WebRequest -Uri 'https://raw.githubusercontent.com/hairball-agent/hairball-agent/main/scripts/install.ps1' -OutFile install.ps1" -ForegroundColor Yellow
