#!/bin/bash
# ============================================================================
# Hairball Agent Installer
# ============================================================================
# Installation script for Linux, macOS, and Android/Termux.
# Uses uv for desktop/server installs and Python's stdlib venv + pip on Termux.
#
# Usage:
#   curl -fsSL https://raw.githubusercontent.com/hairball-agent/hairball-agent/main/scripts/install.sh | bash
#
# Or with options:
...
# Python-driven tool session (e.g. Hairball terminal tool). A pre-set PYTHONPATH
...
# Configuration
UPDATE_REPOSITORY="${HAIRBALL_UPDATE_REPO:-hairball-agent/hairball-agent}"
_UPDATE_REPOSITORY_RE='^[A-Za-z0-9]([A-Za-z0-9_.-]*[A-Za-z0-9])?/[A-Za-z0-9]([A-Za-z0-9_.-]*[A-Za-z0-9])?$'
if [[ ! "$UPDATE_REPOSITORY" =~ $_UPDATE_REPOSITORY_RE ]]; then
    echo "HAIRBALL_UPDATE_REPO must use owner/repository form" >&2
    exit 2
fi
unset _UPDATE_REPOSITORY_RE
REPO_URL_SSH="git@github.com:${UPDATE_REPOSITORY}.git"
REPO_URL_HTTPS="https://github.com/${UPDATE_REPOSITORY}.git"
HAIRBALL_HOME="${HAIRBALL_HOME:-$HOME/.hairball}"
# INSTALL_DIR is resolved AFTER arg parsing and OS detection so we can pick an
# FHS-style layout for root installs.  Track whether the user gave us an
# explicit directory — if so we never override it.
if [ -n "${HAIRBALL_INSTALL_DIR:-}" ]; then
    INSTALL_DIR="$HAIRBALL_INSTALL_DIR"
...
#   code at /usr/local/lib/hairball-agent, command at /usr/local/bin/hairball,
#   data still at /root/.hairball (HAIRBALL_HOME).  Matches Claude Code / Codex CLI
...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo "                   write \$HAIRBALL_HOME/.no-bundled-skills so future"
            echo "                   'hairball update' runs never inject bundled skills either"
            ...
            echo "  --include-desktop  Also build the desktop app (apps/desktop -> Hairball.app)"
            echo "  --dir PATH     Installation directory"
            echo "                   default (non-root):  ~/.hairball/hairball-agent"
            echo "                   default (root, Linux): /usr/local/lib/hairball-agent"
            echo "  --hairball-home PATH  Data directory (default: ~/.hairball, or \$HAIRBALL_HOME)"
            echo "  -h, --help     Show this help"
            echo ""
            echo "Notes:"
            echo "  When running as root on Linux, Hairball installs the code under"
            echo "  /usr/local/lib/hairball-agent and links the command into"
            echo "  /usr/local/bin/hairball (FHS layout — matches Claude Code / Codex CLI)."
            echo "  Data, config, sessions, and logs still live in \$HAIRBALL_HOME"
            echo "  (default /root/.hairball).  This keeps Docker bind-mounted volumes"
            echo "  small and ensures the command is on PATH for all shells."
            echo "  Existing installs at \$HAIRBALL_HOME/hairball-agent are preserved in-place."
            echo "  --ensure DEPS  Install only specified deps: python,node,browser,ripgrep,ffmpeg"
            echo "                   Supported: python, node, browser, ripgrep, ffmpeg"
            echo "                   Does NOT clone repo or create venv"
            echo "  --postinstall  Run post-install setup only (for pip users)"
            echo "                   Installs optional deps + runs hairball setup"
            echo "                   Does NOT clone repo or create venv"
            exit 0
            ;;
        *)
...
# intentional, but they leave the checkout dirty — which forces `hairball update`
...
emit_manifest() {
    ...
    # install.ps1: the signed bootstrap installer (Hairball-Setup) passes it so
    ...
    printf '%s' '{"protocol_version":1,"stages":[{"name":"prerequisites","title":"System prerequisites","category":"runtime","needs_user_input":false},{"name":"repository","title":"Download Hairball Agent","category":"runtime","needs_user_input":false},{"name":"venv","title":"Create Python virtual environment","category":"runtime","needs_user_input":false},{"name":"python-deps","title":"Install Python dependencies","category":"runtime","needs_user_input":false},{"name":"node-deps","title":"Install browser-tool dependencies","category":"runtime","needs_user_input":false},{"name":"path","title":"Install hairball command","category":"runtime","needs_user_input":false},{"name":"config","title":"Prepare config and skills","category":"configuration","needs_user_input":false},{"name":"setup","title":"Configure API keys and settings","category":"configuration","needs_user_input":true},{"name":"gateway","title":"Configure gateway service","category":"configuration","needs_user_input":true},'"$desktop_stage"'{"name":"complete","title":"Finish install","category":"runtime","needs_user_input":false}]}'
    printf '\n'
}
...
# Decide where the repo checkout + venv live, and where the `hairball` command
# symlink goes.  Called after detect_os so $OS/$DISTRO are known.
#
# Defaults:
#   - Non-root, any OS:       INSTALL_DIR = $HAIRBALL_HOME/hairball-agent
#                             command link in $HOME/.local/bin
#   - Termux (any uid):       INSTALL_DIR = $HAIRBALL_HOME/hairball-agent
#                             command link in $PREFIX/bin (already on PATH)
#   - Root on Linux (new):    INSTALL_DIR = /usr/local/lib/hairball-agent
#                             command link in /usr/local/bin
#                             (unless a legacy install already exists at
#                              $HAIRBALL_HOME/hairball-agent — then preserve it)
#
# Always no-op when the user set --dir or $HAIRBALL_INSTALL_DIR.
resolve_install_layout() {
    ...
    # Termux: package manager manages /data/data/..., keep code in HAIRBALL_HOME.
    if is_termux; then
        INSTALL_DIR="$HAIRBALL_HOME/hairball-agent"
        return 0
    fi
    ...
    if [ "$OS" = "linux" ] && [ "$(id -u)" -eq 0 ]; then
        if [ -d "$HAIRBALL_HOME/hairball-agent/.git" ]; then
            INSTALL_DIR="$HAIRBALL_HOME/hairball-agent"
            log_info "Existing install detected at $INSTALL_DIR — keeping legacy layout"
            log_info "  (new root installs use /usr/local/lib/hairball-agent)"
            return 0
        fi
        INSTALL_DIR="/usr/local/lib/hairball-agent"
        ...
        # /usr/local/bin/hairball wrapper unable to exec the bad-interpreter venv
        # python.  See #21457.
        ...
        log_info "  Command: /usr/local/bin/hairball"
        log_info "  Data:    $HAIRBALL_HOME (unchanged)"
...
# Point a Hairball-managed Node's `npm install -g` at a directory that is on
# PATH. npm's default global prefix for a bundled Node is the Node dir itself,
# so global package binaries land in $HAIRBALL_HOME/node/bin — which is NOT on
...
# Hairball's own global installs pass an explicit --prefix and are unaffected.
# Idempotent and a no-op when there is no Hairball-managed npm, so calling it on
# every install run repairs pre-existing installs, not just fresh ones.
configure_managed_node_npm_prefix() {
    [ -x "$HAIRBALL_HOME/node/bin/npm" ] || return 0
    local link_dir
    link_dir="$(get_command_link_dir)"
    mkdir -p "$HAIRBALL_HOME/node/etc"
    printf 'prefix=%s\n' "$(dirname "$link_dir")" > "$HAIRBALL_HOME/node/etc/npmrc"
}

get_hairball_command_path() {
    local link_dir
    link_dir="$(get_command_link_dir)"
    if [ -x "$link_dir/hairball" ]; then
        echo "$link_dir/hairball"
    else
        echo "hairball"
    fi
}
...
detect_os() {
    case "$(uname -s)" in
        ...
        CYGWIN*|MINGW*|MSYS*)
            ...
            log_info "  iex (irm https://raw.githubusercontent.com/hairball-agent/hairball-agent/main/scripts/install.ps1)"
            exit 1
            ;;
        *)
            OS="unknown"
...
install_uv() {
    ...
    # Hairball owns its own uv at $HAIRBALL_HOME/bin/uv.  Always install there —
    # no PATH probing, no conda guards, no multi-location resolution chains.
    # The runtime update path (hairball_cli/managed_uv.py) looks in the same
    # place, so install.sh and `hairball update` stay in sync.
    local _managed_uv="$HAIRBALL_HOME/bin/uv"
    ...
    log_info "Installing managed uv into $HAIRBALL_HOME/bin ..."
    mkdir -p "$HAIRBALL_HOME/bin"

    # Two-stage: download the installer, then run it.  Piping
    # `curl | sh` masks curl failures (sh exits 0 on empty stdin)
    # and conflates network errors with installer errors.
    local _uv_install_log _uv_installer
    _uv_install_log="$(mktemp 2>/dev/null || echo "/tmp/hairball-uv-install.$$.log")"
    _uv_installer="$(mktemp 2>/dev/null || echo "/tmp/hairball-uv-installer.$$.sh")"
    ...
    # directly into $HAIRBALL_HOME/bin instead of ~/.local/bin.
    if UV_UNMANAGED_INSTALL="$HAIRBALL_HOME/bin" sh "$_uv_installer" >>"$_uv_install_log" 2>&1; then
        rm -f "$_uv_installer"
        if [ -x "$_managed_uv" ]; then
            UV_CMD="$_managed_uv"
        else
...
# string clears that floor; anything below it is replaced with the Hairball-
...
check_node() {
    ...
    # Repair pre-existing Hairball-managed installs where `npm install -g` lands
    ...
    # Prefer a Hairball-managed Node from a previous run over a too-old system one.
    if [ -x "$HAIRBALL_HOME/node/bin/node" ] && node_satisfies_build "$("$HAIRBALL_HOME/node/bin/node" --version)"; then
        export PATH="$HAIRBALL_HOME/node/bin:$PATH"
        log_success "Node.js $("$HAIRBALL_HOME/node/bin/node" --version) found (Hairball-managed)"
        HAS_NODE=true
        return 0
    fi

    if command -v node &> /dev/null; then
        log_warn "Node.js $(node --version) is too old for the desktop build (need ^20.19 or >=22.12) — installing Hairball-managed Node $NODE_VERSION LTS..."
...
install_node() {
    ...
    log_info "Extracting to ~/.hairball/node/..."
    ...
    # Place into ~/.hairball/node/ and symlink binaries into the same bin dir
    # the hairball command uses (get_command_link_dir): /usr/local/bin for root
    # FHS installs, $PREFIX/bin on Termux, ~/.local/bin otherwise.
    rm -rf "$HAIRBALL_HOME/node"
    mkdir -p "$HAIRBALL_HOME"
    mv "$extracted_dir" "$HAIRBALL_HOME/node"
    rm -rf "$tmp_dir"

    local node_link_dir
    node_link_dir="$(get_command_link_dir)"
    mkdir -p "$node_link_dir"
    ln -sf "$HAIRBALL_HOME/node/bin/node" "$node_link_dir/node"
    ln -sf "$HAIRBALL_HOME/node/bin/npm"  "$node_link_dir/npm"
    ln -sf "$HAIRBALL_HOME/node/bin/npx"  "$node_link_dir/npx"
    ...
    local installed_ver
    installed_ver=$("$HAIRBALL_HOME/node/bin/node" --version 2>/dev/null)
    log_success "Node.js $installed_ver installed to ~/.hairball/node/"
    HAS_NODE=true
}
...
check_network_prerequisites() {
    ...
    if [ "$DISTRO" = "termux" ]; then
        ...
        log_warn "Network checks failed. Hairball install may complete, but web search and dependency downloads can fail."
    ...
    if [ -n "$pkg_install" ]; then
        ...
        # sudo needs password — ask once for everything
            ...
            if [ "$IS_INTERACTIVE" = true ]; then
                ...
                log_info "Hairball Agent itself does not require or retain root access."
    ...
    if [ -d "$INSTALL_DIR" ]; then
        if [ -d "$INSTALL_DIR/.git" ]; then
            ...
            if [ -n "$(git status --porcelain)" ]; then
                ...
                # index-level conflict state. Mirrors the `hairball update` path
                # (#4735).
                ...
                local stash_name
                stash_name="hairball-install-autostash-$(date -u +%Y%m%d-%H%M%S)"
            ...
            # cannot succeed — mirror ``hairball update`` and reset to the
            ...
            if [ -n "$autostash_ref" ]; then
                ...
                if [ "$restore_now" = "yes" ]; then
                    ...
                    if git stash apply "$autostash_ref"; then
                        ...
                        log_warn "Review git diff / git status if Hairball behaves unexpectedly."
                    else
...
try:
    ...
    extras = []
    for s in specs:
        m = re.search(r"hairball-agent\[([\w-]+)\]", s)
        if m:
...
setup_path() {
    log_info "Setting up hairball command..."

    if [ "$USE_VENV" = true ]; then
        HAIRBALL_BIN="$INSTALL_DIR/venv/bin/hairball"
    else
        HAIRBALL_BIN="$(which hairball 2>/dev/null || echo "")"
        if [ -z "$HAIRBALL_BIN" ]; then
            log_warn "hairball not found on PATH after install"
            return 0
        fi
    fi

    # Verify the entry point script was actually generated
    if [ ! -x "$HAIRBALL_BIN" ]; then
        log_warn "hairball entry point not found at $HAIRBALL_BIN"
    ...
    # Create a user-facing shim for the hairball command.
    # We intentionally clear PYTHONPATH/PYTHONHOME here so inherited env vars
    # can't make this launcher import modules from another checkout.
    mkdir -p "$command_link_dir"
    # Older installs created this path as a symlink to $HAIRBALL_BIN. Without
    # the rm, `cat >` follows the symlink and overwrites the venv pip entry
    # point with this shim — making `exec "$HAIRBALL_BIN"` self-recurse. (#21454)
    rm -f "$command_link_dir/hairball"
    cat > "$command_link_dir/hairball" <<EOF
#!/usr/bin/env bash
unset PYTHONPATH
unset PYTHONHOME
exec "$HAIRBALL_BIN" "\$@"
EOF
    chmod +x "$command_link_dir/hairball"
    log_success "Installed hairball launcher → $command_link_display_dir/hairball"

    if [ "$DISTRO" = "termux" ]; then
        export PATH="$command_link_dir:$PATH"
        log_info "$command_link_display_dir is the native Termux command path"
        log_success "hairball command ready"
        return 0
    fi
    ...
    if [ "$ROOT_FHS_LAYOUT" = true ]; then
        ...
        if env -i HOME="$HOME" TERM="${TERM:-dumb}" bash -i -c 'command -v hairball' \
                >/dev/null 2>&1; then
        ...
        log_info "hairball not on PATH in non-login shells (common on RHEL-family)"
        PATH_LINE='export PATH="/usr/local/bin:$PATH"'
        PATH_COMMENT='# Hairball Agent — ensure /usr/local/bin is on PATH (RHEL non-login shells)'
    ...
    if ! echo "$PATH" | tr ':' '\n' | grep -q "^$command_link_dir$"; then
        ...
        for SHELL_CONFIG in "${SHELL_CONFIGS[@]}"; do
            if ! grep -v '^[[:space:]]*#' "$SHELL_CONFIG" 2>/dev/null | grep -qE 'PATH=.*\.local/bin'; then
                echo "" >> "$SHELL_CONFIG"
                echo "# Hairball Agent — ensure ~/.local/bin is on PATH" >> "$SHELL_CONFIG"
                echo "$PATH_LINE" >> "$SHELL_CONFIG"
        ...
        if [ "$IS_FISH" = "true" ]; then
            if ! grep -q 'fish_add_path.*\.local/bin' "$FISH_CONFIG" 2>/dev/null; then
                echo "" >> "$FISH_CONFIG"
                echo "# Hairball Agent — ensure ~/.local/bin is on PATH" >> "$FISH_CONFIG"
    ...
    # Export for current session so hairball works immediately
    export PATH="$command_link_dir:$PATH"
...
copy_config_templates() {
    ...
    # Create ~/.hairball directory structure (config at top level, code in subdir)
    mkdir -p "$HAIRBALL_HOME"/{cron,sessions,logs,pairing,hooks,image_cache,audio_cache,memories,skills}

    # Create .env at ~/.hairball/.env (top level, easy to find)
    if [ ! -f "$HAIRBALL_HOME/.env" ]; then
        if [ -f "$INSTALL_DIR/.env.example" ]; then
            cp "$INSTALL_DIR/.env.example" "$HAIRBALL_HOME/.env"
            log_success "Created ~/.hairball/.env from template"
        else
            touch "$HAIRBALL_HOME/.env"
            log_success "Created ~/.hairball/.env"
        fi
    else
        log_info "~/.hairball/.env already exists, keeping it"
    fi
    ...
    chmod 600 "$HAIRBALL_HOME/.env"
    configure_browser_env_from_system_browser

    # Create config.yaml at ~/.hairball/config.yaml (top level, easy to find)
    if [ ! -f "$HAIRBALL_HOME/config.yaml" ]; then
        if [ -f "$INSTALL_DIR/cli-config.yaml.example" ]; then
            cp "$INSTALL_DIR/cli-config.yaml.example" "$HAIRBALL_HOME/config.yaml"
            log_success "Created ~/.hairball/config.yaml from template"
        fi
    else
        log_info "~/.hairball/config.yaml already exists, keeping it"
    fi

    # Create SOUL.md if it doesn't exist (global persona file).
    # This MUST match DEFAULT_SOUL_MD in hairball_cli/default_soul.py — the
    ...
    if [ ! -f "$HAIRBALL_HOME/SOUL.md" ]; then
        cat > "$HAIRBALL_HOME/SOUL.md" << 'SOUL_EOF'
You are Hairball Agent, an intelligent AI assistant created by Nous Research. You are helpful, knowledgeable, and direct. You assist users with a wide range of tasks including answering questions, writing and editing code, analyzing information, creative work, and executing actions via your tools. You communicate clearly, admit uncertainty when appropriate, and prioritize being genuinely useful over being verbose unless otherwise directed below. Be targeted and efficient in your exploration and investigations.
SOUL_EOF
        log_success "Created ~/.hairball/SOUL.md (edit to customize personality)"
    fi

    log_success "Configuration directory ready: ~/.hairball/"

    # Seed bundled skills into ~/.hairball/skills/ (manifest-based, one-time per skill)
    if [ "$NO_SKILLS" = true ]; then
        # Blank-slate install: write the opt-out marker and skip seeding.
        # skills_sync.py and `hairball update` both honor this marker, so the
        ...
        printf '%s\n' \
            ...
            "Delete this file to re-enable sync on the next 'hairball update'." \
            > "$HAIRBALL_HOME/.no-bundled-skills" 2>/dev/null || true
        log_info "Skipping bundled skills (--no-skills). Wrote $HAIRBALL_HOME/.no-bundled-skills"
        log_info "  Future 'hairball update' runs will not inject bundled skills. Delete the marker to opt back in."
    else
        log_info "Syncing bundled skills to ~/.hairball/skills/ ..."
        if "$INSTALL_DIR/venv/bin/python" "$INSTALL_DIR/tools/skills_sync.py" 2>/dev/null; then
            log_success "Skills synced to ~/.hairball/skills/"
        else
            # Fallback: simple directory copy if Python sync fails
            if [ -d "$INSTALL_DIR/skills" ] && [ ! "$(ls -A "$HAIRBALL_HOME/skills/" 2>/dev/null | grep -v '.bundled_manifest')" ]; then
                cp -r "$INSTALL_DIR/skills/"* "$HAIRBALL_HOME/skills/" 2>/dev/null || true
                log_success "Skills copied to ~/.hairball/skills/"
            fi
        fi
    fi
}
...
strip_snap_browser_override() {
    ...
    local env_file="$HAIRBALL_HOME/.env"
    ...
    local tmp
    tmp="$(mktemp)" || return 0
    if grep -Ev '^AGENT_BROWSER_EXECUTABLE_PATH=/snap/|^# Hairball Agent browser tools' "$env_file" > "$tmp"; then
        mv "$tmp" "$env_file"
        log_warn "Removed stale Snap browser override (AGENT_BROWSER_EXECUTABLE_PATH=/snap/...) from $env_file"
        log_info "Hairball will use the bundled Chromium instead."
...
_timeout_process_is_running() {
    local cmd_pid="$1"
    local process_state=""

    kill -0 "$cmd_pid" 2>/dev/null || return 1

    # ``kill -0`` alone is not a completion probe on every supported shell.
    # In macOS' Bash 3.2 a completed child can remain as a zombie until
    # ``wait`` consumes it, and both kill -0 and jobs -p still report it.
    # ``ps`` is available on every supported desktop/server platform and lets
    # us distinguish that completed state.  If a minimal ps implementation
    # cannot expose STAT, retain the conservative kill -0 result.
    if command -v ps >/dev/null 2>&1 \
        && process_state="$(ps -o stat= -p "$cmd_pid" 2>/dev/null)"; then
...
#
# $1 (timeout) must be a bare integer number of seconds (the `timeout` binary
# would also accept suffixes like 15m, but we normalize so both paths share one
# contract). On timeout the return code is 124, matching GNU `timeout`.
run_with_timeout() {
    ...
    local old_int_trap old_term_trap timeout_marker watchdog_pid
    timeout_marker="$(mktemp)" || return 1
    old_int_trap="$(trap -p INT || true)"
    old_term_trap="$(trap -p TERM || true)"

    # A separate watchdog avoids one-second polling races on macOS Bash 3.2:
    # the main shell waits for the command immediately, while the watchdog
    # owns the deadline and records timeout intent before terminating the
    # isolated process group. A fast command therefore returns as soon as it
    # finishes, even on a heavily loaded host.
    set -m
    (
        # Handshake before sleeping so the parent never tries to kill a
        # watchdog process group that Bash has not finished creating yet.
        printf 'ready\n' >"$timeout_marker"
        sleep "$timeout_seconds"
        if _timeout_process_is_running "$cmd_pid"; then
            printf 'timeout\n' >"$timeout_marker"
            _stop_timeout_process_group "$cmd_pid"
        fi
    ) &
    watchdog_pid=$!
    set +m

    local watchdog_ready_wait=0
    while [ ! -s "$timeout_marker" ] && [ "$watchdog_ready_wait" -lt 200 ]; do
        sleep 0.01
        watchdog_ready_wait=$((watchdog_ready_wait + 1))
    done
    ...
    trap 'interrupted_rc=130; _stop_timeout_process_group "$cmd_pid"' INT
    trap 'interrupted_rc=143; _stop_timeout_process_group "$cmd_pid"' TERM

    # `|| rc=$?` preserves the child's status without letting `set -e` abort.
    wait "$cmd_pid" 2>/dev/null || rc=$?

    if grep -qx 'timeout' "$timeout_marker" 2>/dev/null; then
        wait "$watchdog_pid" 2>/dev/null || true
        rc=124
    else
        # The watchdog owns a process group so its child `sleep` cannot keep
        # captured installer pipes open after a fast command completes.
        kill -TERM "-$watchdog_pid" 2>/dev/null \
            || kill -TERM "$watchdog_pid" 2>/dev/null \
            || true
        wait "$watchdog_pid" 2>/dev/null || true
...
configure_browser_env_from_system_browser() {
    ...
    {
        echo ""
        echo "# Hairball Agent browser tools — explicit browser override."
...
install_node_deps() {
    ...
    if [ -f "$INSTALL_DIR/ui-tui/package.json" ]; then
        ...
        run_with_timeout "$NODE_DEPS_TIMEOUT" npm install --silent || {
            log_warn "TUI npm install failed or timed out (hairball --tui may not work)"
        }
        log_success "TUI dependencies installed"
    fi

    # Keep the checkout clean so `hairball update` doesn't autostash every run.
...
run_setup_wizard() {
    ...
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Setup wizard skipped (no terminal available). Run 'hairball setup' after install."
        return 0
    fi
    ...
    # Run hairball setup using the venv Python directly (no activation needed).
    # Redirect stdin from /dev/tty so interactive prompts work when piped from curl.
    if [ "$USE_VENV" = true ]; then
        "$INSTALL_DIR/venv/bin/python" -m hairball_cli.main setup < /dev/tty
    else
        python -m hairball_cli.main setup < /dev/tty
    fi
}

maybe_start_gateway() {
    # Check if any messaging platform tokens were configured
    ENV_FILE="$HAIRBALL_HOME/.env"
    if [ ! -f "$ENV_FILE" ]; then
        return 0
    fi
    ...
    log_info "The gateway needs to be running for Hairball to send/receive messages."

    # If WhatsApp is enabled and no session exists yet, run foreground first for QR scan
    WHATSAPP_VAL=$(grep "^WHATSAPP_ENABLED=" "$ENV_FILE" 2>/dev/null | cut -d'=' -f2-)
    WHATSAPP_SESSION="$HAIRBALL_HOME/whatsapp/session/creds.json"
    if [ "$WHATSAPP_VAL" = "true" ] && [ ! -f "$WHATSAPP_SESSION" ]; then
        if [ "$IS_INTERACTIVE" = true ]; then
            echo ""
            log_info "WhatsApp is enabled but not yet paired."
            log_info "Running 'hairball whatsapp' to pair via QR code..."
            echo ""
            if prompt_yes_no "Pair WhatsApp now?" "yes"; then
                HAIRBALL_CMD="$(get_hairball_command_path)"
                $HAIRBALL_CMD whatsapp || true
            fi
        else
            log_info "WhatsApp pairing skipped (non-interactive). Run 'hairball whatsapp' to pair."
        fi
    fi
    ...
    # but opening fails with ENXIO. See #16746.
    if ! (: </dev/tty) 2>/dev/null; then
        log_info "Gateway setup skipped (no terminal available). Run 'hairball gateway install' later."
        return 0
    fi
    ...
    if [ "$should_install_gateway" = true ]; then
        ...
        if [ "$DISTRO" != "termux" ] && command -v systemctl &> /dev/null; then
            ...
            if $HAIRBALL_CMD gateway install 2>/dev/null; then
                log_success "Gateway service installed"
                if $HAIRBALL_CMD gateway start 2>/dev/null; then
                    log_success "Gateway started! Your bot is now online."
                else
                    log_warn "Service installed but failed to start. Try: hairball gateway start"
                fi
            else
                log_warn "Systemd install failed. You can start manually: hairball gateway"
            fi
        else
            if [ "$DISTRO" = "termux" ]; then
            ...
            nohup $HAIRBALL_CMD gateway > "$HAIRBALL_HOME/logs/gateway.log" 2>&1 &
            GATEWAY_PID=$!
            log_success "Gateway started (PID $GATEWAY_PID). Logs: ~/.hairball/logs/gateway.log"
            log_info "To stop: kill $GATEWAY_PID"
            log_info "To restart later: hairball gateway"
            if [ "$DISTRO" = "termux" ]; then
                log_warn "Android may stop background processes when Termux is suspended or the system reclaims resources."
            fi
        fi
    else
        log_info "Skipped. Start the gateway later with: hairball gateway"
    fi
}
...
print_success() {
    ...
    echo ""
    echo -e "   ${YELLOW}Config:${NC}    $HAIRBALL_HOME/config.yaml"
    echo -e "   ${YELLOW}API Keys:${NC}  $HAIRBALL_HOME/.env"
    echo -e "   ${YELLOW}Data:${NC}      $HAIRBALL_HOME/cron/, sessions/, logs/"
    ...
    echo ""
    echo -e "   ${GREEN}hairball${NC}              Start chatting"
    echo -e "   ${GREEN}hairball setup${NC}        Configure API keys & settings"
    echo -e "   ${GREEN}hairball config${NC}       View/edit configuration"
    echo -e "   ${GREEN}hairball config edit${NC}  Open config in editor"
    echo -e "   ${GREEN}hairball gateway install${NC} Install gateway service (messaging + cron)"
    echo -e "   ${GREEN}hairball update${NC}       Update to latest version"
    echo ""

    echo -e "${CYAN}─────────────────────────────────────────────────────────${NC}"
    echo ""
    if [ "$DISTRO" = "termux" ]; then
        echo -e "${YELLOW}⚡ 'hairball' was linked into $(get_command_link_display_dir), which is already on PATH in Termux.${NC}"
        echo ""
    elif [ "$ROOT_FHS_LAYOUT" = true ]; then
        echo -e "${YELLOW}⚡ 'hairball' was linked into /usr/local/bin and is ready to use — no shell reload needed.${NC}"
        echo ""
    else
        echo -e "${YELLOW}⚡ Reload your shell to use 'hairball' command:${NC}"
        echo ""
        LOGIN_SHELL="$(basename "${SHELL:-/bin/bash}")"
...
ensure_browser() {
    if ! command -v node >/dev/null 2>&1; then
        local node_bin="$HAIRBALL_HOME/node/bin/node"
        if [ -x "$node_bin" ]; then
            export PATH="$HAIRBALL_HOME/node/bin:$PATH"
        else
    ...
    local npm_bin
    npm_bin="$(command -v npm 2>/dev/null || echo "$HAIRBALL_HOME/node/bin/npm")"
    if [ ! -x "$npm_bin" ]; then
    ...
    if ! run_with_timeout "$NODE_DEPS_TIMEOUT" "$npm_bin" install -g --prefix "$HAIRBALL_HOME/node" --silent --ignore-scripts \
    ...
    local ab_bin="$HAIRBALL_HOME/node/bin/agent-browser"
    if [ -x "$ab_bin" ]; then
        local ab_log ab_rc
        ab_log="$(mktemp)"
        ab_rc=0
        run_browser_install_with_timeout "$NODE_DEPS_TIMEOUT" "$ab_bin" install \
            >"$ab_log" 2>&1 || ab_rc=$?
        if [ "$ab_rc" -ne 0 ]; then
            if [ "$ab_rc" -eq 124 ]; then
                log_warn "Chromium install timed out after ${NODE_DEPS_TIMEOUT}s; the installer process tree was stopped."
            else
                log_warn "Chromium install failed (exit $ab_rc). Browser tools may not work without a system browser."
            fi
            if [ -s "$ab_log" ]; then
                log_info "Last Chromium installer output:"
                tail -n 20 "$ab_log" >&2
            fi
...
postinstall_mode() {
    ...
    log_info "Post-install mode: setting up Hairball for pip install"
    ...
    HAIRBALL_CMD="$(command -v hairball 2>/dev/null || echo "")"
    if [ -n "$HAIRBALL_CMD" ]; then
        log_info "Running hairball setup..."
        "$HAIRBALL_CMD" setup
    else
        log_warn "hairball command not found on PATH"
        log_info "Try: python -m hairball_cli.main setup"
    fi
}
...
# extract a tree MISSING the electron binary, so the `electron`->`Hairball` rename
...
# _purge_electron_build_cache() used by `hairball desktop`; install.sh was the only
...
install_desktop() {
    ...
    # floor (^20.19 || >=22.12) and prepends the Hairball-managed Node to PATH, so
    ...
    #         retry (matches install.ps1 / `hairball desktop`),
    ...
    local app=""
    if [ "$OS" = "linux" ]; then
        if [ -x "$desktop_dir/release/linux-unpacked/Hairball" ]; then
            app="$desktop_dir/release/linux-unpacked/Hairball"
        elif [ -x "$desktop_dir/release/linux-unpacked/hairball" ]; then
            app="$desktop_dir/release/linux-unpacked/hairball"
        fi
    else
        local cand
        for cand in \
            "$desktop_dir/release/mac-arm64/Hairball.app" \
            "$desktop_dir/release/mac/Hairball.app"; do
            if [ -d "$cand" ]; then
                app="$cand"
                break
            fi
    ...
    # trips Gatekeeper's tamper check ("Hairball is damaged and can't be opened").
    ...
    # checkout stays clean for the next `hairball update`.
    restore_dirty_lockfiles "$INSTALL_DIR"
}
...
run_stage_body() {
    ...
    case "$stage" in
        ...
        desktop)
            ...
            resolve_install_layout
            require_install_dir
            # Each stage runs in its own process, so the Hairball-managed Node
            # provisioned during prerequisites/node-deps (at $HAIRBALL_HOME/node/bin)
        ...
        complete)
            ...
            # $HAIRBALL_HOME. $HAIRBALL_HOME is a shared data dir (it can be
            # bind-mounted into a Docker gateway too), so a stamp there gets
            # clobbered by the container's 'docker' stamp and wrongly blocks
            # 'hairball update' on this host install. See detect_install_method().
...
main() {
    ...
    # Code-scoped stamp: write next to the install tree, not into $HAIRBALL_HOME.
    # $HAIRBALL_HOME is a shared data dir (it can be bind-mounted into a Docker
    # gateway too), so a stamp there gets clobbered by the container's 'docker'
    # stamp and wrongly blocks 'hairball update' on this host install.
