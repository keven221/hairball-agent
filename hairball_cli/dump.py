"""
Dump command for hairball CLI.

Outputs a compact, plain-text summary of the user's Hairball setup
...
from hairball_cli.config import get_hairball_home, get_env_path, get_project_root, load_config
...
def _dotenv_key_names() -> set[str]:
    """Return the set of env-var names assigned a non-empty value in ~/.hairball/.env.
    ...
    exports. ``hairball debug share`` runs in a terminal, so ``os.getenv`` reflects
...
def _get_git_commit(project_root: Path) -> str:
    ...
    to ``<project_root>/.hairball_build_sha`` by the Dockerfile's
    ``HAIRBALL_GIT_SHA`` build-arg (see ``hairball_cli/build_info.py``).
...
def _redact(value: str) -> str:
    ...
    ``hairball dump`` formats empty values as blank, not as ``"(not set)"``).
    """
...
def _gateway_status() -> str:
    ...
    try:
        from hairball_cli.gateway import get_gateway_runtime_snapshot
...
def _count_skills(hairball_home: Path) -> int:
...
def _cron_summary(hairball_home: Path) -> str:
    """Return cron jobs summary."""
    jobs_file = hairball_home / "cron" / "jobs.json"
...
def run_dump(args):
    ...
    lines.append(f"hairball_home:      {display_hairball_home()}")
    ...
    for env_var, label in api_keys:
        ...
        # Set in this (shell) process but absent from ~/.hairball/.env: a managed
        ...
        # A credential added via `hairball auth add openrouter` lives in the
        # credential pool, not as an env var — surface it so the dump doesn't
        # misleadingly read "not set" while `hairball auth list` shows it (#42130).
    ...
    toolsets = config.get("toolsets", ["hairball-cli"])
    ...
    lines.append(f"  cron_jobs:          {_cron_summary(hairball_home)}")
    lines.append(f"  skills:             {_count_skills(hairball_home)}")
