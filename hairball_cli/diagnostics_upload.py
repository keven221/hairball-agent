"""Client for uploading ``hairball debug share`` bundles to managed-internal S3.

This is the opt-in (``--managed``) destination for ``hairball debug share``.
Unlike the public paste.rs path, bundles uploaded here go to a managed-owned
S3 bucket via a short-lived signed URL minted by the managed account service
(NAS).  The bucket auto-expires objects after 14 days, and the contents are
only viewable by managed staff (and allowlisted Discord mods) through a
...
# Base URL of the managed account service that mints the signed upload URL.
...
_USER_AGENT = "hairball-agent/debug-share"


def _diagnostics_base_url() -> str:
    """Return an explicitly configured diagnostics service origin.

    Hairball has no built-in remote diagnostics service.  Keep local debug
    sharing available, but require an operator-owned endpoint before the
    opt-in managed upload path performs network I/O.
    """
    base = os.environ.get("HAIRBALL_DIAGNOSTICS_BASE_URL", NAS_BASE).strip().rstrip("/")
    parsed = urllib.parse.urlsplit(base)
    if parsed.scheme not in {"http", "https"} or not parsed.netloc:
        raise RuntimeError(
            "Managed diagnostics upload is not configured. Set "
            "HAIRBALL_DIAGNOSTICS_BASE_URL to an operator-owned HTTPS service, "
            "or use the local debug bundle/share path."
        )
    return base
...
def request_upload_url(
    ...
    base_url = _diagnostics_base_url()
    req = urllib.request.Request(
        f"{base_url}/api/diagnostics/upload-url",
        data=data,
        method="POST",
        headers={
...
def share_to_managed(report_bundle: bytes) -> dict:
    """Orchestrate the full managed-S3 upload of a gzipped *report_bundle*.
