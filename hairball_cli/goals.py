"""Persistent session goals — the Ralph loop for Hairball.

A goal is a free-form user objective that stays active across turns. After
each turn completes, a small judge call asks an auxiliary model "is this
goal satisfied by the assistant's last response?". If not, Hairball feeds a
...
- When a real user message arrives mid-loop it preempts the continuation
  ...
  will say ``done``).
- This module has zero hard dependency on ``cli.HairballCLI`` or the gateway
...
# Adapted from reference's "let reference draft the goal" guidance.
...
# The five contract fields, in display order. Adapted from ChatGPT's
...
def _get_session_db() -> Optional[Any]:
    """Return a SessionDB instance for the current HAIRBALL_HOME.

    SessionDB has no built-in singleton, but opening a new connection per
    /goal call would thrash the file. We cache one instance per
    ``hairball_home`` path so profile switches still pick up the right DB.
    ...
    if cached is not None:
        return cached
    try:
        # ``hairball_state.DEFAULT_DB_PATH`` is resolved when that module is
        # imported.  A long-lived CLI/gateway/UI process can switch profiles
        # afterwards, so using SessionDB's implicit default here could cache a
        # connection under the current home while it actually points at the
        # previous profile.  Bind the connection to the same home used as the
        # cache key.
...
def run_kanban_goal_loop(
    *,
    ...
    (``hairball -p <profile> chat -q "work kanban task <id>"``). The worker's
