"""Profile distributions — shareable, packaged Hairball profiles via git.

A distribution is a Hairball profile published as a git repository (or
...
* ``hairball profile export/import`` — local backup / restore for a profile
  on your own machine. NOT a distribution format. Stays as-is.
* ``hairball skills install <url>`` — the URL install pattern we're mirroring,
  but at the profile granularity.

Subcommands (all live under ``hairball profile``, not a parallel tree):

    hairball profile install <source> [--name N] [--alias] [--force] [--yes]
    hairball profile update  <name>  [--force-config] [--yes]
    hairball profile info    <name>
...
USER_OWNED_EXCLUDE: frozenset = frozenset({
    ...
    "hairball_state.db", "response_store.db",
    "response_store.db-shm", "response_store.db-wal",
    "gateway.pid", "gateway_state.json", "processes.json",
    "auth.lock", "active_profile", ".update_check",
    "errors.log", ".hairball_history",
    # User data
    ...
    "backups", "cache",
    # Infrastructure
    "hairball-agent", ".worktrees", "profiles", "bin", "node_modules",
...
class DistributionManifest:
    ...
    def from_dict(cls, data: Any) -> "DistributionManifest":
        ...
        return cls(
            ...
            description=str(data.get("description") or ""),
            hairball_requires=str(data.get("hairball_requires") or ""),
    ...
    def to_dict(self) -> Dict[str, Any]:
        ...
        if self.hairball_requires:
            out["hairball_requires"] = self.hairball_requires
        if self.author:
...
def check_hairball_requires(spec: str, current_version: str) -> None:
    ...
    if not ok:
        raise DistributionError(
            f"This distribution requires Hairball {op}{target}, "
...
def _env_template_from_manifest(manifest: DistributionManifest) -> str:
    ...
    lines = [
        "# Environment variables required by this Hairball distribution.",
...
def _stage_source(source: str, workdir: Path) -> Tuple[Path, str]:
    ...
    installed manifest's ``source:`` field so ``hairball profile update`` can
    ...
    if _looks_like_git_url(src_str):
        ...
        if not (cloned / MANIFEST_FILENAME).is_file():
            raise DistributionError(
                ...
                "This repository is not a Hairball profile distribution."
            )
...
def plan_install(
    ...
    if manifest is None:
        raise DistributionError(
            ...
            "this source is not a Hairball distribution."
        )

    # Version check up-front so we fail fast
    check_hairball_requires(manifest.hairball_requires, hairball_version)
    ...
    if canon == "default":
        raise DistributionError(
            ...
            "root profile (~/.hairball).  Pass --name <name> to install under a "
            "new profile."
        )
    manifest.name = canon
...
def install_distribution(
    ...
    with tempfile.TemporaryDirectory(prefix="hairball_dist_install_") as tmp:
        ...
        if plan.existing and not force:
            raise DistributionError(
                ...
                "Use `hairball profile update` to upgrade in place, "
...
def update_distribution(
    ...
    if not existing_manifest.source:
        raise DistributionError(
            ...
            "`hairball profile install <source> --name {canon} --force`."
        )

    with tempfile.TemporaryDirectory(prefix="hairball_dist_update_") as tmp:
        plan = plan_install(
