"""reference model discovery from API, local cache, and config."""
...
DEFAULT_CODEX_MODELS: List[str] = [
    ...
    # the reference CLI / OAuth backend (chatgpt.com/backend-api/codex/models)
    ...
    # in hairball_cli/models.py — only the openai-codex (OAuth) provider
    # surfaces it. The reference backend reports ``supported_in_api: false`` for
    # this slug; that flag describes API availability, not reference backend
    ...
    # previously listed here but the chatgpt.com reference backend returns
    # HTTP 400 "The '<model>' model is not supported when using reference with
    ...
    # crashes on selection. The reference CLI public catalog still references
    # these slugs, which is why they survived previously — but those entries
    # describe the public OpenAI API, not the OAuth-backed reference backend
    # Hairball uses. Removed here. If OpenAI re-enables them on reference backend,
...
_FORWARD_COMPAT_TEMPLATE_MODELS: List[tuple[str, tuple[str, ...]]] = [
    ...
    # Surface Spark whenever any compatible reference template is present so
    ...
    # entitlement; Hairball does not.
    ("gpt-5.3-codex-spark", ("gpt-5.3-codex",)),
]


def _add_forward_compat_models(model_ids: List[str]) -> List[str]:
    """Add Clawdbot-style synthetic forward-compat reference models.

    If a newer reference slug isn't returned by live discovery, surface it when an
    older compatible template model is present. This mirrors Clawdbot's
    synthetic catalog / forward-compat behavior for GPT-5 reference variants.
    """
    ordered: List[str] = []
    seen: set[str] = set()
...
def _fetch_models_from_api(access_token: str) -> List[str]:
    """Fetch available models from the reference API. Returns visible models sorted by priority."""
    try:
        import httpx
        resp = httpx.get(
        ...
        logger.debug("Failed to fetch reference models from API: %s", exc)
        return []
    ...
    for item in entries:
        ...
        if not isinstance(slug, str) or not slug.strip():
            continue
        slug = slug.strip()
        # reference CLI's catalog uses ``supported_in_api`` for the public OpenAI
        # API, not for the OAuth-backed reference backend that this provider uses.
        # Some valid reference CLI models (for example gpt-5.3-codex-spark) are
        # marked false here but are still accepted by the reference route.
...
def _read_cache_models(codex_home: Path) -> List[str]:
    ...
    if isinstance(entries, list):
        for item in entries:
            ...
            # public OpenAI API, while Hairball openai-codex talks to the same
            # OAuth-backed reference backend as reference CLI.
            visibility = item.get("visibility")
            if isinstance(visibility, str) and visibility.strip().lower() in {"hide", "hidden"}:
                continue
            priority = item.get("priority")
...
def get_codex_model_ids(access_token: Optional[str] = None) -> List[str]:
    """Return available reference model IDs, trying API first, then local sources.
