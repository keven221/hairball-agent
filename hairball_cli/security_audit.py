"""On-demand supply-chain audit for Hairball Agent installs.

Scans three surfaces a Hairball user actually controls and we can map to
upstream advisories without auth or extra binaries:

1. The Hairball venv (every PyPI dist via ``importlib.metadata``).
2. Python deps declared by user-installed plugins under ``~/.hairball/plugins``
...
def _discover_plugins(hairball_home: Path) -> list[Component]:
    """Python deps declared by plugins under ``~/.hairball/plugins``.
    ...
    """
    plugins_dir = hairball_home / "plugins"
...
def run_audit(
    *,
    ...
    home = hairball_home or Path(get_hairball_home())
...
def _count_components(
    *, skip_venv: bool, skip_plugins: bool, skip_mcp: bool, hairball_home: Path
) -> int:
    total = 0
    if not skip_venv:
        total += len(_discover_venv())
    if not skip_plugins:
        total += len(_discover_plugins(hairball_home))
    if not skip_mcp:
...
def cmd_security_audit(args: argparse.Namespace) -> int:
    """Implementation of `hairball security audit`."""
    ...
    total = _count_components(
        skip_venv=skip_venv, skip_plugins=skip_plugins, skip_mcp=skip_mcp, hairball_home=home
    )
    if total == 0:
