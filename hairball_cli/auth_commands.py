...
_OAUTH_CAPABLE_PROVIDERS = {"anthropic", "managed", "openai-codex", "xai-oauth", "qwen-oauth", "minimax-oauth"}
...
def auth_add_command(args) -> None:
    ...
    # Matches the reference device_code re-link pattern that predates this.
    ...
    if provider == "anthropic":
        ...
        creds = anthropic_mod.run_hairball_oauth_login_pure()
        if not creds:
        ...
        entry = PooledCredential(
            ...
            priority=0,
            source=f"{SOURCE_MANUAL}:hairball_pkce",
    ...
    if provider == "managed":
        # session-style auto-import: if a shared managed credential lives at
        # <hairball-root>/shared/managed_auth.json (written by any previous
        # successful login), offer to import it instead of running the
        # full device-code flow. This makes `hairball --profile <name>
        ...
        if shared:
            ...
            print()
            if path:
                print(f"Found existing managed OAuth credentials at {path}")
            else:
    ...
    if provider == "openai-codex":
        ...
        # latest login: a second ``hairball auth add openai-codex`` overwrote
        ...
        pool.add_entry(entry)
        # Adding the first reference credential should make it the active provider
    ...
    raise SystemExit(f"`hairball auth add {provider}` is not implemented for auth type {requested_type} yet.")
...
def auth_remove_command(args) -> None:
    ...
    # Unified removal dispatch.  Every credential source Hairball reads from
...
def auth_status_command(args) -> None:
    ...
    if not provider:
        raise SystemExit("Provider is required. Example: `hairball auth status spotify`.")
...
def _interactive_auth() -> None:
    """Interactive credential pool management when `hairball auth` is called bare."""
