"""
Interactive setup wizard for Hairball Agent.
...
from tools.tool_backend_helpers import managed_portal_tools_enabled
from utils import base_url_hostname
from hairball_constants import get_optional_skills_dir
...
# display_hairball_home imported lazily at call sites (stale-module safety during hairball update)
...
from hairball_cli.cli_output import (  # noqa: E402
    print_error,
    print_info,
    print_success,
    print_warning,
)
from hairball_cli.secret_prompt import masked_secret_prompt  # noqa: E402
...
def print_noninteractive_setup_guidance(reason: str | None = None) -> None:
    ...
    print()
    print(color("ฅ Hairball Setup — Non-interactive mode", Colors.CYAN, Colors.BOLD))
    print()
    if reason:
        print_info(reason)
    print_info("The interactive wizard cannot be used here.")
    print()
    print_info("Configure Hairball using environment variables or config commands:")
    print_info("  hairball config set model.provider custom")
    print_info("  hairball config set model.base_url http://localhost:8080/v1")
    print_info("  hairball config set model.default your-model-name")
    print()
    print_info("Or set OPENROUTER_API_KEY / OPENAI_API_KEY in your environment.")
    print_info("Run 'hairball setup' in an interactive terminal to use the full wizard.")
    print()
...
def is_noninteractive() -> bool:
    ...
    ``HAIRBALL_NONINTERACTIVE=1`` (see ``hairball_cli/web_server.py``). In that
    ...
    """
    return os.environ.get("HAIRBALL_NONINTERACTIVE", "").strip().lower() in {
        "1",
        "true",
        "yes",
        "on",
...
def prompt_yes_no(question: str, default: bool = True) -> bool:
    ...
    Non-interactive callers (``HAIRBALL_NONINTERACTIVE=1`` or a closed/redirected
...
def prompt_checklist(title: str, items: list, pre_selected: list = None) -> list:
    ...
    from hairball_cli.curses_ui import curses_checklist
...
def _prompt_api_key(var: dict):
    ...
    if value:
        ...
        print_warning("  Skipped (configure later with 'hairball setup')")


def _print_setup_summary(config: dict, hairball_home):
    ...
    if _vision_backends:
        ...
        tool_status.append(("Vision (image analysis)", False, "run 'hairball setup' to configure"))


    # Web tools (Exa, Parallel, Firecrawl, or Tavily)
    if subscription_features.web.managed_by_managed:
    ...
    if subscription_features.browser.managed_by_managed:
    ...
    # Image generation — FAL (direct or via managed), or any plugin-registered
    # provider (OpenAI, etc.)
    if subscription_features.image_gen.managed_by_managed:
        ...
        try:
            ...
            from hairball_cli.plugins import _ensure_plugins_discovered
    ...
    # Video generation — opt-in via `hairball tools` → Video Generation.
    # Only show the row when a plugin reports available so we don't badger
    # users who don't care about video gen with a "missing" status line.
    if subscription_features.video_gen.managed_by_managed:
        tool_status.append(("Video Generation (FAL via managed subscription)", True, None))
    else:
        try:
            from agent.video_gen_registry import list_providers as _list_video_providers
            from hairball_cli.plugins import _ensure_plugins_discovered as _ensure_plugins
            _ensure_plugins()
            _video_backend = None
    ...
    if subscription_features.tts.managed_by_managed:
        ...
        if neutts_ok:
            ...
            tool_status.append(("Text-to-Speech (NeuTTS — not installed)", False, "run 'hairball setup tts'"))
        ...
        if kittentts_ok:
            ...
            tool_status.append(("Text-to-Speech (KittenTTS — not installed)", False, "run 'hairball setup tts'"))
    else:
        tool_status.append(("Text-to-Speech (Edge TTS)", True, None))

    if subscription_features.modal.managed_by_managed:
        ...
        if subscription_features.modal.direct_override:
            ...
            tool_status.append(("Modal Execution", False, "run 'hairball setup terminal'"))
    elif managed_portal_tools_enabled() and subscription_features.managed_auth_present:
    ...
    # Spotify (OAuth via hairball auth spotify — check auth.json, not env vars)
    try:
    ...
    if disabled_tools:
        print_warning(
            "Some tools are disabled. Run 'hairball setup tools' to configure them,"
        )
    ...
    print(
        f"   {color('Data:', Colors.YELLOW)}      {hairball_home}/cron/, sessions/, logs/"
    )
    print()
    ...
    print()
    print(f"   {color('hairball setup', Colors.GREEN)}          Re-run the full wizard")
    print(f"   {color('hairball setup model', Colors.GREEN)}    Change model/provider")
    print(f"   {color('hairball setup terminal', Colors.GREEN)} Change terminal backend")
    print(f"   {color('hairball setup gateway', Colors.GREEN)}  Configure messaging")
    print(f"   {color('hairball setup tools', Colors.GREEN)}    Configure tool providers")
    print()
    print(f"   {color('hairball config', Colors.GREEN)}         View current settings")
    print(
        f"   {color('hairball config edit', Colors.GREEN)}    Open config in your editor"
    )
    print(f"   {color('hairball config set <key> <value>', Colors.GREEN)}")
    ...
    print()
    print(f"   {color('hairball', Colors.GREEN)}              Start chatting")
    print(f"   {color('hairball gateway', Colors.GREEN)}      Start messaging gateway")
    print(f"   {color('hairball doctor', Colors.GREEN)}       Check for issues")
    print()
...
# between `hairball tools` and `hairball setup tools`).
...
def setup_model_provider(config: dict, *, quick: bool = False):
    ...
    Delegates to ``cmd_model()`` (the same flow used by ``hairball model``)
    ...
    provider added to ``hairball model`` is automatically available here.
    ...
    # Delegate to the shared hairball model flow — handles provider picker,
    # credential prompting, model selection, and config persistence.
    from hairball_cli.main import select_provider_and_model
    try:
        ...
        print_info("You can try again later with: hairball model")
    ...
    # on demand via `hairball auth add`, `hairball setup` vision, and
    # `hairball setup tts`. This keeps both quick and full setup thin.
...
def _install_neutts_deps() -> bool:
    ...
    from hairball_cli.tools_config import _pip_install
...
def _xai_oauth_logged_in_for_setup() -> bool:
    ...
    through ``hairball model`` -> xAI Grok OAuth (SuperGrok / Premium+).
    """
    try:
        from hairball_cli.auth import get_xai_oauth_auth_status
...
def _setup_tts_provider(config: dict):
    ...
    choices = []
    providers = []
    if managed_portal_tools_enabled() and subscription_features.managed_auth_present:
        choices.append("managed Subscription (managed OpenAI TTS, billed to your subscription)")
    ...
    if selected == "managed-openai":
        selected = "openai"
        print_info("OpenAI TTS will use the managed managed gateway and bill to your subscription.")
        if get_env_value("VOICE_TOOLS_OPENAI_KEY") or get_env_value("OPENAI_API_KEY"):
            print_warning(
                "Direct OpenAI credentials are still configured and may take precedence until removed from ~/.hairball/.env."
            )
    ...
    if selected == "neutts":
        ...
        # via the Hairball auth store) > existing XAI_API_KEY > prompt the user.
            ...
            if choice_idx == 0:
                ...
                if api_key:
                    ...
                    print_warning(
                        ...
                        f"via hairball setup model or {_dhh()}/.env to use xAI TTS. "
                        "Falling back to Edge TTS."
                    )
                    selected = "edge"
            else:
...
def setup_tts(config: dict):
    """Standalone TTS setup (for 'hairball setup tts')."""
...
def setup_terminal_backend(config: dict):
    ...
    print_info("Choose where Hairball runs shell commands and code.")
    ...
    if selected_backend == "local":
        ...
        # configurable later via `hairball setup terminal` / config.yaml.
        ...
        # Image and resource limits use defaults; tune via `hairball setup terminal`.
        ...
        if use_managed_modal:
            ...
            print_info("Modal execution will use the managed managed gateway and bill to your subscription.")
...
def _apply_default_agent_settings(config: dict):
    ...
    # bridges it into HAIRBALL_MAX_ITERATIONS at startup. We no longer write
    ...
    remove_env_value("HAIRBALL_MAX_ITERATIONS")
    ...
    print_info("  Run `hairball setup agent` later to customize.")
...
def setup_agent_settings(config: dict):
    ...
    try:
        ...
        if max_iter > 0:
            ...
            # HAIRBALL_MAX_ITERATIONS from agent.max_turns at startup.
    ...
    print_info("  log     — Silent in chat; write every tool call to ~/.hairball/logs/tool_calls.log (gateway only)")
...
def _setup_telegram_auto_result():
    ...
    try:
        from hairball_cli.telegram_managed_bot import auto_setup_telegram_bot_result
    except ImportError:
        return None

    profile_name: str | None = None
    try:
        profile_name = _profile_name_from_hairball_home(Path(get_hairball_home()))
    except Exception:
        pass
...
def _profile_name_from_hairball_home(hairball_home) -> str | None:
    """Return the active profile name when HAIRBALL_HOME is a profile dir."""
    if hairball_home.parent.name == "profiles":
        return hairball_home.name
    return None
...
def _setup_telegram():
    ...
    print()
    print_info("📬 Home Channel: where Hairball delivers cron job results,")
...
def _setup_bluebubbles():
    ...
    print_info("Connects Hairball to iMessage via BlueBubbles — a free, open-source")
...
def _setup_qqbot():
    ...
    from hairball_cli.gateway import _setup_qqbot as _gateway_setup_qqbot
...
def _setup_webhooks():
    ...
    print()
    print_info("   Full guide: ./website/docs/user-guide/messaging/webhooks/")
    print()
    ...
    print_info("   ./website/docs/user-guide/messaging/webhooks/#configuring-routes")
    print()
    print_info("   Open config in your editor:  hairball config edit")
    print_info("   Open config in your editor:  hairball config edit")


def setup_gateway(config: dict):
    """Configure messaging platform integrations."""
    from hairball_cli.gateway import _all_platforms, _platform_status, _configure_platform

    print_header("Messaging Platforms")
    print_info("Connect to messaging platforms to chat with Hairball from anywhere.")
    ...
    if not selected:
        print_info("No platforms selected. Run 'hairball setup gateway' later to configure.")
        return
    ...
    if any_messaging:
        ...
        if missing_home:
            ...
            for plat in missing_home:
                print_info(
                    f"     hairball config set {plat.upper()}_HOME_CHANNEL <channel_id>"
                )
        ...
        if supports_systemd and has_legacy_hairball_units():
            print_legacy_unit_warning()
            print()
        ...
        if service_running:
            ...
            if prompt_yes_no(
                ...
                if supports_systemd and os.geteuid() == 0:  # windows-footgun: ok — guarded by supports_systemd (Linux only)
                    print_info("  Or as a boot-time service: hairball gateway install --system")
                print_info("  Or run in foreground:  hairball gateway")
        else:
            from hairball_constants import is_container
            if is_container():
                print_info("Start the gateway to bring your bots online:")
                print_info("   hairball gateway run          # Run as container main process")
                print_info("")
                ...
                print_info("Start the gateway to bring your bots online:")
                print_info("   hairball gateway              # Run in foreground")
...
def setup_tools(config: dict, first_install: bool = False):
    ...
    Both `hairball setup tools` and `hairball tools` use the same flow:
...
def _get_section_config_summary(config: dict, section_key: str) -> Optional[str]:
    ...
    ``get_env_value`` is the module-level import from hairball_cli.config
    ...
    if section_key == "model":
        ...
        from hairball_cli.gateway import _all_platforms, _platform_status
...
_OPENCLAW_SCRIPT = (
    ...
    / "openclaw-migration"
    / "scripts"
    / "openclaw_to_hairball.py"
)


def _load_openclaw_migration_module():
    """Load the openclaw_to_hairball migration script as a module.
    ...
    spec = importlib.util.spec_from_file_location(
        "openclaw_to_hairball", _OPENCLAW_SCRIPT
    )
...
# Config values may have different semantics between legacy and Hairball.
# Instruction/context files (.md) can contain incompatible setup procedures.
_HIGH_IMPACT_KIND_KEYWORDS = {
    "gateway": "⚠ Gateway/messaging — this will configure Hairball to use your legacy messaging channels",
    "telegram": "⚠ Telegram — this will point Hairball at your legacy Telegram bot",
    "slack": "⚠ Slack — this will point Hairball at your legacy Slack workspace",
    "discord": "⚠ Discord — this will point Hairball at your legacy Discord bot",
    "whatsapp": "⚠ WhatsApp — this will point Hairball at your legacy WhatsApp connection",
    "config": "⚠ Config values — legacy settings may not map 1:1 to Hairball equivalents",
    "soul": "⚠ Instruction file — may contain legacy-specific setup/restart procedures",
    "memory": "⚠ Memory/context file — may reference legacy-specific infrastructure",
    "context": "⚠ Context file — may contain legacy-specific instructions",
}
...
def _print_migration_preview(report: dict):
    ...
    if conflict_items:
        print(color("  Would overwrite (conflicts with existing Hairball config):", Colors.YELLOW))
    ...
    if warnings_shown:
        ...
        print()
        print(color("  Note: legacy config values may have different semantics in Hairball.", Colors.YELLOW))
        print(color("  For example, legacy's tool_call_execution: \"auto\" ≠ Hairball's yolo mode.", Colors.YELLOW))
        print(color("  Instruction files (.md) from legacy may contain incompatible procedures.", Colors.YELLOW))
        print()


def _offer_openclaw_migration(hairball_home: Path) -> bool:
    ...
    print_info(f"Found legacy data at {openclaw_dir}")
    print_info("Hairball can preview what would be imported before making any changes.")
    print()

    if not prompt_yes_no("Would you like to see what can be imported?", default=True):
        print_info(
            "Skipping migration. You can run it later with: hairball claw migrate --dry-run"
        )
        return False
    ...
    try:
        ...
        logger.debug("legacy migration module load error", exc_info=True)
        return False
    ...
    try:
        ...
        dry_migrator = mod.Migrator(
            ...
            target_root=hairball_home.resolve(),
        ...
        logger.debug("legacy migration preview error", exc_info=True)
        return False
    ...
    if preview_count == 0:
        print()
        print_info("Nothing to import from legacy.")
        return False
    ...
    if not prompt_yes_no("Proceed with migration?", default=False):
        print_info(
            "Migration cancelled. You can run it later with: hairball claw migrate"
        )
        print_info(
    ...
    # Execute the migration — overwrite=False so existing Hairball configs are
    ...
    try:
        migrator = mod.Migrator(
            ...
            execute=True,
            workspace_target=None,
            overwrite=False,  # preserve existing Hairball config
            migrate_secrets=True,
            output_dir=None,
            selected_options=selected,
            preset_name="full",
        ...
        logger.debug("legacy migration error", exc_info=True)
        return False
    ...
    print()
    if migrated:
        print_success(f"Imported {migrated} item(s) from legacy.")
    if conflicts:
        print_info(f"Skipped {conflicts} item(s) that already exist in Hairball (use hairball claw migrate --overwrite to force).")
    if skipped:
...
def _run_portal_one_shot(config: dict) -> None:
    """One-shot Managed Portal setup — OAuth + model pick + provider + Tool Gateway.

    Product default disables managed foreign services (no community replacement).
    When ``managed_foreign.backend`` is ``off``, refuse and point users at BYOK.
    """
    try:
        ...
        if is_managed_foreign_off():
            print()
            print_info(managed_foreign_disabled_message())
            print_info("  Recommended:")
            print_info("    hairball setup          # provider API key (models)")
            print_info("    hairball tools          # direct tool API keys")
            return
    except Exception:
        pass
    ...
    print(color("│     ฅ Hairball Setup — Managed Portal (one-shot)             │", Colors.MAGENTA))
    print(
        color(
            "└─────────────────────────────────────────────────────────┘",
            Colors.MAGENTA,
    ...
    print_info("    — all routed through your Managed Portal sub.")
    print()
    print_info("  Sign up: #")
    print()

    # _model_flow_managed handles BOTH the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # managed model picker), then offers the Tool Gateway opt-in and sets
    # provider=managed via the login/model save. This is the same routine quick
    # setup calls, so `hairball portal` == quick setup's managed step.
    try:
        from hairball_cli.main import _model_flow_managed

        _model_flow_managed(config)
    except (KeyboardInterrupt, EOFError, SystemExit):
        # _login_managed raises SystemExit(130)/(1) on cancel/failure; the
        # logged-out path inside _model_flow_managed catches it, but the
        ...
        print_info("  You can retry later with `hairball portal`.")
        return
    except Exception as exc:
        logger.debug("_model_flow_managed error during `hairball portal`: %s", exc)
        print()
        print_error(f"  Managed Portal setup encountered an error: {exc}")
        print_info("  You can retry later with `hairball portal`.")
        return

    # Re-sync the in-memory config from disk — _model_flow_managed (and the
    ...
    print_info("  Run `hairball portal info` to inspect routing.")
    print_info("  Run `hairball` to start chatting.")
...
def run_setup_wizard(args):
    ...
    Supports full, quick, and section-specific setup:
      hairball setup           — full or quick (auto-detected)
      hairball setup model     — just model/provider
      hairball setup tts       — just text-to-speech
      hairball setup terminal  — just terminal backend
      hairball setup gateway   — just messaging platforms
      hairball setup tools     — just tool configuration
      hairball setup agent     — just agent settings
    """
    from hairball_cli.config import is_managed, managed_error
    if is_managed():
    ...
    # --portal: one-shot Managed Portal setup. Skips the rest of the wizard.
    ...
    if section:
        for key, label, func in SETUP_SECTIONS:
            if key == section:
                ...
                print(color(f"│     ฅ Hairball Setup — {label:<34s} │", Colors.MAGENTA))
                print(
                    color(
                        "└─────────────────────────────────────────────────────────┘",
                        Colors.MAGENTA,
    ...
    print(
        color(
            "│             ฅ Hairball Agent Setup Wizard                │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    print(
        color(
            "│  Let's configure your Hairball Agent installation.       │", Colors.MAGENTA
        )
    )
    print(
        color(
    ...
    if is_existing:
        ...
        if quick_requested:
            _run_quick_setup(config, hairball_home)
            return

        print()
        print_header("Reconfigure")
        print_success("You already have Hairball configured.")
        print_info("Running the full wizard — each prompt shows your current value.")
        print_info("Press Enter to keep it, or type a new value to change it.")
        print_info("")
        print_info("Tip: jump straight to a section with 'hairball setup model|terminal|")
        ...
        # Offer legacy migration before configuration begins
        migration_ran = _offer_openclaw_migration(hairball_home)
        if migration_ran:
            config = load_config()

        setup_mode = prompt_choice(
            "How would you like to set up Hairball?",
            [
                "Quick Setup (Managed Portal) — free OAuth login, no API keys, model + tools (recommended)",
        ...
        if setup_mode == 0:
            _run_first_time_quick_setup(config, hairball_home, is_existing)
            return
        if setup_mode == 2:
            _run_blank_slate_setup(config, hairball_home, is_existing)
            return
    ...
    print_info(f"Data folder:  {hairball_home}")
    print_info(f"Install dir:  {PROJECT_ROOT}")
    print()
    print_info("You can edit these files directly or use 'hairball config edit'")

    if migration_ran:
        print()
        print_info("Settings were imported from legacy.")
    ...
    # Tune later with `hairball setup agent`.
    if not is_existing:
    ...
    _print_setup_summary(config, hairball_home)


def _run_first_time_quick_setup(config: dict, hairball_home, is_existing: bool):
    """Streamlined first-time setup via Managed Portal: OAuth, model, terminal & messaging.

    Routes straight to the Managed Portal provider — runs the device-code OAuth
    login, picks a managed model, then configures the terminal backend and (optionally)
    a messaging platform. Applies sensible defaults for everything else (agent
    settings, tools); the user can customize later via ``hairball setup <section>``
    or switch providers with ``hairball model``.
    """
    from hairball_cli.config import load_config

    # Step 1: Managed Portal — OAuth login + model selection.
    # _model_flow_managed() handles both the logged-out path (device-code OAuth,
    # which selects a model internally) and the already-logged-in path (curated
    # managed model picker). Provider is set to "managed" by the login/model save.
    print()
    print_header("Managed Portal")
    print_info("One subscription, 300+ models, plus the Tool Gateway:")
    print_info("  web search, image generation, TTS, browser automation.")
    print_info("Sign up: #")
    print()
    try:
        ...
        print()
        print_info("Managed Portal setup cancelled.")
    except Exception as exc:
        logger.debug("_model_flow_managed error during quick setup: %s", exc)
        print_warning(f"Managed Portal setup encountered an error: {exc}")
        print_info("You can try again later with: hairball model")

    # Re-sync the wizard's config dict from disk — _model_flow_managed (and the
    ...
    gateway_choice = prompt_choice(
        ...
        [
            ...
            "Skip — set up later with 'hairball setup gateway'",
        ],
        0,
    )
    ...
    print()
    print_info("  Configure all settings:    hairball setup")
    if gateway_choice != 0:
        print_info("  Connect Telegram/Discord:  hairball setup gateway")
    print()
...
def _blank_slate_minimal_toolsets(config: dict):
    ...
    2. ``agent.disabled_toolsets`` — a global hard-suppression list (applied last
       ...
       quirks. The user re-enables any of them later via ``hairball tools`` (which
    ...
    try:
        ...
        from hairball_cli.tools_config import CONFIGURABLE_TOOLSETS, _get_plugin_toolset_keys
...
def _blank_slate_minimize_config(config: dict):
    ...
    Everything here is opt-in afterwards via ``hairball setup agent`` /
    ``hairball config set``. We keep only what's needed to run.
    """
...
def _run_blank_slate_setup(config: dict, hairball_home, is_existing: bool):
    ...
    if path == 0:
        ...
        # `hairball update` runs don't re-inject them.
        try:
        ...
        print_info("  Enable tools:        hairball tools")
        print_info("  Seed skills:         hairball skills opt-in --sync")
        print_info("  Add MCP servers:     hairball mcp add")
        print_info("  Enable plugins:      hairball plugins")
        print_info("  Tune agent settings: hairball setup agent")
        print()
        _print_setup_summary(config, hairball_home)
        return

    # ── Walkthrough path — opt in to each capability ──
    _blank_slate_walkthrough(config, hairball_home)


def _blank_slate_walkthrough(config: dict, hairball_home):
    ...
    try:
        ...
        if seed_skills:
            ...
            print_info("`hairball update` runs from re-injecting them. Opt back in any")
            print_info("time with `hairball skills opt-in --sync`.")
    except Exception as exc:
    ...
    if prompt_yes_no("Open the tool selector to enable more tools?", default=False):
        ...
        print_info("Keeping the minimal toolset. Add tools later with `hairball tools`.")
    ...
    if prompt_yes_no("Review and enable built-in plugins now?", default=False):
        print_info("Manage plugins with `hairball plugins list` / `hairball plugins install`.")
    else:
        print_info("No plugins enabled. Add later with `hairball plugins`.")

    # ── MCP servers (off unless chosen) ──
    print()
    print_header("MCP Servers")
    if prompt_yes_no("Add an MCP server now?", default=False):
        print_info("Add servers with `hairball mcp add <name> --url ... | --command ...`.")
    else:
        print_info("No MCP servers configured. Add later with `hairball mcp add`.")
    ...
    print()
    print_success("Blank Slate setup complete — minimal agent ready.")
    print_info("  Enable more tools:   hairball tools")
    print_info("  Seed skills:         hairball skills opt-in --sync")
    print_info("  Add MCP servers:     hairball mcp add")
    print_info("  Tune agent settings: hairball setup agent")
    print()
...
def _run_quick_setup(config: dict, hairball_home):
    ...
    if not has_anything_missing:
        ...
        print()
        print_info("Run 'hairball setup' and choose 'Full Setup' to reconfigure,")
    ...
    if missing_messaging:
        print()
        print_header("Messaging Platforms")
        print_info("Connect Hairball to messaging apps to chat from anywhere.")
        print_info("You can configure these later with 'hairball setup gateway'.")
