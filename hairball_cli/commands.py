"""Slash command definitions and autocomplete for the Hairball CLI.
...
COMMAND_REGISTRY: list[CommandDef] = [
    ...
    CommandDef("rollback", "List or restore filesystem checkpoints", "Session",
               args_hint="[number]"),
    CommandDef("snapshot", "Create or restore state snapshots of Hairball config/state", "Session",
    ...
    CommandDef("steer", "Inject a message after the next tool call without interrupting", "Session",
               args_hint="<prompt>"),
    CommandDef("inject", "Write a next-step note without starting a turn", "Session",
               args_hint="<prompt>"),
    CommandDef("goal", "Set a standing goal Hairball works on across turns until achieved", "Session",
               args_hint="[text | draft <text> | show | pause | resume | clear | status | wait <pid> | unwait]"),
    CommandDef("plan", "Create, review, approve, execute, resume, cancel, or clear a durable Plan", "Session",
               args_hint="[prompt | status | approve <request_id> <revision> <digest> | reject <request_id> <revision> <digest> | execute <plan_id> <revision> <digest> <request_id> | resume ... | cancel | clear]"),
    ...
    CommandDef("codex-runtime", "Toggle codex app-server runtime for ChatGPT models",
    ...
    CommandDef("busy", "Control what Enter does while Hairball is working", "Configuration",
    ...
    CommandDef("reload-mcp", "Reload MCP servers from config", "Tools & Skills",
               aliases=("reload_mcp",)),
    CommandDef("reload-skills", "Re-scan ~/.hairball/skills/ for newly installed or removed skills",
    ...
    CommandDef("restart", "Gracefully restart the gateway after draining active runs", "Session",
               gateway_only=True),
    CommandDef("usage", "Show token usage and rate limits; `reset` redeems a banked reference limit reset", "Info",
               args_hint="[reset [--force]]"),
    CommandDef("credits", "Show managed credit balance and top up", "Info"),
    CommandDef("billing", "Manage managed terminal billing — buy credits, auto-reload, limits", "Info",
               cli_only=True),
    ...
    CommandDef("update", "Update Hairball Agent to the latest version", "Info"),
    CommandDef("version", "Show Hairball Agent version", "Info", aliases=("v",)),
    CommandDef("debug", "Upload debug report (system info + logs) and get shareable links", "Info",
               args_hint="[managed|local]"),
...
def _iter_plugin_command_entries() -> list[tuple[str, str, str]]:
    ...
    :func:`hairball_cli.plugins.PluginContext.register_command`. They behave
    like ``CommandDef`` entries for gateway surfacing: they appear in the
    Telegram command menu, in Slack's ``/hairball`` subcommand mapping, and
...
# Telegram allows up to 100 BotCommands. Hairball ships ~50 built-in commands;
...
Telegram only displays a small BotCommand menu in practice.  The full Hairball
...
def _collect_gateway_skill_entries(
    ...
    try:
        ...
        # ``hairball skills list`` and the agent's ``/skill-name`` dispatch but
...
def telegram_menu_commands(max_commands: int = 100) -> tuple[list[tuple[str, str]], int]:
    ...
    Skills disabled for the ``"telegram"`` platform (via ``hairball skills
...
def discord_skill_commands_by_category(
    ...
    *uncategorized*.
    ...
    parity, external-dir skills are visible via ``hairball skills list`` and
...
# unrelated command landed. These claim their slots right after /hairball,
# ahead of both canonical names and the rest of the aliases. Anything not
# listed here still degrades gracefully (reachable via /hairball <command>).
...
# native slot, the alias spelling stays reachable via /hairball reset).
...
# ``/hairball <command>`` on Slack only. They remain native on every other
# surface (CLI, TUI, Telegram, Discord). Keep this list TIGHT and intentional —
# the telegram-parity test reads it so an entry here is a deliberate
# "Slack-via-/hairball" decision, not a silent clamp.
#   - credits: the billing/top-up surface; reached via /hairball credits on Slack.
#   - billing: the terminal-billing surface (buy/auto-reload/limit); /hairball billing.
#   - moa: high-cost slash mode, available through /hairball moa to avoid
#     displacing existing native Slack slash commands at the 50-command cap.
#   - debug: the log/report upload surface; reached via /hairball debug on Slack.
#   - version: low-frequency build metadata; reached via /hairball version on Slack.
_SLACK_VIA_HAIRBALL_ONLY = frozenset(
    {"credits", "billing", "moa", "debug", "version"}
)
...
def slack_native_slashes() -> list[tuple[str, str, str]]:
    ...
    first-class slash and not a ``/hairball <verb>`` subcommand.
    ...
    can still reach them via ``/hairball <command>``.

    Results are clamped to Slack's 50-command limit with duplicate-name
    avoidance. ``/hairball`` is always reserved as the first entry so the
    legacy ``/hairball <subcommand>`` form keeps working for anything that
    ...
    # Reserve /hairball as the catch-all top-level command.
    entries.append(("hairball", "Talk to Hairball or run a subcommand", "[subcommand] [args]"))
    seen.add("hairball")
    ...
    def _add(name: str, desc: str, hint: str) -> None:
        ...
        if slack_name in _SLACK_RESERVED_COMMANDS:
            return
        if slack_name in _SLACK_VIA_HAIRBALL_ONLY:
            # Intentionally Slack-via-/hairball only (see _SLACK_VIA_HAIRBALL_ONLY).
            return
    ...
    # everything except /hairball, so a new canonical command can never silently
...
def slack_app_manifest(request_url: str = "https://hairball-agent.local/slack/commands") -> dict[str, Any]:
...
def slack_subcommand_map() -> dict[str, str]:
    """Return subcommand -> /command mapping for Slack /hairball handler.

    Maps both canonical names and aliases so /hairball bg do stuff works
    the same as /hairball background do stuff.

    Plugin-registered slash commands are included so ``/hairball <plugin-cmd>``
...
class SlashCommandCompleter(Completer):
    ...
    def _skin_completions(sub_text: str, sub_lower: str):
        ...
        try:
            from hairball_cli.skin_engine import list_skins
            for s in list_skins():
                name = s["name"]
