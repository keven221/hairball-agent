...
class PtySessionRegistry:
    ...
    def iter_keys(self) -> list:
        """Snapshot of registry keys (UI mux discovery; dashboard unused)."""
        return list(self._sessions.keys())

    def get(self, key: str):
        """Return a live registry entry or None."""
        return self._sessions.get(key)

    async def drop(self, key: str) -> None:
        """Close and forget one session immediately.

        For an explicit "give me a fresh terminal" action, where waiting for
        the TTL would leave the replaced shell running.
        """
