"""
Hairball MCP Server — expose messaging conversations as MCP tools.

Starts a stdio MCP server that lets any MCP client (Claude Code, Cursor, reference,
etc.) list conversations, read message history, send messages, poll for live
events, and manage approval requests across all connected platforms.

Matches legacy's 9-tool MCP channel bridge surface:
  conversations_list, conversation_get, messages_read, attachments_fetch,
  events_poll, events_wait, messages_send, permissions_list_open,
  permissions_respond

Plus: channels_list (Hairball-specific extra)

Usage:
    hairball mcp serve
    hairball mcp serve --verbose
...
logger = logging.getLogger("hairball.mcp_serve")
...
def _get_sessions_dir() -> Path:
    """Return the sessions directory using HAIRBALL_HOME."""
    try:
        from hairball_constants import get_hairball_home
        return get_hairball_home() / "sessions"
    except ImportError:
        return Path(os.environ.get("HAIRBALL_HOME", Path.home() / ".hairball")) / "sessions"
...
def _load_channel_directory() -> dict:
    ...
    try:
        from hairball_constants import get_hairball_home
        directory_file = get_hairball_home() / "channel_directory.json"
    except ImportError:
        directory_file = Path(
            os.environ.get("HAIRBALL_HOME", Path.home() / ".hairball")
...
class EventBridge:
    ...
    This is the Hairball equivalent of legacy's WebSocket gateway bridge.
    ...
    def _poll_once(self, db):
        ...
        try:
            from hairball_constants import get_hairball_home
            db_file = get_hairball_home() / "state.db"
        except ImportError:
            db_file = Path(os.environ.get("HAIRBALL_HOME", Path.home() / ".hairball")) / "state.db"
...
def create_mcp_server(event_bridge: Optional[EventBridge] = None) -> "FastMCP":
    """Create and return the Hairball MCP server with all tools registered."""
    ...
    mcp = FastMCP(
        "hairball",
        instructions=(
            "Hairball Agent messaging bridge. Use these tools to interact with "
...
def run_mcp_server(verbose: bool = False) -> None:
    """Start the Hairball MCP server on stdio."""
    if not _MCP_SERVER_AVAILABLE:
        print(
