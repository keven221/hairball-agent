...
_HAIRBALL_CORE_TOOLS = [
    # Web
    "web_search", "web_extract",
    # Terminal + process management
    "terminal", "process",
    # Read the desktop GUI's embedded terminal pane, and close an agent's
    # read-only terminal tab (both gated on HAIRBALL_DESKTOP via check_fn —
    ...
    # Text-to-speech
    "text_to_speech",
    # Planning & memory. plan_exit remains schema-stable across turns; the
    # execution-time Plan gate rejects it outside Plan mode.
    "todo", "plan_exit", "memory",
    ...
    # Collab V2 (check_fn: delegation.collab_v2.enabled; zero cost when off)
    "collab_spawn", "collab_wait", "collab_collect", "collab_send",
    "collab_followup", "collab_interrupt", "collab_list",
    # Cronjob management
    "cronjob",
    # Home Assistant smart home control (gated on HASS_TOKEN via check_fn)
    "ha_list_entities", "ha_get_state", "ha_list_services", "ha_call_service",
    # Kanban multi-agent coordination — only in schema when the agent is
    # spawned as a kanban worker (HAIRBALL_KANBAN_TASK env set) or the current
...
TOOLSETS = {
    ...
    "x_search": {
        "description": (
            ...
            "enable in `hairball tools` → X (Twitter) Search."
        ),
        "tools": ["x_search"],
        "includes": []
    },
    ...
    "video_gen": {
        "description": (
            ...
            "``hairball tools`` → Video Generation."
        ),
    ...
    "delegation": {
        ...
        # collab_* gated by tools/collab_control.check_collab_v2_requirements
        # (delegation.collab_v2.enabled); absent from schema when flag is off.
        "tools": [
            "delegate_task",
            "collab_spawn",
            "collab_wait",
    ...
    "kanban": {
        "description": (
            ...
            "is spawned by the kanban dispatcher (HAIRBALL_KANBAN_TASK env "
    ...
    # Coding posture (base Hairball — CLI/TUI/desktop/ACP). Auto-selected in a
    ...
    # Model-facing LSP (navigation / rename). Not in _HAIRBALL_CORE_TOOLS —
    # messaging platforms must not pay the schema cost. Mounted via coding /
    # hairball-acp and coding_edge_toolsets() on interactive coding surfaces.
    # See docs/design/2026-08-04-hairball-independent-core-knowledge.md#4-coding-tools.
    "lsp": {
        "description": "Language-server tools: definition, references, hover, symbols, diagnostics, rename",
        "tools": ["lsp"],
        "includes": [],
    },

    # Structural search/edit (ast-grep CLI). check_fn hides when binary missing.
    # See CODING_CAPABILITY_ALIGNMENT_ROADMAP W2.
    "ast": {
        "description": "Structural code search/edit via ast-grep patterns",
        "tools": ["ast_grep", "ast_edit"],
        "includes": [],
    },

    # DAP debug subset (debugpy). check_fn hides without debugpy / when disabled.
    # See CODING_CAPABILITY_ALIGNMENT_ROADMAP W3.
    "debug": {
        "description": "Python DAP debugging via debugpy (launch, breakpoints, step)",
        "tools": ["debug"],
        "includes": [],
    },

    "coding": {
        # W4-S2: browser_* demoted from the focus posture set (local agent.log:
        # ~1 browser_* vs hundreds of read_file/lsp). Auto coding still gets
        # browser via hairball-cli/_HAIRBALL_CORE_TOOLS. Re-enable with the
        # ``browser`` toolset / ``hairball tools``.
        "description": "Coding-focused toolset: files, terminal, search, web docs, skills, todo, delegate, vision, lsp, ast, debug",
        "tools": [
            "web_search", "web_extract",
        ...
        # non-configurable-toolset recovery loop in hairball_cli/tools_config.py).
        "posture": True,
    },
    
    # ==========================================================================
    # Full Hairball toolsets (CLI + messaging platforms)
    #
    ...
    # the `hairball send` CLI), not by the model deciding to send on its own.
    # ==========================================================================

    "hairball-acp": {
        # W4-S2: same browser demotion as ``coding`` (lean editor schema).
        "description": "Editor integration (VS Code, Zed, JetBrains) — coding-focused tools without messaging, audio, clarify UI, or browser",
        "tools": [
            "web_search", "web_extract",
            "terminal", "process",
            "read_file", "write_file", "patch", "search_files",
    ...
    "hairball-cli": {
        ...
        "tools": _HAIRBALL_CORE_TOOLS,
        "includes": []
    },

    "hairball-cron": {
        # Mirrors hairball-cli so cron's "default" toolset is the same set of
        # core tools users see interactively — then `hairball tools` filters
        # them down per the platform config. _DEFAULT_OFF_TOOLSETS (moa,
        # homeassistant) are excluded by _get_platform_tools() unless
        # the user explicitly enables them.
        "description": "Default cron toolset - same core tools as hairball-cli; gated by `hairball tools`",
        "tools": _HAIRBALL_CORE_TOOLS,
        "includes": []
    },
    ...
    "hairball-bluebubbles": {
        "description": "BlueBubbles iMessage bot toolset - Apple iMessage via local BlueBubbles server",
        "tools": _HAIRBALL_CORE_TOOLS,
        "includes": []
    },

    "hairball-homeassistant": {
    ...
    "hairball-email": {
        "description": "Email bot toolset - interact with Hairball via email (IMAP/SMTP)",
        "tools": _HAIRBALL_CORE_TOOLS,
        "includes": []
    },
    ...
    "hairball-sms": {
        "description": "SMS bot toolset - interact with Hairball via SMS (Twilio)",
        "tools": _HAIRBALL_CORE_TOOLS,
        "includes": []
    },
    ...
    "hairball-webhook": {
        ...
        "tools": _HAIRBALL_WEBHOOK_SAFE_TOOLS,
        "includes": []
    },
    ...
    "hairball-gateway": {
        ...
        "tools": [],
        "includes": ["hairball-telegram", "hairball-discord", "hairball-whatsapp", "hairball-slack", "hairball-signal", "hairball-bluebubbles", "hairball-homeassistant", "hairball-email", "hairball-sms", "hairball-mattermost", "hairball-matrix", "hairball-dingtalk", "hairball-feishu", "hairball-wecom", "hairball-wecom-callback", "hairball-weixin", "hairball-qqbot", "hairball-webhook", "hairball-yuanbao"]
    }
}
...
def bundle_non_core_tools(toolset_name: str) -> Set[str]:
    """Return a ``hairball-*`` bundle's platform-specific tools, excluding core.

    Platform bundles are defined as ``_HAIRBALL_CORE_TOOLS + [platform extras]``.
    ...
    Bundle nesting is one level deep in practice (only ``hairball-gateway``
...
def resolve_toolset(name: str, visited: Set[str] = None, *, include_registry: bool = True) -> List[str]:
    ...
    if not toolset:
        # Auto-generate a toolset for plugin platforms (hairball-<name>).
        # Gives them _HAIRBALL_CORE_TOOLS plus any tools the plugin registered
        ...
        if include_registry and name.startswith("hairball-"):
            platform_name = name[len("hairball-"):]
            try:
                from gateway.platform_registry import platform_registry
                if platform_registry.is_registered(platform_name):
                    plugin_tools = set(_HAIRBALL_CORE_TOOLS)
                    try:
                        from tools.registry import registry
                        plugin_tools.update(
