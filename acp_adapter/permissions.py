"""ACP permission bridging for Hairball dangerous-command approvals."""
...
# Maps ACP permission option ids to Hairball approval result strings.
...
def _build_permission_options(
    *, allow_permanent: bool, smart_denied: bool = False,
    ...
    """Return ACP options that match Hairball approval semantics."""
    ...
    if not smart_denied:
        options.append(PermissionOption(
            ...
            # hint while keeping Hairball semantics in the option id.
            kind="allow_always",
...
def _map_outcome_to_hairball(outcome: object, *, allowed_option_ids: set[str]) -> str:
    """Map an ACP permission outcome into Hairball approval strings."""
    ...
    if option_id not in allowed_option_ids:
        ...
        return "deny"
    return _OPTION_ID_TO_HAIRBALL.get(option_id, "deny")
...
def make_approval_callback(
    ...
    """
    Return a Hairball-compatible approval callback that bridges to ACP.
    ...
    def _callback(
        ...
        **_: object,
        ...
        return _map_outcome_to_hairball(
            response.outcome,
