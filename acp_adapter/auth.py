"""ACP auth helpers — detect and advertise Hairball authentication methods."""
...
TERMINAL_SETUP_AUTH_METHOD_ID = "hairball-setup"


def detect_provider() -> Optional[str]:
    """Resolve the active Hairball runtime provider, or None if unavailable.
...
def has_provider() -> bool:
    """Return True if Hairball can resolve any runtime provider credentials."""
    return detect_provider() is not None


def build_auth_methods() -> list[Any]:
    """Return registry-compatible ACP auth methods for Hairball.
    ...
    not have Hairball provider credentials configured yet, so Hairball always
    ...
    if provider:
        methods.append(
            AuthMethodAgent(
                ...
                description=(
                    "Authenticate Hairball using the currently configured "
    ...
    methods.append(
        TerminalAuthMethod(
            ...
            name="Configure Hairball provider",
            description=(
                "Open Hairball' interactive model/provider setup in a terminal. "
                "Use this when Hairball has not been configured on this machine yet."
            ),
            type="terminal",
            args=["--setup"],
        )
