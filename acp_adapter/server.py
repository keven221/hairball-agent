"""ACP agent server — exposes Hairball Agent via the Agent Client Protocol."""
...
from tools.approval import (
    reset_hairball_interactive_context,
    set_hairball_interactive_context,
)

logger = logging.getLogger(__name__)

try:
    from hairball_cli import __version__ as HAIRBALL_VERSION
except Exception:
    HAIRBALL_VERSION = "0.0.0"
...
def _path_from_file_uri(uri: str) -> Path | None:
    ...
    /mnt/<drive>/... so Hairball running in WSL can read it.
    """
    raw = (uri or "").strip()
    if not raw:
        return None
    ...
    if path is None:
        return [{
            ...
            "text": _format_resource_text(
                ...
                name=name,
                title=title,
                body="[Resource link only; Hairball cannot read non-file ACP resource URIs directly.]",
            ),
        }]
...
def _content_blocks_to_openai_user_content(
    ...
    """Convert ACP prompt blocks into a Hairball/OpenAI-compatible user content payload."""
...
class HairballACPAgent(acp.Agent):
    """ACP Agent implementation wrapping Hairball AIAgent."""
    ...
    _ADVERTISED_COMMANDS = (
        ...
        {
            "name": "inject",
            "description": "Write a next-step note without starting a turn",
            "input_hint": "note for the next step",
        },
        {
            "name": "version",
            "description": "Show Hairball version",
        },
    )
    ...
    def _session_modes(self, state: SessionState) -> SessionModeState:
        ...
        model picker was visible. Claude/reference expose policy-like controls as ACP
        modes, which coexist with the model picker, so Hairball maps edit approval
    ...
    def _build_model_state(self, state: SessionState) -> SessionModelState | None:
        ...
        try:
            from hairball_cli.models import curated_models_for_provider, normalize_provider, provider_label
    ...
    def _resolve_model_selection(raw_model: str, current_provider: str) -> tuple[str, str]:
        ...
        try:
            from hairball_cli.models import detect_provider_for_model, parse_model_input
    ...
    def _build_usage_update(state: SessionState) -> UsageUpdate | None:
        ...
        the current request pressure.  Hairball estimates ``used`` from the same
    ...
    def _provenance_meta(
        self,
        acp_session_id: str,
        current_hairball_session_id: str,
        previous_hairball_session_id: Optional[str] = None,
    ) -> Optional[dict]:
        """Best-effort ``_meta.hairball.sessionProvenance`` for an ACP session."""
        try:
            return session_provenance_meta(
                self.session_manager._get_db(),
                acp_session_id,
                current_hairball_session_id,
                previous_hairball_session_id=previous_hairball_session_id,
            )
        except Exception:
            logger.debug(
    ...
    def _completion_recovery_meta(
        self,
        session_id: str,
        base_meta: Optional[dict],
    ) -> Optional[dict]:
        """Merge durable completion state into ACP metadata on old-session load."""
        try:
            assessment = (
                self.session_manager._get_db()
                .read_latest_terminal_completion_assessment_for_lineage(session_id)
            )
        except Exception:
            logger.debug(
                "Could not read ACP completion assessment for %s",
                session_id,
                exc_info=True,
            )
            assessment = None
        ...
        hairball = dict(meta.get("hairball") or {})
        hairball["completionAssessment"] = dict(assessment)
        meta["hairball"] = hairball
        return meta
    ...
    async def _send_session_info_update(
        ...
        session_id: str,
        *,
        current_hairball_session_id: Optional[str] = None,
        previous_hairball_session_id: Optional[str] = None,
    ) -> None:
        """Send ACP native session metadata after Hairball changes it.

        When the internal Hairball head rotated (e.g. compression-driven session
        split during a turn), pass ``previous_hairball_session_id`` so the
        attached ``_meta.hairball.sessionProvenance`` flags the rotation reason.
        """
        if not self._conn:
            return
        try:
        ...
        # hairball_state.py schema — only started_at/ended_at). Use "now" as
        ...
        meta = self._provenance_meta(
            session_id,
            current_hairball_session_id or session_id,
            previous_hairball_session_id,
        )
    ...
    async def _register_session_mcp_servers(
        ...
        try:
            ...
            enabled_toolsets = _expand_acp_enabled_toolsets(
                getattr(state.agent, "enabled_toolsets", None) or ["hairball-acp"],
    ...
    async def initialize(
        ...
        **kwargs: Any,
        ...
        return InitializeResponse(
            ...
            agent_info=Implementation(name="hairball-agent", version=HAIRBALL_VERSION),
    ...
    async def authenticate(self, method_id: str, **kwargs: Any) -> AuthenticateResponse | None:
        ...
        # Hairball' threat model (ACP is stdio-only, local-trust), but poor
        ...
        if normalized_method == TERMINAL_SETUP_AUTH_METHOD_ID:
            # Terminal auth launches Hairball setup/model selection out-of-band.
    ...
    async def _replay_session_history(self, state: SessionState) -> None:
        ...
        notifications. Merely restoring server-side state makes Hairball
    ...
    async def load_session(
        ...
        **kwargs: Any,
        ...
        # lifetime. Awaiting the replay here matches reference / Claude Code /
        ...
        return LoadSessionResponse(
            models=self._build_model_state(state),
            modes=self._session_modes(state),
            field_meta=self._completion_recovery_meta(
                session_id,
    ...
    async def prompt(
        ...
        **kwargs: Any,
        ...
        """Run Hairball on the user's prompt and stream events back to the editor."""
        state = self.session_manager.get_session(session_id)
        if state is None:
        ...
        # ACP thought panes should not receive Hairball' local kawaii waiting/status
        ...
        # tools.approval (set_hairball_interactive_context) rather than
        # os.environ["HAIRBALL_INTERACTIVE"], so concurrent executor workers can't
        ...
        # callback shape — not the gateway-queue HAIRBALL_EXEC_ASK path,
        ...
        def _run_agent() -> dict:
            ...
            # Bind HAIRBALL_SESSION_KEY for this session so per-session caches
            ...
            interactive_token = set_hairball_interactive_context(True)
            ...
            previous_session_id = os.environ.get("HAIRBALL_SESSION_ID")
            os.environ["HAIRBALL_SESSION_ID"] = session_id
            try:
                with state.runtime_lock:
                    parked = [str(item).strip() for item in list(state.next_step) if str(item).strip()]
                    state.next_step.clear()
                for note in parked:
                    try:
                        if callable(getattr(agent, "inject", None)):
                            agent.inject(note)
                    except Exception:
                        logger.debug("ACP parked next-step apply failed", exc_info=True)
                ...
                if interactive_token is not None:
                    reset_hairball_interactive_context(interactive_token)
                # Restore HAIRBALL_SESSION_ID symmetrically.
                if previous_session_id is None:
                    os.environ.pop("HAIRBALL_SESSION_ID", None)
                else:
                    os.environ["HAIRBALL_SESSION_ID"] = previous_session_id
                if approval_cb:
                    try:
                        from tools import terminal_tool as _terminal_tool
        ...
        try:
            # Snapshot the internal Hairball DB session id before the turn so we
            ...
            pre_turn_hairball_id = getattr(state.agent, "session_id", None)
            # Wrap the executor call in a fresh copy of the current context so
            # concurrent ACP sessions on the shared ThreadPoolExecutor don't
            # stomp on each other's ContextVar writes (HAIRBALL_SESSION_KEY in
        ...
        # _meta.hairball.sessionProvenance so ACP clients can render the boundary
        # and keep old/new ids in lineage. The ACP session_id is unchanged.
        post_turn_hairball_id = getattr(state.agent, "session_id", None)
        if (
            conn
            and post_turn_hairball_id
            and pre_turn_hairball_id
            and post_turn_hairball_id != pre_turn_hairball_id
        ):
            try:
                await self._send_session_info_update(
                    session_id,
                    current_hairball_session_id=post_turn_hairball_id,
                    previous_hairball_session_id=pre_turn_hairball_id,
                )
            except Exception:
                logger.debug(
        ...
        # Hairball' local "waiting for model response" interrupt status is metadata,
        ...
        if leftover:
            ...
            if agent is not None and callable(getattr(agent, "inject", None)):
                try:
                    parked = bool(agent.inject(leftover))
                except Exception:
                    logger.debug("ACP leftover inject failed", exc_info=True)
            if not parked:
                with state.runtime_lock:
                    state.next_step.append(leftover)
        ...
        assessment = result.get("completion_assessment")
        field_meta = (
            {"hairball": {"completionAssessment": dict(assessment)}}
    ...
    def _cmd_inject(self, args: str, state: SessionState) -> str:
        inject_text = args.strip()
        if not inject_text:
            return "Usage: /inject <note>"
        agent = state.agent
        if agent is not None and callable(getattr(agent, "inject", None)):
            try:
                if agent.inject(inject_text):
                    preview = inject_text[:80] + ("..." if len(inject_text) > 80 else "")
                    return f"Next-step note parked (does not start a turn): {preview}"
            except Exception as exc:
                logger.warning("ACP inject failed for session %s: %s", state.session_id, exc)
                return f"⚠️ Inject failed: {exc}"
        with state.runtime_lock:
            state.next_step.append(inject_text)
            depth = len(state.next_step)
        preview = inject_text[:80] + ("..." if len(inject_text) > 80 else "")
        return f"Next-step note parked (does not start a turn). ({depth} parked): {preview}"

    def _cmd_version(self, args: str, state: SessionState) -> str:
        return f"Hairball Agent v{HAIRBALL_VERSION}"
    ...
    async def set_config_option(
        ...
        """Accept ACP config option updates even when Hairball has no typed ACP config surface yet."""
        state = self.session_manager.get_session(session_id)
        if state is None:
