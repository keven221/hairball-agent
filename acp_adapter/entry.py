"""CLI entry point for the hairball-agent ACP adapter.

Loads environment variables from ``~/.hairball/.env``, configures logging
...
try:
    ...
    # shadowing Hairball's own modules — ``hairball acp`` can be started from any
    # cwd, including a project that has same-named packages on its path.
    hairball_bootstrap.harden_import_path()
...
def _load_env() -> None:
    """Load .env from HAIRBALL_HOME (default ``~/.hairball``)."""
    from hairball_cli.env_loader import load_hairball_dotenv

    hairball_home = get_hairball_home()
    loaded = load_hairball_dotenv(hairball_home=hairball_home)
    if loaded:
        for env_file in loaded:
            logging.getLogger(__name__).info("Loaded env from %s", env_file)
    else:
        logging.getLogger(__name__).info(
            "No .env found at %s, using system env", hairball_home / ".env"
        )
...
def _parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        prog="hairball-acp",
        description="Run Hairball Agent as an ACP stdio server.",
    )
    parser.add_argument("--version", action="store_true", help="Print Hairball version and exit")
    parser.add_argument(
        "--check",
        action="store_true",
    ...
    parser.add_argument(
        "--setup",
        action="store_true",
        help="Run interactive Hairball provider/model setup for ACP terminal auth",
    )
    parser.add_argument(
        "--setup-browser",
        action="store_true",
        help="Install agent-browser + Playwright Chromium into ~/.hairball/node/ "
...
def _print_version() -> None:
    ...
    print(hairball_version)


def _run_check() -> None:
    import acp  # noqa: F401
    from acp_adapter.server import HairballACPAgent  # noqa: F401

    print("Hairball ACP check OK")


def _run_setup() -> None:
    from hairball_cli.main import main as hairball_main

    old_argv = sys.argv[:]
    try:
        sys.argv = [old_argv[0] if old_argv else "hairball", "model"]
        hairball_main()
    finally:
        sys.argv = old_argv
...
def _run_setup_browser(assume_yes: bool = False) -> int:
    ...
    with ``hairball postinstall`` and the runtime lazy installer.
...
def main(argv: list[str] | None = None) -> None:
    ...
    logger.info("Starting hairball-agent ACP adapter")
    ...
    import acp
    from .server import HairballACPAgent
