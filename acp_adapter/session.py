"""ACP session manager — maps ACP sessions to Hairball AIAgent instances.

Sessions are persisted to the shared SessionDB (``~/.hairball/state.db``) so they
...
def _translate_acp_cwd(cwd: str) -> str:
    """Translate Windows ACP cwd values when Hairball itself is running in WSL.

    Windows ACP clients can launch ``hairball acp`` inside WSL while still sending
...
def _normalize_cwd_for_compare(cwd: str | None) -> str:
    ...
    from hairball_constants import windows_path_to_wsl
...
def _register_task_cwd(task_id: str, cwd: str) -> None:
    ...
    Zed can launch Hairball from a Windows workspace while the ACP process runs
    ...
    try:
        ...
        register_task_env_overrides(task_id, {"cwd": translated})
        try:
            ...
            if translated:
                ensure_coding_workspace_loaded(task_id, seed_cwd=translated)
                set_active_coding_task_id(task_id)
        except Exception:
            logger.debug("Failed to seed ACP coding workspace", exc_info=True)
    except Exception:
...
def _expand_acp_enabled_toolsets(
    ...
    expanded: List[str] = []
    for name in list(toolsets or ["hairball-acp"]):
...
def _clear_task_cwd(task_id: str) -> None:
    ...
    try:
        from agent.coding_workspace import clear_coding_workspace

        clear_coding_workspace(task_id)
    except Exception:
        logger.debug("Failed to clear ACP coding workspace", exc_info=True)


@dataclass
class SessionState:
    """Tracks per-session state for an ACP-managed Hairball agent."""
...
class SessionManager:
    """Thread-safe manager for ACP sessions backed by Hairball AIAgent instances.
    ...
    def __init__(self, agent_factory=None, db=None):
        ...
        Args:
            agent_factory: Optional callable that creates an AIAgent-like object.
                           ...
                           using the current Hairball runtime provider configuration.
            db:            Optional SessionDB instance. When omitted, the default
                           SessionDB (``~/.hairball/state.db``) is lazily created.
        """
    ...
    def _get_db(self):
        ...
        Note: we resolve ``HAIRBALL_HOME`` dynamically rather than relying on
        ...
        later (e.g. by the test fixture ``_isolate_hairball_home``).
        """
        ...
        try:
            ...
            self._db_instance = SessionDB(db_path=hairball_home / "state.db")
            return self._db_instance
        except Exception:
    ...
    def _make_agent(
        ...
        *,
        ...
        # reference app-server sessions are spawned lazily on the first turn. Stamp
        # the ACP workspace onto the agent so the reference runtime starts from the
        # editor/session cwd instead of the Hairball daemon's process cwd.
