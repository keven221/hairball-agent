"""ACP tool-call helpers for mapping hairball tools to ACP ToolKind and building content."""
...
# ---------------------------------------------------------------------------
# Map hairball tool names -> ACP ToolKind
# ---------------------------------------------------------------------------
...
def get_tool_kind(tool_name: str) -> ToolKind:
    """Return the ACP ToolKind for a hairball tool, defaulting to 'other'."""
...
def _tool_result_failed(result: Optional[str], tool_name: str | None = None) -> bool:
    """Return True when a structured Hairball tool result clearly failed.
    ...
    # Hairball core/polished tools commonly report tool-level failures as a
...
def _format_read_file_result(result: Optional[str], args: Optional[Dict[str, Any]]) -> Optional[str]:
    ...
    # Hairball read_file output is line-numbered with `|`. If we send it as raw
...
def _format_search_files_result(result: Optional[str]) -> Optional[str]:
    ...
    if isinstance(files, list):
        ...
        if truncated:
            ...
            continuation = (
                f" Continue with offset={next_offset}. You can also narrow "
                "the search with path/file_glob."
                if isinstance(next_offset, int)
                else " Narrow the search, add path/file_glob, or use offset to page."
            )
            lines.extend([
                "",
                "Results truncated." + continuation,
            ])
    ...
    if truncated:
        ...
        continuation = (
            f" Continue with offset={next_offset}. You can also narrow "
            "the search with file_glob."
            if isinstance(next_offset, int)
            else " Narrow the search, add file_glob, or use offset to page."
        )
        lines.extend([
            "",
            "Results truncated." + continuation,
...
def build_tool_start(
    ...
    *,
    ...
    """Create a ToolCallStart event for the given hairball tool invocation."""
