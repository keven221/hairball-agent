...
# Audio file extensions Hairball recognizes for native audio delivery.
...
def _thread_metadata_for_source(source, reply_to_message_id: str | None = None) -> dict | None:
    ...
    value. Telegram private-chat topics created through Hairball' DM-topic helper
...
def _reply_anchor_for_event(event) -> str | None:
    ...
    replying to the triggering message. Hairball-created Telegram private-chat
...
from hairball_constants import get_default_hairball_root, get_hairball_dir, get_hairball_home


@dataclass
class AudioFormat:
    """Raw little-endian PCM contract for one streaming speech session."""
...
class StreamingTTSHandle:
    """Adapter-owned streaming speech handle with shared lifecycle flags."""
...
def streaming_tts_turn_key(
    session_key: str | None,
    turn_marker: Any = None,
    *,
    event: Any = None,
) -> str | None:
    """Build a turn-scoped key so concurrent chats never suppress each other."""
    if not session_key:
        return None
    if turn_marker is None and event is not None:
        turn_marker = getattr(event, "message_id", None) or getattr(
...
def streaming_tts_should_skip_whole_file(
    ...
    *,
    ...
    """Return whether this exact turn already delivered streaming audio."""
...
GATEWAY_SECRET_CAPTURE_UNSUPPORTED_MESSAGE = (
    ...
    "Load this skill in the local CLI to be prompted, or add the key to ~/.hairball/.env manually."
)
...
IMAGE_CACHE_DIR = get_hairball_dir("cache/images", "image_cache")


def _resolve_cache_dir(constant_name: str, new_subpath: str, old_name: str) -> Path:
    """Resolve fresh via get_hairball_dir (active profile), unless a test has
    monkeypatched the constant away from its import-time default."""
    fresh = get_hairball_dir(new_subpath, old_name)
        ...
        for attempt in range(retries + 1):
            try:
                async with client.stream(
                    ...
                    url,
                    headers={
                        "User-Agent": "Mozilla/5.0 (compatible; HairballAgent/1.0)",
                        "Accept": "image/*,*/*;q=0.8",
                    },
                ) as response:
...
AUDIO_CACHE_DIR = get_hairball_dir("cache/audio", "audio_cache")
...
VIDEO_CACHE_DIR = get_hairball_dir("cache/videos", "video_cache")
...
DOCUMENT_CACHE_DIR = get_hairball_dir("cache/documents", "document_cache")
SCREENSHOT_CACHE_DIR = get_hairball_dir("cache/screenshots", "browser_screenshots")
...
_HAIRBALL_ROOT = get_default_hairball_root()
MEDIA_DELIVERY_ALLOW_DIRS_ENV = "HAIRBALL_MEDIA_ALLOW_DIRS"
MEDIA_DELIVERY_TRUST_RECENT_ENV = "HAIRBALL_MEDIA_TRUST_RECENT_FILES"
MEDIA_DELIVERY_TRUST_RECENT_SECONDS_ENV = "HAIRBALL_MEDIA_TRUST_RECENT_SECONDS"
...
MEDIA_DELIVERY_STRICT_ENV = "HAIRBALL_MEDIA_DELIVERY_STRICT"
MEDIA_DELIVERY_SAFE_ROOTS = (
    ...
    _HAIRBALL_HOME / "image_cache",
    _HAIRBALL_HOME / "audio_cache",
    _HAIRBALL_HOME / "video_cache",
    _HAIRBALL_HOME / "document_cache",
    _HAIRBALL_HOME / "browser_screenshots",
    ...
    _HAIRBALL_HOME / "cache" / "images",
    _HAIRBALL_HOME / "cache" / "audio",
    _HAIRBALL_HOME / "cache" / "videos",
    _HAIRBALL_HOME / "cache" / "documents",
    _HAIRBALL_HOME / "cache" / "screenshots",
)
...
def _profile_cache_roots() -> List[Path]:
    """Return per-profile canonical cache roots under the shared Hairball root.

    Profile gateways write generated artifacts to
    ``<root>/profiles/<name>/cache/{images,audio,...}``. The static safe-roots
    list only covers the *active* HAIRBALL_HOME's cache, so a gateway running at
    the root (e.g. ``HAIRBALL_HOME=/opt/data``) while the model emits a
    ...
    is consulted (which otherwise wins when HAIRBALL_HOME is symlinked under a
    denied prefix and $HOME is not that prefix). See issue #31733.
    """
    roots: List[Path] = []
    profiles_dir = _HAIRBALL_ROOT / "profiles"
    try:
...
def _kanban_attachment_roots() -> List[Path]:
    ...
    home_override = os.environ.get("HAIRBALL_KANBAN_HOME", "").strip()
    root = Path(home_override).expanduser() if home_override else _HAIRBALL_ROOT
...
def _media_delivery_denied_paths() -> List[Path]:
    ...
    # The active Hairball profile and shared Hairball root both contain control
    ...
    # HAIRBALL_HOME root. The set mirrors the canonical read guard in
    ...
    # tree, so skills/, logs/, and ad-hoc agent-written files under ~/.hairball
    ...
    for hairball_root in (_HAIRBALL_HOME, _HAIRBALL_ROOT):
        for rel in _ROOT_CREDENTIAL_FILES:
            denied.append(hairball_root / rel)
...
def _path_under_denied_prefix(resolved: Path) -> bool:
    ...
    sub-directories inside home (``~/.ssh``, ``~/.aws``, ...) and Hairball
    secrets (``~/.hairball/.env``, ``auth.json``) are *separate, more-specific*
...
def validate_media_delivery_path(path: str) -> Optional[str]:
    ...
    ``HAIRBALL_MEDIA_DELIVERY_STRICT=1``): the file MUST live under a
    Hairball-managed cache, under an operator-allowlisted root
    (``HAIRBALL_MEDIA_ALLOW_DIRS``), or be freshly produced inside the
    ...
    # credential/secret stores under the Hairball root (~/.hairball/.env,
    ...
    # ``MEDIA:~/.hairball/google_token.json``) remain rejected.
...
_PLAINTEXT_GATEWAY_RESTART_PATTERNS: tuple[re.Pattern[str], ...] = (
    ...
    re.compile(r"^(?:please\s+)?restart\s+(?:the\s+)?hairball\s+gateway[.!?\s]*$", re.IGNORECASE),
    re.compile(r"^(?:please\s+)?restart\s+hairball[.!?\s]*$", re.IGNORECASE),
)
...
def merge_pending_message_event(
    ...
    *,
    ...
    if existing:
        merge_event_message_ids(existing, event)
...
def merge_event_message_ids(target: MessageEvent, incoming: MessageEvent) -> None:
    """Carry durable inbound identities across a platform/base batch merge.

    The implementation lives beside the ledger; this import-safe wrapper keeps
    adapters depending only on their existing ``gateway.platforms.base`` seam.
    Dedupe is best-effort, so metadata bookkeeping can never block a message.
    """
    try:
        from gateway.inbound_ledger import merge_event_message_ids as _merge

        _merge(target, incoming)
    except Exception:
        logger.debug("[%s] inbound identity merge failed", _platform_name(getattr(getattr(target, "source", None), "platform", None)), exc_info=True)
...
class BasePlatformAdapter(ABC):
    ...
    # into the ``HAIRBALL_SESSION_ASYNC_DELIVERY`` contextvar at session-bind
    ...
    # Hairball commands.  Default "/" (most platforms deliver "/approve" etc.
    ...
    def __init__(self, config: PlatformConfig, platform: Platform):
        ...
        self._busy_text_mode: str = (
            os.environ.get("HAIRBALL_GATEWAY_BUSY_TEXT_MODE", "interrupt").strip().lower()
            or "interrupt"
        )
        self._busy_text_debounce_seconds: float = _float_env(
            "HAIRBALL_GATEWAY_BUSY_TEXT_DEBOUNCE_SECONDS", 0.35
        )
        self._busy_text_hard_cap_seconds: float = _float_env(
            "HAIRBALL_GATEWAY_BUSY_TEXT_HARD_CAP_SECONDS", 1.0
        )
    ...
    def prefers_fresh_final_streaming(
        ...
        implementation supports. Telegram is the motivating case: Hairball sends
    ...
    async def send_voice(
        ...
        **kwargs,
        ...
        host filesystem path that would leak the Hairball home layout.
        """
    ...
    def prepare_tts_text(self, text: str) -> str:
        """Prepare a spoken script without changing the persisted reply."""
        try:
    ...
    # Realtime voice adapters override this PCM contract. Existing messaging
    # adapters remain on the whole-file path by returning False.
    def supports_streaming_tts(
    ...
    def _mark_streaming_tts_completed_turn(
        ...
        *,
        ...
        turn_key = self._streaming_tts_turn_key(
            session_key, turn_marker, event=event
        )
    ...
    async def send_video(
        ...
        **kwargs,
        ...
        would leak the Hairball home layout.
        """
    ...
    def _mask_json_string_media(content: str) -> str:
        ...
        Serialized tool results frequently embed a previous reply's text, e.g.::
            ...
            {"result": "MEDIA:/Users/x/.hairball/media/generated/stale.png"}
    ...
    async def _dispatch_active_session_command(
        ...
        _inbound_mark = self._start_inbound_processing_mark(event)
        try:
            response = await self._message_handler(event)
            await self._settle_inbound_processing_mark(_inbound_mark)
            ...
            try:
                from gateway.inbound_ledger import finish_inbound_event

                await asyncio.to_thread(
                    finish_inbound_event, event, ProcessingOutcome.SUCCESS,
                )
            except Exception:
                logger.debug("[%s] inbound completion mark failed", self.name, exc_info=True)
        except Exception as exc:
            try:
                from gateway.inbound_ledger import finish_inbound_event
                ...
                logger.debug("[%s] inbound failure mark failed", self.name, exc_info=True)
    ...
    async def handle_message(self, event: MessageEvent) -> None:
        ...
        # Durable restart-boundary dedupe. Adapter-local TTL caches prevent
        # hot duplicates on several platforms, but they forget on process
        # restart and are not implemented by every adapter. Claim only stable
        # platform message IDs; events without one retain historical behavior.
        # Any ledger failure fails open so this reliability layer can never
        # become a message-loss gate.
        try:
            from gateway.inbound_ledger import claim_inbound_event

            _inbound_claim = await asyncio.to_thread(claim_inbound_event, event)
        except Exception:
            logger.warning(
                "[%s] inbound ledger claim failed; processing event without dedupe",
                self.name,
                exc_info=True,
            )
            _inbound_claim = None
        if _inbound_claim is not None and not _inbound_claim.should_process:
            logger.info(
                "[%s] Dropping durable duplicate inbound event (message_id=%s)",
                self.name,
        ...
        if session_key in self._active_sessions:
            ...
            from hairball_cli.commands import should_bypass_active_session
    ...
    def _start_inbound_processing_mark(
        self, event: MessageEvent,
    ) -> Optional[asyncio.Task]:
        """Persist ``claimed -> processing`` beside, not ahead of, the handler."""
        try:
            from gateway.inbound_ledger import (
                has_inbound_claim,
                mark_inbound_processing,
            )

            if not has_inbound_claim(event):
                return None
            task = asyncio.create_task(asyncio.to_thread(mark_inbound_processing, event))

            def _consume_mark_result(done: asyncio.Task) -> None:
                if done.cancelled():
                    return
                try:
                    done.result()
                    ...
                    logger.debug(
                        "[%s] inbound processing mark failed",
                        self.name,
                        exc_info=True,
                    )

            task.add_done_callback(_consume_mark_result)
            return task
        except Exception:
            logger.debug("[%s] inbound processing mark failed", self.name, exc_info=True)
            return None

    async def _settle_inbound_processing_mark(
        ...
        try:
            ...
            # The done callback records the diagnostic. Dedupe correctness is
            # already established by the atomic claim in handle_message().
            pass
    ...
    def _get_human_delay() -> float:
        ...
        Reads from env vars:
          HAIRBALL_HUMAN_DELAY_MODE: "off" (default) | "natural" | "custom"
          HAIRBALL_HUMAN_DELAY_MIN_MS: minimum delay in ms (default 800, custom mode)
          HAIRBALL_HUMAN_DELAY_MAX_MS: maximum delay in ms (default 2500, custom mode)
        """
        mode = os.getenv("HAIRBALL_HUMAN_DELAY_MODE", "off").lower()
        if mode == "off":
            return 0.0
        if mode == "natural":
        ...
        try:
            min_ms = int(os.getenv("HAIRBALL_HUMAN_DELAY_MIN_MS", "800"))
        except (TypeError, ValueError):
            min_ms = 800
        try:
            max_ms = int(os.getenv("HAIRBALL_HUMAN_DELAY_MAX_MS", "2500"))
        except (TypeError, ValueError):
            max_ms = 2500
        return random.uniform(min_ms / 1000.0, max_ms / 1000.0)
    ...
    async def _process_message_background(self, event: MessageEvent, session_key: str) -> None:
        ...
        try:
            # Preserve the established lifecycle ordering: platform reaction /
            # processing hooks fire on the task's first scheduling slice.  The
            # durable mark follows immediately, but must not insert a thread
            # hop before on_processing_start (fast cancellation relies on it).
            ...
            if response:
                ...
                if text_content and not _tts_caption_delivered:
                    ...
                    # output. Best-effort at every step — ledger trouble must
                    # never block or delay the actual send. Slash-command and
                    # ephemeral replies are cheap to regenerate and are not
                    # recorded.
                    _obligation_id = None
            ...
            try:
                ...
                await asyncio.to_thread(finish_inbound_event, event, outcome)
            except Exception:
                logger.debug("[%s] inbound cancellation mark failed", self.name, exc_info=True)
            raise
        except Exception as e:
            ...
            # not at the top of this task.  _hairball_run_generation is set on
            ...
            _callback_generation = getattr(
                interrupt_event,
                "_hairball_run_generation",
                None,
            )
            completed_audio_key = self._streaming_tts_turn_key(
                session_key,
                _callback_generation,
                event=event,
            )
            if completed_audio_key:
                completed_audio_turns = getattr(
                    self, "_streaming_tts_completed_turns", None
                )
                if completed_audio_turns is not None:
                    completed_audio_turns.discard(completed_audio_key)
