...
class WhatsAppBehaviorMixin:
    ...
    DEFAULT_REPLY_PREFIX: str = "⚕ *Hairball Agent*\n────────────\n"
...
def resolve_whatsapp_bridge_dir() -> Path:
    """Resolve the WhatsApp bridge directory, mirroring to HAIRBALL_HOME if needed.

    When the install tree is read-only (e.g., Docker /opt/hairball), this function
    mirrors the bridge source to a writable HAIRBALL_HOME location and returns that
    ...
    # Try HAIRBALL_HOME location first
    hairball_home = get_hairball_home()
    hairball_home_bridge = hairball_home / "scripts" / "whatsapp-bridge"
    ...
    # Install dir is read-only, mirror to HAIRBALL_HOME if needed
    if hairball_home_bridge.exists():
        return hairball_home_bridge

    # Mirror the bridge source to HAIRBALL_HOME
    try:
        hairball_home_bridge.parent.mkdir(parents=True, exist_ok=True)
        shutil.copytree(
            install_bridge,
            hairball_home_bridge,
            dirs_exist_ok=False,
        )
        return hairball_home_bridge
    except Exception:
