...
class QQAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # to ~/.hairball/.update_response. Set here so the cross-adapter gateway
    ...
    async def _open_ws(self, gateway_url: str) -> None:
        ...
        # Honor WSL proxy env for QQ WebSocket. Hairball upgrades overwrite this
    ...
    async def _listen_loop(self) -> None:
        ...
        Close code handling follows the legacy qqbot reference implementation:
    ...
    async def _send_identify(self) -> None:
        ...
        identify_payload = {
            ...
            "d": {
                ...
                "properties": {
                    "$os": "macOS",
                    "$browser": "hairball-agent",
                    "$device": "hairball-agent",
                },
            },
        }
        try:
    ...
    async def _default_interaction_dispatch(
        ...
        - ``update_prompt:<answer>`` →
          writes the answer to ``~/.hairball/.update_response`` for the
          detached ``hairball update --gateway`` process to consume.
    ...
    def _write_update_response(answer: str, operator: str = "") -> None:
        ...
        ``hairball update --gateway`` watcher polls this file for a ``y``/``n``
    ...
    async def _process_attachments(
        ...
        Mirrors legacy's ``processAttachments`` — handles images, voice, and
    ...
    def _resolve_stt_config(self) -> Optional[Dict[str, str]]:
        ...
        # 1. Plugin-specific STT config (matches legacy's channels.qqbot.stt)
        ...
        # 2. QQ-specific env vars (set by `hairball setup gateway` / `hairball gateway`)
    ...
    async def send_update_prompt(
        ...
        ``gateway/run.py``'s ``hairball update --gateway`` watcher. Button
        ...
        ``~/.hairball/.update_response`` so the detached update process
        can read it.
        """
