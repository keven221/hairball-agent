...
def _resolve_profile_path(path_value: Any) -> Optional[Path]:
    """Resolve a user path, mapping ~/.hairball to the active profile home."""
    ...
    if raw == "~/.hairball":
        return hairball_home
    if raw.startswith("~/.hairball/"):
        return hairball_home / raw.removeprefix("~/.hairball/")
    path = Path(raw).expanduser()
    if path.is_absolute():
        return path
    return hairball_home / path


def _resolve_script_path(script_value: Any) -> tuple[Optional[Path], Optional[str]]:
    """Resolve a route script under HAIRBALL_HOME/scripts."""
    if not isinstance(script_value, str) or not script_value.strip():
        return None, "script path is empty"
    from hairball_constants import get_hairball_home

    scripts_root = (get_hairball_home() / "scripts").resolve()
    raw_text = os.path.expandvars(script_value.strip())
    if raw_text == "~/.hairball" or raw_text.startswith("~/.hairball/"):
...
class WebhookRouteProcessor:
    ...
    def run_route_script(self, script_value: Any, payload: dict) -> tuple[bool, Optional[dict]]:
        ...
        if (
            ...
            or transformed.get("__hairball_ignore__") is True
        ):
            return False, None
