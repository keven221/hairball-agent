...
- POST /v1/chat/completions        — OpenAI Chat Completions format (stateless; opt-in session continuity via X-Hairball-Session-Id header; opt-in long-term memory scoping via X-Hairball-Session-Key header)
- POST /v1/responses               — OpenAI Responses API format (stateful via previous_response_id; X-Hairball-Session-Key supported)
- GET  /v1/responses/{response_id} — Retrieve a stored response
- DELETE /v1/responses/{response_id} — Delete a stored response
- GET  /v1/models                  — lists hairball-agent and any configured model_routes aliases
- GET  /v1/capabilities            — machine-readable API capabilities for external UIs
- GET  /api/sessions               — list client-visible Hairball sessions
- POST /api/sessions               — create an empty Hairball session
...
AnythingLLM, NextChat, ChatBox, etc.) can connect to hairball-agent
...
from gateway.run_event_hub import RunEventCursorExpired, RunEventHub, RunEventLagged
...
def _hairball_version() -> str:
    """Return the hairball-agent version string, or "dev" if it can't be resolved.

    Tries the installed package metadata first (authoritative for a pip/uv
    install), then the in-tree ``hairball_cli.__version__`` (covers editable /
...
HAIRBALL_APP_SERVER_PROTOCOL_NAME = "hairball-app-server"
HAIRBALL_APP_SERVER_PROTOCOL_VERSION = 1
HAIRBALL_APP_SERVER_MIN_PROTOCOL_VERSION = 1
    ...
    def __init__(self, max_size: int = MAX_STORED_RESPONSES, db_path: str = None):
        ...
        if db_path is None:
            try:
                ...
                db_path = str(get_hairball_home() / "response_store.db")
            except Exception:
                db_path = ":memory:"
        ...
        # gracefully on NFS/SMB/FUSE-mounted HAIRBALL_HOME (same filesystem
        # issue addressed for state.db/kanban.db — see
        # hairball_state._WAL_INCOMPAT_MARKERS).
...
def _plan_result_metadata(result: Any) -> Dict[str, Any]:
    """Project exact Plan execution facts onto the API transport.

    The remote host owns tool execution but not the gateway's approval ledger.
    Return only the canonical execution id, selected ids, and Todo-derived
    step statuses so the owning gateway can settle that ledger without
    inferring success from assistant prose.
    """

    if not isinstance(result, dict):
        return {}
    metadata: Dict[str, Any] = {}
    execution_id = str(result.get("plan_execution_id") or "").strip()
    if execution_id:
        metadata["plan_execution_id"] = execution_id

    selected = result.get("plan_selected_step_ids")
    if isinstance(selected, list):
        ...
        if selected_ids:
            metadata["plan_selected_step_ids"] = selected_ids

    raw_outcomes = result.get("plan_step_outcomes")
    if isinstance(raw_outcomes, list):
        ...
        for raw in raw_outcomes:
            ...
            if step_id and status:
                outcomes.append({"id": step_id, "status": status})
        if outcomes:
            metadata["plan_step_outcomes"] = outcomes
    return metadata
...
def _derive_chat_session_id(
    ...
    the same Hairball session (and therefore the same Docker container sandbox
...
class APIServerAdapter(BasePlatformAdapter):
    ...
    and routes them through hairball-agent's AIAgent.
    """
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Bounded canonical event journals. Every SSE client gets an independent
        # cursor; disconnecting one client never consumes or deletes another's
        # events. Agent/control lifetime remains owned by the fields below.
        self._run_event_hub = RunEventHub(max_events=2048, subscriber_queue_size=512)
    ...
    def _resolve_max_concurrent_runs() -> int:
        ...
        default = 10
        try:
            from hairball_cli.config import cfg_get, load_config
    ...
    @staticmethod
    def _check_protocol_version(request: "web.Request") -> Optional["web.Response"]:
        """Validate an explicitly requested Hairball App Server protocol version.

        Older clients send no version header and retain the existing behavior.
        New clients can fail early and deterministically instead of discovering
        an incompatible stream shape halfway through a run.
        """
        raw = request.headers.get("X-Hairball-Protocol-Version", "").strip()
        if not raw:
            return None
        try:
            requested = int(raw)
            ...
            return web.json_response(
                _openai_error(
                    "X-Hairball-Protocol-Version must be an integer",
                    code="invalid_protocol_version",
                ),
                status=400,
                headers={
                    "X-Hairball-Protocol-Version": str(
                        HAIRBALL_APP_SERVER_PROTOCOL_VERSION
                    )
                },
            )
        if (
            requested < HAIRBALL_APP_SERVER_MIN_PROTOCOL_VERSION
            or requested > HAIRBALL_APP_SERVER_PROTOCOL_VERSION
        ):
            payload = _openai_error(
                f"Unsupported Hairball App Server protocol version: {requested}",
                code="unsupported_protocol_version",
            )
            payload["error"]["supported_versions"] = [
                HAIRBALL_APP_SERVER_PROTOCOL_VERSION
            ]
            return web.json_response(
                payload,
    ...
    def _check_auth(self, request: "web.Request") -> Optional["web.Response"]:
        ...
        """
        protocol_error = self._check_protocol_version(request)
    ...
    def _parse_session_key_header(
        ...
        """Extract and validate the ``X-Hairball-Session-Key`` header.

        The session key is a stable per-channel identifier that scopes
        long-term memory (e.g. Honcho sessions) across transcripts.  It
        is independent of ``X-Hairball-Session-Id``: callers may send
        ...
        """
        raw = request.headers.get("X-Hairball-Session-Key", "").strip()
        if not raw:
            return None, None

        if not self._api_key:
            logger.warning(
                "X-Hairball-Session-Key rejected: no API key configured. "
                "Set API_SERVER_KEY to enable long-term memory scoping."
            )
            return None, web.json_response(
                _openai_error(
                    "X-Hairball-Session-Key requires API key authentication. "
    ...
    def _ensure_session_db(self):
        ...
        Sessions are persisted to ``state.db`` so that ``hairball sessions list``
    ...
    def _create_agent(
        ...
        gateway platforms), falling back to the hairball-api-server default.

        ``gateway_session_key`` is a stable per-channel identifier supplied
        by the client (via ``X-Hairball-Session-Key``).  Unlike ``session_id``
    ...
    async def _handle_health(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response(
            {"status": "ok", "platform": "hairball-agent", "version": _hairball_version()}
        )
    ...
    async def _handle_health_detailed(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            ...
            "readiness": readiness,
            "platform": "hairball-agent",
            "version": _hairball_version(),
    ...
    async def _handle_models(self, request: "web.Request") -> "web.Response":
        """GET /v1/models — list hairball-agent and any configured model_routes aliases."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_capabilities(self, request: "web.Request") -> "web.Response":
        ...
        every Hairball version exposes the same endpoints.
        """
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({
            "object": "hairball.api_server.capabilities",
            "platform": "hairball-agent",
            "model": self._model_name,
            "protocol": {
                "name": HAIRBALL_APP_SERVER_PROTOCOL_NAME,
                "version": HAIRBALL_APP_SERVER_PROTOCOL_VERSION,
                "min_compatible_version": HAIRBALL_APP_SERVER_MIN_PROTOCOL_VERSION,
                "request_header": "X-Hairball-Protocol-Version",
            },
            "auth": {
                "type": "bearer",
            ...
            "runtime": {
                ...
                "split_runtime": False,
                "description": (
                    "The API server creates a server-side Hairball AIAgent; "
            ...
            "features": {
                ...
                "run_status": True,
                "run_events_sse": True,
                "run_events_multicast": True,
                "run_events_replay": True,
                "run_events_cursor_header": "Last-Event-ID",
                "run_stop": True,
                ...
                "session_continuity_header": "X-Hairball-Session-Id",
                "session_key_header": "X-Hairball-Session-Key",
    ...
    def _session_response(session: Dict[str, Any]) -> Dict[str, Any]:
        ...
        safe_keys = (
            ...
            "cost_actual_call_count", "cost_total_call_count", "cost_status",
            "cost_source",
    ...
    async def _handle_list_sessions(self, request: "web.Request") -> "web.Response":
        """GET /api/sessions — list persisted Hairball sessions."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
    ...
    async def _handle_create_session(self, request: "web.Request") -> "web.Response":
        """POST /api/sessions — create an empty Hairball session row."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        return web.json_response({"object": "hairball.session", "session": self._session_response(session)}, status=201)
    ...
    async def _handle_get_session(self, request: "web.Request") -> "web.Response":
        ...
        if err:
            return err
        return web.json_response({"object": "hairball.session", "session": self._session_response(session)})
    ...
    async def _handle_delete_session(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({"object": "hairball.session.deleted", "id": session_id, "deleted": bool(deleted)})
    ...
    async def _handle_fork_session(self, request: "web.Request") -> "web.Response":
        ...
        # Match CLI semantics, but commit parent transition + child transcript
        # as one SessionDB transaction. A failed fork leaves the source live.
        try:
            fork = db.fork_session(
                source_id,
                fork_id,
            ...
            return web.json_response(_openai_error(str(exc), code="invalid_title"), status=400)
        return web.json_response({"object": "hairball.session", "session": self._session_response(fork)}, status=201)
    ...
    async def _handle_session_chat(self, request: "web.Request") -> "web.Response":
        ...
        headers = {"X-Hairball-Session-Id": effective_session_id or session_id}
        if gateway_session_key:
            headers["X-Hairball-Session-Key"] = gateway_session_key
        return web.json_response(
            {
                "object": "hairball.session.chat.completion",
    ...
    async def _handle_session_chat_stream(self, request: "web.Request") -> "web.StreamResponse":
        ...
        headers = {
            ...
            "X-Hairball-Session-Id": session_id,
        }
        if gateway_session_key:
            headers["X-Hairball-Session-Key"] = gateway_session_key
    ...
    async def _handle_chat_completions(self, request: "web.Request") -> "web.Response":
        ...
        hairball_turn = body.get("hairball") or {}
        if not isinstance(hairball_turn, dict):
            return web.json_response(
                _openai_error("hairball must be an object", code="invalid_hairball_turn"),
                status=400,
            )
        raw_agent_mode = str(hairball_turn.get("agent_mode") or "").strip()
        if raw_agent_mode:
            try:
                from agent.plan_execution import normalize_agent_mode

                agent_mode = str(normalize_agent_mode(raw_agent_mode))
            except Exception:
                return web.json_response(
                    _openai_error(
                        "hairball.agent_mode is invalid",
            ...
            agent_mode = None
        turn_system_message = hairball_turn.get("turn_system_message")
        if turn_system_message is not None and not isinstance(turn_system_message, str):
            return web.json_response(
                _openai_error(
                    "hairball.turn_system_message must be a string",
                    code="invalid_turn_system_message",
                ),
                status=400,
            )
        plan_execution_id = hairball_turn.get("plan_execution_id")
        if plan_execution_id is not None and not isinstance(plan_execution_id, str):
            return web.json_response(
                _openai_error(
                    "hairball.plan_execution_id must be a string",
                    code="invalid_plan_execution_id",
                ),
                status=400,
            )
        turn_system_message = str(turn_system_message or "").strip() or None
        plan_execution_id = str(plan_execution_id or "").strip() or None
        plan_context_mode = hairball_turn.get("plan_context_mode")
        if plan_context_mode is not None:
            try:
                from agent.plan_handoff import normalize_plan_context_mode

                plan_context_mode = normalize_plan_context_mode(plan_context_mode)
            except Exception:
                return web.json_response(
                    _openai_error(
                        "hairball.plan_context_mode is invalid",
                        code="invalid_plan_context_mode",
                    ),
                    status=400,
                )
        plan_file = hairball_turn.get("plan_file")
        plan_content = hairball_turn.get("plan_content")
        for field_name, field_value in (
            ...
            if field_value is not None and not isinstance(field_value, str):
                return web.json_response(
                    _openai_error(
                        f"hairball.{field_name} must be a string",
                        code=f"invalid_{field_name}",
                    ),
                    status=400,
                )
        plan_file = str(plan_file or "").strip() or None
        plan_content = str(plan_content or "") or None
        plan_steps = hairball_turn.get("plan_steps")
        selected_plan_step_ids = hairball_turn.get("selected_plan_step_ids")
        if plan_steps is not None:
            if (
                not isinstance(plan_steps, list)
                or not plan_steps
                or len(plan_steps) > 80
                or any(not isinstance(step, dict) for step in plan_steps)
            ):
                return web.json_response(
                    _openai_error(
                        "hairball.plan_steps must be a non-empty list of objects",
                        code="invalid_plan_steps",
                    ),
                    status=400,
                )
        if selected_plan_step_ids is not None:
            if (
                not isinstance(selected_plan_step_ids, list)
                or not selected_plan_step_ids
                or len(selected_plan_step_ids) > 80
                or any(not isinstance(step_id, str) for step_id in selected_plan_step_ids)
            ):
                return web.json_response(
                    _openai_error(
                        "hairball.selected_plan_step_ids must be a non-empty list of strings",
                        code="invalid_selected_plan_step_ids",
                    ),
                    status=400,
                )
            if plan_steps is None:
                return web.json_response(
                    _openai_error(
                        "hairball.plan_steps is required with a step selection",
        ...
        if plan_steps is not None:
            try:
                from agent.plan_commands import prepare_transport_plan_steps

                prepare_transport_plan_steps(
                    plan_steps,
                    selected_plan_step_ids,
                )
            except Exception as exc:
                code = str(getattr(exc, "code", "invalid_plan_steps"))
                return web.json_response(
                    _openai_error(str(exc), code=code),
                    status=400,
                )
        if turn_system_message and len(turn_system_message) > 12_000:
            return web.json_response(
                _openai_error(
                    "hairball.turn_system_message is too long",
                    code="turn_system_message_too_long",
                ),
                status=400,
            )
        if plan_execution_id and len(plan_execution_id) > 512:
            return web.json_response(
                _openai_error(
                    "hairball.plan_execution_id is too long",
                    code="plan_execution_id_too_long",
                ),
                status=400,
            )
        if plan_file and len(plan_file) > 4096:
            return web.json_response(
                _openai_error("hairball.plan_file is too long", code="plan_file_too_long"),
                status=400,
            )
        if plan_content and len(plan_content.encode("utf-8")) > 524_288:
            return web.json_response(
                _openai_error("hairball.plan_content is too large", code="plan_content_too_large"),
                status=400,
            )
        ...
        # stable per-channel identifier via X-Hairball-Session-Key.  This
        # is independent of X-Hairball-Session-Id: the key persists across
        ...
        # Allow caller to continue an existing session by passing X-Hairball-Session-Id.
        ...
        provided_session_id = request.headers.get("X-Hairball-Session-Id", "").strip()
        if provided_session_id:
            if not self._api_key:
                logger.warning(
                    "Session continuation via X-Hairball-Session-Id rejected: "
            ...
            # conversation map to the same Hairball session.  The first user
        ...
        if stream:
            ...
            def _on_tool_start(tool_call_id, function_name, function_args):
                """Emit ``hairball.tool.progress`` with ``status: running``.
            ...
            agent_task = asyncio.ensure_future(self._run_agent(
                ...
                plan_context_mode=plan_context_mode,
                plan_file=plan_file,
                plan_content=plan_content,
                plan_steps=plan_steps,
                selected_plan_step_ids=selected_plan_step_ids,
            ))
        ...
        response_headers = {
            "X-Hairball-Session-Id": result.get("session_id", session_id),
        }
        if gateway_session_key:
            response_headers["X-Hairball-Session-Key"] = gateway_session_key
        ...
        if not final_response and (is_failed or is_partial):
            ...
            err_body["error"]["hairball"] = {
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
                **_plan_result_metadata(result),
            }
            response_headers["X-Hairball-Completed"] = "false"
            response_headers["X-Hairball-Partial"] = "true" if is_partial else "false"
            return web.json_response(err_body, status=502, headers=response_headers)

        # Soft-partial path: we have *some* text but the run did not complete
        # (e.g. truncation with partial buffered output). Still 200 but signal
        # truncation via finish_reason="length" + Hairball-specific extras.
        response_data = {
            "id": completion_id,
        ...
        if isinstance(completion_assessment, dict):
            response_data.setdefault("hairball", {})["completion_assessment"] = dict(
                completion_assessment
            )
        plan_metadata = _plan_result_metadata(result)
        if plan_metadata:
            response_data.setdefault("hairball", {}).update(plan_metadata)
        if is_partial or is_failed or not completed:
            response_data.setdefault("hairball", {}).update({
                "completed": completed,
                "partial": is_partial,
                "failed": is_failed,
                "error": err_msg,
            ...
            response_headers["X-Hairball-Partial"] = "true" if is_partial else "false"
            if err_msg:
                response_headers["X-Hairball-Error"] = _redact_api_error_text(err_msg, limit=200)
    ...
    async def _write_sse_chat_completion(
        ...
        if session_id:
            sse_headers["X-Hairball-Session-Id"] = session_id
        if gateway_session_key:
            sse_headers["X-Hairball-Session-Key"] = gateway_session_key
        ...
        try:
            ...
            async def _emit(item):
                ...
                as a custom ``event: hairball.tool.progress`` SSE event so
                ...
                if isinstance(item, tuple) and len(item) == 2 and item[0] == "__tool_progress__":
                    ...
                    await response.write(
                        f"event: hairball.tool.progress\ndata: {event_data}\n\n".encode()
                    )
                else:
                    content_chunk = {
                        "id": completion_id, "object": "chat.completion.chunk",
            ...
            if isinstance(completion_assessment, dict):
                finish_chunk.setdefault("hairball", {})[
                    "completion_assessment"
                ] = dict(completion_assessment)
            plan_metadata = _plan_result_metadata(result)
            if plan_metadata:
                finish_chunk.setdefault("hairball", {}).update(plan_metadata)
            if finish_reason != "stop":
                ...
                finish_chunk.setdefault("hairball", {}).update({
                    "completed": completed,
                    "partial": is_partial,
                    "failed": is_failed,
                    "error": err_msg,
            ...
            if agent is not None:
                self._interrupt_active_agent(agent, "SSE client disconnected")
    ...
    async def _write_sse_responses(
        ...
        try:
            ...
            agent = agent_ref[0] if agent_ref else None
            if agent is not None:
                self._interrupt_active_agent(agent, "SSE task cancelled")
            if not agent_task.done():
                agent_task.cancel()
    ...
    async def _handle_responses(self, request: "web.Request") -> "web.Response":
        ...
        response_headers = {"X-Hairball-Session-Id": session_id}
        if gateway_session_key:
            response_headers["X-Hairball-Session-Key"] = gateway_session_key
        return web.json_response(response_data, headers=response_headers)
    ...
    async def _run_agent(
        ...
        turn_system_message: Optional[str] = None,
        plan_execution_id: Optional[str] = None,
        plan_context_mode: Optional[str] = None,
        plan_file: Optional[str] = None,
        plan_content: Optional[str] = None,
        plan_steps: Optional[List[Dict[str, Any]]] = None,
        selected_plan_step_ids: Optional[List[str]] = None,
    ) -> tuple:
        """
        ...
        *route* is an optional ``model_routes`` entry (resolved from the
        ...
        def _run():
            ...
            try:
                ...
                effective_turn_system_message = turn_system_message
                published_execution_id = str(plan_execution_id or "").strip()
                execution_attr_missing = object()
                previous_execution_id = getattr(
                    agent, "_plan_execution_id", execution_attr_missing
                )
                previous_execution_session_id = getattr(
                    agent,
                    "_plan_execution_session_id",
                    execution_attr_missing,
                )
                if published_execution_id:
                    # Publish immutable host ownership before exposing the
                    # agent reference to disconnect/stop handlers.
                    agent._plan_execution_id = published_execution_id
                    agent._plan_execution_session_id = str(
                        session_id or effective_task_id
                    )
                ...
                if plan_context_mode:
                    from agent.plan_handoff import prepare_plan_handoff

                    effective_plan_file = str(plan_file or "").strip()
                    if plan_content is not None:
                        from agent.plan_execution import resolve_plan_file

                        remote_plan_file = resolve_plan_file(
                            str(session_id or effective_task_id),
                            db=getattr(agent, "_session_db", None),
                        )
                        remote_plan_file.parent.mkdir(parents=True, exist_ok=True)
                        remote_plan_file.write_text(str(plan_content), encoding="utf-8")
                        effective_plan_file = str(remote_plan_file)
                        from agent.plan_commands import render_build_switch_instruction

                        effective_turn_system_message = render_build_switch_instruction(
                            effective_plan_file
                        )
                    handoff = prepare_plan_handoff(
                        conversation_history,
                        context_mode=plan_context_mode,
                    ...
                    if not handoff.dispatch:
                        ...
                        if plan_execution_id:
                            result["plan_execution_id"] = plan_execution_id
                        usage = {
                            "input_tokens": 0,
                            "output_tokens": 0,
                            "total_tokens": 0,
                ...
                if effective_turn_system_message:
                    run_kwargs["system_message"] = effective_turn_system_message
                if plan_execution_id:
                    run_kwargs["plan_execution_id"] = plan_execution_id
                if plan_steps is not None:
                    run_kwargs["plan_steps"] = plan_steps
                    run_kwargs["selected_plan_step_ids"] = selected_plan_step_ids
                result = agent.run_conversation(**run_kwargs)
                if plan_execution_id:
                    # Transport correlation belongs to the API envelope, not
                    # the model/tool loop.
                    result["plan_execution_id"] = plan_execution_id
                usage = {
                ...
                # (e.g. X-Hairball-Session-Id header) can track compression-
                ...
                if "published_execution_id" in locals() and published_execution_id:
                    if getattr(agent, "_plan_execution_id", None) == published_execution_id:
                        if previous_execution_id is execution_attr_missing:
                            delattr(agent, "_plan_execution_id")
                        else:
                            agent._plan_execution_id = previous_execution_id
                    if (
                        getattr(agent, "_plan_execution_session_id", None)
                        == str(session_id or effective_task_id)
                    ):
                        if previous_execution_session_id is execution_attr_missing:
                            delattr(agent, "_plan_execution_session_id")
                        else:
                            agent._plan_execution_session_id = (
    ...
    def _interrupt_active_agent(self, agent: Any, reason: str) -> None:
        """Interrupt one API run and cancel only its bound Plan execution."""
        ...
        try:
            ...
            pass
        execution_id = str(
            getattr(agent, "_plan_execution_id", "") or ""
        ).strip()
        bound_session_id = getattr(agent, "_plan_execution_session_id", "")
        if not isinstance(bound_session_id, str):
            bound_session_id = ""
        live_session_id = getattr(agent, "session_id", "")
        if not isinstance(live_session_id, str):
            live_session_id = ""
        session_id = str(bound_session_id or live_session_id or "").strip()
        ...
        try:
            ...
            cancel_active_plan_turn(
                ...
                operation_id=f"api-plan-interrupt:{execution_id}",
                db=getattr(agent, "_session_db", None),
            )
        except Exception as exc:
            logger.warning("Could not cancel active API Plan execution: %s", exc)
    ...
    def _make_run_event_callback(self, run_id: str, loop: "asyncio.AbstractEventLoop"):
        """Return a callback that publishes through the canonical run journal."""
        def _push(event: Dict[str, Any]) -> None:
            def _publish_on_loop() -> None:
                if not self._run_event_hub.has_run(run_id):
                    return
                ...
                self._run_event_hub.publish(run_id, event)

            try:
                loop.call_soon_threadsafe(_publish_on_loop)
            except Exception:
                pass
        ...
        def _callback(event_type: str, tool_name: str = None, preview: str = None, args=None, **kwargs):
            ...
            elif event_type == "tool.output_risk":
    ...
    async def _handle_runs(self, request: "web.Request") -> "web.Response":
        ...
        self._run_event_hub.open(run_id, created_at=created_at)
        self._run_approval_sessions[run_id] = approval_session_key

        event_cb = self._make_run_event_callback(run_id, loop)

        def _put_event_if_active(event: Dict[str, Any]) -> None:
            """Publish only while this run still owns retained transport state."""
            self._run_event_hub.publish(run_id, event)
        ...
        async def _run_and_close():
            try:
                ...
                def _approval_notify(approval_data: Dict[str, Any]) -> None:
                    ...
                    try:
                        loop.call_soon_threadsafe(_publish_approval)
                    except Exception:
                        pass
                ...
                try:
                    ...
                    pass
                # Close only the event journal. Run control cleanup remains
                # independent so a replay client cannot affect agent lifetime.
                self._run_event_hub.close(run_id)
        ...
        response_headers = (
            {"X-Hairball-Session-Key": gateway_session_key} if gateway_session_key else {}
        )
        return web.json_response(
    ...
    async def _handle_run_events(self, request: "web.Request") -> "web.StreamResponse":
        """Replay and follow one run with an independent SSE cursor."""
        auth_err = self._check_auth(request)
        if auth_err:
            return auth_err
        ...
        raw_after = str(raw_after or "").strip()
        try:
            ...
            return web.json_response(
                _openai_error(
                    "Run event cursor must be a non-negative integer",
        ...
        for _ in range(20):
            if self._run_event_hub.has_run(run_id):
                break
        ...
        try:
            subscription = self._run_event_hub.subscribe(run_id, after=after)
        except RunEventCursorExpired as exc:
            payload = _openai_error(str(exc), code="event_cursor_expired")
            payload["error"].update(
                {
                    "requested_cursor": exc.requested,
                    "first_available_cursor": exc.first_available,
                    "last_available_cursor": exc.last_available,
                }
            )
        ...
        try:
            while True:
                ...
                if event is None:
                    ...
                    break
                if isinstance(event, RunEventLagged):
                    payload = {
                        "error": {
                            "message": (
                                "SSE consumer fell behind the bounded live queue; "
                                "reconnect with Last-Event-ID"
                            ),
                            "type": "invalid_request_error",
                            "code": "event_consumer_lagged",
                            "last_available_cursor": event.last_available,
                        }
                    }
                    await response.write(
                        (
    ...
    async def _handle_run_approval(self, request: "web.Request") -> "web.Response":
        ...
        return web.json_response({
            "object": "hairball.run.approval_response",
            "run_id": run_id,
            "choice": choice,
            "resolved": resolved,
        })
    ...
    async def _handle_stop_run(self, request: "web.Request") -> "web.Response":
        ...
        if agent is not None:
            self._interrupt_active_agent(agent, "Stop requested via API")
    ...
    def _sweep_orphaned_runs_once(self, now: Optional[float] = None) -> None:
        ...
        if now is None:
            now = time.time()
        stale = self._run_event_hub.expire(now=now, ttl=self._RUN_STREAM_TTL)
        for run_id in stale:
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            # Store the adapter after native routes are registered. Local Hairball-Relay
            ...
            # that is the exact surface the hairball-0day campaign abused to write
            # ~/.hairball/config.yaml and plant persistence. Sandboxing (Docker /
            ...
            if is_network_accessible(self._host):
                try:
                    from hairball_cli.config import load_config as _load_cfg
                    _backend = (
    ...
    async def disconnect(self) -> None:
        ...
        event_hub = getattr(self, "_run_event_hub", None)
        if event_hub is not None:
            try:
                event_hub.clear()
            except Exception:
                logger.debug(
                    "Failed to clear run event hub for %s", self.name, exc_info=True,
                )
