...
Module-level run.py helpers a handler needs (``_hairball_home``,
...
from hairball_cli.config import atomic_config_write, cfg_get, clear_model_endpoint_credentials
from utils import (
    atomic_json_write,
...
def _model_switch_skew_guard() -> Optional[str]:
    ...
    return t(
        ...
        error=(
            ...
            f"crash — restart the gateway to load the new code: hairball gateway restart"
        ),
    )
...
class GatewaySlashCommandsMixin:
    ...
    def _typed_command_prefix_for(self, platform) -> str:
        """Return the prefix users can always type to reach Hairball commands.
        ...
        if action in {"pause", "resume"}:
            ...
            if action == "pause":
                ...
                return (
                    ...
                    f"`hairball gateway restart` to reset."
                )
            # action == "resume"
            if platform not in failed:
                return (
    ...
    async def _handle_restart_command(self, event: MessageEvent) -> Union[str, EphemeralReply]:
        ...
        from gateway.run import _hairball_home
        ...
        try:
            ...
            atomic_json_write(
                _hairball_home / ".restart_notify.json",
                notify_data,
                indent=None,
            )
        ...
        try:
            ...
            atomic_json_write(
                _hairball_home / ".restart_last_processed.json",
                dedup_data,
                indent=None,
            )
        except Exception as e:
    ...
    async def _handle_version_command(self, event: MessageEvent) -> str:
        """Handle /version — show the running Hairball Agent version."""
    ...
    async def _handle_help_command(self, event: MessageEvent) -> str:
        ...
        from hairball_cli.commands import gateway_help_lines
        lines = [
    ...
    async def _handle_model_command(self, event: MessageEvent) -> Optional[str]:
        ...
        """
        from gateway.run import _hairball_home, _load_gateway_config
        import yaml
        ...
        user_provs = None
        custom_provs = None
        config_path = _hairball_home / "config.yaml"
        try:
            ...
            if has_picker:
                ...
                if providers:
                    ...
                    async def _on_model_selected(
                        ...
                        try:
                            from hairball_cli.context_switch_guard import (
                        ...
                        if persist_global:
                            try:
                                ...
                                from hairball_cli.config import save_config
    ...
    async def _handle_codex_runtime_command(self, event: MessageEvent) -> str:
        ...
        Same surface as the CLI handler in cli.py:
            ...
            /codex-runtime auto             — Hairball default runtime
    ...
    async def _handle_personality_command(self, event: MessageEvent) -> str:
        ...
        if not personalities:
            return t("gateway.personality.none_configured", path=display_hairball_home())
    ...
    async def _handle_plan_command(self, event: "MessageEvent") -> str | None:
        """Handle durable Plan controls or scope the next gateway agent turn."""
        ...
        session_id = str(getattr(session_entry, "session_id", None) or "")
        if not session_id:
            return "Plan unavailable: no active session"
        args = str(event.get_command_args() or "").strip()
        db = getattr(self, "_session_db", None)
        if db is not None and not hasattr(db, "append_kernel_event_if_kind_seq"):
        ...
        try:
            ...
            context = handle_plan_command(
                ...
                args,
                surface="gateway",
                actor=str(getattr(event.source, "user_id", None) or "user"),
                db=db,
            )
        except Exception as exc:
            code = str(getattr(exc, "code", None) or "plan_command_failed")
            return f"Plan error [{code}]: {exc}"
        if context.get("kind") in {"prompt", "execute"}:
            ...
            context["operation_id"] = str(
                ...
                or f"gateway-plan-{uuid.uuid4().hex}"
            )
            event.text = str(context.get("message") or "")
            event._hairball_plan_context = context
            return None
        return str(context.get("message") or "")
    ...
    async def _handle_rollback_command(self, event: MessageEvent) -> str:
        ...
        try:
            import yaml as _y
            _cfg_path = _hairball_home / "config.yaml"
            if _cfg_path.exists():
    ...
    async def _handle_reasoning_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _hairball_home, _platform_config_key
        import yaml
    ...
    async def _handle_skills_command(self, event: MessageEvent) -> str:
        ...
        the pending JSON file under ``~/.hairball/pending/skills/``. (Note this is
        the write-approval ``diff <id>``; the CLI also has an unrelated
        ``hairball skills diff <name>`` that diffs a bundled skill vs stock.)
        """
        from gateway.run import _hairball_home
        from hairball_cli.write_approval_commands import handle_pending_subcommand
        from tools import write_approval as wa
        ...
        # the real review surface. (Note: `hairball skills diff <name>` is a
        ...
        if args and args[0].lower() == "diff" and len(out) > 3000:
            ...
            out = (out[:3000]
                   + "\n… (truncated — full diff in "
                     f"~/.hairball/pending/skills/{pending_id}.json)")
        return out

    async def _handle_fast_command(self, event: MessageEvent) -> str:
        """Handle /fast — mirror the CLI Priority Processing toggle in gateway chats."""
        from gateway.run import _hairball_home, _load_gateway_config, _resolve_gateway_model
        import yaml
    ...
    async def _handle_verbose_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _hairball_home, _load_gateway_config, _platform_config_key
    ...
    async def _handle_footer_command(self, event: MessageEvent) -> str:
        ...
        """
        from gateway.run import _hairball_home, _load_gateway_config, _platform_config_key, _resolve_gateway_model
    ...
    async def _handle_sessions_command(self, event: MessageEvent) -> str:
        ...
        from hairball_cli.session_listing import (
    ...
    async def _handle_branch_command(self, event: MessageEvent) -> str:
        ...
        # SessionDB owns the branch invariant: child row, complete replay,
        # counters, title, and lineage marker commit together. Gateway keeps
        # the parent live, matching its existing multi-chat semantics.
        try:
            branch = await self._session_db.fork_session(
                parent_session_id,
                new_session_id,
                messages=history,
        ...
        if not new_entry:
            try:
                ...
                logger.exception(
                    "Failed to roll back orphan branch session %s after gateway switch failure",
                    new_session_id,
                )
    ...
    async def _handle_credits_command(self, event: MessageEvent) -> str:
        """Handle /credits -- show managed credit balance and the top-up handoff.
        ...
        lines: list[str] = ["💳 **Account credits**"]
    ...
    async def _handle_usage_command(self, event: MessageEvent) -> str:
        ...
        # `/usage reset [--force]` — redeem one banked reference rate-limit reset
        ...
        # ── Account credits magnitudes + monthly-grant % gauge ─────────────
        # Shared with the CLI / TUI /usage block via managed_credits_lines(): a single
        # auth-gate + portal-fetch + render path (which also honors the dev fixture).
        # Run off the event loop. The helper gates on "a managed account is logged in"
        # — NOT the inference provider and NOT nested under `if provider:` — so a
        # managed-credentialled user running inference elsewhere (or with none resident)
        ...
        try:
            ...
            credits_lines = await asyncio.to_thread(managed_credits_lines, markdown=True)
        except Exception:
        ...
        if agent and hasattr(agent, "session_total_tokens") and agent.session_api_calls > 0:
            ...
            try:
                ...
                if cost.status == "included":
                    ...
                    prefix = "" if cost.status == "actual" else "~"
                    cost_line = t(
                        "gateway.usage.label_cost",
                        prefix=prefix,
                        amount=f"{cost.display_cost_usd:.4f}",
                    )
                    if cost.actual_cost_usd is not None and not cost.actual_complete:
                        cost_line += (
    ...
    async def _handle_insights_command(self, event: MessageEvent) -> str:
        ...
        try:
            ...
            def _run_insights():
                insights_db = SessionDB()
                try:
                    engine = InsightsEngine(insights_db)
                    report = engine.generate(days=days, source=source)
                    return engine.format_gateway(report)
                finally:
                    # This helper runs inside the executor above, so closing
                    # the synchronous SQLite handle here cannot block the
                    # gateway event loop.  ``finally`` also covers report
                    # generation/formatting failures.
                    insights_db.close()
    ...
    async def _handle_bundles_command(self, event: MessageEvent) -> str:
        ...
        if not bundles:
            return (
                ...
                "  `hairball bundles create <name> --skill <s1> --skill <s2>`\n"
    ...
    async def _handle_debug_command(self, event: MessageEvent) -> str:
        ...
        full log uploads should use ``hairball debug share`` from the CLI.
        """
        import asyncio
        from hairball_cli.debug import (
    ...
    async def _handle_update_command(self, event: MessageEvent) -> str:
        """Handle /update command — update Hairball Agent to the latest version.

        Spawns ``hairball update`` in a detached session (via ``setsid``) so it
        survives the gateway restart that ``hairball update`` may trigger. Marker
        files are written so either the current gateway process or the next one
        can notify the user when the update finishes.
        """
        from gateway.run import _hairball_home, _resolve_hairball_bin
        import json
        import shutil
        import subprocess
        from datetime import datetime
        ...
        if is_managed():
            return f"✗ {format_managed_message('update Hairball Agent')}"
        ...
        hairball_cmd = _resolve_hairball_bin()
        if not hairball_cmd:
            return t("gateway.update.hairball_cmd_not_found")

        pending_path = _hairball_home / ".update_pending.json"
        output_path = _hairball_home / ".update_output.txt"
        exit_code_path = _hairball_home / ".update_exit_code"
        session_key = self._session_key_for_source(event.source)
        pending = {
        ...
        # Spawn `hairball update --gateway` detached so it survives gateway restart.
        ...
        # gateway can stream it to the messenger in near-real-time.
        #
        # Windows: no bash/setsid chain.  Run `hairball update --gateway`
        ...
        try:
            if sys.platform == "win32":
                ...
                # hairball_cmd is a list of argv parts we can pass directly
                ...
                hairball_cmd_str = " ".join(shlex.quote(part) for part in hairball_cmd)
                update_cmd = (
                    f"PYTHONUNBUFFERED=1 {hairball_cmd_str} update --gateway"
