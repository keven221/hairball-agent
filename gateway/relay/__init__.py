"""Relay/connector support package for the Hairball gateway.
...
def relay_connection_auth() -> tuple[Optional[str], Optional[str]]:
    ...
    Both come from enrollment (``hairball gateway enroll`` writes them to
    ``~/.hairball/.env``): ``GATEWAY_RELAY_ID`` identifies the enrolled instance,
...
def relay_endpoint() -> Optional[str]:
    ...
    ``HAIRBALL_DASHBOARD_PUBLIC_URL``); a hosted/NAS container has the same var
...
def relay_wake_url() -> Optional[str]:
    ...
    sets it explicitly (or passes ``--wake-url`` to ``hairball gateway enroll``).
...
def _resolve_relay_identity_token() -> str:
    ...
    ``hairball gateway enroll`` CLI. Two modes, in precedence order:

      1. **Generic OIDC client-credentials** (air-gapped / self-hosted-IdP, NO
         Managed Portal): when ``gateway.idp.token_url`` (or
      ...
      2. **Managed Portal** (default): ``resolve_managed_access_token()`` — existing
    ...
    if not token_url:
        # Mode 2 — Managed Portal (default, unchanged behaviour).
        from hairball_cli.auth import resolve_managed_access_token

        return resolve_managed_access_token()
...
def self_provision_relay() -> bool:
    ...
    is already present, AND the agent can resolve its own managed access token. In
    that case the runtime resolves the agent's own managed access token (the same
    ``resolve_managed_access_token()`` the enroll CLI / dashboard register use),
    ...
    up. The creds live ONLY in process memory — never written to ``~/.hairball/.env``.
    ...
    sets neither ``HAIRBALL_MANAGED`` nor a ``.managed`` marker), so gating on it
    ...
    self-guarding:
      ...
      - A self-hosted operator who ran ``hairball gateway enroll``: has a PINNED
        ``GATEWAY_RELAY_SECRET`` -> skipped (the secret-present guard below).
      - A self-hosted box with a relay URL but no NAS identity:
        ``resolve_managed_access_token()`` fails -> graceful no-op.
    ...
    try:
        ...
        host = ""
    gateway_id = os.environ.get("GATEWAY_RELAY_ID", "").strip() or f"gw-{host or 'hairball'}"
