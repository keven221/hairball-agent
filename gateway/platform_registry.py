...
class PlatformEntry:
    ...
    # Env vars this platform needs (for ``hairball setup`` display).
    ...
    # built-ins).  Used by ``hairball gateway setup`` to auto-enable the
...
class PlatformRegistry:
    ...
    def __init__(self) -> None:
        ...
        # plugin-discovery time added several seconds to *every* `hairball`
        # invocation -- including plain `hairball chat`, which never touches any
        ...
        # `hairball setup`/`gateway status`, send_message).
    ...
    def _resolve_all(self) -> None:
        ...
        gateway startup, ``hairball setup``/``gateway status``, channel
