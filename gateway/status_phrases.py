...
Hairball' long-running gateway status surface into short status lines suitable
for chat surfaces.

Built-in defaults live in ``gateway/assets/status_phrases.yaml``. Users can add
portable, profile-relative phrase catalogs under ``HAIRBALL_HOME`` either by using
conventional paths::

    ~/.hairball/status_phrases.yaml
    ~/.hairball/status_phrases/*.yaml
...
or by pointing config at a relative file/directory::
    ...
    display:
      status_phrases:
        path: status_phrases/whatsapp.yaml  # relative to HAIRBALL_HOME
...
# These are Hairball UI surfaces, not app/vendor/domain buckets.  Keep this
...
def resolve_status_phrase_catalog(user_config: Mapping[str, Any] | None, platform_key: str | None = None) -> dict[str, list[str]]:
    ...
    _merge_phrase_paths(catalog, list(_CONVENTIONAL_RELATIVE_PATHS), base_dir=hairball_home)

    display = (user_config or {}).get("display") if isinstance(user_config, Mapping) else None
    if not isinstance(display, Mapping):
        return catalog

    _merge_phrase_config(catalog, display.get("generic_status_phrases"), base_dir=hairball_home)
    _merge_phrase_config(catalog, display.get("status_phrases"), base_dir=hairball_home)
    ...
    if platform_key and isinstance(platforms, Mapping):
        ...
        if isinstance(platform_display, Mapping):
            _merge_phrase_config(catalog, platform_display.get("generic_status_phrases"), base_dir=hairball_home)
            _merge_phrase_config(catalog, platform_display.get("status_phrases"), base_dir=hairball_home)
    return catalog
...
def classify_status_context(
    ...
    *,
    ...
    """Classify an internal gateway event into a Hairball UI-surface bucket."""
