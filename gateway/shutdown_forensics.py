...
def snapshot_shutdown_context(received_signal: Any = None) -> Dict[str, Any]:
    ...
    * Wall-clock and monotonic timestamps for cross-correlating later phases
    ...
    try:
        hairball_home_str = os.environ.get("HAIRBALL_HOME")
        if hairball_home_str:
            takeover_path = Path(hairball_home_str) / ".gateway-takeover.json"
            if takeover_path.exists():
                try:
                    ...
                    pass
            planned_stop_path = Path(hairball_home_str) / ".gateway-planned-stop.json"
...
def spawn_async_diagnostic(
    ...
    *,
    ...
    or compete with platform teardown.  The subprocess uses a detached
    Python watchdog so a wedged ``ps`` still self-cleans within
    ``timeout_seconds``.  Using the current Python executable keeps this path
    portable to macOS, where the GNU ``timeout`` command is not installed by
    default.
...
def check_systemd_timing_alignment(drain_timeout: float) -> Optional[Dict[str, Any]]:
    ...
    upgraded hairball-agent but never re-ran ``hairball setup`` to regenerate
    ...
    try:
        # /proc/self/cgroup gives us "0::/user.slice/.../hairball-gateway.service"
    ...
    # that's the common case for hairball.
    ...
    # constant in hairball_cli/gateway.py.
    headroom = 30.0
