...
Defenses 1 and 2 (the ``_HAIRBALL_GATEWAY`` guard on ``hairball gateway
...
raw ``terminal("launchctl kickstart -k gui/<uid>/ai.hairball.gateway")``,
...
State lives in ``<HAIRBALL_HOME>/gateway/restart_loop.json`` so it is
...
def _state_path():
    return get_hairball_home() / "gateway" / "restart_loop.json"
