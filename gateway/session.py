...
# ``HAIRBALL_AUTO_CONTINUE_FRESHNESS`` at startup.
...
def auto_continue_freshness_window() -> float:
    ...
    ``HAIRBALL_AUTO_CONTINUE_FRESHNESS`` (bridged from ``config.yaml``
    ...
    """
    raw = os.environ.get("HAIRBALL_AUTO_CONTINUE_FRESHNESS")
...
# ``sessions_dir / f"{session_id}.json"`` in hairball_state, request-dump
...
def _discord_tools_loaded() -> bool:
    ...
    Two conditions must hold:
      1. The `discord` or `discord_admin` toolset is enabled for the
         Discord platform via `hairball tools` (opt-in, default OFF).
...
def build_session_context_prompt(
    ...
    *,
    ...
    if context.source.platform == Platform.SLACK:
        ...
        # `discord` / `discord_admin` via `hairball tools` AND the bot
    ...
    lines.append(
        f"- `\"local\"` → Save to local files only ({display_hairball_home()}/cron/output/)"
    )
...
class SessionEntry:
    ...
    # Parked next-step notes (inject / leftover). Persisted; never auto-wakes.
    ...
    def to_dict(self) -> Dict[str, Any]:
        result = {
            ...
            "cost_actual_call_count": self.cost_actual_call_count,
            "cost_total_call_count": self.cost_total_call_count,
        ...
        parked = [str(item).strip() for item in (self.next_step or []) if str(item).strip()]
        if parked:
            result["next_step"] = parked
        return result
    ...
    def from_dict(cls, data: Dict[str, Any]) -> "SessionEntry":
        ...
        return cls(
            ...
            cost_actual_call_count=data.get("cost_actual_call_count", 0),
            cost_total_call_count=data.get("cost_total_call_count", 0),
            cost_status=data.get("cost_status", "unknown"),
            cost_source=data.get("cost_source", "none"),
            ...
            next_step=[
                str(item).strip()
                for item in (data.get("next_step") or [])
                if str(item).strip()
            ]
            or None,
        )
...
class SessionStore:
    ...
    def _save_sessions_json(self, data: Dict[str, Any]) -> None:
        ...
        data = {
            "_README": (
                ...
                "lives in the gateway_routing table in ~/.hairball/state.db). "
                "Maps messaging session keys (agent:main:<platform>:...) to "
                "active session IDs. This is NOT the session list. ALL "
                "sessions (CLI, TUI, and gateway) live in ~/.hairball/state.db "
                "and are shown by `hairball sessions list` and `/sessions`. "
    ...
    def turn_inbox(self, session_key: str):
        """Return the live projection for a gateway session's durable inbox."""
        from agent.turn_inbox import load_durable_turn_inbox, load_inbox_snapshot

        with self._lock:
            self._ensure_loaded_locked()
            entry = self._entries.get(str(session_key or ""))
            if entry is None:
                return None
            parked = [
                str(item).strip()
                for item in (entry.next_step or [])
                if str(item).strip()
            ]
            snapshot = {
                "next_step": [
                    {"id": uuid.uuid4().hex, "role": "user", "content": item}
                    for item in parked
                ]
            }
            if self._db is None:
                return load_inbox_snapshot(snapshot)
            inbox = load_durable_turn_inbox(
                entry.session_id,
                db=self._db,
                snapshot=snapshot,
            )
            projected = [
                ...
                for item in inbox.next_step
                if str(item.get("content") or "").strip()
            ]
            if projected != parked:
                entry.next_step = projected or None
                self._save()
            return inbox

    def persist_turn_inbox(self, session_key: str, inbox: Any) -> None:
        """Refresh the legacy routing mirror from the durable live projection."""
        with self._lock:
            self._ensure_loaded_locked()
            entry = self._entries.get(str(session_key or ""))
            if entry is None:
            ...
            projected = [
                str(item.get("content") or "").strip()
                for item in getattr(inbox, "next_step", ()) or ()
                if str(item.get("content") or "").strip()
            ]
            current = [
                str(item).strip()
    ...
    def enqueue_next_step(self, session_key: str, text: str) -> int:
        ...
        with self._lock:
            ...
            if entry is None:
                return 0
            parked = [str(item).strip() for item in (entry.next_step or []) if str(item).strip()]
            if self._db is None:
                if cleaned:
                    parked.append(cleaned)
                    entry.next_step = parked
                    self._save()
                return len(parked)

            from agent.turn_inbox import load_durable_turn_inbox, new_inbox_message
            ...
            if cleaned:
                inbox.append("next-step", new_inbox_message(cleaned))
            projected = [
                str(item.get("content") or "").strip()
                for item in inbox.next_step
                if str(item.get("content") or "").strip()
            ...
            if projected != parked:
                entry.next_step = projected or None
                self._save()
            return len(inbox.next_step)

    def take_next_step(self, session_key: str) -> List[str]:
        """Claim parked next-step notes. Leaves next-turn / follow-up intact."""
        with self._lock:
            self._ensure_loaded_locked()
            entry = self._entries.get(str(session_key or ""))
            if entry is None:
            ...
            if self._db is not None:
                from agent.turn_inbox import load_durable_turn_inbox
                ...
                claimed = inbox.claim("next-step", 0)
                if entry.next_step is not None:
            ...
            if not entry.next_step:
                return []
            parked = [str(item).strip() for item in list(entry.next_step) if str(item).strip()]
            entry.next_step = None
            self._save()
            return parked
