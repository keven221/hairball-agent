...
from hairball_constants import get_hairball_dir, get_hairball_home
...
PAIRING_DIR = get_hairball_dir("platforms/pairing", "pairing")
...
def _sync_allowlist_remove(platform: str, user_id: str) -> None:
    ...
    if len(remaining) == len(ids):
        return  # Not present.
    try:
        from hairball_cli.config import save_env_value, remove_env_value
...
def _merge_pairing_dir(active_dir: Path, alternate_dir: Path) -> None:
    ...
    Older installs use ``{HAIRBALL_HOME}/pairing`` while newer code/docs may
    write ``{HAIRBALL_HOME}/platforms/pairing``. If both directories exist, the
...
class PairingStore:
    ...
    ``<HAIRBALL_HOME>/profiles/<name>/pairing/`` (per-profile, used by
    multiplexing gateways so each profile has its own whitelist).
    Without a profile, storage is the global ``<HAIRBALL_HOME>/pairing/``
    directory (backward-compat for the ``hairball pairing`` CLI).
    """

    def __init__(self, profile: Optional[str] = None):
        # Resolve storage directory lazily — tests use a temp HAIRBALL_HOME
        # and PairingStore may be constructed before the env is set.
        if profile:
            from hairball_constants import get_hairball_home
            self._dir = get_hairball_home() / "profiles" / profile / "pairing"
        else:
    ...
    def _load_json(self, path: Path) -> dict:
        if path.exists():
            try:
                ...
                # the file, then the gateway process — running as `hairball` after
                ...
                logger.warning(
                    ...
                    "If you ran `docker exec <container> hairball pairing approve ...` as root, "
                    "re-run with `docker exec -u hairball <container> ...` and "
                    "chown the existing file to the hairball user, or restart the "
