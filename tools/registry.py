"""Central registry for all hairball-agent tools.
...
# timescales. Cache results for ~30 s so env-var flips via ``hairball tools``
...
def invalidate_check_fn_cache() -> None:
    ...
    affect tool availability (e.g. ``hairball tools enable``)."""
    with _check_fn_cache_lock:
...
class ToolRegistry:
    ...
    def _plugin_owner_of(self, handler: Callable) -> Optional[str]:
        ...
        if isinstance(mod, str) and mod.startswith("hairball_plugins."):
            return mod
        return None
    ...
    def register(
        ...
        with self._lock:
            ...
            self._tools[name] = ToolEntry(
                name=name,
                toolset=toolset,
                # Registration crosses an ownership boundary.  Keep a private
                # snapshot so a plugin/MCP adapter cannot mutate the live
                # model contract later by retaining and editing its input
                # dictionary (which would also bypass the generation cache).
    ...
    def deregister(self, name: str) -> None:
        ...
        with self._lock:
            ...
            if not entry.toolset.startswith("mcp-"):
                ...
                # (``hairball_plugins.{name}``), not the exact module string.
                # A handler defined in ``hairball_plugins.pkg.handlers`` is
                # still owned by the ``hairball_plugins.pkg`` package — exact
                ...
                if (
                    caller_mod.startswith("hairball_plugins.")
                    and not same_plugin
    ...
    def get_definitions(self, tool_names: Set[str], quiet: bool = False) -> List[dict]:
        ...
        etc.); TTL chosen so env-var changes (``hairball tools enable foo``)
        ...
        for name in sorted(tool_names):
            ...
            # Every consumer receives its own nested schema.  Several later
            # assembly stages tailor descriptions/properties per session; a
            # shallow copy would let one session poison the registry cache and
            # every neighboring surface.
            schema_with_name = copy.deepcopy(entry.schema)
    ...
    def get_schema(self, name: str) -> Optional[dict]:
        ...
        """
        entry = self.get_entry(name)
        return copy.deepcopy(entry.schema) if entry else None
