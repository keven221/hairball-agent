...
CHECKPOINT_PATH = get_hairball_home() / "processes.json"
...
class ProcessSession:
    ...
    remote_start_time: str = ""                 # /proc start ticks inside the backend
    process_token: str = ""                     # random remote adoption nonce
    reconnect_spec: Dict[str, Any] = field(default_factory=dict)
    log_path: str = ""
    pid_path: str = ""
    exit_path: str = ""
    token_path: str = ""
...
class ProcessRegistry:
    ...
    def __init__(self, *, auto_recover: bool = False):
        ...
        self._owner_id = f"process-registry:{os.getpid()}:{uuid.uuid4().hex}"
    ...
    @staticmethod
    def _durable_store():
        from tools.background_process_store import background_process_store
        return background_process_store

    @staticmethod
    def _redact_command(command: str) -> str:
        from agent.redact import redact_sensitive_text
        return redact_sensitive_text(command or "", force=True, code_file=True)

    @staticmethod
    def _artifact_file(session_id: str) -> Path:
        return get_hairball_home() / "artifacts" / "processes" / f"{session_id}.log"

    def _sync_artifact(self, session: ProcessSession) -> tuple[str, str]:
        """Atomically persist a force-redacted full output artifact."""
        ...
        path = Path(session.artifact_path or self._artifact_file(session.id))
        path.parent.mkdir(parents=True, exist_ok=True)
        tmp = path.with_name(f".{path.name}.{os.getpid()}.{threading.get_ident()}.tmp")
        tmp.write_text(safe_output, encoding="utf-8", errors="replace")
        os.replace(tmp, path)
        session.artifact_path = str(path)
        digest = hashlib.sha256(safe_output.encode("utf-8")).hexdigest()
        return str(path), digest

    def _watcher_record(self, session: ProcessSession) -> Dict[str, Any]:
        return {
    ...
    def _durable_record(self, session: ProcessSession) -> Dict[str, Any]:
        return {
            "process_id": session.id,
            "task_id": session.task_id,
            "session_key": session.session_key,
            "command_redacted": self._redact_command(session.command),
            "cwd": session.cwd,
            "pid": session.pid,
            "pid_scope": session.pid_scope,
            "remote_start_time": session.remote_start_time,
            "process_token": session.process_token,
            "reconnect": dict(session.reconnect_spec),
            "paths": {
                "log": session.log_path,
                "pid": session.pid_path,
                "exit": session.exit_path,
                "token": session.token_path,
            },
            "started_at": session.started_at,
            "watcher": self._watcher_record(session),
            "artifact_path": session.artifact_path,
            "idempotency_key": session.idempotency_key,
        }

    def _claim_spawn_slot(self, session: ProcessSession) -> Optional[ProcessSession]:
        """Reserve an idempotent dispatch before executing its command."""
        won, row = self._durable_store().claim_spawn(self._durable_record(session))
        if won:
            return None
        if not row:
            raise RuntimeError(
                "background dispatch already exists but its durable identity is unavailable"
            )
        ...
        # A competing Hairball process owns the live poller.  Returning the
        # same durable identity is enough for the replayed terminal call; most
        # importantly, this process does not execute the command again.
        paths = dict(row.get("paths") or {})
        duplicate = ProcessSession(
            id=process_id,
            command=str(row.get("command_redacted") or ""),
            task_id=str(row.get("task_id") or ""),
            session_key=str(row.get("session_key") or ""),
            pid=row.get("pid"),
            pid_scope=str(row.get("pid_scope") or "host"),
            remote_start_time=str(row.get("remote_start_time") or ""),
            process_token=str(row.get("process_token") or ""),
            reconnect_spec=dict(row.get("reconnect") or {}),
            log_path=str(paths.get("log") or ""),
            pid_path=str(paths.get("pid") or ""),
            exit_path=str(paths.get("exit") or ""),
            token_path=str(paths.get("token") or ""),
            artifact_path=str(row.get("artifact_path") or ""),
            idempotency_key=str(row.get("idempotency_key") or ""),
            cwd=row.get("cwd"),
            started_at=float(row.get("started_at") or time.time()),
            detached=True,
            exited=row.get("state") == "finished",
            exit_code=row.get("exit_code"),
            completion_reason=str(row.get("completion_reason") or "exited"),
            termination_source=str(row.get("termination_source") or ""),
        )
        artifact_path = duplicate.artifact_path
        if artifact_path:
            try:
                duplicate.output_buffer = Path(artifact_path).read_text(
    ...
    def _persist_running(self, session: ProcessSession) -> None:
        try:
            path, _ = self._sync_artifact(session)
            record = self._durable_record(session)
            record["artifact_path"] = path
            self._durable_store().upsert_running(record)
        except Exception as exc:
            logger.warning("Could not persist background process %s: %s", session.id, exc)

    def _existing_idempotent_session(self, key: str) -> Optional[ProcessSession]:
        if not key:
            return None
        with self._lock:
            for session in list(self._running.values()) + list(self._finished.values()):
                if session.idempotency_key == key:
                    return session
        try:
            row = self._durable_store().find_by_idempotency_key(key)
        except Exception:
            row = None
        if row:
            self.recover_from_checkpoint()
            return self.get(str(row.get("process_id") or ""))
        return None

    def _ensure_recovered(self) -> None:
        if not self._auto_recover or self._recovery_attempted:
            return
    ...
    def _daemon_term_grace_seconds() -> float:
        ...
        """
        try:
            from hairball_cli.config import read_raw_config, cfg_get, DEFAULT_CONFIG
            cfg = read_raw_config()
    ...
    def spawn_local(
        ...
        Args:
            use_pty: If True, use a pseudo-terminal via ptyprocess for interactive
                     CLI tools (reference, Claude Code, Python REPL). Falls back to
                     subprocess.Popen if ptyprocess is not installed.
        """
        existing = self._existing_idempotent_session(idempotency_key)
        if existing is not None:
            return existing
        ...
        session.artifact_path = str(self._artifact_file(session.id))
        duplicate = self._claim_spawn_slot(session)
        ...
        if use_pty:
            ...
            try:
                ...
                launch_args = [user_shell, "-lic", f"set +m; {command}"]
                build_process_args = getattr(local_environment, "build_process_args", None)
                if callable(build_process_args):
                    launch_args = build_process_args(launch_args, cwd=session.cwd)
        ...
        try:
            ...
            try:
                ...
                pass
            session.exited = True
            session.exit_code = proc.returncode if proc.returncode is not None else -1
            session.completion_reason = "failed_start"
            session.termination_source = "failed_start"
            self._move_to_finished(session)
            raise
    ...
    def spawn_via_env(
        ...
        reconnect_getter = getattr(env, "get_background_reconnect_spec", None)
        reconnect_spec = reconnect_getter() if callable(reconnect_getter) else {}
        session = ProcessSession(
            id=f"proc_{uuid.uuid4().hex[:12]}",
            command=command,
            task_id=task_id,
            ...
            reconnect_spec=dict(reconnect_spec or {}),
            idempotency_key=idempotency_key,
        )
        session.artifact_path = str(self._artifact_file(session.id))
        ...
        log_path = f"{temp_dir}/hairball_bg_{session.id}.log"
        pid_path = f"{temp_dir}/hairball_bg_{session.id}.pid"
        exit_path = f"{temp_dir}/hairball_bg_{session.id}.exit"
        token_path = f"{temp_dir}/hairball_bg_{session.id}.token"
        session.log_path = log_path
        session.pid_path = pid_path
        session.exit_path = exit_path
        ...
        quoted_token = shlex.quote(session.process_token)
        bg_command = (
            f"mkdir -p {quoted_temp_dir} && "
            f"( nohup bash -lc {quoted_command} > {quoted_log_path} 2>&1; "
            f"rc=$?; printf '%s\\n' \"$rc\" > {quoted_exit_path} ) & "
            f"pid=$!; "
            f"start=$(awk '{{print $22}}' /proc/$pid/stat 2>/dev/null || true); "
            f"printf '%s %s %s\\n' \"$pid\" \"$start\" {quoted_token} > {quoted_pid_path}; "
            f"printf '%s\\n' {quoted_token} > {quoted_token_path}; "
            f"printf '%s %s\\n' \"$pid\" \"$start\""
        )
        ...
        try:
            ...
            for line in output.splitlines():
                ...
                if parts and parts[0].isdigit():
                    session.pid = int(parts[0])
                    if len(parts) > 1:
                        session.remote_start_time = parts[1]
                    break
        ...
        if not session.exited:
            self._persist_running(session)
            try:
                if not self._durable_store().claim_lease(
                    session.id, self._owner_id, ttl=15,
                ):
                    raise RuntimeError("background process lease was claimed by another Hairball process")
                session.lease_owner = self._owner_id
            except Exception as exc:
                session.exited = True
                session.exit_code = -1
                session.completion_reason = "lost"
                session.termination_source = "lease_conflict"
                session.output_buffer = str(exc)
    ...
    def _reader_loop(self, session: ProcessSession):
        ...
        try:
            ...
            while True:
                ...
                try:
                    self._sync_artifact(session)
                except Exception:
                    logger.debug("Could not update process artifact %s", session.id, exc_info=True)
        except Exception as e:
    ...
    def _env_poller_loop(
        ...
        session.log_path = session.log_path or log_path
        session.pid_path = session.pid_path or pid_path
        session.exit_path = session.exit_path or exit_path
        quoted_log_path = shlex.quote(log_path)
        quoted_pid_path = shlex.quote(pid_path)
        quoted_exit_path = shlex.quote(exit_path)
        quoted_token_path = shlex.quote(session.token_path) if session.token_path else ""
        quoted_token = shlex.quote(session.process_token) if session.process_token else "''"
        ...
        while not session.exited:
            ...
            try:
                if session.lease_owner:
                    if not self._durable_store().heartbeat(
                        session.id, session.lease_owner, ttl=15,
                    ):
                        logger.info(
                            "Stopping duplicate poller for %s after lease loss",
                            session.id,
                        )
                        return
                ...
                # Check that the remote PID still has all three identities:
                # our random nonce, the original /proc start ticks, and live
                # PID state. This is the sandbox equivalent of the host PID
                # recycling guard used by detached local recovery.
                if session.process_token:
                    token_check = (
                        f"test \"$(cat {quoted_token_path} 2>/dev/null)\" = {quoted_token} && "
                        if quoted_token_path else ""
                    )
                    liveness_command = (
                        "read pid started token < " + quoted_pid_path + " 2>/dev/null; "
                        + token_check
                        + f"test \"$token\" = {quoted_token} && "
                        + "current=$(awk '{print $22}' /proc/$pid/stat 2>/dev/null); "
                        + "test -n \"$pid\" && "
                        + "{ test -z \"$started\" || test \"$current\" = \"$started\"; } && "
                        + "kill -0 \"$pid\" 2>/dev/null; echo $?"
                    )
                else:
                    # Compatibility for in-memory sessions created before the
                    # verified reconnect metadata existed. They are never
                    # adopted across restart, but their current-process poll
                    # behavior remains unchanged.
                    liveness_command = (
                        f"kill -0 \"$(cat {quoted_pid_path} 2>/dev/null)\" "
                        "2>/dev/null; echo $?"
                    )
                check = env.execute(liveness_command, timeout=5)
                ...
                logger.warning(
                    "Remote background poll failed for %s (%d/3): %s",
                    session.id,
                    transient_failures,
                    exc,
                )
                if transient_failures < 3:
                    continue
                # Re-resolve the exact backend once before classifying the
                # process as lost. A transient SSH control socket or restarted
                # client must not discard a still-running remote command.
                try:
                    if session.reconnect_spec:
                        from tools.terminal_tool import reconnect_environment
                        env = reconnect_environment(session.reconnect_spec)
                        session.env_ref = env
                        transient_failures = 0
                        continue
                except Exception as reconnect_exc:
                    logger.warning(
                        "Remote background reconnect failed for %s: %s",
                        session.id,
                        reconnect_exc,
                    )
                session.exited = True
    ...
    def _move_to_finished(self, session: ProcessSession):
        ...
        durable_first = was_running
        try:
            artifact_path, artifact_digest = self._sync_artifact(session)
            store = self._durable_store()
            if store.get(session.id) is not None:
                durable_first = store.mark_finished(
                    session.id,
                    exit_code=session.exit_code,
                    completion_reason=session.completion_reason,
                    termination_source=session.termination_source,
                    ...
                    event=event if session.notify_on_complete else None,
                )
            if session.lease_owner:
            ...
            try:
                ...
                db = SessionDB()
                try:
                    db.apply_kernel_ledger_migration()
                    byte_count = Path(artifact_path).stat().st_size
                    db.record_kernel_artifact({
                        "artifact_id": f"background-process:{session.id}",
                        "session_id": session.session_key or session.task_id or session.id,
                        "kind": "background_process_output",
                        "media_type": "text/plain",
                        "byte_count": byte_count,
                        "digest": artifact_digest,
                        "path": artifact_path,
                        "summary": f"Background process {session.id} output",
                        "truncated": False,
                ...
                logger.warning(
                    "Could not index background artifact %s: %s",
                    session.id,
                    artifact_exc,
                )
        except Exception as durable_exc:
            logger.warning(
                "Could not finalize durable background process %s: %s",
                session.id,
                durable_exc,
            )
        ...
        if was_running and durable_first and session.notify_on_complete:
    ...
    def has_pending_completion_delivery(
        self,
        session_id: str,
        *,
        inline_drain: bool = False,
    ) -> bool:
        """Whether ``notify_on_complete`` still owns a future agent wake.

        A short process can exit before the model finishes the turn that
        launched it.  At that point ``session.exited`` is true, but the durable
        completion outbox is still pending because UI/TUI/Gateway consumers
        defer injection while that turn is busy.  Stop orchestration must not
        race that outbox with a second continuation prompt.

        ``inline_drain`` models the classic CLI's special poll de-duplication:
        an exited process observed with ``process(action='poll')`` is skipped
        by the CLI post-turn drain, while watcher surfaces still deliver it.
        This method is read-only; it never dequeues, claims, or acknowledges an
        event.
        """
        ...
        if session is None or not bool(getattr(session, "notify_on_complete", False)):
            return False
        with self._lock:
            if sid in self._completion_consumed:
                return False
            if inline_drain and sid in self._poll_observed:
                return False
        if not bool(getattr(session, "exited", False)):
            return True

        try:
            durable = self._durable_store().get(sid)
        except Exception:
            # A legacy in-memory process may not have a durable row.  During
            # its launching turn, an unconsumed notify-on-complete exit still
            # owns the post-turn wake on the existing delivery rail.
            logger.debug(
                "Could not inspect durable completion delivery for %s",
                sid,
                exc_info=True,
            )
            return True
        if durable is None:
            return True
        return bool(durable.get("event")) and str(
            durable.get("delivery_state") or ""
        ) == "pending"

    def is_session_waiting(self, session_id: str) -> bool:
        """Whether a goal loop parked on this session should still be parked.

        Used by the goal-loop wait barrier (``hairball_cli.goals``) to support
    ...
    def _reconcile_local_exit(self, session: "ProcessSession") -> None:
        ...
        a daemon spawned by `hairball update` restarting the gateway) is still
        ...
        *,
        ...
        try:
            if session._pty:
                ...
                # Non-local -- verify nonce + /proc start ticks before kill.
                # Never trust a bare in-sandbox PID after reconnect.
                kill_command = (
                    self._remote_identity_command(session, kill=True)
                    if session.process_token and session.pid_path
                    else f"kill {session.pid} 2>/dev/null"
                )
                killed = session.env_ref.execute(kill_command, timeout=5)
                if not session.process_token:
                    lines = ["0"]
                else:
                    lines = str(killed.get("output", "")).strip().splitlines()
                if not lines or lines[-1].strip() != "0":
                    return {
                        "status": "already_exited",
                        "session_id": session.id,
                        "error": "Remote process identity no longer matches; refused to kill a stale PID",
                    }
    ...
    def request_close_terminal(self, session_id: str) -> dict:
        ...
        if sink is None:
            return {
                "status": "error",
                "error": "close_terminal is only available in the Hairball desktop app.",
            }
            ...
            with self._lock:
                ...
                for s in self._running.values():
                    if not s.exited:
                        ...
                        entries.append({
                            "session_id": s.id,
                            "command": self._redact_command(s.command),
                            "pid": s.pid,
                            "pid_scope": s.pid_scope,
                            "host_start_time": s.host_start_time,
                            "remote_start_time": s.remote_start_time,
                            "process_token": s.process_token,
                            "reconnect_spec": s.reconnect_spec,
                            "log_path": s.log_path,
                            "pid_path": s.pid_path,
    ...
    @staticmethod
    def _remote_identity_command(session: ProcessSession, *, kill: bool = False) -> str:
        pid_path = shlex.quote(session.pid_path)
        token_path = shlex.quote(session.token_path) if session.token_path else ""
        token = shlex.quote(session.process_token)
        token_check = (
            f"test \"$(cat {token_path} 2>/dev/null)\" = {token} && "
            if token_path else ""
        )
        action = "kill \"$pid\" 2>/dev/null" if kill else "kill -0 \"$pid\" 2>/dev/null"
        return (
            f"read pid started nonce < {pid_path} 2>/dev/null; "
            + token_check
            + f"test \"$nonce\" = {token} && "
            + "current=$(awk '{print $22}' /proc/$pid/stat 2>/dev/null); "
            + "test -n \"$pid\" && "
    ...
    def _remote_identity_is_live(self, session: ProcessSession, env: Any) -> bool:
        result = env.execute(self._remote_identity_command(session), timeout=10)
        output = str(result.get("output", "")).strip().splitlines()
        return bool(output and output[-1].strip() == "0")

    @staticmethod
    def _restore_watcher(session: ProcessSession, pending_watchers: List[Dict[str, Any]]) -> None:
        if session.watcher_interval <= 0:
            return
        pending_watchers.append({
            "session_id": session.id,
    ...
    def recover_from_checkpoint(self) -> int:
        ...
        if CHECKPOINT_PATH.exists():
            try:
                for entry in json.loads(CHECKPOINT_PATH.read_text(encoding="utf-8")):
                    if isinstance(entry, dict) and entry.get("session_id"):
                        entries_by_id[str(entry["session_id"])] = dict(entry)
            except Exception:
                pass

        try:
            store = self._durable_store()
            store.restore_pending_events(self.completion_queue)
            for row in store.list_recoverable():
                process_id = str(row.get("process_id") or "")
                if not process_id:
                    continue
                watcher = dict(row.get("watcher") or {})
                paths = dict(row.get("paths") or {})
                entry = entries_by_id.get(process_id, {})
                entry.update({
                    "session_id": process_id,
                    "command": row.get("command_redacted") or entry.get("command", "unknown"),
                    "pid": row.get("pid"),
                    "pid_scope": row.get("pid_scope") or "sandbox",
                    "remote_start_time": row.get("remote_start_time") or "",
                    "process_token": row.get("process_token") or "",
                    "reconnect_spec": row.get("reconnect") or {},
                    "log_path": paths.get("log", ""),
                    "pid_path": paths.get("pid", ""),
                ...
                entries_by_id[process_id] = entry
        except Exception as exc:
            logger.warning("Could not read durable background process rows: %s", exc)
        ...
        for entry in entries_by_id.values():
            ...
            if not session_id or self.get(session_id) is not None:
                continue
            pid = entry.get("pid")
            if not pid:
                continue
            ...
            if pid_scope != "host":
                reconnect_spec = dict(entry.get("reconnect_spec") or {})
                required = (
                    reconnect_spec,
                    entry.get("process_token"),
                    entry.get("pid_path"),
                    entry.get("log_path"),
                    entry.get("exit_path"),
                )
                if not all(required):
                    # Historical entries intentionally remain non-recoverable:
                    # an in-sandbox PID alone is unsafe to adopt on the host.
                    logger.info(
                        "Skipping recovery for non-host process without verified reconnect metadata: %s",
                        entry.get("command", "unknown")[:60],
                    )
                    continue
                try:
                    store = self._durable_store()
                    if not store.claim_lease(session_id, self._owner_id, ttl=15):
                        continue
                    from tools.terminal_tool import reconnect_environment
                    env = reconnect_environment(reconnect_spec)
                    session = ProcessSession(
                        id=session_id,
                        ...
                        pid=int(pid),
                        pid_scope="sandbox",
                        remote_start_time=str(entry.get("remote_start_time") or ""),
                        process_token=str(entry.get("process_token") or ""),
                        reconnect_spec=reconnect_spec,
                        ...
                        artifact_path=str(entry.get("artifact_path") or self._artifact_file(session_id)),
                    ...
                    if not self._remote_identity_is_live(session, env):
                        try:
                            log_result = env.execute(
                                f"cat {shlex.quote(session.log_path)} 2>/dev/null",
                                timeout=10,
                            )
                            session.output_buffer = str(log_result.get("output", ""))
                            exit_result = env.execute(
                                f"cat {shlex.quote(session.exit_path)} 2>/dev/null",
                                timeout=5,
                            )
                            session.exit_code = int(
                                str(exit_result.get("output", "-1")).strip().splitlines()[-1]
                            )
                        except Exception:
                            session.exit_code = -1
                        session.exited = True
                    ...
                    with self._lock:
                        self._running[session.id] = session
                    self._restore_watcher(session, self.pending_watchers)
                    reader = threading.Thread(
                        target=self._env_poller_loop,
                        args=(session, env, session.log_path, session.pid_path, session.exit_path),
                        daemon=True,
                        name=f"proc-reconnect-{session.id}",
                    )
                    session._reader_thread = reader
                    reader.start()
                    recovered += 1
                    logger.info(
                        "Recovered remote background process %s on %s",
                        session.id,
                        reconnect_spec.get("env_type", "remote"),
                    )
                except Exception as exc:
                    try:
                        self._durable_store().release_lease(
                            session_id, self._owner_id,
                        )
                    except Exception:
                        pass
                    logger.warning(
                        "Could not reconnect remote background process %s: %s",
                        session_id,
                        exc,
                    )
                continue
...
process_registry = ProcessRegistry(auto_recover=True)
...
def format_process_notification(evt: dict) -> "str | None":
    ...
    if evt_type == "async_delegation":
        # Collab Mailbox Single Rail: content via collab_collect — never
        # synthesize an inject turn from async_delegation events.
        try:
            ...
            if should_suppress_inject(evt):
                acknowledge_suppressed_inject(evt, "format-skip")
                return None
        except Exception:
            if evt.get("suppress_inject") or evt.get("delivery_channel") == "collab_mailbox":
                return None
    ...
    if _exit in {-15, 143, "-15", "143"}:
        _signal = ", SIGTERM"
    if _reason == "killed":
        _status = f"terminated by {_source or 'Hairball'}"
    elif _reason == "lost":
