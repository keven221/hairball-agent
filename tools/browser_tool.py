...
supports multiple backends — **Browser Use** (cloud, default for managed
...
from hairball_constants import agent_browser_runnable, get_hairball_home
from utils import env_int, is_truthy_value
from hairball_cli.config import DEFAULT_CONFIG, cfg_get
...
# means a compromised transitive dependency could read every Hairball secret
...
def _build_browser_env() -> dict:
    ...
    Strips Hairball-managed secrets (provider keys, gateway tokens, GitHub auth,
    infra secrets) then re-adds only the browser-backend keys the worker needs.
    The ``hairball_subprocess_env`` import is deferred to keep ``browser_tool``
    ...
    env = hairball_subprocess_env(inherit_credentials=False)
...
# Serializes the one supported implicit browser launch: the Hairball-managed
# default local CDP endpoint.  Browser tool calls may arrive concurrently from
# subagents, so probing + launching without a lock can spawn multiple Chromium
# processes against the same profile and port.
_default_cdp_launch_lock = threading.Lock()
...
def _browser_candidate_path_dirs() -> list[str]:
    ...
    hairball_node_bin = str(hairball_home / "node" / "bin")
    hairball_node_root = str(hairball_home / "node")
    hairball_nm_bin = str(hairball_home / "node_modules" / ".bin")
    return [hairball_node_bin, hairball_node_root, hairball_nm_bin, *list(_discover_homebrew_node_dirs()), *_SANE_PATH_DIRS]
...
def _create_command_output_files(command: str) -> tuple[int, str, int, str]:
    """Create private CLI capture files outside agent-browser's socket tree.

    A cold agent-browser daemon owns ``AGENT_BROWSER_SOCKET_DIR`` and may
    replace that directory while it initializes.  Capture files stored inside
    the socket tree can therefore disappear after ``Popen`` has inherited
    their descriptors but before Hairball reads them, producing a misleading
    ``_stdout_open`` FileNotFoundError.  System temporary files are still
    private (``mkstemp`` uses mode 0600), survive daemon socket housekeeping,
    and their unique names also prevent concurrent calls for the same session
    and command from truncating one another.
    """

    safe_command = re.sub(r"[^A-Za-z0-9_.-]+", "-", command).strip("-.") or "command"
    temp_root = _socket_safe_tmpdir()
    created: list[tuple[int, str]] = []
    try:
        stdout_fd, stdout_path = tempfile.mkstemp(
            prefix=f"hairball-browser-{safe_command}-",
            suffix=".stdout",
            dir=temp_root,
        )
        created.append((stdout_fd, stdout_path))
        stderr_fd, stderr_path = tempfile.mkstemp(
            prefix=f"hairball-browser-{safe_command}-",
            suffix=".stderr",
            dir=temp_root,
        )
        created.append((stderr_fd, stderr_path))
        return stdout_fd, stdout_path, stderr_fd, stderr_path
    except Exception:
        for fd, path in created:
            try:
                ...
                pass
            _unlink_command_output_files(path)
        raise
...
def _prepare_default_local_cdp_for_command(cdp_url: str) -> str:
    """Make Hairball's default local CDP endpoint ready for a real tool call.

    ``browser.cdp_url`` and ``BROWSER_CDP_URL`` remain authoritative: remote,
    custom-port, and concrete websocket endpoints are returned untouched and
    never fall back to another backend.  The sole self-healing case is the
    Hairball-managed discovery endpoint on localhost:9222, which is the same
    endpoint ``/browser connect`` already auto-launches with an isolated
    ``~/.hairball/chrome-debug`` profile.

    This helper is deliberately called from session creation rather than
    ``_get_cdp_override``.  Reading config, computing tool availability, or
    running ``doctor`` must never launch a browser as a side effect.
    """
    ...
    ready = is_browser_debug_ready(DEFAULT_BROWSER_CDP_URL, timeout=0.5)
    if not ready:
        with _default_cdp_launch_lock:
            ready = is_browser_debug_ready(DEFAULT_BROWSER_CDP_URL, timeout=0.5)
            if not ready:
                launch = launch_chrome_debug(port=9222)
                if launch.launched:
                    deadline = time.monotonic() + 10.0
                    while time.monotonic() < deadline:
                        if is_browser_debug_ready(DEFAULT_BROWSER_CDP_URL, timeout=0.5):
                            ready = True
                            break
                        time.sleep(0.5)
                if not ready:
                    hint = launch.hint or ""
                    logger.warning(
                        "Hairball default local CDP could not be started at %s%s",
                        DEFAULT_BROWSER_CDP_URL,
                        f": {_sanitize_url_for_logs(hint)}" if hint else "",
                    )

    if ready:
        return _resolve_cdp_override(DEFAULT_BROWSER_CDP_URL)
    return raw
...
# plugins drop in under ``~/.hairball/plugins/browser/<vendor>/``.
...
def _get_cloud_provider() -> Optional[CloudBrowserProvider]:
    ...
    fallback. If unset, fall back to Browser Use (managed managed gateway or
    ...
    browser plugins (``~/.hairball/plugins/browser/<vendor>/``) participate
    ...
    if resolved is None:
        # Auto-detect path: Browser Use first (managed managed gateway or
...
def _lightpanda_fallback_reason(engine: str, command: str, result: Dict[str, Any]) -> Optional[str]:
    ...
    the fallback result so CLI/TUI/gateway users can see when Hairball silently
...
def _run_chrome_fallback_command(
    ...
    def _run_tmp(cmd: str, cmd_args: List[str]) -> Dict[str, Any]:
        ...
        stdout_fd, stdout_path, stderr_fd, stderr_path = _create_command_output_files(cmd)
        try:
            ...
            #   in the Hairball CLI where sys.stdout and sys.stderr both report
...
def _socket_safe_tmpdir() -> str:
    ...
    append ``agent-browser-hairball_…`` the resulting socket path exceeds the
...
def _emergency_cleanup_all_sessions():
    ...
    crashed hairball processes — this way every clean hairball exit sweeps
    ...
    # Sweep orphans from other crashed hairball processes.  Safe even if we
    # never used the browser — uses owner_pid liveness to avoid reaping
    # daemons owned by other live hairball processes.
    try:
...
def _write_owner_pid(socket_dir: str, session_name: str) -> None:
    """Record the current hairball PID as the owner of a browser socket dir.

    Written atomically to ``<socket_dir>/<session_name>.owner_pid`` so the
    orphan reaper can distinguish daemons owned by a live hairball process
...
def _reap_orphaned_browser_sessions():
    ...
    daemons whose owning hairball process is no longer alive.

    Ownership detection priority:
      1. ``<session>.owner_pid`` file (written by current code) — if the
         referenced hairball PID is alive, leave the daemon alone regardless
         of whether it's in *this* process's ``_active_sessions``.  This is
         cross-process safe: two concurrent hairball instances won't reap each
         other's daemons.
    ...
    socket_dirs += glob.glob(os.path.join(tmpdir, "agent-browser-hairball_*"))
    ...
    for socket_dir in socket_dirs:
        ...
        if owner_alive is True:
            # Owner is alive — this session belongs to a live hairball process.
            continue
...
def _get_session_info(task_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    if cdp_override and not force_local:
        cdp_override = _prepare_default_local_cdp_for_command(cdp_override)
...
def _find_windows_bundled_agent_browser(
    package_roots: Optional[List[Path]] = None,
) -> Optional[str]:
    """Return a runnable native executable from the installed npm package.

    Some agent-browser releases contain a zero-byte win32-arm64 placeholder.
    Its JavaScript shim selects that file on Windows ARM64 and fails with
    ``spawn EFTYPE`` even though the same official package's x64 executable
    runs through Windows' x64 compatibility layer. Prefer the host-native
    binary, then the package's x64 binary, before falling back to an implicit
    ``npx`` network install.
    """
    if os.name != "nt":
        return None
    ...
    if package_roots is None:
        ...
        package_roots = [
            get_hairball_home() / "node" / "node_modules" / "agent-browser",
    ...
    names = (
        ...
        else ["agent-browser-win32-x64.exe", "agent-browser-win32-arm64.exe"]
    )
    seen: set[str] = set()
...
def _find_agent_browser(*, validate: bool = True) -> str:
    ...
    Checks in order: current PATH, Homebrew/common bin dirs, Hairball-managed
    ...
    # node_modules binary, which disappears on the next ``hairball update`` and
    ...
    # Build an extended search PATH including Hairball-managed Node, macOS
    ...
    # Windows ARM64 packages can expose a broken/empty native shim target.
    # Reuse a runnable binary from that same installed package before npx is
    # allowed to fetch another version from the network.
    windows_native = _find_windows_bundled_agent_browser()
    if windows_native:
        _cached_agent_browser = windows_native
        _agent_browser_resolved = True
        return _cached_agent_browser
    ...
    try:
        ...
        if ensure_dependency("browser"):
            candidates = [
                ...
                shutil.which("agent-browser", path=str(get_hairball_home() / "node_modules" / ".bin")),
                shutil.which("agent-browser", path=str(get_hairball_home() / "node" / "bin")),
                shutil.which("agent-browser", path=str(get_hairball_home() / "node")),
            ]
...
def _terminate_browser_command_tree(proc: subprocess.Popen) -> None:
    """Stop a timed-out browser CLI without leaking npm/node descendants."""
    if os.name == "nt":
        _terminate_chromium_installer_tree(proc)
        return
    try:
        proc.kill()
    except OSError:
...
def _explicit_browser_executable_args(session_info: Dict[str, Any]) -> List[str]:
    """Translate Hairball's explicit local browser override to CLI arguments."""
    if session_info.get("cdp_url"):
        return []
    path = os.environ.get("AGENT_BROWSER_EXECUTABLE_PATH", "").strip()
    if not path or not (os.path.isfile(path) or shutil.which(path)):
        return []
...
def _run_browser_command(
    ...
    if engine != "lightpanda":
        backend_args += _explicit_browser_executable_args(session_info)
    ...
    try:
        ...
        os.makedirs(task_socket_dir, mode=0o700, exist_ok=True)
        # Record this hairball PID as the session owner (cross-process safe
        ...
        stdout_fd, stdout_path, stderr_fd, stderr_path = _create_command_output_files(command)
        try:
        ...
        try:
            ...
            _terminate_browser_command_tree(proc)
            proc.wait()
...
def _maybe_start_recording(task_id: str):
    ...
    try:
        ...
        recordings_dir = hairball_home / "browser_recordings"
...
def browser_vision(question: str, annotate: bool = False, task_id: Optional[str] = None) -> Union[str, Dict[str, Any]]:
    ...
    conversation so the model can inspect it on the next turn; otherwise Hairball
    ...
    screenshots_dir = get_hairball_dir("cache/screenshots", "browser_screenshots")
...
def _terminate_chromium_installer_tree(proc: subprocess.Popen) -> None:
    """Stop an installer and every downloader process it spawned."""
    if os.name == "nt":
        try:
            subprocess.run(
    ...
    # _run_chromium_installer starts a new POSIX session, so the installer's
    # PID is also the process-group ID and cannot overlap Hairball itself.
    try:
    ...
    # The launcher may have exited while a native downloader ignored SIGTERM.
    # Probe the whole group rather than treating launcher exit as tree exit.
    try:
...
def _run_chromium_installer(
    ...
    *,
    ...
    """Run a Chromium installer with a hard timeout and process-tree cleanup."""
    command = install_cmd
    popen_kwargs: dict = {
    ...
    if os.name == "nt":
        # agent-browser/npm/npx commonly resolve to .cmd launchers on Windows.
        # Route through cmd.exe explicitly while keeping a distinct process
        # group that taskkill /T can terminate on timeout.
        command = [
            os.environ.get("ComSpec", "cmd.exe"),
            "/d",
            "/s",
            "/c",
            subprocess.list2cmdline(install_cmd),
        ]
    ...
    # Browser installation is non-interactive. Keep stdin explicit at the
    # callsite so the TUI subprocess guard can verify the RPC fd is isolated.
    proc = subprocess.Popen(command, stdin=subprocess.DEVNULL, **popen_kwargs)
    try:
        ...
        raise subprocess.TimeoutExpired(install_cmd, timeout, output=stdout, stderr=stderr)

    return subprocess.CompletedProcess(
        install_cmd, proc.returncode, stdout=stdout, stderr=stderr
    )
        ...
        try:
            ...
            if _requires_real_termux_browser_install(browser_cmd):
                ...
                if _running_in_docker():
                    ...
                    print("       docker pull ghcr.io/hairball-agent/hairball-agent:latest")
                else:
