...
class TodoStore:
    ...
    def __init__(self):
        ...
        self._plan_scope_snapshot: Optional[List[Dict[str, str]]] = None
        self._plan_scope_items: Dict[str, str] = {}
        self._plan_scope_order: List[str] = []

    def begin_plan_scope(self, steps: List[Dict[str, Any]]) -> List[Dict[str, str]]:
        """Install approved Plan steps as the turn-local Todo source of truth.

        A Build turn must not let ``todo(..., merge=False)`` silently delete an
        approved step and thereby escape the normal pending-Todo settle check.
        The caller restores the pre-turn Todo list with :meth:`end_plan_scope`;
        Plan execution therefore does not leak its checklist into later normal
        chat turns on a reused Agent instance.
        """

        if self._plan_scope_snapshot is not None:
            raise RuntimeError("a Plan Todo scope is already active")
        normalized: List[Dict[str, str]] = []
        seen: set[str] = set()
        for index, raw in enumerate(steps or [], start=1):
            if not isinstance(raw, dict):
                continue
            item_id = str(raw.get("id") or f"step-{index}").strip()
            ...
            if status not in VALID_STATUSES:
                status = "pending"
            normalized.append(
                {"id": item_id, "content": self._cap_content(content), "status": status}
            )
        if not normalized:
            return []
        self._plan_scope_snapshot = self.read()
        self._plan_scope_items = {item["id"]: item["content"] for item in normalized}
        self._plan_scope_order = [item["id"] for item in normalized]
        self._items = [item.copy() for item in normalized]
        return self.read()

    def plan_scope_outcomes(self) -> List[Dict[str, str]]:
        """Return exact approved-step outcomes while a Plan scope is active."""

        if self._plan_scope_snapshot is None:
            return []
        by_id = {item["id"]: item for item in self._items}
        return [
            {
                "id": item_id,
                "content": self._plan_scope_items[item_id],
                "status": str(by_id.get(item_id, {}).get("status") or "pending"),
            }
            for item_id in self._plan_scope_order
        ]

    def end_plan_scope(self) -> List[Dict[str, str]]:
        """Return Plan outcomes and restore the Todo list from before Build."""

        outcomes = self.plan_scope_outcomes()
        if self._plan_scope_snapshot is not None:
            self._items = [item.copy() for item in self._plan_scope_snapshot]
        self._plan_scope_snapshot = None
        self._plan_scope_items = {}
        self._plan_scope_order = []
        return outcomes

    def _enforce_plan_scope(self, prior_status: Dict[str, str]) -> None:
        """Keep every approved step present and its reviewed text immutable."""
        ...
        for item_id in self._plan_scope_order:
            ...
            status = str(item.get("status") or prior_status.get(item_id) or "pending")
            if status not in VALID_STATUSES:
                status = "pending"
            protected.append(
                {
        ...
        protected_ids = set(self._plan_scope_order)
        self._items = protected + [
            item for item in self._items if item["id"] not in protected_ids
        ]
    ...
    def write(self, todos: List[Dict[str, Any]], merge: bool = False) -> List[Dict[str, str]]:
        ...
        """
        prior_status = {item["id"]: item["status"] for item in self._items}
        if not merge:
            ...
            for item in self._items:
                ...
                if current["id"] not in seen:
                    ...
                    seen.add(current["id"])
            self._items = rebuilt
        self._enforce_plan_scope(prior_status)
    ...
    def active_items(self) -> List[Dict[str, str]]:
        """Return pending/in-progress items as a defensive ordered copy.

        The list remains the single canonical Todo state.  Turn-settle,
        compression, and UI adapters consume this projection instead of
        maintaining a second completion ledger that can drift from the tool.
        """
        return [
            item.copy()
            for item in self._items
