...
One ``CDPSupervisor`` runs per Hairball ``task_id`` that has a reachable CDP
...
def _redact_cdp_error_text(exc: object) -> str:
    ...
    those credentials never reach Hairball logs or tracebacks. Falls back to a
...
DIALOG_BRIDGE_HOST = "hairball-dialog-bridge.invalid"
...
(() => {
  if (window.__hairballDialogBridgeInstalled) return;
  window.__hairballDialogBridgeInstalled = true;
  const ENDPOINT = "http://hairball-dialog-bridge.invalid/";
