...
Dependencies (optional):
    ...
    or: pip install hairball-agent[voice]
"""
...
def _import_numpy():
    """Lazy-import numpy without initializing the audio output backend."""
    import numpy as np
    return np


def _sounddevice_output_allowed() -> bool:
    """Whether sounddevice may be used for output on this platform.

    On macOS, local output is routed through ``afplay``. Initializing
    PortAudio only to play a tone can otherwise trigger an unrelated media
    permission prompt. Audio input remains available to the recorder.
    """
    return platform.system() != "Darwin"


def _play_int16_via_tempfile(audio, sample_rate: int) -> None:
    """Play synthesized int16 mono PCM through the normal file player."""
    temp_path = None
    try:
        ...
        logger.debug("Tone tempfile playback failed: %s", exc)
    finally:
        if temp_path:
            try:
                os.unlink(temp_path)
...
_TEMP_DIR = os.path.join(tempfile.gettempdir(), "hairball_voice")
...
def _get_beep_volume() -> float:
    """Read and validate ``voice.beep_volume`` from config.yaml."""
    try:
        ...
        raw = voice_config.get("beep_volume", _DEFAULT_BEEP_VOLUME)
    except Exception:
        return _DEFAULT_BEEP_VOLUME
    try:
        value = float(raw)
        ...
        return _DEFAULT_BEEP_VOLUME
    if isinstance(raw, bool) or value < 0.0 or value > 1.0 or math.isnan(value):
        return _DEFAULT_BEEP_VOLUME
    return value
...
def play_beep(frequency: int = 880, duration: float = 0.12, count: int = 1) -> None:
    ...
    try:
        ...
        for i in range(count):
            ...
            parts.append((tone * _get_beep_volume() * 32767).astype(np.int16))
            if i < count - 1:
...
# ============================================================================
# Audio activity and ambient thinking cue
# ============================================================================
...
def mark_audio_output_active(active: bool) -> None:
    """Reference-count real audio output across nested playback paths."""
...
def is_audio_output_active() -> bool:
    """Return True only while audio is actually leaving the speakers."""
    with _audio_output_lock:
...
def thinking_sound_enabled() -> bool:
    """Return the ``voice.thinking_sound`` config gate (default True)."""
    try:
        from hairball_cli.config import load_config

        voice_config = load_config().get("voice", {})
        if isinstance(voice_config, dict):
            value = voice_config.get("thinking_sound", True)
...
def _synth_thinking_blip(np, frequency: float) -> Any:
    """Synthesize one quiet, click-free status blip."""
    duration = 0.16
    sample_count = int(SAMPLE_RATE * duration)
    timeline = np.linspace(0, duration, sample_count, endpoint=False)
    glide = np.linspace(1.0, 0.72, sample_count)
    phase = 2 * np.pi * np.cumsum(frequency * glide) / SAMPLE_RATE
    tone = 0.8 * np.sin(phase) + 0.2 * np.sin(phase / 2.0)
    attack = int(0.02 * SAMPLE_RATE)
    envelope = np.ones(sample_count)
    envelope[:attack] = np.linspace(0.0, 1.0, attack)
    envelope *= np.exp(-timeline * 14.0)
    volume = _get_beep_volume() * 0.5
    return (tone * envelope * volume * 32767).astype(np.int16)


def _thinking_sound_loop(stop_event: threading.Event, should_play) -> None:
    if not _sounddevice_output_allowed():
        return
    try:
        sd, np = _import_audio()
    ...
    blips = [_synth_thinking_blip(np, pitch) for pitch in (392.0, 329.6)]
    index = 0
    while not stop_event.is_set():
        try:
            if should_play is None or should_play():
                blip = blips[index % len(blips)]
                sd.play(blip, samplerate=SAMPLE_RATE)
                stop_event.wait(len(blip) / SAMPLE_RATE + 0.02)
                sd.stop()
                index += 1
        except Exception as exc:
            logger.debug("Thinking sound playback failed: %s", exc)
            return
        stop_event.wait(0.8 + random.random() * 0.4)


def start_thinking_sound(should_play=None) -> bool:
    """Start the idempotent ambient thinking cue."""
    ...
    with _thinking_lock:
        ...
        _thinking_stop = stop_event
    threading.Thread(
        target=_thinking_sound_loop,
        args=(stop_event, should_play),
        daemon=True,
...
def stop_thinking_sound() -> None:
    """Stop the ambient thinking cue immediately and idempotently."""
    global _thinking_stop
    with _thinking_lock:
        stop_event, _thinking_stop = _thinking_stop, None
...
def _load_voice_stop_phrases() -> tuple:
    """Return normalized ``voice.stop_phrases``; an empty list disables it."""
    try:
        from hairball_cli.config import load_config

        voice_config = load_config().get("voice", {})
        if isinstance(voice_config, dict):
            raw = voice_config.get("stop_phrases", DEFAULT_VOICE_STOP_PHRASES)
            ...
            if isinstance(raw, (list, tuple)):
                return tuple(
                    ...
                    for phrase in raw
                    if isinstance(phrase, (str, int, float))
...
def is_voice_stop_phrase(
    transcript: str,
    stop_phrases: Optional[tuple] = None,
) -> bool:
    """Match only a whole utterance, never a phrase embedded in a request."""
    if not transcript:
        return False
...
def voice_stop_hint() -> str:
    """Return the UI hint for the first configured stop phrase."""
...
def transcribe_recording(wav_path: str, model: Optional[str] = None) -> Dict[str, Any]:
    ...
    # A configured stop phrase must survive the hallucination filter. This is
    # important for phrases such as "bye" that also occur in silence output.
    if result.get("success"):
        transcript = result.get("transcript", "")
        if is_whisper_hallucination(transcript) and not is_voice_stop_phrase(transcript):
            logger.info("Filtered Whisper hallucination: %r", transcript)
...
def _play_audio_file_impl(file_path: str) -> bool:
    ...
    # Try sounddevice for WAV files when output initialization is safe.
...
# ============================================================================
# The listener starts before model generation, calibrates in the quiet room,
# freezes that floor during playback, and uses a majority window so brief
# intra-word dips do not prevent a real user interruption from registering.
...
def _voice_debug_enabled() -> bool:
    return os.environ.get("HAIRBALL_VOICE_DEBUG", "").strip() == "1"


def _vad_log(message: str) -> None:
    logger.debug(message)
    if _voice_debug_enabled():
        try:
            print(f"[voice-vad] {message}", file=sys.stderr, flush=True)
        except Exception:
            pass
...
def full_duplex_listen(
    ...
    """Listen across a whole agent turn and capture a spoken interruption.

    ``on_trigger`` is called with ``generation`` or ``playback`` before the
    utterance is collected. The host can then interrupt model generation and
    local playback without coupling that policy to the audio detector.
    """
    try:
        sd, np = _import_audio()
    except (ImportError, OSError):
    ...
    calibration_blocks = max(1, calibration_ms // 30)
    ...
    trigger_multiplier = (
        float(multiplier) if multiplier else DEFAULT_BARGE_MULTIPLIER
    )

    ambient = deque(maxlen=100)
    pre_roll = deque(maxlen=max(1, pre_roll_ms // 30))
    recent_above = deque(maxlen=trip_blocks)
        ...
        with sd.InputStream(
            ...
            while not should_stop():
                ...
                if not floor_locked:
                    if not playing:
                        ambient.append(rms)
                    if len(ambient) >= calibration_blocks or playing:
                        percentile = (
                            float(np.percentile(list(ambient), 90))
                            if ambient
                            else float(SILENCE_RMS_THRESHOLD)
                        )
                        quiet_floor = max(percentile, float(SILENCE_RMS_THRESHOLD))
                        floor_locked = True
                        _vad_log(
                            f"calibrated quiet floor={quiet_floor:.0f} "
                            f"({len(ambient)} blocks, mult={trigger_multiplier:g})"
                        )
                    if not floor_locked:
                        continue
                ...
                trigger = quiet_floor * trigger_multiplier
                if playing:
                ...
                _vad_log(
                    f"voice interruption ({phase}) rms={rms:.0f} "
                ...
                if on_trigger is not None:
                    try:
                        ...
                        logger.debug("Full-duplex trigger callback failed: %s", exc)
                ...
                for _ in range(max_blocks):
                    ...
                    if quiet_blocks >= endpoint_blocks:
                        break
