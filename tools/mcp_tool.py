...
transport, discovers their tools, and registers them into the hairball-agent
tool registry so the agent can call them like any built-in tool.

Configuration is read from ~/.hairball/config.yaml under the ``mcp_servers`` key.
...
# per-profile log file (~/.hairball/logs/mcp-stderr.log), tagged with the
...
def _check_logging_callback_support() -> bool:
    ...
    server emits, so server-side diagnostics never reach Hairball' logs.
    """
    if not _MCP_AVAILABLE:
        return False
    try:
...
_SAFE_ENV_KEYS = frozenset({
    ...
    # Packaged Hairball executes stdio MCP servers through a Python watchdog
    # and an Electron-backed ``node`` wrapper.  These values are paths and
    # interpreter policy, not credentials.  Dropping them makes the watchdog
    # write bytecode into the signed application and makes the Node wrapper
    # exit before the real MCP server starts.
    "PYTHONPYCACHEPREFIX", "PYTHONDONTWRITEBYTECODE", "HAIRBALL_ELECTRON_NODE",
})
...
def _resolve_stdio_command(command: str, env: dict) -> tuple[str, dict]:
    ...
    if os.sep not in resolved_command:
        ...
        if which_hit:
            ...
            hairball_home = os.path.expanduser(
                os.getenv(
                    "HAIRBALL_HOME", os.path.join(os.path.expanduser("~"), ".hairball")
                )
            )
            candidates = [
                os.path.join(hairball_home, "node", "bin", resolved_command),
                ...
                # image (which the Hairball Docker image copies node + npm +
...
# ---------------------------------------------------------------------------
# MCP ImageContent block → Hairball MEDIA tag
# ---------------------------------------------------------------------------
...
def _cache_mcp_image_block(block) -> str:
    ...
    return a ``MEDIA:<path>`` tag that Hairball gateways know how to render.
...
def _render_mcp_resource_block(block, server_name: str = "") -> str:
    ...
    - ``EmbeddedResource`` with blob contents → bytes are decoded (size-capped)
      and materialized into the Hairball document cache; returns a marker with
...
class ElicitationHandler:
    ...
    Form-mode elicitations are routed through Hairball' existing approval
    ...
    async def __call__(self, context, params):
        ...
        # the agent's contextvars (HAIRBALL_SESSION_PLATFORM etc.). When
...
class MCPServerTask:
    ...
    def __init__(self, name: str):
        ...
        # context doesn't inherit HAIRBALL_SESSION_PLATFORM, so the
    ...
    def _make_logging_callback(self):
        ...
        server into Hairball' logging (agent.log via hairball_logging), tagged
    ...
    async def _run_stdio(self, config: dict):
        ...
        if not _MCP_AVAILABLE:
            raise ImportError(
                ...
                "  pip install 'hairball-agent[mcp]'\n"
                "or (full install):\n"
                "  pip install 'hairball-agent[all]'"
            )
        ...
        # ungraceful exit of this Hairball process (kill -9, crash, force-quit)
                ...
                if (
                    ...
                    post_resp = await client.post(
                        ...
                        content=(
                            ...
                            '"capabilities":{},'
                            '"clientInfo":{"name":"hairball-probe",'
                            '"version":"0.1"}}}'
                        ),
                    )
    ...
    async def run(self, config: dict):
        ...
        # routes those requests through Hairball' approval system.
        ...
        while True:
            try:
                ...
                # and the MCP server would stay dead until Hairball is fully
...
def _handle_auth_error_and_retry(
    ...
    return json.dumps({
        "error": (
            ...
            f"Run `hairball mcp login {server_name}` (or delete the tokens "
            f"file under ~/.hairball/mcp-tokens/ and restart). Do NOT retry "
...
def _wrap_with_home_override(coro: "Coroutine") -> "Coroutine":
    """Carry the caller's context-local HAIRBALL_HOME override into ``coro``.
    ...
    try:
        ...
        home_override = get_hairball_home_override()
    except Exception:
        return coro
    ...
    async def _scoped():
        token = set_hairball_home_override(home_override)
        try:
            return await coro
        finally:
...
def _run_on_mcp_loop(coro_or_factory, timeout: float = 30):
    ...
    # Propagate the context-local HAIRBALL_HOME override onto the MCP loop.
    ...
    # vanish here: OAuth token stores and any other get_hairball_home()
...
def _load_mcp_config() -> Dict[str, dict]:
    """Read ``mcp_servers`` from the Hairball config file.
    ...
    ``os.environ`` (which includes ``~/.hairball/.env`` loaded at startup).
    """
    try:
        from hairball_cli.config import load_config
        from utils import env_var_enabled as _env_enabled

        if _env_enabled("HAIRBALL_SAFE_MODE"):
            return {}
        config = load_config()
...
def _make_tool_handler(server_name: str, tool_name: str, tool_timeout: float):
    ...
    def _handler(args: dict, **kwargs) -> str:
        ...
        async def _call():
            ...
            # flow through Hairball' MEDIA: tag convention and out to messaging
            ...
            # Hairball' MEDIA tag + cache_image_from_bytes) was the cleaner of
...
def sanitize_mcp_name_component(value: str) -> str:
    ...
    Preserves Hairball's historical behavior of converting hyphens to
...
# Native MCP tool-name prefix. Hairball uses the ``mcp__<server>__<tool>``
# convention shared by Claude Code, reference, and OpenCode (anomalyco/opencode
...
def _convert_mcp_schema(server_name: str, mcp_tool) -> dict:
    """Convert an MCP tool listing to the Hairball registry schema format.
...
def probe_mcp_server_tools() -> Dict[str, List[tuple]]:
    ...
    Designed for ``hairball tools`` interactive configuration — connects to each
    enabled server, grabs tool names and descriptions, then disconnects.
    Does NOT register tools in the Hairball registry.
...
# mid-read could otherwise expose a half-updated list. The session execution
# router is published under the same lock.
...
def refresh_agent_mcp_tools(
    ...
    *after* that snapshot — a slow HTTP/OAuth server that misses the bounded
    ...
    families directly onto ``agent.tools`` *after* that — session-internal,
    external
    ...
    surface.  The new ``(tools, valid_tool_names, session_tool_router)``
    snapshot is published together
    ...
    """
    from model_tools import build_session_tool_router, get_tool_definitions
    from tools.registry import registry
    ...
    new_router = build_session_tool_router(
        model_visible_specs=new_defs,
        enabled_toolsets=enabled,
        disabled_toolsets=disabled,
        visible_toolsets=getattr(agent, "visible_toolsets", None),
        quiet_mode=True,
    ...
    with _agent_tools_lock:
        ...
        current_router = getattr(agent, "_session_tool_router", None)
        current_authorized = getattr(
            current_router, "authorized_tool_names", frozenset(current)
        )
        if (
            new_names == current
            and new_router.authorized_tool_names == current_authorized
        ):
        ...
        agent._session_tool_router = new_router
...
def _reinject_post_build_tools(agent, tools_list: list, name_set: set) -> set:
    """Append non-registry tool families onto staged locals.
...
def _kill_orphaned_mcp_children(
    ...
    survivors, avoiding shared-resource collisions when multiple hairball
