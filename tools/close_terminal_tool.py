#!/usr/bin/env python3
"""Close a read-only agent terminal tab in the Hairball desktop GUI.
...
``read_terminal`` it is gated on ``HAIRBALL_DESKTOP`` so it never appears outside
the GUI.
"""
...
def check_close_terminal_requirements() -> bool:
    """Desktop GUI only — HAIRBALL_DESKTOP is set on the gateway the app spawns."""
    return env_var_enabled("HAIRBALL_DESKTOP")
...
CLOSE_TERMINAL_SCHEMA = {
    ...
    "description": (
        ...
        "the Hairball desktop GUI (the tabs mirroring terminal(background=true) runs). "
