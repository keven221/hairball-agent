...
_FENCE_MARKER_RE = re.compile(r"'?\x07?__HAIRBALL_FENCE_[A-Za-z0-9]+__\x07?'?")
_EMBEDDED_LINE_GUTTER_RE = re.compile(r"^\s*(?P<line>\d+)\s+\|(?:\s|$)")


def _has_dominant_embedded_line_gutter(content: str) -> bool:
    """Detect source packets that already carry their own line coordinates.

    Hairball's compact ``LINE|CONTENT`` gutter is useful for ordinary source
    files, but adding it to a document whose body already consists of rows such
    as ``0001 | ...`` creates two competing coordinate systems.  Preserve the
    embedded gutter only when it clearly dominates the non-empty content and
    advances consecutively.  A reset to line 1 is allowed so one document can
    contain multiple independently numbered source sections.

    Requiring whitespace before the embedded ``|`` keeps this distinct from
    Hairball's own compact ``1|content`` display.  Sparse examples and broken
    sequences continue to receive the normal physical-file gutter.
    """
    ...
    embedded_numbers: List[int] = []
    for line in non_empty:
        match = _EMBEDDED_LINE_GUTTER_RE.match(line)
        if match is not None:
            embedded_numbers.append(int(match.group("line")))

    if len(embedded_numbers) < 2:
        return False
    if len(embedded_numbers) / len(non_empty) < 0.6:
        return False

    return all(
        current == previous + 1 or current == 1
        for previous, current in zip(embedded_numbers, embedded_numbers[1:])
    )
...
def _strip_terminal_fence_leaks(text: str) -> str:
    ...
    for line in text.splitlines(keepends=True):
        had_terminal_wrapper = "__HAIRBALL_FENCE_" in line or "\x1b]" in line
...
@dataclass
class SearchMatch:
    """A single search match or context row."""
    path: str
    line_number: int
    content: str
    mtime: float = 0.0  # Modification time for sorting
    # True for rg/grep ``-C`` neighbor rows. Kept in ``matches`` for display /
    # densify, but excluded from ``total_count`` (real hit count only).
...
class SearchResult:
    ...
    # Real pattern hits in the *returned page* (not a cross-page global
    # total — Cursor agent Grep likewise only exposes the current page).
    # Context rows from ``context>0`` do not increment this.
    total_count: int = 0
    # Content-mode rows in the returned page, including context neighbors.
    total_lines: int = 0
    # Applied pagination (echoed like Cursor "limit/offset" footer).
    offset: int = 0
    limit: int = 0
    truncated: bool = False
    ...
    def _densify_matches(self) -> Optional[str]:
        ...
        for m in self.matches:
            ...
            # Match rows use ripgrep's ``:`` separator; context rows use ``-``
            # so models can tell hits from neighbors without a decode step.
            sep = "-" if m.is_context else ":"
            lines.append(f"  {m.line_number}{sep} {m.content.rstrip()}")
        return "\n".join(lines)

    def to_dict(self, densify: bool = False) -> dict:
        result: dict[str, object] = {"total_count": self.total_count}
        # Mirror Cursor: page hit count + expanded row count; echo limit/offset
        # so models paginate from the footer instead of inventing globals.
        if self.matches or self.total_lines:
            result["total_lines"] = self.total_lines or len(self.matches)
        if self.limit or self.offset or self.truncated:
...
class FileOperations(ABC):
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
               ...
               context_after: int = 0) -> SearchResult:
...
def _maybe_warn_line_oriented_newline_pattern(
    result: SearchResult, pattern: str, *, multiline: bool = False,
) -> SearchResult:
    """Attach a newline-regex warning only when search found no usable results."""
    if multiline or result.total_count != 0 or not _pattern_has_regex_newline(pattern):
        return result
    if result.error and not _is_line_oriented_newline_error(result.error):
        return result
    result.error = None
    result.warning = (
        "0 results found. Note: content search is line-oriented by default. "
        "Set multiline=true for ripgrep -U across line breaks, or use "
        "context=N / escape as `\\\\n` for a literal backslash+n."
    )
    return result


def _resolve_content_context_window(
    context: int = 0,
    context_before: int = 0,
    context_after: int = 0,
) -> tuple[int, int, bool]:
    """Resolve Cursor-style context window.

    Returns ``(before, after, asymmetric)``.
    - If either before/after is set: use ``-B``/``-A``; missing side falls
      back to symmetric ``context``.
    - Else: both sides equal ``context`` (caller emits ``-C``).
    """
    context = max(0, _coerce_int(context, 0))
    context_before = max(0, _coerce_int(context_before, 0))
    context_after = max(0, _coerce_int(context_after, 0))
    asymmetric = context_before > 0 or context_after > 0
    if asymmetric:
        before = context_before if context_before > 0 else context
        after = context_after if context_after > 0 else context
    else:
        before = after = context
    return before, after, asymmetric


def _append_rg_grep_context_flags(
    cmd_parts: list,
    *,
    before: int,
    after: int,
    ...
    """Append ``-C`` or ``-B``/``-A`` for content search."""
    if asymmetric:
        if before > 0:
        ...
        cmd_parts.extend(["-C", str(before)])


def _page_content_matches(
    ...
    after: int = 0,
    context: int = 0,
) -> tuple[List[SearchMatch], int, int, bool]:
    """Page content results.

    Without context, ``limit``/``offset`` slice the flat row list.
    With context, they apply to real hits only; at most ``before``/``after``
    neighbor rows on each side stay attached (Cursor head_limit ≈ match
    budget). Legacy callers may pass symmetric ``context`` only.

    Returns ``(page, hit_count, row_total, truncated_by_page)``.
    """
    if context and not (before or after):
        before = after = max(0, int(context))
    before = max(0, int(before or 0))
    after = max(0, int(after or 0))

    row_total = len(matches)
    hit_indices = [i for i, m in enumerate(matches) if not m.is_context]
    hit_count = len(hit_indices)
    if before <= 0 and after <= 0:
        page = matches[offset:offset + limit]
        return page, hit_count, row_total, row_total > offset + limit

    if not hit_indices:
        # Context-only buffer (partial parse / diagnostics): row-page as-is.
        page = matches[offset:offset + limit]
        return page, 0, row_total, row_total > offset + limit

    selected = hit_indices[offset:offset + limit]
    if not selected:
        return [], hit_count, row_total, hit_count > offset
    ...
    for hi in selected:
        ...
        while (
            j >= 0
            and taken < before
            and matches[j].is_context
            and matches[j].path == hit.path
        ):
            include.add(j)
            taken += 1
            j -= 1
        ...
        while (
            ...
            taken += 1
            j += 1
    page = [matches[i] for i in sorted(include)]
    return page, hit_count, row_total, hit_count > offset + limit
...
class ShellFileOperations(FileOperations):
    ...
    def begin_patch_transaction(self, paths: List[str]) -> Dict[str, Any]:
        """Create byte-exact, same-directory backups for a V4A transaction.

        The backup work runs inside the selected terminal backend, so local,
        Docker, SSH, Modal, Daytona, and Windows paths are all interpreted by
        the filesystem that will actually receive the patch.  Backups live
        beside their targets so rollback can use an atomic ``os.replace``.
        """
        unique_paths = list(dict.fromkeys(self._expand_path(str(path)) for path in paths))
        tx_id = uuid.uuid4().hex
        snippet = (
            "import json, os, pathlib, shutil, sys\n"
            f"paths = {unique_paths!r}\n"
            f"tx_id = {tx_id!r}\n"
            "entries = []\n"
            "try:\n"
            ...
            "        backup = path.with_name('.hairball-patch-backup.' + tx_id + '.' + str(index))\n"
            "        existed = os.path.lexists(str(path))\n"
            "        if existed:\n"
            "            if path.is_dir() and not path.is_symlink():\n"
            "                raise IsADirectoryError(str(path))\n"
            "            if path.is_symlink():\n"
            "                os.symlink(os.readlink(str(path)), str(backup))\n"
            "            else:\n"
            "                shutil.copy2(str(path), str(backup), follow_symlinks=False)\n"
            "        entries.append({'path': str(path), 'existed': existed, "
            ...
            "    raise\n"
            "print('HAIRBALL_PATCH_TX:' + json.dumps(entries, separators=(',', ':')))\n"
        )
        interpreter = "python3" if self._has_command("python3") else "python"
        result = self._exec(f"{interpreter} -c {self._escape_shell_arg(snippet)}")
        if result.exit_code != 0:
            raise RuntimeError(
                "Could not snapshot files before applying the patch: "
                + ((result.stdout or "").strip() or "unknown backend error")
            )
        marker = "HAIRBALL_PATCH_TX:"
        payload = next(
            (line[len(marker):] for line in reversed(result.stdout.splitlines()) if line.startswith(marker)),
            None,
        )
        if payload is None:
            raise RuntimeError("Patch snapshot backend returned no transaction record")
        try:
            entries = json.loads(payload)
        except json.JSONDecodeError as exc:
            raise RuntimeError("Patch snapshot backend returned invalid transaction data") from exc
        return {"kind": "shell", "entries": entries}

    def rollback_patch_transaction(self, transaction: Dict[str, Any]) -> Optional[str]:
        """Restore a snapshot byte-for-byte; return an error instead of hiding it."""
        entries = list(transaction.get("entries") or [])
        snippet = (
            "import os, pathlib\n"
            f"entries = {entries!r}\n"
            "errors = []\n"
            ...
            "                raise FileNotFoundError('missing rollback backup for ' + str(path))\n"
        ...
        result = self._exec(f"{interpreter} -c {self._escape_shell_arg(snippet)}")
        if result.exit_code != 0:
            return (result.stdout or "").strip() or "unknown rollback error"
        if self._lsp_local_only():
            try:
                ...
                svc = get_service()
                if svc is not None:
                    svc.reconcile_restored_files_sync(
                        {
                            str(entry["path"]): bool(entry["existed"])
                            for entry in entries
                        }
                    )
            except Exception:  # noqa: BLE001
                # LSP is an enrichment layer. A successful byte-exact disk
                # rollback must remain successful even when an LSP server is
                # unavailable or shutting down.
                pass
        return None

    def commit_patch_transaction(self, transaction: Dict[str, Any]) -> Optional[str]:
        """Discard successful-transaction backups; return any cleanup warning."""
        entries = list(transaction.get("entries") or [])
        snippet = (
            "import os\n"
            f"entries = {entries!r}\n"
            ...
            "    if backup and os.path.lexists(backup):\n"
            "        try: os.unlink(backup)\n"
            "        except Exception as exc: errors.append(backup + ': ' + str(exc))\n"
            "if errors: raise RuntimeError('; '.join(errors))\n"
        )
        interpreter = "python3" if self._has_command("python3") else "python"
        result = self._exec(f"{interpreter} -c {self._escape_shell_arg(snippet)}")
        if result.exit_code != 0:
            return (result.stdout or "").strip() or "unknown cleanup error"
        return None
    ...
    def _add_line_numbers(self, content: str, start_line: int = 1) -> str:
        ...
        Documents that already contain a dominant, consecutive ``0001 |``
        gutter keep that original coordinate system.  Adding Hairball's
        physical-file gutter on top would produce ambiguous rows such as
        ``9|0001 |...`` and make source citations refer to the wrong line
        space.  Long-line truncation still applies in either display mode.
        """
        from tools.tool_output_limits import get_max_line_length
        max_line_length = get_max_line_length()
        lines = content.split('\n')
        numbered = []
        preserve_embedded_gutter = _has_dominant_embedded_line_gutter(content)
        for i, line in enumerate(lines, start=start_line):
            # Truncate long lines
            if len(line) > max_line_length:
                line = line[:max_line_length] + "... [truncated]"
            numbered.append(line if preserve_embedded_gutter else f"{i}|{line}")
        return '\n'.join(numbered)
    ...
    def _atomic_write(self, path: str, content: str) -> "ExecuteResult":
        ...
        ``.hairball-tmp`` file next to the user's data, and the original file
        ...
        tmpl = self._escape_shell_arg(".hairball-tmp.XXXXXX")
        ...
        script = (
            ...
            '|| mktemp "$d/.hairball-tmp.$$.XXXXXX" 2>/dev/null '
            '|| { tmp="$d/.hairball-tmp.$$"; : > "$tmp" && echo "$tmp"; })"; '
    ...
    def write_file(self, path: str, content: str) -> WriteResult:
        ...
        # Create parent directories BEFORE LSP snapshot.  snapshot_baseline may
        # spawn a language server with cwd=workspace root; if the target
        # path's parents do not exist yet, spawn races and falsely marks
        # the server broken.  OMP avoids this by assuming session.cwd
        # already exists — we mkdir first instead.
        ...
        # clean the temp up on any failure so we never leak a ``.hairball-tmp``
        ...
        # Optional LSP format-on-write (config default false). Runs after
        # the first atomic write so the server can didOpen/didChange from
        # disk; if formatting changes the buffer, rewrite once. Failures
        # keep the original content — enrichment must never break a write.
        final_content = self._maybe_lsp_format(path, content)
        if final_content != content:
            rewrite = self._atomic_write(path, final_content)
            if rewrite.exit_code != 0:
                # Keep the first successful write; ignore format rewrite failure.
        ...
        if lint_result.success or lint_result.skipped:
            # OMP-2: diagnostics attach must never fail the write.
            try:
    ...
    def _lsp_config_section(self) -> dict:
        """Best-effort ``lsp`` config dict (never raises)."""
        try:
    ...
    def _format_on_write_enabled(self) -> bool:
        """Read ``lsp.format_on_write`` (default false). Never raises."""
        return bool(self._lsp_config_section().get("format_on_write", False))

    def _lsp_writethrough_enabled(self) -> bool:
        """Read ``lsp.writethrough`` (default true). Never raises."""
        return bool(self._lsp_config_section().get("writethrough", True))

    def _lsp_inline_diagnostics_timeout_s(self) -> float:
        """Inline wait for post-write diagnostics (OMP 500ms default).

        Never raises; clamps to a small positive window so writes cannot
        block for the long ``wait_timeout`` used by interactive ``lsp`` tool.
        """
        raw = self._lsp_config_section().get("inline_diagnostics_timeout_ms", 500)
        try:
            ms = float(raw)
        except (TypeError, ValueError):
            ms = 500.0
        # Floor 50ms / ceiling 5s — keep writethrough snappy.
        ms = max(50.0, min(ms, 5000.0))
        return ms / 1000.0

    def _biome_enabled(self) -> bool:
        """Read ``lsp.biome`` (default true). Never raises."""
        try:
            from agent.lsp.biome_client import biome_enabled

            return biome_enabled()
        except Exception:  # noqa: BLE001
            return True

    def _maybe_biome_format(self, path: str, content: str) -> Optional[str]:
        """Try Biome CLI format; return new text or None if unused/failed."""
        if not content or not self._biome_enabled():
            return None
        try:
            from agent.lsp.biome_client import find_biome, format_file, supports_path
        except Exception:  # noqa: BLE001
            return None
        if not supports_path(path) or not find_biome():
            return None
        try:
            formatted = format_file(path, content)
        except Exception:  # noqa: BLE001
            return None
        if not isinstance(formatted, str) or formatted == content:
            return None
        return formatted

    def _maybe_biome_diagnostics(self, path: str) -> str:
        """Best-effort Biome CLI lint block (OMP-style, not LSP ServerDef)."""
        if not self._biome_enabled():
            return ""
        try:
            from agent.lsp.biome_client import find_biome, lint_file, supports_path
        ...
        if not diagnostics:
            return ""
        try:
            # Biome's common findings (e.g. noUnusedVariables) are warnings.
            # Include ERROR+WARN so post-write feedback is visible; INFO/HINT
            # stay filtered like the LSP path.
            block = report_for_file(path, diagnostics, severities=frozenset({1, 2}))
            if not block:
                return ""
            return truncate("Biome diagnostics for this file:\n" + block)
        except Exception:  # noqa: BLE001
            return ""

    def _maybe_lsp_format(self, path: str, content: str) -> str:
        """Best-effort format-on-write; return formatted text or ``content``.

        Order: Biome CLI (JS/TS/JSON/CSS when ``biome`` is on PATH) then
        language-server ``textDocument/formatting``. Gated by
        ``lsp.format_on_write`` (default false) and local backends only.
        Never raises; failures return the original ``content``.
        """
        if not content or not self._lsp_local_only():
            return content
        if not self._format_on_write_enabled():
            return content
        biome_fmt = self._maybe_biome_format(path, content)
        if biome_fmt is not None:
            return biome_fmt
        try:
            from agent.lsp import get_service
            svc = get_service()
        ...
        if svc is None or not svc.enabled_for(path):
            return content
        try:
            formatted = svc.format_document_sync(path, content)
        except Exception:  # noqa: BLE001
            return content
        if not isinstance(formatted, str) or formatted == content:
            return content
    ...
    def _maybe_lsp_diagnostics(
        ...
        *,
        ...
        """Best-effort semantic diagnostics for ``path``.

        Combines language-server publishDiagnostics (delta vs baseline)
        with optional Biome CLI lint for JS/TS/JSON/CSS. Returns a
        formatted block, or empty string when nothing is available.
        ...
        Wait budget is ``lsp.inline_diagnostics_timeout_ms`` (default
        500, Oh My Pi writethrough). Timeout → empty diagnostics; the
        write itself already succeeded and must not fail.
        """
        if not self._lsp_local_only():
            return ""
        if not self._lsp_writethrough_enabled():
            return ""

        parts: list[str] = []
        inline_timeout = self._lsp_inline_diagnostics_timeout_s()

        # When Biome CLI is available for this path, it owns the post-write
        # diagnostics field (OMP-style CLI rail). Skip language-server
        # publishDiagnostics here to avoid dual-source forks; the model
        # ``lsp`` tool still reaches the language server for navigation.
        biome_primary = False
        try:
            from agent.lsp.biome_client import find_biome, supports_path

            biome_primary = bool(
                self._biome_enabled() and supports_path(path) and find_biome()
            )
        except Exception:  # noqa: BLE001
            biome_primary = False

        if biome_primary:
            biome_block = self._maybe_biome_diagnostics(path)
            if biome_block:
                parts.append(biome_block)
            if not parts:
                return ""
            try:
                from agent.lsp.reporter import truncate
        ...
        if svc is not None and svc.enabled_for(path):
            line_shift = None
            if (
    ...
    def search(self, pattern: str, path: str = ".", target: str = "content",
        ...
        Args:
            ...
            context: Symmetric lines of context (rg -C)
            context_before: Lines before each match (rg -B)
            context_after: Lines after each match (rg -A)
            case_insensitive: Match ignoring case (rg/grep -i)
            fixed_string: Treat pattern as literal text (rg/grep -F)
            word: Match whole words only (rg/grep -w)
            multiline: Allow matches across lines (rg -U)
            max_count: Cap matches per file (rg/grep --max-count); 0 = unlimited
        
        Returns:
            SearchResult with matches or file list
        """
        offset, limit = normalize_search_pagination(offset, limit)
        max_count = max(0, _coerce_int(max_count, 0))
    ...
    def _search_files_rg(self, pattern: str, path: str, limit: int, offset: int) -> SearchResult:
        ...
        # Fetch one sentinel beyond the requested window.  Equality with the
        # old ``limit + offset`` cap could not distinguish "exactly exhausted"
        # from "more results exist", so callers never received a reliable
        # continuation signal.
        ...
        page = all_files[offset:page_end]
        ...
        return SearchResult(
            ...
            offset=offset,
            limit=limit,
            truncated=len(all_files) > page_end or bool(limit_reason),
            limit_reason=limit_reason,
        )
    ...
    def _search_content(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        return _maybe_warn_line_oriented_newline_pattern(
            result, pattern, multiline=multiline,
        )

    @staticmethod
    def _append_content_match_flags(
        cmd_parts: list,
        *,
        ...
        """Append shared rg/grep match-mode flags (order-stable for tests)."""
        if case_insensitive:
            cmd_parts.append("-i")
        if fixed_string:
            cmd_parts.append("-F")
        if word:
            cmd_parts.append("-w")
        # Multiline is ripgrep-only (-U); grep callers leave multiline=False.
        if multiline:
            cmd_parts.append("-U")
    ...
    def _search_with_rg(self, pattern: str, path: str, file_glob: Optional[str],
        ...
        before, after, asymmetric = _resolve_content_context_window(
            context, context_before, context_after,
        )
        # Offset pagination requires a stable order across separate tool
        # calls.  Ripgrep's default parallel traversal order is intentionally
        # unspecified and can change between identical invocations, causing
        # duplicates and omissions across pages.  ``--sort path`` makes the
        # ordering deterministic without reducing the model's search surface.
        cmd_parts = [
            "rg", "--sort", "path",
            "--line-number", "--no-heading", "--with-filename",
        ]
        self._append_content_match_flags(
            cmd_parts,
            case_insensitive=case_insensitive,
            fixed_string=fixed_string,
            word=word,
        ...
        _append_rg_grep_context_flags(
            cmd_parts, before=before, after=after, asymmetric=asymmetric,
        )
        if max_count > 0:
        ...
        # Fetch one hit beyond the requested window as a continuation sentinel.
        # Context mode pages by hits, so budget enough neighbor rows for that
        # sentinel too.  This stays bounded; we do not collect the whole search.
        page_end = limit + offset
        sentinel_hit_count = page_end + 1
        if before > 0 or after > 0:
            fetch_limit = sentinel_hit_count * (1 + before + after) + 200
        else:
            fetch_limit = sentinel_hit_count
        ...
        if output_mode == "files_only":
            ...
            page_counts = dict(count_items[offset:page_end])
            return SearchResult(
                counts=page_counts,
                total_count=sum(page_counts.values()),
                offset=offset,
                limit=limit,
                truncated=len(count_items) > page_end or bool(limit_reason),
                limit_reason=limit_reason,
            )
            ...
            page, _buf_hits, _buf_rows, page_trunc = _page_content_matches(
                matches, offset, limit, before=before, after=after,
            )
            page_hits = sum(1 for m in page if not m.is_context)
            return SearchResult(
                matches=page,
                total_count=page_hits,
                total_lines=len(page),
                offset=offset,
                limit=limit,
                truncated=page_trunc or bool(limit_reason),
                limit_reason=limit_reason,
            )
    ...
    def _search_with_grep(self, pattern: str, path: str, file_glob: Optional[str],
                          limit: int, offset: int, output_mode: str, context: int,
                          case_insensitive: bool = False, fixed_string: bool = False,
                          word: bool = False, max_count: int = 0,
                          context_before: int = 0, context_after: int = 0) -> SearchResult:
        ...
        if max_count > 0:
            cmd_parts.extend(["-m", str(max_count)])
        ...
        # Grep does not provide a stable traversal order.  For plain match
        # rows, sort before taking the bounded page window.  Context output
        # must retain grep's match/neighbor adjacency, so it keeps grep's
        # native order rather than corrupting context groups merely to sort.
        # Ripgrep (the primary path) supports stable sorting with context.
        page_end = limit + offset
        sentinel_hit_count = page_end + 1
        # Fetch generously enough to retain one real-hit sentinel after
        # context expansion.
        if before > 0 or after > 0:
            fetch_limit = sentinel_hit_count * (1 + before + after) + 200
        else:
