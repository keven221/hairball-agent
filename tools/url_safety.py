...
def normalize_url_for_request(url: str) -> str:
    """Return an ASCII-safe HTTP URL for Hairball-owned URL tools.
...
def _global_allow_private_urls() -> bool:
    ...
    1. ``HAIRBALL_ALLOW_PRIVATE_URLS`` env var  (``true``/``1``/``yes``)
    ...
    env_val = os.getenv("HAIRBALL_ALLOW_PRIVATE_URLS", "").strip().lower()
...
def is_safe_url(url: str) -> bool:
    ...
    ``HAIRBALL_ALLOW_PRIVATE_URLS=true``), private-IP blocking is skipped.
...
def ssrf_safe_async_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("hairball_ssrf_async_origin_schemes")
...
def ssrf_safe_http_transport(**kwargs: Any) -> Any:
    ...
    schemes_by_origin_var = contextvars.ContextVar("hairball_ssrf_origin_schemes")
...
def _install_ssrf_guard_on_async_transport(transport: Any, schemes_by_origin_var: Any) -> None:
    ...
    if transport is None or state.get("_hairball_ssrf_guarded", False):
        return
    ...
    transport._hairball_ssrf_guarded = True
