...
_PATTERNS: List[Tuple[str, str, str]] = [
    ...
    # Tool handlers often return JSON strings, so the quote after ``style=``
    # may be escaped once or multiple times before this scanner sees it.
    # Backslashes are serialization syntax, not a trust boundary; accept the
    # run immediately before the quote while keeping the precise hidden-div
    # structure and ``display:none`` signal.
    ...
    (r'unset\s+\w*(?:CLAUDE|CODEX|HAIRBALL|AGENT|OPENAI|ANTHROPIC)\w*', "env_var_unset_agent", "context"),
    ...
    (r'\$HOME/\.hairball/\.env|\~/\.hairball/\.env', "hairball_env", "strict"),
    ...
    (r'(update|modify|edit|write|change|append|add\s+to)\s+[^\n]{0,2048}\.hairball/(config\.yaml|SOUL\.md)', "hairball_config_mod", "strict"),
...
def scan_for_threats(content: str, scope: str = "context") -> List[str]:
    ...
    # bypassing keyword checks (e.g. ``ｃａｔ ~/.hairball/.env``).  NOTE: this
