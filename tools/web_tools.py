...
Backend is selected during ``hairball tools`` setup (web.backend in config.yaml).
When available, Hairball can route Firecrawl calls through a managed-hosted tool-gateway
for managed Subscribers only.
...
- Firecrawl: https://docs.firecrawl.dev/introduction (search, extract; direct or derived firecrawl-gateway.<domain> for managed Subscribers)
...
def _env_value(name: str) -> str:
    """Resolve ``name`` via Hairball config-aware env, falling back to process env.

    Mirrors the SearXNG provider's ``_searxng_url()`` so that values set
    through Hairball' config/.env layer (``hairball config set``, ``hairball tools``)
...
def _load_web_config() -> dict:
    """Load the ``web:`` section from ~/.hairball/config.yaml."""
    try:
...
def _get_backend() -> str:
    ...
    Reads ``web.backend`` from config.yaml (set by ``hairball tools``).
    ...
    # pre-empted by a managed OAuth token whose subscription tier may not
...
def _is_backend_available(backend: str) -> bool:
    ...
    if backend == "xai":
        ...
        # runs on every web_search dispatch + every `hairball tools` repaint.
        try:
...
def _web_requires_env() -> list[str]:
    ...
    set.  Gating them on ``managed_portal_tools_enabled()`` only saved
    string noise in the metadata list, but cost a synchronous HTTP
    refresh against the Managed Portal on every CLI startup (invoked at
...
def _store_full_text(url: str, content: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_hairball_dir("cache/web", "web_cache")
...
def _search_fallback_backends(primary: str) -> list:
    """Backends to try when the configured search backend fails at runtime.

    Read from ``web.search_fallbacks`` in config.yaml. This is runtime
    *failure* recovery, distinct from the config-resolution fallbacks in
    :func:`_get_backend` — a quota-exhausted or rate-limited vendor is
    perfectly "available" by every cheap probe, so selection alone cannot
    save the call. An empty list disables the behavior entirely.
    """
    raw = _load_web_config().get("search_fallbacks")
    if isinstance(raw, str):
        candidates = [p.strip() for p in raw.split(",")]
    elif isinstance(raw, (list, tuple)):
        candidates = [str(p or "").strip() for p in raw]
    else:
        return []
    out = []
...
def _search_succeeded(payload) -> bool:
    return isinstance(payload, dict) and bool(payload.get("success"))


def _run_search_provider(provider, query: str, limit: int) -> dict:
    """Call one provider's ``search()``, normalizing a raise into the
    documented ``{"success": False, "error": str}`` failure shape so the
    fallback walk treats exceptions and error payloads identically."""
    try:
        payload = provider.search(query, limit)
    except Exception as exc:  # noqa: BLE001 — any provider failure is a fallback trigger
        return {"success": False, "error": f"{provider.name}: {exc}"}
    if not isinstance(payload, dict):
        return {
            "success": False,
            "error": f"{provider.name} returned {type(payload).__name__}, expected dict",
        }
    return payload


def _search_with_fallbacks(provider, query: str, limit: int) -> dict:
    """Search via *provider*, walking ``web.search_fallbacks`` on failure.

    Failure modes of the primary backend are preserved exactly when no
    fallback serves the call: a raise stays a raise (callers rely on the
    ``Error searching web: ...`` envelope the outer handler builds) and an
    error payload stays that same payload. The success path returns the
    primary payload untouched. Only a call actually served by a fallback
    carries ``backend`` / ``fallback_from`` — a downgrade the user did not
    ask for must be visible in the result, not silent.
    """
    logger.info("Web search via %s: '%s' (limit: %d)", provider.name, query, limit)
    fallbacks = _search_fallback_backends(provider.name)
    primary_exc = None
    try:
        payload = provider.search(query, limit)
        ...
        if not fallbacks:
            raise
        primary_exc = exc
        payload = {"success": False, "error": f"{provider.name}: {exc}"}
    else:
        if _search_succeeded(payload):
            return payload
        if not isinstance(payload, dict):
            return payload

    primary_error = str(payload.get("error") or "unknown error")
    attempted = []
    for name in fallbacks:
        ...
        alt = _registered_web_provider(name)
        if alt is None or not alt.supports_search():
            continue
        # Registry aliases (and test stubs) can resolve a different config name
        # to the provider that just failed. Retrying a quota-dead vendor under
        # another name buys nothing.
        if alt is provider or alt.name == provider.name:
            continue
        try:
            if not alt.is_available():
                continue
        except Exception as exc:  # noqa: BLE001 — a broken probe means unusable
            logger.debug("Fallback backend %r availability probe raised: %s", name, exc)
            continue

        attempted.append(alt.name)
        logger.warning(
            "Web search backend %r failed (%s); falling back to %r",
            provider.name, primary_error, alt.name,
        )
        alt_payload = _run_search_provider(alt, query, limit)
        if _search_succeeded(alt_payload):
            served = dict(alt_payload)
            served["backend"] = alt.name
            served["fallback_from"] = {"backend": provider.name, "error": primary_error}
            return served
    ...
    if attempted:
        failed = dict(payload)
        failed["fallback_tried"] = attempted
        return failed
    return payload
            ...
            if disabled_key:
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        f"Re-enable it with `hairball plugins enable {disabled_key}` "
                ...
                response_data = {
                    ...
                    "error": (
                        ...
                        "Run `hairball tools` to set one up."
                    ),
                }
        else:
            response_data = _search_with_fallbacks(provider, query, limit)
                        ...
                        return json.dumps(
                            {
                                ...
                                "error": (
                                    ...
                                    f"`hairball plugins enable {disabled_key}` "
                                    "(or remove it from plugins.disabled)."
                                ),
                            },
                            ensure_ascii=False,
...
if __name__ == "__main__":
    ...
    from hairball_cli.config import get_env_value as _gev
