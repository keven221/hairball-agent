...
_YOLO_MODE_FROZEN: bool = is_truthy_value(os.getenv("HAIRBALL_YOLO_MODE", ""))
...
# os.environ["HAIRBALL_INTERACTIVE"] races: one session's restore in `finally`
...
# single-threaded CLI callers that still export HAIRBALL_INTERACTIVE.
_hairball_interactive_ctx: contextvars.ContextVar[Optional[str]] = contextvars.ContextVar(
    "hairball_interactive",
    default=None,
)


def set_hairball_interactive_context(interactive: bool) -> contextvars.Token:
    """Bind interactive mode for the current context (thread or asyncio task).

    Use this instead of mutating ``os.environ["HAIRBALL_INTERACTIVE"]`` from
    concurrent executor threads. When unset (default), interactive detection
    falls back to the ``HAIRBALL_INTERACTIVE`` env var for legacy callers.
    """
    return _hairball_interactive_ctx.set("1" if interactive else "")


def reset_hairball_interactive_context(token: contextvars.Token) -> None:
    """Restore the prior value from :func:`set_hairball_interactive_context`."""
    _hairball_interactive_ctx.reset(token)
...
def _is_interactive_cli() -> bool:
    ...
    back to the ``HAIRBALL_INTERACTIVE`` env var for single-threaded callers.
    """
    ctx_val = _hairball_interactive_ctx.get()
    if ctx_val is not None:
        return is_truthy_value(ctx_val)
    return env_var_enabled("HAIRBALL_INTERACTIVE")
...
def get_current_tool_call_id() -> str:
    """Return the model tool-call id bound around registry dispatch."""
...
def get_current_session_key(default: str = "default") -> str:
    ...
    return get_session_env("HAIRBALL_SESSION_KEY", default)
...
def _get_session_platform() -> str:
    ...
    try:
        ...
        return os.getenv("HAIRBALL_SESSION_PLATFORM", "") or ""


def _is_gateway_approval_context() -> bool:
    """True when this call is inside a gateway/API session.

    Legacy gateway integrations set HAIRBALL_GATEWAY_SESSION in process env.
    Newer concurrent gateway paths bind HAIRBALL_SESSION_PLATFORM via
    ...
    from a gateway platform (cron binds HAIRBALL_SESSION_PLATFORM via
    ...
    """
    if env_var_enabled("HAIRBALL_CRON_SESSION"):
        return False
    if env_var_enabled("HAIRBALL_GATEWAY_SESSION"):
        return True
    return bool(_get_session_platform())

# Sensitive write targets that should trigger approval even when referenced
# via shell expansions like $HOME or $HAIRBALL_HOME, or by the resolved absolute
# active profile home path such as /home/hairball/.hairball/config.yaml. The
# resolved-absolute form is folded into the ~/.hairball/ patterns at detection
...
# go stale when HAIRBALL_HOME is set after this module is imported, e.g. under the
# hermetic test conftest or any deferred-profile-resolution path).
_SSH_SENSITIVE_PATH = r'(?:~|\$home|\$\{home\})/\.ssh(?:/|$)'
_HAIRBALL_ENV_PATH = (
    r'(?:~\/\.hairball/|'
    r'(?:\$home|\$\{home\})/\.hairball/|'
    r'(?:\$hairball_home|\$\{hairball_home\})/)'
    r'\.env\b'
)
# ~/.hairball/config.yaml IS the security policy: approvals.mode, yolo, and the
...
# theater. Mirrors _HAIRBALL_ENV_PATH; matches the HAIRBALL_HOME override form as
# well as ~/.hairball/.
_HAIRBALL_CONFIG_PATH = (
    r'(?:~\/\.hairball/|'
    r'(?:\$home|\$\{home\})/\.hairball/|'
    r'(?:\$hairball_home|\$\{hairball_home\})/)'
    r'config\.yaml\b'
...
_SENSITIVE_WRITE_TARGET = (
    ...
    rf'{_HAIRBALL_ENV_PATH}|'
    rf'{_HAIRBALL_CONFIG_PATH}|'
    rf'{_SHELL_RC_FILES}|'
...
DANGEROUS_PATTERNS = [
    ...
    # `hairball` and `gateway` (e.g. `hairball -p ade gateway restart`) so a
    # profile flag can't slip the agent past the guard.
    (r'\bhairball\s+(?:-{1,2}\S+(?:\s+\S+)?\s+)*gateway\s+(stop|restart)\b', "stop/restart hairball gateway (kills running agents)"),
    (r'\bhairball\s+update\b', "hairball update (restarts gateway, kills running agents)"),
    # Docker container lifecycle — any user with docker.sock mounted (a common
    # Docker Compose pattern) gives the agent the ability to restart/stop/kill
    # containers without approval.  These are agent-initiated lifecycle operations
    # that should always require user consent, just like `hairball gateway restart`
    ...
    (r'gateway\s+run\b.*(&\s*$|&\s*;|\bdisown\b|\bsetsid\b)', "start gateway outside systemd (use 'systemctl --user restart hairball-gateway')"),
    (r'\bnohup\b.*gateway\s+run\b', "start gateway outside systemd (use 'systemctl --user restart hairball-gateway')"),
    # Self-termination protection: prevent agent from killing its own process
    (r'\b(pkill|killall)\b.*\b(hairball|gateway|cli\.py)\b', "kill hairball/gateway process (self-termination)"),
    # Self-termination via kill + command substitution (pgrep/pidof).
    # The name-based pattern above catches `pkill hairball` but not
    # `kill -9 $(pgrep -f hairball)` because the substitution is opaque
    ...
    # the `hairball gateway stop|restart` pattern above by driving launchd
    # directly against the service label (commonly `ai.hairball.gateway`).
    # Catch the operations that stop, restart, or unload it.
    (r'\blaunchctl\s+(stop|kickstart|bootout|unload|kill|disable|remove)\b.*\b(hairball|ai\.hairball)\b', "stop/restart hairball launchd service (kills running agents)"),
    # File copy/move/edit into sensitive system paths (/etc/ and macOS
    # /private/etc/ mirror).
    (rf'\b(cp|mv|install)\b.*\s{_SYSTEM_CONFIG_PATH}', "copy/move file into system config path"),
    (rf'\b(cp|mv|install)\b.*\s["\']?{_PROJECT_SENSITIVE_WRITE_TARGET}["\']?{_COMMAND_TAIL}', "overwrite project env/config file"),
    # cp/mv/install OVERWRITING a sensitive credential/SSH/shell-rc/Hairball file.
    # The tee/redirection patterns above already gate _SENSITIVE_WRITE_TARGET
    # (~/.ssh/*, ~/.netrc/.pgpass/.npmrc/.pypirc, shell rc files,
    # ~/.hairball/config.yaml/.env), but cp/mv/install was only paired for /etc and
    ...
    # In-place edit of a Hairball-managed security file (~/.hairball/config.yaml or
    ...
    (rf'\bsed\s+-[^\s]*i.*(?:{_HAIRBALL_CONFIG_PATH}|{_HAIRBALL_ENV_PATH})', "in-place edit of Hairball config/env"),
    (rf'\bsed\s+--in-place\b.*(?:{_HAIRBALL_CONFIG_PATH}|{_HAIRBALL_ENV_PATH})', "in-place edit of Hairball config/env (long flag)"),
    ...
    (rf'\b(?:perl|ruby)\b.*(?:^|\s)-[^\s]*i\b.*(?:{_HAIRBALL_CONFIG_PATH}|{_HAIRBALL_ENV_PATH})', "in-place edit of Hairball config/env (perl/ruby)"),
...
def _normalize_command_for_detection(command: str) -> str:
    ...
    # ~/ and ~/.hairball/ forms so static user-sensitive patterns catch
    ...
    # it tracks HOME / HAIRBALL_HOME even when those are set after this module is
    ...
    #
    # Fold the (more specific) Hairball home first: on Windows it nests under the
    # user home (C:\Users\alice\AppData\...\hairball), so folding the user home
    # first would eat the prefix the Hairball-home fold needs.
    command = _rewrite_resolved_hairball_home(command)
...
def _home_prefix_fold_regex(path: str):
    ...
    components below the root — so a stray HOME / HAIRBALL_HOME such as ``/``,
...
def _fold_home_prefixes(command: str, paths, replacement: str) -> str:
    ...
    *replacement* has no trailing separator (``~`` / ``~/.hairball``); the matched
...
def _rewrite_resolved_hairball_home(command: str) -> str:
    """Rewrite the resolved absolute Hairball home prefix to ``~/.hairball/``.

    Resolves the active ``HAIRBALL_HOME`` at call time (and its symlink-resolved
    form) and folds an occurrence of ``<home>/`` in *command* into
    ``~/.hairball/`` so the static ``_HAIRBALL_CONFIG_PATH`` / ``_HAIRBALL_ENV_PATH``
    patterns match. In Docker and gateway deployments the agent often references
    the resolved absolute path directly (e.g. ``sed -i ...
    /home/hairball/.hairball/config.yaml``) rather than ``~``, ``$HOME``, or
    ``$HAIRBALL_HOME``. Matches both POSIX and Windows separators. No-op when the
    path can't be resolved or doesn't appear.
    """
    try:
        from hairball_constants import get_hairball_home
        home = get_hairball_home().expanduser()
        candidates = [
            str(home),
        ...
        return command
    return _fold_home_prefixes(command, candidates, "~/.hairball")
...
def _is_verification_artifact_cleanup(command: str) -> bool:
    """Return whether *command* only removes one Hairball ad-hoc temp script."""
    try:
    ...
    # macOS commonly reports its per-user temp directory below
    # ``/var/folders`` while ``realpath`` canonicalizes it to
    # ``/private/var/folders``.  ``/tmp`` has the same aliasing shape.  Accept
    # those two OS-owned spellings, but do not trust an arbitrary symlink
    # returned by a patched/custom tempfile provider: the canonical target
    # remains the only accepted spelling in that case.
    accepted_operands = {os.path.join(temp_dir, basename)}
    if configured_temp_dir == "/tmp" or configured_temp_dir.startswith(
        "/var/folders/"
    ):
        accepted_operands.add(os.path.join(configured_temp_dir, basename))
    ...
    if os.path.dirname(target) != temp_dir:
        return False
    return re.fullmatch(r"hairball-(?:verify|ad-hoc)-[A-Za-z0-9_.-]+", basename) is not None
...
def is_approval_bypass_active() -> bool:
    """Return True when the user has opted out of Hairball approval prompts.

    Collapses the canonical three-source bypass check used across the codebase
    into one place:
      - process-scoped ``--yolo`` / ``HAIRBALL_YOLO_MODE`` (frozen at import time
...
def _smart_approve(command: str, description: str) -> str:
    ...
    Inspired by ChatGPT's Smart Approvals guardian subagent
    (openai/codex#13860).
    """
    try:
    ...
    *,
    ...
    Args:
        ...
        fail_closed_when_no_human: When True, a non-interactive non-gateway
            ...
            HAIRBALL_INTERACTIVE unset) BLOCKS instead of auto-approving. The
    ...
    if not is_cli and not is_gateway:
        ...
        if env_var_enabled("HAIRBALL_CRON_SESSION"):
            ...
            logger.warning(
                ...
                "BLOCKED (fail-closed). Set HAIRBALL_INTERACTIVE or "
                "HAIRBALL_GATEWAY_SESSION to answer the prompt.",
        ...
        logger.warning(
            "%s (pattern: %s): %s — set HAIRBALL_INTERACTIVE or "
            "HAIRBALL_GATEWAY_SESSION to require approval.",
            autoapprove_log_prefix, pattern_key, description,
        )
        return {"approved": True, "message": None}

    if is_gateway or env_var_enabled("HAIRBALL_EXEC_ASK"):
...
def request_tool_approval(
    ...
    *,
    ...
    (a bare script with no ``HAIRBALL_INTERACTIVE``) fails CLOSED — a plugin-
...
def check_all_command_guards(command: str, env_type: str,
    ...
    is_cli = _is_interactive_cli()
    is_gateway = _is_gateway_approval_context()
    is_ask = env_var_enabled("HAIRBALL_EXEC_ASK")
