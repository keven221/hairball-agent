...
observability, AND a collab mailbox ``agent_completed`` envelope is posted
for ``collab_wait`` / ``collab_collect``.

**Single Rail (product upgrade 2026-07-21):** every ``async_delegation``
event is ``suppress_inject`` / ``delivery_channel=collab_mailbox``. CLI /
gateway / TUI / UI inject consumers **ack only** — they never forge a new
parent turn. Content re-enters via ``collab_collect`` in the parent's tool
loop (reference V2-style mailbox; Hairball keeps an explicit collect tool).
    ...
    with _DB_LOCK, _connect() as conn:
        ...
        for row in rows:
            ...
            event = {
                ...
                "batch_goals": task.get("batch_goals"),
...
def claim_event_delivery(evt: Dict[str, Any], consumer: str) -> Optional[str]:
    """Claim one durable completion across CLI, TUI, and gateway consumers."""
    if evt.get("type") == "completion":
        ...
        return background_process_store.claim_delivery(process_id, consumer)
...
def complete_event_delivery(evt: Dict[str, Any], claim_id: str) -> None:
    if claim_id and evt.get("type") == "completion":
        from tools.background_process_store import background_process_store
        background_process_store.complete_delivery(
            str(evt.get("session_id") or ""), claim_id,
        )
        return
...
def release_event_delivery(evt: Dict[str, Any], claim_id: str) -> None:
    if claim_id and evt.get("type") == "completion":
        from tools.background_process_store import background_process_store
        background_process_store.release_delivery(
            str(evt.get("session_id") or ""), claim_id,
        )
        return
    if claim_id and evt.get("type") == "async_delegation":
...
def get_durable_delegation(delegation_id: str) -> Optional[Dict[str, Any]]:
    with _DB_LOCK, _connect() as conn:
        row = conn.execute(
            """SELECT origin_session, state, dispatched_at, completed_at,
                      result_json, delivery_state, delivery_attempts, task_json
    ...
    return {
        ...
        "task": json.loads(row[7]) if row[7] else {},
    }
...
def dispatch_async_delegation(
    *,
    ...
    batch_goals: Optional[List[str]] = None,
    collab_agent_id: str = "",
    collab_nickname: str = "",
    collab_path: str = "",
) -> Dict[str, Any]:
    ...
    if batch_id:
        ...
        record["batch_count"] = int(batch_count or 0)
        record["task_index"] = task_index
        record["batch_goals"] = list(batch_goals or [])
    # Collab identity is immutable dispatch metadata.  It must be present
    # before the worker can complete; post-dispatch stamping races an
    # immediately-finishing runner and produces an orphaned mailbox envelope.
    if collab_agent_id:
        record["collab_agent_id"] = str(collab_agent_id)
        record["collab_nickname"] = str(collab_nickname or "")
        record["collab_path"] = str(collab_path or collab_agent_id)
        ...
        if running >= max_async_children:
            return {
                ...
                "error": (
                    ...
                    f"running). Wait for one to finish (collab_wait → "
                    f"collab_collect), or run this task synchronously "
                    f"(background=false). Raise delegation.max_concurrent_children in "
                    f"config.yaml to allow more concurrent background subagents."
                ),
            }
        record["delivery_channel"] = "collab_mailbox"
    ...
    try:
        ...
        # get_hairball_home() under the right profile.
...
def should_suppress_inject(evt: Optional[Dict[str, Any]]) -> bool:
    """True when async completion must not synthesize a parent inject turn.

    Single Rail: every ``type==async_delegation`` / ``async_delegation_batch``
    event is mailbox-owned. Legacy flags (``suppress_inject`` /
    ``delivery_channel``) still honored.
    """
    if not isinstance(evt, dict):
        return False
    evt_type = str(evt.get("type") or "")
    if evt_type in ("async_delegation", "async_delegation_batch"):
        return True
    if evt.get("suppress_inject"):
        return True
    return str(evt.get("delivery_channel") or "") == "collab_mailbox"


def acknowledge_suppressed_inject(evt: Dict[str, Any], consumer: str = "inject-skip") -> None:
    """Mark a mailbox-owned completion delivered so inject consumers never retry."""
    if not should_suppress_inject(evt):
        return
    claim = claim_event_delivery(evt, consumer)
    if claim:
        complete_event_delivery(evt, claim)
    else:
        # Already claimed/delivered (e.g. collab_mailbox at finalize) — ok.
        did = str(evt.get("delegation_id") or "")
        if did:
            mark_completion_delivered(did)


def _seal_mailbox_delivery(
    evt: Dict[str, Any],
    *,
    summary: Any = None,
    error: Any = None,
) -> None:
    """Stamp suppress + durable delivered + post agent_completed doorbell."""
    evt["delivery_channel"] = "collab_mailbox"
    evt["suppress_inject"] = True
    did = str(evt.get("delegation_id") or "")
    if did:
        claim_id = f"collab_mailbox:{os.getpid()}:{uuid.uuid4().hex}"
        if claim_completion_delivery(did, claim_id):
            complete_completion_delivery(did, claim_id)
        else:
            mark_completion_delivered(did)
    parent_key = str(
        evt.get("session_key") or evt.get("parent_session_id") or ""
    ).strip()
    if not parent_key:
        return
    agent_id = str(
        evt.get("collab_agent_id") or evt.get("delegation_id") or ""
    ).strip()
    try:
        ...
        logger.debug(
            "async_delegation: mailbox post failed for %s", did, exc_info=True
        )
...
def _push_completion_event(
    ...
    Single Rail: always ``suppress_inject`` + durable claim + mailbox doorbell.
    Content re-enters via ``collab_collect`` only.
    """
    try:
        ...
        evt.update(
            {
                ...
                "batch_goals": record.get("batch_goals"),
            }
        )
    # Optional collab_v2 projection fields (read-only UI / list).
    for _ck in ("collab_agent_id", "collab_nickname", "collab_path"):
        if record.get(_ck):
            evt[_ck] = record[_ck]
    _persist_completion(evt, result)
    _seal_mailbox_delivery(evt, summary=summary, error=error)
    try:
...
def dispatch_async_delegation_batch(
    *,
    ...
    When the batch finishes, a SINGLE completion event is pushed (observability)
    and one mailbox ``agent_completed`` envelope is posted. Summaries are
    collected via ``collab_collect`` — no inject parent turn.

    ``delegation_id`` may be supplied by the caller so a pre-created live
    transcript directory (``cache/delegation/live/<id>/``) shares the same
    handle as the returned async unit.
        ...
        if running >= max_async_children:
            return {
                ...
                "error": (
                    f"Async delegation capacity reached ({max_async_children} "
                    f"running). Wait for one to finish (collab_wait → "
                    f"collab_collect), or raise "
                    f"delegation.max_concurrent_children in config.yaml to "
                    f"allow more concurrent background units."
                ),
            }
        _records[delegation_id] = record
...
def _finalize_batch(
    ...
    evt = {
        ...
        # The full per-task results list — collect / observability.
    ...
    for _ck in ("collab_agent_id", "collab_nickname", "collab_path"):
        if event_record.get(_ck):
            evt[_ck] = event_record[_ck]
    _persist_completion(evt, combined)
    _seal_mailbox_delivery(
        evt, summary=batch_summary, error=combined.get("error")
    )
    try:
        process_registry.completion_queue.put(evt)
    except Exception as exc:  # pragma: no cover
