...
# Must match hairball_cli.runtime_provider.RUNTIME_PROVIDER_TYPE_CUSTOM.
_RUNTIME_PROVIDER_CUSTOM = "custom"
from tools import file_state
from tools.delegation_profiles import (
    apply_profile_tool_denies,
    build_profile_param_description,
    profile_schema_enum,
    resolve_delegation_profile,
)
...
def _expand_parent_toolsets(parent_toolsets: set) -> set:
    ...
    When a parent uses a composite toolset like ``hairball-cli`` (which bundles
...
def _build_child_system_prompt(
    ...
    *,
    ...
    child_depth: int = 1,
    profile_addendum: Optional[str] = None,
    plan_reference: Optional[tuple[str, str]] = None,
) -> str:
    """Build a focused system prompt for a child agent.

    When role='orchestrator', appends a delegation-capability block
    modeled on legacy's buildSubagentSystemPrompt (canSpawn branch at
    inspiration/openclaw/src/agents/subagent-system-prompt.ts:63-95).
    The depth note is literal truth (grounded in the passed config) so
    the LLM doesn't confabulate nesting capabilities that don't exist.

    ``profile_addendum`` (from a resolved DelegationProfile) is appended
    after the base / orchestrator blocks so profile rules stay last.
    """
    parts = [
    ...
    if plan_reference is not None:
        plan_path, plan_content = plan_reference
        parts.append(
            "\nAPPROVED PLAN:\n"
            "This session is executing an approved plan. Your assigned task "
            "is one part of it. Use the full plan below to keep your work "
            "consistent with decisions already made. If the assigned task and "
            "the overall plan conflict, the assigned task wins. The plan is a "
            "read-only context snapshot; do not re-read or edit its path.\n\n"
            f'<plan path="{escape(str(plan_path), quote=True)}">\n'
            f"{plan_content}\n"
            "</plan>"
        )
    ...
    if profile_addendum and str(profile_addendum).strip():
        parts.append("\n" + str(profile_addendum).strip())
    return "\n".join(parts)


def _load_approved_plan_reference(parent_agent: Any) -> Optional[tuple[str, str]]:
    """Load only a turn-scoped approved Build plan for child context."""

    if str(getattr(parent_agent, "agent_mode", "") or "").casefold() != "build":
        return None
    if not str(getattr(parent_agent, "_plan_execution_id", "") or "").strip():
        return None
    plan_path = str(getattr(parent_agent, "_plan_file_path", "") or "").strip()
    if not plan_path:
        return None
    try:
        content = Path(plan_path).read_text(encoding="utf-8")
    except (OSError, UnicodeError):
        logger.warning("Approved plan could not be loaded for subagent handoff: %s", plan_path)
        return None
...
def _build_child_progress_callback(
    ...
    *,
    ...
    delegation_id_ref: Optional[Dict[str, Any]] = None,
) -> Optional[callable]:
    ...
    # Stack of (tool_name, args, tool_call_id) so tool.completed can attach the
    # matching input (executor passes args=None on completed). FIFO for
    # parallel same-named tools.
    _tool_arg_stack: List[tuple] = []
    ...
    def _identity_kwargs() -> Dict[str, Any]:
        ...
        # Filled after async dispatch returns; sync path leaves it empty.
        if delegation_id_ref:
            deleg = str(delegation_id_ref.get("delegation_id") or "").removeprefix("bg:").strip()
            if deleg:
                kw["delegation_id"] = deleg
            # Optional collab_v2 projection (absent on plain delegate_task).
            for _ck in ("collab_agent_id", "collab_nickname", "collab_path"):
                _cv = delegation_id_ref.get(_ck)
                if _cv:
                    kw[_ck] = _cv
    ...
    def _callback(
        ...
        if event_type == "subagent.progress":
            # Lifecycle progress (batch tool rollups, max-iter "Requesting
            # summary…"). Not a DelegateEvent — must bypass enum normalisation
            # or the line is silently dropped and UI looks finished at the
            # tool cap while the child is still summarising.
            _relay("subagent.progress", preview=preview or tool_name or "", **kwargs)
            return
        ...
        if event == DelegateEvent.TASK_TOOL_COMPLETED:
            # No CLI spinner noise (parity with prior noop display), but relay
            # result + args so UIs can show the same Input/Output cards as
            # parent tools / deferred-completion tool_call_completed.
            matched_args = None
            matched_call_id = str(kwargs.get("tool_call_id") or kwargs.get("call_id") or "")
            name_key = str(tool_name or "")
            # Prefer exact tool_call_id; else oldest open same-name call (FIFO).
            for i, item in enumerate(_tool_arg_stack):
                stacked_name = item[0] if item else ""
                stacked_args = item[1] if len(item) > 1 else None
                stacked_id = item[2] if len(item) > 2 else ""
                if stacked_name != name_key:
                    continue
                if matched_call_id and stacked_id and stacked_id != matched_call_id:
                    continue
                matched_args = stacked_args
                if not matched_call_id:
                    matched_call_id = str(stacked_id or "")
                del _tool_arg_stack[i]
                break
            raw_result = kwargs.get("result")
            result_text = "" if raw_result is None else str(raw_result)
            if len(result_text) > 8000:
                result_text = result_text[:8000] + "…"
            preview_text = result_text.replace("\n", " ").strip()
            ...
            _relay(
                ...
                matched_args if matched_args is not None else args,
                result=result_text,
                is_error=bool(kwargs.get("is_error")),
                duration=kwargs.get("duration"),
                phase="completed",
                tool_call_id=matched_call_id or None,
        ...
        _tool_count[0] += 1
        # Stable per-call id so UIs can merge result onto the matching start
        # when several same-named tools run back-to-back / in parallel.
        call_seq = _tool_count[0]
        tool_call_id = str(
            kwargs.get("tool_call_id")
            or kwargs.get("call_id")
            or f"{tool_name or 'tool'}:{call_seq}"
        )
        if subagent_id is not None:
            with _active_subagents_lock:
        ...
        if parent_cb:
            safe_args = args if isinstance(args, dict) else None
            _tool_arg_stack.append((str(tool_name or ""), safe_args, tool_call_id))
            if len(_tool_arg_stack) > 64:
                del _tool_arg_stack[: len(_tool_arg_stack) - 64]
            _relay(
                "subagent.tool",
                tool_name,
                preview,
...
def _build_child_agent(
    ...
    role: str = "leaf",
    # Optional DelegationProfile (already resolved). None = inherit parent.
    profile=None,
):
    """
    ...
    model on OpenRouter while the parent runs on Managed Portal).
    """
    ...
    # Profile toolsets narrow the child when the caller did not pass an
    # explicit toolsets list (model path always passes None today).
    requested_toolsets = toolsets
    if (not requested_toolsets) and profile is not None and getattr(
        profile, "toolsets", None
    ):
        requested_toolsets = list(profile.toolsets)

    profile_inherit_mcp = True
    if profile is not None and hasattr(profile, "inherit_mcp"):
        profile_inherit_mcp = bool(profile.inherit_mcp)

    if requested_toolsets:
        # Intersect with parent — subagent must not gain tools the parent lacks.
        # Expand composite toolsets (e.g. hairball-cli) so that individual
        # toolset names (e.g. web, terminal) are recognised during intersection.
        expanded_parent = _expand_parent_toolsets(parent_toolsets)
        child_toolsets = [t for t in requested_toolsets if t in expanded_parent]
        if _get_inherit_mcp_toolsets() and profile_inherit_mcp:
    ...
    profile_addendum = None
    if profile is not None:
        profile_addendum = getattr(profile, "system_prompt_addendum", None) or None
    child_prompt = _build_child_system_prompt(
        ...
        plan_reference=_load_approved_plan_reference(parent_agent),
    )
    # Extract parent's API key so subagents inherit auth (e.g. Managed Portal).
    ...
    # Shared with the parent delegate_task call so async dispatch can stamp
    # delegation_id onto every later child progress event.
    _deleg_ref = getattr(parent_agent, "_delegate_batch_id_ref", None)
    child_progress_cb = _build_child_progress_callback(
        ...
        delegation_id_ref=_deleg_ref if isinstance(_deleg_ref, dict) else None,
    )
    ...
    # Profile metadata + hard tool denies (e.g. explore strips write_file).
    child._delegate_profile = getattr(profile, "name", None) if profile else None
    if profile is not None and getattr(profile, "deny_tools", None):
        apply_profile_tool_denies(child, profile.deny_tools)
...
def _dump_subagent_timeout_diagnostic(
    *,
    ...
    writes a dedicated log under ``~/.hairball/logs/subagent-<sid>-<ts>.log``
...
def _spill_summary_to_file(task_index: int, summary: str) -> Optional[str]:
    ...
    try:
        ...
        cache_dir = get_hairball_dir("cache/delegation", "delegation_cache")
...
def _run_single_child(
    ...
    **_kwargs,
    ...
    ``conversation_history`` is optional (collab followup continuity). When
    None, behavior matches the historical delegate_task path exactly.
    """
    ...
    def _emit_native_lifecycle(status: str, *, summary: str = "", duration: float = 0.0,
                               api_calls: int = 0, exit_reason: str = "") -> None:
        """Record actual child lifecycle facts without depending on UI progress."""
        recorder = getattr(parent_agent, "_native_turn_events", None)
        if recorder is None or not _subagent_id:
            return
        try:
            if status == "spawned":
                recorder.subagent_spawned(
                    _subagent_id,
                    objective=goal,
            ...
            logger.debug("native subagent lifecycle event failed", exc_info=True)

    try:
        _heartbeat_thread.start()
        _emit_native_lifecycle("spawned")
        if child_progress_cb:
            try:
        ...
        def _run_with_thread_capture():
            ...
            # Optional resume history for collab followup; omit when None so
            # delegate_task keeps the identical call shape.
        ...
        io_by_id: Dict[str, Dict[str, Any]] = {}
                ...
                if msg.get("role") == "assistant":
                    for tc in msg.get("tool_calls") or []:
                        ...
                        try:
                            ...
                            parsed_args = {"raw": str(raw_args or "")[:8000]}
                        entry_t = {
                        ...
                        entry_io = {
                            "tool": fn.get("name", "unknown"),
                            "args": parsed_args if isinstance(parsed_args, dict) else {"raw": parsed_args},
                            "tool_call_id": tc.get("id") or "",
                        }
                        tool_trace.append(entry_t)
                        tool_io.append(entry_io)
                        tc_id = tc.get("id")
                        if tc_id:
                            trace_by_id[tc_id] = entry_t
                            io_by_id[tc_id] = entry_io
        ...
        entry: Dict[str, Any] = {
            ...
            # parent thread can fire subagent_complete with the correct role.
            ...
            "_child_actual_cost_usd": (
                ...
                if isinstance(
                    getattr(child, "session_actual_cost_usd", None),
                    (int, float),
                )
                else None
            ),
            "_child_cost_actual_call_count": (
                int(getattr(child, "session_cost_actual_call_count", 0) or 0)
                if isinstance(
                    getattr(child, "session_cost_actual_call_count", 0), int
                )
                else 0
            ),
            "_child_cost_total_call_count": (
                int(getattr(child, "session_cost_total_call_count", 0) or 0)
                if isinstance(
                    getattr(child, "session_cost_total_call_count", 0), int
                )
                else 0
            ),
        }
        # Collab followup continuity (and any caller that wants resume) —
        # not serialised into the parent model tool result for batch paths.
        if isinstance(messages, list):
            entry["messages"] = messages
        # A child may have an opt-in completion contract of its own.  Preserve
        # its compact terminal assessment for the parent to inspect, but do
        # not reinterpret it as the delegation status or as proof for the
        # parent's separate contract.
        child_completion_assessment = result.get("completion_assessment")
        if isinstance(child_completion_assessment, dict):
            entry["completion_assessment"] = dict(child_completion_assessment)
        _child_sid = getattr(child, "session_id", None)
        if _child_sid:
            entry["child_session_id"] = str(_child_sid)
        if status == "failed":
...
def delegate_task(
    ...
    The 'profile' parameter selects a built-in agent profile (e.g. 'explore'
    for read-only search). Omit / 'default' / 'worker' keeps inherit-parent
    toolsets. Per-task profile beats the top-level one.
    ...
    # Resolve top-level profile early so a typo fails before any child build.
    try:
        resolve_delegation_profile(profile)
    except ValueError as exc:
        return tool_error(str(exc))

    # Background (async) delegation now applies to BOTH single tasks and
    # batches. A batch becomes N independent async dispatches on the daemon
    # executor. Single Rail: results land in the collab mailbox (no inject
    # parent turn); parent uses collab_wait → collab_collect. There's no
    ...
    # UI-only refs: legacy owns execution as ONE async batch unit, but Hairball
    # needs stable per-child display keys so live tool streams attach to the
    # correct Agents/Workspace row instead of a transient sa-* placeholder.
    _child_delegation_refs: List[Dict[str, Any]] = []
    _prev_deleg_ref = getattr(parent_agent, "_delegate_batch_id_ref", None)
    children = []
    try:
        for i, t in enumerate(task_list):
            _child_delegation_ref: Dict[str, Any] = {}
            _child_delegation_refs.append(_child_delegation_ref)
            try:
                parent_agent._delegate_batch_id_ref = _child_delegation_ref
            except Exception:
                pass
            # Per-task role beats top-level; normalise again so unknown
            # per-task values warn and degrade to leaf uniformly.
            effective_role = _normalize_role(t.get("role") or top_role)
            # Per-task profile beats top-level; resolve here so one bad
            # task name fails the whole call before any sibling is built.
            task_profile_name = t["profile"] if "profile" in t else profile
            try:
                effective_profile = resolve_delegation_profile(task_profile_name)
            except ValueError as exc:
                return tool_error(str(exc))
            child = _build_child_agent(
                ...
                # Model path has no toolsets arg; profiles may narrow via
                # DelegationProfile.toolsets inside _build_child_agent.
                toolsets=None,
                model=creds["model"],
        ...
        try:
            parent_agent._delegate_batch_id_ref = _prev_deleg_ref
        except Exception:
            pass

    def _execute_and_aggregate() -> dict:
        """Run all built children (1 or N), join on them, aggregate results,
        fire subagent_complete hooks + cost rollup, and return the combined result
        ...
        # Fire subagent_complete observation hooks once per child, serialised on
        # the parent thread.  ``subagent_stop`` is reserved for the Codex-style
        # child round-end control hook inside the child's StopController.
        ...
        # subagent_complete hook loop so we don't walk `results` twice.
        _children_cost_total = 0.0
        _children_actual_cost_total: float | None = None
        _children_actual_call_count = 0
        _children_billable_call_count = 0
        for entry in results:
            child_role = entry.pop("_child_role", None)
            child_cost = entry.pop("_child_cost_usd", 0.0)
            child_actual_cost = entry.pop("_child_actual_cost_usd", None)
            child_actual_calls = entry.pop("_child_cost_actual_call_count", 0)
            child_total_calls = entry.pop("_child_cost_total_call_count", 0)
            # Resume history is for collab/internal callers only — never ship
            # full child message lists back into the parent model tool result.
            entry.pop("messages", None)
            entry.pop("child_session_id", None)
            try:
                if child_cost:
                ...
                pass
            try:
                if child_actual_cost is not None:
                    _children_actual_cost_total = (
                        (_children_actual_cost_total or 0.0)
                        + float(child_actual_cost)
                    )
                _children_actual_call_count += max(0, int(child_actual_calls or 0))
                _children_billable_call_count += max(0, int(child_total_calls or 0))
            except (TypeError, ValueError):
                pass
            ...
            try:
                ...
                logger.debug("subagent_complete hook invocation failed", exc_info=True)
        ...
        # fixtures, etc.).
        if _children_billable_call_count or _children_actual_cost_total is not None:
            try:
                from agent.usage_pricing import accumulate_session_cost

                accumulate_session_cost(
                    parent_agent,
                    estimated_cost_usd=_children_cost_total,
                    actual_cost_usd=_children_actual_cost_total,
                    actual_call_count=_children_actual_call_count,
                    billable_call_count=_children_billable_call_count,
                    estimated_status="estimated",
                    estimated_source="subagent",
                )
            except Exception:
                logger.debug("Subagent actual-cost rollup failed", exc_info=True)
        elif _children_cost_total > 0.0:
            try:
    ...
    # Source: legacy tools/delegate_tool.py delegate_task background path.
    ...
    if background:
        ...
        # conversation (issue #10760). Fall back to SYNCHROMANAGED execution: the
        ...
        if not _async_ok:
            ...
            if isinstance(_sync_result, dict):
                _sync_result["note"] = (
                    ...
                    "after the turn ends), so the subagent(s) ran SYNCHROMANAGEDLY and "
        ...
        try:
            ...
            _source = get_session_env("HAIRBALL_SESSION_SOURCE", "")
            _origin_ui_session_id = get_session_env("HAIRBALL_UI_SESSION_ID", "")
            ...
            # Desktop/TUI and hairball-ui all route via the durable AIAgent
            # session id (UI tab id). Compression can rotate that id mid-turn;
            # capturing the approval context key instead orphans the completion.
            if _source in {"tui", "desktop", "hairball-ui"}:
        ...
        if not _session_key:
            ...
            # during gateway/TUI turns and HAIRBALL_SESSION_KEY is not in the CLI
        ...
        _batch_ready = threading.Event()

        def _batch_runner():
            _batch_ready.wait(timeout=5.0)
        ...
        if dispatch.get("status") == "dispatched":
            n = len(_goals)
            base_delegation_id = str(dispatch.get("delegation_id") or "").removeprefix("bg:")
            for idx, ref in enumerate(_child_delegation_refs):
                if not isinstance(ref, dict):
                    continue
                ref["delegation_id"] = (
                    f"{base_delegation_id}:{idx}" if n > 1 and base_delegation_id else base_delegation_id
                )
            _batch_ready.set()
            note = (
                "Subagent is running in the background (collab mailbox — no "
                "inject turn). Continue local work; collab_wait wakes with "
                "summaries when ready (collab_collect optional). Prefer "
                "collab_spawn for new parallel work."
                if n == 1 else
                f"{n} subagents are running in parallel in the background "
                f"(collab mailbox — no inject turn). Continue local work; "
                f"collab_wait until ready (summaries autosurface; "
                f"collab_collect optional). Prefer collab_spawn for new "
                f"parallel work."
            )
            payload = {
                "status": "dispatched",
        ...
        if isinstance(_cap_result, dict):
            _cap_result["note"] = (
                ...
                "SYNCHROMANAGEDLY and the result is included above. Raise "
...
def _resolve_delegation_credentials(cfg: dict, parent_agent) -> dict:
    ...
    if not api_key:
        raise ValueError(
            ...
            f"Set the appropriate environment variable or run 'hairball auth'."
        )
...
def _load_config() -> dict:
    """Load delegation config from the active Hairball config.

    Prefer the shared persistent loader because it follows the active
    HAIRBALL_HOME/profile. ``cli.CLI_CONFIG`` is a legacy fallback for entry
    ...
    ``HAIRBALL_IGNORE_USER_CONFIG=1`` (``hairball chat --ignore-user-config``) is
    ...
    """
    prefer_legacy = os.environ.get("HAIRBALL_IGNORE_USER_CONFIG") == "1"
...
def _build_top_level_description() -> str:
    ...
    return (
        ...
        "PARALLELISM REQUIREMENT: when the user asks for multiple independent "
        "items, numbered requirements, comparisons across files/apps, or a broad "
        "investigation with separable workstreams, you MUST use 'tasks' and put "
        "one independent workstream in each task. Do NOT pack a numbered list or "
        "multi-part request into one 'goal'; that starts only one child agent and "
        "defeats parallel orchestration.\n\n"
        "BOTH MODES RUN IN THE BACKGROUND. delegate_task returns immediately. "
        "Results do NOT auto-inject a new parent turn — use collab_wait "
        "(summaries usually included on wake; collab_collect optional/idempotent; "
        "same mailbox rail as collab_spawn), then synthesize once. Prefer "
        "collab_spawn for new parallel multi-agent work.\n\n"
        ...
        "- Broad codebase discovery / \"where is X?\" sweeps → "
        "profile='explore' (read-only; returns a summary, not dumps)\n\n"
...
def _build_dynamic_schema_overrides() -> dict:
    ...
    overrides_params["properties"]["profile"]["description"] = build_profile_param_description()
    overrides_params["properties"]["profile"]["enum"] = profile_schema_enum()
    # Deep-copy tasks.items.properties so enum rewrites do not mutate the
    # module-level DELEGATE_TASK_SCHEMA (shallow property copy still shares
    # nested dicts).
    tasks_prop = dict(overrides_params["properties"]["tasks"])
    tasks_items = dict(tasks_prop.get("items") or {})
    task_item_props = {
        k: dict(v) for k, v in (tasks_items.get("properties") or {}).items()
    }
    if "profile" in task_item_props:
        task_item_props["profile"]["description"] = (
            "Per-task profile override. See top-level 'profile' for semantics."
        )
        task_item_props["profile"]["enum"] = profile_schema_enum()
        tasks_items["properties"] = task_item_props
        ...
        "properties": {
            "goal": {
                ...
                "description": (
                    ...
                    "conversation history. Paths, IDs, and other identifiers "
                    "the user named must be copied verbatim into goal (and "
                    "context) — do not substitute a 'similar' path or file "
            ...
            "context": {
                ...
                "description": (
                    ...
                    "Copy user-named paths and identifiers verbatim; do not "
                    "rewrite them to a different tree or app surface."
                ),
            },
            "tasks": {
                "type": "array",
                "items": {
                    ...
                    "properties": {
                        "goal": {
                            "type": "string",
                            "description": (
                                "Task goal. Copy user-named paths/IDs "
                                "verbatim — do not substitute a similar path."
                            ),
                        },
                        "context": {
                            "type": "string",
                            "description": (
                                "Task-specific context. Paths and identifiers "
                                "verbatim; no equivalent-path swaps."
                            ),
                        },
                        "role": {
                            "type": "string",
                        ...
                        "profile": {
                            "type": "string",
                            "enum": ["default", "worker", "explore"],
                            "description": (
                                "Per-task profile override. See top-level "
                                "'profile' for semantics."
                            ),
                        },
                    },
                    "required": ["goal"],
            ...
            "background": {
                ...
                "description": (
                    ...
                    "cannot) opt in or out. Collect via collab_wait → "
                    "collab_collect (no inject turn). Setting this has no "
