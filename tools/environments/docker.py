...
from tools.environments.local import (
    _HAIRBALL_PROVIDER_ENV_BLOCKLIST,
    _is_hairball_internal_secret,
)
...
def _load_hairball_env_vars() -> dict[str, str]:
    """Load ~/.hairball/.env values without failing Docker command execution."""
    try:
...
def _get_active_profile_name() -> str:
    """Return the active Hairball profile name, or ``"default"`` on any error.
...
def reap_orphan_containers(
    *,
    ...
    """Remove stale hairball-tagged containers left behind by prior processes.

    Targets containers that match all of:

    * ``label=hairball-agent=1`` (created by this codebase)
    * ``status=exited`` (running containers are NEVER reaped — they may
      belong to a sibling Hairball process whose reuse path will pick them
      up; killing them would crash the sibling mid-command)
    * (optional) ``label=hairball-profile=<profile_filter>`` (sweep only the
      caller's profile by default; a hairball process in profile A must not
    ...
    * ``State.FinishedAt`` older than *max_age_seconds* ago (so a sibling
    ...
    even with the cleanup-fix in the prior commit, a hard-killed Hairball
    process leaves its container behind permanently because there's no
    subsequent Hairball process scheduled to reuse that exact (task, profile)
    pair.
    """
    docker = docker_exe or find_docker() or "docker"
    filters = ["--filter", "label=hairball-agent=1", "--filter", "status=exited"]
    if profile_filter:
        filters.extend(["--filter", f"label=hairball-profile={_sanitize_label_value(profile_filter)}"])
...
def find_docker() -> Optional[str]:
    ...
    Resolution order:
    1. ``HAIRBALL_DOCKER_BINARY`` env var — explicit override (e.g. ``/usr/bin/podman``)
    ...
    override = os.getenv("HAIRBALL_DOCKER_BINARY")
    if override and os.path.isfile(override) and os.access(override, os.X_OK):
        _docker_executable = override
        logger.info("Using HAIRBALL_DOCKER_BINARY override: %s", override)
        return override
...
#   SETUID/SETGID - the image's init drops from root to the 'hairball'
...
def _image_uses_init_entrypoint(docker_exe: str, image: str) -> bool:
    ...
    ``hairball-agent:latest``) already provide their own PID-1 init and execute
...
class DockerEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        # (TERMINAL_SANDBOX_DIR, default ~/.hairball/sandboxes/). Non-persistent
        ...
        # s6-overlay images (e.g. hairball-agent:latest) already use /init as PID 1
        ...
        container_name = f"hairball-{uuid.uuid4().hex[:8]}"
        # Labels make hairball-created containers identifiable to:
        #   * the orphan reaper (`hairball-agent=1` for the global sweep filter)
        #   * future cross-process reuse (`hairball-task-id`, `hairball-profile`)
        #   * operators running `docker ps --filter label=hairball-agent=1`
        # Values are limited to the safe character set defined by
        # _sanitize_label_value(); the active Hairball profile is captured at
        ...
        label_args = [
            "--label", "hairball-agent=1",
            "--label", f"hairball-task-id={task_label}",
            "--label", f"hairball-profile={profile_name}",
        ]
        ...
        self._labels = {
            "hairball-agent": "1",
            "hairball-task-id": task_label,
            "hairball-profile": profile_name,
        }

        # Cross-process container reuse (issue #20561 — docs claim "ONE long-lived
        # container shared across sessions").  If a prior Hairball process
    ...
    def _build_init_env_args(self) -> list[str]:
        ...
        # win over the generic Hairball secret blocklist. Only implicit passthrough
        # keys are filtered. Also strip Hairball-internal dynamic secrets
        # (AUXILIARY_*_API_KEY / _BASE_URL, GATEWAY_RELAY_* auth) that the
        # name-based blocklist doesn't cover — see _is_hairball_internal_secret.
        _implicit_forward = {
            k for k in passthrough_keys if not _is_hairball_internal_secret(k)
        }
        forward_keys = explicit_forward_keys | (_implicit_forward - _HAIRBALL_PROVIDER_ENV_BLOCKLIST)
        hairball_env = _load_hairball_env_vars() if forward_keys else {}
        for key in sorted(forward_keys):
            value = os.getenv(key)
            if not value:
                value = hairball_env.get(key)
            if value:
                exec_env[key] = value
    ...
    def _recreate_container(self) -> bool:
        ...
        task_label = self._labels.get("hairball-task-id", "")
        profile_label = self._labels.get("hairball-profile", "")
        ...
        if not self._container_id:
            ...
            try:
                ...
                new_name = f"hairball-{_uuid.uuid4().hex[:8]}"
    ...
    def _find_reusable_container(self, task_label: str, profile_label: str) -> Optional[tuple[str, str]]:
        ...
        matches containers that happened to be named ``hairball-*`` but were
        ...
        try:
            result = subprocess.run(
                [
                    ...
                    "--filter", "label=hairball-agent=1",
                    "--filter", f"label=hairball-task-id={task_label}",
                    "--filter", f"label=hairball-profile={profile_label}",
                    "--format", "{{.ID}}\t{{.State}}",
                ],
                capture_output=True,
                text=True,
        ...
        # container) but can happen if a previous Hairball process crashed
    ...
    def cleanup(self, *, force_remove: bool = False):
        ...
        container shared across sessions" and stopping it on every Hairball exit
        breaks that promise:
        ...
        * The user-visible difference between "ONE long-lived container" and
        ...
        Hairball process touches a labeled container for ``2 × lifetime_seconds``
        it gets ``docker rm -f``'d at the next Hairball startup. That covers the
        ...
        # outlives Hairball processes, processes inside it stay alive, and
        ...
        import threading
        t = threading.Thread(target=_do_cleanup, daemon=True, name=f"hairball-cleanup-{log_id}")
        t.start()
    ...
    def wait_for_cleanup(self, timeout: float = 30.0) -> bool:
        ...
        before the Python process exits — without this, ``hairball /quit``
