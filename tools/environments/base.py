"""Base class for all Hairball execution environment backends.
...
# HAIRBALL_DEBUG_INTERRUPT=1 to log loop entry/exit, periodic heartbeats, and
# every is_interrupted() state change from _wait_for_process.  Off by default
# to avoid flooding production gateway logs.
_DEBUG_INTERRUPT = bool(os.getenv("HAIRBALL_DEBUG_INTERRUPT"))
...
class _BoundedOutputCollector:
    ...
    def __init__(
        ...
        *,
        ...
        artifact_transform: Callable[[str], str] | None = None,
    ):
        ...
        self._artifact_transform = artifact_transform
        self._lock = threading.Lock()
        if artifact_path is not None:
            try:
                artifact_path.parent.mkdir(parents=True, exist_ok=True)
                self._artifact_file = artifact_path.open("w", encoding="utf-8", errors="replace")
            except Exception:
                self._artifact_file = None
    ...
    def append(self, text: str) -> None:
        ...
        with self._lock:
            if self._artifact_file is not None:
                try:
                    artifact_text = (
                        self._artifact_transform(text)
                        if self._artifact_transform is not None
                        else text
                    )
                    self._artifact_file.write(artifact_text)
                except Exception:
                    try:
...
def get_sandbox_dir() -> Path:
    ...
    Configurable via TERMINAL_SANDBOX_DIR. Defaults to {HAIRBALL_HOME}/sandboxes/.
    """
    custom = os.getenv("TERMINAL_SANDBOX_DIR")
    if custom:
        p = Path(custom)
    else:
        p = get_hairball_home() / "sandboxes"
...
def _cwd_marker(session_id: str) -> str:
    return f"__HAIRBALL_CWD_{session_id}__"
...
class BaseEnvironment(ABC):
    """Common interface and unified execution flow for all Hairball backends.
    ...
    def set_background_reconnect_spec(self, spec: dict) -> None:
        """Attach a non-secret descriptor used to re-open this backend.

        Environment construction owns the descriptor because it is the only
        place that knows the selected backend and effective config.  The
        process registry persists a copy; credentials are deliberately
        resolved again from the current Hairball profile after restart.
        """
        self._background_reconnect_spec = dict(spec or {})

    def get_background_reconnect_spec(self) -> dict:
        """Return the non-secret descriptor for background process recovery."""
        return dict(getattr(self, "_background_reconnect_spec", {}) or {})
    ...
    def __init__(self, cwd: str, timeout: int, env: dict = None):
        ...
        self._snapshot_path = f"{temp_dir}/hairball-snap-{self._session_id}.sh"
        self._cwd_file = f"{temp_dir}/hairball-cwd-{self._session_id}.txt"
    ...
    def init_session(self):
        ...
        # would be unique on newer Bash versions, but macOS's system Bash 3.2
        # does not define it at all: every writer then used the literal
        # ``.tmp.`` path and clobbered another writer before the atomic mv.
        # ``mktemp`` creates a unique file beside the final snapshot, keeping
        # the subsequent rename on the same filesystem. The static template is
        # still shell-quoted for Windows/Git-Bash drive letters and spaces.
        _snap_template = self._quote_shell_path(self._snapshot_path + ".tmp.XXXXXX")
        bootstrap = (
            f"umask 077\n"
            f"__hairball_snap_tmp=$(mktemp {_snap_template}) || exit 1\n"
            f"export -p > \"$__hairball_snap_tmp\"\n"
            ...
            f"__hairball_fns=$(declare -F | awk '{{print $3}}' | grep -vE '^_[^_]') || true\n"
            f"[ -n \"$__hairball_fns\" ] && declare -f $__hairball_fns "
            f">> \"$__hairball_snap_tmp\" 2>/dev/null || true\n"
            f"alias -p >> \"$__hairball_snap_tmp\"\n"
            f"echo 'shopt -s expand_aliases' >> \"$__hairball_snap_tmp\"\n"
            f"echo 'set +e' >> \"$__hairball_snap_tmp\"\n"
            f"echo 'set +u' >> \"$__hairball_snap_tmp\"\n"
            ...
            f"mv -f \"$__hairball_snap_tmp\" {_quoted_snap} || rm -f \"$__hairball_snap_tmp\"\n"
    ...
    def _command_prelude(self) -> str:
        """Return backend-specific setup inserted after the session snapshot.

        Remote backends deliberately keep this empty: host-local compatibility
        must never leak a host executable path into a container or remote shell.
        """
        return ""

    def _command_postlude(self) -> str:
        """Return backend-specific cleanup inserted before snapshot persistence."""
        return ""
    ...
    def _wrap_command(self, command: str, cwd: str) -> str:
        ...
        # macOS Bash 3.2 has no BASHPID, which made concurrent writers share a
        # literal temp name. The template remains beside the destination, so
        # ``mv`` still provides an atomic same-filesystem publication.
        _snap_template = self._quote_shell_path(self._snapshot_path + ".tmp.XXXXXX")
        ...
        parts.append("__hairball_ec=$?")

        postlude = self._command_postlude()
        if postlude:
            parts.append(postlude)

        # Restrict Hairball metadata files without changing the user's command
        ...
        if self._snapshot_ready:
            parts.append(
                f"__hairball_snap_tmp='' ; "
                f"__hairball_snap_tmp=$(mktemp {_snap_template} 2>/dev/null) && "
                f"{{ export -p > \"$__hairball_snap_tmp\" && "
                f"mv -f \"$__hairball_snap_tmp\" {_quoted_snap}; }} "
                f"2>/dev/null || rm -f \"$__hairball_snap_tmp\" 2>/dev/null || true"
            )
        ...
        parts.append("exit $__hairball_ec")
    ...
    def _embed_stdin_heredoc(command: str, stdin_data: str) -> str:
        ...
        delimiter = f"HAIRBALL_STDIN_{uuid.uuid4().hex[:12]}"
    ...
    def _wait_for_process(
        ...
        *,
        ...
        if bounded_capture:
            try:
                artifact_root = get_hairball_home() / "terminal-artifacts"
                artifact_path = artifact_root / f"terminal-{uuid.uuid4().hex[:12]}.log"
            except Exception:
                artifact_path = None
        ...
        if artifact_path is not None:
            try:
                ...
                artifact_transform = lambda text: redact_terminal_output(
                    text,
                    redaction_command,
                    force=True,
                )
        ...
        def _raw_output_meta() -> dict:
            output.close_artifact()
            if not bounded_capture:
                return {}
            if output.total_chars <= capture_limit:
            ...
            return {
                ...
                "raw_output_artifact": f"artifact://{Path(path).stem}",
                "raw_output_chars": output.total_chars,
            }
        ...
        # --- Debug tracing (opt-in via HAIRBALL_DEBUG_INTERRUPT=1) -------------
        ...
        try:
            ...
            while proc.poll() is None:
                ...
                if is_interrupted():
                    ...
                    payload.update(_raw_output_meta())
                    return payload
        ...
        payload = {"output": output.render(), "returncode": proc.returncode}
        payload.update(_raw_output_meta())
        return payload
    ...
    def _extract_cwd_from_output(self, result: dict):
        """Parse the __HAIRBALL_CWD_{session}__ marker from stdout output.

        Updates self.cwd and strips the marker from result["output"].
        Used by remote backends (Docker, SSH, Modal, Daytona, Singularity).
        """
        cwd_path = self._consume_cwd_marker(result)
        if cwd_path:
            self.cwd = cwd_path

    def _consume_cwd_marker(self, result: dict) -> str | None:
        """Strip the command CWD marker and return its path without storing it."""
        ...
        return cwd_path or None

    def _strip_cwd_marker_from_artifact(self, result: dict) -> None:
        """Remove the wrapper's trailing CWD marker from a saved raw-output file."""
        path = result.get("raw_output_path")
        if not path:
            return
        try:
            artifact_path = Path(str(path))
            marker = self._cwd_marker.encode("utf-8")
            with artifact_path.open("r+b") as f:
                ...
                if last == -1:
                    return
                first = tail.rfind(marker, 0, last)
                if first == -1 or first == last:
                    return
                line_start = tail.rfind(b"\n", 0, first)
                if line_start == -1:
                    line_start = first
                truncate_at = size - window + line_start
                f.truncate(max(0, truncate_at))
                removed = max(0, size - truncate_at)
                raw_chars = result.get("raw_output_chars")
                if isinstance(raw_chars, int):
                    result["raw_output_chars"] = max(0, raw_chars - removed)
        except Exception:
            logger.debug("Could not strip raw-output CWD marker", exc_info=True)
    ...
    def execute(
        ...
        *,
        ...
        result = self._wait_for_process(
            ...
            redaction_command=command,
        )
        if track_cwd:
            self._update_cwd(result)
        else:
            # Internal control-plane operations (for example execute_code's
            # remote RPC polling) must consume the private marker without
            # replacing the session cwd that model-facing tools share.
            self._consume_cwd_marker(result)
        self._strip_cwd_marker_from_artifact(result)
