...
_SNAPSHOT_STORE = get_hairball_home() / "singularity_snapshots.json"
...
def _get_scratch_dir() -> Path:
    ...
    if scratch.exists() and os.access(scratch, os.W_OK):
        user_scratch = scratch / os.getenv("USER", "hairball") / "hairball-agent"
...
class SingularityEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        task_id: str = "default",
        reconnect_instance_id: str = "",
    ):
        super().__init__(cwd=cwd, timeout=timeout)
        self.executable = _ensure_singularity_available()
        self.image = _get_or_build_sif(image, self.executable)
        self.instance_id = reconnect_instance_id or f"hairball_{uuid.uuid4().hex[:12]}"
        ...
        if self._persistent:
            overlay_base = _get_scratch_dir() / "hairball-overlays"
            overlay_base.mkdir(parents=True, exist_ok=True)
            self._overlay_dir = overlay_base / f"overlay-{task_id}"
            self._overlay_dir.mkdir(parents=True, exist_ok=True)

        if reconnect_instance_id:
            probe = subprocess.run(
                [self.executable, "exec", f"instance://{self.instance_id}", "true"],
                capture_output=True,
                text=True,
                timeout=30,
                stdin=subprocess.DEVNULL,
            ...
            if probe.returncode != 0:
                raise RuntimeError(
                    f"Singularity instance {self.instance_id!r} is not available for reconnect"
                )
