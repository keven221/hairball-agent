"""Shared Hairball-side execution flow for Modal transports.

This module deliberately stops at the Hairball boundary:
- command preparation
...
def wrap_modal_stdin_heredoc(command: str, stdin_data: str) -> str:
    ...
    marker = f"HAIRBALL_EOF_{uuid.uuid4().hex[:8]}"
...
class BaseModalExecutionEnvironment(BaseEnvironment):
    ...
    def execute(
        ...
        *,
        ...
        _ = bounded_capture
        # The managed gateway owns cwd tracking and this adapter never mutates
        # ``self.cwd`` from command output. Accept the shared interface flag so
        # internal control-plane callers work across every remote backend.
        _ = track_cwd
