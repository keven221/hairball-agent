...
wrapper, while preserving Hairball' persistent snapshot behavior across sessions.
"""
...
_SNAPSHOT_STORE = get_hairball_home() / "modal_snapshots.json"
...
class ModalEnvironment(BaseEnvironment):
    ...
    def __init__(
        ...
        async def _create_sandbox(image_spec: Any):
            app = await _modal.App.lookup.aio("hairball-agent", create_if_missing=True)
        ...
        try:
            if reconnect_sandbox_id:
                self._sandbox = self._worker.run_coroutine(
                    _modal.Sandbox.from_id.aio(reconnect_sandbox_id), timeout=60,
                )
            else:
        ...
        self._sync_manager = FileSyncManager(
            get_files_fn=lambda: iter_sync_files("/root/.hairball"),
    ...
    def _modal_bulk_download(self, dest: Path) -> None:
        """Download remote .hairball/ as a tar archive.

        Modal sandboxes always run as root, so /root/.hairball is hardcoded
        (consistent with iter_sync_files call on line 269).
        """
        async def _download():
            proc = await self._sandbox.exec.aio(
                "bash", "-c", "tar cf - -C / root/.hairball"
            )
