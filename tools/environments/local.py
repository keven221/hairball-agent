...
_READ_ONLY_LOCAL_PROCESS: ContextVar[bool] = ContextVar(
    "hairball_read_only_local_process",
    default=False,
)
...
# Hairball-internal env vars that should NOT leak into terminal subprocesses.
_HAIRBALL_PROVIDER_ENV_FORCE_PREFIX = "_HAIRBALL_FORCE_"

# Hairball-managed AWS *inference* credentials for ``auth_type="aws_sdk"``
# providers (Bedrock).  Scoped DELIBERATELY NARROW: this lists only the
# Bedrock-specific bearer token, which is a Hairball inference secret exactly
...
def _build_provider_env_blocklist() -> frozenset:
    ...
    try:
        from hairball_cli.config import OPTIONAL_ENV_VARS
    ...
    blocked.update({
        ...
        "HAIRBALL_DASHBOARD_SESSION_TOKEN",
    ...
    # Hairball-managed inference credential — Claude subscription auth is not a
    # working Hairball provider path.  Stripping it broke agent-spawned
...
_HAIRBALL_PROVIDER_ENV_BLOCKLIST = _build_provider_env_blocklist()
...
# project's dependencies into the Hairball venv path instead of the project's own
# ``.venv`` — silently clobbering the Hairball environment (e.g. a project pinned
# to a different Python version overwrites it and breaks the gateway). The
# Hairball venv stays reachable via PATH (its bin dir is first), so stripping
# these markers is safe and only prevents the cross-project clobber (#23473).
_ACTIVE_VENV_MARKER_VARS = ("VIRTUAL_ENV", "CONDA_PREFIX")


def _is_hairball_internal_secret(key: str) -> bool:
    """Return True for Hairball-internal secrets injected under *dynamic* names.

    ``_HAIRBALL_PROVIDER_ENV_BLOCKLIST`` is name-based and derived from the
    ...
    This is the single source of truth for "Hairball-internal dynamic secret"
    across every spawn path — the terminal ``_make_run_env`` /
    ``_sanitize_subprocess_env`` filters, the Docker passthrough filter, and the
    non-terminal :func:`hairball_subprocess_env` helper all call it, so the
...
def _inject_context_hairball_home(env: dict) -> None:
    """Bridge the context-local Hairball home override into subprocess env."""
    try:
        from hairball_constants import get_hairball_home_override

        value = get_hairball_home_override()
        if value:
            env["HAIRBALL_HOME"] = value
    except Exception:
        pass
...
def _inject_session_context_env(env: dict) -> None:
    ...
    (HAIRBALL_SESSION_*) are bridged onto the child env here.
...
def _sanitize_subprocess_env(base_env: dict | None, extra_env: dict | None = None) -> dict:
    """Filter Hairball-managed secrets from a subprocess environment."""
    try:
    ...
    for key, value in (base_env or {}).items():
        if key.startswith(_HAIRBALL_PROVIDER_ENV_FORCE_PREFIX):
            continue
        if _is_hairball_internal_secret(key):
            continue
        if key not in _HAIRBALL_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    for key, value in (extra_env or {}).items():
        if key.startswith(_HAIRBALL_PROVIDER_ENV_FORCE_PREFIX):
            real_key = key[len(_HAIRBALL_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_hairball_internal_secret(real_key):
                continue
            sanitized[real_key] = value
        elif _is_hairball_internal_secret(key):
            continue
        elif key not in _HAIRBALL_PROVIDER_ENV_BLOCKLIST or _is_passthrough(key):
            sanitized[key] = value

    _inject_context_hairball_home(sanitized)
...
# legitimate child Hairball spawns needs them, and they are the highest-value
...
# narrow subset of _HAIRBALL_PROVIDER_ENV_BLOCKLIST; provider keys are handled by
# the conditional Tier-2 strip in hairball_subprocess_env().
_ALWAYS_STRIP_KEYS: frozenset[str] = frozenset({
    ...
    # _is_hairball_internal_secret, but _ID has no secret suffix, so it must be
...
def hairball_subprocess_env(*, inherit_credentials: bool = False) -> dict[str, str]:
    ...
    single source of truth (``_HAIRBALL_PROVIDER_ENV_BLOCKLIST``).  The terminal
    ...
    * **Tier 1 (always):** ``_ALWAYS_STRIP_KEYS`` — gateway bot tokens, GitHub
      ...
      ``inherit_credentials``.  No child Hairball spawns legitimately needs them.
    * **Tier 2 (conditional):** the rest of ``_HAIRBALL_PROVIDER_ENV_BLOCKLIST``
    ...
    for key in _ALWAYS_STRIP_KEYS:
        env.pop(key, None)
    # Internal routing hints and Hairball-internal dynamic secrets
    ...
    # legitimate use for them. See :func:`_is_hairball_internal_secret`.
    for key in list(env):
        if key.startswith(_HAIRBALL_PROVIDER_ENV_FORCE_PREFIX):
            env.pop(key, None)
        elif _is_hairball_internal_secret(key):
    ...
    if not inherit_credentials:
        ...
        for key in _HAIRBALL_PROVIDER_ENV_BLOCKLIST:
            env.pop(key, None)

    # Windows UTF-8 safety for spawned processes (#31420).
    env.setdefault("PYTHONUTF8", "1")

    _inject_context_hairball_home(env)
    from hairball_constants import apply_subprocess_home_env
    ...
    # copies os.environ, whose HAIRBALL_SESSION_* mirror is a last-writer-wins
...
def _find_bash() -> str:
    ...
    custom = os.environ.get("HAIRBALL_GIT_BASH_PATH")
    if custom and os.path.isfile(custom):
        candidates.append(custom)

    # Prefer our own portable Git install — a broken or partially-uninstalled
    # system Git (or a stale HAIRBALL_GIT_BASH_PATH pointing at one) must not
    ...
    #   PortableGit: %LOCALAPPDATA%\hairball\git\bin\bash.exe   (primary)
    #   MinGit:      %LOCALAPPDATA%\hairball\git\usr\bin\bash.exe (legacy/32-bit fallback)
    _local_appdata = os.environ.get("LOCALAPPDATA", "")
    _hairball_portable_git = os.path.join(_local_appdata, "hairball", "git") if _local_appdata else ""
    if _hairball_portable_git:
        for candidate in (
            os.path.join(_hairball_portable_git, "bin", "bash.exe"),        # PortableGit (primary)
            os.path.join(_hairball_portable_git, "usr", "bin", "bash.exe"), # MinGit fallback
        ):
    ...
    # HAIRBALL_GIT_BASH_PATH pointing at a broken Git-for-Windows install
    # (``Directory \\drivers\\etc does not exist``) must not win over a
    # healthy portable Git under %LOCALAPPDATA%\\hairball\\git.
    for candidate in candidates:
        if _bash_starts(candidate):
            if candidate != custom and custom and os.path.isfile(custom):
                logger.warning(
                    "HAIRBALL_GIT_BASH_PATH=%s fails to start; using %s instead",
                    custom,
                    candidate,
                )
            return candidate
    ...
    raise RuntimeError(
        "Git Bash not found. Hairball Agent requires Git for Windows on Windows.\n"
        "Install it from: https://git-scm.com/download/win\n"
        "Or set HAIRBALL_GIT_BASH_PATH to your bash.exe location."
    )
...
# Cached directory containing the ``hairball`` console-script.
# ``_SENTINEL`` distinguishes "not resolved yet" from a resolved ``None``.
_SENTINEL = object()
_HAIRBALL_BIN_DIR: "str | None | object" = _SENTINEL


def _resolve_hairball_bin_dir() -> str | None:
    """Return the directory holding the ``hairball`` console-script, or None.
    ...
    hairball install dir (``~/.local/bin``, the venv ``bin``/``Scripts``, pipx,
    nix) is absent from that PATH, so plugins shelling out to bare ``hairball``
    via the terminal tool hit ``command not found`` (exit 127) even though
    ``hairball`` works fine in the user's own interactive terminal.

    We resolve the install dir once (it never changes within a process) and
    prepend-if-missing it to the subshell PATH so bare ``hairball`` resolves
    regardless of how the gateway was started.

    Resolution order (cheap, no heavy imports):
      1. ``shutil.which("hairball")`` — normal PATH-installed shim.
      2. The directory of ``sys.argv[0]`` when it's an absolute path to a
         real ``hairball`` executable (covers nix-store / venv wrappers).
      3. The directory of ``sys.executable`` — the running interpreter's
         venv ``bin``/``Scripts`` is where its console-scripts live.
    """
    global _HAIRBALL_BIN_DIR
    if _HAIRBALL_BIN_DIR is not _SENTINEL:
        return _HAIRBALL_BIN_DIR  # type: ignore[return-value]

    candidate: str | None = None

    which = shutil.which("hairball")
    if which:
    ...
    if candidate is None:
        ...
        if (
            os.path.isabs(argv0)
            and (base == "hairball" or base.startswith("hairball."))
    ...
    if candidate is None:
        ...
        if exe_dir:
            shim = "hairball.exe" if _IS_WINDOWS else "hairball"
    ...
    _HAIRBALL_BIN_DIR = candidate
    return candidate


def _prepend_hairball_bin_dir(existing_path: str) -> str:
    """Prepend the hairball install dir to ``existing_path`` if it's missing.
    ...
    """
    bin_dir = _resolve_hairball_bin_dir()
    if not bin_dir:
        return existing_path
    sep = os.pathsep
    entries = [e for e in existing_path.split(sep) if e] if existing_path else []
...
def _apply_windows_msys_bash_env_defaults(env: dict) -> None:
    ...
    Windows commands such as ``tasklist``, ``schtasks``, and ``wmic``.  Hairball
...
def _make_run_env(env: dict) -> dict:
    ...
    for k, v in merged.items():
        if k.startswith(_HAIRBALL_PROVIDER_ENV_FORCE_PREFIX):
            real_key = k[len(_HAIRBALL_PROVIDER_ENV_FORCE_PREFIX):]
            if _is_hairball_internal_secret(real_key):
                continue
            run_env[real_key] = v
        elif _is_hairball_internal_secret(k):
            continue
        elif k not in _HAIRBALL_PROVIDER_ENV_BLOCKLIST or _is_passthrough(k):
            run_env[k] = v
    ...
    if path_key is not None:
        ...
        # Ensure the hairball install dir is reachable so plugins can shell out
        # to bare ``hairball`` via the terminal tool even when the gateway was
        # launched without it on PATH (systemd, service managers, cron, etc.).
        run_env[path_key] = _prepend_hairball_bin_dir(new_path)

    _inject_context_hairball_home(run_env)
...
def _resolve_shell_init_files() -> list[str]:
    ...
    an explicit list — once they have, Hairball trusts them.
    """
...
class LocalEnvironment(BaseEnvironment):
    ...
    def build_process_args(self, args: list[str], *, cwd: str | None = None) -> list[str]:
        """Apply the host-native launch boundary used by every local spawn."""

        from tools.environments.native_sandbox import wrap_local_process_args

        return wrap_local_process_args(
            args,
            cwd=cwd or self.cwd,
            config=self.local_config,
            read_only_workspace=_READ_ONLY_LOCAL_PROCESS.get(),
        )

    def execute_read_only(self, command: str, **kwargs):
        """Execute one foreground command with the workspace mounted read-only.

        ContextVar scoping keeps concurrent terminal workers independent while
        reusing the established BaseEnvironment execution and capture path.
        """

        token = _READ_ONLY_LOCAL_PROCESS.set(True)
        try:
            return super().execute(command, **kwargs)
        finally:
            _READ_ONLY_LOCAL_PROCESS.reset(token)
    ...
    def get_temp_dir(self) -> str:
        ...
        **Windows:** hardcoded ``/tmp`` is wrong in two ways — native Python
        ...
        ``HAIRBALL_HOME`` instead — single-word path, guaranteed to exist, same
        string resolves in both Git Bash and native Python.
        """
        if _IS_WINDOWS:
            # Derive a Windows-safe temp dir under HAIRBALL_HOME.  Using
            ...
            try:
                ...
                cache_dir = get_hairball_home() / "cache" / "terminal"
            except Exception:
                cache_dir = Path(tempfile.gettempdir()) / "hairball_terminal"
    ...
    def _command_prelude(self) -> str:
        """Make the running Python available only when the shell lacks it.

        macOS commonly exposes ``python3`` but not ``python``.  Agent-authored
        commands routinely use the latter.  The compatibility path is local
        only, is evaluated after the user's snapshot has restored PATH, and
        never replaces an existing project/user ``python``.
        """
        executable = str(sys.executable or "")
        directory = os.path.dirname(executable)
        if not directory or not os.path.isfile(executable) or not os.access(executable, os.X_OK):
            return ""
        quoted_directory = self._quote_shell_path(directory)
        return (
            "if ! python -c 'import sys' >/dev/null 2>&1; then "
            f"export PATH={quoted_directory}:$PATH; "
            f"export __HAIRBALL_TEMP_PYTHON_SHIM_DIR={quoted_directory}; "
            "fi"
        )

    def _command_postlude(self) -> str:
        """Keep the compatibility PATH entry out of the persistent snapshot."""
        return (
            'if [ -n "${__HAIRBALL_TEMP_PYTHON_SHIM_DIR:-}" ]; then '
            'PATH="${PATH#"$__HAIRBALL_TEMP_PYTHON_SHIM_DIR:"}"; '
            "unset __HAIRBALL_TEMP_PYTHON_SHIM_DIR; "
            "fi"
        )
    ...
    def _run_bash(self, cmd_string: str, *, login: bool = False,
        ...
        args = self.build_process_args(args, cwd=_popen_cwd)
        ...
        if not _IS_WINDOWS:
            try:
                proc._hairball_pgid = os.getpgid(proc.pid)
        ...
        try:
            if _IS_WINDOWS:
                ...
                try:
                    ...
                    pgid = getattr(proc, "_hairball_pgid", None)
                    if pgid is None:
                        raise
