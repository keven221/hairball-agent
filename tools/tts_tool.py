...
- Users can declare any number of named providers with ``type: command``
  under ``tts.providers.<name>`` in ``~/.hairball/config.yaml``. Hairball
...
Configuration is loaded from ~/.hairball/config.yaml under the 'tts:' key.
...
def get_env_value(name, default=None):
    ...
    Tests may monkeypatch and later restore ``hairball_cli.config.get_env_value``
    before this module is imported. Resolve the helper at call time so TTS does
    not keep a stale imported function for the rest of the test process.
    """
    try:
        from hairball_cli.config import get_env_value as _get_env_value
    except ImportError:
...
def _resolve_provider_key(env_var: str, provider_id: str) -> str:
    """Resolve a TTS key through the shared profile-safe voice resolver."""
    try:
...
from tools.xai_http import hairball_xai_user_agent
...
def _elevenlabs_environment_kwargs(el_config: Dict[str, Any]) -> Dict[str, Any]:
    """Build ElevenLabs client kwargs honoring config base/wss URLs."""
...
# The managed OpenAI audio gateway (Managed Portal proxy) only proxies these speech
...
# Base URL now resolved via hairball_cli.models.deepinfra_base_url (shared).
...
def _get_default_output_dir() -> str:
    ...
    return str(get_hairball_dir("cache/audio", "audio_cache"))
...
# ===========================================================================
# Config loader -- reads tts: section from ~/.hairball/config.yaml
# ===========================================================================
def _load_tts_config() -> Dict[str, Any]:
    """
    Load TTS configuration from ~/.hairball/config.yaml.
    ...
    try:
        ...
        logger.debug("hairball_cli.config not available, using default TTS config")
        return {}
    except Exception as e:
...
def _get_provider(tts_config: Dict[str, Any]) -> str:
    ...
    ``hairball tools``); otherwise the historical Edge backend remains active.
    """
...
# custom voice-cloning scripts, etc.) into Hairball without any Python code
...
#           output_format: wav
#
# Hairball writes the input text to a temp UTF-8 file, runs the command with
...
def _render_command_tts_template(
    ...
    def replace_match(match: re.Match[str]) -> str:
        ...
        token = f"__HAIRBALL_TTS_PLACEHOLDER_{len(replacements)}__"
        replacements.append((
            token,
...
# file, so retired models disappear from hairball the next time the
...
def _generate_deepinfra_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    the shared ``hairball_cli.models`` helpers so every DeepInfra surface
    resolves them identically.
    """
    api_key = (get_env_value("DEEPINFRA_API_KEY") or "").strip()
    if not api_key:
        raise ValueError(
            "DEEPINFRA_API_KEY not set. Run `hairball setup` to configure, "
    ...
    from hairball_cli.models import deepinfra_base_url, deepinfra_model_ids
...
def _generate_xai_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if not api_key:
        raise ValueError("No xAI credentials found. Configure xAI OAuth in `hairball model` or set XAI_API_KEY.")
    ...
    # output_format when Hairball actually needs a non-default format/override.
    ...
    response = requests.post(
        ...
        headers={
            ...
            "User-Agent": hairball_xai_user_agent(),
        },
        json=payload,
        timeout=60,
    )
...
def _generate_gemini_tts(text: str, output_path: str, tts_config: Dict[str, Any]) -> str:
    ...
    if urlparse(base_url).hostname == "generativelanguage.googleapis.com":
        try:
            import hairball_cli as _hairball_cli

            _hairball_version = str(_hairball_cli.__version__)
        except Exception:
            _hairball_version = "0.0.0"
        # Include Hairball client context following Gemini's partner
...
def _get_piper_voices_dir() -> Path:
    """Return the directory where Hairball caches Piper voice models.

    Resolves to ``~/.hairball/cache/piper-voices/`` under the active
    HAIRBALL_HOME so voice downloads follow profile boundaries.
    """
    from hairball_constants import get_hairball_dir
    root = Path(get_hairball_dir("cache/piper-voices", "piper_voices_cache"))
...
def text_to_speech_tool(
    ...
    Reads provider/voice config from ~/.hairball/config.yaml (tts: section).
    ...
    platform = get_session_env("HAIRBALL_SESSION_PLATFORM", "").lower()
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Mistral provider selected but 'mistralai' package not installed. "
                             "Run: pip install 'hairball-agent[mistral]'"
                }, ensure_ascii=False)
            ...
            if not _check_neutts_available():
                return json.dumps({
                    ...
                    "error": "NeuTTS provider selected but neutts is not installed. "
                             "Run hairball setup and choose NeuTTS, or install espeak-ng and run python -m pip install -U neutts[all]."
                }, ensure_ascii=False)
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "KittenTTS provider selected but 'kittentts' package not installed. "
                             "Run 'hairball setup tts' and choose KittenTTS, or install manually: "
            ...
            try:
                ...
                return json.dumps({
                    ...
                    "error": "Piper provider selected but 'piper-tts' package not installed. "
                             "Run 'hairball tools' and select Piper under TTS, or install manually: "
...
def check_tts_requirements() -> bool:
    ...
    if provider == "edge":
        from tools.lazy_deps import is_usable_or_installable

        return is_usable_or_installable("tts.edge") or _check_neutts_available()
    if provider == "elevenlabs":
        from tools.lazy_deps import is_usable_or_installable

        return bool(get_env_value("ELEVENLABS_API_KEY")) and is_usable_or_installable("tts.elevenlabs")
    if provider == "openai":
        try:
            _import_openai_client()
        except ImportError:
    ...
    if provider == "mistral":
        ...
        return bool(get_env_value("MISTRAL_API_KEY")) and is_usable_or_installable("tts.mistral")
    if provider == "neutts":
...
def _resolve_openai_audio_client_config() -> tuple[str, str, bool]:
    ...
    ``is_managed`` is True when the config resolves to the managed managed audio
    ...
    if managed_gateway is None:
        ...
        if managed_portal_tools_enabled() or prefers_gateway("tts"):
            message += (
                ". "
                + managed_tool_gateway_unavailable_message(
...
def _has_openai_audio_backend() -> bool:
    """Return True when OpenAI audio has config, direct, or managed credentials."""
...
TTS_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "output_path": {
                "type": "string",
                "description": f"Optional custom file path to save the audio. Defaults to {display_hairball_home()}/audio_cache/<timestamp>.mp3"
            },
            "speed": {
                "type": "number",
                "description": "Playback speed multiplier from 0.25 to 4.0. Overrides the configured speed for this call only."
            },
            "instructions": {
                "type": "string",
                "description": "Optional voice-design guidance such as tone, emotion, pacing, accent, or whispering. Used by compatible OpenAI speech models and ignored by providers that do not support it."
            },
            "provider": {
                "type": "string",
                "description": "Optional provider override for this call. When omitted, Hairball uses the configured tts.provider."
            }
        },
        "required": ["text"]
    }
