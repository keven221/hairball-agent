...
def _sanitize_single_tool(tool: dict) -> dict:
    ...
    # Strip top-level combinators that strict backends (OpenAI's reference
...
def _strip_top_level_combinators(params: dict, *, path: str = "<tool>") -> dict:
    ...
    OpenAI's reference backend (``chatgpt.com/backend-api/codex``) is stricter
...
def strip_pattern_and_format(tools: list[dict]) -> tuple[list[dict], int]:
    ...
    for tool in tools:
        ...
        # (used by codex_responses API mode — xAI, ChatGPT, etc.)
