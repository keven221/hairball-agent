...
it is automatically downloaded from GitHub releases to $HAIRBALL_HOME/bin/tirith.
...
def _get_hairball_home() -> str:
    """Return the Hairball home directory, respecting HAIRBALL_HOME env var."""
    return str(get_hairball_home())


def _failure_marker_path() -> str:
    """Return the path to the install-failure marker file."""
    return os.path.join(_get_hairball_home(), ".tirith-install-failed")
...
def _hairball_bin_dir() -> str:
    """Return $HAIRBALL_HOME/bin, creating it if needed."""
    d = os.path.join(_get_hairball_home(), "bin")
...
def _install_tirith(*, log_failures: bool = True) -> tuple[str | None, str]:
    """Download and install tirith to $HAIRBALL_HOME/bin/tirith.
    ...
    try:
        ...
        dest = os.path.join(_hairball_bin_dir(), "tirith")
        try:
...
def _resolve_tirith_path(configured_path: str) -> str:
    ...
    2. $HAIRBALL_HOME/bin/tirith (previously auto-installed)
    3. Auto-install from GitHub releases → $HAIRBALL_HOME/bin/tirith
    ...
    hairball_bin = os.path.join(_hairball_bin_dir(), "tirith")
    if os.path.isfile(hairball_bin) and os.access(hairball_bin, os.X_OK):
        _resolved_path = hairball_bin
        _install_failure_reason = ""
        _clear_install_failed()
        return hairball_bin
