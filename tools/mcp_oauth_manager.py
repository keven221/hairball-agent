...
`mcp_tool.py`, and `hairball_cli/mcp_config.py`. This module is the ONLY place
...
- reference's ``refresh_oauth_if_needed`` / ``persist_if_needed``
...
# ---------------------------------------------------------------------------
# HairballMCPOAuthProvider — OAuthClientProvider subclass with disk-watch
# ---------------------------------------------------------------------------


def _make_hairball_provider_class() -> Optional[type]:
    ...
    class HairballMCPOAuthProvider(OAuthClientProvider):
        ...
        def __init__(
            ...
            **kwargs: Any,
            ...
            self._hairball_server_name = server_name
            ...
            self._hairball_preregistered = preregistered
        ...
        async def _initialize(self) -> None:
            ...
            Paired with :class:`HairballTokenStorage` persisting an absolute
            ...
            if (
                isinstance(storage, HairballTokenStorage)
            ...
            if (
                ...
                try:
                    ...
                    logger.debug(
                        ...
                        "failed (non-fatal): %s",
                        self._hairball_server_name, exc,
                    )
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                for url in build_protected_resource_metadata_discovery_urls(
                    ...
                    try:
                        ...
                        logger.debug(
                            ...
                            self._hairball_server_name, url, exc,
                        )
                        continue
                ...
                for url in build_oauth_authorization_server_metadata_discovery_urls(
                    ...
                    if asm:
                        ...
                        storage = self.context.storage
                        from tools.mcp_oauth import HairballTokenStorage
                        if isinstance(storage, HairballTokenStorage):
                            storage.save_oauth_metadata(asm)
                        logger.debug(
                            "MCP OAuth '%s': pre-flight ASM discovered "
                            "token_endpoint=%s",
                            self._hairball_server_name, asm.token_endpoint,
                        )
                        break
        ...
        def _persist_oauth_metadata_if_changed(self) -> None:
            ...
            storage = self.context.storage
            from tools.mcp_oauth import HairballTokenStorage
            if not isinstance(storage, HairballTokenStorage):
                return
        ...
        async def _maybe_flag_poisoned_client(self, response: Any) -> None:
            ...
            and is handled by ``hairball mcp reauth``.
            ...
            back to ``hairball mcp reauth``.
            """
            try:
                if self._hairball_preregistered:
                    return
    ...
    return HairballMCPOAuthProvider


# Cached at import time. Tested and used by :class:`MCPOAuthManager`.
_HAIRBALL_PROVIDER_CLS: Optional[type] = _make_hairball_provider_class()
...
class MCPOAuthManager:
    ...
    def _build_provider(
        ...
        Constructs :class:`HairballMCPOAuthProvider` directly using the helpers
        ...
        Returns None if the MCP SDK's OAuth support is unavailable.
        """
        if _HAIRBALL_PROVIDER_CLS is None:
            logger.warning(
        ...
        storage = HairballTokenStorage(server_name)

        if not _is_interactive() and not storage.has_cached_tokens():
            raise OAuthNonInteractiveError(
                "MCP OAuth for "
                f"'{server_name}': non-interactive environment and no "
                "cached tokens found. Run `hairball mcp login "
    ...
    def remove(self, server_name: str) -> None:
        ...
        Called by ``hairball mcp remove <name>`` and (indirectly) by
        ``hairball mcp login <name>`` during forced re-auth.
        """
        with self._entries_lock:
