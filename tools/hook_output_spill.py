...
This mirrors what reference does for its ``PreToolUse``/``Stop``/feedback
...
Config (``config.yaml``)::
    ...
    hooks:
      output_spill:
        ...
        directory: null        # default: <HAIRBALL_HOME>/hook_outputs
...
* Never raises. Any I/O error (disk full, permission denied, missing
  HAIRBALL_HOME, etc.) falls back to a byte-length truncation with an
...
def _resolve_spill_dir(directory_override: Optional[str], session_id: Optional[str]) -> Path:
    ...
    if directory_override:
        ...
        try:
            ...
            base = Path(get_hairball_home()) / "hook_outputs"
        except Exception:
            # Last-resort fallback: HAIRBALL_HOME env var, then ~/.hairball
            home = os.environ.get("HAIRBALL_HOME") or os.path.expanduser("~/.hairball")
