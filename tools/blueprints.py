...
`hairball skills publish` for sharing. No new source type, no new store, no new
...
@dataclass
class BlueprintSpec:
    """Parsed ``metadata.hairball.blueprint`` automation spec for a skill."""
...
def parse_blueprint(skill_md_text: str) -> Optional[BlueprintSpec]:
    ...
    A skill is a blueprint iff ``metadata.hairball.blueprint`` is a mapping containing
    ...
    hairball = meta.get("hairball") if isinstance(meta, dict) else None
    blueprint = hairball.get("blueprint") if isinstance(hairball, dict) else None
    if blueprint is None:
        return None
    if not isinstance(blueprint, dict):
        raise BlueprintError("metadata.hairball.blueprint must be a mapping")
...
def export_blueprint(job: Dict[str, Any], body: str, *, blueprint_name: Optional[str] = None) -> str:
    ...
    and emit a SKILL.md (with a ``metadata.hairball.blueprint`` block) they can hand
    to ``hairball skills publish`` to share. ``body`` is the plain-language
