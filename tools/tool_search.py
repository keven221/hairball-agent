"""Progressive tool disclosure ("tool search") for Hairball Agent.
...
``tool_call`` — and surfaced on demand. Core Hairball tools do not defer by
default; a session may explicitly provide a narrower initial-visibility set
without narrowing its authorization.

Design constraints this module is built around (see ``openclaw-tool-search-report``
for the full rationale):

* Core tools defined in ``toolsets._HAIRBALL_CORE_TOOLS`` are never deferred
  by the general threshold policy. They may be explicitly deferred by a
  session-static visibility scope; this does not grant tools or mutate the
  model schema between turns.
...
* The catalog is stateless across turns and tools-array assemblies. It is
  ...
  from legacy's cron regression (openclaw/openclaw#84141): a session-keyed
...
def _core_tool_names() -> frozenset[str]:
    ...
    """
    try:
        from toolsets import _HAIRBALL_CORE_TOOLS
        return frozenset(_HAIRBALL_CORE_TOOLS)
    except Exception:
        return frozenset()
...
def is_deferrable_tool_name(name: str) -> bool:
    ...
    OR it is not in ``_HAIRBALL_CORE_TOOLS``. Core tools are never deferred
...
def classify_tools(
    ...
    *,
    forced_deferred_names: Optional[Iterable[str]] = None,
    ...
    """
    forced = frozenset(str(name) for name in (forced_deferred_names or ()) if name)
    ...
    for td in tool_defs:
        ...
        if name in BRIDGE_TOOL_NAMES:
            ...
            # but be defensive.
            continue
        if name in forced or is_deferrable_tool_name(name):
...
def search_catalog(
    ...
    """Return one stable result page for ``query`` ranked by BM25.
    ...
    return [entry for _, entry in scored[offset:offset + limit]]
...
def bridge_tool_schemas(deferred_count: int) -> List[Dict[str, Any]]:
    ...
    desc_search = (
        ...
        "Returns up to ``limit`` matches with name and description. If "
        "``next_offset`` is present, pass it back as ``offset`` for the next "
        "non-overlapping page. Follow "
            ...
            "function": {
                ...
                "parameters": {
                    ...
                    "properties": {
                        ...
                        "offset": {
                            "type": "integer",
                            "description": "Pagination offset returned as next_offset by a previous search. Default 0.",
                        },
                    },
                    "required": ["query"],
                },
...
def assemble_tool_defs(
    ...
    *,
    ...
    tools. Core tools are never deferred by threshold/config alone; callers
    may explicitly include authorized core names in ``forced_deferred_names``.
    ...
    if not forced and not should_activate(config, deferrable_tokens, context_length):
...
def _format_search_hit(entry: CatalogEntry) -> Dict[str, Any]:
    ...
    return {
        ...
        # Return the callable contract with the hit so a capable model can
        # search once and immediately call it. tool_describe remains available
        # for models that prefer a separate discovery step.
...
def dispatch_tool_search(args: Dict[str, Any],
                         *,
                         ...
                         config: Optional[ToolSearchConfig] = None,
                         forced_deferred_names: Optional[Iterable[str]] = None) -> str:
    ...
    page = search_catalog(catalog, query, limit=limit + 1, offset=offset)
...
def scoped_deferrable_names(
    ...
    *,
    ...
    for td in tool_defs:
        ...
        if name and (name in forced or is_deferrable_tool_name(name)):
            names.add(name)
...
def resolve_underlying_call(
    args: Dict[str, Any],
    *,
    allowed_deferred_names: Optional[Iterable[str]] = None,
    ...
    * the trajectory recorder.
    ...
    allowed = (
        frozenset(str(item) for item in allowed_deferred_names if item)
        if allowed_deferred_names is not None
        else None
    )
    if allowed is not None and name not in allowed:
        return None, {}, f"'{name}' is not available in this session through tool_call"
    if allowed is None and not is_deferrable_tool_name(name):
        return None, {}, (
