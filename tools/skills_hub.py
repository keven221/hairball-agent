#!/usr/bin/env python3
"""
Skills Hub — Source adapters and hub state management for the Hairball Skills Hub.
...
Used by hairball_cli/skills_hub.py for CLI commands and the /skills slash command.
"""
...
def _hairball_home() -> Path:
    return get_hairball_home()


def _skills_dir() -> Path:
    forced = _override("SKILLS_DIR")
    return Path(forced) if forced is not None else _hairball_home() / "skills"
...
class GitHubSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hairball_meta = metadata.get("hairball", {})
            if isinstance(hairball_meta, dict):
                tags = hairball_meta.get("tags", [])
        if not tags:
...
class UrlSource(SkillSource):
    ...
    def inspect(self, identifier: str) -> Optional[SkillMeta]:
        ...
        if isinstance(metadata, dict):
            hairball_meta = metadata.get("hairball", {})
            if isinstance(hairball_meta, dict):
                raw_tags = hairball_meta.get("tags", [])
...
class OptionalSkillSource(SkillSource):
    ...
    These skills are official (maintained by Hairball) but not activated
    by default — they don't appear in the system prompt and aren't copied to
    ~/.hairball/skills/ during setup.  They are discoverable via the Skills Hub
    (search / install / inspect) and labelled "official" with "builtin" trust.
    """

    OFFICIAL_REPO = "managed/hairball-agent"
    ...
    def _scan_all(self) -> List[SkillMeta]:
        ...
        for skill_md in sorted(self._optional_dir.rglob("SKILL.md")):
            ...
            if isinstance(meta_block, dict):
                hairball_meta = meta_block.get("hairball", {})
                if isinstance(hairball_meta, dict):
                    tags = hairball_meta.get("tags", [])
...
# ---------------------------------------------------------------------------
# Hairball centralized index source
# ---------------------------------------------------------------------------

HAIRBALL_INDEX_URL = "./website/docs/api/skills-index.json"
HAIRBALL_INDEX_TTL = 6 * 3600  # 6 hours


def _hairball_index_cache_file() -> Path:
    return _index_cache_dir() / "hairball-index.json"


def _load_hairball_index(*, allow_network: bool = True) -> Optional[dict]:
    """Fetch the centralized skills index, with local cache.

    The index is a JSON file hosted on the docs site, rebuilt daily by CI.
    We cache it locally for HAIRBALL_INDEX_TTL seconds to avoid repeated
    downloads within a session.
    """
    # Check local cache
    hairball_index_cache_file = _hairball_index_cache_file()
    if hairball_index_cache_file.exists():
        try:
            age = time.time() - hairball_index_cache_file.stat().st_mtime
            if age < HAIRBALL_INDEX_TTL:
                return json.loads(hairball_index_cache_file.read_text())
        except (OSError, json.JSONDecodeError):
            pass

    # Browse-hub landing pages must never turn a 41 MB cold index download into
    # a blocking page request.  Their source router opts out of network I/O and
    # starts the same loader in the background instead.  Search/install/CLI/TUI
    # keep the default and therefore retain the original complete behavior.
    ...
    for accept_encoding in ("gzip, deflate", "identity"):
        try:
            ...
            if resp.status_code != 200:
                logger.debug("Hairball index fetch returned %d", resp.status_code)
                return _load_stale_index_cache()
            data = resp.json()
            break
            ...
            logger.debug(
                "Hairball index decode failed (Accept-Encoding=%s): %s",
                accept_encoding,
                e,
            )
            continue
        except (httpx.HTTPError, json.JSONDecodeError) as e:
            logger.debug("Hairball index fetch failed: %s", e)
            return _load_stale_index_cache()
    ...
    # Cache locally
    try:
        hairball_index_cache_file.parent.mkdir(parents=True, exist_ok=True)
        tmp_cache = hairball_index_cache_file.with_name(
            f".{hairball_index_cache_file.name}.{os.getpid()}.{threading.get_ident()}.tmp"
        )
        try:
            tmp_cache.write_text(json.dumps(data))
            os.replace(tmp_cache, hairball_index_cache_file)
        finally:
            try:
...
def refresh_hairball_index_in_background() -> bool:
    """Refresh the shared index without blocking a UI request.

    Returns whether a refresh is running.  A process-wide single-flight guard
    prevents repeated Hub polls from starting duplicate 41 MB downloads.
    """
    ...
    threading.Thread(
        target=_refresh,
        name="hairball-skills-index-refresh",
        daemon=True,
    ).start()
    return True


class HairballIndexSource(SkillSource):
    """Skill source backed by the centralized Hairball Skills Index.
    ...
    def __init__(self, auth: GitHubAuth, *, allow_network: bool = True):
    ...
    def _ensure_loaded(self) -> dict:
        if not self._loaded:
            self._index = _load_hairball_index(allow_network=self.allow_network)
            self._loaded = True
    ...
    def fetch(self, identifier: str) -> Optional[SkillBundle]:
        ...
        if resolved:
            ...
            if bundle:
                bundle.source = entry.get("source", "hairball-index")
    ...
    def _to_meta(entry: dict) -> SkillMeta:
        return SkillMeta(
            name=entry.get("name", ""),
            description=entry.get("description", ""),
            source=entry.get("source", "hairball-index"),
            identifier=entry.get("identifier", ""),
...
def create_source_router(
    ...
    *,
    ...
    sources: List[SkillSource] = [
        ...
        HairballIndexSource(
            auth=auth,
            allow_network=allow_index_network,
...
def parallel_search_sources(
    ...
    *on_source_done* is an optional callback ``(source_id, count) -> None``
    ...
    if _effective_filter == "all":
        for src in sources:
            if (src.source_id() == "hairball-index"
