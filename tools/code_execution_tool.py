...
Lets the LLM write a Python script that calls Hairball tools via RPC,
collapsing multi-step tool chains into a single inference turn.

Architecture (two transports):

  **Local backend (UDS):**
  1. Parent generates a `hairball_tools.py` stub module with UDS RPC functions
  ...
  **Remote backends (file-based RPC):**
  1. Parent generates `hairball_tools.py` with file-based RPC stubs
...
from tools.file_tools import SEARCH_FILES_SCHEMA as _SEARCH_FILES_SCHEMA
from tools.thread_context import propagate_context_to_thread

# Availability gate.  On Windows we fall back to loopback TCP for the
# sandbox RPC transport (AF_UNIX is unreliable on Windows Python) — see
# ``_use_tcp_rpc`` in ``_execute_local`` below.  That makes execute_code
# available on every platform Hairball itself runs on.
...
# match a safe prefix, the operational HAIRBALL_ allowlist, or (on Windows) an
# OS-essential name.
#
# NB: the broad "HAIRBALL_" prefix was deliberately removed (#27303) — it leaked
# HAIRBALL_*-named config that lacks a secret substring (e.g. HAIRBALL_BASE_URL,
# HAIRBALL_KANBAN_DB, HAIRBALL_*_WEBHOOK).  The child only needs the few
# location/profile vars in _HAIRBALL_CHILD_ALLOWED below; HAIRBALL_RPC_SOCKET /
# HAIRBALL_RPC_DIR / TZ / HOME are injected explicitly after scrubbing.
...
# Operational HAIRBALL_* vars the child legitimately needs by exact name — these
# are non-secret runtime-location flags (the same set hairball_cli treats as the
# runtime location) that repo-root modules a sandbox script imports may read at
# import time.  None match _SECRET_SUBSTRINGS.
_HAIRBALL_CHILD_ALLOWED = frozenset({
    "HAIRBALL_HOME",
    "HAIRBALL_PROFILE",
    "HAIRBALL_CONFIG",
    "HAIRBALL_ENV",
...
def _scrub_child_env(source_env, is_passthrough=None, is_windows=None):
    ...
    Rules (order matters):
      ...
      4. Operational HAIRBALL_* vars (_HAIRBALL_CHILD_ALLOWED) pass by exact name.
    ...
    scrubbed = {}
    # Non-secret HAIRBALL_* vars dropped by the tightened allowlist (#27303). The
    # broad "HAIRBALL_" prefix used to pass these through; now only the
    # operational set does. The drop is intentional (those vars can carry
    # config like HAIRBALL_KANBAN_DB / HAIRBALL_BASE_URL), but a sandbox script
    ...
    for k, v in source_env.items():
        ...
        if any(k.startswith(p) for p in _SAFE_ENV_PREFIXES):
            scrubbed[k] = v
            continue
        if k in _HAIRBALL_CHILD_ALLOWED:
            scrubbed[k] = v
            continue
        if is_windows and k.upper() in _WINDOWS_ESSENTIAL_ENV_VARS:
            scrubbed[k] = v
            continue
        if k.startswith("HAIRBALL_"):
            # Non-secret (secrets were already dropped above) and not in any
            # allowlist — a deliberately-dropped HAIRBALL_* var.
            _dropped_hairball.append(k)
    if _dropped_hairball:
        logger.debug(
            "execute_code: dropped %d non-allowlisted HAIRBALL_* var(s) from the "
            ...
            len(_dropped_hairball),
            ", ".join(sorted(_dropped_hairball)),
        )
    return scrubbed
...
# ---------------------------------------------------------------------------
# hairball_tools.py code generator
# ---------------------------------------------------------------------------


def _schema_annotation(prop: dict) -> str:
    """Map a JSON-schema property to the small annotation set used in stubs."""
    return {
        "array": "list",
        "boolean": "bool",
        "integer": "int",
        ...
        "object": "dict",
        "string": "str",
    }.get(prop.get("type"), "object")


def _stub_literal(value: Any) -> str:
    """Render deterministic Python literals while keeping strings readable."""
    if isinstance(value, str):
        return json.dumps(value, ensure_ascii=False)
    return repr(value)


def _build_schema_backed_stub(
    schema: dict,
    *,
    allowed_params: tuple[str, ...],
    docstring: str,
) -> tuple[str, str, str, str]:
    """Build a sandbox stub from canonical schema plus an explicit allowlist.

    The schema owns parameter types, defaults, and requiredness.  The allowlist
    owns the security boundary: adding a property to the real tool never
    exposes it inside ``execute_code`` until this policy is deliberately
    updated.
    """
    ...
    unknown = set(allowed_params) - set(properties)
    if unknown:
        raise ValueError(
            f"Sandbox stub policy for {func_name!r} references unknown "
            f"schema parameter(s): {sorted(unknown)}"
        )

    # Required parameters must precede defaulted parameters in Python.
    ordered = [name for name in allowed_params if name in required]
    ordered.extend(name for name in allowed_params if name not in required)

    signature_parts = []
    for name in ordered:
        prop = properties[name]
        annotation = _schema_annotation(prop)
        if name in required:
            signature_parts.append(f"{name}: {annotation}")
        else:
            default = prop.get("default")
            signature_parts.append(
                f"{name}: {annotation} = {_stub_literal(default)}"
            )

    args_expr = "{" + ", ".join(
        f"{json.dumps(name)}: {name}" for name in ordered
    ) + "}"
    return (
        func_name,
...
# Parameter-level policy is separate from the tool-level
# SANDBOX_ALLOWED_TOOLS boundary above.  Keep this explicit: canonical schema
# changes provide facts, not permission to expand the sandbox surface.
_SEARCH_FILES_SANDBOX_PARAMS = (
    "pattern",
    "target",
    "path",
    "file_glob",
...
_SEARCH_FILES_STUB = _build_schema_backed_stub(
    _SEARCH_FILES_SCHEMA,
    allowed_params=_SEARCH_FILES_SANDBOX_PARAMS,
    docstring=(
        '"""Search file contents (target="content") or find files by name '
        '(target="files"). Returns a paginated result dict."""'
    ),
)
...
_TOOL_STUBS = {
    ...
    "read_file": (
        "read_file",
        "path: str, offset: int = 1, limit: int = 500, line_anchors: bool = False",
        '"""Read a file (1-indexed lines). line_anchors=True returns stable anchors usable by patch mode="anchors"."""',
        '{"path": path, "offset": offset, "limit": limit, "line_anchors": line_anchors}',
    ),
    "write_file": (
        "write_file",
        "path: str, content: str, expected_hash: str = None, cross_profile: bool = False",
        '"""Write content to a file (always overwrites). expected_hash is the optional content_hash from read_file and rejects stale overwrites. Returns dict with status. cross_profile=True opts out of the cross-Hairball-profile soft guard."""',
        '{"path": path, "content": content, "expected_hash": expected_hash, "cross_profile": cross_profile}',
    ),
    "search_files": _SEARCH_FILES_STUB,
    "patch": (
        "patch",
        'path: str = None, old_string: str = None, new_string: str = None, replace_all: bool = False, mode: str = "replace", patch: str = None, expected_hash: str = None, expected_hashes: dict = None, cross_profile: bool = False, edits: list = None',
        '"""Targeted replace, V4A multi-file patch, or atomic anchor edits. In one anchor edits batch, copy all anchors from the same read snapshot without shifting later line numbers for earlier operations. expected_hash protects one target; expected_hashes protects V4A sources. cross_profile=True opts out of the cross-Hairball-profile soft guard."""',
        '{"path": path, "old_string": old_string, "new_string": new_string, "replace_all": replace_all, "mode": mode, "patch": patch, "edits": edits, "expected_hash": expected_hash, "expected_hashes": expected_hashes, "cross_profile": cross_profile}',
    ),
    "terminal": (
        "terminal",
...
def generate_hairball_tools_module(enabled_tools: List[str],
                                 transport: str = "uds") -> str:
    """
    Build the source code for the hairball_tools.py stub module.
...
"""Auto-generated Hairball tools RPC stubs."""
...
def _connect():
    ...
    HAIRBALL_RPC_SOCKET can be either:
    ...
    global _sock
    if _sock is None:
        endpoint = os.environ["HAIRBALL_RPC_SOCKET"]
...
def _call(tool_name, args):
    ...
    request = json.dumps({
        "tool": tool_name,
        "args": args,
        "token": os.environ.get("HAIRBALL_RPC_TOKEN", ""),
    }) + "\\n"
    with _call_lock:
        conn = _connect()
...
"""Auto-generated Hairball tools RPC stubs (file-based transport)."""
import json, os, shlex, tempfile, threading, time

_RPC_DIR = os.environ.get("HAIRBALL_RPC_DIR") or os.path.join(tempfile.gettempdir(), "hairball_rpc")
_seq = 0
...
def _rpc_server_loop(
    ...
    try:
        ...
        if conn is None:
            return
        # Poll only for teardown; a timeout is not an idle disconnect.  The
        # generated client intentionally keeps one connection across widely
        # spaced tool calls, while execute_code teardown must still be able to
        # reap this thread after the child exits.  A single 300s recv() leaked
        # the thread (and any dispatcher patched by its caller) across turns.
        conn.settimeout(0.1)
...
def _get_or_create_env(task_id: str):
    ...
    from tools.terminal_tool import (
        ...
        _resolve_container_task_id, _container_config_from_env_config,
...
def _execute_remote(
    ...
    The script and the generated hairball_tools.py module are shipped to
    ...
    sandbox_dir = f"{temp_dir}/hairball_exec_{sandbox_id}"
    ...
    try:
        ...
        py_check = env.execute(
            ...
            cwd="/", timeout=15, track_cwd=False,
        )
        ...
        tools_src = generate_hairball_tools_module(
            list(sandbox_tools), transport="file",
        )
        _ship_file_to_remote(env, f"{sandbox_dir}/hairball_tools.py", tools_src)
        ...
        env_prefix = (
            f"HAIRBALL_RPC_DIR={shlex.quote(f'{sandbox_dir}/rpc')} "
            f"HAIRBALL_RPC_TOKEN={shlex.quote(rpc_token)} "
            f"PYTHONDONTWRITEBYTECODE=1"
        )
        tz = os.getenv("HAIRBALL_TIMEZONE", "").strip()
        if tz:
...
def execute_code(
    ...
    # --- Set up temp directory with hairball_tools.py and script.py ---
    tmpdir = tempfile.mkdtemp(prefix="hairball_sandbox_")
    ...
    # selector from HAIRBALL_RPC_SOCKET (path vs. ``tcp://host:port``).
    ...
    if _use_tcp_rpc:
        ...
        sock_path = os.path.join(_sock_tmpdir, f"hairball_rpc_{uuid.uuid4().hex}.sock")
    ...
    try:
        # Write the auto-generated hairball_tools module.
        ...
        tools_src = generate_hairball_tools_module(list(sandbox_tools))
        with open(os.path.join(tmpdir, "hairball_tools.py"), "w", encoding="utf-8") as f:
            f.write(tools_src)
        ...
        #   connect.  HAIRBALL_RPC_SOCKET is set to ``tcp://127.0.0.1:<port>``
        ...
        child_env["HAIRBALL_RPC_SOCKET"] = rpc_endpoint
        child_env["HAIRBALL_RPC_TOKEN"] = rpc_token
        ...
        # Ensure the hairball-agent root is importable in the sandbox so
        # repo-root modules are available to child scripts.  We also prepend
        # the staging tmpdir so ``from hairball_tools import ...`` resolves even
        # when the subprocess CWD is not tmpdir (project mode).
        _hairball_root = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
        _existing_pp = child_env.get("PYTHONPATH", "")
        _pp_parts = [tmpdir, _hairball_root]
        if _existing_pp:
            _pp_parts.append(_existing_pp)
        child_env["PYTHONPATH"] = os.pathsep.join(_pp_parts)
        # Inject user's configured timezone so datetime.now() in sandboxed
        # code reflects the correct wall-clock time.  Only TZ is set —
        # HAIRBALL_TIMEZONE is an internal Hairball setting and must not leak
        # into child processes.
        _tz_name = os.getenv("HAIRBALL_TIMEZONE", "").strip()
        if _tz_name:
            child_env["TZ"] = _tz_name
        child_env.pop("HAIRBALL_TIMEZONE", None)

        from hairball_constants import apply_subprocess_home_env
        ...
        _child_args = [_child_python, _script_path]
        if (os.getenv("TERMINAL_ENV") or "local").strip().lower() == "local":
            from tools.environments.native_sandbox import wrap_local_process_args
            from tools.terminal_tool import (
                _get_env_config,
                _local_config_from_env_config,
            )

            _child_args = wrap_local_process_args(
                _child_args,
                cwd=_child_cwd,
                config=_local_config_from_env_config(_get_env_config()),
                extra_writable_roots=(tmpdir,),
            )
        ...
        # but scripts can still read secrets from disk (e.g. open('~/.hairball/.env')).
...
def _load_config() -> dict:
    ...
    ``hairball --tui`` where it is never used.  Read the lightweight raw config
...
def _get_execution_mode() -> str:
    ...
    Mode semantics:
      ...
      - ``strict``: scripts run in an isolated temp directory with
        ``sys.executable`` (hairball-agent's python). Reproducible and the
...
# Per-tool documentation details for the execute_code description. Signatures
# come from the exact stub definitions above so prompt help cannot drift from
# the generated ``hairball_tools.py`` module.
_TOOL_DOC_DETAILS = {
    "web_search": (
    ...
    "web_extract": (
        ...
        "    No LLM summarization. Pages over char_limit (default 15000) are "
        "head+tail truncated; full text is stored on disk."
    ),
    "read_file": (
        'Lines are 1-indexed. Returns {"content": "...", "total_lines": N}'
    ),
    "write_file": "Always overwrites the entire file.",
    "search_files": (
        'target: "content" searches inside files; "files" finds files by name. '
        "Pagination metadata includes truncated, next_offset, and _hint."
    ),
    "patch": (
        'mode="replace" performs targeted replacement; mode="patch" applies '
        "a V4A multi-file patch."
    ),
    "terminal": (
...
_TOOL_DOC_LINES = [
    (
        name,
        f"  {_TOOL_STUBS[name][0]}({_TOOL_STUBS[name][1]}) -> dict\n"
        f"    {_TOOL_DOC_DETAILS[name]}",
    )
    for name in _TOOL_DOC_ORDER
]
...
def build_execute_code_schema(enabled_sandbox_tools: set = None,
    ...
    When tools are disabled via ``hairball tools`` (e.g. web is turned off),
    ...
    # temp-dir staging and hairball-agent's own python.
    if mode == "strict":
        cwd_note = (
            "Scripts run in their own temp dir, not the session's CWD — use absolute paths "
            "(os.path.expanduser('~/.hairball/.env')) or terminal()/read_file() for user files."
        )
    else:
        cwd_note = (
    ...
    description = (
        "Run a Python script that can call Hairball tools programmatically. "
        ...
        f"Available via `from hairball_tools import ...`:\n\n"
        f"{tool_lines}\n\n"
        ...
        "Also available (no import needed — built into hairball_tools):\n"
            ...
            "properties": {
                "code": {
                    ...
                    "description": (
                        ...
                        f"`from hairball_tools import {import_str}` "
