...
<url>``) is spawned as a direct child of the Hairball process. Hairball's own
...
Hairball process dies hard — ``kill -9``, an OS-level crash, a force-quit of
...
parent dies, so nothing reaps these until the next Hairball startup's opt-in
