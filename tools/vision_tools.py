...
managed, reference, native Anthropic, or a custom OpenAI-compatible endpoint.
...
def _resolve_download_timeout() -> float:
    env_val = os.getenv("HAIRBALL_VISION_DOWNLOAD_TIMEOUT", "").strip()
    if env_val:
        try:
...
def _resolve_vision_cpu_workers() -> int:
    ...
    Resolution order: HAIRBALL_VISION_MAX_CONCURRENCY env →
    ...
    """
    env_val = os.getenv("HAIRBALL_VISION_MAX_CONCURRENCY", "").strip()
    if env_val:
        try:
            parsed = int(env_val)
            if parsed >= 1:
...
def _normalize_to_supported_image(
    ...
    out_dir = get_hairball_dir("cache/vision", "temp_vision_images")
...
async def _vision_analyze_native(
    ...
    try:
        ...
        temp_dir = get_hairball_dir("cache/vision", "temp_vision_images")
...
async def vision_analyze_tool(
    ...
    Note:
        - For URLs, temporary images are stored under $HAIRBALL_HOME/cache/vision/ and cleaned up
...
if __name__ == "__main__":
    ...
    if not api_available:
        ...
        print("Configure a supported multimodal backend (OpenRouter, managed, reference, Anthropic, or a custom OpenAI-compatible endpoint).")
        sys.exit(1)
    else:
...
async def video_analyze_tool(
    ...
    try:
        ...
        if local_path.is_file():
            ...
            if blocked:
                raise PermissionError(blocked["message"])
            temp_dir = get_hairball_dir("cache/video", "temp_video_files")
