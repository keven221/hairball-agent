...
etc.) surface as specific blocked checks via `hairball computer-use doctor`
...
# fetches the latest release, so a `HAIRBALL_CUA_DRIVER_VERSION` env var would
# only have *looked* like it pinned. For a reproducible version, point
# `HAIRBALL_CUA_DRIVER_CMD` at a specific binary instead.

_CUA_DRIVER_CMD = os.environ.get("HAIRBALL_CUA_DRIVER_CMD", "cua-driver")
...
def _cua_telemetry_disabled() -> bool:
    """True when Hairball should disable cua-driver telemetry for this user.
...
def _resolve_mcp_invocation(
    ...
    *,
    ...
    Surface 8 of managed/hairball-agent#47072: instead of hardcoding
...
def cua_driver_binary_available() -> bool:
    """True if `cua-driver` is on $PATH or HAIRBALL_CUA_DRIVER_CMD resolves."""
...
def cua_driver_update_nudge() -> Optional[str]:
    ...
    return (
        ...
        f"update with `hairball computer-use install --upgrade`."
    )
...
def cua_driver_install_hint() -> str:
    ...
    return (
        ...
        "  hairball computer-use install\n"
        "Or run the upstream installer directly:\n"
        f"{installer}\n"
        "Or run `hairball tools` and enable the Computer Use toolset to install it automatically."
    )
...
def _parse_elements_from_structured(raw_elements: List[Dict[str, Any]]) -> List[UIElement]:
    """Surface 2 of managed/hairball-agent#47072: read the canonical
    ...
    for raw in raw_elements:
        ...
        attributes = {
            ...
            if key not in {
                "element_index", "role", "label", "frame", "element_token",
            }
        }
        elements.append(UIElement(
            index=idx,
...
def _normalize_element_bounds_to_capture(
    elements: List[UIElement], width: int, height: int,
) -> None:
    """Convert driver AX screen-point frames to screenshot-local pixels.

    cua-driver actions that take ``x``/``y`` consume pixels from the exact
    get_window_state screenshot, while macOS AX frames are reported in global
    screen points. The window element is the stable transform anchor on every
    platform: translating by its origin and scaling by screenshot/window size
    makes the bounds returned to the model usable verbatim for pixel fallback,
    element-to-element drag, and coordinate-targeted typing.

    The helper mutates the per-capture UIElement objects only. The original
    frame is retained in ``attributes['screen_bounds']`` for diagnostics.
    """
    ...
    root = next(
        (
            e for e in elements
            if e.role in {"AXWindow", "Window"}
            and e.bounds[2] > 0 and e.bounds[3] > 0
        ),
        None,
    )
    if root is None:
        return
    root_x, root_y, root_w, root_h = root.bounds
    scale_x = float(width) / float(root_w)
    scale_y = float(height) / float(root_h)
    if not (0.1 <= scale_x <= 8.0 and 0.1 <= scale_y <= 8.0):
        return
    for element in elements:
        x, y, w, h = element.bounds
        if w < 0 or h < 0:
            continue
        element.attributes.setdefault("screen_bounds", [x, y, w, h])
        element.bounds = (
            int(round((x - root_x) * scale_x)),
            int(round((y - root_y) * scale_y)),
            int(round(w * scale_x)),
            int(round(h * scale_y)),
        )
...
class _CuaDriverSession:
    ...
    def __init__(self, bridge: _AsyncBridge) -> None:
        ...
        self._started = False
        # Surface 4 of managed/hairball-agent#47072: per-tool
    ...
    async def _lifecycle_coro(self) -> None:
        ...
        try:
            ...
            params = StdioServerParameters(
                ...
                # sanitize Hairball-managed secrets out of the child env.
                env=_sanitize_subprocess_env(cua_driver_child_env()),
            )
    ...
    def _start_lifecycle_locked(self) -> None:
        ...
        if not self._ready_event.wait(timeout=30.0):
            ...
            raise RuntimeError(
                ...
                "Run `hairball computer-use doctor` and check "
                f"{display_hairball_home()}/logs/agent.log for the phase timings."
            )
...
def _extract_tool_result(mcp_result: Any) -> Dict[str, Any]:
    ...
    managed/hairball-agent#47072). Each entry corresponds index-for-index
...
class CuaDriverBackend(ComputerUseBackend):
    ...
    def __init__(self) -> None:
        ...
        # Surface 6 of managed/hairball-agent#47072: per-snapshot
        ...
        #   - Gets a distinct agent-cursor color per Hairball run, with
        ...
        #     concurrent Hairball runs / subagents don't step on each
        #     other.
        # We mint a UUID4-based id once per CuaDriverBackend instance —
        # one Hairball run = one backend = one session — and pass it as
        ...
        self._session_id: str = f"hairball-{uuid.uuid4().hex[:12]}"

    # ── Lifecycle ──────────────────────────────────────────────────
    def start(self) -> None:
        _maybe_nudge_update()
        # The MCP client SDK (`mcp`) is an optional dependency (the
        # `computer-use` / `mcp` extras), not part of Hairball' minimal core.
    ...
    def capture(self, mode: str = "som", app: Optional[str] = None) -> CaptureResult:
        ...
        Maps hairball `capture(mode, app)` → cua-driver `list_windows` +
        `get_window_state` (ax/som) or `screenshot` (vision).
        """
        # Step 1: enumerate on-screen windows to find target pid/window_id.
        # Surface 3 of managed/hairball-agent#47072: read the canonical
        ...
        # Selecting a new capture target invalidates every element handle and
        # coordinate from the previous window immediately. A failed/vision-only
        # capture must never leave an older snapshot actionable.
        ...
        if mode == "vision":
            ...
            # Surface 2 of managed/hairball-agent#47072: prefer the
        ...
        _normalize_element_bounds_to_capture(elements, width, height)
    ...
    def click(
        ...
        *,
        ...
        element_action: Optional[str] = None,
    ) -> ActionResult:
        pid = self._active_pid
        window_id = self._active_window_id
        if pid is None or window_id is None:
        ...
        # managed/hairball-agent#47072). cua-driver-rs gained an explicit
        ...
        if modifiers and tool == "click":
            args["modifier"] = modifiers
        if element_action and tool == "click":
            args["action"] = element_action
        mode_error = self._apply_delivery_mode(tool, args, delivery_mode)
    ...
    def drag(
        ...
        *,
        ...
        if from_element is not None and to_element is not None:
            source = self._snapshot_elements.get(from_element)
            target = self._snapshot_elements.get(to_element)
            if source is None or target is None:
                return ActionResult(ok=False, action="drag",
                                    message="Element drag requires both indices from the latest capture.")
            args["from_x"], args["from_y"] = source.center()
            args["to_x"], args["to_y"] = target.center()
        ...
        if duration_ms is not None:
            args["duration_ms"] = max(0, min(10000, int(duration_ms)))
        if steps is not None:
            args["steps"] = max(1, min(200, int(steps)))
        mode_error = self._apply_delivery_mode("drag", args, delivery_mode)
        if mode_error is not None:
            return mode_error
        ...
        *,
        ...
        if modifiers:
            return ActionResult(
                ok=False,
                action="scroll",
                message="cua-driver scroll does not accept modifier keys.",
            )
        args: Dict[str, Any] = {
            "pid": pid,
            "window_id": window_id,
        ...
        if element is not None:
            ...
            args["x"] = x
            args["y"] = y
        mode_error = self._apply_delivery_mode("scroll", args, delivery_mode)
        if mode_error is not None:
            return mode_error
    ...
    def type_text(
        ...
        *,
        ...
        mode_error = self._apply_delivery_mode("type_text", args, delivery_mode)
        if mode_error is not None:
            return mode_error
    ...
    def key(
        ...
        *,
        ...
        tool = "hotkey" if modifiers else "press_key"
        args: Dict[str, Any] = {"pid": pid, "window_id": window_id}
        if modifiers:
            args["keys"] = modifiers + [key_name]
            # hotkey has a pixel targeting form but no element-index form.
            if element is not None:
                target = self._snapshot_elements.get(element)
                if target is None:
                    return ActionResult(
                        ok=False,
                        action="hotkey",
                        message="Target element is not from the latest capture.",
                    )
                args["x"], args["y"] = target.center()
            elif x is not None and y is not None:
                args["x"], args["y"] = x, y
        else:
            args["key"] = key_name
    ...
    def _apply_delivery_mode(
        ...
        """Attach a requested foreground escalation only when supported.

        Background is cua-driver's default and stays omitted for backwards
        compatibility. Foreground is never guessed: the model requests it
        after a verified background miss, and an older driver that lacks the
        capability gets a truthful error instead of a fake accepted parameter.
        """
        mode = (delivery_mode or "background").strip().lower()
        if mode not in {"background", "foreground"}:
            return ActionResult(
                ok=False,
                action=tool,
                message="delivery_mode must be background or foreground.",
            )
        if mode == "foreground":
            if not self._session.supports_capability("input.delivery_mode", tool=tool):
                return ActionResult(
                    ok=False,
                    action=tool,
                    message=(
                        "Installed cua-driver does not advertise foreground "
                        f"delivery for {tool}; update the driver and retry."
                    ),
                )
