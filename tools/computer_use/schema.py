    ...
    "parameters": {
        ...
        "properties": {
            ...
            "element": {
                "type": "integer",
                "description": (
                    "The element index returned by the latest capture for "
                    "the target window. Use it with click, type, key, scroll, "
                    "or set_value. Hairball binds it to cua-driver's opaque "
                    "snapshot token internally so stale captures fail closed."
                ),
            },
            "coordinate": {
                "type": "array",
                ...
                "maxItems": 2,
                "description": (
                    "Window-local screenshot pixels [x, y], in the exact "
                    "coordinate frame returned by capture. Use for canvas, "
                    "video, WebGL, custom-drawn controls, or a web input whose "
                    "AX element action did not land."
                ),
            },
            "button": {
                "type": "string",
            ...
            "delivery_mode": {
                ...
                "description": (
                    "Action delivery ladder. `background` is the default and "
                    "does not raise or steal focus. Use `foreground` only "
                    "after a background action was re-captured and proven not "
                    "to have landed; the driver briefly fronts the exact "
                    "window, acts, then restores the previous app."
                ),
            },
            "element_action": {
                "type": "string",
                ...
                "description": (
                    "Optional native AX action for action='click'. Omit for "
                    "the normal `press`; otherwise use only an action "
                    "advertised by the captured element."
                ),
            },
            # ── drag ───────────────────────────────────────────────
            ...
            "duration_ms": {
                ...
                "minimum": 0,
                "maximum": 10000,
                "description": "Drag duration in milliseconds. Driver default: 500.",
            },
            "steps": {
                "type": "integer",
                "minimum": 1,
                "maximum": 200,
                "description": "Intermediate drag events. Driver default: 20.",
            },
            # ── scroll ─────────────────────────────────────────────
            "direction": {
                "type": "string",
            ...
            "scroll_by": {
                "type": "string",
                "enum": ["line", "page"],
                "description": "Scroll granularity. Defaults to line.",
            },
            # ── set_value ──────────────────────────────────────────
            "value": {
                "type": "string",
            ...
            "text": {
                "type": "string",
                "description": (
                    "Text to type. Prefer passing `element` so native fields "
                    "are targeted directly without a separate click. For a "
                    "web or custom input, pass `coordinate` to pixel-focus "
                    "and type in one driver call."
                ),
            },
            "delay_ms": {
                "type": "integer",
                "minimum": 0,
                "maximum": 200,
                "description": "Delay per character for synthesized-key fallback.",
            },
            "keys": {
                "type": "string",
                "description": (
