...
`health_report` (surfaced by `hairball computer-use doctor`) reports the
...
# regardless of approval level (e.g. logout kills the session Hairball runs in).
_BLOCKED_KEY_COMBOS = {
...
def _get_backend() -> ComputerUseBackend:
    ...
    with _backend_lock:
        if _backend is None:
            backend_name = os.environ.get("HAIRBALL_COMPUTER_USE_BACKEND", "cua").lower()
            if backend_name in {"cua", "cua-driver", ""}:
                ...
                raise RuntimeError(f"Unknown HAIRBALL_COMPUTER_USE_BACKEND={backend_name!r}")
            try:
                _backend.start()
            except Exception:
...
def handle_computer_use(args: Dict[str, Any], **kwargs) -> Any:
    ...
    try:
        ...
        return json.dumps({
            ...
            "hint": "If the cua-driver binary is missing, run `hairball computer-use install`. "
...
def _dispatch(backend: ComputerUseBackend, action: str, args: Dict[str, Any]) -> Any:
    ...
    delivery_mode = str(args.get("delivery_mode") or "background").lower()
    if delivery_mode not in {"background", "foreground"}:
        return json.dumps({
            "error": "bad delivery_mode; use background|foreground",
        })
    ...
    if action in {"click", "double_click", "right_click", "middle_click"}:
        ...
        res = backend.click(
            ...
            element_action=args.get("element_action"),
        )
        return _maybe_follow_capture(backend, res, capture_after)
    ...
    if action == "type":
        ...
        if args.get("element") is not None:
            target_kwargs["element"] = args["element"]
        if coord and coord[0] is not None:
            target_kwargs["x"], target_kwargs["y"] = coord[0], coord[1]
        if args.get("delay_ms") is not None:
            target_kwargs["delay_ms"] = args["delay_ms"]
        if "delivery_mode" in args:
            target_kwargs["delivery_mode"] = delivery_mode
        res = backend.type_text(args.get("text", ""), **target_kwargs)
        return _maybe_follow_capture(backend, res, capture_after)
    ...
    if action == "key":
        ...
        if "delivery_mode" in args:
            target_kwargs["delivery_mode"] = delivery_mode
        res = backend.key(args.get("keys", ""), **target_kwargs)
        return _maybe_follow_capture(backend, res, capture_after)
...
def _capture_response(cap: CaptureResult, max_elements: int = _DEFAULT_MAX_ELEMENTS) -> Any:
    ...
    if cap.png_b64 and cap.mode != "ax" and not image_too_small:
        ...
        # parts (Surface 7 of managed/hairball-agent#47072 — trycua/cua#1961
...
def _route_capture_through_aux_vision(
    ...
    The captured base64 PNG is materialised to ``$HAIRBALL_HOME/cache/vision/``
    ...
    try:
        ...
        if _mime_for_ext == "image/jpeg" or (not _mime_for_ext and cap.png_b64[:8].startswith("/9j/")):
            ...
            ext = ".png"
        cache_dir = get_hairball_dir("cache/vision", "temp_vision_images")
    ...
    if isinstance(result_json, str):
        try:
            ...
            if isinstance(parsed, dict):
                # vision_analyze_tool returns a human-readable `analysis`
                # even on provider/auth failures. Treating that text as a
                # successful visual observation made a 401 look like the
                # screenshot had been understood. Only a positively marked
                # success may become `vision_analysis`; every other shape
                # follows the honest vision_unavailable fallback above.
...
def _format_elements(elements: List[UIElement], max_lines: int = 40) -> List[str]:
    ...
    for e in elements[:max_lines]:
        ...
        value_hint = f" value={str(value)[:60]!r}" if value not in (None, "", e.label) else ""
        out.append(f"  #{e.index} {e.role} {label!r}{value_hint} @ {e.bounds}"
...
def _element_to_dict(e: UIElement) -> Dict[str, Any]:
    ...
    for key in ("value", "enabled", "selected", "parent_index", "depth", "actions"):
        if key in e.attributes:
            payload[key] = e.attributes[key]
    return payload
...
def check_computer_use_requirements() -> bool:
    ...
    `hairball computer-use doctor` if their session is incomplete (e.g. no
    DISPLAY set).
    """
