"""
Lazy dependency installer for opt-in Hairball Agent backends.

Many Hairball features (Mistral TTS, ElevenLabs TTS, Honcho memory, Bedrock,
...
(``hairball-agent[all]``) and install them eagerly at setup time. That has
two problems:
...
remediation hint pointing at ``hairball tools`` or the manual pip command.
...
* **Durable-target mode (immutable images).** When the deployment seals the
  agent's own venv (the Docker image sets ``HAIRBALL_DISABLE_LAZY_INSTALLS=1``
  and makes ``/opt/hairball`` read-only), setting
  ``HAIRBALL_LAZY_INSTALL_TARGET`` redirects lazy installs to a writable
  ...
  installed package cannot brick Hairball, which is what made it safe to seal
...
LAZY_DEPS: dict[str, tuple[str, ...]] = {
    ...
    # (HAIRBALL_DISABLE_LAZY_INSTALLS=1) and lazy installs are redirected to the
    ...
    "platform.discord": (
        ...
        "aiohttp==3.14.3",  # Patched security floor; keep in sync with pyproject messaging extras.
    ),
    "platform.slack": (
        "slack-bolt==1.29.0",
        "slack-sdk==3.43.0",
    ...
    # Dashboard (`hairball dashboard`)
    "tool.dashboard": (
        "fastapi==0.141.1",
        "uvicorn[standard]==0.41.0",
        "starlette==1.6.0",  # Security floor; keep lazy-install in sync with pyproject [web].
        "python-multipart==0.0.32",  # FastAPI UploadFile/Form; mirror pyproject [web].
    ),
    ...
    "tool.computer_use": (
        "mcp==1.29.0",
        "starlette==1.6.0",  # Security floor; mirror pyproject [computer-use].
    ),
    # HF Agent Trace Viewer upload (hairball trace upload / /upload-trace).
...
_LAZY_TARGET_ENV = "HAIRBALL_LAZY_INSTALL_TARGET"
...
def _allow_lazy_installs() -> bool:
    ...
    2. ``HAIRBALL_DISABLE_LAZY_INSTALLS=1`` seals the *agent venv* (set by the
    ...
    if os.environ.get("HAIRBALL_DISABLE_LAZY_INSTALLS") == "1":
...
def _unsupported_feature_reason(feature: str) -> Optional[str]:
    ...
    ``hairball update`` lazy-refresh pass.
    """
    if sys.platform == "win32" and feature == "platform.matrix":
        return (
            ...
            "from sdist. Run Hairball under WSL to use Matrix on Windows."
        )
    return None
...
def _is_satisfied(spec: str) -> bool:
    ...
    ``hairball update`` propagate pin bumps in :data:`LAZY_DEPS` to already-
...
def _core_constraints_file() -> Optional[Path]:
    ...
    * A backend that *requires* a version conflicting with core fails loudly
    ...
    try:
        ...
        if not lines:
            return None
        fd, path = tempfile.mkstemp(prefix="hairball-core-constraints-", suffix=".txt")
...
def _venv_pip_install(specs: tuple[str, ...], *, timeout: int = 300) -> _InstallResult:
    ...
    * **Durable-target.** When :data:`_LAZY_TARGET_ENV` is set, installs into
    ...
    Mirrors the strategy in ``hairball_cli.tools_config._pip_install`` but
    ...
    try:
        ...
        uv_env = hairball_subprocess_env(inherit_credentials=False)
...
def is_usable_or_installable(feature: str) -> bool:
    """Return whether ``ensure(feature)`` can succeed, without installing.

    Tool-schema availability checks run before a model chooses a tool. They
    must be side-effect free: dependency installation belongs to the actual
    invocation path. This mirrors :func:`ensure`'s allowlist, platform, safety,
    and user-policy gates without executing pip.
    """
    if feature not in LAZY_DEPS:
        return False
    ...
    if _unsupported_feature_reason(feature):
        return False
    if any(not _spec_is_safe(spec) for spec in feature_specs(feature)):
        return False
...
def active_features() -> list[str]:
    ...
    Used by ``hairball update`` to figure out which lazy backends need a
...
def refresh_active_features(*, prompt: bool = False) -> dict[str, str]:
    ...
    Intended for ``hairball update``. Never raises; lazy-install failures
