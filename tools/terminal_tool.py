...
- Persistent filesystems do NOT guarantee the same live sandbox or long-running processes survive cleanup, idle reaping, or Hairball exit
...
# display_hairball_home imported lazily at call site (stale-module safety during hairball update)
...
def _check_disk_usage_warning():
    ...
    try:
        ...
        # Get total size of hairball directories
        total_bytes = 0
        import glob
        for path in glob.glob(str(scratch_dir / "hairball-*")):
...
def _get_sudo_password_cache_scope() -> str:
    ...
    try:
        ...
        session_key = get_session_env("HAIRBALL_SESSION_KEY", "")
    except Exception:
        session_key = os.getenv("HAIRBALL_SESSION_KEY", "")
    if session_key:
...
def _handle_sudo_failure(output: str, env_type: str) -> str:
    ...
    """
    is_gateway = env_var_enabled("HAIRBALL_GATEWAY_SESSION")
...
def _prompt_for_sudo_password(timeout_seconds: int = 45) -> str:
    ...
    Only works in interactive mode (HAIRBALL_INTERACTIVE=1).
    ...
    try:
        os.environ["HAIRBALL_SPINNER_PAUSE"] = "1"
        time.sleep(0.2)
        ...
        if "HAIRBALL_SPINNER_PAUSE" in os.environ:
            del os.environ["HAIRBALL_SPINNER_PAUSE"]
...
def _transform_sudo_command(command: str | None) -> tuple[str | None, str | None]:
    ...
    (HAIRBALL_INTERACTIVE=1 or a registered sudo password callback):
    ...
    # interactive Hairball password prompt or the sudo -S password-pipe path.
...
For servers/watchers, do NOT use shell-level background wrappers (nohup/disown/setsid/trailing '&') in foreground mode. Use background=true so Hairball can track lifecycle and output.
After starting a server, verify readiness with a health check or log signal, then run tests in a separate terminal() call. Avoid blind sleep loops.
Use process(action="poll") for progress checks, process(action="wait") to block until done.
Working directory: Use 'workdir' for per-command cwd.
PTY mode: Set pty=true for interactive CLI tools (reference, Claude Code, Python REPL).
...
def _maybe_reap_docker_orphans(container_config: Dict[str, Any]) -> None:
    ...
    Sweeps long-Exited containers labeled ``hairball-agent=1`` for the current
    profile that match the issue #20561 leak class — containers left behind
    by Hairball processes that exited without firing ``atexit`` (SIGKILL,
    ...
    * ``terminal.docker_orphan_reaper: false`` disables it entirely (the
      ...
      Hairball processes in the same profile and don't trust the
    ...
    * ``_docker_orphan_reaper_ran`` flag — sweep runs once per Python
    ...
    # 2 × lifetime_seconds gives sibling Hairball processes a generous grace
...
def _resolve_container_task_id(task_id: Optional[str]) -> str:
    ...
    Exception: RL / benchmark environments (TerminalBench2, HairballSweEnv, ...)
...
def _parse_env_var(name: str, default: str, converter: Any = int, type_label: str = "integer"):
    ...
    try:
        ...
        raise ValueError(
            ...
            f"Check ~/.hairball/.env or environment variables."
        )
...
def _is_ssh_remote_tilde_cwd(backend: str, cwd: str) -> bool:
    ...
    expand itself, so the Hairball host/container must NOT ``expanduser`` it.

    SSH ``cwd`` is interpreted by the *remote* shell (``cd ~`` / ``cd ~/x``
    over ``ssh ... bash -c``). Expanding ``~`` locally would rewrite it to the
    Hairball host HOME (often ``/opt/data`` under Docker) and inject a
...
def _get_env_config() -> Dict[str, Any]:
    ...
    if env_type == "local":
        local_writable_roots = _parse_env_var(
            "TERMINAL_LOCAL_WRITABLE_ROOTS", "[]", json.loads, "valid JSON"
        )
        if not isinstance(local_writable_roots, list):
            logger.warning(
                "Ignoring TERMINAL_LOCAL_WRITABLE_ROOTS because it is not a JSON list"
            )
    ...
    return {
        ...
        "local_network": os.getenv(
            "TERMINAL_LOCAL_NETWORK", "true"
        ).lower() in {"true", "1", "yes"},
        "local_writable_roots": local_writable_roots,
        ...
        # Startup orphan reaper for hairball-tagged containers left behind by
...
def _container_config_from_env_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """Project canonical terminal config into the container factory contract.

    Environment creation is reachable through terminal, file tools, prompt
    probing, and execute_code. Keeping this projection in one place prevents a
    first-called surface from silently dropping accepted Docker/Modal options
    and then publishing that incomplete environment through the shared cache.
    """
    return {
...
def _local_config_from_env_config(config: Dict[str, Any]) -> Dict[str, Any]:
    """Project canonical terminal config into the local factory contract."""

    return {
        "persistent": config.get("local_persistent", False),
        "network": config.get("local_network", True),
        "writable_roots": list(config.get("local_writable_roots", []) or []),
    }
...
def _background_backend_id(env_type: str, env: Any) -> str:
    """Return a stable, non-secret identity for one execution backend."""
    if env_type == "docker":
        return str(getattr(env, "_container_id", "") or "")
    if env_type == "singularity":
        return str(getattr(env, "instance_id", "") or "")
    if env_type == "modal":
        managed_id = getattr(env, "_sandbox_id", "")
        if managed_id:
            return str(managed_id)
        sandbox = getattr(env, "_sandbox", None)
        return str(
            getattr(sandbox, "object_id", "")
    ...
    if env_type == "ssh":
        return f"{getattr(env, 'user', '')}@{getattr(env, 'host', '')}:{getattr(env, 'port', 22)}"
    if env_type == "local":
        return f"host:{platform.node()}"
    return ""


def _bind_background_reconnect_spec(
    env: Any,
    *,
    env_type: str,
    image: str,
    ...
    """Attach the safe descriptor persisted by ``ProcessRegistry``."""
    setter = getattr(env, "set_background_reconnect_spec", None)
    if callable(setter):
        setter({
            "schema": 1,
            "env_type": env_type,
            "image": image if env_type in {"docker", "singularity", "modal", "daytona"} else "",
            "cwd": cwd,
            "timeout": int(timeout),
            "task_id": task_id,
            "backend_id": _background_backend_id(env_type, env),
...
def _create_environment(env_type: str, image: str, cwd: str, timeout: int,
    ...
    reconnect = dict(reconnect_spec or {})

    def _finish(env: Any, *, modal_backend: str = "") -> Any:
        return _bind_background_reconnect_spec(
            env,
            env_type=env_type,
            image=image,
            cwd=cwd,
    ...
    if env_type == "local":
        resolved_local_config = local_config
        if resolved_local_config is None:
            resolved_local_config = _local_config_from_env_config(_get_env_config())
        return _finish(_LocalEnvironment(
            cwd=cwd,
            timeout=timeout,
        ...
        # prior Hairball processes that hit SIGKILL / OOM / a closed terminal
        ...
        singularity_reconnect = (
            {"reconnect_instance_id": reconnect.get("backend_id", "")}
            if reconnect else {}
        )
        ...
        if modal_state["selected_backend"] == "managed":
            if reconnect and reconnect.get("modal_backend") not in {"", "managed"}:
                raise RuntimeError("Modal reconnect backend changed from direct to managed")
            managed_reconnect = (
                {"reconnect_sandbox_id": reconnect.get("backend_id", "")}
                if reconnect else {}
            )
        ...
        if modal_state["selected_backend"] != "direct":
            ...
            if managed_portal_tools_enabled():
                message = (
                    "Modal backend selected but no direct Modal credentials/config or managed tool gateway was found."
                )
            raise ValueError(message)

        if reconnect and reconnect.get("modal_backend") not in {"", "direct"}:
            raise RuntimeError("Modal reconnect backend changed from managed to direct")
        direct_reconnect = (
            {"reconnect_sandbox_id": reconnect.get("backend_id", "")}
            if reconnect else {}
        )
...
def reconnect_environment(spec: dict):
    """Re-open the exact backend named by a persisted background descriptor.

    Credentials and policy are always resolved from the current profile; the
    descriptor contains only backend identity.  A mismatch fails closed so a
    stale PID can never be adopted inside a newly-created sandbox.
    """
    spec = dict(spec or {})
    env_type = str(spec.get("env_type") or "")
    if env_type not in {"docker", "singularity", "modal", "daytona", "ssh"}:
        raise RuntimeError(f"Unsupported background reconnect backend: {env_type or 'missing'}")

    task_id = str(spec.get("task_id") or "default")
    with _env_lock:
        cached = _active_environments.get(task_id)
    if cached is not None:
        actual = _background_backend_id(env_type, cached)
        expected = str(spec.get("backend_id") or "")
        if expected and actual != expected:
            raise RuntimeError(
                f"Cached {env_type} backend identity changed; refusing background reconnect"
            )
        return cached
    ...
    container_config = (
        _container_config_from_env_config(config)
        if env_type in {"docker", "singularity", "modal", "daytona"}
        else None
    )
    image = str(spec.get("image") or config.get(f"{env_type}_image", ""))
    cwd = str(spec.get("cwd") or config.get("cwd") or "/root")
    timeout = int(spec.get("timeout") or config.get("timeout") or 180)
    ...
    expected = str(spec.get("backend_id") or "")
    actual = _background_backend_id(env_type, env)
    if not expected or actual != expected:
        try:
            ...
            raise RuntimeError(
                f"{env_type} backend identity changed; refusing to adopt a stale remote PID"
            )
...
def _foreground_background_guidance(command: str) -> str | None:
    ...
    if _SHELL_LEVEL_BACKGROUND_RE.search(unquoted):
        return (
            ...
            "Use terminal(background=true) so Hairball can track the process, then run "
...
def terminal_tool(
    ...
    """
    try:
        from agent.plan_execution import plan_terminal_read_only

        plan_read_only = plan_terminal_read_only()
        ...
        if plan_read_only:
            if background or pty:
                return json.dumps({
                    ...
                    "exit_code": -1,
                    "error": (
                        "Plan mode terminal commands must be foreground and "
                        "non-interactive."
                    ),
                    "status": "blocked",
                    "code": "plan_persistent_process_denied",
                }, ensure_ascii=False)
            from tools.environments.native_sandbox import native_sandbox_available

            if env_type != "local" or not native_sandbox_available():
                return json.dumps({
                    "output": "",
                    "exit_code": -1,
                    "error": (
                        "Plan mode terminal access requires Hairball's native "
                        "read-only local sandbox on this host. Use read_file, "
                        "search_files, LSP, or an explore subagent instead."
                    ),
                    "status": "blocked",
                    "code": "plan_read_only_sandbox_unavailable",
                }, ensure_ascii=False)
        ...
        # Hard-block: gateway lifecycle commands (systemctl/launchctl/hairball
        # restart|stop targeting hairball-gateway) must never run inside the
        ...
        # never restart. This mirrors the `hairball gateway restart` guard in
        # hairball_cli/gateway.py and the cron-path guard in hairball_cli/cron.py,
        # but applies unconditionally (force=True cannot help here).
        if os.environ.get("_HAIRBALL_GATEWAY") == "1":
            from hairball_cli.cron import _contains_gateway_lifecycle_command
            if _contains_gateway_lifecycle_command(command):
                return json.dumps({
                    ...
                    "error": (
                        ...
                        "Run `hairball gateway restart` from a separate shell outside "
                        "the running gateway."
                    ),
                    "status": "error",
                }, ensure_ascii=False)
        ...
        if background:
            ...
            try:
                ...
                idempotency_key = (
                    f"terminal-background:{session_key}:{tool_call_id}"
                ...
                if env_type == "local":
                    ...
                    if callable(getattr(env, "build_process_args", None)):
                        local_spawn_kwargs["local_environment"] = env
                ...
                # hairball-agent dev work. May 2026 PRs that surfaced this
                ...
                if background and command:
                    ...
                    if _bad_shape:
                        ...
                        canonical_hint = (
                            ...
                            "repeatedly in hairball-agent dev work (PRs #31329, "
                            ...
                            "name='github/hairball-agent-dev', "
                ...
                if background and (notify_on_complete or watch_patterns):
                    ...
                    if not _async_ok():
                        ...
                        _gw_platform = _gse("HAIRBALL_SESSION_PLATFORM", "")
                        if _gw_platform:
                            _gw_chat_id = _gse("HAIRBALL_SESSION_CHAT_ID", "")
                            _gw_thread_id = _gse("HAIRBALL_SESSION_THREAD_ID", "")
                            _gw_user_id = _gse("HAIRBALL_SESSION_USER_ID", "")
                            _gw_user_name = _gse("HAIRBALL_SESSION_USER_NAME", "")
                            _gw_message_id = _gse("HAIRBALL_SESSION_MESSAGE_ID", "")
            ...
            while retry_count <= max_retries:
                try:
                    ...
                    if plan_read_only:
                        result = env.execute_read_only(command, **execute_kwargs)
                    else:
            ...
            raw_output_path = str(result.get("raw_output_path") or "")
            raw_output_artifact = str(result.get("raw_output_artifact") or "")
            raw_output_chars = result.get("raw_output_chars")
            ...
            if sudo_cache_cleared:
                has_sudo_prompt_callback = _get_sudo_password_callback() is not None
                if has_sudo_prompt_callback or env_var_enabled("HAIRBALL_INTERACTIVE"):
                    output += (
            ...
            # Truncate the FINAL human-readable output if too long, keeping
            # both head and tail. The raw-output hint consumes the same budget:
            # truncating the body to MAX and appending the hint afterwards
            # silently exceeded ``tool_output.max_bytes`` on every large
            # capture. Structured ``raw_output_*`` fields below remain the
            # authoritative full references even when an extremely small
            # budget cannot fit the inline path.
            from tools.tool_output_limits import get_max_bytes
            MAX_OUTPUT_CHARS = get_max_bytes()
            raw_output_suffix = ""
            if raw_output_path and raw_output_artifact:
                full_suffix = (
                    f"\n\n[raw output: {raw_output_artifact}]\n"
                    f"Full output saved to: {raw_output_path}"
                )
                artifact_only_suffix = f"\n\n[raw output: {raw_output_artifact}]"
                if len(full_suffix) < MAX_OUTPUT_CHARS:
                    raw_output_suffix = full_suffix
                elif len(artifact_only_suffix) < MAX_OUTPUT_CHARS:
                    raw_output_suffix = artifact_only_suffix

            body_budget = max(0, MAX_OUTPUT_CHARS - len(raw_output_suffix))
            if len(output) > body_budget:
                total_chars = len(output)
                # Notice length depends on the omitted count. Recompute until
                # its digit width stabilizes, then divide the remaining body
                # budget 40/60 so early errors and recent output both survive.
                omitted = max(0, total_chars - body_budget)
                truncated_notice = ""
                for _ in range(3):
                    truncated_notice = (
                        f"\n\n... [OUTPUT TRUNCATED - {omitted} chars omitted "
                        f"out of {total_chars} total] ...\n\n"
                    )
                    content_budget = max(0, body_budget - len(truncated_notice))
                    new_omitted = max(0, total_chars - content_budget)
                    if new_omitted == omitted:
                        break
                    omitted = new_omitted
                if body_budget >= len(truncated_notice):
                    content_budget = body_budget - len(truncated_notice)
                    head_chars = int(content_budget * 0.4)
                    tail_chars = content_budget - head_chars
                    tail = output[-tail_chars:] if tail_chars else ""
                    output = output[:head_chars] + truncated_notice + tail
                else:
                    output = output[:body_budget]
            output = output.rstrip() + raw_output_suffix
            ...
            if env_type == "local":
                try:
                    from tools.environments.native_sandbox import (
                        is_likely_native_sandbox_denial,
                    )

                    if is_likely_native_sandbox_denial(returncode, output):
                        result_dict["status"] = "sandbox_denied"
                        result_dict["sandbox_denied"] = True
                        result_dict["hint"] = (
                            "The command reached a path or operation outside the local "
                            "workspace sandbox. Keep the operation inside the active "
                            "workspace/temp directory, or ask the user to add an explicit "
                            "terminal.local_writable_roots entry for the required path."
                        )
                except Exception:
                    logger.debug("native sandbox denial classification skipped", exc_info=True)
            if raw_output_path:
                result_dict["raw_output_path"] = raw_output_path
            if raw_output_artifact:
                result_dict["raw_output_artifact"] = raw_output_artifact
            if raw_output_chars is not None:
                result_dict["raw_output_chars"] = raw_output_chars
            try:
...
TERMINAL_SCHEMA = {
    ...
    "parameters": {
        ...
        "properties": {
            ...
            "pty": {
                "type": "boolean",
                "description": "Run in pseudo-terminal (PTY) mode for interactive CLI tools like reference, Claude Code, or Python REPL. Only works with local and SSH backends. Default: false.",
                "default": False
            },
            "notify_on_complete": {
                "type": "boolean",
