"""Windows UTF-8 bootstrap for Hairball entry points.
...
should be imported at the very top of every Hairball entry point
(``hairball``, ``hairball-agent``, ``hairball-acp``, ``python -m gateway.run``,
...
def harden_import_path(src_root: str | None = None) -> None:
    """Stop a package in the current directory from shadowing Hairball modules.

    Hairball ships top-level modules with common names (``utils``, ``proxy``,
    ...
    We drop the relative forms outright, then force the real Hairball source root
    ...
    """
    root = src_root or os.environ.get("HAIRBALL_PYTHON_SRC_ROOT") or os.path.dirname(
...
def activate_durable_lazy_target() -> None:
    ...
    (``HAIRBALL_LAZY_INSTALL_TARGET``, e.g. ``/opt/data/lazy-packages``).
    ...
    """
    if not os.environ.get("HAIRBALL_LAZY_INSTALL_TARGET", "").strip():
        return
    try:
...
# Apply on import — entry points just need ``import hairball_bootstrap``
# (or ``from hairball_bootstrap import apply_windows_utf8_bootstrap``) at
