#!/usr/bin/env python3
"""
SQLite State Store for Hairball Agent.
...
- WAL mode for concurrent readers + one writer when the SQLite runtime is safe;
  rollback-journal mode on runtimes affected by SQLite's WAL-reset corruption bug
...
def _delete_completion_contracts_for_sessions(conn, session_ids: List[str]) -> None:
    """Apply hard-delete retention to the opt-in completion audit tables.

    These tables are deliberately absent until the experiment is enabled, so
    every ordinary session delete must remain a no-op when the migration has
    never run.  Child rows are removed explicitly rather than relying on the
    connection's foreign-key pragma, keeping cleanup deterministic for older
    databases and import/test connections too.
    """
    ids = list(dict.fromkeys(str(session_id) for session_id in session_ids if session_id))
    if not ids:
        return
    table = conn.execute(
        "SELECT 1 FROM sqlite_master WHERE type = 'table' AND name = 'completion_contracts'"
    ).fetchone()
    if table is None:
        return
    placeholders = ",".join("?" * len(ids))
    rows = conn.execute(
        f"SELECT contract_id FROM completion_contracts WHERE session_id IN ({placeholders})",
        ids,
    ).fetchall()
    contract_ids = [str(row["contract_id"]) for row in rows]
    if not contract_ids:
        return
    contract_placeholders = ",".join("?" * len(contract_ids))
    conn.execute(
        f"DELETE FROM completion_assessments WHERE contract_id IN ({contract_placeholders})",
        contract_ids,
    )
    conn.execute(
        f"DELETE FROM completion_evidence WHERE contract_id IN ({contract_placeholders})",
        contract_ids,
    )
    conn.execute(
        f"DELETE FROM completion_contracts WHERE contract_id IN ({contract_placeholders})",
        contract_ids,
    )
...
def _delete_delegate_children(conn, parent_ids: List[str]) -> List[str]:
    ...
    if ids:
        ph = ",".join("?" * len(ids))
        _delete_completion_contracts_for_sessions(conn, ids)
...
DEFAULT_DB_PATH = get_hairball_home() / "state.db"
...
def _sqlite_wal_reset_bug_is_fixed(
    version: Optional[Tuple[int, int, int]] = None,
) -> bool:
    """Return whether *version* contains SQLite's WAL-reset corruption fix.

    The upstream fix first shipped in 3.51.3 and was backported to the 3.50
    and 3.44 release lines as 3.50.7 and 3.44.6. Other older release lines
    remain affected. See https://www.sqlite.org/wal.html#the_wal_reset_bug.
    """
    current = version or sqlite3.sqlite_version_info
    normalized = tuple(current[:3])
    if normalized >= (3, 51, 3):
        return True
    if normalized[:2] == (3, 50):
        return normalized >= (3, 50, 7)
    if normalized[:2] == (3, 44):
        return normalized >= (3, 44, 6)
    return False
...
# hairball_cli/kanban_db.py for ~30 call sites) would re-log the same
...
def _enforce_macos_synchromanaged_full(conn: sqlite3.Connection) -> None:
...
def apply_wal_with_fallback(
    ...
    *,
    ...
    """Use WAL when it is safe, otherwise use rollback-journal DELETE mode.

    Returns the journal mode actually set (``"wal"`` or ``"delete"``).

    SQLite versions before 3.51.3 are affected by the WAL-reset corruption
    race, except for the 3.50.7 and 3.44.6 backports. On an affected runtime
    we deliberately use DELETE mode. This preserves correctness for Hairball's
    multi-process databases at the cost of serializing readers during writes.
    ...
    Shared by :class:`SessionDB` and ``hairball_cli.kanban_db.connect`` so
    both databases get identical fallback behavior.

    Never performs a filesystem-error fallback from an already-WAL database.
    The vulnerable-runtime branch is different: SQLite itself performs an
    exclusive, atomic journal-mode transition, or raises if active connections
    prevent it.
    """
    if not _sqlite_wal_reset_bug_is_fixed():
        ...
        if mode != "delete":
            raise sqlite3.OperationalError(
                "SQLite runtime "
                f"{sqlite3.sqlite_version} is affected by the WAL-reset "
                "corruption bug and the database could not be switched to "
                "journal_mode=DELETE; stop other Hairball processes or upgrade "
                "SQLite to 3.51.3+, 3.50.7, or 3.44.6"
            )
        _log_wal_reset_workaround_once(db_label)
        return "delete"
    ...
    try:
        ...
        if current_mode and current_mode[0] == "wal":
            ...
            _enforce_macos_synchromanaged_full(conn)
            return "wal"
    except sqlite3.OperationalError:
        pass
...
def _log_wal_fallback_once(db_label: str, exc: Exception) -> None:
    ...
    connection on every operation — see hairball_cli/kanban_db.py) would
...
def _log_wal_reset_workaround_once(db_label: str) -> None:
    """Log the vulnerable-runtime DELETE fallback once per DB label."""
    warning_key = f"wal-reset:{db_label}"
    with _wal_fallback_warned_lock:
        if warning_key in _wal_fallback_warned_paths:
            return
        _wal_fallback_warned_paths.add(warning_key)
    logger.warning(
        "%s: SQLite %s is affected by the WAL-reset corruption bug; "
        "using journal_mode=DELETE for correctness. Upgrade SQLite to "
        "3.51.3+, 3.50.7, or 3.44.6 to restore WAL concurrency. See "
...
def _db_opens_cleanly(db_path: Path) -> Optional[str]:
    ...
    try:
        ...
        probe_session_id = f"_hairball_fts_health_probe_{time.time_ns()}"
        try:
...
CREATE TABLE IF NOT EXISTS sessions (
    ...
    cost_actual_call_count INTEGER NOT NULL DEFAULT 0,
    cost_total_call_count INTEGER NOT NULL DEFAULT 0,
    cost_status TEXT,
    cost_source TEXT,
...
CREATE TABLE IF NOT EXISTS workflow_runs (
    ...
    real_side_effect INTEGER NOT NULL DEFAULT 0,
    ...
    lease_expires_at REAL,
    heartbeat_at REAL,
    mode TEXT NOT NULL DEFAULT 'hairball_runner',
...
CREATE UNIQUE INDEX IF NOT EXISTS idx_workflow_runs_idempotency
    ON workflow_runs(workflow_id, trigger, idempotency_key)
    WHERE idempotency_key IS NOT NULL AND idempotency_key != '';
...
class SessionDB:
    ...
    # With multiple hairball processes (gateway + CLI sessions + worktree agents)
    ...
    def __init__(self, db_path: Path = None, read_only: bool = False):
        ...
        try:
            ...
            # ``hairball_state._set_last_init_error(None)`` explicitly.
    ...
    def _warn_fts5_unavailable(self, exc: sqlite3.OperationalError) -> None:
        ...
        logger.warning(
            ...
            "disabled. Run `hairball update` to rebuild the venv with a "
    ...
    def _sqlite_supports_fts5(self, cursor: sqlite3.Cursor) -> bool:
        try:
            cursor.execute("CREATE VIRTUAL TABLE temp._hairball_fts5_probe USING fts5(x)")
            cursor.execute("DROP TABLE temp._hairball_fts5_probe")
            return True
        except sqlite3.OperationalError as exc:
                ...
                try:
                    cursor.execute(
                        """INSERT OR IGNORE INTO session_model_usage (
                               ...
                               cost_actual_call_count, cost_total_call_count,
                           ...
                           SELECT id, COALESCE(model, 'unknown'),
                                  ...
                                  COALESCE(cost_actual_call_count, 0),
                                  COALESCE(cost_total_call_count, 0),
    ...
    def fork_session(
        ...
        *,
        ...
        """Atomically fork a session and its active replay transcript.

        ``messages=None`` performs a byte-preserving SQLite copy of every
        active message.  Callers that expose a message-level fork may provide
        an already validated prefix instead.  Session creation, transcript
        insertion, counters, title, lineage marker, and the optional parent
        transition commit together; a failure therefore cannot leave a
        half-copied branch or an ended parent without a child.

        Routing identity (user/chat/thread/session_key), usage, billing, and
        live execution state are intentionally not inherited.  Those belong
        to the surface that activates the new branch, not to conversation
        replay.
        """
        parent_id = str(source_session_id or "").strip()
        child_id = str(new_session_id or "").strip()
        if not parent_id:
        ...
        if re.search(r"[\r\n\x00]", child_id):
            raise ValueError("invalid new_session_id")

        explicit_title = self.sanitize_title(title) if title is not None else None
        supplied_messages = copy.deepcopy(messages) if messages is not None else None
        config_updates = copy.deepcopy(model_config_updates or {})

        def _next_title(conn: sqlite3.Connection, base_title: str) -> str:
            ...
            for existing_title in existing:
                suffix = re.match(r"^.* #(\d+)$", existing_title or "")
                if suffix:
        ...
        def _do(conn: sqlite3.Connection) -> Dict[str, Any]:
            ...
            if conn.execute(
                "SELECT 1 FROM sessions WHERE id = ?", (child_id,)
            ).fetchone():
                raise ValueError(f"session already exists: {child_id}")

            parent_config: Dict[str, Any] = {}
            if parent["model_config"]:
                try:
                    decoded = json.loads(parent["model_config"])
                    if isinstance(decoded, dict):
                        parent_config = decoded
                except (TypeError, json.JSONDecodeError):
                    logger.warning(
                        "Ignoring malformed model_config while branching session %s",
                        parent_id,
                    )
            parent_config.update(config_updates)
            parent_config["_branched_from"] = parent_id

            branch_title = explicit_title
            if branch_title is None:
                branch_title = _next_title(conn, parent["title"] or "branch")
            conflict = conn.execute(
                "SELECT id FROM sessions WHERE title = ?", (branch_title,)
            ).fetchone()
            if conflict is not None:
                raise ValueError(
                    f"Title '{branch_title}' is already in use by session {conflict['id']}"
                )
            ...
            conn.execute(
                ...
                (
                    child_id,
                    source or parent["source"],
                    model if model is not None else parent["model"],
            ...
            if supplied_messages is None:
                ...
                counts = conn.execute(
                    ...
                    "COALESCE(SUM(CASE WHEN tool_calls IS NULL THEN 0 "
                    "WHEN json_type(tool_calls) = 'array' THEN json_array_length(tool_calls) "
                    "ELSE 1 END), 0) AS tools "
                    "FROM messages WHERE session_id = ? AND active = 1",
                    (child_id,),
                ).fetchone()
                message_count = int(counts["messages"] or 0)
                tool_call_count = int(counts["tools"] or 0)
            else:
                message_count, tool_call_count = self._insert_message_rows(
                    conn, child_id, supplied_messages
                )

            conn.execute(
                "UPDATE sessions SET message_count = ?, tool_call_count = ? WHERE id = ?",
                (message_count, tool_call_count, child_id),
            )
            if end_parent:
                conn.execute(
                    "UPDATE sessions SET ended_at = ?, end_reason = 'branched' WHERE id = ?",
                    (started_at, parent_id),
                )
            row = conn.execute(
    ...
    def _backfill_gateway_metadata_from_sessions_json(
        ...
        """
        sessions_file = get_hairball_home() / "sessions" / "sessions.json"
    ...
    def update_token_counts(
        ...
        cost_actual_call_count: int = 0,
        cost_total_call_count: int = 0,
        ...
        if absolute:
            ...
            sql = """UPDATE sessions SET
                   ...
                   END,
                   cost_actual_call_count = COALESCE(cost_actual_call_count, 0) + ?,
                   cost_total_call_count = COALESCE(cost_total_call_count, 0) + ?,
                   cost_status = COALESCE(?, cost_status),
                   cost_source = COALESCE(?, cost_source),
                   pricing_version = COALESCE(?, pricing_version),
                   billing_provider = COALESCE(billing_provider, ?),
        ...
        has_accounted_usage = bool(
            ...
            or cost_actual_call_count or cost_total_call_count
        )
        params = (
            input_tokens,
            output_tokens,
        ...
        def _do(conn):
            ...
            if record_model_usage:
                self._record_model_usage(
                    ...
                    cost_actual_call_count=cost_actual_call_count,
                    cost_total_call_count=cost_total_call_count,
    ...
    def _record_model_usage(
        ...
        *,
        ...
        cost_actual_call_count: int,
        cost_total_call_count: int,
        ...
        conn.execute(
            """INSERT INTO session_model_usage (
               ...
               DO UPDATE SET
                   ...
                   cost_actual_call_count = cost_actual_call_count + excluded.cost_actual_call_count,
                   cost_total_call_count = cost_total_call_count + excluded.cost_total_call_count,
            ...
            (
                ...
                int(cost_actual_call_count or 0),
                int(cost_total_call_count or 0),
                cost_status,
                cost_source,
                now,
                now,
                    ...
                    COALESCE(
                        (SELECT SUBSTR(REPLACE(REPLACE(m.content, X'0A', ' '), X'0D', ' '), 1, 63)
                         ...
                         WHERE m.session_id = s.id AND m.role = 'user' AND m.content IS NOT NULL
                           AND COALESCE(m.finish_reason, '') != 'hook_prompt'
    ...
    def get_messages_as_conversation(
        ...
        for row in rows:
            ...
            # that replay reasoning (OpenRouter, OpenAI, managed) receive
    ...
    def list_recent_user_messages(
        ...
        with self._lock:
            cursor = self._conn.execute(
                ...
                "AND COALESCE(finish_reason, '') != 'hook_prompt'"
                f"{active_clause} "
                "ORDER BY id DESC LIMIT ?",
    ...
    def search_sessions_by_id(
        ...
        a session id from logs, CLI output, or another Hairball surface and jump
    ...
    def _completion_contract_exports_for_sessions(
        self, session_ids: Sequence[str]
    ) -> List[Dict[str, Any]]:
        """Return immutable completion facts for an already-resolved lineage.

        A compression continuation receives a new session id, while completion
        contracts remain bound to the original turn's id.  Export is the
        existing logical-session read seam, so aggregate there rather than
        copying or rewriting an immutable contract during rotation.
        """
        ids = [str(session_id) for session_id in session_ids if session_id]
        if not ids or not self.completion_contract_ready():
            return []
        placeholders = ",".join("?" for _ in ids)
        with self._lock:
            rows = self._conn.execute(
                "SELECT contract_id FROM completion_contracts "
        ...
        for row in rows:
            ...
            contract = self.read_completion_contract(contract_id)
            if contract is None:
                continue
            exports.append(
                {
                    "contract": contract,
                    "evidence": self.read_completion_evidence(contract_id),
                    "assessments": self.read_completion_assessments(contract_id),
                }
            )
        return exports
    ...
    def export_session_lineage(self, session_id: str) -> Optional[Dict[str, Any]]:
        ...
        base["completion_contracts"] = self._completion_contract_exports_for_sessions(
            base["lineage_session_ids"]
        )
        return base
            ...
            for item in normalized:
                ...
                conn.execute(
                       ...
                       )""",
                    {
                        ...
                        "cost_actual_call_count": self._int_or_default(
                            raw.get("cost_actual_call_count")
                        ),
                        "cost_total_call_count": self._int_or_default(
                            raw.get("cost_total_call_count")
                        ),
    ...
    def delete_session(
        ...
        def _do(conn):
            ...
            _delete_completion_contracts_for_sessions(conn, [session_id])
    ...
    def delete_session_if_empty(
        ...
        and ``hairball sessions list`` output. (Pattern ported from
    ...
    def delete_sessions(
        ...
        * On-disk transcript / ``request_dump_*`` files are cleaned up
        ...
        def _do(conn):
            ...
            _delete_completion_contracts_for_sessions(conn, existing)
    ...
    def delete_empty_sessions(
        ...
        * Cleans up on-disk transcript files (``.json`` / ``.jsonl`` /
        ...
        def _do(conn):
            ...
            _delete_completion_contracts_for_sessions(conn, list(session_ids))
            conn.execute(
                f"UPDATE sessions SET parent_session_id = NULL "
    ...
    def _prune_filter_where(
        *,
        ...
        the provider actual total only when every billable call reported one;
        otherwise they keep using the complete estimate because a partial
        actual value is only a subtotal.
        ...
        if min_cost is not None:
            clauses.append(
                "CASE WHEN COALESCE(s.cost_total_call_count, 0) > 0 "
                "AND COALESCE(s.cost_actual_call_count, 0) >= "
                "COALESCE(s.cost_total_call_count, 0) "
                "THEN COALESCE(s.actual_cost_usd, s.estimated_cost_usd, 0) "
                "ELSE COALESCE(s.estimated_cost_usd, 0) END >= ?"
            )
    ...
    # ── Kernel v2 event ledger ────────────────────────────────────────────
    #
    # The ledger lives in this database (one state.db, not a second store) but
    # is created on explicit opt-in rather than from ``_init_schema``. Every
    # existing Hairball process opens ``state.db`` on startup; adding five
    # tables to that hot path would make a kernel-v2 experiment able to break
    # CLI/gateway/cron startup for users who never enabled it. The Telegram
    # topic tables below set the same precedent.

    KERNEL_LEDGER_SCHEMA_VERSION = 1
    _KERNEL_LEDGER_META_KEY = "kernel_ledger_schema_version"

    # Completion contracts are a separately opt-in experiment.  They are not
    # part of SCHEMA_SQL for the same reason as the kernel ledger: opening a
    # normal Hairball session must not mutate or pay for an unused capability.
    # The data is still kept in the existing state.db so replay has one source
    # of truth rather than a second side database.
    COMPLETION_CONTRACT_SCHEMA_VERSION = 1
    _COMPLETION_CONTRACT_META_KEY = "completion_contract_schema_version"

    def kernel_ledger_ready(self) -> bool:
        """True when the kernel ledger tables exist at the current version."""
        try:
            with self._lock:
                row = self._conn.execute(
                    "SELECT value FROM state_meta WHERE key = ?",
                    (self._KERNEL_LEDGER_META_KEY,),
                ).fetchone()
        except sqlite3.Error:
            return False
        if not row:
            return False
        try:
            return int(row[0]) >= self.KERNEL_LEDGER_SCHEMA_VERSION
        except (TypeError, ValueError):
            return False

    def apply_kernel_ledger_migration(self) -> None:
        """Create the append-only kernel event ledger tables.

        Idempotent: safe to call on every kernel session open. Returns early
        once ``state_meta`` records the current ledger version so a hot path
        that calls this per turn does not pay for a write transaction.
        """
        if self.kernel_ledger_ready():
            return
        ...
        def _do(conn):
            conn.executescript(
                ...
                CREATE UNIQUE INDEX IF NOT EXISTS idx_kernel_events_event_id
                    ON kernel_events(event_id);
                CREATE INDEX IF NOT EXISTS idx_kernel_events_turn
                    ON kernel_events(session_id, turn_id, seq);
                CREATE INDEX IF NOT EXISTS idx_kernel_events_kind
                    ON kernel_events(session_id, kind, seq);
                CREATE INDEX IF NOT EXISTS idx_kernel_events_tool_call
                    ON kernel_events(session_id, tool_call_id, seq)
                    WHERE tool_call_id != '';

                CREATE TABLE IF NOT EXISTS kernel_session_versions (
                    session_id TEXT PRIMARY KEY,
                    kernel_version TEXT NOT NULL,
                    prompt_epoch TEXT NOT NULL DEFAULT '',
                    tool_schema_epoch TEXT NOT NULL DEFAULT '',
                ...
                CREATE TABLE IF NOT EXISTS kernel_projector_checkpoints (
                    session_id TEXT NOT NULL,
                    projector TEXT NOT NULL,
                    last_seq INTEGER NOT NULL DEFAULT 0,
                    updated_at REAL NOT NULL,
                    state_json TEXT NOT NULL DEFAULT '{}',
                    PRIMARY KEY (session_id, projector)
                );
                ...
                CREATE INDEX IF NOT EXISTS idx_kernel_artifacts_session
                    ON kernel_artifacts(session_id, created_at);
            ...
            conn.execute(
                ...
                (
                    self._KERNEL_LEDGER_META_KEY,
                    str(self.KERNEL_LEDGER_SCHEMA_VERSION),
                ),
            )
    ...
    def append_kernel_events(self, session_id: str, events: List[Dict[str, Any]]) -> int:
        """Append *events* to the session's ledger and return the last seq.

        Sequence numbers are assigned inside the write transaction from
        ``MAX(seq)``, so two processes appending to the same session cannot mint
        the same ``seq`` — ``BEGIN IMMEDIATE`` serializes them and the
        ``(session_id, seq)`` primary key is the backstop.

        Re-appending an ``event_id`` that is already durable is ignored rather
        than raising: a surface that retries a flush after a dropped connection
        must not create a duplicate durable fact.
        """
        if not events:
            return 0
        ...
        def _do(conn):
            row = conn.execute(
                "SELECT COALESCE(MAX(seq), 0) FROM kernel_events WHERE session_id = ?",
                (session_id,),
            ).fetchone()
            ...
            for event in events:
                cursor = conn.execute(
                    ...
                    INSERT OR IGNORE INTO kernel_events (
                        session_id, seq, event_id, timestamp, branch_id,
                        turn_id, agent_id, kind, schema_version,
                        causation_id, correlation_id, tool_call_id, payload_json
                    ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                    """,
                    (
                        session_id,
    ...
    def append_kernel_event_if_kind_seq(
        self,
        session_id: str,
        *,
        kind: str,
        expected_kind_seq: int,
        event: Dict[str, Any],
        guard_kind: Optional[str] = None,
        expected_guard_kind_seq: Optional[int] = None,
    ) -> Dict[str, Any]:
        """CAS-append one event against the latest sequence for ``kind``.

        The global kernel sequence is shared by every event family, so a
        projector cannot safely compare against ``MAX(seq)`` for the whole
        session.  This primitive compares only the requested kind inside the
        same ``BEGIN IMMEDIATE`` transaction that allocates the next global
        sequence.  It is used by durable state machines such as Plan execution
        to reject stale cross-process transitions without creating another DB.

        ``guard_kind`` optionally adds a second CAS predicate in the same
        write transaction.  It lets one event family prove that the state
        machine it depends on has not advanced between its read and append;
        this is how a Plan cancellation and a new side-effect claim receive a
        deterministic database order instead of racing in user space.

        Repeating the same globally unique ``event_id`` is idempotent and
        returns the original row even when ``expected_kind_seq`` is now stale.
        The input event dictionary is never mutated.
        """
        if not session_id:
        ...
        guard_event_kind = str(guard_kind or "").strip()
        if (guard_event_kind and expected_guard_kind_seq is None) or (
            expected_guard_kind_seq is not None and not guard_event_kind
        ):
            raise ValueError(
                "guard_kind and expected_guard_kind_seq must be provided together"
            )

        self.apply_kernel_ledger_migration()

        def _do(conn):
            existing = conn.execute(
                "SELECT * FROM kernel_events WHERE event_id = ?",
                (event_id,),
            ).fetchone()
            if existing is not None:
                row = dict(existing)
                if row.get("session_id") != session_id or row.get("kind") != event_kind:
                    return {
                        "ok": False,
                        "duplicate": False,
                        "reason": "event_id_collision",
                        "current_seq": int(row.get("seq") or 0),
                        "event": row,
                    }
                return {
                    "ok": True,
            ...
            if guard_event_kind:
                guard_row = conn.execute(
                    "SELECT COALESCE(MAX(seq), 0) FROM kernel_events "
                    "WHERE session_id = ? AND kind = ?",
                    (session_id, guard_event_kind),
                ).fetchone()
                current_guard_seq = int(guard_row[0]) if guard_row else 0
                if current_guard_seq != int(expected_guard_kind_seq or 0):
                    return {
                        "ok": False,
                        "duplicate": False,
                        "reason": "stale_guard_kind_seq",
                        "current_seq": current_guard_seq,
                        "current_guard_seq": current_guard_seq,
                    }
            ...
            current_seq = int(current_row[0]) if current_row else 0
            if current_seq != int(expected_kind_seq or 0):
                return {
                    "ok": False,
                    "duplicate": False,
                    "reason": "stale_kind_seq",
            ...
            next_seq = (int(global_row[0]) if global_row else 0) + 1
            conn.execute(
                """
    ...
    def read_kernel_events(
        ...
        *,
        ...
        """Read ledger rows for *session_id* in seq order.

        ``after_seq`` is the resume cursor a disconnected surface replays from,
        so it is exclusive: passing the last seq you already rendered returns
        only what you missed.
        """
        sql = "SELECT * FROM kernel_events WHERE session_id = ? AND seq > ?"
    ...
    def get_kernel_session_version(self, session_id: str) -> Optional[Dict[str, Any]]:
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM kernel_session_versions WHERE session_id = ?",
                (session_id,),
            ).fetchone()
        return dict(row) if row else None

    def pin_kernel_session_version(
        self,
        session_id: str,
        *,
        kernel_version: str,
        ...
        """Pin the kernel version for a session, first writer wins.

        A session must never switch kernels mid-life, so an existing row is
        returned unchanged instead of being overwritten. Callers compare the
        returned ``kernel_version`` against what they asked for to detect that
        they were handed an already-pinned session.
        """
        now = time.time()
        ...
        def _do(conn):
            conn.execute(
                """
                INSERT OR IGNORE INTO kernel_session_versions (
                    session_id, kernel_version, prompt_epoch, tool_schema_epoch,
                    event_schema_version, protocol_version,
    ...
    def get_kernel_projector_checkpoint(
        ...
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM kernel_projector_checkpoints "
                "WHERE session_id = ? AND projector = ?",
                (session_id, projector),
            ).fetchone()
        return dict(row) if row else None
    ...
    def save_kernel_projector_checkpoint(
        ...
        *,
        ...
        """Persist a projector's position, never moving it backwards.

        A replay that re-reads older events must not rewind the checkpoint,
        because another reader may already have consumed past that point.
        """
        now = time.time()
        ...
        def _do(conn):
            conn.execute(
                """
                INSERT INTO kernel_projector_checkpoints (
                    session_id, projector, last_seq, updated_at, state_json
                ) VALUES (?, ?, ?, ?, ?)
                ON CONFLICT(session_id, projector) DO UPDATE SET
                    last_seq = MAX(kernel_projector_checkpoints.last_seq, excluded.last_seq),
                    updated_at = excluded.updated_at,
                    state_json = excluded.state_json
                """,
                (session_id, projector, int(last_seq), now, state_json),
            )
        ...
        def _do(conn):
            conn.execute(
                ...
                INSERT INTO kernel_artifacts (
                    artifact_id, session_id, kind, media_type, byte_count,
                    digest, path, summary, truncated, created_at, metadata_json
                ) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
                ...
                (
                    ...
                    artifact.get("media_type") or "text/plain",
                    int(artifact.get("byte_count") or 0),
    ...
    def get_kernel_artifact(self, artifact_id: str) -> Optional[Dict[str, Any]]:
        with self._lock:
            row = self._conn.execute(
                "SELECT * FROM kernel_artifacts WHERE artifact_id = ?",
                (artifact_id,),
            ).fetchone()
        return dict(row) if row else None
    ...
    def list_kernel_artifacts(self, session_id: str) -> List[Dict[str, Any]]:
        with self._lock:
            rows = self._conn.execute(
                "SELECT * FROM kernel_artifacts WHERE session_id = ? "
    ...
    def claim_kernel_idempotency_key(
        ...
        *,
        ...
        """Claim ``(scope, key)`` exactly once.

        Returns ``None`` when this caller won the claim (proceed with the
        side effect), or the previously stored row when someone already claimed
        it (skip the side effect and reuse that result). This is what keeps a
        retried mailbox delivery or a re-run ``collect`` from injecting the same
        body into a parent context twice.
        """
        now = time.time()

        def _do(conn):
            cursor = conn.execute(
                "INSERT OR IGNORE INTO kernel_idempotency_keys "
                "(scope, key, session_id, result_json, created_at) "
                "VALUES (?, ?, ?, ?, ?)",
                (scope, key, session_id, result_json, now),
            )
            if cursor.rowcount:
                return None
            return dict(
                conn.execute(
                    "SELECT * FROM kernel_idempotency_keys WHERE scope = ? AND key = ?",
                    (scope, key),
                ).fetchone()
            )
    ...
    # ── Completion-contract evidence ledger ───────────────────────────────

    def completion_contract_ready(self) -> bool:
        """True when opt-in completion-contract tables are available."""
        try:
            with self._lock:
                row = self._conn.execute(
                    "SELECT value FROM state_meta WHERE key = ?",
                    (self._COMPLETION_CONTRACT_META_KEY,),
                ).fetchone()
        except sqlite3.Error:
            return False
        if not row:
            return False
        try:
            return int(row[0]) >= self.COMPLETION_CONTRACT_SCHEMA_VERSION
        except (TypeError, ValueError):
            return False

    def apply_completion_contract_migration(self) -> None:
        """Create the opt-in, append-only completion evidence tables.

        A completion assessment must survive process restart and replay, but
        must never force a schema migration on users who have not enabled the
        experiment.  Event ids and contract/evidence sequence keys make a
        retry after a dropped response idempotent rather than duplicating a
        claim in a resumed session.
        """
        if self.completion_contract_ready():
            return
        ...
        def _do(conn):
            conn.executescript(
                ...
                CREATE INDEX IF NOT EXISTS idx_completion_contracts_session_turn
                    ON completion_contracts(session_id, turn_id);
                ...
                CREATE TABLE IF NOT EXISTS completion_evidence (
                    ...
                    FOREIGN KEY(contract_id) REFERENCES completion_contracts(contract_id)
                        ON DELETE CASCADE
                );

                CREATE INDEX IF NOT EXISTS idx_completion_evidence_contract_sequence
                    ON completion_evidence(contract_id, sequence);
                ...
                CREATE TABLE IF NOT EXISTS completion_assessments (
                    ...
                    payload_json TEXT NOT NULL,
                    created_at REAL NOT NULL,
                    PRIMARY KEY(contract_id, evidence_sequence, phase),
                    FOREIGN KEY(contract_id) REFERENCES completion_contracts(contract_id)
                        ON DELETE CASCADE
                );

                CREATE INDEX IF NOT EXISTS idx_completion_assessments_contract
                    ON completion_assessments(contract_id, evidence_sequence, phase);
                """
            )
            conn.execute(
                "INSERT INTO state_meta(key, value) VALUES (?, ?) "
                ...
                (
                    self._COMPLETION_CONTRACT_META_KEY,
                    str(self.COMPLETION_CONTRACT_SCHEMA_VERSION),
                ),
            )
    ...
    @staticmethod
    def _completion_payload(row: Mapping[str, Any]) -> Dict[str, Any]:
        """Read one canonical payload without exposing DB implementation keys."""
        try:
    ...
    def upsert_completion_contract(self, contract: Mapping[str, Any]) -> bool:
        """Persist one immutable contract; return whether this call inserted it.

        Replaying the exact same immutable contract is safe.  A conflicting
        payload or an attempt to reuse a session/turn with another contract is
        a caller bug and raises rather than silently changing historical work.
        """
        if not self.completion_contract_ready():
            raise RuntimeError("completion contract store is not initialized")
        ...
        if not contract_id or not session_id or not turn_id or not digest or parser_version <= 0:
            raise ValueError("completion contract requires id, session, turn, digest and parser version")
        payload_json = json.dumps(payload, ensure_ascii=False, sort_keys=True, default=str)
        now = time.time()

        def _do(conn):
            existing = conn.execute(
                "SELECT contract_id, payload_json FROM completion_contracts "
                "WHERE contract_id = ? OR (session_id = ? AND turn_id = ?)",
                (contract_id, session_id, turn_id),
            ).fetchone()
            if existing is not None:
                if str(existing["contract_id"]) == contract_id and str(existing["payload_json"]) == payload_json:
                    return False
                raise ValueError("conflicting completion contract for immutable session turn")
            conn.execute(
                "INSERT INTO completion_contracts "
                "(contract_id, session_id, turn_id, source_digest, parser_version, payload_json, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?, ?)",
                (contract_id, session_id, turn_id, digest, parser_version, payload_json, now),
            )
            return True

        return self._execute_write(_do)

    def read_completion_contract(self, contract_id: str) -> Optional[Dict[str, Any]]:
        if not self.completion_contract_ready():
            return None
        with self._lock:
            row = self._conn.execute(
                "SELECT payload_json FROM completion_contracts WHERE contract_id = ?",
                (str(contract_id),),
            ).fetchone()
        return self._completion_payload(dict(row)) if row else None

    def append_completion_evidence(
        self, contract_id: str, evidence: Mapping[str, Any]
    ) -> bool:
        """Append one ordered fact; duplicate delivery is a no-op, conflict raises."""
        if not self.completion_contract_ready():
            raise RuntimeError("completion contract store is not initialized")
        ...
        if not evidence_id or sequence <= 0 or not kind:
            raise ValueError("completion evidence requires id, positive sequence and kind")
        payload_json = json.dumps(payload, ensure_ascii=False, sort_keys=True, default=str)
        now = time.time()

        def _do(conn):
            if conn.execute(
                "SELECT 1 FROM completion_contracts WHERE contract_id = ?", (contract_id,)
            ).fetchone() is None:
                raise ValueError("unknown completion contract")
            existing = conn.execute(
                "SELECT evidence_id, sequence, payload_json FROM completion_evidence "
                "WHERE evidence_id = ? OR (contract_id = ? AND sequence = ?)",
                (evidence_id, contract_id, sequence),
            ).fetchone()
            if existing is not None:
                if str(existing["evidence_id"]) == evidence_id and str(existing["payload_json"]) == payload_json:
                    return False
                if int(existing["sequence"]) == sequence:
                    raise ValueError("conflicting completion evidence sequence")
                raise ValueError("conflicting completion evidence id")
            conn.execute(
                "INSERT INTO completion_evidence "
                "(evidence_id, contract_id, sequence, kind, payload_json, created_at) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (evidence_id, contract_id, sequence, kind, payload_json, now),
            )
            return True

        return self._execute_write(_do)

    def read_completion_evidence(self, contract_id: str) -> List[Dict[str, Any]]:
        if not self.completion_contract_ready():
            return []
        with self._lock:
            rows = self._conn.execute(
                "SELECT payload_json FROM completion_evidence WHERE contract_id = ? "
                "ORDER BY sequence, evidence_id",
                (str(contract_id),),
            ).fetchall()
        return [self._completion_payload(dict(row)) for row in rows]
    ...
    def save_completion_assessment(
        ...
        """Write a replayable assessment snapshot once for its evidence cursor."""
        if not self.completion_contract_ready():
            raise RuntimeError("completion contract store is not initialized")
        sequence = int(evidence_sequence)
        phase = str(phase or "")
        if sequence < 0 or not phase:
            raise ValueError("completion assessment requires sequence and phase")
        ...
        def _do(conn):
            if conn.execute(
                ...
                raise ValueError("unknown completion contract")
            existing = conn.execute(
                "SELECT payload_json FROM completion_assessments "
                "WHERE contract_id = ? AND evidence_sequence = ? AND phase = ?",
                (contract_id, sequence, phase),
            ).fetchone()
            if existing is not None:
                if str(existing["payload_json"]) == payload_json:
                    return False
                raise ValueError("conflicting completion assessment snapshot")
            conn.execute(
                "INSERT INTO completion_assessments "
                "(contract_id, evidence_sequence, phase, payload_json, created_at) "
                "VALUES (?, ?, ?, ?, ?)",
                (contract_id, sequence, phase, payload_json, now),
            )
            return True

        return self._execute_write(_do)

    def read_completion_assessments(self, contract_id: str) -> List[Dict[str, Any]]:
        if not self.completion_contract_ready():
            return []
        with self._lock:
            rows = self._conn.execute(
                "SELECT payload_json FROM completion_assessments WHERE contract_id = ? "
                "ORDER BY evidence_sequence, phase",
                (str(contract_id),),
            ).fetchall()
        return [self._completion_payload(dict(row)) for row in rows]

    def read_latest_terminal_completion_assessment_for_lineage(
        self, session_id: str
    ) -> Optional[Dict[str, Any]]:
        """Read the latest terminal assessment for one logical conversation.

        Compression rotates a conversation onto a new session id without
        rewriting immutable completion facts.  Resume therefore reads across
        the existing compression lineage.  ``get_compression_lineage`` keeps
        explicit branches isolated, so a branch never inherits the parent's
        assessment merely because it has a ``parent_session_id``.
        """
        if not session_id or not self.completion_contract_ready():
            return None
        lineage_ids = self.get_compression_lineage(str(session_id))
        if not lineage_ids:
            return None
        ...
        with self._lock:
            row = self._conn.execute(
                ...
                "FROM completion_assessments AS ca "
                "JOIN completion_contracts AS cc "
                "ON cc.contract_id = ca.contract_id "
                f"WHERE cc.session_id IN ({placeholders}) AND ca.phase = ? "
                "ORDER BY ca.created_at DESC, cc.created_at DESC, "
                "ca.evidence_sequence DESC, cc.contract_id DESC LIMIT 1",
                [*lineage_ids, "terminal"],
            ).fetchone()
        return self._completion_payload(dict(row)) if row else None
    ...
    def apply_telegram_topic_migration(self) -> None:
        ...
        reconciliation. Operators must be able to upgrade Hairball, keep the old
    ...
    def bind_telegram_topic(
        ...
        *,
        ...
        """Bind one Telegram DM topic thread to one Hairball session.

        A Hairball session may only be linked to one Telegram topic in MVP.
    ...
    def is_telegram_session_linked_to_topic(self, *, session_id: str) -> bool:
        """Return True if a Hairball session is already bound to any Telegram DM topic.
    ...
    def optimize_fts(self) -> int:
        ...
        disabled via ``HAIRBALL_DISABLE_FTS_TRIGRAM`` or not yet created), so
