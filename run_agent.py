...
def _launch_cwd_for_session(source: str) -> Optional[str]:
    ...
    launched from is meaningful for ``hairball -c`` / ``--resume`` (relaunch
...
def _session_source_for_agent(platform: Optional[str]) -> str:
    try:
        ...
        source = os.environ.get("HAIRBALL_SESSION_SOURCE", "")
...
from hairball_cli.timeouts import (
...
_loaded_env_paths = load_hairball_dotenv(hairball_home=_hairball_home, project_env=_project_env)
...
from model_tools import (
    build_session_tool_router,  # noqa: F401  # shared by agent init/refresh
    ...
    session_tool_router_for_agent,  # noqa: F401  # execution snapshot adapter
...
def _routermint_headers() -> dict:
    ...
    from hairball_cli import __version__ as _HAIRBALL_VERSION

    return {
        "User-Agent": f"HairballAgent/{_HAIRBALL_VERSION}",
    }
...
def _safe_session_filename_component(session_id: str) -> str:
    ...
    ``X-Hairball-Session-Id`` API header) and are otherwise interpolated raw
    into on-disk artifact filenames under ``~/.hairball/sessions/``.  Without
...
class _StreamErrorEvent(Exception):
    ...
    Some session-style Responses backends (xAI for subscription/quota
...
class AIAgent:
    ...
    _TOOL_CALL_ARGUMENTS_CORRUPTION_MARKER = (
        "[hairball-agent: tool call arguments were corrupted in this session and "
    ...
    def __init__(
        ...
        visible_toolsets: List[str] = None,
    ):
    ...
    def reset_session_state(
        ...
        # Provider-reported USD is separate from the full estimate. None means
        # no response reported a charge; the call counters make partial
        # coverage explicit for mixed-route/MoA sessions.
        self.session_actual_cost_usd = None
        self.session_cost_actual_call_count = 0
        self.session_cost_total_call_count = 0
    ...
    def _ensure_lmstudio_runtime_loaded(self, config_context_length: Optional[int] = None) -> None:
        """
        Preload the LM Studio model with at least Hairball' minimum context.
        """
        if (self.provider or "").strip().lower() != "lmstudio":
            return
        try:
            from agent.model_metadata import MINIMUM_CONTEXT_LENGTH
            from hairball_cli.models import ensure_lmstudio_model_loaded
    ...
    def _vprint(self, *args, force: bool = False, **kwargs):
        ...
        parseable single-query flows such as ``hairball chat -q``. In that mode,
    ...
    def _emit_notice(self, notice) -> None:
        ...
        callback and observer-hook errors — a notice must NEVER break the agent
        loop or become invisible because an extension failed (D-D fail-open).
        """
        try:
            ...
            logger.debug("on_notification hook error in _emit_notice", exc_info=True)
    ...
    def _emit_notice_clear(self, key: str) -> None:
        ...
        try:
            ...
            logger.debug(
                "on_notification_clear hook error in _emit_notice_clear",
                exc_info=True,
            )
    ...
    def _resolved_api_call_timeout(self) -> float:
        ...
        Priority:
          ...
          3. ``HAIRBALL_API_TIMEOUT`` env var (legacy escape hatch)
          4. 1800.0s default
        ...
        helper, the hardcoded ``HAIRBALL_API_TIMEOUT`` fallback would always be
        ...
        if cfg is not None:
            return cfg
        return env_float("HAIRBALL_API_TIMEOUT", 1800.0)
    ...
    def _resolved_api_call_stale_timeout_base(self) -> tuple[float, bool]:
        ...
        Priority:
          ...
          3. ``HAIRBALL_API_CALL_STALE_TIMEOUT`` env var
          4. 90.0s default (time-to-first-byte for non-streaming / reference
        ...
        env_timeout = os.getenv("HAIRBALL_API_CALL_STALE_TIMEOUT")
    ...
    def _codex_silent_hang_hint(self, model: Optional[str] = None) -> Optional[str]:
        ...
        reference silent-reject configuration, else ``None``.

        The ChatGPT backend (``chatgpt.com/backend-api/codex``) has
        ...
        Currently flagged: ``gpt-5.5`` family on the reference backend.  See
        hairball-agent #21444 for the symptom history.  The upstream backend
        ...
        if not re.search(r"(?:^|[/\-_])gpt-5\.5(?:$|[\-_])", model_lower):
            return None
        return (
            f"reference backend appears to be silently rejecting {eff_model!r} "
            ...
            "Some ChatGPT accounts do not support `gpt-5.4-codex`. "
            "See hairball-agent#21444 for symptom history."
        )
    ...
    def _provider_model_requires_responses_api(
        ...
        *,
        ...
        # managed serves GPT-5.x models via its OpenAI-compatible chat
    ...
    def _decorate_xai_entitlement_error(detail: str) -> str:
        ...
        SuperGrok subscribers can use Hairball against xai-oauth.  Lots of
    ...
    def _hook_payload_max_chars() -> int:
        raw = os.getenv("HAIRBALL_PLUGIN_PAYLOAD_MAX_CHARS", "50000")
        try:
    ...
    def _invoke_api_request_error_hook(
        ...
        *,
        ...
        # ``monkeypatch.setattr("hairball_cli.plugins.has_hook", ...)`` still
        ...
        try:
            from hairball_cli import plugins as _plugins
    ...
    def _redact_message_content(content):
        ...
        Respects ``HAIRBALL_REDACT_SECRETS`` via ``redact_sensitive_text`` —
    ...
    def _save_session_log(self, messages: List[Dict[str, Any]] = None):
        ...
        whose external tooling consumes ``~/.hairball/sessions/session_{sid}.json``
        ...
        # untrusted input (X-Hairball-Session-Id header) and must not escape
        ...
        try:
            ...
            for msg in messages:
                ...
                # output, or user paste. Respects HAIRBALL_REDACT_SECRETS via
    ...
    def interrupt(self, message: str = None) -> None:
        ...
        # reference app-server owns its model/tool loop and watches a private
        # interrupt event rather than Hairball's per-thread flag.
        if getattr(self, "api_mode", None) == "codex_app_server":
            ...
            if callable(_request_interrupt):
                try:
                    ...
                    logger.debug(
                        "Failed to interrupt reference app-server turn",
                        exc_info=True,
                    )
    ...
    def _ensure_turn_inbox(self):
        inbox = getattr(self, "_turn_inbox", None)
        if inbox is None:
            from agent.turn_inbox import TurnInbox, load_durable_turn_inbox

            sid = str(getattr(self, "session_id", "") or "").strip()
            db = getattr(self, "_session_db", None)
            if sid and not getattr(self, "_persist_disabled", False):
                try:
                    inbox = load_durable_turn_inbox(sid, db=db)
                except Exception:
                    logger.debug(
                        "Durable turn inbox unavailable for %s; using memory",
                        sid,
                        exc_info=True,
                    )
                    inbox = TurnInbox()
    ...
    def _pending_steer(self) -> Optional[str]:
        ...
        if inbox is None:
            return None
        return inbox.next_step_text()

    @_pending_steer.setter
    def _pending_steer(self, value: Optional[str]) -> None:
        self._ensure_turn_inbox().replace_next_step_text(value)

    def steer(self, text: str) -> bool:
        """
        Queue a real user message for the next safe step without interrupting.

        Unlike interrupt(), this does NOT stop the current tool call. The
        text is durably stashed and the agent loop claims it at a step
        boundary after all required tool results are present.
        ...
        if not text or not text.strip():
            return False
        cleaned = text.strip()
        from agent.turn_inbox import new_inbox_message

        _lock = getattr(self, "_pending_steer_lock", None)
        if _lock is None:
            # Test stubs that built AIAgent via object.__new__ skip __init__.
            self._ensure_turn_inbox().append("next-step", new_inbox_message(cleaned))
            return True
        with _lock:
            self._ensure_turn_inbox().append("next-step", new_inbox_message(cleaned))
        return True
    ...
    def inject(self, text: str) -> bool:
        """Park a next-step note without interrupting or waking a turn.

        Same inbox list as ``steer()``. Wake is a surface concern: idle
        inject must not start ``run_conversation``. Does not call
    ...
    def redirect(self, text: str) -> bool:
        ...
        During a normal model request this cancels only that request; the
        conversation loop retains completed messages/tool results, records
        the displayed partial reasoning as plain assistant context, appends
        the correction as a real user message, and retries. During tool
        execution it degrades to ``steer()`` so the tool can finish at a safe
        boundary. reference app-server has a native ``turn/steer`` operation and
        uses it directly instead of cancelling.
        ...
        # reference owns its internal reasoning/tool loop, so use its first-class
        ...
        if getattr(self, "api_mode", None) == "codex_app_server":
            ...
            if callable(_native_steer):
                ...
                try:
                    ...
                    logger.debug("reference app-server turn/steer failed", exc_info=True)
                    return False
    ...
    def _drain_pending_steer(self) -> Optional[str]:
        ...
        Safe to call from the agent execution thread at a step boundary.
        Returns None when no steer is pending.
        """
        _lock = getattr(self, "_pending_steer_lock", None)
        if _lock is None:
            claimed = self._ensure_turn_inbox().claim("next-step", 0)
            if not claimed:
                return None
            return "\n".join(str(row.get("content") or "") for row in claimed)
        with _lock:
            claimed = self._ensure_turn_inbox().claim("next-step", 0)
        if not claimed:
            return None
    ...
    def _record_file_mutation_result(
        ...
        """
        # Reconcile the original workspace after EVERY tool.  This is what
        # catches edits made through terminal/scripts/LSP instead of only the
        # two structured file tools.
        try:
            from agent.turn_workspace_tracker import reconcile_turn_workspace

            reconcile_turn_workspace(self)
        except Exception:
            pass
        ...
        try:
            ...
            structured_paths = workspace_mutation_paths(tool_name, result)
        except Exception:
            structured_paths = ()
        if structured_paths:
            changed = getattr(self, "_turn_file_mutation_paths", None)
            if isinstance(changed, set):
                changed.update(structured_paths)
    ...
    def _format_file_mutation_failure_footer(cls, failed: Dict[str, Dict[str, Any]]) -> str:
        ...
        (e.g. ``~/.hairball/config.yaml``) to a messaging channel (#35584).
        """
        if not failed:
            return ""
        lines = [
            "⚠️ File-mutation verifier: "
            f"{len(failed)} file mutation attempt(s) failed. The final workspace "
            "may still contain a later edit because terminal, IDE, or another "
            "tool can modify the same file. Run `git status` or `read_file` to "
            "confirm; other tools may still have changed it."
        ]
        shown = 0
    ...
    def _turn_completion_explainer_enabled(self) -> bool:
        ...
        True).  ``HAIRBALL_TURN_COMPLETION_EXPLAINER`` env var overrides
        config.  Exposed as a method so tests can patch a single seam,
        mirroring ``_file_mutation_verifier_enabled``.
        """
        try:
            import os as _os
            env = _os.environ.get("HAIRBALL_TURN_COMPLETION_EXPLAINER")
            if env is not None:
                return env.strip().lower() not in {"0", "false", "no", "off"}
            # Read from the persisted config.yaml so gateway and CLI share
            # the same setting.  Import lazily to avoid a startup-time cycle.
    ...
    def _apply_pending_steer_to_tool_results(self, messages: list, num_tool_msgs: int) -> None:
        """Forwarder kept for the stable agent/tool-executor callable contract."""
    ...
    def _touch_activity(self, desc: str) -> None:
        ...
        process is a dispatcher-spawned worker (HAIRBALL_KANBAN_TASK set),
    ...
    def _capture_credits(self, http_response: Any) -> None:
        """Parse x-managed-credits-* headers, cache CreditsState, fire threshold notices.
        ...
        """
        # Dev test fixture (HAIRBALL_DEV_CREDITS_FIXTURE): inject a chosen notice state
        ...
        if _fixture is not None:
            ...
            logger.info(
                ...
                "(real headers bypassed — `echo clear` / unset HAIRBALL_DEV_CREDITS_FIXTURE to restore)",
        ...
        if not headers:
            return
        _dev = is_truthy_value(os.environ.get("HAIRBALL_DEV_CREDITS"))
        ...
        if state is None:
            if _dev:
                logger.info(
                    "credits ▸ response had no valid x-managed-credits-* headers "
                    "(miss — producer off / non-managed path / >TTL stale)"
                )
            return
        ...
        if _dev:
            # HAIRBALL_DEV_CREDITS: stream each capture to agent.log — watch live with
            # `hairball logs -f` (grep 'credits ▸'). Dev-only; silent for normal users.
    ...
    def _is_thinking_only_assistant(
        ...
        *,
        ...
        if isinstance(rd, list) and rd:
            return True
        # reference Responses stores encrypted reasoning state under a separate
    ...
    def _build_keepalive_http_client(base_url: str = "", *, verify: Any = True) -> Any:
        ...
        ``HAIRBALL_CA_BUNDLE`` settings.  It is passed on the client AND on
    ...
    def _try_refresh_codex_client_credentials(self, *, force: bool = True) -> bool:
        ...
        # pool entry (``hairball auth add xai-oauth``) whose tokens belong to
    ...
    def _try_refresh_managed_client_credentials(
        ...
        *,
        ...
        try:
            ...
            logger.debug("managed credential refresh failed: %s", exc)
            return False
        ...
        self._client_kwargs["api_key"] = self.api_key
        self._client_kwargs["base_url"] = self.base_url
        # managed requests should not inherit OpenRouter-only attribution headers.
    ...
    def _try_refresh_copilot_client_credentials(self) -> bool:
        ...
        try:
            from hairball_cli.copilot_auth import resolve_copilot_token
    ...
    def _stream_writer_is_current(self, token: int) -> bool:
        """True when ``token`` is still the active writer (#65991)."""
        return token == getattr(self, "_stream_writer_token", token)

    def _stream_writer_superseded(self) -> bool:
        """True when this thread claimed the sink but a newer attempt superseded it."""
    ...
    def _provider_supports_vision_tool_messages(self) -> bool:
        ...
        Some providers (e.g. Xiaomi) support multimodal user messages
    ...
    def _tool_result_content_for_active_model(self, tool_name: str, result: Any) -> Any:
        ...
        if self._model_supports_vision():
            ...
            # tool content (e.g. Xiaomi's 400 "text is not set"), or if
    ...
    def _try_strip_image_parts_from_tool_messages(
        ...
        *,
        ...
        (e.g. Xiaomi's 400 "text is not set"; see issue #27344).  Walks
    ...
    def _anthropic_preserve_dots(self) -> bool:
        ...
        Xiaomi keeps dots (e.g. mimo-v2.5, mimo-v2.5-pro).
    ...
    def _supports_reasoning_extra_body(self) -> bool:
        ...
        known reasoning-capable model families and direct Managed Portal.
        """
        if base_url_host_matches(self._base_url_lower, "example.invalid"):
            return True
        if (
            ...
            try:
                from hairball_cli.models import github_model_reasoning_efforts
    ...
    def _lmstudio_reasoning_options_cached(self) -> list[str]:
        ...
        if cached is not None:
            ...
            if opts or (_time.monotonic() - ts) < 60:
                return opts
        try:
            from hairball_cli.models import lmstudio_model_reasoning_options
    ...
    def _needs_thinking_reasoning_pad(self) -> bool:
        ...
        #15250, #17400). Xiaomi thinking mode has the same requirement.
    ...
    def _needs_mimo_tool_reasoning(self) -> bool:
        """Return True when the current provider is Xiaomi thinking mode.

        provider thinking mode requires ``reasoning_content`` on every assistant
    ...
    def _sanitize_tool_calls_for_strict_api(api_msg: dict, model: "str | None" = None) -> dict:
        """Strip reference Responses API fields from tool_calls for strict providers.
        ...
        original messages list retains call_id/response_item_id for reference
        Responses API compatibility (e.g. if the session falls back to a
        reference provider later).
    ...
    def _should_sanitize_tool_calls(self) -> bool:
        ...
        reference Responses API uses fields like call_id and response_item_id
        ...
        Returns:
            bool: True if sanitization is needed (non-reference API), False otherwise.
        """
    ...
    def _execute_tool_calls(self, assistant_message, messages: list, effective_task_id: str, api_call_count: int = 0) -> None:
        ...
        try:
            ...
            # One hidden Todo reconciliation point after the complete tool
            # batch.  The controller owns the mutation counter and bounded
            # budget; this adapter only appends its typed message on the
            # existing assistant→tool→user seam.
            if (
                not self._interrupt_requested
                and self._tool_guardrail_halt_decision is None
            ):
                try:
                    controller = getattr(self, "_stop_controller", None)
                    nudge = (
                        controller.take_mid_run_nudge(self.valid_tool_names)
                    ...
                    logger.debug("mid-run Todo reconciliation skipped", exc_info=True)
    ...
    def _dispatch_delegate_task(self, function_args: dict) -> str:
        ...
        # with a handle (one per task). Results do NOT auto-inject a parent
        # turn: the parent uses collab_wait → collab_collect (mailbox Single
        # Rail, same as collab_spawn). This applies to BOTH a single task and
        # a fan-out batch (each task becomes its own independent background
        # subagent). The one exception:
    ...
    def run_conversation(
        ...
        """Run one native turn with optional turn-scoped Plan metadata.

        Mode and execution identity are host metadata, not prompt history or
        tool-schema state.  They are installed only for this call and always
        restored so a reused UI/gateway/background agent cannot inherit the
        preceding Plan turn's ownership.
        """
        from agent.conversation_loop import run_conversation
        execution_id = (
            str(plan_execution_id).strip()
            if plan_execution_id is not None
            else ""
        )
        plan_scope_started = False
        if execution_id:
            try:
                from agent.plan_commands import (
                    load_claimed_plan_steps,
                    prepare_transport_plan_steps,
                )

                claimed_steps = load_claimed_plan_steps(
                    str(getattr(self, "session_id", "") or task_id or ""),
                    execution_id,
                    db=getattr(self, "_session_db", None),
                )
                if not claimed_steps and plan_steps is not None:
                    claimed_steps = prepare_transport_plan_steps(
                        plan_steps,
                        selected_plan_step_ids,
                    )
                if claimed_steps:
                    self._todo_store.begin_plan_scope(claimed_steps)
                    plan_scope_started = True
            except Exception:
                if plan_steps is not None:
                    # Explicit proxy metadata is part of the authorization
                    # contract.  Invalid or unbindable rows must fail before
                    # model/tool dispatch instead of silently executing an
                    # unscoped Plan.
                    raise
                # Authorization already happened at the product boundary.  A
                # storage/read failure must remain visible in logs, while bare
                # transport tests that only bind an execution id stay usable.
                logger.exception(
                    "failed to hydrate protected Plan Todo scope execution=%s",
                    execution_id,
                )

        def _run_native(native_system_message):
            nonlocal plan_scope_started
            try:
                ...
                if plan_scope_started:
                    self._todo_store.end_plan_scope()
                    plan_scope_started = False
                raise
            if plan_scope_started:
                outcomes = self._todo_store.end_plan_scope()
                plan_scope_started = False
                if isinstance(result, dict):
                    result["plan_step_outcomes"] = outcomes
                    result["plan_selected_step_ids"] = [
                        str(item.get("id") or "") for item in outcomes
                    ]
            return result

        if agent_mode is None and not execution_id:
            return _run_native(system_message)

        missing = object()
        previous_execution_id = getattr(self, "_plan_execution_id", missing)
        if agent_mode is None:
            self._plan_execution_id = execution_id
            try:
                return _run_native(system_message)
            finally:
                if previous_execution_id is missing:
                    delattr(self, "_plan_execution_id")
                else:
                    self._plan_execution_id = previous_execution_id
        ...
        previous_mode = getattr(self, "agent_mode", missing)
        previous_plan_file = getattr(self, "_plan_file_path", missing)
        previous_mode_instruction = getattr(
            self, "_agent_mode_instruction", missing
        )
        prior_mode_value = (
            str(previous_mode) if previous_mode is not missing else "discover"
        )
        next_epoch = int(getattr(self, "_agent_mode_epoch", 0) or 0) + 1
        transition = transition_mode(
            {
                "mode": normalize_agent_mode(prior_mode_value),
                "mode_epoch": next_epoch - 1,
            },
            normalize_agent_mode(agent_mode, default=prior_mode_value),
            actor=str(getattr(self, "platform", None) or "agent"),
            epoch=next_epoch,
        )
        if execution_id:
            self._plan_execution_id = execution_id
        elif previous_execution_id is not missing:
            delattr(self, "_plan_execution_id")
        self.agent_mode = str(transition["mode"])
        self._agent_mode_epoch = int(transition["mode_epoch"])
        try:
            ...
            native_system_message = system_message
            if self.agent_mode in {"plan", "build"}:
                from agent.plan_commands import (
                ...
                default_instruction = (
                    render_plan_mode_instruction(
                        ...
                        exists=Path(self._plan_file_path).is_file(),
                    )
                    if self.agent_mode == "plan"
                    else render_build_switch_instruction(self._plan_file_path)
                )
                self._agent_mode_instruction = str(
                    system_message or default_instruction
                ).strip()
                # Per-turn mode guidance is volatile input, not session identity.
                # Passing it through the system-message seam either disappears on
                # a restored cached prompt or poisons a fresh session's prefix.
                native_system_message = None
            else:
                self._agent_mode_instruction = ""
            return _run_native(native_system_message)
        finally:
            if previous_mode is missing:
                delattr(self, "agent_mode")
            else:
                self.agent_mode = previous_mode
            if previous_plan_file is missing:
                if hasattr(self, "_plan_file_path"):
                    delattr(self, "_plan_file_path")
            else:
                self._plan_file_path = previous_plan_file
            if previous_mode_instruction is missing:
                if hasattr(self, "_agent_mode_instruction"):
                    delattr(self, "_agent_mode_instruction")
            else:
                self._agent_mode_instruction = previous_mode_instruction
            if previous_execution_id is missing:
                if hasattr(self, "_plan_execution_id"):
                    delattr(self, "_plan_execution_id")
            else:
                self._plan_execution_id = previous_execution_id
