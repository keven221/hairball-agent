"""Centralized logging setup for Hairball Agent.

Provides a single ``setup_logging()`` entry point that both the CLI and
gateway call early in their startup path.  All log files live under
``~/.hairball/logs/`` (profile-aware via ``get_hairball_home()``).
...
Component separation:
    ...
    gui.log receives dashboard-side records from ``hairball_cli.web_server``,
    ``hairball_cli.pty_bridge``, ``tui_gateway.*``, and ``uvicorn.*``.
...
# essentially always in Hairball (TUI, gateway, ``hy_memory`` server, MCP
...
from hairball_constants import get_config_path, get_hairball_home
...
def _install_session_record_factory() -> None:
    ...
    if getattr(current_factory, "_hairball_session_injector", False):
    ...
    _session_record_factory._hairball_session_injector = True  # type: ignore[attr-defined]
...
# Used by _ComponentFilter and exposed for ``hairball logs --component``.
COMPONENT_PREFIXES = {
    ...
    # ``hairball logs --component gateway``.
    "gateway": ("gateway", "hairball_plugins", "plugins.platforms"),
    "agent": ("agent", "run_agent", "model_tools", "batch_runner"),
    "tools": ("tools",),
    "cli": ("hairball_cli", "cli"),
    "cron": ("cron",),
    "gui": (
        "hairball_cli.web_server",
        "hairball_cli.pty_bridge",
        "tui_gateway",
        "uvicorn",
    ),
}
...
def setup_logging(
    *,
    hairball_home: Optional[Path] = None,
    ...
    """Configure the Hairball logging subsystem.
    ...
    *force* is ``True``.
    ...
    ----------
    hairball_home
        Override for the Hairball home directory.  Falls back to
        ``get_hairball_home()`` (profile-aware).
    log_level
    ...
    home = hairball_home or get_hairball_home()
    log_dir = home / "logs"
...
def setup_verbose_logging() -> None:
    ...
    for h in root.handlers:
        if isinstance(h, logging.StreamHandler) and not isinstance(h, RotatingFileHandler):
            if getattr(h, "_hairball_verbose", False):
                return

    handler = logging.StreamHandler(_safe_stderr())
    handler.setLevel(logging.DEBUG)
    handler.setFormatter(RedactingFormatter(_LOG_FORMAT_VERBOSE, datefmt="%H:%M:%S"))
    handler._hairball_verbose = True  # type: ignore[attr-defined]
...
class _ManagedRotatingFileHandler(RotatingFileHandler):
    ...
    1.  In managed mode (NixOS), the stateDir uses setgid (2770) so new files
        inherit the hairball group. However, both ``_open()`` (initial creation)
    ...
    def __init__(self, *args, **kwargs):
        from hairball_cli.config import is_managed
...
# the module header): when several Hairball processes log to the same file, an
    ...
    *root* directly, so emitting threads never block on file I/O or the
    ...
    with _queue_state_lock:
        if _log_queue is None:
            ...
            qh._hairball_queue = True  # type: ignore[attr-defined]
...
def drain_log_queue(timeout: float = 1.0) -> None:
    ...
    t = threading.Thread(target=_drain, name="hairball-log-drain", daemon=True)
    t.start()
    t.join(timeout)
...
def _reset_queued_handlers() -> None:
    ...
    with _queue_state_lock:
        ...
        for h in list(root.handlers):
            if getattr(h, "_hairball_queue", False):
