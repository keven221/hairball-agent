...
description: Migrate a user's legacy customization footprint into Hairball Agent. Imports Hairball-compatible memories, SOUL.md, command allowlists, user skills, and selected workspace assets from ~/.openclaw, then reports exactly what could not be migrated and why.
version: 1.0.0
author: Hairball Agent (Hairball)
license: MIT
platforms: [linux, macos, windows]
metadata:
  hairball:
    tags: [Migration, legacy, Hairball, Memory, Persona, Import]
    related_skills: [hairball-agent]
---

# legacy -> Hairball Migration

Use this skill when a user wants to move their legacy setup into Hairball Agent with minimal manual cleanup.
...
```bash
hairball claw migrate              # Full interactive migration
...
hairball claw migrate --source /custom/path/.openclaw  # Custom source
```
...
**First-time setup:** The `hairball setup` wizard automatically detects `~/.openclaw` and offers migration before configuration begins.

## What this skill does

It uses `scripts/openclaw_to_hairball.py` to:

- import `SOUL.md` into the Hairball home directory as `SOUL.md`
- transform legacy `MEMORY.md` and `USER.md` into Hairball memory entries
- merge legacy command approval patterns into Hairball `command_allowlist`
- migrate Hairball-compatible messaging settings such as `TELEGRAM_ALLOWED_USERS`, and map legacy workspace settings to Hairball working-directory configuration
- copy legacy skills into `~/.hairball/skills/openclaw-imports/`
- optionally copy the legacy workspace instructions file into a chosen Hairball workspace
- mirror compatible workspace assets such as `workspace/tts/` into `~/.hairball/tts/`
- archive non-secret docs that do not have a direct Hairball destination
...
- `scripts/openclaw_to_hairball.py`

When this skill is installed from the Skills Hub, the normal location is:

- `~/.hairball/skills/migration/openclaw-migration/scripts/openclaw_to_hairball.py`

Do not guess a shorter path like `~/.hairball/skills/openclaw-migration/...`.

Before running the helper:

1. Prefer the installed path under `~/.hairball/skills/migration/openclaw-migration/`.
...
With `--migrate-secrets`, it will also import a small allowlisted set of Hairball-compatible secrets, currently:
...
Hairball CLI supports the `clarify` tool for interactive prompts, but it is limited to:
...
- `{"question":"One or more imported legacy skills already exist in Hairball. How should I handle those skill conflicts?","choices":["keep existing skills","overwrite conflicting skills with backup","import conflicting skills under renamed folders"]}`
...
- `{"question":"Do you want to copy the legacy workspace instructions file into a Hairball workspace?","choices":["skip workspace instructions","copy to a workspace path","decide later"]}`
...
13. If a renamed imported skill has an empty `details.backup`, do not imply the existing Hairball skill was renamed or backed up. Say only that the imported copy was placed in the new destination and reference `details.renamed_from` as the pre-existing folder that remained in place.
...
```bash
python3 ~/.hairball/skills/migration/openclaw-migration/scripts/openclaw_to_hairball.py
```

When using the terminal tool, prefer an absolute invocation pattern such as:

```json
{"command":"python3 /home/USER/.hairball/skills/migration/openclaw-migration/scripts/openclaw_to_hairball.py","workdir":"/home/USER"}
```

Dry run with the user-data preset:

```bash
python3 ~/.hairball/skills/migration/openclaw-migration/scripts/openclaw_to_hairball.py --preset user-data
```

Execute a user-data migration:

```bash
python3 ~/.hairball/skills/migration/openclaw-migration/scripts/openclaw_to_hairball.py --execute --preset user-data --skill-conflict skip
```

Execute a full compatible migration:

```bash
python3 ~/.hairball/skills/migration/openclaw-migration/scripts/openclaw_to_hairball.py --execute --preset full --migrate-secrets --skill-conflict skip
```

Execute with workspace instructions included:

```bash
python3 ~/.hairball/skills/migration/openclaw-migration/scripts/openclaw_to_hairball.py --execute --preset user-data --skill-conflict rename --workspace-target "/absolute/workspace/path"
```
...
2. Do not migrate secrets by default. Tokens, auth blobs, device credentials, and raw gateway config should stay out of Hairball unless the user explicitly asks for secret migration.
3. Do not silently overwrite non-empty Hairball targets unless the user explicitly wants that. The helper script will preserve backups when overwriting is enabled.
4. Always give the user the skipped-items report. That report is part of the migration, not an optional extra.
5. Prefer the primary legacy workspace (`~/.openclaw/workspace/`) over `workspace.default/`. Only use the default workspace as fallback when the primary files are missing.
6. Even in secret-migration mode, only migrate secrets with a clean Hairball destination. Unsupported auth blobs must still be reported as skipped.
...
- Hairball persona state imported
- Hairball memory files populated with converted legacy knowledge
- legacy skills available under `~/.hairball/skills/openclaw-imports/`
