#!/usr/bin/env python3
"""legacy -> Hairball migration helper.

This script migrates the parts of an legacy user footprint that map cleanly
into Hairball Agent, archives selected unmapped docs for manual review, and
...
SKILL_CATEGORY_DESCRIPTION = (
    "Skills migrated from an legacy workspace."
)
...
MIGRATION_OPTION_METADATA: Dict[str, Dict[str, str]] = {
    "soul": {
        "label": "SOUL.md",
        "description": "Import the legacy persona file into Hairball.",
    },
    "workspace-agents": {
        "label": "Workspace instructions",
        "description": "Copy the legacy workspace instructions file into a chosen workspace.",
    },
    "memory": {
        "label": "MEMORY.md",
        "description": "Import long-term memory entries into Hairball memories.",
    },
    "user-profile": {
        "label": "USER.md",
        "description": "Import user profile entries into Hairball memories.",
    },
    "messaging-settings": {
        "label": "Messaging settings",
        "description": "Import Hairball-compatible messaging settings such as allowlists and working directory.",
    },
    "secret-settings": {
        "label": "Allowlisted secrets",
        "description": "Import the small allowlist of Hairball-compatible secrets when explicitly enabled.",
    },
    "command-allowlist": {
        "label": "Command allowlist",
        "description": "Merge legacy exec approval patterns into Hairball command_allowlist.",
    },
    "skills": {
        "label": "User skills",
        "description": "Copy legacy skills into ~/.hairball/skills/openclaw-imports/.",
    },
    "tts-assets": {
        "label": "TTS assets",
        "description": "Copy compatible workspace TTS assets into ~/.hairball/tts/.",
    },
    "discord-settings": {
        "label": "Discord settings",
        "description": "Import Discord bot token and allowlist into Hairball .env.",
    },
    "slack-settings": {
        "label": "Slack settings",
        "description": "Import Slack bot/app tokens and allowlist into Hairball .env.",
    },
    "whatsapp-settings": {
        "label": "WhatsApp settings",
        "description": "Import WhatsApp allowlist into Hairball .env.",
    },
    "signal-settings": {
        "label": "Signal settings",
        "description": "Import Signal account, HTTP URL, and allowlist into Hairball .env.",
    },
    "provider-keys": {
        "label": "Provider API keys",
        "description": "Import model provider API keys into Hairball .env (requires --migrate-secrets).",
    },
    "model-config": {
        "label": "Default model",
        "description": "Import the default model setting into Hairball config.yaml.",
    },
    "tts-config": {
        "label": "TTS configuration",
        "description": "Import TTS provider and voice settings into Hairball config.yaml.",
    },
    "shared-skills": {
        "label": "Shared skills",
        "description": "Copy shared legacy skills from ~/.openclaw/skills/ into Hairball.",
    },
    "daily-memory": {
        "label": "Daily memory files",
        "description": "Merge daily memory entries from workspace/memory/ into Hairball MEMORY.md.",
    },
    "archive": {
    ...
    "mcp-servers": {
        "label": "MCP servers",
        "description": "Import MCP server definitions from legacy into Hairball config.yaml.",
    },
    "plugins-config": {
        "label": "Plugins configuration",
        "description": "Archive legacy plugin configuration and installed extensions for manual review.",
    },
    "cron-jobs": {
        "label": "Cron / scheduled tasks",
        "description": "Import cron job definitions. Archive for manual recreation via 'hairball cron'.",
    },
    "hooks-config": {
        "label": "Hooks and webhooks",
        "description": "Archive legacy hook configuration (internal hooks, webhooks, Gmail integration).",
    },
    "agent-config": {
        "label": "Agent defaults and multi-agent setup",
        "description": "Import agent defaults (compaction, context, thinking) into Hairball config. Archive multi-agent list.",
    },
    "gateway-config": {
    ...
    "session-config": {
        ...
        "description": "Import session reset policies (daily/idle) into Hairball session_reset config.",
    },
    "full-providers": {
        "label": "Full model provider definitions",
        "description": "Import custom model providers (baseUrl, apiType, headers) into Hairball custom_providers.",
    },
    "deep-channels": {
    ...
    "browser-config": {
        ...
        "description": "Import browser automation settings into Hairball config.yaml.",
    },
    "tools-config": {
        "label": "Tools configuration",
        "description": "Import tool settings (exec timeout, sandbox, web search) into Hairball config.yaml.",
    },
    "approvals-config": {
        "label": "Approval rules",
        "description": "Import approval mode and rules into Hairball config.yaml approvals section.",
    },
    "memory-backend": {
        "label": "Memory backend configuration",
        "description": "Archive legacy memory backend settings (QMD, vector search, citations) for manual review.",
    },
    "skills-config": {
        "label": "Skills registry configuration",
        "description": "Archive per-skill enabled/config/env settings from legacy skills.entries.",
    },
    "ui-identity": {
        "label": "UI and identity settings",
        "description": "Archive legacy UI theme, assistant identity, and display preferences.",
    },
    "logging-config": {
        "label": "Logging and diagnostics",
        "description": "Archive legacy logging and diagnostics configuration.",
    },
}
...
# Inspired by legacy's src/plugin-sdk/migration.ts so both sides speak the
...
def resolve_secret_input(value: Any, env: Optional[Dict[str, str]] = None) -> Optional[str]:
    """Resolve an legacy SecretInput value to a plain string.
...
def dump_yaml_file(path: Path, data: Dict[str, Any]) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required to update Hairball config.yaml")
    ensure_parent(path)
    path.write_text(
...
# ── Brand rewriting ─────────────────────────────────────────
# Replace legacy brand names with Hairball in migrated text so that
# memory entries, user profiles, SOUL.md, and workspace instructions
# read as self-referential to the new agent identity.
#
# Case-preserving: ``legacy`` → ``Hairball`` (prose), but lowercase matches
# like ``openclaw`` → ``hairball`` (so filesystem paths like ``~/.openclaw``
# become ``~/.hairball`` — the real Hairball home — not the broken ``~/.Hairball``).
_REBRAND_PATTERNS: List[Tuple[re.Pattern, str]] = [
    (re.compile(r'\bOpen[\s-]?Claw\b', re.IGNORECASE), 'Hairball'),
    (re.compile(r'\bClawdBot\b', re.IGNORECASE), 'Hairball'),
    (re.compile(r'\bMoltBot\b', re.IGNORECASE), 'Hairball'),
]
...
def _case_preserving_replacement(replacement: str):
    ...
    Keeps ``legacy`` → ``Hairball`` but maps ``openclaw`` → ``hairball`` so a
    filesystem path like ``~/.openclaw/config.yaml`` rewrites to
    ``~/.hairball/config.yaml`` (the real Hairball home) instead of the broken
    ``~/.Hairball/config.yaml``.
    """
...
def rebrand_text(text: str) -> str:
    """Replace legacy / ClawdBot / MoltBot brand names with Hairball.
...
#
# Modelled on legacy's src/plugin-sdk/migration.ts so both migration tools
...
def write_report(output_dir: Path, report: Dict[str, Any]) -> None:
    ...
    lines = [
        "# legacy -> Hairball Migration Report",
        "",
...
class Migrator:
    def __init__(
        ...
        # legacy's extensions/migrate-hairball/apply.ts "blocked by earlier
        ...
        # Many users (especially those who started before the legacy rebrand)
    ...
    # Option ids that mutate the Hairball config.yaml file.  Once any one of
    ...
    def source_candidate(self, *relative_paths: str) -> Optional[Path]:
        for rel in relative_paths:
            ...
            if candidate.exists():
                return candidate
            # legacy renamed workspace/ to workspace-main/ (and workspace-{agentId}
        ...
        # before the legacy rebrand (when the project was named clawd /
    ...
    def migrate(self) -> Dict[str, Any]:
        if not self.source_root.exists():
            self.record("source", self.source_root, None, "error", "legacy directory does not exist")
    ...
    def _build_warnings(self, summary: Dict[str, int]) -> List[str]:
        ...
        Modelled on legacy's extensions/migrate-hairball/plan.ts warnings[].
    ...
    def _build_next_steps(self, summary: Dict[str, int]) -> List[str]:
        ...
        if summary.get("migrated", 0) > 0:
            ...
            steps.append(
                "Start a new Hairball session (or /reset) to pick up the imported config."
            )
        if summary.get("conflict", 0) > 0:
            steps.append(
    ...
    def migrate_soul(self) -> None:
        ...
        if not source:
            self.record("soul", None, self.target_root / "SOUL.md", "skipped", "No legacy SOUL.md found")
            return
    ...
    def migrate_command_allowlist(self) -> None:
        ...
        if not source.exists():
            self.record("command-allowlist", None, destination, "skipped", "No legacy exec approvals file found")
            return
        if yaml is None:
        ...
        if not destination.exists():
            self.record("command-allowlist", source, destination, "skipped", "Hairball config.yaml does not exist yet")
            return
    ...
    def load_openclaw_env(self) -> Dict[str, str]:
        """Load the legacy .env file for secrets that live there instead of config."""
    ...
    def migrate_messaging_settings(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if isinstance(workspace, str) and workspace.strip():
            ...
            # Skip if the workspace points inside the legacy source directory —
            # that path will be stale after migration and would cause the Hairball
            # gateway to use the old legacy workspace as its cwd, picking up
            # legacy's AGENTS.md, MEMORY.md, etc.
            try:
        ...
        if additions:
            ...
            self.record("messaging-settings", self.source_root / "openclaw.json", self.target_root / ".env", "skipped", "No Hairball-compatible messaging settings found")
    ...
    def migrate_secret_settings(self, config: Dict[str, Any]) -> None:
        ...
        if secret_additions:
            ...
            self.record(
                ...
                self.target_root / ".env",
                "skipped",
                "No allowlisted Hairball-compatible secrets found",
                supported_targets=sorted(SUPPORTED_SECRET_TARGETS),
            )
    ...
    def migrate_provider_keys(self, config: Dict[str, Any]) -> None:
                ...
                if not api_key:
                    ...
                    if isinstance(raw_key, dict) and raw_key.get("source") in {"file", "exec"}:
                        self.record(
                            ...
                            f"that cannot be auto-migrated. Add this key manually via: hairball config set",
                        )
                    continue
        ...
        # Also check the legacy .env file — many users store keys there
        ...
        for oc_key, hairball_key in env_key_mapping.items():
            val = openclaw_env.get(oc_key, "").strip()
            if val and hairball_key not in secret_additions:
                secret_additions[hairball_key] = val

        # Check the openclaw.json "env" sub-object — some legacy setups
        ...
        if isinstance(json_env, dict):
            ...
            for src in sources:
                for oc_key, hairball_key in env_key_mapping.items():
                    val = src.get(oc_key)
                    if isinstance(val, str) and val.strip() and hairball_key not in secret_additions:
                        secret_additions[hairball_key] = val.strip()
    ...
    def migrate_model_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if model_value is None:
            self.record("model-config", source_path, destination, "skipped", "No default model found in legacy config")
            return
        ...
        # Resolve a model alias against the legacy model catalog.
        # legacy stores agents.defaults.model as either a bare string or
        ...
        hairball_config = load_yaml_file(destination)
        current_model = hairball_config.get("model")
        ...
        if self.execute:
            backup_path = self.maybe_backup(destination)
            existing_model = hairball_config.get("model")
            if isinstance(existing_model, dict):
                existing_model["default"] = model_str
            else:
                hairball_config["model"] = {"default": model_str}
            dump_yaml_file(destination, hairball_config)
    ...
    def migrate_tts_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not isinstance(tts, dict) or not tts:
            self.record("tts-config", source_path, destination, "skipped", "No TTS configuration found in legacy config")
            return
        ...
        if isinstance(provider, str) and provider in {"elevenlabs", "openai", "edge", "microsoft"}:
            # legacy renamed "edge" to "microsoft"; Hairball still uses "edge"
        ...
        # in legacy (not messages.tts.elevenlabs directly)
        ...
        hairball_config = load_yaml_file(destination)
        existing_tts = hairball_config.get("tts", {})
        ...
        if self.execute:
            ...
            hairball_config["tts"] = merged_tts
            dump_yaml_file(destination, hairball_config)
    ...
    def migrate_shared_skills(self) -> None:
        # Check all legacy skill sources: managed, personal, project-level
        skill_sources = [
        ...
        if not found_any:
            ...
            self.record("shared-skills", None, destination_root, "skipped", "No shared legacy skills directories found")
    ...
    def migrate_skills(self) -> None:
        ...
        if not source_root or not source_root.exists():
            self.record("skills", None, destination_root, "skipped", "No legacy skills directory found")
            return
    ...
    def archive_docs(self) -> None:
        ...
        for candidate in candidates:
            if candidate:
                self.archive_path(candidate, reason="No direct Hairball destination; archived for manual review")
        ...
        partially_extracted = [
            ("openclaw.json", "Selected Hairball-compatible values were extracted; raw legacy config was not copied."),
            ("credentials/telegram-default-allowFrom.json", "Selected Hairball-compatible values were extracted; raw credentials file was not copied."),
        ]
    ...
    def migrate_mcp_servers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        if not mcp_raw:
            self.record("mcp-servers", None, None, "skipped", "No MCP servers found in legacy config")
            return

        hairball_cfg_path = self.target_root / "config.yaml"
        hairball_cfg = load_yaml_file(hairball_cfg_path)
        existing_mcp = hairball_cfg.get("mcp_servers") or {}
        added = 0
        ...
        for name, srv in mcp_raw.items():
            ...
            if name in existing_mcp and not self.overwrite:
                self.record("mcp-servers", f"mcp.servers.{name}", f"mcp_servers.{name}", "conflict",
                            "MCP server already exists in Hairball config")
                continue

            hairball_srv: Dict[str, Any] = {}
            # STDIO transport
            if srv.get("command"):
                hairball_srv["command"] = srv["command"]
                if srv.get("args"):
                    hairball_srv["args"] = srv["args"]
                if srv.get("env"):
                    hairball_srv["env"] = srv["env"]
                if srv.get("cwd"):
                    hairball_srv["cwd"] = srv["cwd"]
            # HTTP/SSE transport
            if srv.get("url"):
                hairball_srv["url"] = srv["url"]
                if srv.get("headers"):
                    hairball_srv["headers"] = srv["headers"]
                if srv.get("auth"):
                    hairball_srv["auth"] = srv["auth"]
            # Common fields
            if srv.get("enabled") is False:
                hairball_srv["enabled"] = False
            if srv.get("timeout"):
                hairball_srv["timeout"] = srv["timeout"]
            if srv.get("connectTimeout"):
                hairball_srv["connect_timeout"] = srv["connectTimeout"]
            # Tool filtering
            tools_cfg = srv.get("tools") or {}
            if tools_cfg.get("include") or tools_cfg.get("exclude"):
                hairball_srv["tools"] = {}
                if tools_cfg.get("include"):
                    hairball_srv["tools"]["include"] = tools_cfg["include"]
                if tools_cfg.get("exclude"):
                    hairball_srv["tools"]["exclude"] = tools_cfg["exclude"]
            # Sampling
            sampling = srv.get("sampling")
            if sampling and isinstance(sampling, dict):
                hairball_srv["sampling"] = {
                    k: v for k, v in {
            ...
            existing_mcp[name] = hairball_srv
            added += 1
        ...
        if added > 0 and self.execute:
            self.maybe_backup(hairball_cfg_path)
            hairball_cfg["mcp_servers"] = existing_mcp
            dump_yaml_file(hairball_cfg_path, hairball_cfg)
        ...
        if cron:
            ...
            if self.archive_dir and self.execute:
                ...
                self.record("cron-jobs", "openclaw.json cron.*", str(dest), "archived",
                            "Cron config archived. Use 'hairball cron' to recreate jobs manually.")
            else:
    ...
    def migrate_agent_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map agent defaults
        agent_cfg = hairball_cfg.get("agent") or {}
        ...
        if defaults.get("thinkingDefault"):
            # Map legacy thinking -> Hairball reasoning_effort
        ...
        if compaction:
            compression = hairball_cfg.get("compression") or {}
            ...
            if compaction.get("model"):
                aux = hairball_cfg.setdefault("auxiliary", {})
                aux_comp = aux.setdefault("compression", {})
                aux_comp["model"] = compaction["model"]
            hairball_cfg["compression"] = compression
            changes = True

        # Map humanDelay
        human_delay = defaults.get("humanDelay") or {}
        if human_delay:
            hd = hairball_cfg.get("human_delay") or {}
    ...
    def migrate_gateway_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        self.record("gateway-config", "openclaw.json gateway.*", "archive/gateway-config.json",
                    "archived", "Gateway config archived. Use 'hairball gateway' to configure.")

        # Extract gateway auth token to .env if present
        auth = gateway.get("auth") or {}
        if auth.get("token") and self.migrate_secrets:
            self._set_env_var("HAIRBALL_GATEWAY_TOKEN", auth["token"], "gateway.auth.token")
    ...
    def migrate_session_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        hairball_cfg_path = self.target_root / "config.yaml"
        hairball_cfg = load_yaml_file(hairball_cfg_path)
        sr = hairball_cfg.get("session_reset") or {}
        changes = False

        # legacy uses session.reset (structured) and session.resetTriggers (string array)
    ...
    def migrate_full_providers(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        hairball_cfg_path = self.target_root / "config.yaml"
        hairball_cfg = load_yaml_file(hairball_cfg_path)
        custom_providers = hairball_cfg.get("custom_providers") or []
        added = 0
        ...
        if added > 0 and self.execute:
            self.maybe_backup(hairball_cfg_path)
            hairball_cfg["custom_providers"] = custom_providers
            dump_yaml_file(hairball_cfg_path, hairball_cfg)
    ...
    def migrate_deep_channels(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map Discord-specific settings to Hairball config
        discord_cfg = channels.get("discord") or {}
        if discord_cfg:
            hairball_cfg_path = self.target_root / "config.yaml"
            hairball_cfg = load_yaml_file(hairball_cfg_path)
            discord_hairball = hairball_cfg.get("discord") or {}
            changed = False
            if "requireMention" in discord_cfg:
                discord_hairball["require_mention"] = discord_cfg["requireMention"]
                changed = True
            if discord_cfg.get("autoThread") is not None:
                discord_hairball["auto_thread"] = discord_cfg["autoThread"]
                changed = True
            if changed and self.execute:
                hairball_cfg["discord"] = discord_hairball
                dump_yaml_file(hairball_cfg_path, hairball_cfg)
    ...
    def migrate_browser_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        hairball_cfg_path = self.target_root / "config.yaml"
        hairball_cfg = load_yaml_file(hairball_cfg_path)
        browser_hairball = hairball_cfg.get("browser") or {}
        changed = False

        # Map fields that have Hairball equivalents
        if browser.get("cdpUrl"):
            browser_hairball["cdp_url"] = browser["cdpUrl"]
            changed = True
        if browser.get("headless") is not None:
            browser_hairball["headless"] = browser["headless"]
            changed = True

        if changed:
            hairball_cfg["browser"] = browser_hairball
            if self.execute:
                self.maybe_backup(hairball_cfg_path)
                dump_yaml_file(hairball_cfg_path, hairball_cfg)
    ...
    def migrate_tools_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map exec timeout -> terminal timeout (field is timeoutSec in legacy)
        ...
        # Map web search API key (path: tools.web.search.brave.apiKey in legacy)
    ...
    def migrate_approvals_config(self, config: Optional[Dict[str, Any]] = None) -> None:
        ...
        # Map approval mode (nested under approvals.exec.mode in legacy)
        ...
        if mode:
            ...
            hairball_mode = mode_map.get(mode, "manual")
            hairball_cfg.setdefault("approvals", {})["mode"] = hairball_mode
            if self.execute:
                self.maybe_backup(hairball_cfg_path)
                dump_yaml_file(hairball_cfg_path, hairball_cfg)
            self.record("approvals-config", "openclaw.json approvals.mode",
                        "config.yaml approvals.mode", "migrated", f"Mapped '{mode}' -> '{hairball_mode}'")
    ...
    def generate_migration_notes(self) -> None:
        if not self.output_dir:
            return
        notes = [
            "# legacy -> Hairball Migration Notes",
            "",
        ...
        if archived:
            notes.extend([
                ...
                "",
                "These legacy configurations were archived because they don't have a",
                "direct 1:1 mapping in Hairball. Review each file and recreate manually:",
                "",
            ])
        ...
        if conflicts:
            notes.extend([
                "## Conflicts (Existing Hairball Config Not Overwritten)",
                "",
                "These items already existed in your Hairball config. Re-run with",
        ...
        notes.extend([
            "## IMPORTANT: Archive the legacy Directory",
            "",
            "After migration, your legacy directory still exists on disk with workspace",
            "state files (todo.json, sessions, logs). If the Hairball agent discovers these",
            "directories, it may read/write to them instead of the Hairball state, causing",
            "confusion (e.g., cron jobs reading a different todo list than interactive sessions).",
            "",
            "**Strongly recommended:** Run `hairball claw cleanup` to rename the legacy",
            ...
            "or todo lists, run `hairball claw cleanup` to fix it.",
            "",
            "## Hairball-Specific Setup",
            "",
            "After migration, you may want to:",
            "- Run `hairball claw cleanup` to archive the legacy directory (prevents state confusion)",
            "- Run `hairball setup` to configure any remaining settings",
            "- Run `hairball mcp list` to verify MCP servers were imported correctly",
        ])

        if has_cron_config_archive:
            notes.append("- Run `hairball cron` to recreate scheduled tasks (see archive/cron-config.json)")
        elif has_cron_store_archive:
            notes.append("- Run `hairball cron` to recreate scheduled tasks (see archived cron-store)")
        ...
        notes.extend([
            "- Run `hairball gateway install` if you need the gateway service",
            "- Review `~/.hairball/config.yaml` for any adjustments",
            "",
        ])
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Migrate legacy user state into Hairball Agent.")
    parser.add_argument("--source", default=str(Path.home() / ".openclaw"), help="legacy home directory")
    parser.add_argument("--target", default=os.environ.get("HAIRBALL_HOME") or str(Path.home() / ".hairball"), help="Hairball home directory")
    parser.add_argument(
        "--workspace-target",
    ...
    parser.add_argument("--overwrite", action="store_true", help="Overwrite existing Hairball targets after backing them up")
    parser.add_argument(
        "--migrate-secrets",
        action="store_true",
        help="Import a narrow allowlist of Hairball-compatible secrets into the target env file",
    )
    parser.add_argument(
        "--skill-conflict",
...
def main() -> int:
    ...
    print()
    print("  ╔══════════════════════════════════════════════════════╗")
    print(f"  ║   legacy -> Hairball Migration   [{mode_label:>8s}]   ║")
    print("  ╠══════════════════════════════════════════════════════╣")
    ...
    if migrated:
        ...
        for item in migrated:
            ...
            if dest.startswith(str(report["target_root"])):
                dest = "~/.hairball/" + dest[len(str(report["target_root"])) + 1:]
    ...
    if args.execute:
        print()
        print("  Next steps:")
        print("    1. Review ~/.hairball/config.yaml")
        print("    2. Run: hairball mcp list")
        if any(i["kind"] == "cron-jobs" and i["status"] == "archived" for i in items):
            print("    3. Recreate cron jobs: hairball cron")
