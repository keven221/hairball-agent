...
- Skill helper paths:
  - `${HAIRBALL_SKILL_DIR}/scripts/publish.sh` for publishing sites
  - `${HAIRBALL_SKILL_DIR}/scripts/drive.sh` for private Drive storage

## Create a site

```bash
PUBLISH="${HAIRBALL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --client hairball
```
...
```bash
PUBLISH="${HAIRBALL_SKILL_DIR}/scripts/publish.sh"
bash "$PUBLISH" {file-or-dir} --slug {slug} --client hairball
```
...
```bash
DRIVE="${HAIRBALL_SKILL_DIR}/scripts/drive.sh"
bash "$DRIVE" default
...
| `--client {name}`      | Agent name for attribution (e.g. `hairball`)    |
