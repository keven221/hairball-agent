#!/usr/bin/env python3
"""Canvas LMS API CLI for Hairball Agent.
...
def _check_config():
    ...
    if missing:
        hairball_env = os.path.join(
            os.environ.get("HAIRBALL_HOME", os.path.expanduser("~/.hairball")), ".env"
        )
        print(
            f"Missing required environment variables: {', '.join(missing)}\n"
            f"Set them in {hairball_env} or export them in your shell.\n"
...
def main():
    parser = argparse.ArgumentParser(
        description="Canvas LMS API CLI for Hairball Agent"
    )
