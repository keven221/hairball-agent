...
$HAIRBALL_OSINT_CACHE/icij/ (default: ~/.cache/hairball-osint/icij/) and search
...
def _cache_dir() -> Path:
    base = os.environ.get("HAIRBALL_OSINT_CACHE")
    if base:
        return Path(base) / "icij"
    return Path.home() / ".cache" / "hairball-osint" / "icij"
...
def _download(dest: Path, force: bool = False) -> Path:
    ...
    req = urllib.request.Request(
        BULK_URL,
        headers={"User-Agent": "hairball-agent osint-investigation skill"},
    )
...
def main() -> int:
    ...
    p.add_argument(
        ...
        type=Path,
        default=None,
        help="Override cache directory (default: $HAIRBALL_OSINT_CACHE/icij or ~/.cache/hairball-osint/icij)",
    )
    p.add_argument(
        "--force-refresh",
        action="store_true",
