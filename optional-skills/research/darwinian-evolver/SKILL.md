...
version: 0.1.0
author: Bihruze (Asahi0x), Hairball Agent
license: MIT
...
import upstream classes into Hairball itself.
...
```bash
mkdir -p ~/.hairball/cache/darwinian-evolver && cd ~/.hairball/cache/darwinian-evolver
...
```bash
cd ~/.hairball/cache/darwinian-evolver/darwinian_evolver \
...
Open `~/.hairball/cache/darwinian-evolver/darwinian_evolver/darwinian_evolver/lineage_visualizer.html`
...
SKILL_DIR=~/.hairball/skills/research/darwinian-evolver
DE_DIR=~/.hairball/cache/darwinian-evolver/darwinian_evolver
...
7. **AGPL.** Never `from darwinian_evolver import ...` inside Hairball core.
   Custom driver scripts under `~/.hairball/skills/...` are user-side and fine.
