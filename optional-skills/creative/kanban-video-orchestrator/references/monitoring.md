...
hairball kanban watch --tenant <project-slug>

# Snapshot of the board
hairball kanban list --tenant <project-slug>
hairball kanban list --tenant <project-slug> --json     # machine-readable

# Per-status counts + oldest-ready age
hairball kanban stats --tenant <project-slug>

# Visual dashboard (browser)
hairball dashboard

# Inspect a specific task (includes comments + events)
hairball kanban show <task-id>

# Follow a single task's event stream
hairball kanban tail <task-id>
```

Verify available subcommands with `hairball kanban --help` — the kanban CLI
...
|---------|--------------|--------|
| Task RUNNING but no heartbeat in 2+ min | Worker stuck, infinite loop, blocked on input | `hairball kanban show <id>` — read the worker's last events. The dispatcher SIGTERMs tasks that exceed their `max-runtime`; if you need to stop one earlier, `hairball kanban block <id>` then `hairball kanban archive <id>`, and create a re-run task. |
| Same task retried 2+ times | Reproducible failure (missing key, bad spec, broken tool) | `hairball kanban show <id>` to read failure events. Fix root cause before re-running. |
| RUNNING longer than max_runtime | Task is slow but progressing OR genuinely stuck | Check heartbeats with `hairball kanban tail <id>`. If progressing, the dispatcher will SIGTERM eventually anyway — raise `max-runtime` on a re-created task. |
...
| Tasks created but never picked up | Profile not running, or tenant mismatch, or dispatcher not running | Check `hairball profile list` (profile exists?), `hairball status` (gateway/dispatcher up?), and verify tenant. |
...
hairball kanban comment <renderer-task-id> "Scene 3 looks too sparse \
— increase visual density. Tighten color palette to brand spec."

# 2. Create a re-render task with the original as parent
hairball kanban create "Scene 3 — re-render with feedback" \
...
NEW_TASK_ID=$(hairball kanban create "Generate SRT captions from voiceover" \
    --assignee captioner \
    --workspace dir:"$HOME/projects/video-pipeline/<slug>" \
    --tenant <slug> \
...
hairball kanban link "$NEW_TASK_ID" <editor-task-id>
```
...
hairball kanban block <task-id>
hairball kanban archive <task-id>

# Diagnose what happened
hairball kanban show <task-id>      # task body, comments, recent events
hairball kanban tail <task-id>      # follow the live event stream
hairball kanban log <task-id>       # worker process log
```
...
3. Re-fire the initial `hairball kanban create` for the director
...
while true; do
    clear
    hairball kanban list --tenant <slug>
    echo "---"
    hairball kanban stats --tenant <slug>
    sleep 30
done
```

For a live event feed, run `hairball kanban watch --tenant <slug>` in a
...
- **Profile process not running.** Tasks queue indefinitely in READY if no
  worker for that profile is online. Check `hairball profile list` and start
