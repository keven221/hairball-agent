...
the actual `setup.sh` that creates the project workspace, configures Hairball
...
> [Hairball/kanban-video-pipeline](https://github.com/Hairball/kanban-video-pipeline).
...
2. **Create profiles** — `hairball profile create <name> --clone`
3. **Configure profiles** — patch each profile's
   `~/.hairball/profiles/<name>/config.yaml` to set toolsets, always_load skills,
   and `cwd`
4. **Write SOUL.md per profile** — the personality + role definition
5. **Copy any provided assets + write `brief.md`, `TEAM.md`, and `taste/`**
6. **Fire the initial kanban task** — `hairball kanban create` assigned to the director
...
```bash
hairball profile create director --clone 2>/dev/null || true
```
...
Each profile has a YAML config at `~/.hairball/profiles/<name>/config.yaml`. The
...
p = os.path.expanduser(f"~/.hairball/profiles/{profile}/config.yaml")
with open(p) as f:
...
PyYAML must be installed in the user's Python (it ships with most Hairball
...
Each profile gets a `SOUL.md` at `~/.hairball/profiles/<name>/SOUL.md` that
...
```bash
hairball kanban create "Direct production of <video title>" \
    --assignee director \
...
the Hairball `.env` (`${HAIRBALL_HOME:-$HOME/.hairball}/.env`) and macOS Keychain
(if on macOS):
...
check_key() {
    ...
    local kc_account="$2"
    local kc_service="$3"
    local _hairball_env="${HAIRBALL_HOME:-$HOME/.hairball}/.env"
    if grep -q "^${var}=" "$_hairball_env" 2>/dev/null && \
       [ -n "$(grep "^${var}=" "$_hairball_env" | cut -d= -f2-)" ]; then
        return 0
    fi
    ...
    echo "ERROR: ${var} not set in ${_hairball_env} or Keychain (${kc_account}/${kc_service})"
    return 1
}

check_key ELEVENLABS_API_KEY hairball ELEVENLABS_API_KEY || exit 1
check_key OPENROUTER_API_KEY hairball OPENROUTER_API_KEY || exit 1
# ...
```
