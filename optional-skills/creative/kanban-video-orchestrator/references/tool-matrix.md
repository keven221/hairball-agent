...
Maps each role archetype to the Hairball skills it should `always_load` and the
toolsets it needs. Only references skills that ship in the public hairball-agent
...
## Hairball skills relevant to video production

### Visual / rendering skills (`hairball-agent/skills/creative/`)
...
### Design / pre-production skills (`hairball-agent/skills/creative/`)
...
### Audio / media skills (`hairball-agent/skills/creative/` + `skills/media/`)
...
These are **not** Hairball skills but external CLIs / APIs that profiles invoke.
...
## Built-in Hairball tools for media review

These are native Hairball tools — not invoked via terminal but through their own
...
key is present in `${HAIRBALL_HOME:-~/.hairball}/.env` (or macOS Keychain) before firing the kanban.
...
| Anthropic | `ANTHROPIC_API_KEY` | every Hairball profile (Claude) |

If a key is missing, prompt the user to add it. Storage methods, in order of
preference: macOS Keychain → `${HAIRBALL_HOME:-~/.hairball}/.env` → environment variable.
...
When a new Hairball-public video skill ships:
