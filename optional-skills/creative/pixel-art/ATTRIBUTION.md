...
- `generate_video()` split: the Pollinations text-to-image call was removed
  (Hairball uses its own `image_generate` + `pixel_art()` pipeline for base
...
- Pollinations text-to-image generation (`pixelart_image.py`,
  `generate_base()` in `pixelart_video.py`) — Hairball has `image_generate`
...
- License: MIT (inherits from hairball-agent repo)
- Original author of the skill shell: dodo-reach
- Expansion with palettes + video: Hairball Agent contributors
