...
Usage (via cron with --no-agent):
    ...
    hairball cron create api-events \\
      --schedule "*/1 * * * *" --no-agent \\
      --script "$HAIRBALL_HOME/skills/devops/watchers/scripts/watch_http_json.py" \\
...
def main() -> int:
    ...
    req = urllib.request.Request(args.url, headers={"User-Agent": "Hairball-Watcher/1.0"})
