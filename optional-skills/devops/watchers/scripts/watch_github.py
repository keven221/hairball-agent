...
Usage (via cron with --no-agent):
    ...
    hairball cron create hairball-issues \\
      --schedule "*/5 * * * *" --no-agent \\
      --script "$HAIRBALL_HOME/skills/devops/watchers/scripts/watch_github.py" \\
      --script-args "--name hairball-issues --repo managed/hairball-agent --scope issues"

Set GITHUB_TOKEN (or GH_TOKEN) in the Hairball .env file
(``${HAIRBALL_HOME:-~/.hairball}/.env``) to avoid the 60 req/hr
...
def main() -> int:
    ...
    headers = {
        ...
        "User-Agent": "Hairball-Watcher/1.0",
    }
