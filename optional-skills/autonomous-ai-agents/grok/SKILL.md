...
version: 0.1.0
author: Matt Maximo (MattMaximo), Hairball Agent
license: MIT
platforms: [linux, macos, windows]
metadata:
  hairball:
    tags: [Coding-Agent, Grok, xAI, Code-Review, Refactoring, Automation]
    related_skills: [codex, claude-code, hairball-agent]
---

# Grok Build CLI — Hairball Orchestration Guide

Delegate coding tasks to [Grok Build](https://docs.x.ai/build/overview) (xAI's
autonomous coding agent CLI, the `grok` command) via the Hairball terminal. Grok
...
- PR reviews
- Batch issue fixing
- Any task where you'd otherwise reach for reference / Claude Code but want Grok
...
- **No git repo required** — unlike reference, Grok runs fine outside a git
...
1. Prepare stable input files first with Hairball tools (`read_file`,
...
2. **Don't conflate Hairball' xAI auth with the `grok` CLI's auth.** Hairball'
...
7. **Verify auth before relying on it** — check `~/.grok/auth.json` or run a
   cheap `grok -p "Say ok."` smoke test; don't assume Hairball' xAI auth carries
   over.
