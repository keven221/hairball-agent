---
name: honcho
description: Configure and use Honcho memory with Hairball -- cross-session user modeling, multi-profile peer isolation, observation config, dialectic reasoning, session summaries, and context budget enforcement. Use when setting up Honcho, troubleshooting memory, managing profiles with Honcho peers, or tuning observation, recall, and dialectic settings.
version: 2.0.0
...
# Honcho Memory for Hairball

Honcho provides AI-native cross-session user modeling. It learns who the user is across conversations and gives every Hairball profile its own peer identity while sharing a unified view of the user.
...
See: https://docs.honcho.dev/v3/guides/integrations/hairball#running-honcho-locally-with-hairball

### Verify

```bash
hairball honcho status    # shows resolved config, connection test, peer info
```
...
3. **AI peer card** -- the identity card for this Hairball profile's AI peer
...
Honcho models conversations as interactions between **peers**. Hairball creates two peers per session:

- **User peer** (`peerName`): represents the human. Honcho builds a user representation from observed messages.
- **AI peer** (`aiPeer`): represents this Hairball instance. Each profile gets its own AI peer so agents develop independent views.
...
| `per-session` | New Honcho session each Hairball run |
| `global` | Single session across all directories |

Manual override: `hairball honcho map my-project-name`
...
Each Hairball profile gets its own Honcho AI peer while sharing the same workspace (user context). This means:
...
```bash
hairball profile create coder --clone
# creates host block hairball.coder, AI peer "coder", inherits config from default
```

What `--clone` does for Honcho:
1. Creates a `hairball.coder` host block in `honcho.json`
...
```bash
hairball honcho sync    # creates host blocks for all profiles that don't have one yet
```
...
Guidelines for Hairball when Honcho memory is active.
...
Config file: `$HAIRBALL_HOME/honcho.json` (profile-local) or `~/.honcho/config.json` (global).
...
Run `hairball honcho setup`. Ensure `memory.provider: honcho` is in `~/.hairball/config.yaml`.

### Memory not persisting across sessions
Check `hairball honcho status` -- verify `saveMessages: true` and `writeFrequency` isn't `session` (which only writes on exit).

### Profile not getting its own peer
Use `--clone` when creating: `hairball profile create <name> --clone`. For existing profiles: `hairball honcho sync`.
...
| Command | Description |
|---------|-------------|
| `hairball honcho setup` | Interactive setup wizard (cloud/local, identity, observation, recall, sessions) |
| `hairball honcho status` | Show resolved config, connection test, peer info for active profile |
| `hairball honcho enable` | Enable Honcho for the active profile (creates host block if needed) |
| `hairball honcho disable` | Disable Honcho for the active profile |
| `hairball honcho peer` | Show or update peer names (`--user <name>`, `--ai <name>`, `--reasoning <level>`) |
| `hairball honcho peers` | Show peer identities across all profiles |
| `hairball honcho mode` | Show or set recall mode (`hybrid`, `context`, `tools`) |
| `hairball honcho tokens` | Show or set token budgets (`--context <N>`, `--dialectic <N>`) |
| `hairball honcho sessions` | List known directory-to-session-name mappings |
| `hairball honcho map <name>` | Map current working directory to a Honcho session name |
| `hairball honcho identity` | Seed AI peer identity or show both peer representations |
| `hairball honcho sync` | Create host blocks for all Hairball profiles that don't have one yet |
| `hairball honcho migrate` | Step-by-step migration guide from legacy native memory to Hairball + Honcho |
| `hairball memory setup` | Generic memory provider picker (selecting "honcho" runs the same wizard) |
| `hairball memory status` | Show active memory provider and config |
| `hairball memory off` | Disable external memory provider |
