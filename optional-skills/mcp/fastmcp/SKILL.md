...
- smoke-test a server with the FastMCP CLI before wiring it into Hairball or another client
...
Use `native-mcp` when the server already exists and only needs to be connected to Hairball. Use `mcporter` when the goal is ad-hoc CLI access to an existing MCP server instead of building one.
...
```bash
python ~/.hairball/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py \
...
```bash
python ~/.hairball/skills/mcp/fastmcp/scripts/scaffold_fastmcp.py --list
```
...
When the goal is Hairball integration, either:

- configure the server in `~/.hairball/config.yaml` using the `native-mcp` skill, or
...
### Hairball cannot see the deployed server

The server-building part may be correct while the Hairball config is not. Load the `native-mcp` skill and configure the server in `~/.hairball/config.yaml`, then restart Hairball.
