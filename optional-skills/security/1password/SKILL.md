...
version: 1.0.0
author: arceus77-7, enhanced by Hairball Agent
license: MIT
...
- `tmux` available for stable authenticated sessions during Hairball terminal calls (desktop app flow only)
...
### Service Account (recommended for Hairball)

Set `OP_SERVICE_ACCOUNT_TOKEN` in `${HAIRBALL_HOME:-~/.hairball}/.env` (the skill will prompt for this on first load).
...
## Hairball Execution Pattern (desktop app flow)

Hairball terminal commands are non-interactive by default and can lose auth context between calls.
...
```bash
SOCKET_DIR="${TMPDIR:-/tmp}/hairball-tmux-sockets"
mkdir -p "$SOCKET_DIR"
SOCKET="$SOCKET_DIR/hairball-op.sock"
