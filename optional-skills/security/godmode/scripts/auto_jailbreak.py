...
Usage in execute_code:
    exec(open(os.path.expanduser(
        os.path.join(os.environ.get("HAIRBALL_HOME", os.path.expanduser("~/.hairball")), "skills/red-teaming/godmode/scripts/auto_jailbreak.py")
    )).read())
...
try:
    ...
    _SKILL_DIR = Path(os.getenv("HAIRBALL_HOME", Path.home() / ".hairball")) / "skills" / "red-teaming" / "godmode"
...
# ═══════════════════════════════════════════════════════════════════
# Hairball config paths
# ═══════════════════════════════════════════════════════════════════

HAIRBALL_HOME = Path(os.getenv("HAIRBALL_HOME", Path.home() / ".hairball"))
CONFIG_PATH = HAIRBALL_HOME / "config.yaml"
PREFILL_PATH = HAIRBALL_HOME / "prefill.json"
...
MODEL_STRATEGIES = {
    ...
    # managed/Hairball models — already uncensored, just needs clean prompt
    "hairball": {
        "order": ["prefill_only"],
        "system_templates": {},
    },
...
def _detect_model_family(model: str) -> str:
    ...
    if "grok" in model_lower or "x-ai" in model_lower:
        return "grok"
    if "hairball" in model_lower or "managed" in model_lower:
        return "hairball"
...
def _get_current_model() -> tuple:
    """Read current model and provider from Hairball config.yaml.
...
def _write_prefill(prefill_messages: list):
    """Write prefill messages to ~/.hairball/prefill.json."""
    ...
    if winning_strategy:
        ...
        if not dry_run:
            ...
            if verbose:
                ...
                print()
                print("[DONE] Jailbreak locked in. Restart Hairball for changes to take effect.")
        else:
            if verbose:
...
def undo_jailbreak(verbose=True):
    ...
    if verbose:
        print("[UNDO] Jailbreak removed. Restart Hairball for changes to take effect.")
