...
## 5. GODMODE FAST — Hairball 4 405B (Zero Refusal)

**Model:** `hairball/hairball-4-405b`
**Strategy:** Hairball models are already uncensored, so this combo is about speed — instant token streaming with zero refusal checking needed. Uses the same Pliny Love divider format as a formality.
...
## Using with Hairball Agent

### As ephemeral system prompt (config.yaml)

Pick a template above and set it in `~/.hairball/config.yaml`:
