...
All per-subject data lives under PDD_DATA_DIR (default: $HAIRBALL_HOME/unbroker),
which is the same trust boundary Hairball uses for .env and OAuth tokens.
"""
...
def hairball_home() -> Path:
    return Path(os.environ.get("HAIRBALL_HOME") or (Path.home() / ".hairball"))


def data_dir() -> Path:
    override = os.environ.get("PDD_DATA_DIR")
    return Path(override) if override else hairball_home() / "unbroker"
