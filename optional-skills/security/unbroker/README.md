...
The design is **Hairball-native**: a small deterministic Python CLI (`scripts/pdd.py`) owns the state
...
```bash
hairball skills install official/security/unbroker
```

Then start a new Hairball session and drive it (below). The skill works zero-config; a few optional
...
Drive it from a Hairball session:
...
- **PII handling.** Dossiers live under the Hairball home directory (`0600`, optionally
