...
## 5. Hairball Configuration

### Config location

`$HAIRBALL_HOME/config.yaml` (defaults to `~/.hairball/config.yaml` when `HAIRBALL_HOME` is unset)
...
Restart the Hairball session for changes to take effect. The MCP connection is
