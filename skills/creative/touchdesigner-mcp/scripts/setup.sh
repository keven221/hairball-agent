...
HAIRBALL_HOME_DIR="${HAIRBALL_HOME:-$HOME/.hairball}"
HAIRBALL_CFG="${HAIRBALL_HOME_DIR}/config.yaml"
MCP_PORT=40404
...
# ── 3. Ensure Hairball config has twozero_td MCP entry ──
if [[ ! -f "$HAIRBALL_CFG" ]]; then
    echo -e " ${FAIL} Hairball config not found at ${HAIRBALL_CFG}"
    manual_steps+=("Create ${HAIRBALL_CFG} with twozero_td MCP server entry")
elif grep -q 'twozero_td' "$HAIRBALL_CFG" 2>/dev/null; then
    echo -e " ${OK} twozero_td MCP entry exists in Hairball config"
else
    echo -e " ${WARN} Adding twozero_td MCP entry to Hairball config..."
    python3 -c "
import yaml, sys, copy

cfg_path = '$HAIRBALL_CFG'
...
" 2>/dev/null && echo -e " ${OK} twozero_td MCP entry added to config" \
              || { echo -e " ${FAIL} Could not update config (is PyYAML installed?)"; \
                   manual_steps+=("Add twozero_td MCP entry to ${HAIRBALL_CFG} manually"); }
    manual_steps+=("Restart Hairball session to pick up config change")
fi
