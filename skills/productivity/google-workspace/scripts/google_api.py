#!/usr/bin/env python3
"""Google Workspace API CLI for Hairball Agent.

Uses the Google Workspace CLI (`gws`) when available, but preserves the
existing Hairball-facing JSON contract and falls back to the Python client
...
from _hairball_home import get_hairball_home
...
def _gws_binary() -> str | None:
    override = os.getenv("HAIRBALL_GWS_BIN")
    if override:
        return override
...
def main():
    parser = argparse.ArgumentParser(description="Google Workspace API for Hairball Agent")
