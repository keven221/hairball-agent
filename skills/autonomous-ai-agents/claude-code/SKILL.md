...
metadata:
  hairball:
    ...
    related_skills: [codex, hairball-agent, opencode]
---

# Claude Code — Hairball Orchestration Guide

Delegate coding tasks to [Claude Code](https://code.claude.com/docs/en/cli-reference) (Anthropic's autonomous coding agent CLI) via the Hairball terminal. Claude Code v2.x can read files, write code, run shell commands, spawn subagents, and manage git workflows autonomously.
...
Hairball interacts with Claude Code in two fundamentally different ways. Choose based on the task.
...
1. **Interactive mode REQUIRES tmux** — Claude Code is a full TUI app. Using `pty=true` alone in Hairball terminal works but tmux gives you `capture-pane` for monitoring and `send-keys` for input, which is essential for orchestration.
