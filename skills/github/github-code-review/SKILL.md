...
if command -v gh &>/dev/null && gh auth status &>/dev/null; then
  ...
  if [ -z "$GITHUB_TOKEN" ]; then
    if _hairball_env="${HAIRBALL_HOME:-$HOME/.hairball}/.env"; [ -f "$_hairball_env" ] && grep -q "^GITHUB_TOKEN=" "$_hairball_env"; then
      GITHUB_TOKEN=$(grep "^GITHUB_TOKEN=" "$_hairball_env" | head -1 | cut -d= -f2 | tr -d '\n\r')
...
curl -s -X POST \
  ...
  -d "{
    \"commit_id\": \"$HEAD_SHA\",
    \"event\": \"COMMENT\",
    \"body\": \"Code review from Hairball Agent\",
    \"comments\": [
...
```bash
source "${HAIRBALL_HOME:-$HOME/.hairball}/skills/github/github-auth/scripts/gh-env.sh"
...
gh pr review $PR_NUMBER --approve --body "Reviewed by Hairball Agent. Code looks clean — good test coverage, no security concerns."
