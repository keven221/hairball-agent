#!/usr/bin/env bash
# GitHub environment detection helper for Hairball Agent skills.
#
...
if command -v gh &>/dev/null && gh auth status &>/dev/null 2>&1; then
    ...
    GH_AUTH_METHOD="curl"
elif _hairball_env="${HAIRBALL_HOME:-$HOME/.hairball}/.env"; [ -f "$_hairball_env" ] && grep -q "^GITHUB_TOKEN=" "$_hairball_env" 2>/dev/null; then
