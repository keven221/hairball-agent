...
echo it back; combined with how Hairball replays history this lands on the
...
class DeepSeekProfile(ProviderProfile):
    ...
    def tool_control_capabilities(
        ...
        **context: Any,
        ...
        caps = super().tool_control_capabilities(
            model=model,
        ...
        enabled = not (
            ...
            and (
                ...
                or str(reasoning_config.get("effort") or "").casefold() == "none"
            )
        )
        if not enabled:
            return caps
        # DeepSeek V4/R1 thinking supports tools but rejects the tool_choice
        # parameter itself. Omission keeps thinking and full autonomy.
        return replace(
            caps,
            supports_explicit_auto_tool_choice=False,
            source="provider:deepseek:thinking",
            reason="DeepSeek thinking mode requires implicit automatic tool choice",
        )
