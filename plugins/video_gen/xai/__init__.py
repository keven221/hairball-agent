...
def _xai_user_agent() -> str:
    try:
        ...
        return hairball_xai_user_agent()
    except Exception:
        return "hairball-agent/video_gen"
...
def _raise_if_blocked_local_input(ref: str) -> None:
    """Refuse to read a local media path that Hairball' read deny-list blocks.
...
class XAIVideoGenProvider(VideoGenProvider):
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        ...
        # hook (``hairball_cli/tools_config.py``) so the picker doesn't blindly
...
def _auth_required_response(prompt: str) -> Dict[str, Any]:
    return error_response(
        error=(
            "No xAI credentials found. Sign in via `hairball auth add xai-oauth` "
    ...
    *,
    ...
    try:
        ...
        storage_options = build_xai_storage_options(
            "video_gen",
            filename_prefix="hairball-xai-video",
            extension="mp4",
        )
