"""langfuse — Hairball plugin for Langfuse observability.

Traces Hairball conversations, LLM calls, and tool usage to Langfuse.

Activation is handled by the Hairball plugin system — standalone plugins only
load when listed in ``plugins.enabled`` (via ``hairball plugins enable
observability/langfuse`` or ``hairball tools → Langfuse Observability``). At
runtime the plugin also requires the ``langfuse`` SDK and credentials; if
either is missing the hooks are inert.

Required env vars (set via ``hairball tools`` or ~/.hairball/.env):
  HAIRBALL_LANGFUSE_PUBLIC_KEY  - Langfuse project public key (pk-lf-...)
  HAIRBALL_LANGFUSE_SECRET_KEY  - Langfuse project secret key (sk-lf-...)
  HAIRBALL_LANGFUSE_BASE_URL    - Langfuse server URL (default: https://cloud.langfuse.com)

Optional env vars:
  HAIRBALL_LANGFUSE_ENV         - environment tag (e.g. "production", "local")
  HAIRBALL_LANGFUSE_RELEASE     - release/version tag
  HAIRBALL_LANGFUSE_SAMPLE_RATE - sampling rate 0.0–1.0 (default: 1.0)
  HAIRBALL_LANGFUSE_MAX_CHARS   - max chars per field (default: 12000)
  HAIRBALL_LANGFUSE_DEBUG       - set to "true" for verbose logging
"""
...
def _debug_enabled() -> bool:
    return _env_bool("HAIRBALL_LANGFUSE_DEBUG")
...
def _get_langfuse() -> Optional[Langfuse]:
    ...
    Activation of this plugin is controlled by the Hairball plugin system —
    ...
    public_key = _env("HAIRBALL_LANGFUSE_PUBLIC_KEY") or _env("LANGFUSE_PUBLIC_KEY")
    secret_key = _env("HAIRBALL_LANGFUSE_SECRET_KEY") or _env("LANGFUSE_SECRET_KEY")
    ...
    placeholder_issues = [
        msg
        for msg in (
            _validate_langfuse_key("HAIRBALL_LANGFUSE_PUBLIC_KEY", public_key),
            _validate_langfuse_key("HAIRBALL_LANGFUSE_SECRET_KEY", secret_key),
        )
        if msg
    ]
    if placeholder_issues:
        logger.warning(
            ...
            "or unset HAIRBALL_LANGFUSE_PUBLIC_KEY / HAIRBALL_LANGFUSE_SECRET_KEY to "
    ...
    base_url = _env("HAIRBALL_LANGFUSE_BASE_URL") or _env("LANGFUSE_BASE_URL") or "https://cloud.langfuse.com"
    environment = _env("HAIRBALL_LANGFUSE_ENV") or _env("LANGFUSE_ENV")
    release = _env("HAIRBALL_LANGFUSE_RELEASE") or _env("LANGFUSE_RELEASE")
    sample_rate = _env("HAIRBALL_LANGFUSE_SAMPLE_RATE")
    ...
    if sample_rate:
        try:
            ...
            logger.warning("Invalid HAIRBALL_LANGFUSE_SAMPLE_RATE=%r", sample_rate)
...
def _trace_key(
    ...
    *,
    ...
    Older Hairball paths only expose ``task_id``/``session_id``. Newer paths
...
def _safe_value(value: Any, *, max_chars: Optional[int] = None, depth: int = 0,
    ...
    max_chars = max_chars if max_chars is not None else int(_env("HAIRBALL_LANGFUSE_MAX_CHARS", "12000") or "12000")
    if depth > 4:
        return "<max-depth>"
    ...
    if propagate_attributes is not None:
        try:
            with propagate_attributes(
                ...
                trace_name="Hairball turn",
                tags=["hairball", "langfuse"],
            ):
...
def on_pre_llm_call(*, task_id: str = "", session_id: str = "", platform: str = "", model: str = "",
    ...
    # Older Hairball branches used pre_llm_call for request-scoped tracing and
    # passed the actual API messages. Current Hairball also has a turn-scoped
    ...
    # messages is a list only for legacy Hairball branches that fired
    # pre_llm_call with API messages directly. Current Hairball fires
...
def register(ctx) -> None:
    ...
    # Hairball versions.  pre_api_request / post_api_request fire per API
