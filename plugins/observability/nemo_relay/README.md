...
Optional Hairball observability plugin that maps Hairball observer hooks to
...
not replace Hairball Agent's planner, tools, memory, model provider routing, or
CLI UX. Instead, this plugin lets Hairball emit NeMo Relay lifecycle events for
the work Hairball already owns: sessions, turns, provider/API calls, tool calls,
approval prompts, and delegated subagents.

With this plugin enabled, Hairball Agent can:

- Preserve Hairball execution as NeMo Relay scopes, LLM spans, tool spans, and
  mark events.
...
```bash
hairball plugins enable observability/nemo_relay
```

The `HAIRBALL_NEMO_RELAY_*` environment variables below only configure an
already-enabled plugin. They do not enable plugin discovery by themselves.

For isolated test homes, enable the plugin in the same `HAIRBALL_HOME` that the
agent run will use:

```bash
env HAIRBALL_HOME=/tmp/hairball-nemo-relay-test \
  hairball plugins enable observability/nemo_relay
```

Runs started with `--ignore_user_config` skip the enabled-plugin state from
`HAIRBALL_HOME`, so local E2E tests should omit that flag unless the test harness
loads `observability/nemo_relay` explicitly another way.

`HAIRBALL_HOME` is the Hairball profile/config home used by both
`hairball plugins enable ...` and the later `hairball chat ...` run. If unset,
Hairball uses the user's default home, usually `~/.hairball`. For isolated smoke
tests, choose any writable temporary directory and use the same value for every
command in that test:

```bash
export HAIRBALL_HOME=/tmp/hairball-nemo-relay-test
hairball plugins enable observability/nemo_relay
hairball chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```

For source checkouts, make sure the `hairball` command you run is built from the
...
uv run hairball plugins enable observability/nemo_relay
uv run hairball chat --query 'Reply exactly ok' --provider custom --model qwen3.6:35b
```
...
```bash
uv build --wheel
python -m pip install --force-reinstall dist/hairball_agent-*.whl
python -m pip install "nemo-relay>=0.5,<0.7"
hairball plugins enable observability/nemo_relay
```

The plugin fails open when `nemo-relay` is not installed. Install and test it against the supported official NeMo Relay 0.5/0.6 PyPI releases:
...
The plugin can configure exporters directly from `HAIRBALL_NEMO_RELAY_*`
...
```bash
export HAIRBALL_NEMO_RELAY_ATOF_ENABLED=1
export HAIRBALL_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=.nemo-relay/atof
export HAIRBALL_NEMO_RELAY_ATIF_ENABLED=1
export HAIRBALL_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=.nemo-relay/atif
```

Optional overrides:

- `HAIRBALL_NEMO_RELAY_ATOF_FILENAME`
- `HAIRBALL_NEMO_RELAY_ATOF_MODE` (`append` or `overwrite`)
- `HAIRBALL_NEMO_RELAY_ATIF_FILENAME_TEMPLATE`
- `HAIRBALL_NEMO_RELAY_ATIF_AGENT_NAME`
- `HAIRBALL_NEMO_RELAY_ATIF_AGENT_VERSION`
- `HAIRBALL_NEMO_RELAY_ATIF_MODEL_NAME`
- `HAIRBALL_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE` (`embedded` by default; set `all` to also write standalone child files)

### NeMo Relay Component Config

To initialize NeMo Relay from a component config, create a `plugins.toml` file
and point Hairball at it:

```bash
export HAIRBALL_NEMO_RELAY_PLUGINS_TOML=.nemo-relay/plugins.toml
```
...
agent_name = "Hairball Agent"
agent_version = "local"
```

When `HAIRBALL_NEMO_RELAY_PLUGINS_TOML` is set and initializes successfully, NeMo
Relay owns exporter lifecycle through that config. The direct
`HAIRBALL_NEMO_RELAY_ATOF_*` fallback setup is skipped. If the same
`plugins.toml` observability config enables `atif`, the direct
`HAIRBALL_NEMO_RELAY_ATIF_*` fallback setup is also skipped so Hairball does not
double-export trajectories on teardown. If `plugins.toml` initialization fails,
Hairball keeps the direct env-var fallbacks active for that run.
...
exposes `llm.execute(...)` / `tools.execute(...)`, Hairball routes LLM and tool
...
For the full generic Hairball middleware contract, see
[`docs/middleware/README.md`](../../../docs/middleware/README.md).

## Canonical Local Examples

The observe-only examples in this section use the supported `nemo-relay>=0.5,<0.7`
distribution and a local Ollama model served through the OpenAI-compatible API.

```bash
pip install "nemo-relay>=0.5,<0.7"

export HAIRBALL_HOME=/tmp/hairball-nemo-relay-docs/hairball-home
mkdir -p "$HAIRBALL_HOME"

cat > "$HAIRBALL_HOME/config.yaml" <<'YAML'
model:
  provider: custom
  default: qwen3.6:35b
...
This run starts a parent Hairball session, delegates to a child subagent, has the
child call `terminal`, and writes both ATOF and ATIF.

```bash
export HAIRBALL_NEMO_RELAY_ATOF_ENABLED=1
export HAIRBALL_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hairball-nemo-relay-docs/subagent/atof
export HAIRBALL_NEMO_RELAY_ATOF_FILENAME=nested-subagent-atof.jsonl
export HAIRBALL_NEMO_RELAY_ATOF_MODE=overwrite
export HAIRBALL_NEMO_RELAY_ATIF_ENABLED=1
export HAIRBALL_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hairball-nemo-relay-docs/subagent/atif
export HAIRBALL_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='nested-subagent-atif-{session_id}.json'
export HAIRBALL_NEMO_RELAY_ATIF_AGENT_NAME='Hairball Agent E2E'
export HAIRBALL_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
export HAIRBALL_NEMO_RELAY_ATIF_SUBAGENT_EXPORT_MODE=all
...
{"kind":"mark","name":"hairball.subagent.start","metadata":{"parent_session_id":"docs-parent-session","session_id":"docs-child-session","subagent_id":"sa-0-docs","child_role":"leaf"}}
...
```json
{
  ...
  "session_id": "docs-parent-session",
  "agent": {"name": "Hairball Agent E2E", "version": "docs-example", "model_name": "qwen3.6:35b"},
  "steps": [
    {
      "source": "agent",
...
message. Hairball dispatches the read-only tools as one batch, and NeMo Relay
records both tool invocations.

```bash
mkdir -p /tmp/hairball-nemo-relay-docs/workdir
printf 'docs_parallel_alpha_function\n' > /tmp/hairball-nemo-relay-docs/workdir/alpha.txt
printf 'docs_parallel_beta_function\n' > /tmp/hairball-nemo-relay-docs/workdir/beta.txt
cd /tmp/hairball-nemo-relay-docs/workdir

export HAIRBALL_NEMO_RELAY_ATOF_ENABLED=1
export HAIRBALL_NEMO_RELAY_ATOF_OUTPUT_DIRECTORY=/tmp/hairball-nemo-relay-docs/parallel/atof
export HAIRBALL_NEMO_RELAY_ATOF_FILENAME=parallel-tools-atof.jsonl
export HAIRBALL_NEMO_RELAY_ATOF_MODE=overwrite
export HAIRBALL_NEMO_RELAY_ATIF_ENABLED=1
export HAIRBALL_NEMO_RELAY_ATIF_OUTPUT_DIRECTORY=/tmp/hairball-nemo-relay-docs/parallel/atif
export HAIRBALL_NEMO_RELAY_ATIF_FILENAME_TEMPLATE='parallel-tools-atif-{session_id}.json'
export HAIRBALL_NEMO_RELAY_ATIF_AGENT_NAME='Hairball Agent E2E'
export HAIRBALL_NEMO_RELAY_ATIF_AGENT_VERSION=docs-example
...
- Hairball sessions map to `agent` scopes.
- Hairball API request hooks map to `llm` scope start/end events.
- Hairball tool hooks map to `tool` scope start/end events.
...
The `observability/nemo_relay` plugin uses Hairball execution middleware to hand
...
Enable it for Hairball:

```bash
export HAIRBALL_NEMO_RELAY_PLUGINS_TOML=/tmp/hairball-middleware-test/plugins.toml
```

When the adaptive component is enabled and the installed NeMo Relay runtime
exposes `llm.execute(...)` and `tools.execute(...)`, Hairball routes execution
through these boundaries:

```text
Hairball provider call
  -> llm_execution middleware
    -> nemo_relay.llm.execute(...)
      -> Hairball provider adapter next_call(...)

Hairball tool call
  -> tool_execution middleware
    -> nemo_relay.tools.execute(...)
      -> Hairball tool dispatcher next_call(...)
```
...
middleware for a local Hairball run. This path requires a NeMo Relay runtime that
supports `[components.config.tool_parallelism]`; the `nemo-relay>=0.5,<0.7`
install used by the earlier observability-only examples does not support this
adaptive config.

```bash
export HAIRBALL_HOME=/tmp/hairball-middleware-test/hairball-home
mkdir -p "$HAIRBALL_HOME" /tmp/hairball-middleware-test/nemo-relay
...
cat > /tmp/hairball-middleware-test/nemo-relay/plugins.toml <<'TOML'
version = 1
...
[components.config.atof]
enabled = true
output_directory = "/tmp/hairball-middleware-test/atof"
filename = "middleware-events.jsonl"
mode = "overwrite"

[components.config.atif]
enabled = true
output_directory = "/tmp/hairball-middleware-test/atif"
filename_template = "middleware-trajectory-{session_id}.json"
agent_name = "Hairball Middleware E2E"
agent_version = "local"
...
export HAIRBALL_NEMO_RELAY_PLUGINS_TOML=/tmp/hairball-middleware-test/nemo-relay/plugins.toml
