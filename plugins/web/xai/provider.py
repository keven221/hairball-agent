...
``{title, url, description, position}`` rows every other Hairball web
provider produces.
...
prefers Hairball-managed xAI Grok OAuth (via ``hairball auth``) and falls back
to ``XAI_API_KEY`` (resolved through ``~/.hairball/.env``, then
``os.environ``).
"""
...
class XAIWebSearchProvider(WebSearchProvider):
    ...
    def is_available(self) -> bool:
        ...
        every ``hairball tools`` repaint and at tool-registration time.
    ...
    def search(self, query: str, limit: int = 5) -> Dict[str, Any]:
        ...
        if not api_key:
            return {
                "success": False,
                "error": (
                    "No xAI credentials found. Run `hairball auth` to sign in with "
