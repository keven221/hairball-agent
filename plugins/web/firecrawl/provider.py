...
Env vars::
    ...
    FIRECRAWL_GATEWAY_URL=...        # managed tool-gateway (subscribers)
...
def _is_tool_gateway_ready() -> bool:
    """Return True when gateway URL + managed Subscriber token are available.
...
def check_firecrawl_api_key() -> bool:
    ...
    existing tests and the ``hairball tools`` setup flow.
    """
...
def _firecrawl_backend_help_suffix() -> str:
    ...
    if not _wt.managed_portal_tools_enabled():
        return ""
    return (
...
def _raise_web_backend_configuration_error() -> None:
    ...
    if _wt.managed_portal_tools_enabled():
        message += (
            " With your managed subscription you can also use the Tool Gateway. "
            "run `hairball tools` and select managed Subscription as the web provider."
        )
    else:
...
def _get_firecrawl_client() -> Any:
    ...
    if direct_config is not None and not _wt.prefers_gateway("web"):
        ...
        # Surface an explicitly invalid gateway scheme before the optional
        # resolver degrades an unavailable gateway to ``None``.  Keep this in
        # the gateway branch so a valid direct Firecrawl configuration is not
        # blocked by an unrelated dormant gateway setting.
        from tools.managed_tool_gateway import get_tool_gateway_scheme
