...
Auto-tracks and cleans up ephemeral files created during Hairball Agent
sessions — test scripts, temp outputs, cron logs, stale chrome profiles.
Scoped strictly to `$HAIRBALL_HOME` and `/tmp/hairball-*`.
...
| Hook | Behaviour |
|---|---|
| `post_tool_call` | When `write_file` / `terminal` / `patch` creates a file matching `test_*`, `tmp_*`, or `*.test.*` inside `HAIRBALL_HOME`, track it silently as `test` / `temp` / `cron-output`. |
...
| empty dirs under HAIRBALL_HOME | always | Never |
...
- `is_safe_path()` rejects anything outside `HAIRBALL_HOME` or `/tmp/hairball-*`
- Windows mounts (`/mnt/c` etc.) are rejected
- The state directory `$HAIRBALL_HOME/disk-cleanup/` is itself excluded
- `$HAIRBALL_HOME/logs/`, `memories/`, `sessions/`, `skills/`, `plugins/`,
