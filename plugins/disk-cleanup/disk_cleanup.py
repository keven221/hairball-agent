"""disk_cleanup — ephemeral file cleanup for Hairball Agent.
...
Rules:
  ...
  - empty dirs    → always delete (under HAIRBALL_HOME)
  - research      → keep 10 newest, prompt for older (deep only)
  - chrome-profile→ prompt after 14 days (deep only)
  - >500 MB files → prompt always (deep only)

Scope: strictly HAIRBALL_HOME and /tmp/hairball-*
Never touches: ~/.hairball/logs/ or any system directory.
"""
...
try:
    ...
    def get_hairball_home() -> Path:  # type: ignore[no-redef]
        val = (os.environ.get("HAIRBALL_HOME") or "").strip()
        return Path(val).resolve() if val else (Path.home() / ".hairball").resolve()
...
def get_state_dir() -> Path:
    """State dir — separate from ``$HAIRBALL_HOME/logs/``."""
    return get_hairball_home() / "disk-cleanup"
...
def get_log_file() -> Path:
    """Audit log — intentionally NOT under ``$HAIRBALL_HOME/logs/``."""
...
def is_safe_path(path: Path) -> bool:
    """Accept only paths under HAIRBALL_HOME or ``/tmp/hairball-*``.

    Rejects Windows mounts (``/mnt/c`` etc.) and any system directory.
    """
    hairball_home = get_hairball_home()
    try:
        path.resolve().relative_to(hairball_home)
        return True
    except (ValueError, OSError):
        pass
    # Allow /tmp/hairball-* explicitly
    parts = path.parts
    if len(parts) >= 3 and parts[1] == "tmp" and parts[2].startswith("hairball-"):
        return True
    return False
...
_EMPTY_DIR_PROTECTED_TOP_LEVEL = frozenset({
    ...
    "hairball-agent", "backups", "profiles", ".worktrees",
})
...
# Paths under $HAIRBALL_HOME that must NEVER be deleted by quick(),
...
def _is_protected_cron_path(p: Path) -> bool:
    ...
    history at once.
    """
    # Lazily build the set once per process so HAIRBALL_HOME is resolved
    # exactly once.
    if not _PROTECTED_CRON_PATHS:
        hairball_home = get_hairball_home()
        for parent in ("cron", "cronjobs"):
            base = hairball_home / parent
...
def track(path_str: str, category: str, silent: bool = False) -> bool:
    ...
    if not is_safe_path(path):
        _log(f"REJECT: {path} (outside HAIRBALL_HOME)")
        return False
...
def quick() -> Dict[str, Any]:
    ...
    # Remove empty dirs under HAIRBALL_HOME, but never recurse into known
    # durable state trees.  Some installs place the Hairball checkout, venv,
    # and desktop build under HAIRBALL_HOME; a full rglob over that tree can
    # stall the gateway event loop for minutes.
    hairball_home = get_hairball_home()
    empty_removed = 0
    sweep_stack: List[Tuple[Path, bool]] = []
    try:
        for top in hairball_home.iterdir():
            if (
                top.is_dir()
...
def guess_category(path: Path) -> Optional[str]:
    ...
    hairball_home = get_hairball_home()
    try:
        rel = path.resolve().relative_to(hairball_home)
        top = rel.parts[0] if rel.parts else ""
        if top in {
            "disk-cleanup", "logs", "memories", "sessions", "config.yaml",
            "skills", "plugins", ".env", "USER.md", "MEMORY.md", "SOUL.md",
            "auth.json", "hairball-agent",
        }:
            return None
        ...
        # Path isn't under HAIRBALL_HOME (e.g. /tmp/hairball-*) — fall through.
        pass
