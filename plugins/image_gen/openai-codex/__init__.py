"""OpenAI image generation backend — ChatGPT/reference OAuth variant.
...
the reference Responses API ``image_generation`` tool instead of the
``images.generate`` REST endpoint. This lets users who are already
authenticated with reference/ChatGPT generate images without configuring a
...
Output is saved as PNG under ``$HAIRBALL_HOME/cache/images/``. Source images for
...
class CodexImageGenerationUnsupportedError(RuntimeError):
    """The active reference account cannot use the hosted image tool."""


_IMAGE_GENERATION_UNAVAILABLE_MESSAGE = (
    "Image generation is not enabled for the current reference account. "
...
def _is_image_generation_unsupported_error(status_code: int, body: str) -> bool:
    """Match only reference's account-capability rejection for the image tool."""
    if status_code != 400:
        return False
    try:
...
# reference Responses surface used for the request. The chat model itself is only
...
def _read_codex_access_token() -> Optional[str]:
    """Return a usable reference OAuth token, or None.
    ...
    try:
        ...
        logger.debug("Could not resolve reference access token: %s", exc)
        return None
...
def _data_url_to_input_image_url(value: str) -> str:
    ...
    if not header_lc.startswith("data:image/") or ";base64" not in header_lc:
        raise ValueError("Only base64 data:image URLs are supported as reference image inputs")
...
def _local_image_to_data_url(value: str) -> str:
    ...
    try:
        ...
        logger.debug("reference image input read guard unavailable: %s", exc)
...
def _build_responses_payload(
    *,
    ...
    """Build the reference Responses request body for an image_generation call."""
...
def _iter_sse_json(response: Any):
    ...
    The ChatGPT/reference backend can emit image-generation events newer than the
...
def _collect_image_b64(
    ...
    *,
    ...
    """Stream a reference Responses image_generation call and return the b64 image."""
    import httpx
    ...
    with httpx.Client(timeout=timeout, headers=headers) as http:
        with http.stream("POST", f"{_CODEX_BASE_URL}/responses", json=payload) as response:
            try:
                ...
                body = full_body[:500]
                raise RuntimeError(
                    f"reference Responses API returned HTTP {exc.response.status_code}: {body}"
                ) from exc
...
class OpenAICodexImageGenProvider(ImageGenProvider):
    """gpt-image-2 routed through ChatGPT/reference OAuth instead of an API key."""
    ...
    def get_setup_schema(self) -> Dict[str, Any]:
        return {
            "name": "OpenAI (reference auth)",
            "badge": "free",
            "tag": "gpt-image-2 via ChatGPT/reference OAuth — no API key required; supports text and image inputs",
            "env_vars": [],
            "post_setup_hint": (
                "Sign in with `hairball auth codex` (or `hairball setup` → reference) "
                "if you haven't already. No API key needed."
            ),
        }

    def capabilities(self) -> Dict[str, Any]:
        # The reference Responses image_generation tool accepts source/reference
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not _read_codex_access_token():
            return error_response(
                error=(
                    "No reference/ChatGPT OAuth credentials available. Run "
                    "`hairball auth codex` (or `hairball setup` → reference) to sign in."
                ),
        ...
        try:
            ...
            return error_response(
                error=f"Invalid image input for reference image editing: {exc}",
        ...
        try:
            ...
            logger.debug(
                "reference account does not expose image generation",
                exc_info=True,
            )
            return error_response(
            ...
            logger.debug("reference image generation failed", exc_info=True)
            return error_response(
                error=f"OpenAI image generation via reference auth failed: {exc}",
        ...
        if not b64:
            return error_response(
                error="reference response contained no image_generation_call result",
...
def register(ctx) -> None:
    """Plugin entry point — register the reference-backed image-gen provider."""
