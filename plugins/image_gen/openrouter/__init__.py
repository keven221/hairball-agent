"""OpenRouter-compatible image generation backend (OpenRouter + Managed Portal).

Both OpenRouter and the Managed Portal inference endpoint speak the same
...
Managed Portal proxies OpenRouter, so one implementation services both — we only
swap the resolved ``(base_url, api_key)``. Credentials are resolved through the
agent's existing :func:`~hairball_cli.runtime_provider.resolve_runtime_provider`,
which already understands OpenRouter's key pool and the managed OAuth device-code
...
# image_gen.model from ``hairball tools``): use exactly that model (no auto
...
class OpenRouterCompatImageProvider(ImageGenProvider):
    ...
    Instantiated once per backend (OpenRouter, Managed Portal). The two differ only
    ...
    def _resolve_model_chain(self, explicit: Optional[str] = None) -> list[str]:
        ...
        by ``hairball tools``) → the quality-first default chain.
    ...
    def generate(
        ...
        **kwargs: Any,
        ...
        if not api_key or not base_url:
            return error_response(
                error=(
                    ...
                    f"Configure {self._display} in `hairball tools` → Image Generation."
                ),
                error_type="missing_api_key",
                provider=self._name,
                aspect_ratio=aspect_ratio,
        ...
        headers = {
            ...
            # OpenRouter attribution headers (harmless against Managed Portal).
            "HTTP-Referer": "https://github.com/managed/hairball-agent",
...
def _build_providers() -> List[OpenRouterCompatImageProvider]:
    return [
        ...
        OpenRouterCompatImageProvider(
            ...
            runtime_name="managed",
            config_key="managed",
            model_env_var="MANAGED_IMAGE_MODEL",
            setup_schema={
                "name": "Managed Portal (image)",
                "badge": "subscription",
                "tag": "Reference-grounded image generation via Managed Portal (OpenRouter-backed)",
                "env_vars": [],
...
def register(ctx: Any) -> None:
    """Register the OpenRouter + Managed Portal image gen providers."""
