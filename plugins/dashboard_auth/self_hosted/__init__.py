...
A standards-compliant OpenID Connect Relying Party for the ``hairball dashboard``
OAuth gate. Unlike the bundled ``managed`` provider (which encodes Managed Portal's
...
:class:`~hairball_cli.dashboard_auth.DashboardAuthProvider` methods and touches
...
``hairball_cli/dashboard_auth/routes.py``; this provider only:
  ...
  4. verifies the **ID token** (RS256/ES256) against the discovered
     ...
     onto a :class:`~hairball_cli.dashboard_auth.Session`.
...
provider verifies its *access* token because Managed Portal mints a custom JWT
...
same precedence convention as the ``managed`` plugin)::
    ...
    dashboard:
      oauth:
        ...
        self_hosted:
          issuer: https://auth.example.com/application/o/hairball/   # required
          client_id: hairball-dashboard                              # required
          scopes: "openid profile email"                           # optional
          # client_secret: set ONLY for a confidential client. It is a
          # credential — prefer the env var / ~/.hairball/.env over config.yaml.

    # Environment overrides (Docker/Fly secret injection)
    HAIRBALL_DASHBOARD_OIDC_ISSUER
    HAIRBALL_DASHBOARD_OIDC_CLIENT_ID
    HAIRBALL_DASHBOARD_OIDC_SCOPES        # optional; defaults to "openid profile email"
    HAIRBALL_DASHBOARD_OIDC_CLIENT_SECRET # optional; set for a confidential client
...
class SelfHostedOIDCProvider(DashboardAuthProvider):
    ...
    def _session_from_tokens(
        ...
        *,
        ...
        OAuth access token is intentionally NOT stored — Hairball does not call
    ...
    def _validate_redirect_uri(self, redirect_uri: str) -> None:
        ...
        redirect_uris are permitted. Mirrors the managed provider.
        """
...
def register(ctx) -> None:
    ...
    client_id are configured (via ``HAIRBALL_DASHBOARD_OIDC_*`` env vars or the
    ...
    issuer = _resolve_setting(
        "HAIRBALL_DASHBOARD_OIDC_ISSUER", oidc_cfg.get("issuer")
    )
    client_id = _resolve_setting(
        "HAIRBALL_DASHBOARD_OIDC_CLIENT_ID", oidc_cfg.get("client_id")
    )
    scopes = (
        _resolve_setting("HAIRBALL_DASHBOARD_OIDC_SCOPES", oidc_cfg.get("scopes"))
        or _DEFAULT_SCOPES
    )
    # Optional — set only for a confidential client. A credential, so the
    # canonical home is the env var / ~/.hairball/.env; config.yaml is supported
    # for precedence symmetry. Empty ⇒ public client (unchanged behaviour).
    client_secret = _resolve_setting(
        "HAIRBALL_DASHBOARD_OIDC_CLIENT_SECRET", oidc_cfg.get("client_secret")
    )
    ...
    if not issuer or not client_id:
        LAST_SKIP_REASON = (
            ...
            "(HAIRBALL_DASHBOARD_OIDC_ISSUER + HAIRBALL_DASHBOARD_OIDC_CLIENT_ID) "
