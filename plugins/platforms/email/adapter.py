"""
Email platform adapter for the Hairball gateway.

Allows users to interact with Hairball by sending emails.
...
def _send_imap_id(imap: "imaplib.IMAP4") -> None:
    ...
    try:
        ...
        imap.xatom(
            "ID",
            f'("name" "hairball-agent" "version" "{_hairball_version}" '
            '"vendor" "managed" '
            '"support-email" "noreply@example.invalid")',
        )
...
class EmailAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # send-helper, and `hairball config show` already read. Without the
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        if missing:
            message = (
                ...
                + ", ".join(missing)
                + ". Set it via `hairball gateway setup` (env) or platforms.email "
                "in config.yaml."
            )
    ...
    def _send_email(
        ...
        subject = ctx.get("subject", "Hairball Agent")
        ...
        msg_id = f"<hairball-{uuid.uuid4().hex[:12]}@{self._message_id_domain()}>"
...
# hairball_cli/gateway.py, and the _send_email dispatch in
