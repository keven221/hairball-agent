...
Credential storage mirrors every other Hairball channel:

    * runtime SDK creds  -> ``~/.hairball/.env``  (``PHOTON_PROJECT_ID`` =
      project id, ``PHOTON_PROJECT_SECRET``) via ``save_env_value``
    * management metadata -> ``~/.hairball/auth.json`` under
...
try:
    import httpx
except ImportError:  # pragma: no cover - httpx is a hairball dependency
...
# registers Hairball as its own client_id.
...
# Default name of the project Hairball provisions for the operator.
DEFAULT_PROJECT_NAME = "Hairball Agent"
...
# ---------------------------------------------------------------------------
# auth.json helpers — share the file with the rest of hairball-agent.

def _auth_json_path() -> Path:
    """Resolve ``~/.hairball/auth.json`` honouring the active Hairball profile."""
    try:
        from hairball_constants import get_hairball_home
        return Path(get_hairball_home()) / "auth.json"
    except Exception:
        return Path(os.path.expanduser("~/.hairball")) / "auth.json"
...
def load_project_credentials() -> Tuple[Optional[str], Optional[str]]:
    ...
    Precedence: process env (``~/.hairball/.env`` is loaded into the gateway's
...
def store_project_credentials(
    *,
    ...
    The runtime SDK creds land in ``~/.hairball/.env`` via the same
...
def _persist_runtime_env(spectrum_project_id: str, project_secret: str) -> None:
    """Write the SDK creds to ``~/.hairball/.env`` (canonical runtime store).
    ...
    try:
        ...
        logger.warning("photon: hairball_cli.config unavailable — skipping .env write")
        return
    try:
...
def print_credential_summary(emit: Any = print) -> None:
    ...
    labels["device_token"] = (
        ...
        else "✗ missing (run `hairball photon setup`)"
    )
    ...
    labels["phone_number"] = (
        phone if phone else "✗ missing (run `hairball photon setup --phone ...`)"
    )
    labels["assigned_phone_number"] = (
        assigned if assigned else "✗ missing (run `hairball photon setup`)"
    )
...
def credential_summary() -> Dict[str, str]:
    ...
    def _present_phone() -> str:
        ...
        return phone or "✗ missing (run `hairball photon setup --phone ...`)"

    def _present_assigned_phone() -> str:
        _phone, assigned = load_user_numbers()
        return assigned or "✗ missing (run `hairball photon setup`)"
