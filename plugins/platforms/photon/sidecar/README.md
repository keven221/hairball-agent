...
Small Node helper that bridges Hairball Agent to Photon's Spectrum SDK
(`spectrum-ts`).  Hairball is Python; Photon has no public HTTP
...
- exposes a loopback-only HTTP control channel for the Python adapter
  to push send/typing requests (auth via `X-Hairball-Sidecar-Token`)
...
The Hairball plugin's `hairball photon setup` command runs `npm install`
here automatically.
