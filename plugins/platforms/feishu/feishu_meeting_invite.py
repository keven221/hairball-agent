...
should go back to the inviter through the normal Hairball gateway pipeline, so
...
async def handle_meeting_invited_event(adapter: Any, data: Any) -> None:
    ...
    event = MessageEvent(
        ...
        source=source,
        raw_message=data,
        # Keep the synthetic-event public contract (no message_id) while
        # exposing a stable internal identity to the shared inbound ledger.
        metadata={"gateway_event_id": dedup_key},
    )
