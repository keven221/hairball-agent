...
def _discord_ready_timeout_seconds() -> float:
    ...
    raw = os.getenv("HAIRBALL_GATEWAY_PLATFORM_CONNECT_TIMEOUT", "").strip()
    if raw:
        try:
            return max(0.0, float(raw))
        except ValueError:
            logger.warning(
                "Ignoring invalid HAIRBALL_GATEWAY_PLATFORM_CONNECT_TIMEOUT=%r",
                raw,
            )
    return 30.0
...
def _read_dm_role_auth_guild() -> Optional[int]:
    ...
    policy, ``~/.hairball/.env`` is for secrets only, and this is a
...
class DiscordAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        self._text_batch_delay_seconds = env_float("HAIRBALL_DISCORD_TEXT_BATCH_DELAY_SECONDS", 0.6)
        self._text_batch_split_delay_seconds = env_float("HAIRBALL_DISCORD_TEXT_BATCH_SPLIT_DELAY_SECONDS", 2.0)
        ...
        self._liveness_interval_seconds = env_float(
            "HAIRBALL_DISCORD_LIVENESS_INTERVAL_SECONDS", 60.0
        )
        self._liveness_failure_threshold = env_int(
            "HAIRBALL_DISCORD_LIVENESS_FAILURE_THRESHOLD", 3
        )
    ...
    def _handle_bot_task_done(self, task: asyncio.Task) -> None:
        ...
        marked running, the Discord websocket is dead while the Hairball gateway
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        try:
            ...
            intents.members = (
                ...
                # Server Members intent — exactly the migrate-from-legacy path
    ...
    async def cancel_background_tasks(self) -> None:
        ...
        disconnect budget (``HAIRBALL_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT``, default
    ...
    def _text_batch_flush_deadline_seconds(self) -> float:
        ...
        raw = os.getenv("HAIRBALL_GATEWAY_ADAPTER_DISCONNECT_TIMEOUT", "").strip()
        if raw:
            try:
                parsed = float(raw)
                if parsed > 0:
    ...
    def _command_sync_state_path(self) -> _Path:
        ...
        directory = get_hairball_home() / _DISCORD_COMMAND_SYNC_STATE_SUBDIR
        try:
    ...
    def _canonicalize_app_command_payload(self, payload: Dict[str, Any]) -> Dict[str, Any]:
        """Reduce command payloads to the semantic fields Hairball manages."""
    ...
    async def play_ack_in_voice(self, guild_id: int, phrase: Optional[str] = None) -> bool:
        ...
        audio_path = os.path.join(
            tempfile.gettempdir(), "hairball_voice",
    ...
    def _is_pairing_approved_user(self, user_id: str) -> bool:
        """True when the Discord user has an explicit Hairball pairing grant."""
    ...
    def _register_slash_commands(self) -> None:
        ...
        @tree.command(name="reset", description="Reset your Hairball session")
        ...
        @tree.command(name="status", description="Show Hairball session status")
        ...
        @tree.command(name="stop", description="Stop the running Hairball agent")
        ...
        @tree.command(name="reload-skills", description="Re-scan ~/.hairball/skills/ for new or removed skills")
        ...
        @tree.command(name="update", description="Update Hairball Agent to the latest version")
        async def slash_update(interaction: discord.Interaction):
            await self._run_simple_slash(interaction, "/update", "Update initiated~")

        @tree.command(name="restart", description="Gracefully restart the Hairball gateway")
        ...
        @tree.command(name="thread", description="Create a new thread and start a Hairball session in it")
        @discord.app_commands.describe(
            name="Thread name",
            message="Optional first message to send to Hairball in the thread",
        ...
        # hairball_cli/commands.py automatically appear as Discord slash
        ...
        dropped_over_cap = 0
        try:
            from hairball_cli.commands import COMMAND_REGISTRY, _is_gateway_available, _resolve_config_gates
        ...
        try:
            from hairball_cli.commands import _iter_plugin_command_entries
    ...
    def _register_skill_group(self, tree) -> None:
        ...
        try:
            ...
            cmd = discord.app_commands.Command(
                name="skill",
                description="Run a Hairball skill",
    ...
    def _refresh_skill_catalog_state(self) -> None:
        ...
        """
        from hairball_cli.commands import discord_skill_commands_by_category
    ...
    async def _handle_thread_create_slash(
        ...
        # If a message was provided, kick off a new Hairball session in the thread
    ...
    async def _create_thread(
        ...
        *,
        ...
        try:
            ...
            try:
                seed_content = starter_message or f"\U0001f9f5 Thread created by Hairball: **{name}**"
    ...
    def _derive_auto_thread_name(self, content: str) -> str:
        ...
        Hairball has an LLM-generated session title and can safely rename
        ...
        thread_name = content[:80] if content else "Hairball"
        if len(content) > 80:
    ...
    async def _auto_create_thread(self, message: 'DiscordMessage') -> Optional[Any]:
        ...
        for attempt in range(2):
            try:
                ...
                try:
                    setattr(thread, "_hairball_auto_thread_initial_name", thread_name)
                except Exception:
                    pass
                return thread
            except Exception as direct_error:
                ...
                try:
                    seed_msg = await message.channel.send(
                        f"\U0001f9f5 Thread created by Hairball: **{thread_name}**"
                    )
                    thread = await seed_msg.create_thread(
                        name=thread_name,
    ...
    async def rename_thread(
        ...
        *,
        ...
        if edit is None:
            return False
        try:
            await edit(name=cleaned, reason="Hairball semantic session title")
            logger.info(
    ...
    async def create_handoff_thread(
        ...
        reason = "Hairball session handoff"
        ...
        try:
            ...
            if send is None:
                return None
            seed_msg = await send(f"\U0001f9f5 Hairball handoff: **{thread_name}**")
            thread = await seed_msg.create_thread(
                name=thread_name,
                auto_archive_duration=1440,
                reason=reason,
    ...
    async def send_exec_approval(
        ...
        try:
            ...
            prompt_prefix = (
                ...
                "Do you want Hairball to run this command?\n\n"
                "**Requested command:**\n```bash\n"
            )
            if smart_denied:
    ...
    async def send_clarify(
        ...
        try:
            ...
            embed = discord.Embed(
                title="❓ Hairball needs your input",
                description=body,
                color=discord.Color.orange(),
            )
            ...
            content = self._self_contained_prompt_content(
                "❓ **Hairball needs your input**", str(question or "").strip(),
                tail=clarify_tail,
            )
    ...
    async def send_update_prompt(
        ...
        Used by the gateway ``/update`` watcher when ``hairball update --gateway``
    ...
    async def _handle_message(self, message: DiscordMessage, role_authorized: bool = False) -> None:
                ...
                if thread:
                    ...
                    try:
                        await message.channel.send(
                            "⚠️ Hairball could not create a Discord thread for "
        ...
        source = self.build_source(
            ...
            auto_thread_initial_name=(
                getattr(auto_threaded_channel, "_hairball_auto_thread_initial_name", None)
...
def _component_check_auth(
    ...
    the pairing store (``hairball pairing approve``).
    ...
    # so users approved via ``hairball pairing approve`` can interact with
...
def _define_discord_view_classes() -> None:
    ...
    class UpdatePromptView(discord.ui.View):
        """Interactive Yes/No buttons for ``hairball update`` prompts.
...
# is not in this process (e.g. ``hairball cron`` running standalone) and no live
...
def interactive_setup() -> None:
    ...
    from hairball_cli.cli_output import (
        prompt,
        prompt_yes_no,
        print_header,
        print_info,
...
def _apply_yaml_config(yaml_cfg: dict, discord_cfg: dict) -> dict | None:
    ...
    if lis is not None and not os.getenv("HAIRBALL_DISCORD_LIVENESS_INTERVAL_SECONDS"):
        os.environ["HAIRBALL_DISCORD_LIVENESS_INTERVAL_SECONDS"] = str(lis)
    lft = discord_cfg.get("liveness_failure_threshold")
    if lft is not None and not os.getenv("HAIRBALL_DISCORD_LIVENESS_FAILURE_THRESHOLD"):
        os.environ["HAIRBALL_DISCORD_LIVENESS_FAILURE_THRESHOLD"] = str(lft)
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``hairball_cli.gateway.get_env_value`` at call time (not via
    ...
    """
    import hairball_cli.gateway as gateway_mod
...
def register(ctx) -> None:
    """Plugin entry point — called by the Hairball plugin system."""
    ctx.register_platform(
        ...
        install_hint="pip install 'hairball-agent[messaging]'",
        # Interactive setup wizard — replaces the central
        # hairball_cli/setup.py::_setup_discord function.  Same shape as Teams.
