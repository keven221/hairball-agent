...
# ``model_tools`` import time, so ``hairball status``, ``hairball chat``, etc.
...
# ``hairball_plugins.platforms__google_chat.adapter`` once the plugin
...
#     fresh "Hairball is thinking…" card during the small window between
...
def _mime_for_message_type(mime: str) -> MessageType:
    """Map a MIME string to a hairball MessageType.
...
class GoogleChatAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig):
        ...
        # Persisted to ${HAIRBALL_HOME}/google_chat_thread_counts.json so
        ...
        try:
            ...
            _hairball_home = _get_hairball_home()
        except (ModuleNotFoundError, ImportError):
            _hairball_home = _Path.home() / ".hairball"
        self._thread_count_store = _ThreadCountStore(
            _hairball_home / "google_chat_thread_counts.json"
        )
        ...
        # they don't sit in the chat forever as "Hairball is thinking…".
    ...
    def _bot_id_cache_path(self) -> _Path:
        ...
        base = os.getenv("HAIRBALL_HOME", str(_Path.home() / ".hairball"))
    ...
    async def _dispatch_message(self, msg: Dict[str, Any], envelope: Dict[str, Any]) -> None:
        ...
        try:
            ...
            if text.startswith("/setup-files") and event.source is not None:
                ...
                # ${HAIRBALL_HOME}/google_chat_user_tokens/<sanitized>.json
    ...
    async def _build_message_event(
        ...
        """Parse a Chat API message into a hairball MessageEvent."""
        space = envelope.get("space") or msg.get("space") or {}
    ...
    async def send_typing(self, chat_id: str, metadata: Any = None) -> None:
        """Post a visible 'Hairball is thinking…' marker message.
        ...
        "Hairball is thinking…" stuck in chat forever, plus one card that
        ...
        body: Dict[str, Any] = {"text": "Hairball is thinking…"}
        if thread_id:
            body["thread"] = {"name": thread_id}
    ...
    async def stop_typing(self, chat_id: str) -> None:
        ...
        Google Chat has no separate typing API: the "Hairball is thinking…"
    ...
    async def on_processing_complete(
        ...
        see "Hairball is thinking…" stuck forever.
        """
    ...
    async def _consume_typing_card_with_text(
        ...
        creating a fresh "Hairball is thinking…" card during any subsequent
...
def interactive_setup() -> None:
    """Walk the user through Google Chat configuration via ``hairball setup``.

    The setup wizard at ``hairball_cli/gateway.py`` calls this for plugin
    ...
    prompt for env vars, persist them to ``~/.hairball/.env`` so the next
    ...
    print_info("  3. Create a Pub/Sub topic (e.g. hairball-chat-events) and a Pull subscription.")
    ...
    print()
    print_success("Google Chat configuration saved to ~/.hairball/.env")
    print_info("Restart the gateway: hairball gateway restart")
...
async def _standalone_send(
    ...
    *,
    ...
    runner is not in this process (e.g. ``hairball cron`` running as a
    separate process from ``hairball gateway``).  Without this hook,
...
def register(ctx) -> None:
    """Plugin entry point — called by the Hairball plugin system at startup.
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'hairball-agent[google_chat]'",
        ...
        # the "Hairball is thinking..." marker patches and edit overhead.
        ...
        platform_hint=(
            ...
            "are generating a response, a 'Hairball is thinking…' marker message "
