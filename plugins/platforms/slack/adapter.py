...
class SlackAdapter(BasePlatformAdapter):
        ...
        try:
            ...
            if "im:history" in granted and "mpim:history" not in granted:
                ...
                logger.warning(
                    ...
                    "Regenerating the app from `hairball slack` produces a "
    ...
    async def connect(self, *, is_reconnect: bool = False) -> bool:
        ...
        tokens_file = get_hairball_home() / "slack_tokens.json"
        ...
        try:
            ...
            # Hairball does not currently need to route them into the agent loop.
            ...
            # /model work directly without /hairball prefix). A single regex
            ...
            # manifest (see `hairball slack manifest`). In Socket Mode, Slack
            ...
            from hairball_cli.commands import slack_native_slashes
            import re as _re
            ...
            if _slash_names:
                ...
                _slash_pattern = _re.compile(r"^/hairball$")

            @self._app.command(_slash_pattern)
            async def handle_hairball_command(ack, command):
            ...
            for _action_id in (
                "hairball_approve_once",
                "hairball_approve_session",
                "hairball_approve_always",
                "hairball_deny",
            ):
            ...
            for _action_id in (
                "hairball_confirm_once",
                "hairball_confirm_always",
                "hairball_confirm_cancel",
            ):
                self._app.action(_action_id)(self._handle_slash_confirm_action)

            self._app.action("hairball_feedback")(self._handle_feedback_action)
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Hairball handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    def _dm_top_level_threads_as_sessions(self) -> bool:
        ...
        own Hairball session — matching the per-thread behavior channels already
    ...
    async def _handle_app_context_changed(
        ...
        # Hairball owns. Do not write it into _channel_team: channel IDs can be
    ...
    async def _handle_slack_message(
        ...
        if original_text.startswith("!"):
            try:
                from hairball_cli.commands import is_gateway_known_command

                first_token = original_text[1:].split(maxsplit=1)[0]
                # Strip "@suffix" the same way get_command() does, so
                # forms like ``!stop@hairball`` still resolve.
            ...
            actions = [
                {
                    ...
                    "style": "primary",
                    "action_id": "hairball_approve_once",
                    "value": session_key,
                },
            ]
            if not smart_denied:
                actions.append({
                    ...
                    "action_id": "hairball_approve_session",
                    "value": session_key,
                })
                if allow_permanent:
                    actions.append({
                        ...
                        "action_id": "hairball_approve_always",
                        "value": session_key,
                    })
            actions.append({
                "type": "button",
                    ...
                    "elements": [
                        {
                            ...
                            "style": "primary",
                            "action_id": "hairball_confirm_once",
                            "value": value,
                        },
                        {
                            "type": "button",
                            "text": {"type": "plain_text", "text": "Always Approve"},
                            "action_id": "hairball_confirm_always",
                            "value": value,
                        },
                        {
                            "type": "button",
                            ...
                            "style": "danger",
                            "action_id": "hairball_confirm_cancel",
                            "value": value,
                        },
                    ],
                },
    ...
    async def _handle_slash_confirm_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "hairball_confirm_once": "once",
            "hairball_confirm_always": "always",
            "hairball_confirm_cancel": "cancel",
        }
    ...
    async def _handle_approval_action(self, ack, body, action) -> None:
        ...
        choice_map = {
            "hairball_approve_once": "once",
            "hairball_approve_session": "session",
            "hairball_approve_always": "always",
            "hairball_deny": "deny",
        }
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        The legacy ``/hairball <subcommand> [args]`` form is preserved for
        backward compatibility with older workspace manifests and for users
        who want a single entry point for free-form questions (``/hairball
        ...
        if slash_name in {"hairball", ""}:
            # Legacy /hairball <subcommand> [args] routing + free-form questions.
            # Empty slash_name falls into this branch for backward compat
            # with any caller that didn't populate command["command"].
            from hairball_cli.commands import slack_subcommand_map

            subcommand_map = slack_subcommand_map()
            subcommand_map["compact"] = "/compress"
            # Guard against whitespace-only text where ``text`` is truthy but
            # ``text.split()`` returns ``[]`` (e.g. user sends ``/hairball   ``).
        ...
        # "Running /cmd…" ack shown by handle_hairball_command).
        # Only stash for COMMAND events (text starts with "/") — free-form
        # questions via "/hairball <question>" must produce public replies so
    ...
    # Slack file downloads attach the bot Bearer token. A forged file object
    # from a malicious workspace app or compromised event stream could otherwise
    # exfiltrate the token to ANY public host — a hole the generic private-IP
    # SSRF check cannot close. (CDN allowlist + DNS-pin client.)
    ...
    def _resolve_download_token(self, url: str, team_id: str = "") -> str:
        ...
        2. team_id parsed from the file URL itself — Slack private file URLs
           embed the workspace id as ``files-pri/<TEAM_ID>-<FILE_ID>/...``.
    ...
    async def _download_slack_file(
        ...
        # objects and legitimately only ever point at the Slack CDN.
...
# ``hairball_cli/{setup,gateway}.py``, and the ``_send_slack`` dispatch in
...
def interactive_setup() -> None:
    ...
    offers to set a home channel. Replaces ``hairball_cli/setup.py::_setup_slack``.
    """
    ...
    def _write_slack_manifest_and_instruct() -> None:
        """Generate the Slack manifest, write it under HAIRBALL_HOME, and print
        paste-into-Slack instructions. Failures are non-fatal."""
        try:
            from hairball_cli.slack_cli import _build_full_manifest
            from hairball_constants import get_hairball_home
            import json as _json

            manifest = _build_full_manifest(
                bot_name="Hairball",
                bot_description="Your Hairball agent on Slack",
            )
            target = Path(get_hairball_home()) / "slack-manifest.json"
            ...
            print_info(
                "   Re-run `hairball slack manifest --write` anytime to refresh after "
                "Hairball adds new commands."
            )
        except Exception as e:
    ...
    if existing:
        ...
        if not prompt_yes_no("Reconfigure Slack?", False):
            ...
            if prompt_yes_no(
                ...
                "list? (recommended after `hairball update`)",
                True,
            ):
...
def _is_connected(config) -> bool:
    ...
    Looks up via ``hairball_cli.gateway.get_env_value`` at call time (not via the
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install 'hairball-agent[slack]'",
        # Interactive setup wizard — replaces hairball_cli/setup.py::_setup_slack
        # and the static _PLATFORMS["slack"] dict in hairball_cli/gateway.py.
