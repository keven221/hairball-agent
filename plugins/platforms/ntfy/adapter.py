"""ntfy platform adapter (Hairball plugin).
...
a Hairball dependency.

This adapter ships as a Hairball platform plugin under
``plugins/platforms/ntfy/``. The Hairball plugin loader scans the
    ...
    platforms:
      ntfy:
        ...
        extra:
          ...
          topic: "hairball-in"              # subscribe topic (incoming)
          publish_topic: "hairball-out"     # optional — defaults to topic
...
_ECHO_TAG = "hairball-agent"  # tag added to outgoing messages for echo-loop prevention
...
async def _standalone_send(
    ...
    *,
    ...
    ``hairball cron`` running standalone). Without this hook,
...
def register(ctx) -> None:
    ...
    ctx.register_platform(
        ...
        install_hint="pip install httpx   # already a Hairball dependency",
        # Env-driven auto-configuration: seeds PlatformConfig.extra so
        # env-only setups show up in `hairball gateway status` without
