"""
LINE Messaging API platform adapter for Hairball Agent.
...
to Hairball Agent. It deliberately ports the *strongest* idea from each into
...
class LineAdapter(BasePlatformAdapter):
    ...
    async def _handle_media(self, request) -> Any:
        ...
        ``/private/tmp`` on macOS), and ``HAIRBALL_HOME``. PR #8398.
        """
        from aiohttp import web
        ...
        try:
            ...
            hairball_home = Path(get_hairball_home()).resolve()
        except Exception:
            hairball_home = Path.home().joinpath(".hairball").resolve()
...
def is_connected(config) -> bool:
    """Surface in ``hairball status`` even before the adapter is instantiated."""
...
def _env_enablement() -> Optional[Dict[str, Any]]:
    ...
    Lets ``hairball status`` reflect a LINE configuration that lives entirely
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hairball setup line``.

    Mirrors the irc/teams style: prompts for the two required vars, plus
    one optional public URL. Writes to ``~/.hairball/.env`` via ``hairball_cli.config``.
    """
    print()
    ...
    try:
        from hairball_cli.config import get_env_var, set_env_var
    except ImportError:
        print("hairball_cli.config not available; set LINE_* vars manually in ~/.hairball/.env")
        return
