"""SimpleX Chat platform adapter (Hairball plugin).
...
This adapter ships as a Hairball platform plugin under
``plugins/platforms/simplex/``. The Hairball plugin loader scans the
...
Optional environment variables:
    ...
    HAIRBALL_SIMPLEX_TEXT_BATCH_DELAY
...
discoverable and ``hairball setup`` can describe it even when websockets is
...
# Imported at module top because they're stdlib-only inside Hairball — no
...
class SimplexAdapter(BasePlatformAdapter):
    ...
    def __init__(self, config: PlatformConfig, **kwargs):
        ...
        self._text_batch_delay = float(
            os.getenv("HAIRBALL_SIMPLEX_TEXT_BATCH_DELAY", "0.8")
        )
        ...
        msg_event = MessageEvent(
            ...
            message_id=(
                str(
                    meta.get("itemId")
                    or meta.get("item_id")
                    or chat_item_data.get("itemId")
                    or chat_item_data.get("item_id")
                    or ""
                ).strip()
                or None
            ),
...
def interactive_setup() -> None:
    """Minimal stdin wizard for ``hairball setup gateway`` → SimpleX.

    Prompts for the WebSocket URL and the optional allowlist / groups /
    auto-accept / home channel. Writes to ``~/.hairball/.env`` via
    ``hairball_cli.config``.
    """
    print()
    ...
    try:
        ...
        print(
            "hairball_cli.config not available; set SIMPLEX_* vars manually in "
            "~/.hairball/.env"
        )
        return
