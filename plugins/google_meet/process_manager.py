...
session-scoped state file under ``$HAIRBALL_HOME/workspace/meetings/.active.json``
...
# File + directory layout (under $HAIRBALL_HOME):
#
#   workspace/meetings/
...
def _root() -> Path:
    return Path(get_hairball_home()) / "workspace" / "meetings"
...
def start(
    ...
    *,
    ...
    If a bot is already running for this hairball install, leave it first —
    ...
    env = os.environ.copy()
    env["HAIRBALL_MEET_URL"] = url
    env["HAIRBALL_MEET_OUT_DIR"] = str(out)
    env["HAIRBALL_MEET_GUEST_NAME"] = guest_name
    if headed:
        env["HAIRBALL_MEET_HEADED"] = "1"
    if auth_state:
        env["HAIRBALL_MEET_AUTH_STATE"] = auth_state
    if duration:
        env["HAIRBALL_MEET_DURATION"] = duration
    # v2: realtime mode + passthroughs. The bot defaults to transcribe
    # mode if HAIRBALL_MEET_MODE isn't set, matching v1 behavior.
    if mode:
        env["HAIRBALL_MEET_MODE"] = mode
    if realtime_model:
        env["HAIRBALL_MEET_REALTIME_MODEL"] = realtime_model
    if realtime_voice:
        env["HAIRBALL_MEET_REALTIME_VOICE"] = realtime_voice
    if realtime_instructions:
        env["HAIRBALL_MEET_REALTIME_INSTRUCTIONS"] = realtime_instructions
    if realtime_api_key:
        env["HAIRBALL_MEET_REALTIME_KEY"] = realtime_api_key
