...
Let the hairball agent join a Google Meet call, transcribe it, optionally speak
...
```
┌─ gateway (Linux box, where hairball runs) ────────────────────────────┐
│                                                                      │
...
│                                                                      │
│   NodeServer (from `hairball meet node run`)                           │
│     │                                                                │
...
| `__init__.py` | `register(ctx)` — registers 5 tools + `on_session_end` hook + `hairball meet` CLI |
...
| `cli.py` | `hairball meet setup / auth / join / status / transcript / say / stop / node ...` |
...
| `node/registry.py` | v3: `$HAIRBALL_HOME/workspace/meetings/nodes.json` |
| `node/server.py` | v3: `NodeServer` (runs on host machine) |
| `node/client.py` | v3: `NodeClient` (used by tool handlers + CLI on gateway) |
| `node/cli.py` | v3: `hairball meet node {run,list,approve,remove,status,ping}` |
| `SKILL.md` | agent usage guide |

## Local quick start

```bash
hairball plugins enable google_meet
hairball meet install                                      # pip + Chromium
hairball meet setup                                        # preflight
hairball meet auth                                         # optional
hairball meet join https://meet.google.com/abc-defg-hij    # transcribe
```
...
```bash
hairball meet install --realtime                     # installs pulseaudio-utils
echo 'OPENAI_API_KEY=sk-...' >> ~/.hairball/.env
hairball meet join https://meet.google.com/abc-defg-hij --mode realtime
# then from the agent or CLI:
hairball meet say "Good morning everyone, I'm the note-taker bot."
```

macOS:
```bash
hairball meet install --realtime     # runs: brew install blackhole-2ch ffmpeg
# then — manually! — open System Settings → Sound → Input → BlackHole 2ch
echo 'OPENAI_API_KEY=sk-...' >> ~/.hairball/.env
hairball meet join https://meet.google.com/abc-defg-hij --mode realtime
```

On macOS, hairball will **not** switch your system audio input automatically — the
...
hairball plugins enable google_meet
hairball meet node run --display-name my-mac --host 0.0.0.0 --port 18789
# prints the bearer token on first run; copy it
```

On the gateway:
```bash
hairball meet node approve my-mac ws://<mac-ip>:18789 <token>
hairball meet node ping my-mac
