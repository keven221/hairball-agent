"""`hairball meet node ...` subcommand tree.

Wired into the existing ``hairball meet`` parser by the plugin's top-level
...
def register_cli(subparser: argparse.ArgumentParser) -> None:
    ...
    *subparser* is the ``hairball meet node`` argparse object — typically
    ...
    run.add_argument("--display-name", default="hairball-meet-node")
...
def node_command(args: argparse.Namespace) -> int:
    """Dispatch for ``hairball meet node ...``.
    ...
    if cmd == "run":
        ...
        print(f"             hairball meet node approve <name> ws://<host>:{args.port} {token}")
        try:
