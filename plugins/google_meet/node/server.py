...
Launched by ``hairball meet node run``.

Token handling
--------------
On first boot we mint 32 hex chars of entropy and persist them at
``$HAIRBALL_HOME/workspace/meetings/node_token.json``. Subsequent boots
...
via ``hairball meet node approve <name> <url> <token>``.
...
def _default_token_path() -> Path:
    return Path(get_hairball_home()) / "workspace" / "meetings" / "node_token.json"
...
class NodeServer:
    ...
    def __init__(
        ...
        display_name: str = "hairball-meet-node",
    ) -> None:
        self.host = host
        self.port = port
