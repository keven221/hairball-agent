...
Resolution order for config file:
  1. $HAIRBALL_HOME/honcho.json  (instance-local, enables isolated Hairball instances)
...
from hairball_cli.profiles import _get_default_hairball_home
...
def profile_host_key(profile: str | None) -> str:
    """Return the safe Honcho host key for a Hairball profile."""
...
def resolve_active_host() -> str:
    """Derive the Honcho host key from the active Hairball profile.

    Resolution order:
      1. HAIRBALL_HONCHO_HOST env var (explicit override)
      2. Active profile name via profiles system -> ``hairball.<profile>``
      3. Fallback: ``"hairball"`` (default profile)
    """
    explicit = os.environ.get("HAIRBALL_HONCHO_HOST", "").strip()
    if explicit:
        return explicit
...
def resolve_config_path() -> Path:
    ...
    Resolution order:
      1. $HAIRBALL_HOME/honcho.json      (profile-local, if it exists)
      2. ~/.hairball/honcho.json          (default profile — shared host blocks live here)
      3. ~/.honcho/config.json          (global, cross-app interop)

    Returns the global path if none exist (for first-time setup writes).
    """
    local_path = get_hairball_home() / "honcho.json"
    if local_path.exists():
        return local_path

    # Default profile's config — host blocks accumulate here via setup/clone
    default_path = _get_default_hairball_home() / "honcho.json"
...
class HonchoClientConfig:
    ...
    # Max chars of dialectic result to inject into Hairball system prompt
    ...
    # True when Honcho was explicitly configured for this host (hosts.hairball
    ...
    def from_global_config(
        ...
        Resolution: $HAIRBALL_HOME/honcho.json -> ~/.honcho/config.json -> env vars.
        When host is None, derives it from the active Hairball profile.
        """
        resolved_host = host or resolve_active_host()
        ...
        # A hosts.hairball block or explicit enabled flag means the user
    ...
    def resolve_session_name(
        ...
        Resolution order:
          ...
          2. per-session strategy — Hairball session_id ({timestamp}_{hex}); authoritative,
             so a generated title never remaps a live conversation
          3. Manual directory override from sessions map
          4. Hairball session title (from /title command; non-per-session)
...
def get_honcho_client(config: HonchoClientConfig | None = None) -> Honcho:
    ...
    if not config.api_key and not config.base_url:
        raise ValueError(
            ...
            "then run 'hairball honcho setup' or set HONCHO_API_KEY. "
    ...
    def _build() -> "Honcho":
        ...
        # the "go run hairball honcho setup" hint they used to.
        try:
        ...
        try:
            ...
            raise ImportError(
                ...
                "(or run `hairball honcho setup` to configure)."
            )
        ...
        if not resolved_base_url or resolved_timeout is None:
            try:
                ...
                hairball_cfg = load_config()
                honcho_cfg = hairball_cfg.get("honcho", {})
