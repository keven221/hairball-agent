...
``hairball://`` handler. Endpoints are env-overridable with local-dev defaults
...
# The loopback redirect registered for the Hairball OAuth client. IP-literal so
...
_DEFAULT_CLIENT_ID = "hairball-agent"
...
def begin_authorization(
    ...
    *,
    ...
    (``hairball-desktop`` / ``hairball-cli``) so the consent side can attribute
...
_CALLBACK_HTML = (
    ...
    b"<div>Connected to Honcho. You can close this tab and return to Hairball.</div>"
)
...
def start_loopback_flow_background(
    *,
    ...
    # the request, where a context-local HAIRBALL_HOME override can't reach.
