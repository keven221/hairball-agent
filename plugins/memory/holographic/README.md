...
```bash
hairball memory setup    # select "holographic"
```

Or manually:
```bash
hairball config set memory.provider holographic
```

## Config

Config in `config.yaml` under `plugins.hairball-memory-store`:

| Key | Default | Description |
|-----|---------|-------------|
| `db_path` | `$HAIRBALL_HOME/memory_store.db` | SQLite database path |
