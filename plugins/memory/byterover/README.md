...
```bash
hairball memory setup    # select "byterover"
```

Or manually:
```bash
hairball config set memory.provider byterover
# Optional cloud sync:
echo "BRV_API_KEY=your-key" >> ~/.hairball/.env
```
...
Working directory: `$HAIRBALL_HOME/byterover/` (profile-scoped).
