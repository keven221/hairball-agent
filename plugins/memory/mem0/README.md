...
```bash
hairball memory setup    # select "mem0"
```

Or manually:
```bash
hairball config set memory.provider mem0
echo "MEM0_API_KEY=your-key" >> ~/.hairball/.env
```

## Config

Behavioral settings live in `$HAIRBALL_HOME/mem0.json` (set them via `hairball memory setup`). Only the secret `MEM0_API_KEY` belongs in `~/.hairball/.env`.
...
| `user_id` | `hairball-user` | User identifier on Mem0 |
| `agent_id` | `hairball` | Agent identifier |
...
2. Point the plugin at it — via the setup wizard:
   ```bash
   hairball memory setup    # select "mem0" → "Self-hosted server"
   # Or non-interactive:
   hairball memory setup mem0 --mode selfhosted --host http://localhost:8888 --api-key your-admin-api-key
   ```
   or via env vars:
   ```bash
   echo "MEM0_HOST=http://localhost:8888" >> ~/.hairball/.env
   echo "MEM0_API_KEY=your-admin-api-key" >> ~/.hairball/.env
   ```
   or in `$HAIRBALL_HOME/mem0.json`:
   ```json
   {
   ...
   ```
3. Start a fresh Hairball session and call `mem0_search` — it connects to your server.
...
```bash
hairball memory setup mem0 --mode oss \
...
```bash
hairball memory setup mem0 --mode oss --oss-llm-key sk-...
```

Or edit `$HAIRBALL_HOME/mem0.json` directly:
```json
{
  "mode": "oss",
  "oss": {
    ...
    "vector_store": {"provider": "qdrant", "config": {"path": "~/.hairball/mem0_qdrant"}}
  }
}
```
...
```bash
hairball memory setup mem0 --mode platform --api-key sk-...
```

### Dry Run (preview without writing)

```bash
hairball memory setup mem0 --mode oss --oss-llm-key sk-... --dry-run
```
...
- Check `user_id` matches between sessions (`$HAIRBALL_HOME/mem0.json`).
