...
Configuration
-------------
Secret (lives in $HAIRBALL_HOME/.env or the environment):
...
Behavioral settings (live in $HAIRBALL_HOME/mem0.json, set via `hairball memory
setup`):
  ...
  agent_id           — Agent identifier (default: hairball)
...
_DEFAULT_USER_ID = "hairball-user"
...
def _load_config() -> dict:
    """Load config from env vars, with $HAIRBALL_HOME/mem0.json overrides.
    ...
    config = {
        ...
        "agent_id": os.environ.get("MEM0_AGENT_ID", "hairball"),
        "oss": {},
    }
    ...
    config_path = get_hairball_home() / "mem0.json"
...
class Mem0MemoryProvider(MemoryProvider):
    ...
    def save_config(self, values, hairball_home):
        """Write config to $HAIRBALL_HOME/mem0.json."""
        import json
    ...
    def post_setup(self, hairball_home: str, config: dict) -> None:
        ...
        post_setup(hairball_home, config)
    ...
    def initialize(self, session_id: str, **kwargs) -> None:
        ...
        #   1. Operator-configured MEM0_USER_ID (env or $HAIRBALL_HOME/mem0.json) —
        ...
        self._agent_id = self._config.get("agent_id", "hairball")
