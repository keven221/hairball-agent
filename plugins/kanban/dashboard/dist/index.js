/**
 * Hairball Kanban — Dashboard Plugin
 *
 * Board view for the multi-agent collaboration board backed by
 * ~/.hairball/kanban.db. Calls the plugin's backend at /api/plugins/kanban/
 * and tails task_events over a WebSocket for live updates.
 *
 * Plain IIFE, no build step. Uses window.__HAIRBALL_PLUGIN_SDK__ for React +
...
(function () {
  ...
  const SDK = window.__HAIRBALL_PLUGIN_SDK__;
  if (!SDK) return;
  ...
  const COLUMN_DOT = {
    triage: "hairball-kanban-dot-triage",
    todo: "hairball-kanban-dot-todo",
    ready: "hairball-kanban-dot-ready",
    running: "hairball-kanban-dot-running",
    blocked: "hairball-kanban-dot-blocked",
    done: "hairball-kanban-dot-done",
    archived: "hairball-kanban-dot-archived",
  };
  ...
  const MIME_TASK = "text/x-hairball-task";
  ...
  const DOCS_URL = "https://hairball.local/docs/user-guide/features/kanban";
  const DOCS_TUTORIAL_URL = "https://hairball.local/docs/user-guide/features/kanban-tutorial";
  ...
  const LS_BOARD_KEY = "hairball.kanban.selectedBoard";
  ...
  function renderMarkdown(src) {
    ...
    html = html.replace(/\u0000CODE(\d+)\u0000/g, (_m, i) =>
      `<pre class="hairball-kanban-md-code"><code>${escapeHtml(blocks[Number(i)])}</code></pre>`,
    );
    return html;
  }
  ...
  function sanitizeMarkdownAttrs(tag, attrs) {
    ...
    if (tag === "pre" && /\sclass=(["'])hairball-kanban-md-code\1/i.test(attrs)) {
      return ' class="hairball-kanban-md-code"';
    }
    return "";
  }
  ...
  function MarkdownBlock(props) {
    ...
    if (!enabled) {
      return h("pre", { className: "hairball-kanban-pre" }, props.source || "");
    }
    return h("div", {
      className: "hairball-kanban-md",
  ...
  // standard `drop` event and our `hairball-kanban:drop` event.
  // -------------------------------------------------------------------------
  ...
  function attachTouchDrag(el, taskId) {
    ...
    function onDown(e) {
      ...
      proxy.classList.add("hairball-kanban-touch-proxy");
      ...
      function move(ev) {
        ...
        if (target !== lastTarget) {
          if (lastTarget) lastTarget.classList.remove("hairball-kanban-column--drop");
          if (target) target.classList.add("hairball-kanban-column--drop");
          lastTarget = target;
        }
      }
      function up() {
        ...
        if (lastTarget) {
          lastTarget.classList.remove("hairball-kanban-column--drop");
          const status = lastTarget.getAttribute("data-kanban-column");
          const isTrash = lastTarget.hasAttribute("data-kanban-trash");
          if (isTrash) {
            lastTarget.dispatchEvent(new CustomEvent("hairball-kanban:delete", {
              detail: { taskId },
              bubbles: true,
            }));
          } else if (status) {
            lastTarget.dispatchEvent(new CustomEvent("hairball-kanban:drop", {
  ...
  function KanbanPage() {
    ...
    useEffect(function () {
      ...
      function openWs() {
        ...
        // loopback. Reading window.__HAIRBALL_SESSION_TOKEN__ directly (the old
   ...
   const deleteTask = useCallback(function (taskId) {
    ...
    const selectedBoardMeta = boardList.find(function (item) {
      return item.slug === board;
    }) || null;

    return h(ErrorBoundary, null,
      h("div", { className: "hairball-kanban flex flex-col gap-4" },
        h(BoardSwitcher, {
          board: board,
          boardList: boardList,
  ...
  function AttentionStrip(props) {
    ...
    return h("div", {
      className: cn(
        "hairball-kanban-attention",
        "hairball-kanban-attention--" + topSev,
      ),
    },
      h("div", { className: "hairball-kanban-attention-bar" },
        h("span", { className: "hairball-kanban-attention-icon" },
          topSev === "critical" ? "!!!" : topSev === "error" ? "!!" : "⚠"),
        h("span", { className: "hairball-kanban-attention-text" },
          diagTasks.length === 1
      ...
      expanded
        ? h("div", { className: "hairball-kanban-attention-list" },
            diagTasks.map(function (task) {
              ...
              return h("div", {
                key: task.id,
                className: cn(
                  "hairball-kanban-attention-row",
                  "hairball-kanban-attention-row--" + sev,
                ),
              },
                h("span", { className: "hairball-kanban-attention-row-sev" },
                  sev === "critical" ? "!!!" : sev === "error" ? "!!" : "⚠"),
                h("span", { className: "hairball-kanban-attention-row-id" }, task.id),
                h("span", { className: "hairball-kanban-attention-row-title" },
                  task.title || tx(t, "untitled", "(untitled)")),
                h("span", { className: "hairball-kanban-attention-row-meta" },
                  task.assignee ? "@" + task.assignee : tx(t, "unassigned", "unassigned"),
                  " \u00b7 ",
                  kinds.length > 0 ? kinds.join(", ") : tx(t, "diagnostic", "diagnostic"),
                ),
                h("button", {
                  className: "hairball-kanban-attention-row-btn",
  ...
  function DiagnosticActionButton(props) {
    ...
    const cls = cn(
      "hairball-kanban-diag-action-btn",
      action.suggested ? "hairball-kanban-diag-action-btn--suggested" : "",
    );
    ...
    return h("span", { className: cls + " hairball-kanban-diag-action-btn--unknown" },
      label);
  }
  ...
  function DiagnosticCard(props) {
    ...
    const execAction = function (action) {
      ...
      if (action.kind === "comment") {
        ...
        const ta = document.querySelector(".hairball-kanban-drawer-comment-row input, .hairball-kanban-drawer-comment-row textarea");
        if (ta) {
    ...
    const sevClass = "hairball-kanban-diag--" + (diag.severity || "warning");
    return h("div", { className: cn("hairball-kanban-diag", sevClass) },
      h("div", { className: "hairball-kanban-diag-header" },
        h("span", { className: "hairball-kanban-diag-sev" },
          diag.severity === "critical" ? "!!!" :
          diag.severity === "error" ? "!!" : "\u26a0"),
        h("span", { className: "hairball-kanban-diag-title" },
          diag.title),
      ),
      h("div", { className: "hairball-kanban-diag-detail" },
        diag.detail),
      diag.data && Object.keys(diag.data).length > 0
        ? h("div", { className: "hairball-kanban-diag-data" },
            Object.keys(diag.data).map(function (k) {
              const v = diag.data[k];
              if (Array.isArray(v) && v.length > 0 && typeof v[0] === "string" &&
                  v[0].indexOf("t_") === 0) {
                // Task-id list — render as chips.
                return h("div", { key: k, className: "hairball-kanban-diag-data-row" },
                  h("span", { className: "hairball-kanban-diag-data-key" }, k + ":"),
                  v.map(function (x) {
                    return h("code", {
                      key: x, className: "hairball-kanban-event-phantom-chip",
                    }, x);
                  }),
                );
              }
              return h("div", { key: k, className: "hairball-kanban-diag-data-row" },
                h("span", { className: "hairball-kanban-diag-data-key" }, k + ":"),
                h("span", { className: "hairball-kanban-diag-data-val" },
      ...
      reassignAction
        ? h("div", { className: "hairball-kanban-diag-reassign-row" },
            h("span", { className: "hairball-kanban-diag-reassign-label" },
              tx(t, "reassignTo", "Reassign to:")),
            h("select", {
              className: "hairball-kanban-recovery-select",
        ...
        : null,
      h("div", { className: "hairball-kanban-diag-actions" },
      ...
      msg
        ? h("div", {
            className: cn(
              "hairball-kanban-diag-msg",
              msg.ok ? "hairball-kanban-diag-msg--ok" : "hairball-kanban-diag-msg--err",
            ),
          }, msg.text)
        : null,
    );
  ...
  function DiagnosticsSection(props) {
    ...
    return h("div", { className: "hairball-kanban-section" },
      h("div", { className: "hairball-kanban-section-head-row" },
        h("span", { className: "hairball-kanban-section-head" },
          hasOpenDiags
            ? h("span", { className: "hairball-kanban-section-head-warning" },
                `\u26a0 ${tx(t, "diagnostics", "Diagnostics")} (${diags.length})`)
            : tx(t, "diagnostics", "Diagnostics"),
        ),
        h("button", {
          className: "hairball-kanban-section-toggle",
          onClick: function () { setOpen(function (x) { return !x; }); },
          type: "button",
        }, open ? tx(t, "hide", "Hide") : tx(t, "show", "Show")),
      ),
      open
        ? h("div", { className: "hairball-kanban-diag-list" },
  ...
  function DocsLink() {
    return h("a", {
      ...
      className: "hairball-kanban-docs-link",
      title: "Open Hairball Kanban docs in a new tab",
      "aria-label": "Hairball Kanban documentation",
    }, "?");
  }
  ...
  function OrchestrationPanel() {
    ...
    // each triage card (or runs `hairball kanban decompose <id>`) and tasks
    ...
    return h(Card, { className: "p-3" },
      h(CardContent, { className: "p-2 flex flex-col gap-3" },
        ...
        msg ? h("div", {
          className: msg.ok ? "hairball-kanban-msg-ok" : "hairball-kanban-msg-err",
        }, msg.text) : null,
  ...
  function BoardSwitcher(props) {
    ...
    if (!shouldShow) {
      return h("div", {
        className: "hairball-kanban-boardswitcher-compact",
    ...
    return h("div", { className: "hairball-kanban-boardswitcher" },
      h("div", { className: "hairball-kanban-boardswitcher-inner" },
  ...
  function NewBoardDialog(props) {
    ...
    return h("div", {
      className: "hairball-kanban-dialog-backdrop",
      onClick: function (e) { if (e.target === e.currentTarget) props.onCancel(); },
    },
      h("form", {
        className: "hairball-kanban-dialog",
        onSubmit: onSubmit,
      },
        h("div", { className: "hairball-kanban-dialog-title" },
        ...
        h("div", { className: "flex flex-col gap-3" },
          ...
          h("div", { className: "flex flex-col gap-1" },
            h(Label, { className: "text-xs" }, "Project directory", " ",
              h("span", { className: "text-muted-foreground" },
                tx(t, "projectDirectoryHint", "(optional)"))),
            h(Input, {
            ...
            h("div", { className: "text-[11px] text-muted-foreground" },
              "Absolute path to the project folder. Tasks default to a preserved workspace instead of temporary scratch files."),
          ),
        ...
        h("div", { className: "hairball-kanban-dialog-actions" },
          h(Button, {
            type: "button",
  ...
  function BoardToolbar(props) {
    ...
    return h("div", { className: "flex flex-wrap items-end gap-3" },
      ...
      h("div", { className: "flex flex-col gap-1",
                 title: "Filter by assigned Hairball profile. Profiles are the named agent identities that claim and work on tasks." },
  ...
  function BulkActionBar(props) {
    ...
    return h("div", { className: "hairball-kanban-bulk" },
      h("span", { className: "hairball-kanban-bulk-count" },
      ...
      h("div", { className: "hairball-kanban-bulk-priority",
      ...
      h("div", { className: "hairball-kanban-bulk-reassign",
                 title: "Reassign selected tasks to a different Hairball profile. Pick a profile (or unassign) and click Apply." },
        h(Select, Object.assign({
          value: assignee,
          className: "h-7 text-xs",
      ...
      h("label", { className: "hairball-kanban-bulk-reclaim-first", title: "Reclaim any active claims before reassigning" },
        h(Checkbox, {
  ...
  function TrashDropZone(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("hairball-kanban:delete", onTouchDelete);
      return function () { el.removeEventListener("hairball-kanban:delete", onTouchDelete); };
    }, [props.onDelete]);
    ...
    return h("div", {
      ...
      className: cn(
        "hairball-kanban-trash",
        dragOver ? "hairball-kanban-trash--drop" : "",
        props.draggingTaskId ? "hairball-kanban-trash--active" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("span", { className: "hairball-kanban-trash-icon" }, "🗑️"),
      h("span", { className: "hairball-kanban-trash-label" },
  ...
  function BoardColumns(props) {
    ...
    const isPanBlockedTarget = useCallback(function (target) {
      ...
      if (target.closest && target.closest(".hairball-kanban-card")) return true;
      if (target.closest && target.closest(".hairball-kanban-column-add")) return true;
      if (target.closest && target.closest(".hairball-kanban-col-check")) return true;
    ...
    const stopPan = useCallback(function () {
      ...
      if (el) {
        ...
        el.classList.remove("hairball-kanban-columns--panning");
        el.style.userSelect = "";
      }
    ...
    const handleMouseDown = useCallback(function (e) {
      ...
      setIsPanning(true);
      el.classList.add("hairball-kanban-columns--panning");
    ...
    const handleDragStart = useCallback(function (e) {
      const card = e.target.closest && e.target.closest(".hairball-kanban-card");
      if (!card) return;
    ...
    return h("div", {
      ref: columnsRef,
      className: cn(
        "hairball-kanban-columns",
        isScrollable ? "hairball-kanban-columns--scrollable" : "",
        isPanning ? "hairball-kanban-columns--panning" : "",
      ),
      onDragStart: handleDragStart,
      onDragEnd: handleDragEnd,
  ...
  function Column(props) {
    ...
    useEffect(function () {
      ...
      el.addEventListener("hairball-kanban:drop", onTouchDrop);
      return function () { el.removeEventListener("hairball-kanban:drop", onTouchDrop); };
    ...
    return h("div", {
      ...
      className: cn(
        "hairball-kanban-column",
        dragOver ? "hairball-kanban-column--drop" : "",
      ),
      onDragOver: handleDragOver,
      onDragLeave: handleDragLeave,
      onDrop: handleDrop,
    },
      h("div", { className: "hairball-kanban-column-header",
                 title: colHelp || "" },
        h(Checkbox, {
          className: "hairball-kanban-col-check",
        ...
        h("span", { className: cn("hairball-kanban-dot", COLUMN_DOT[props.column.name]) }),
        h("span", { className: "hairball-kanban-column-label" },
          colLabel || props.column.name),
        h("span", { className: "hairball-kanban-column-count",
                    title: `${props.column.tasks.length} task${props.column.tasks.length === 1 ? "" : "s"} in this column` },
          props.column.tasks.length),
        h("button", {
          type: "button",
          className: "hairball-kanban-column-add",
          title: tx(t, "createTask", "Create task in this column"),
          onClick: function () { setShowCreate(function (v) { return !v; }); },
        }, showCreate ? "×" : "+"),
      ),
      h("div", { className: "hairball-kanban-column-sub" },
        colHelp || ""),
      ...
      h("div", { className: "hairball-kanban-column-body" },
        props.column.tasks.length === 0
          ? h("div", { className: "hairball-kanban-empty" }, tx(t, "noTasks", "— no tasks —"))
          : lanes
            ? lanes.map(function (lane) {
                return h("div", { key: lane.assignee, className: "hairball-kanban-lane" },
                  h("div", { className: "hairball-kanban-lane-head" },
                    h("span", { className: "hairball-kanban-lane-name" }, lane.assignee),
                    h("span", { className: "hairball-kanban-lane-count" }, lane.tasks.length),
                  ),
  ...
  function stalenessClass(task) {
    ...
    if (age >= tier.red)   return "hairball-kanban-card--stale-red";
    if (age >= tier.amber) return "hairball-kanban-card--stale-amber";
    return "";
  }
  ...
  function TaskCard(props) {
    ...
    const handleDragStart = function (e) {
      ...
      const selectedCards = document.querySelectorAll(".hairball-kanban-card--selected");
      if (selectedCards.length > 1 && props.selected) {
        const ghost = document.createElement("div");
        ghost.className = "hairball-kanban-drag-ghost";
    ...
    return h("div", {
      ...
      "data-task-id": t.id,
      className: cn(
        "hairball-kanban-card",
        props.selected ? "hairball-kanban-card--selected" : "",
        props.failed ? "hairball-kanban-card--failed" : "",
        props.draggingSource ? "hairball-kanban-card--dragging-source" : "",
        stalenessClass(t),
      ),
      draggable: true,
      tabIndex: 0,
      ...
      h(Card, null,
        h(CardContent, { className: "hairball-kanban-card-content" },
          h("div", { className: "hairball-kanban-card-row" },
            h("label", {
              className: "hairball-kanban-card-check-wrap",
              title: tx(i18n, "selectForBulk", "Select for bulk actions"),
              onClick: function (e) { e.stopPropagation(); },
            },
              h(Checkbox, {
                className: "hairball-kanban-card-check",
            ...
            h("span", { className: "hairball-kanban-card-id",
                        title: `Task id: ${t.id}. Use this id with kanban_show, /kanban show, or hairball kanban show.` }, t.id),
            t.warnings && t.warnings.count > 0
              ? h("span", {
                  className: cn(
                    "hairball-kanban-warning-badge",
                    "hairball-kanban-warning-badge--" + (t.warnings.highest_severity || "warning"),
                  ),
                  title: (
            ...
            needsAssignee
              ? h(Badge, {
                  variant: "outline",
                  className: "hairball-kanban-needs-assignee",
          ...
          h("div", { className: "hairball-kanban-card-title" },
            t.title || tx(i18n, "untitled", "(untitled)")),
          h("div", { className: "hairball-kanban-card-row hairball-kanban-card-meta" },
            t.assignee
              ? h("span", { className: "hairball-kanban-assignee",
                            title: `Assigned to Hairball profile @${t.assignee}` }, "@", t.assignee)
              : h("span", { className: "hairball-kanban-unassigned",
                            title: needsAssignee
            ...
            t.comment_count > 0
              ? h("span", { className: "hairball-kanban-count",
            ...
            t.link_counts && (t.link_counts.parents + t.link_counts.children) > 0
              ...
              : null,
            h("span", { className: "hairball-kanban-ago",
  ...
  function InlineCreate(props) {
    ...
    const submit = function () {
      ...
      // A scratch workspace is deleted on completion. Never forward a stale
      // project path if the user switches from dir/worktree back to scratch.
      if (workspaceKind !== "scratch" && wpTrim) body.workspace_path = wpTrim;
    ...
    return h("div", { className: "hairball-kanban-inline-create" },
      h("textarea", {
        value: title,
      ...
      h("div", { className: "flex gap-2" },
        h(Input, {
          ...
          title: props.columnName === "triage"
            ? "Hairball profile that will spec this task (default: the dispatcher's configured specifier). Leave blank to let the dispatcher pick."
            : "Hairball profile to assign. Leave blank and the dispatcher will pick from available profiles when the task is Ready.",
      ...
      h("div", { className: "flex gap-2" },
        h(Select, Object.assign({
          ...
          h(SelectOption, { value: "scratch" }, "Temporary — deleted on completion"),
          h(SelectOption, { value: "worktree" }, "Git worktree — preserved"),
          h(SelectOption, { value: "dir" }, "Directory — preserved"),
        ),
      ...
      h("div", { className: "text-[11px] text-muted-foreground" },
        workspaceKind === "scratch"
          ...
          : workspaceKind === "worktree"
            ? "The project stays intact; Hairball creates and preserves a separate Git worktree."
            : "The selected directory and files are preserved after completion."),
      h(Select, Object.assign({
        value: parent,
        className: "h-7 text-xs",
  ...
  function TaskDrawer(props) {
    ...
    return h("div", { className: "hairball-kanban-drawer-shade", onClick: props.onClose },
      h("div", {
        className: "hairball-kanban-drawer",
        onClick: function (e) { e.stopPropagation(); },
      },
        h("div", { className: "hairball-kanban-drawer-head" },
          h("span", { className: "text-xs text-muted-foreground" }, props.taskId),
          h("button", {
            type: "button",
            onClick: props.onClose,
            className: "hairball-kanban-drawer-close",
        ...
        data ? h("div", { className: "hairball-kanban-drawer-comment-row" },
          h(Input, {
            value: newComment,
  ...
  function AttachmentsSection(props) {
    ...
    return h("div", { className: "hairball-kanban-section" },
      h("div", { className: "hairball-kanban-section-head" },
      ...
      atts.length === 0
        ...
        : atts.map(function (a) {
            return h("div", {
              ...
              h("button", {
                type: "button",
                className: "hairball-kanban-attachment-link truncate",
                title: a.filename,
  ...
  function TaskDetail(props) {
    ...
    const isParentTask = links.children.length > 0;
    const childResults = props.data.child_results || [];
    var finalResult = t.result || t.latest_summary || null;

    return h("div", { className: "hairball-kanban-drawer-body" },
      h("div", { className: "hairball-kanban-drawer-title" },
        h("span", { className: cn("hairball-kanban-dot", COLUMN_DOT[t.status]) }),
        props.editing
          ? h(TitleEditor, {
              initial: t.title || "",
          ...
          : h("span", {
              className: "hairball-kanban-drawer-title-text",
              title: tx(i18n, "clickToEdit", "Click to edit"),
              onClick: function () { props.setEditing(true); },
            }, t.title || tx(i18n, "untitled", "(untitled)")),
      ),
      h("div", { className: "hairball-kanban-drawer-meta" },
      ...
      isParentTask ? h("div", { className: "hairball-kanban-section" },
        h("div", { className: "hairball-kanban-section-head" }, "Parent task"),
        h("div", { className: "text-xs text-muted-foreground" },
          "This parent task orchestrates child work. Review each child result before accepting the combined outcome."),
      ) : null,
      isParentTask ? h("div", { className: "hairball-kanban-section" },
        h("div", { className: "hairball-kanban-section-head" }, "Child Results"),
        childResults.length === 0
          ? h("div", { className: "text-xs text-muted-foreground italic" },
              "No child result has been recorded yet.")
          : childResults.map(function (child) {
              const childResult = child.result || child.latest_summary || null;
              return h("div", { key: child.id, className: "hairball-kanban-comment" },
                h("div", { className: "hairball-kanban-comment-head" },
                  h("span", { className: "hairball-kanban-comment-author" },
                    `${child.id} — ${child.title || "Untitled child"}`),
                  h("span", { className: "hairball-kanban-comment-ago" }, child.status || "unknown"),
                ),
                childResult
                  ? h(MarkdownBlock, { source: childResult, enabled: props.renderMarkdown })
                  : h("div", { className: "text-xs text-muted-foreground italic" },
                      "No final result was recorded for this child."),
              );
            }),
      ) : null,
      (t.status === "done" || finalResult) ? h("div", { className: "hairball-kanban-section" },
        h("div", { className: "hairball-kanban-section-head" },
          t.result ? "Final Result" : "Final Result (run summary)"),
        finalResult
      ...
      h("div", { className: "hairball-kanban-section" },
        h("div", { className: "hairball-kanban-section-head" },
        ...
        comments.length === 0
          ...
          : comments.map(function (c) {
              return h("div", { key: c.id, className: "hairball-kanban-comment" },
                h("div", { className: "hairball-kanban-comment-head" },
                  h("span", { className: "hairball-kanban-comment-author" }, c.author || "anon"),
                  h("span", { className: "hairball-kanban-comment-ago" },
      ...
      h("div", { className: "hairball-kanban-section" },
        ...
        events.slice().reverse().slice(0, 20).map(function (e) {
          ...
          return h("div", {
            key: e.id,
            className: cn(
              "hairball-kanban-event",
              isDiag ? "hairball-kanban-event--hallucination" : "",
            ),
          },
            isDiag
              ? h("div", { className: "hairball-kanban-event-header" },
                  h("span", { className: "hairball-kanban-event-warning-icon" }, "⚠"),
                  h("span", { className: "hairball-kanban-event-warning-label" },
                    getDiagnosticEventLabel(i18n, e.kind) || e.kind),
                  h("span", { className: "hairball-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                )
              : h("div", { className: "hairball-kanban-event-header-plain" },
                  h("span", { className: "hairball-kanban-event-kind" }, e.kind),
                  h("span", { className: "hairball-kanban-event-ago" },
                    timeAgo ? timeAgo(e.created_at) : ""),
                ),
            isDiag && phantoms.length > 0
              ? h("div", { className: "hairball-kanban-event-phantom-row" },
                  h("span", { className: "hairball-kanban-event-phantom-label" },
                    tx(i18n, "phantomIds", "Phantom ids:")),
                  phantoms.map(function (pid) {
                    return h("code", {
                      key: pid,
                      className: "hairball-kanban-event-phantom-chip",
                    }, pid);
                  }),
                )
              : null,
            e.payload && !isDiag
              ? h("code", { className: "hairball-kanban-event-payload" },
    ...
    return h("div", { className: "hairball-kanban-section" },
      h("div", { className: "hairball-kanban-section-head-row" },
        ...
        !showAll
          ? h("button", {
              type: "button",
              onClick: function () { setExpanded(true); },
              className: "hairball-kanban-edit-link",
      ...
      visible.map(function (r) {
        const outcomeClass = r.ended_at
          ? `hairball-kanban-run--${r.outcome || r.status || "ended"}`
          : "hairball-kanban-run--active";
        return h("div", { key: r.id, className: cn("hairball-kanban-run", outcomeClass) },
          h("div", { className: "hairball-kanban-run-head" },
            h("span", { className: "hairball-kanban-run-outcome" },
              r.ended_at ? (r.outcome || r.status || tx(t, "ended", "ended")) : tx(t, "active", "active")),
            h("span", { className: "hairball-kanban-run-profile" },
              r.profile ? `@${r.profile}` : tx(t, "noProfile", "(no profile)")),
            h("span", { className: "hairball-kanban-run-elapsed" }, fmtElapsed(r)),
            h("span", { className: "hairball-kanban-run-ago" },
              timeAgo ? timeAgo(r.started_at) : ""),
          ),
          r.summary
            ? h("div", { className: "hairball-kanban-run-summary" }, r.summary)
            : null,
          r.error
            ? h("div", { className: "hairball-kanban-run-error" }, r.error)
            : null,
          (r.metadata && Object.keys(r.metadata).length > 0)
            ? (function () {
                ...
                return h("details", {
                    className: "hairball-kanban-run-meta-block",
                    open: !collapsed,
                  },
                  h("summary", { className: "hairball-kanban-run-meta-label" }, "Metadata"),
                  h("code", { className: "hairball-kanban-run-meta" }, json),
                );
              })()
            : null,
        );
  ...
  function WorkerLogSection(props) {
    ...
    if (state.loading) {
      ...
      body = h("pre", { className: "hairball-kanban-pre hairball-kanban-log" },
        data.content || "(empty)");
    }
  ...
  function MetaRow(props) {
    return h("div", { className: "hairball-kanban-meta-row" },
      h("span", { className: "hairball-kanban-meta-label" }, props.label),
      h("span", { className: "hairball-kanban-meta-value" }, props.value),
    );
  }
  ...
  function TitleEditor(props) {
    ...
    return h("div", { className: "hairball-kanban-edit-row" },
      h(Input, {
  ...
  function AssigneeEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "hairball-kanban-meta-row" },
        h("span", { className: "hairball-kanban-meta-label" }, tx(t, "assignee", "Assignee")),
        h("span", {
          className: "hairball-kanban-meta-value hairball-kanban-editable",
  ...
  function PriorityEditor(props) {
    ...
    if (!editing) {
      return h("div", { className: "hairball-kanban-meta-row" },
        h("span", { className: "hairball-kanban-meta-label" }, tx(t, "priority", "Priority")),
        h("span", {
          className: "hairball-kanban-meta-value hairball-kanban-editable",
          onClick: function () { setEditing(true); },
  ...
  function BodyEditor(props) {
    ...
    return h("div", { className: "hairball-kanban-section" },
      h("div", { className: "hairball-kanban-section-head-row" },
        h("span", { className: "hairball-kanban-section-head" }, tx(t, "description", "Description")),
        editing
      ...
      editing
        ? h("textarea", {
            className: "hairball-kanban-textarea",
            value: v,
            rows: 8,
            onChange: function (e) { setV(e.target.value); },
          })
  ...
  function DependencyEditor(props) {
    ...
    return h("div", { className: "hairball-kanban-section" },
      h("div", { className: "hairball-kanban-section-head" }, tx(t, "dependencies", "Dependencies")),
      h("div", { className: "hairball-kanban-deps-row" },
        h("span", { className: "hairball-kanban-deps-label" }, tx(t, "parents", "Parents:")),
        h("div", { className: "hairball-kanban-deps-chips" },
          (links.parents || []).length === 0
            ? h("span", { className: "hairball-kanban-deps-empty" }, tx(t, "none", "none"))
            : (links.parents || []).map(function (id) {
                return h("span", { key: id, className: "hairball-kanban-dep-chip" },
                  id,
                  h("button", {
                    type: "button",
                    className: "hairball-kanban-dep-chip-x",
      ...
      h("div", { className: "hairball-kanban-deps-row" },
        h("span", { className: "hairball-kanban-deps-label" }, tx(t, "children", "Children:")),
        h("div", { className: "hairball-kanban-deps-chips" },
  ...
  function StatusActions(props) {
    ...
    return h("div", null,
      h("div", { className: "hairball-kanban-actions" },
        specifyButton,
        decomposeButton,
      ...
      specifyMsg ? h("div", {
        className: specifyMsg.ok
          ? "hairball-kanban-msg-ok"
          : "hairball-kanban-msg-err",
  ...
  function HomeSubsSection(props) {
    ...
    return h("div", { className: "hairball-kanban-section" },
      ...
      h("div", { className: "hairball-kanban-home-subs" },
        channels.map(function (hc) {
          ...
          return h(Button, {
            ...
            className: hc.subscribed
              ? "hairball-kanban-home-sub hairball-kanban-home-sub--on"
              : "hairball-kanban-home-sub",
          }, label);
        })
      )
    );
  ...
  if (window.__HAIRBALL_PLUGINS__ && typeof window.__HAIRBALL_PLUGINS__.register === "function") {
    window.__HAIRBALL_PLUGINS__.register("kanban", KanbanPage);
  }
})();
